...
//!
//! Mirrors `zeus_constants.get_zeus_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\zeus
//!   macOS:   ~/.zeus
//!   Linux:   ~/.zeus  (override via $ZEUS_HOME)
//!
//! NOTE (macOS): Python's get_zeus_home(), scripts/install.sh, and the
//! Electron desktop's resolveZeusAgentHome() ALL use ~/.zeus on macOS — there
...
/// Returns the canonical ZeusAgent home directory, respecting $ZEUS_HOME if set.
pub fn zeus_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("ZEUS_HOME") {
    ...
    {
        // %LOCALAPPDATA%\zeus — matches scripts/install.ps1's $ZeusAgentHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("zeus");
        }
    }

    // macOS + Linux + fallback: ~/.zeus (matches Python get_zeus_home(),
    // install.sh, and the Electron desktop's resolveZeusAgentHome()).
...
pub fn bootstrap_cache_dir() -> PathBuf {
    zeus_home().join("bootstrap-cache")
}
...
/// ZEUS_HOME so it survives repo checkout deletion (unlike anything under
/// zeus-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\zeus\zeus-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_zeus`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under ZEUS_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves ZEUS_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    zeus_home().join(".zeus-update-in-progress")
}
...
pub fn copy_self_to_zeus_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to ZEUS_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_ZEUS_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".zeus-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under ZEUS_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[zeus-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("ZEUS_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_zeus_home() -> String {
    zeus_home().to_string_lossy().into_owned()
}
