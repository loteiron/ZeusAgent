...
/// read side does not see EOF until the last of them closes it. `zeus update`
...
/// stranded with it.
///
/// Same bound `Invoke-ZeusAgentStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `zeus_home_override` propagates to the child as $ZEUS_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, zeus_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = zeus_home_override {
        cmd.env("ZEUS_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, zeus_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = zeus_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_zeus_home() {
    ...
    /// child. `zeus update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]
