...
export const $zeusHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.zeus/logs/bootstrap-installer.log')
    $zeusHome.set('~/.zeus')
  ...
  try {
    const [logPath, zeusHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_zeus_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $zeusHome.set(zeusHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch ZeusAgent" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (ZeusAgent-Setup.exe --update);
...
export async function launchZeusAgentDesktop(): Promise<void> {
  ...
  await invoke('launch_zeus_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'ZeusAgent repository' },
