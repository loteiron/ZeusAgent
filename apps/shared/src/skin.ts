/**
 * Canonical ZeusAgent skin — the theme SDK's cross-surface contract.
 *
 * A skin is authored once as YAML in `$ZEUS_HOME/skins/<name>.yaml` (or a
 * built-in), resolved by the Python skin engine (`zeus_cli/skin_engine.py`),
 ...
 *   • CLI     → `zeus_cli/skin_engine` → prompt_toolkit / Rich styles (Python)
 *
...
export interface ZeusAgentSkin {
  name?: string
  description?: string
  colors?: SkinColors
