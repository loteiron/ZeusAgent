...
import { saveZeusAgentConfigRecord } from '@/zeus'
...
export function RealProfileConsentDialog({ tabId }: RealProfileConsentDialogProps) {
  ...
  const { data: config } = useZeusAgentConfigRecord()
  const setConfig = zeusConfigCacheWriter()
  ...
  const enable = useCallback(async () => {
    ...
    try {
      await saveZeusAgentConfigRecord(next)
