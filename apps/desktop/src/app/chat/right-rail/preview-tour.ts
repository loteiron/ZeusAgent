...
function buildTourScript(action: TourAction): string {
  return `(function () {
  var w = window;
  if (!w.__zeusTourEngine) {
    ${driverIife}
    w.__zeusTourHolder = {};
    w.__zeusTourCollect = (${collectTourTargets.toString()});
    w.__zeusTourEngine = (${runTourEngine.toString()});
  }
  if (!document.getElementById('__zeus-tour-style')) {
    var style = document.createElement('style');
    style.id = '__zeus-tour-style';
    style.textContent = ${JSON.stringify(driverCss)};
    (document.head || document.documentElement).appendChild(style);
  }
  return JSON.stringify(w.__zeusTourEngine(
    w.driver.js.driver,
    w.__zeusTourHolder,
