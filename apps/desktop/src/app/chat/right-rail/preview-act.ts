...
const preamble = () => `  var w = window;
  ...
  w.__zeusActHolder = w.__zeusActHolder || {};
  w.__zeusAct = ${actEngineSource()};
  w.__zeusWatch_fn = (${watchInPage.toString()});
  w.__zeusWatchTag = ${WATCH_TAG};
  var holder = w.__zeusActHolder;
  var act = function (a) { return w.__zeusAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__zeusWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  document.addEventListener('pointerdown', function (e) {
    w.__zeusHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'ZEUS-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
