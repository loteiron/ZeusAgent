...
import type { SessionInfo } from '@/zeus'
...
// zero ZeusAgent sessions, and those arrive continuously — so once the user dragged
...
export function useRepoWorktreeMap(
  repoPaths: string[],
  enabled: boolean
): [Record<string, ZeusAgentGitWorktree[]>, boolean] {
  const [map, setMap] = useState<Record<string, ZeusAgentGitWorktree[]>>({})
