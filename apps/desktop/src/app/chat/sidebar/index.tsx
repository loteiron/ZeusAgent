...
import { searchSessions, type SessionInfo, type SessionSearchResult } from '@/zeus'
...
// highlight markers (sqlite snippet() delimiters — see zeus_state_search.py).
...
export function ChatSidebar({
  ...
  useEffect(() => {
    ...
    window.addEventListener('zeus:new-session-shortcut', onShortcut)

    return () => {
      window.removeEventListener('zeus:new-session-shortcut', onShortcut)
      clearTimeout(timeout)
    }
  }, [])
  ...
  // A turn can `git worktree add/remove` in the terminal (e.g. you ask ZeusAgent to
  ...
  return (
    <Sidebar
      className={cn(
        ...
        // paints itself fully.
        'z-sidebar relative h-full min-w-0 overflow-hidden border-t-0 border-b-0 text-foreground transition-none',
    ...
    >
      <SidebarContent className="gap-0 overflow-hidden bg-transparent px-2.5">
        <SidebarGroup className="shrink-0 p-0 pb-2 pt-[calc(var(--titlebar-height)+0.375rem)]">
          <SidebarGroupContent>
            <div className="z-sidebar-brand mb-4 flex items-center gap-2.5 px-2 py-2">
              <BrandMark className="size-9" />
              <div className="min-w-0">
                <p className="text-sm font-semibold tracking-tight text-foreground">ZeusAgent</p>
                <p className="text-[0.625rem] font-medium tracking-[0.14em] text-(--ui-text-tertiary)">
                ...
                const button = (
                  <SidebarMenuButton
                    ...
                    className={cn(
                      ...
                      // thread.tsx.
                      'z-sidebar-nav flex h-9 w-full justify-start gap-2 rounded-md border border-transparent px-2 text-left text-[0.8125rem] font-medium text-(--ui-text-secondary) transition-colors duration-100 ease-out [-webkit-app-region:no-drag] hover:bg-(--ui-control-hover-background) hover:text-foreground hover:transition-none',
                      active &&
                    ...
                    data-tip-region=""
                    data-zeus-primary={isNewSession || undefined}
                    onClick={() => {
