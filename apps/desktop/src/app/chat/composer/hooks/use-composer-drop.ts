...
import { extractDroppedFiles, ZEUS_PATHS_MIME, partitionDroppedFiles } from '../../hooks/use-composer-actions'
...
export function useComposerDrop({
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLFormElement>) => {
    if (!onAttachDroppedItems || !dragHasAttachments(event.dataTransfer, ZEUS_PATHS_MIME)) {
      return
    }
  ...
  const handleInputDragOver = (event: ReactDragEvent<HTMLDivElement>) => {
    if (!dragHasAttachments(event.dataTransfer, ZEUS_PATHS_MIME)) {
      return
    }
