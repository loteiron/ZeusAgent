...
import type { ZeusAgentReadDirEntry, ZeusAgentReadDirResult } from '@/global'
...
export type ProjectTreeEntry = ZeusAgentReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: ZeusAgentReadDirEntry) {
...
async function filterIgnored(entries: ZeusAgentReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<ZeusAgentReadDirResult> {
  if (!window.zeusDesktop) {
