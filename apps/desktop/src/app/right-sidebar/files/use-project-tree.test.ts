...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { zeusDesktop: { readDir: typeof readDir } }).zeusDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): ZeusAgentReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { zeusDesktop: unknown }).zeusDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: ZeusAgentReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<ZeusAgentReadDirResult>(resolve => {
  ...
  it('discards a stale live refresh after the active registered connection changes', async () => {
    let resolveRefreshFromA: ((result: ZeusAgentReadDirResult) => void) | undefined
  ...
  it('discards a stale child read after the active registered connection changes', async () => {
    let resolveChildFromA: ((result: ZeusAgentReadDirResult) => void) | undefined
  ...
  it('discards a stale root read after the active registered connection changes', async () => {
    let resolveFirst: ((result: ZeusAgentReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<ZeusAgentReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.zeusDesktop is missing', async () => {
    delete (window as unknown as { zeusDesktop?: unknown }).zeusDesktop
  ...
  it('keeps an unreadable root on screen while it re-probes', async () => {
    ...
    let releaseProbe: ((value: ZeusAgentReadDirResult) => void) | undefined
  ...
  it('keeps loaded rows on screen while the root refreshes', async () => {
    ...
    let releaseRefresh: ((value: ZeusAgentReadDirResult) => void) | undefined
  ...
  it('clears the rows when the same path is re-read from another backend', async () => {
    ...
    let releaseFromB: ((value: ZeusAgentReadDirResult) => void) | undefined
