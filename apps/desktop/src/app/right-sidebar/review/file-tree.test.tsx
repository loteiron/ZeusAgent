...
const file = (path: string): ZeusAgentReviewFile => ({
  added: 1,
  path,
  removed: 0,
  staged: false,
...
function filesUnderPublish(count: number): ZeusAgentReviewFile[] {
  return Array.from({ length: count }, (_, i) => file(`publish/file-${String(i).padStart(4, '0')}.so`))
}

function topLevelFiles(count: number): ZeusAgentReviewFile[] {
