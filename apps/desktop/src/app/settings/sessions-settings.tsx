...
import type { ZeusAgentConfigRecord, SessionInfo } from '@/types/zeus'
...
function AutoArchiveSetting() {
  ...
  const persist = useCallback(
    async (autoArchive: boolean, archiveDays: number) => {
      ...
      try {
        await saveZeusAgentConfig(updated)
      } catch (err) {
...
// / Program Files), which buried any files ZeusAgent wrote there.
function DefaultProjectDirSetting() {
  ...
  useEffect(() => {
    ...
    // / Storybook / non-Electron context `window.zeusDesktop` is
    ...
    const settings = window.zeusDesktop?.settings
