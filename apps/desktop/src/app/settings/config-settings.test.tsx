...
const getZeusAgentConfigSchema = vi.fn()
const saveZeusAgentConfig = vi.fn()
const getElevenLabsVoices = vi.fn()

vi.mock('@/zeus', () => ({
  getZeusAgentConfigRecord: () => getZeusAgentConfigRecord(),
  getZeusAgentConfigSchema: () => getZeusAgentConfigSchema(),
  saveZeusAgentConfig: (config: unknown, profile?: string) => saveZeusAgentConfig(config, profile),
...
beforeEach(() => {
  ...
  getZeusAgentConfigSchema.mockResolvedValue({ fields: {} })
...
describe('ConfigSettings autosave', () => {
  it('sends a later revert instead of diffing it away against the stale page-load baseline', async () => {
    getZeusAgentConfigRecord.mockResolvedValue({ checkpoints: { enabled: false }, other: 'untouched' })
    ...
    try {
      ...
      await waitFor(() => expect(saveZeusAgentConfig).toHaveBeenCalledTimes(1))
      expect(saveZeusAgentConfig.mock.calls[0][0]).toEqual({ checkpoints: { enabled: true } })

      // Revert: flip it back to its original value and let autosave fire again.
      toggle.click()
      await vi.advanceTimersByTimeAsync(700)

      await waitFor(() => expect(saveZeusAgentConfig).toHaveBeenCalledTimes(2))
      ...
      expect(saveZeusAgentConfig.mock.calls[1][0]).toEqual({ checkpoints: { enabled: false } })
    } finally {
      vi.useRealTimers()
    }
  })
