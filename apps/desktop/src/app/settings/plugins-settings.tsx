...
// User-installed plugins first — mirrors `zeus plugins list --user`.
...
function reveal(file: string) {
  void window.zeusDesktop?.revealPath?.(file)?.catch(() => undefined)
}

async function revealPluginsDir() {
  try {
    // Electron owns the local plugin root — deriving it from the backend's
    // zeus_home breaks against a remote backend (#66899).
    const dir = await window.zeusDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.zeusDesktop?.openDir?.(dir)
...
// Agent plugins live under the BACKEND's zeus home (profile-aware), so the
// path comes from the gateway — not from the renderer's local ZEUS_HOME.
...
async function revealAgentPluginsDir(request: GatewayRequest) {
  try {
    ...
    const opened = await window.zeusDesktop?.openDir?.(`${home}/plugins`)
            ...
            <SelectContent>
              {profiles.map(profile => (
                <SelectItem key={profile.name} value={profile.name}>
                  {profile.is_default ? 'ZeusAgent (default)' : profile.name}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
