...
import { disconnectOAuthProvider, listOAuthProviders } from '@/zeus'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/zeus'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.zeusDesktop?.terminal)
...
//      in zeus_cli/provider_catalog.py) — the SAME provider identity
//      `zeus model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // ZeusAgent can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider ZeusAgent can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // ZeusAgent never deletes creds another tool owns behind a silent API call.
