...
import { getElevenLabsVoices, getZeusAgentConfigSchema, saveZeusAgentConfig } from '@/zeus'
...
import type { ConfigFieldSchema, ZeusAgentConfigRecord } from '@/types/zeus'

import { zeusConfigCacheWriter, useZeusAgentConfigRecord } from '../hooks/use-config-record'
...
function ConfigSettingsInner({
  ...
  const [config, setConfig] = useState<ZeusAgentConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useZeusAgentConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => zeusConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['zeus-config-schema'] : ['zeus-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getZeusAgentConfigSchema(scopeProfile),
    staleTime: 5 * 60 * 1000
  })
  ...
  // never touched — possibly changed out-of-band by `zeus config set`
  // while this page sat open — is never resent with its stale value.
  const configBaselineRef = useRef<ZeusAgentConfigRecord | null>(null)
    ...
    const t = window.setTimeout(() => {
      ...
      saveQueueRef.current = saveQueueRef.current.then(async () => {
        try {
          ...
          const result = await saveZeusAgentConfig(patch, scopeProfile)
  ...
  const applyConfig = (next: ZeusAgentConfigRecord) => {
    saveVersionRef.current += 1
    setConfig(next)
    setSaveVersion(saveVersionRef.current)
  }

  const updateConfig = (next: ZeusAgentConfigRecord) => {
