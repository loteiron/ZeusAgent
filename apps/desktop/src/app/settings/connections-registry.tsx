 ...
 * agent sources (local runtime + any number of remote gateways / ZeusAgent Cloud
...
export function ConnectionsRegistrySection() {
  ...
  useEffect(() => {
    if (!editorWantsOauth || !editorUrl || !window.zeusDesktop?.probeConnectionConfig) {
      setAuthProbe(null)
  ...
  const signInOauth = useCallback(async () => {
    ...
    try {
      const result = await window.zeusDesktop.oauthLoginConnectionConfig(editorUrl)
  ...
  // Fan out `zeus update` to every eligible source; per-connection results
