...
import type { ToolsetConfig } from '@/types/zeus'
...
beforeEach(() => {
  ...
  getZeusAgentConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getZeusAgentConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveZeusAgentConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveZeusAgentConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
