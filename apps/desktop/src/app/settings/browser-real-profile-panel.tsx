...
import { type ProfileScope, saveZeusAgentConfigRecord } from '@/zeus'
 ...
 * the default browser's profile (cookies/logins) into a ZeusAgent-owned copy;
...
export function BrowserRealProfilePanel({ profile }: BrowserRealProfilePanelProps) {
  ...
  const { data: config } = useZeusAgentConfigRecord(profile)
  ...
  const toggle = useCallback(
    async (on: boolean) => {
      ...
      try {
        await saveZeusAgentConfigRecord(next, profile)
        notify({
          kind: 'info',
