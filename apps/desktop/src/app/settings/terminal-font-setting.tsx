...
import { setZeusAgentConfigCache, useZeusAgentConfigRecord } from '../hooks/use-config-record'
...
function fontFamilyFromConfig(config: ZeusAgentConfigRecord): string {
...
export function TerminalFontSetting() {
  ...
  const { data: loadedConfig } = useZeusAgentConfigRecord()
  ...
  const [staleConfig, setStaleConfig] = useState<ZeusAgentConfigRecord | null>(null)
  ...
  useEffect(() => {
    ...
    // holds — successful saves write it back via setZeusAgentConfigCache, so
        ...
        <div className="mt-3 space-y-2">
          <div className="flex items-center gap-3">
            <Input
              ...
              className="flex-1"
              disabled={draft === null}
              list="zeus-terminal-font-families"
          ...
          <datalist id="zeus-terminal-font-families">
