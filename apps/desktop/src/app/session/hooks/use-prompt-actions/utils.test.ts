...
describe('renderRpcResult', () => {
  ...
  describe('session.save', () => {
    it('echoes the saved file path', () => {
      expect(renderRpcResult({ file: '/home/user/.zeus/sessions/saved/x.json' }, 'save')).toBe(
        'Saved transcript to /home/user/.zeus/sessions/saved/x.json'
      )
    })
  })
  ...
  describe('session.status', () => {
    it('passes through the multi-line plain-text output verbatim', () => {
      const output = 'ZeusAgent TUI Status\n\nSession ID: s-1\nModel: nous-hermes-3 (unknown)'
  ...
  describe('session.usage', () => {
    it('formats calls / input / output / total with thousands separators', () => {
      expect(renderRpcResult({ calls: 12, input: 1_234_567, output: 89_012, total: 1_323_579 }, 'usage')).toBe(
        `Usage: 12 calls · ${new Intl.NumberFormat().format(1_234_567)} in / ${new Intl.NumberFormat().format(89_012)} out · ${new Intl.NumberFormat().format(1_323_579)} total`
      )
    })
