...
import { getZeusAgentConfig, getZeusAgentConfigDefaults } from '@/zeus'
...
interface ZeusAgentConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useZeusAgentConfig({ activeSessionIdRef }: ZeusAgentConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshZeusAgentConfig = useCallback(
    async (force = false, shouldPublish: () => boolean = () => true) => {
      ...
      try {
        const [config, defaults] = await Promise.all([getZeusAgentConfig(), getZeusAgentConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying ZeusAgent' built-in medium over the user's config.
        if (!canPublish()) {
          return
        }
  ...
  return { refreshZeusAgentConfig, sttEnabled, voiceMaxRecordingSeconds }
}
