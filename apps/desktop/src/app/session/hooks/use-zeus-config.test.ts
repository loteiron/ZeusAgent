...
import { getZeusAgentConfig } from '@/zeus'
...
import { useZeusAgentConfig } from './use-zeus-config'

vi.mock('@/zeus', () => ({
  getZeusAgentConfig: vi.fn(),
  getZeusAgentConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'zeus.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getZeusAgentConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getZeusAgentConfig>>)

describe('useZeusAgentConfig refreshZeusAgentConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to ZeusAgent' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useZeusAgentConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshZeusAgentConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useZeusAgentConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getZeusAgentConfig>>>()
    vi.mocked(getZeusAgentConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useZeusAgentConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshZeusAgentConfig(true)
    })
    expect(getZeusAgentConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getZeusAgentConfig>>)
  ...
  it('does not publish config after its switch loses ownership', async () => {
    const staleConfig = deferred<Awaited<ReturnType<typeof getZeusAgentConfig>>>()
    vi.mocked(getZeusAgentConfig).mockReturnValueOnce(staleConfig.promise)
    const { result } = renderHook(() => useZeusAgentConfig({ activeSessionIdRef: { current: null } }))
    let ownsSwitch = true

    let refresh!: Promise<void>
    act(() => {
      refresh = result.current.refreshZeusAgentConfig(false, () => ownsSwitch)
    })
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getZeusAgentConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getZeusAgentConfig>>>()
    vi.mocked(getZeusAgentConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshZeusAgentConfig(true)
      refreshC = result.current.refreshZeusAgentConfig(true)
    })
