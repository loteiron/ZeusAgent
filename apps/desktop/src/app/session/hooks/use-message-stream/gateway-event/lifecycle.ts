...
export function handleLifecycleEvent(ctx: GatewayEventContext): boolean {
  ...
  if (event.type === 'gateway.ready') {
    ...
    ingestBackendSkin((payload as { skin?: ZeusAgentSkin } | undefined)?.skin, { apply: false })
  ...
  if (event.type === 'skin.changed') {
    // A runtime skin switch (ZeusAgent activating an authored skin, or `/skin`
    // on another surface). Only the active source+profile's change repaints.
    if (fromActiveSource()) {
      ingestBackendSkin(payload as ZeusAgentSkin | undefined, { apply: true })
    }
