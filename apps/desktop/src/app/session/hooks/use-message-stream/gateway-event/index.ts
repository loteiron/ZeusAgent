import { registryBackendScopeKey } from '@zeus/shared'
...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  const { activeSessionIdRef, compactedTurnRef, refreshZeusAgentConfig, sessionStateByRuntimeIdRef } = deps
  ...
  // refreshZeusAgentConfig — two REST calls (config + defaults) per event, per
