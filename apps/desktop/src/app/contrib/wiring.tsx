...
import { getLatestSessionMessages } from '@/zeus'
...
import { useZeusAgentConfigRecord } from '../hooks/use-config-record'
...
import { useZeusAgentConfig } from '../session/hooks/use-zeus-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshZeusAgentConfig, sttEnabled, voiceMaxRecordingSeconds } = useZeusAgentConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('zeus:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('zeus:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshZeusAgentConfig(true)
    void refreshActiveProfile()
    resetProjectTreeState()
  }, [gatewayScope, refreshCurrentModel, refreshZeusAgentConfig])
  ...
  const configRecord = useZeusAgentConfigRecord()
