...
import { pathFromZeusAgentDeepLink, resolveZeusAgentOpenPath } from '@/lib/zeus-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    // "About ZeusAgent" — it is the OS-standard affordance for updating THIS app,
    ...
    const unsubscribe = window.zeusDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow('client'))
  ...
  useEffect(() => {
    window.zeusDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.zeusDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.zeusDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // zeus://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.zeusDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future zeusDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveZeusAgentOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.zeusDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (zeus://index-network/intent/1, zeus://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromZeusAgentDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.zeusDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.zeusDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.zeusDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.zeusDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
