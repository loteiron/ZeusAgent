...
import { getZeusAgentConfigRecord, saveMcpServers } from '@/zeus'
...
import { setZeusAgentConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `zeus://mcp/install` deep links. The payload is
 ...
 * confirming lets ZeusAgent spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getZeusAgentConfigRecord())
      ...
      setZeusAgentConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
