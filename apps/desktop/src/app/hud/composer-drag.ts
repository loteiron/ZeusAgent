...
function setWorkspaceTransfer(transferring: boolean): void {
  window.zeusDesktop?.hud?.setWorkspaceTransfer?.(transferring)
}

function moveHud(state: PressState): void {
  window.zeusDesktop?.hud?.moveBy?.({
    width: state.originW,
...
function armGrab(state: PressState, workspaceTransfer: boolean): void {
  ...
  window.zeusDesktop?.hud?.beginMove?.()
}
 ...
 * (see zeus:hud:move-by — a transparent frameless window drifts wider on
...
export function useHudComposerDrag(
  ...
  const reset = useCallback(() => {
    ...
    if (state) {
      if (state.armed) {
        window.zeusDesktop?.hud?.endMove?.()
      }
