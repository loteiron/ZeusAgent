import { isGatewayReauthRequired, JsonRpcGatewayError, resolveGatewayWsUrl } from '@zeus/shared'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.zeusDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: ZeusAgentGateway | null) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    // composer stayed disabled forever on "Starting ZeusAgent...". Once the
    ...
    const attemptReconnect = async (manual?: { profile: string; activationEpoch: number }) => {
      ...
      try {
        ...
        // "Starting ZeusAgent…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to ZeusAgent gateway". resolveGatewayWsUrl
        ...
        if (!manual || (isActivePrimary() && gatewayActivationEpoch() === manual.activationEpoch)) {
          reconcileBusyStatesOnReconnect()
          await callbacksRef.current.refreshZeusAgentConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshZeusAgentConfig(false, ownsSwitch).catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startZeusAgent re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new ZeusAgentGateway()
    ...
    async function boot() {
      try {
        ...
        // hanging "Starting ZeusAgent…" forever.
        const wsUrl = await withTimeout(
          resolveGatewayWsUrl(desktop, conn),
          RECONNECT_ATTEMPT_TIMEOUT_MS,
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshZeusAgentConfig(),
          ...
          // retry it), not brick boot behind the "ZeusAgent couldn't start"
