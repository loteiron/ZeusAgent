...
vi.mock('@/zeus', async importOriginal => {
  const actual = await importOriginal<typeof ZeusAgentModule>()

  class FakeZeusAgentGateway {
  ...
  return { ...actual, ZeusAgentGateway: FakeZeusAgentGateway }
})
...
const fakeGateway = { connectionState: 'open' } as unknown as ZeusAgentGateway
...
async function activateRemoteGateway() {
  ...
  setPrimaryGateway(primary as unknown as ZeusAgentGateway, 'default')
  $gateway.set(primary as unknown as ZeusAgentGateway)
...
describe('useGatewayRequest', () => {
  ...
  it('surfaces a missing optional scoped mint bridge without falling back to a stale ticket or local lookup', async () => {
    const { desktop, gateway } = await activateRemoteGateway()
    Reflect.deleteProperty(window.zeusDesktop, 'getGatewayWsUrlFor')
  ...
  it('rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)', async () => {
    ...
    } as unknown as ZeusAgentGateway

    const getConnection = vi.fn(() => new Promise(() => undefined))

    ;(window as unknown as { zeusDesktop: unknown }).zeusDesktop = { getConnection }
    $gateway.set(dropped)
