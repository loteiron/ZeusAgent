import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@zeus/shared'
...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as ZeusAgentGateway | null
  const gatewayRef = useRef<ZeusAgentGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.zeusDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<ZeusAgentGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as ZeusAgentGateway | null
      }),
    []
  )
