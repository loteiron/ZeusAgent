 ...
 * composer with the reply scrolling above it, so ZeusAgent can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.zeusDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.zeusDesktop?.hud
...
export function resetHudLayout(): void {
  void window.zeusDesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.zeusDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.zeusDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
