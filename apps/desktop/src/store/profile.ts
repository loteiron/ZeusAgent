import { LOCAL_CONNECTION_ID } from '@zeus/shared'
import { atom, batch, computed } from 'nanostores'

import type { ZeusAgentConnection } from '@/global'
import { getProfiles, zeusApi, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/zeus'
...
import type { ProfileInfo } from '@/types/zeus'
...
// /api/profiles/active `current`). "default" is the root ~/.zeus. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'zeus.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'zeus.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await zeusApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new ZEUS_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.zeusDesktop.profile.set(name)
}
...
async function resolveConnectionForProfile(profile: string): Promise<ZeusAgentConnection | null> {
  const getConnection = window.zeusDesktop?.getConnection
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<ZeusAgentConnection | null> {
  const getConnectionFor = window.zeusDesktop?.getConnectionFor
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'zeus.desktop.showAllProfiles'
...
export function selectProfile(name: string): void {
  ...
  void Promise.all([activateOnCurrentSource(target), shouldRememberStartupProfile])
    .then(([, shouldRemember]) => {
      if (shouldRemember) {
        return window.zeusDesktop?.profile?.remember(target)
      }
...
async function isLocalDesktopProfile(target: string): Promise<boolean> {
  const getConnectionConfig = window.zeusDesktop?.getConnectionConfig
...
// Switch to the default (root ~/.zeus) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.zeusDesktop?.touchBackend?.(target).catch(() => undefined)
}
