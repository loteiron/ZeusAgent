import type { ConnectionState } from '@zeus/shared'
...
import type { SessionInfo, UsageStats } from '@/types/zeus'
...
const COMPOSER_MODEL_KEY = 'zeus.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'zeus.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'zeus.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'zeus.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'zeus.desktop.composer.fast'
...
function composerScopeForConnection(connection: ZeusAgentConnection | null): string | null {
  if (!connection) {
    return null
  }
...
const LAST_SESSION_KEY = 'zeus.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'zeus.desktop.lastRoute'
...
function workspaceCwdKey(connection: ZeusAgentConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(shouldPublish: () => boolean = () => true): Promise<string> {
  const settings = window.zeusDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(shouldPublish: () => boolean = () => true): Promise<void> {
  const sanitize = window.zeusDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<ZeusAgentConnection | null>(null)
...
const SESSION_OWNER_HINTS_KEY = 'zeus.desktop.sessionOwnerHints.v1'
 ...
 * ZeusAgentConnection lookup is unavailable. */
...
export const setConnection = (next: Updater<ZeusAgentConnection | null>) => {
...
// until config loads, and re-seeded on every profile switch by useZeusAgentConfig.
