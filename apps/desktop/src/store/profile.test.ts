...
const { getProfiles } = await import('@/zeus')
...
const remoteConn = (over: Partial<ZeusAgentConnection> = {}): ZeusAgentConnection =>
  ({ baseUrl: 'https://zeus-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as ZeusAgentConnection
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { zeusDesktop: { getConnection } })
