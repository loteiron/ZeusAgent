...
/**
 * Feature store for backend (agent) plugins — the native ZeusAgent plugins plus
 ...
 * primitives `zeus plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native ZeusAgent. */
  portable?: boolean
}
 ...
 *  ZEUS_HOME. Always refetches (it's a cheap local disk scan on the
