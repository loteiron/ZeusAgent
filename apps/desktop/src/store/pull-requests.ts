...
import type { ZeusAgentBranchPullRequest } from '@/global'
import { scanSessionPullRequests, type SessionInfo } from '@/zeus'
...
export const $pullRequestsByBranch = atom<Record<string, ZeusAgentBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'zeus.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('zeus.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: ZeusAgentBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
