...
const agentConn = (over: Partial<ZeusAgentConnection> = {}): ZeusAgentConnection =>
  ({ baseUrl: 'https://homelab.invalid', mode: 'remote', profile: 'research', ...over }) as ZeusAgentConnection

const localConn = (over: Partial<ZeusAgentConnection> = {}): ZeusAgentConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as ZeusAgentConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<ZeusAgentConnection>>()

const getConnectionFor =
  vi.fn<(payload: { connectionId?: null | string; profile?: null | string }) => Promise<ZeusAgentConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { zeusDesktop: { getConnection, getConnectionFor }, localStorage })
...
describe('ensureGatewayAgent commit hook (beforeActivate) — the Sessions switcher door (#93937)', () => {
  ...
  it('a descriptor lookup that never answers is bounded, fails open, and releases the mutex', async () => {
    ...
    try {
      getConnectionFor.mockImplementationOnce(() => new Promise<ZeusAgentConnection>(() => undefined))
