...
export function setZoomPercent(percent: number): void {
  window.zeusDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.zeusDesktop?.zoom) {
  void window.zeusDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.zeusDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
