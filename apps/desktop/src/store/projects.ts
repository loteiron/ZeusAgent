...
import type { ZeusAgentGitBaseBranch, ZeusAgentGitBranch } from '@/global'
import { getZeusAgentConfig, zeusApi, type ZeusAgentGateway } from '@/zeus'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/zeus'
...
const PROJECT_SCOPE_KEY = 'zeus.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('ZeusAgent gateway is not connected')
  }
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== projectProfile()) {
    throw new Error('Active ZeusAgent profile changed while connecting')
  }
...
async function refreshProjectTreeAcrossProfiles(): Promise<void> {
  ...
  try {
    const res = await zeusApi<ProjectTreePayload>({
...
const repoScanStates = new WeakMap<ZeusAgentGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<ZeusAgentGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  if (isDesktopFsRemoteMode()) {
    ...
    // with `scan: true` — added in #81723) so repos with zero ZeusAgent
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getZeusAgentConfig(context.profile))
...
// repo, returning where ZeusAgent should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<ZeusAgentGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<ZeusAgentGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.zeusDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.zeusDesktop?.writeClipboard?.(path)
  }
}
