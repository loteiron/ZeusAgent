 ...
 *  Working in `~/www/zeus-agent`, the draft says "zeus-agent" constantly
 *  — a "Use skill: zeus-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`zeus-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/zeus-agent-dev`, `plugin:skill`). */
