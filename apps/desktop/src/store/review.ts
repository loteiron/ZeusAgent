...
import type { ZeusAgentReviewFile, ZeusAgentReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — ZeusAgent' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'zeus.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'zeus.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'zeus.desktop.reviewTreeMode'
const SELECTED_KEY = 'zeus.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<ZeusAgentReviewFile[]>([])
...
export const $reviewShipInfo = atom<ZeusAgentReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['zeusDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: ZeusAgentReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly ZeusAgentReviewFile[], path: string): ZeusAgentReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.zeusDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.zeusDesktop?.openExternal?.(url)
    }
