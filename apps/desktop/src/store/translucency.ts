 ...
 * `@zeus/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-zeus-glass]`
 * block in styles.css).
 */
 ...
 * sets `zeusDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.zeusDesktop?.glassSupported === 'boolean'
    ? window.zeusDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.zeusDesktop?.translucencySupported === 'boolean'
    ? window.zeusDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-zeus-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-zeus-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-zeus-glass-on', glassLive)
  root.toggleAttribute('data-zeus-glass', glassOn)
  root.toggleAttribute('data-zeus-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-zeus-glass-scope', scope)
  } else {
    root.removeAttribute('data-zeus-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.zeusDesktop?.setTranslucency?.(state)
  })
