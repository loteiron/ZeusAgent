...
import { type GatewayEvent, LOCAL_CONNECTION_ID, registryBackendScopeKey } from '@zeus/shared'
...
const TILES_KEY = 'zeus.desktop.sessionTiles.v2'
const LEGACY_TILES_KEY = 'zeus.desktop.sessionTiles.v1'
...
const BOT_CHAT_SCOPE_KEY = 'zeus.desktop.botChatSessions.v1'
 ...
 *  (zeus-agent#90102). The tile bucket is a Local Storage cache, and a
 ...
 * restore re-dials the profile's backend, whose ensure_zeus_home() re-creates
 * the profile directory the delete just removed (zeus-agent#94235). Same
...
export function dropTilesForProfile(
  ...
  const ownerMatches = (owner: SessionProfileRoute | undefined): boolean => {
    ...
    // would orphan a live conversation (zeus-agent#94235). Tiles persisted
...
const $focusedTreePaneId = computed([$activeTreeGroup, $layoutTree, $workspaceMode], (groupId, tree, workspaceMode) => {
  ...
  // it to the sidebar group, whose active pane ('zeus-bots:pane') is not a
...
// Dev hook for automation (mirrors __ZEUS_LAYOUT_TREE__).
if ((import.meta.env.DEV || import.meta.env.VITE_PERF_PROBE === '1') && typeof window !== 'undefined') {
  ;(window as unknown as Record<string, unknown>).__ZEUS_SESSION_TILES__ = {
