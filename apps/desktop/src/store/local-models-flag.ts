 ...
 *
 * Local models ship on main behind `--local` (either `zeus desktop --local`
 * or the flag on ZeusAgent.exe itself). The flag is strict: without it the GUI
...
export const $localModelsEnabled = atom<boolean>(
  typeof window !== 'undefined' && window.zeusDesktop?.localModelsEnabled === true
)
