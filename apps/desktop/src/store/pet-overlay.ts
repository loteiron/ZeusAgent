 ...
 * app's bounds and stays visible while ZeusAgent is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'zeus.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'zeus.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.zeusDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.zeusDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.zeusDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.zeusDesktop?.petOverlay?.close()
}
