import { type ConnectionState, type GatewayEvent, registryBackendScopeKey, resolveGatewayWsUrl } from '@zeus/shared'
import { atom } from 'nanostores'

import type { ZeusAgentConnection } from '@/global'
import { ZeusAgentGateway, setApiRequestConnection } from '@/zeus'
...
function dialProfile(
  desktop: NonNullable<typeof window.zeusDesktop>,
  profile: string,
...
const isOpen = (gateway: ZeusAgentGateway | null): boolean => gateway?.connectionState === 'open'
...
interface RegistryConfig {
  ...
  onActiveConnectionChanged?: (connection: ZeusAgentConnection) => void
  /**
...
interface Secondary {
  ...
  connection: ZeusAgentConnection | null
  gateway: ZeusAgentGateway
...
interface GatewayRegistryState {
  ...
  primaryGateway: ZeusAgentGateway | null
  ...
  $gateway: ReturnType<typeof atom<ZeusAgentGateway | null>>
  $activeProfile: ReturnType<typeof atom<string>>
}

const STATE_KEY = Symbol.for('zeus.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<ZeusAgentGateway | null>(null),
...
export function setPrimaryGateway(gateway: ZeusAgentGateway | null, profile = 'default'): void {
...
export function setPrimaryGatewayConnection(connection: Pick<ZeusAgentConnection, 'connectionId'> | null): void {
...
export function activeGateway(): ZeusAgentGateway | null {
  if (g.activeKey === g.primaryProfile) {
...
function applyActive(profile: string, activationEpoch: number): boolean {
  ...
  // Push the active scope's registry connection into the zeus module (null
...
function publishActiveConnection(connection: ZeusAgentConnection): void {
...
async function gatewayForProfile(
  ...
  leaseRequest = false,
  spawnPriority: SpawnPriority = 'background'
): Promise<{ gateway: ZeusAgentGateway | null; key: string; release: () => void; scopeProfile: boolean }> {
  const key = normKey(profile)
...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`ZeusAgent gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.zeusDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update ZeusAgent Desktop.')
  }
...
export async function ensureActiveGatewayOpen(): Promise<ZeusAgentGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "ZeusAgent gateway is not connected" on the Sessions `+` action (#88880).
