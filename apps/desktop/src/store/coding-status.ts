...
import type { ZeusAgentGitWorktree, ZeusAgentRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: ZeusAgentGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, ZeusAgentRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, ZeusAgentGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<ZeusAgentRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<ZeusAgentGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<ZeusAgentRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<ZeusAgentGitWorktree[]>>()
const $noRepoStatus = atom<ZeusAgentRepoStatus | null>(null)
const $noWorktrees = atom<ZeusAgentGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<ZeusAgentRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<ZeusAgentGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: ZeusAgentRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: ZeusAgentGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<ZeusAgentRepoStatus | null>
  seq: number
  target: string
}
