 ...
 * `ZeusAgentPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/zeus'
...
export type { PluginRestOptions } from '@/zeus'
 ...
 *  under `zeus.plugin.<id>.` — plugins can't read or clobber each other. */
 ...
 *  in one attributed namespace instead of the raw `window.zeusDesktop`
...
export interface PluginOs {
   ...
   *  the user is away from ZeusAgent — use `host.notify` for the in-app toast.
   *  Throttled per plugin; reserve it for genuinely notable events.
   *  Supports `icon`, `activate` (e.g. `zeus://index-network/intent/1`),
...
export interface ZeusAgentPlugin {
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `zeus.plugin.${pluginId}.${key}`
...
function createPluginOs(pluginId: string): PluginOs {
  const attempt = async (run: (bridge: NonNullable<typeof window.zeusDesktop>) => Promise<boolean>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.zeusDesktop
  ...
  const attemptPath = async (run: (bridge: NonNullable<typeof window.zeusDesktop>) => Promise<null | string>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.zeusDesktop
