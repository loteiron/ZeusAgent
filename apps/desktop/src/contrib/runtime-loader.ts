 ...
 *   (`@zeus/plugin-sdk` / `react*` -> live shim blobs, see sdk/runtime.ts)
 *   -> blob `import()` -> validate default ZeusAgentPlugin -> register(ctx)
 *
 ...
 * and the two on-disk doors — `<zeus home>/desktop-plugins/<name>/plugin.js`
 * and the unified agent-plugin half `<zeus home>/plugins/<name>/desktop/
...
import { createPluginContext, type ZeusAgentPlugin } from './plugin'
...
interface LoadOptions {
   ...
   *  unified agent-plugin root sets this so `~/.zeus/plugins` keeps its
...
/** Rewrite ONLY mapped import specifiers (@zeus/plugin-sdk, react*) to their
...
export async function loadRuntimePlugin(
  ...
  try {
    ...
    if (unsupported.length > 0) {
      throw new Error(
        `unsupported import${unsupported.length > 1 ? 's' : ''}: ${unsupported.join(', ')} — ` +
          `runtime plugins may only import @zeus/plugin-sdk and react`
      )
    }

    const url = URL.createObjectURL(new Blob([rewriteSpecifiers(source)], { type: 'text/javascript' }))

    let mod: { default?: ZeusAgentPlugin }
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      throw new Error(`${origin} has no valid default ZeusAgentPlugin export`)
    }

    // A disk/runtime copy of a plugin that now ships BUNDLED (e.g. a
    // standalone install of zeus-bots predating its adoption in-tree) must
...
//  - `<zeus home>/desktop-plugins/<name>/plugin.js` — the standalone door
//    (agent- or user-written desktop-only plugins);
//  - `<zeus home>/plugins/<name>/desktop/plugin.js` — the desktop HALF of a
 ...
 *  backend's zeus_home — #66899). `agentPluginsRoot` is optional: older
...
async function diskRoots(): Promise<DiskRoot[]> {
  ...
  if (unified) {
    // Opt-in by default: `~/.zeus/plugins` is installed-but-inert until the
...
async function readPluginSourceText(file: string): Promise<string> {
  ...
  if (result.truncated) {
    throw new PluginSourceOversizeError(
      "plugin.js exceeds this shell's 512 KiB read limit — update ZeusAgent Desktop to load larger plugins"
    )
  }
