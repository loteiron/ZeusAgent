...
import type { ModelOptionProvider, OAuthProvider } from '@/types/zeus'
import { getGlobalModelOptions } from '@/zeus'
...
// onboarding / Providers key form lists every `api_key` provider `zeus model`
...
export function DesktopOnboardingOverlay({
  ...
  return (
    <div
      className={cn(
        'z-onboarding fixed inset-0 z-(--z-onboarding) flex items-center justify-center bg-(--ui-chat-surface-background) p-6 transition-opacity duration-[520ms] ease-out',
    ...
    >
      <div
        className={cn(
          'z-onboarding-card relative w-full max-w-[58rem] transition-all duration-500 ease-out',
          bare
            ? ''
      ...
      >
        ...
        <div className="z-onboarding-form grid gap-3 p-5">
...
function Header() {
  ...
  return (
    <div className="z-onboarding-heading">
      <div className="z-onboarding-identity">
        <BrandMark className="size-11" />
        <span>ZEUS / AGENT</span>
      </div>
      <h2>{t.onboarding.headerTitle}</h2>
      <p>{t.onboarding.headerDesc}</p>
      <svg aria-hidden="true" className="z-onboarding-lines" fill="none" viewBox="0 0 300 180">
        <path d="M-20 40H190L90 140H320M-20 70H120L40 150H320M-20 10H260L160 110H320" stroke="currentColor" />
      </svg>
    </div>
  )
}
...
const SHOW_ALL_KEY = 'zeus-onboarding-show-all-v1'
  ...
  return (
    <div className="grid gap-4">
      ...
      <div className="grid max-h-[42dvh] gap-2 overflow-y-auto p-1 sm:grid-cols-2">
        {options.map(o => (
          <button
            aria-pressed={option.envKey === o.envKey}
            className={cn(
              'rounded-[8px] border bg-background/60 p-3 text-left transition-colors hover:bg-accent/50',
              option.envKey === o.envKey ? 'border-primary bg-primary/10' : 'border-(--ui-stroke-tertiary)'
            )}
            key={o.envKey}
            onClick={() => pick(o)}
            type="button"
