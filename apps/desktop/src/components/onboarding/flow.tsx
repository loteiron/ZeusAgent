...
import { ConnectionLabel, StartChatButton } from './glyph'
...
export function FlowPanel({
  ...
  if (flow.status === 'success') {
    return <ConnectionLabel text={t.onboarding.connectedPicking(title)} />
  }
...
function ConfirmingModelPanel({
  ...
  return (
    <div className="grid place-items-center gap-7 py-6 text-center">
      <ConnectionLabel leaving={leaving} text={t.onboarding.connectedProvider(flow.label)} />
      ...
      >
        ...
        <p className="font-mono text-base">{flow.currentModel}</p>
...
export function Status({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-2.5 py-1 text-sm text-muted-foreground" role="status">
      <Loader className="size-7" type="storm-orbit" />
      {children}
    </div>
  )
}
