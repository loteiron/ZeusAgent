...
const FALLBACK_COPY: IntroCopy[] = [
  ...
  {
    headline: 'What should ZeusAgent look at?',
...
function fallbackCopyForPersonality(personalityKey: string): IntroCopy[] {
  ...
  return [
    ...
    {
      headline: `What does ${label} ZeusAgent need to see?`,
    ...
    {
      headline: `What should ${label} ZeusAgent tackle?`,
...
export function Intro({ personality, seed }: IntroProps) {
  ...
  return (
    <div
      className="z-welcome pointer-events-none flex w-full min-w-0 justify-center px-4 py-8 text-muted-foreground sm:px-8"
      data-slot="aui_intro"
    >
      <div className="z-welcome-copy w-full min-w-0 max-w-[38rem] text-start">
        <div className="mb-8 flex items-center gap-3">
          <BrandMark className="size-12" />
          <Wordmark text={WORDMARK} />
        </div>
        <h1 className="mb-4 text-[clamp(1.6rem,3.2vw,2.6rem)] leading-[1.15] font-semibold tracking-tight text-foreground">
          {copy.headline}
        </h1>
        <p className="m-0 max-w-[32rem] text-[0.9375rem] leading-7">{copy.body}</p>
      </div>
    </div>
  )
}
