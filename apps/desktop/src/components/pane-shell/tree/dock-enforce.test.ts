...
const TREE_KEY = 'zeus.desktop.layoutTree.v2'
const USER_PLACED_KEY = 'zeus.desktop.userPlacedPanes.v1'
const LEGACY_HEAL_KEY = 'zeus.desktop.paneDockHeals.v1'
  ...
  children: [
    {
      ...
      children: [
        ...
        { type: 'group', id: 'g-bots', panes: ['zeus-bots:pane'], active: 'zeus-bots:pane' }
      ]
    },
...
describe('enforced dock (stacked Bots pane → sessions-zone tab, every boot)', () => {
  ...
  it('re-homes a stacked bots pane into the sessions tab strip, keeping sessions active', async () => {
    ...
    const group = model.findGroupOfPane(tree.$layoutTree.get()!, 'zeus-bots:pane')!

    expect(group.panes).toEqual(['sessions', 'zeus-bots:pane'])
    ...
    expect(JSON.stringify(persisted)).toContain('"panes":["sessions","zeus-bots:pane"]')
  })

  it('re-homes even a USER-PLACED pane — the owner invariant beats the drag record', async () => {
    window.localStorage.setItem(USER_PLACED_KEY, JSON.stringify(['zeus-bots:pane']))
  ...
  it('re-homes even when the retired heal token was already burned, and clears the stale ledger', async () => {
    window.localStorage.setItem(LEGACY_HEAL_KEY, JSON.stringify(['zeus-bots:pane:sessions-tab-v1']))
  ...
  it('is idempotent within a boot and does not fight an intra-session drag', async () => {
    ...
    expect(model.findGroupOfPane(tree.$layoutTree.get()!, 'zeus-bots:pane')!.panes).toContain('sessions')
    ...
    expect(group.panes).toEqual(['zeus-bots:pane'])
  })
  ...
  it('re-homes again on the NEXT boot after a drag persisted the stacked shape', async () => {
    ...
    const group = second.model.findGroupOfPane(second.tree.$layoutTree.get()!, 'zeus-bots:pane')!
      ...
      children: [
        {
          type: 'group',
          id: 'g-left',
          panes: ['sessions', 'zeus-bots:pane'],
          active: 'zeus-bots:pane',
          headerHidden: true
        },
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' }
      ]
  ...
  it('re-homes an edge-enforced pane stranded in the sessions tab strip', async () => {
      ...
      children: [
        {
          type: 'group',
          id: 'g-sessions',
          panes: ['sessions', 'zeus-bots:pane', 'zeus-bots:routines'],
          active: 'zeus-bots:pane'
        },
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' }
      ]
    ...
    const botsGroup = model.findGroupOfPane(tree.$layoutTree.get()!, 'zeus-bots:pane')!
    const routinesGroup = model.findGroupOfPane(tree.$layoutTree.get()!, 'zeus-bots:routines')!

    expect(botsGroup.panes).toEqual(['sessions', 'zeus-bots:pane'])
    expect(botsGroup.active).toBe('zeus-bots:pane')
    expect(routinesGroup.panes).toEqual(['zeus-bots:routines'])
    ...
    const dockedRoutinesTree = {
      ...
      children: [
        ...
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' },
        {
          type: 'group',
          id: 'g-routines',
          panes: ['zeus-bots:routines'],
          active: 'zeus-bots:routines'
        }
      ]
    }
