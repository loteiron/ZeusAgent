...
function sessionsBotsTree() {
  ...
  registerPane('zeus-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'zeus-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    ...
    expect(setStripTabHidden('zeus-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('zeus-bots:pane')
    expect($hiddenStripTabs.get()).toContain('zeus-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('zeus-bots:pane')

    expect(setStripTabHidden('zeus-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('zeus-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('zeus-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('zeus-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('zeus.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('zeus-bots:pane')
    ...
    revealTreePane('zeus-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('zeus-bots:pane')
    expect(window.localStorage.getItem('zeus.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'zeus-bots:pane', title: 'zeus-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'zeus-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
