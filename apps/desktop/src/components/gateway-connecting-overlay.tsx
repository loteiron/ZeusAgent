...
// Keep a stable, readable status while the gateway starts.
...
export function GatewayConnectingOverlay() {
  ...
  return (
    ...
    >
      <div
        className={cn(
          'flex flex-col items-center gap-5 text-(--z-storm) transition duration-300 ease-out',
          leaving ? 'translate-y-2 opacity-0 saturate-0' : 'translate-y-0 opacity-100 saturate-100'
        )}
        role="status"
      >
        <BrandMark className="size-16" />
        <span className="text-sm font-semibold tracking-[0.12em]">ZEUS / AGENT</span>
        <span className="inline-flex items-center gap-2 text-xs font-medium tracking-wider">
