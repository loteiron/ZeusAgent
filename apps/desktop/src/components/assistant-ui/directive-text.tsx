...
const ZEUS_REF_TYPES = WIRE_REFERENCE_KINDS
type ZeusAgentRefType = (typeof ZEUS_REF_TYPES)[number]
...
const ZEUS_DIRECTIVE_RE = referenceRe()
...
export const zeusDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(ZEUS_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...zeusDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing ZeusAgent directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.zeusDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.zeusDesktop?.readFileDataUrl(id)
