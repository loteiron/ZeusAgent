...
describe('delivery command detection', () => {
  it('matches the canonical delivery command', () => {
    const cmd = 'zeus -p turqoise chat --in ~ -c "Bot Chat" -Q -q "Message from 🤖 ZeusAgent (@zeus): hi there"'

    expect(deliveryTargetFromCommand(cmd)).toBe('turqoise')
  })

  it('matches with a cd prefix and timeout wrapper', () => {
    const cmd = 'cd ~ && timeout 240 zeus -p mr-tester chat --in "~" -Q -q "Message from 🤖 ZeusAgent: hello"'
  ...
  it('ignores ordinary terminal commands', () => {
    ...
    expect(deliveryTargetFromCommand('zeus -p turqoise chat -q "plain question"')).toBeNull()
    expect(deliveryTargetFromCommand('zeus sessions list')).toBeNull()
  })
})

describe('reply extraction', () => {
  it('strips session_id bookkeeping and keeps the reply', () => {
    const output = 'session_id: 20260813_220347_f69ac6\nHi ZeusAgent! Good to hear from you.'

    expect(replyTextFromResult({ output })).toBe('Hi ZeusAgent! Good to hear from you.')
  })
