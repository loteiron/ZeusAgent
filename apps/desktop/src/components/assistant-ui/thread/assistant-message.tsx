...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's ZEUS_HOME/logs. On a
  ...
  const openLogs = useCallback(async () => {
    try {
      const root = await window.zeusDesktop?.logsRoot?.()
      ...
      const result = await window.zeusDesktop?.openDir?.(root)
