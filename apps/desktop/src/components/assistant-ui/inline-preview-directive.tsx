 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.zeus.send(prompt)` (or declarative
 * `data-zeus-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'zeus-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'zeus-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `zeus.send(prompt)`.
 ...
 *  wires `data-zeus-send` so declarative HTML works with zero script:
 *  `<button data-zeus-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.zeus={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-zeus-send]"):null;' +
    'if(el)send(el.getAttribute("data-zeus-send")||"")},true)})()</script>'
  )
}
...
export function InlinePreviewDirective({
  ...
  // URL connection — including a same-machine `zeus serve` — renders live.)
