import type { GatewayWsUrlResult } from '@zeus/shared'
import type { TranslucencyState } from '@zeus/shared/translucency'
...
declare global {
  interface Window {
    zeusDesktop: {
      ...
      // Resume this session in the user's own terminal emulator (`zeus --tui
      ...
      // ZeusAgent Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new ZEUS_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: ZeusAgentApiRequest) => Promise<T>
      notify: (payload: ZeusAgentNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<ZeusAgentReadFileTextResult>
      ...
      readPluginSource?: (filePath: string) => Promise<ZeusAgentReadFileTextResult>
      selectPaths: (options?: ZeusAgentSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<ZeusAgentPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<ZeusAgentPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<ZeusAgentPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: ZeusAgentActiveWork) => void
      setTitleBarTheme?: (payload: ZeusAgentTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<ZeusAgentReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<ZEUS_HOME>/desktop-plugins),
      ...
      /** LOCAL `<ZEUS_HOME>/logs` (profile-aware) — error card "Open Logs". */
      logsRoot?: () => Promise<string>
      // Local AGENT-plugin root (<ZEUS_HOME>/plugins), same Electron-local
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<ZeusAgentGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<ZeusAgentGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<ZeusAgentGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<ZeusAgentRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: ZeusAgentReviewScope, baseRef?: null | string) => Promise<ZeusAgentReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: ZeusAgentReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<ZeusAgentReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<ZeusAgentRepoPullRequests>
          ...
          fetchPrComment: (repoPath: string, url: string) => Promise<ZeusAgentPrComment | null>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: ZeusAgentTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<ZeusAgentTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: ZeusAgentWindowState) => void) => () => void
      ...
      onPreviewFileChanged: (callback: (payload: ZeusAgentPreviewFileChanged) => void) => () => void
...
export interface ZeusAgentTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface ZeusAgentTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `zeus update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  zeusRoot?: string
...
export interface ZeusAgentConnection {
  baseUrl: string
  darwinMajor?: number
...
export interface ZeusAgentTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface ZeusAgentActiveWork {
  count: number
  titles: string[]
}

export interface ZeusAgentWindowState {
  darwinMajor?: number
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a ZeusAgent Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted ZeusAgent Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected ZeusAgent Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the zeus gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered ZeusAgent Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface ZeusAgentApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface ZeusAgentNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface ZeusAgentPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface ZeusAgentReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface ZeusAgentPreviewWatch {
  id: string
  path: string
}
...
export interface ZeusAgentGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface ZeusAgentGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface ZeusAgentGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface ZeusAgentRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface ZeusAgentRepoStatus {
  branch: null | string
...
// turn began.
export type ZeusAgentReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface ZeusAgentReviewFile {
  path: string
  added: number
  removed: number
...
export interface ZeusAgentReviewList {
  files: ZeusAgentReviewFile[]
...
export interface ZeusAgentReviewPr {
  url: string
  state: string
  number: number
}
...
export interface ZeusAgentBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface ZeusAgentRepoPullRequests {
  ghReady: boolean
  prs: ZeusAgentBranchPullRequest[]
}
...
export interface ZeusAgentPrComment {
  author: string
  body: string
  diffHunk: string
  kind: 'issue' | 'review'
...
export interface ZeusAgentReviewShipInfo {
  ghReady: boolean
  pr: ZeusAgentReviewPr | null
}

export interface ZeusAgentReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface ZeusAgentReadDirResult {
  entries: ZeusAgentReadDirEntry[]
  error?: string
}

export interface ZeusAgentPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface ZeusAgentSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
