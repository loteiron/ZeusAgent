...
import { getZeusAgentConfigRecord, type ZeusAgentConfigRecord, saveZeusAgentConfig } from '@/zeus'
...
export interface I18nConfigClient {
  getConfig: () => Promise<ZeusAgentConfigRecord>
  saveConfig: (config: ZeusAgentConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.zeusDesktop?.api) {
      return Promise.resolve({})
    }

    return getZeusAgentConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.zeusDesktop?.api) {
    ...
    return saveZeusAgentConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: ZeusAgentConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: ZeusAgentConfigRecord, locale: Locale): ZeusAgentConfigRecord {
