import { JsonRpcGatewayClient } from '@zeus/shared'

import type { ZeusAgentApiRequest } from '@/global'
...
// "Timed out connecting to ZeusAgent backend" that hangs the UI (#48504).
//
...
export class ZeusAgentGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'ZeusAgent gateway connection closed',
      connectErrorMessage: 'Could not connect to ZeusAgent gateway',
...
// process to route them to that host (zeus:api's registry branch). Null
...
export function zeusApi<T>(request: ZeusAgentApiRequest): Promise<T> {
  return window.zeusDesktop.api<T>({ ...connectionScoped(), ...request })
}
