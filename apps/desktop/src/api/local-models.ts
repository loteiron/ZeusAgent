import type { LocalCatalogModel, LocalHardware, LocalModelsStatus, LocalRuntimeJob } from '@/types/zeus'

import { zeusApi, profileScoped } from './client'
...
export function getLocalModelsStatus(): Promise<LocalModelsStatus> {
  return zeusApi<LocalModelsStatus>({
    ...profileScoped(),
...
export function getLocalCatalog(): Promise<{ models: LocalCatalogModel[] }> {
  return zeusApi<{ models: LocalCatalogModel[] }>({
    ...profileScoped(),
...
export function installLocalRuntime(backend?: string): Promise<{ backend: string; job_id: string; tag: string }> {
  return zeusApi<{ backend: string; job_id: string; tag: string }>({
    ...profileScoped(),
...
export function quickstartLocalModels(modelId?: string): Promise<QuickstartResponse> {
  return zeusApi<QuickstartResponse>({
    ...profileScoped(),
...
export function downloadLocalModel(modelId: string): Promise<{ already_downloaded?: boolean; job_id: null | string }> {
  return zeusApi<{ already_downloaded?: boolean; job_id: null | string }>({
    ...profileScoped(),
...
export function getLocalRuntimeJob(jobId: string): Promise<LocalRuntimeJob> {
  return zeusApi<LocalRuntimeJob>({
    ...profileScoped(),
...
export function getLocalModelsJobs(): Promise<{ jobs: LocalRuntimeJob[] }> {
  return zeusApi<{ jobs: LocalRuntimeJob[] }>({
    ...profileScoped(),
...
export function searchHFModels(q: string, limit = 20): Promise<{ hits: HFSearchHit[] }> {
  return zeusApi<{ hits: HFSearchHit[] }>({
    ...profileScoped(),
...
export function listHFRepoFiles(repo: string): Promise<{ files: HFFileGroup[] }> {
  return zeusApi<{ files: HFFileGroup[] }>({
    ...profileScoped(),
...
export function downloadBrowsedModel(
  ...
  return zeusApi<{ already_downloaded?: boolean; job_id: null | string; model_id: string }>({
    ...profileScoped(),
    body: { paths, repo },
    method: 'POST',
...
export function sideloadLocalModel(
  ...
  return zeusApi<{ already_present?: boolean; model_id: string; ok: boolean }>({
    ...profileScoped(),
    body: { path },
    method: 'POST',
