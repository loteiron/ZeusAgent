...
import { atom, host } from '@zeus/plugin-sdk'
...
const GROUP_CHAT_SYNC_META_KEY = 'zeus-bots-groups'
...
/** The v3 envelope stored under the default profile's `zeus-bots-groups`
 ...
 *  clients (ZeusAgent Go, headless backends) see rooms regardless of which
 ...
 *  renamed bot never shows up as its raw profile id or a stale "ZeusAgent"
 *  (community report, Aug 21 2026: renamed default still read "ZeusAgent is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as ZeusAgent (matching displayName and the
 *  @zeus handle) so the main agent never loses its name in rooms. */
export function groupSpeakerLabel(name?: null | string) {
  ...
  // Core profile display_name (`zeus profile rename …` / dashboard) from
  ...
  return trimmed.toLowerCase() === 'default' ? 'ZeusAgent' : trimmed
}
