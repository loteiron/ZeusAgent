/**
 * Routines: the ZeusAgent cron jobs scoped to the bot you're chatting with — the
...
// zeus -p <bot> chat delegation wrapper so the run reaches that bot's
...
export function routinePrompt(
  ...
  return (
    ...
    `zeus -p ${shellQuote(bot)} chat -c ${shellQuote(`Routine: ${title}`)} -q ${shellQuote(`[Scheduled routine] ${instruction}`)}\n\n` +
...
// ZeusAgent-native schedule string; Advanced exposes it raw.
...
/** Compose the ZeusAgent schedule string from picker state. */
