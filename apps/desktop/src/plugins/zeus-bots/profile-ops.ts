...
import { forgetSessionUnread, host, queryClient } from '@zeus/plugin-sdk'
...
export function mergeServerMeta(roster: RosterRow[], fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['zeus-bots']
...
/** Permanently delete a bot's ZeusAgent profile, then remove plugin-local state
 ...
 * directory (zeus-agent#52279). That is the "can't delete a bot" error. */
