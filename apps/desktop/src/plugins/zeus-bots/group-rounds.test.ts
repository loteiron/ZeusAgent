...
describe('routing', () => {
  ...
  it('resolves @zeus to the default member', async () => {
    const { rounds } = await loadRoom()
    ...
    const parsed = rounds.parseGroupChatMentions('@zeus take a look', members)
...
describe('turn prompt', () => {
  it('addresses the default profile as @zeus', async () => {
    const { rounds } = await loadRoom()
    ...
    expect(own).toMatch(/You are @zeus,/)
    ...
    expect(peer).toMatch(/group chat with @zeus/)
  })
