...
import * as sdk from '@zeus/plugin-sdk'
import { host } from '@zeus/plugin-sdk'
...
// grandfather, or recover. Stored-id pins (ui_meta['zeus-bots'].chat) were
 ...
 *  click path's tile-staleness probe (zeus-agent#90102), which must
...
async function openStoredBotChat(
  ...
  if (!storedId || typeof host.openSession !== 'function') {
    throw new Error('This ZeusAgent Desktop version cannot open stored sessions')
  }
  ...
  // even though the session is fine (zeus-agent#89617) — clicking Retry
  ...
  // until an app restart (zeus-agent#93604). A resume is cheap and
  ...
  if (route && typeof host.requestProfile !== 'function') {
    throw new Error(
      getPluginCtx()?.i18n?.t('bot.remoteConnectionsUnsupported') ??
        'Update ZeusAgent Desktop to chat with bots on other connections.'
    )
  }
...
export async function ensureBotMetadata(bot: RosterRow): Promise<BotMeta> {
  ...
  const server = row?.ui_meta?.['zeus-bots']
