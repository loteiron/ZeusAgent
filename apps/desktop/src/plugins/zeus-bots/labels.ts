...
export function displayName(bot: Partial<RosterRow>, meta?: BotMeta | null): string {
  ...
  // "ZeusAgent". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `zeus profile rename
  ...
  // never generic "ZeusAgent" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as ZeusAgent (the agent it is)
