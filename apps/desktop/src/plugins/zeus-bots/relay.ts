...
import { host, LruCache } from '@zeus/plugin-sdk'
...
// to (local, remote URL, SSH, ZeusAgent Cloud, docker) must be able to find
...
// zeus_cli/config_defaults.py and tui_gateway/methods_bot_relay.py and fails
  ...
  try {
    ...
    return profiles
      .map(profile => ({
        ...
        title: String(profile?.ui_meta?.['zeus-bots']?.title || profile?.display_name || ''),
