...
vi.mock('@zeus/plugin-sdk', () => ({ host: hostMock, LruCache: UnboundedCache }))

vi.mock('./data', () => ({
  botHandle: (name: string) => (name === 'default' ? 'zeus' : name),
...
describe('the roster loop pushes the OTHER connections’ agents', () => {
  it('gives each gateway a union roster that excludes its own agents', async () => {
    ...
    expect(syncs[1].params.agents).toEqual([
      expect.objectContaining({ connection_id: 'a', handle: 'zeus', profile: 'default' })
    ])
