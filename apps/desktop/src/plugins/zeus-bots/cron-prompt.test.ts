 ...
 * job for a different profile keeps the `zeus -p <bot> chat` delegation
...
/** Run the delegation command under a `zeus` stub that prints its argv, so
...
describe('direct vs delegated execution', () => {
  ...
  it('keeps the delegation wrapper for a different active profile', () => {
    ...
    expect(prompt).toMatch(/zeus -p 'research' chat/)
