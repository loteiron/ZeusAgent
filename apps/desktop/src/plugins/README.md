...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `ZeusAgentPlugin` and
...
`$ZEUS_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`zeus-desktop-plugins` skill.
