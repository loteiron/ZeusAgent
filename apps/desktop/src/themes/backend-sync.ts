/**
 * Live skin sync from the ZeusAgent backend.
 *
 * The backend resolves the active skin (built-in or `$ZEUS_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (ZeusAgent
 * authoring/activating a skin from a prompt, or `/skin` elsewhere) repaints.
 */

import type { ZeusAgentSkin } from '@zeus/shared/skin'
...
const BACKEND_THEMES_KEY = 'zeus-desktop-backend-themes-v1'

const readCached = (): Record<string, DesktopTheme> =>
  Object.fromEntries(
    Object.entries(readJson<Record<string, unknown>>(BACKEND_THEMES_KEY) ?? {}).filter(
      (entry): entry is [string, DesktopTheme] =>
        !BUILTIN_THEMES[entry[0]] && !LEGACY_SKIN_NAMES[entry[0]] && isValidTheme(entry[1])
    )
  )
 ...
 */
export function ingestBackendSkin(skin: ZeusAgentSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  // (zeus), so we never register a converted theme under `default`. It is still a
  // valid apply TARGET though: a runtime switch back to `default` must repaint the
  // desktop to its own default (setTheme normalizes `default` → zeus). So we only
  ...
  if (!LEGACY_SKIN_NAMES[name] && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as ZeusAgentSkin)
