...
describe('ingestBackendSkin', () => {
  ...
  it('seed does not paint, but a later same-name skin.changed applies (missed-activation recovery)', () => {
    ...
    // restarted). ZeusAgent re-affirms it — `zeus config set display.skin neon`
    // or a `zeus skin set` recolor. That explicit event must repaint even
    ...
    ingestBackendSkin(skin('forest'), { apply: true }) // ZeusAgent authored a new skin
    expect($pendingSkinApply.get()).toBe('forest')
  })
  ...
  it('applies a runtime switch back to default (repaints the desktop to its own default)', () => {
    ...
    ingestBackendSkin(skin('default'), { apply: true }) // ZeusAgent switched back to default
  ...
  it('hydrates the registry from storage on load, so a persisted pick resolves before the gateway connects', async () => {
    ingestBackendSkin(skin('neon'), { apply: false })
    expect(JSON.parse(window.localStorage.getItem('zeus-desktop-backend-themes-v1') ?? '{}').neon?.name).toBe('neon')
  ...
  it('drops junk and built-in names from the cached registry', async () => {
    ...
    window.localStorage.setItem('zeus-desktop-backend-themes-v1', JSON.stringify({ mono, bad: { name: 'bad' } }))
