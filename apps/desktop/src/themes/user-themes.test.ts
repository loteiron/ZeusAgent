...
import { BUILTIN_THEMES, DEFAULT_SKIN_NAME, LEGACY_SKIN_NAMES } from './presets'
import {
  $marketplaceInstalls,
  $userThemes,
  installUserTheme,
...
describe('user theme registry', () => {
  ...
  it('installs a theme into the merged registry and persists it', () => {
    ...
    expect(window.localStorage.getItem('zeus-desktop-user-themes-v1')).toContain(theme.name)
  })
  ...
  it.each(Object.keys(LEGACY_SKIN_NAMES))('rejects an imported %s alias without changing saved themes', name => {
    const existing = installUserTheme(makeTheme('Custom'))
    const saved = window.localStorage.getItem('zeus-desktop-user-themes-v1')

    const imported = convertVscodeColorTheme(
      { name: 'Imported palette', colors: { 'editor.background': '#101014', 'editor.foreground': '#fafafa' } },
      { slug: name }
    ).theme

    expect(() => installUserTheme(imported)).toThrow(/reserved|built-in/)
    expect(isUserTheme(name)).toBe(false)
    expect(window.localStorage.getItem('zeus-desktop-user-themes-v1')).toBe(saved)
    expect(resolveTheme(existing.name)).toBe(existing)
    expect(resolveTheme(name)).toBe(BUILTIN_THEMES[DEFAULT_SKIN_NAME])
  })

  it('allows a namespaced VS Code import whose display label happens to be Nous', () => {
    const imported = installUserTheme(makeTheme('Nous'))

    expect(imported.name).toBe('vsc-nous')
    expect(resolveTheme(imported.name)).toBe(imported)
    expect(listAllThemes()).toContain(imported)
  })
