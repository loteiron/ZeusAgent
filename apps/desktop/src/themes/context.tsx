...
import { BUILTIN_THEME_LIST, DEFAULT_SKIN_NAME, DEFAULT_TYPOGRAPHY, LEGACY_SKIN_NAMES, zeusTheme } from './presets'
...
const SKIN_KEY = 'zeus-desktop-theme-v2'
const MODE_KEY = 'zeus-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'zeus-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'zeus-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'zeus-desktop-active-profile-v1'
...
const RETIRED_SKINS = new Set(Object.keys(LEGACY_SKIN_NAMES))
...
export function getBaseColors(skinName: string, mode: 'light' | 'dark'): DesktopThemeColors {
  const seed = resolveTheme(skinName) ?? zeusTheme
...
const NEUTRAL_CHROME = { light: '#EAF0F7', dark: '#101D31' } as const
...
const chromeBackground = (background: string, isDark: boolean, isZeus: boolean) =>
  isZeus ? background : mix(background, NEUTRAL_CHROME[isDark ? 'dark' : 'light'], isDark ? 0.26 : 0.08)

const mixesFor = (isDark: boolean, isZeus: boolean): Record<string, string> => ({
  '--theme-mix-chrome': isZeus ? '100%' : isDark ? '74%' : '92%',
  '--theme-mix-sidebar': '100%',
  '--theme-mix-card': isZeus ? '100%' : isDark ? '38%' : '22%',
  '--theme-mix-elevated': isZeus ? '100%' : isDark ? '46%' : '28%',
  '--theme-mix-bubble': isZeus ? '100%' : isDark ? '46%' : '0%'
})
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  const typo = { ...DEFAULT_TYPOGRAPHY, ...zeusTheme.typography, ...theme.typography }
  ...
  root.dataset.zeusTheme = skinName
  root.dataset.zeusMode = rendered
  ...
  for (const [k, v] of Object.entries({ ...seeds, ...mixesFor(isDark, skinName === 'zeus'), ...palette })) {
    root.style.setProperty(k, v)
  }

  const chromeBg = chromeBackground(c.background, isDark, skinName === 'zeus')

  window.zeusDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('zeus-boot-background', chromeBg)
    window.localStorage.setItem('zeus-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.zeusThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.zeusDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (ZeusAgent authoring/activating a skin from a
