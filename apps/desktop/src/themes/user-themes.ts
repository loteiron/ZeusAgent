...
const USER_THEMES_KEY = 'zeus-desktop-user-themes-v1'
...
function readStored(): Record<string, DesktopTheme> {
  try {
    ...
    for (const [key, value] of Object.entries(parsed)) {
      // Built-in names and migration aliases have the same reserved ownership
      // at restore time as they do during a new import.
      if (!BUILTIN_THEMES[key] && !LEGACY_SKIN_NAMES[key] && isValidTheme(value)) {
        out[key] = value
      }
    }
...
export function installUserTheme(theme: DesktopTheme): DesktopTheme {
  ...
  if (LEGACY_SKIN_NAMES[theme.name]) {
    throw new Error(`"${theme.name}" is reserved for Zeus theme compatibility. Choose a different theme name.`)
  }
...
export function resolveTheme(name: string): DesktopTheme | undefined {
  name = LEGACY_SKIN_NAMES[name] ?? name
...
export function listAllThemes(): DesktopTheme[] {
  ...
  return [
    ...
    ...Object.values(backend).filter(theme => !user[theme.name] && !LEGACY_SKIN_NAMES[theme.name]),
    ...Object.values(user).filter(theme => !LEGACY_SKIN_NAMES[theme.name])
  ]
}
