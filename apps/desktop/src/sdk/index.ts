/**
 * @zeus/plugin-sdk — THE plugin language. The vscode-module model: plugin
 ...
 *    object as `window.__ZEUS_PLUGIN_SDK__` and maps the import to it, so a
...
import { deleteProfile, getLogs, getStatus, zeusApi, type ZeusAgentGateway } from '@/zeus'
...
import type { PaginatedSessions, UsageStats } from '@/types/zeus'
...
export interface PluginProfileRoute {
  ...
  profile: string
  /** Backend ZeusAgent profile served by that route. */
...
async function requestPluginProfile<T>(
  ...
  const getAgentRoster = window.zeusDesktop?.getAgentRoster
...
async function pluginRouteStillRegistered(route: PluginProfileRoute): Promise<boolean> {
  const getProfileRoutes = window.zeusDesktop?.getProfileRoutes
...
//
// ensureGatewayProfile awaits the store's dial, and ZeusAgentGateway.connect() has
...
export const host = {
  ...
  deleteProfile: async (profile: string | PluginProfileRoute): Promise<void> => {
    ...
    // restores on relaunch and re-creates the deleted profile (zeus-agent#94235).
    dropTilesForProfile(
  ...
  connections: async () => {
    const bridge = window.zeusDesktop?.connections

    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update ZeusAgent Desktop.')
    }
  ...
  agents: async () => {
    const roster = window.zeusDesktop?.getAgentRoster

    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update ZeusAgent Desktop.')
    }
  ...
  newChat: (profile?: null | string | PluginProfileRoute, options: PluginNewChatOptions = {}): void => {
    if (options.workspaceMode === 'bots') {
      ...
      if (!openTab) {
        notify({ kind: 'error', message: 'Update ZeusAgent Desktop to open another Bot chat.' })
   ...
   *
   *  `isStaleTile` (zeus-agent#90102): the caller's reconciliation probe
  ...
  profileRoutes: async () => {
    ...
    if (!getProfileRoutes) {
      throw new Error('ZeusAgent Desktop connection routing unavailable')
    }
  ...
  listPersistedSessions: async (
    ...
    return zeusApi<PaginatedSessions>({
  ...
  setPersistedSessionHidden: async (
    ...
    return zeusApi<{ ok: boolean; hidden: boolean }>({
      ...(route ? { connectionId: route.connectionId } : {}),
  ...
  request: async <T>(method: string, params: Record<string, unknown> = {}): Promise<T> => {
    ...
    if (!gateway) {
      throw new Error('ZeusAgent gateway unavailable')
    }
 ...
 *  "ZEUS AGENT" with it, a `chat.empty` contribution writes its own name. */
...
export type { ZeusAgentGateway } from '@/zeus'
...
export type { ZeusAgentOpenTarget } from '@/lib/zeus-open-target'
...
/** ZeusAgent' reasoning levels + their compact labels, so a plugin surfacing a
...
export type { RpcEvent, StatusResponse } from '@/types/zeus'
