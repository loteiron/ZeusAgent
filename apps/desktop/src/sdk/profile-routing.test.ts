...
vi.mock('@/zeus', () => ({ deleteProfile: vi.fn(), getLogs: vi.fn(), getStatus: vi.fn(), zeusApi: vi.fn() }))
...
const { deleteProfile, zeusApi } = await import('@/zeus')
...
describe('connection-aware plugin host APIs', () => {
  it('retires a profile gateway before deleting it', async () => {
    ...
    // profile's backend, re-creating its ZEUS_HOME (#94235).
  ...
  it('refreshes the profile inventory before asking Electron for routes', async () => {
    ...
    ;(window as unknown as { zeusDesktop: unknown }).zeusDesktop = { getProfileRoutes }
  ...
  it('reads and hides persisted sessions through the source primary without activating the profile', async () => {
    ...
    expect(zeusApi).toHaveBeenNthCalledWith(
      1,
    ...
    expect(zeusApi).toHaveBeenNthCalledWith(2, {
      connectionId: 'source-a',
      path: '/api/sessions/bot-chat',
      method: 'PATCH',
      body: { hidden: true, profile: 'backend-worker' }
    })
    expect(vi.mocked(zeusApi).mock.calls.every(([request]) => !('profile' in request))).toBe(true)
