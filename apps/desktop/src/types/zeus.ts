...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when ZeusAgent doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in zeus_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `zeus model` uses — instead of
...
export interface ZeusAgentConfig {
  agent?: {
...
export type ZeusAgentConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `zeus model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// non-desktop) ZeusAgent simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (zeus.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors zeus_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with ZeusAgent, 'hub' = installed. */
  provenance?: 'agent' | 'bundled' | 'hub'
}

/** One entry of the built-in optional-skills catalog (optional-skills/ in the
 *  repo) — official skills that ship with ZeusAgent but install on demand. */
...
export interface ToolProvider {
   ...
   *  CLI `zeus tools` active-provider detection). */
  is_active: boolean
 ...
 *  not ZeusAgent); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** zeus-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** zeus-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/zeus/update/check` — the backend's own update state.
