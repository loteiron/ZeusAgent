...
//
// When someone is in a hands-free "Hey ZeusAgent" voice chat, the natural way to
...
// phrase (optionally addressed to ZeusAgent), so a real turn that merely contains
...
// Optional address prefixes so "zeus stop" / "ok stop" / "hey zeus, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey zeus', 'hey zeus,', 'zeus', 'zeus,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("zeus") is not a stop command on its own.
      continue
    }
 ...
 * to ZeusAgent). Returns false for anything that merely contains "stop" as part of
