...
// ZeusAgent is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.zeusMode
 ...
 * the only carriers of ZeusAgent' palette — are dropped so the paste target's
