...
/** Why a known ZeusAgent command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active ZeusAgent profile', surface: action('profile') },
  {
    name: '/skin',
...
function isKnownZeusAgentSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * ZeusAgent' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownZeusAgentSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with ZeusAgent and were never asked for. It is
