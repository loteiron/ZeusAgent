...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __zeusWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__zeusWatch || (win.__zeusWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('zeus-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('zeus-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __zeusWatchTag?: number }).__zeusWatchTag
