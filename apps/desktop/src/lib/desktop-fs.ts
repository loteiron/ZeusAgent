import { zeusApi } from '@/api/client'
import type {
  ZeusAgentConnection,
  ZeusAgentReadDirResult,
  ZeusAgentReadFileTextResult,
  ZeusAgentSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: ZeusAgentConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: ZeusAgentConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<ZeusAgentReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<ZeusAgentReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<ZeusAgentReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<ZeusAgentReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.zeusDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: ZeusAgentSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
