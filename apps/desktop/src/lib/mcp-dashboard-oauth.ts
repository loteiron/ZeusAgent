...
const UPDATE_BACKEND = 'Update the ZeusAgent backend to support Desktop MCP OAuth callbacks.'
...
export async function completeMcpDesktopOAuth({
  ...
  const bridge = window.zeusDesktop.mcpOauth

  // A legacy null connection can resolve to a remote registry primary.
  if (!bridge && scope.connectionId !== 'local') {
    throw new Error('Update ZeusAgent Desktop to support MCP OAuth callbacks.')
  }
  ...
  try {
    ...
    await window.zeusDesktop.openExternal(authUrl)
    let pollFailures = 0
