 ...
 * product name (`ZeusAgent`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/ZeusAgent'
 *
 * This is a hard failure with no obvious cause for the user — `zeus desktop`
 ...
 * (the mac-unpacked ZeusAgent.app bundle) and Windows (win-unpacked), so we
 ...
 * ZeusAgent.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `zeus desktop --build-only` (zeus_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'ZeusAgent.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'ZeusAgent'}.exe`
