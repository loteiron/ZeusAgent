...
const LEGACY_STORAGE_KEY = 'zeus.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'zeus.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.zeusDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `zeus-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const zeusHome = join(sandbox, 'zeus-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(zeusHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(zeusHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        ZEUS_DESKTOP_APP_NAME: `ZeusAgentShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        ZEUS_DESKTOP_ZEUS_ROOT: target.targetRoot,
        ZEUS_DESKTOP_IGNORE_EXISTING: '1',
        ZEUS_DESKTOP_USER_DATA_DIR: userData,
        ZEUS_HOME: zeusHome,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'zeus-short-session-ab-'))
  const prepared = []
