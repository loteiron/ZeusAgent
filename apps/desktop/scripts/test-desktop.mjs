...
// no longer bundles the ZeusAgent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'ZeusAgent.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'ZeusAgent'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'ZeusAgent.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'ZeusAgent'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default ZEUS_HOME for non-sandboxed runs -- matches main.ts's
// resolveZeusAgentHome(). On Windows it's %LOCALAPPDATA%\zeus; elsewhere
// it's ~/.zeus. The fresh-install sandbox launchFresh() sets its own
// ZEUS_HOME and never touches this.
const DEFAULT_ZEUS_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'zeus')
  }
  return path.join(os.homedir(), '.zeus')
})()
const VENV_ROOT = path.join(DEFAULT_ZEUS_HOME, 'zeus-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'zeus-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.ZEUS_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `ZeusAgent-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `ZeusAgent-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `ZeusAgent-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'ZeusAgent-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.ZEUS_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.ZEUS_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const zeusHome = path.join(sandbox, 'zeus-home')
  ...
  env.ZEUS_DESKTOP_CWD = cwd
  env.ZEUS_DESKTOP_IGNORE_EXISTING = '1'
  env.ZEUS_DESKTOP_TEST_MODE = 'fresh-install'
  env.ZEUS_DESKTOP_USER_DATA_DIR = userDataDir
  env.ZEUS_HOME = zeusHome
  delete env.ZEUS_DESKTOP_ZEUS
  delete env.ZEUS_DESKTOP_ZEUS_ROOT
  ...
  console.log(`  ZEUS_HOME: ${zeusHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(zeusHome, 'zeus-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The ZeusAgent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of zeus_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'zeus-agent', 'zeus_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing ZeusAgent
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + ZEUS_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  ZEUS_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
