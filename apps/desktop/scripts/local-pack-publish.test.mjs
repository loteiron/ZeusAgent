...
function fakePackager(metadata) {
  const info = {
    ...
    appInfo: { version: '0.0.0', channel: null, updaterCacheDirName: 'zeus' },
    config: {},
    options: {}
  }
  return {
...
describe('local desktop pack stays out of the publish path', () => {
  test('the pack script pins an explicit publish policy', () => {
    ...
    // absent, and `zeus desktop` runs the pack with CI=1 (_npm_lifecycle_env).
  ...
  test('publish resolution succeeds with a GitHub token present', async () => {
    ...
    // The declared URL is upstream provenance, not an invented ZeusAgent release repository.
    const declaredUrl = new URL(desktopPkg.repository.url.replace(/^git\+/, ''))
    const [owner, repository] = declaredUrl.pathname.slice(1).replace(/\.git$/, '').split('/')
    assert.equal(configs[0].owner, owner)
    assert.equal(configs[0].repo, repository)
  })
