...
// Node's native recursive copy can fast-fail on Unicode Windows source paths.
// A filter selects its JS traversal while retaining symlink and mode semantics.
const recursiveCopyOptions = { recursive: true, filter: () => true }
...
function copyBuildRelease(srcDir, destDir) {
  ...
  for (const entry of readdirSync(srcDir, { withFileTypes: true })) {
    if (entry.isDirectory()) {
      cpSync(join(srcDir, entry.name), join(destDir, entry.name), recursiveCopyOptions)
      continue
    }
  ...
  if (existsSync(prebuildDir)) {
    ...
    for (const entry of readdirSync(prebuildDir, { withFileTypes: true })) {
      if (entry.name === 'conpty' && entry.isDirectory()) {
        cpSync(join(prebuildDir, 'conpty'), join(destPrebuild, 'conpty'), recursiveCopyOptions)
        continue
      }
