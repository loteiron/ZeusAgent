...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `zeus-dev-mock-${Date.now()}`))
  ...
  return { root, zeusHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(zeusHome, mockUrl) {
  fs.writeFileSync(
    path.join(zeusHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(zeusHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.zeusHome, mock.url)
  console.log(`  ZEUS_HOME: ${sandbox.zeusHome}`)
  ...
  const env = {
    ...
    ZEUS_DESKTOP_APP_NAME: `ZeusAgentDevMock-${Date.now()}`,
  }
