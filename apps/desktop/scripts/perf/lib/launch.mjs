...
//   · its own ZEUS_HOME      → its own backend + sessions, no shared state.
//   · its own --remote-debugging-port → a private CDP endpoint.
//   · ZEUS_DESKTOP_BOOT_FAKE=1 → deterministic boot overlay.
...
// Seed an isolated ZEUS_HOME with just enough config (NOT sessions) so the
// spawned instance reaches an empty chat view instead of the onboarding wizard.
// A separate ZEUS_HOME dir means a separate gateway lock — no collision with
...
export async function startIsolatedInstance({
  ...
  const home = zeusHome ?? mkTemp('zeus-perf-home-')
  const userData = userDataDir ?? mkTemp('zeus-perf-ud-')
  const devUrl = prod ? null : `http://127.0.0.1:${devPort}`

  if (seedConfig && !zeusHome) {
    seedConfigFrom(join(homedir(), '.zeus'), home)
  }
  ...
  try {
    ...
    // ZEUS_HOME (backend + sessions). No DEV_SERVER env in prod → dist load.
    const electronBin = require('electron')
    // NB: do NOT set ZEUS_DESKTOP_BOOT_FAKE here — it injects artificial
    ...
    const env = {
      ...
      ZEUS_DESKTOP_CDP_PORT: String(port),
      XCURSOR_SIZE: '24'
    }

    if (devUrl) {
      env.ZEUS_DESKTOP_DEV_SERVER = devUrl
    }
...
export async function coldStartSamples({ runs = 3, port = 9222, devPort = 5174, prod = false, warm = true } = {}) {
  ...
  if (warm) {
    ...
    const home = mkdtempSync(join(tmpdir(), 'zeus-perf-cold-home-'))
    const userDataDir = mkdtempSync(join(tmpdir(), 'zeus-perf-cold-ud-'))
    seedConfigFrom(join(homedir(), '.zeus'), home)
