...
test.each(['prebuilds', 'build'])('nested %s runtime payload survives Unicode source paths', (layout) => {
  const tmp = fs.mkdtempSync(join(os.tmpdir(), 'zeus-stage-unicode-'))
  try {
    const srcRoot = join(tmp, 'Türkçe kaynak')
    const destRoot = join(tmp, 'Türkçe hedef')
    const runtime = layout === 'prebuilds'
      ? join('prebuilds', `${process.platform}-${process.arch}`)
      : join('build', 'Release')
    makeFakeNodePty(srcRoot)
    makeFakeNode(join(srcRoot, runtime, 'pty.node'), process.platform)
    fs.mkdirSync(join(srcRoot, runtime, 'conpty', 'nested'), { recursive: true })
    const payloads = {
      'conpty.dll': Buffer.from([0, 1, 255, 128]),
      [join('nested', 'OpenConsole.exe')]: Buffer.from([77, 90, 13, 10, 0]),
    }
    for (const [name, bytes] of Object.entries(payloads)) {
      fs.writeFileSync(join(srcRoot, runtime, 'conpty', name), bytes)
    }
    // A native copy crash must fail this contract without terminating the test worker.
    const moduleUrl = new URL('./stage-native-deps.mjs', import.meta.url).href
    const result = spawnSync(process.execPath, ['--input-type=module', '--eval',
      `import { stageNodePtyInto } from ${JSON.stringify(moduleUrl)};
       stageNodePtyInto(process.argv[1], process.argv[2]);`, srcRoot, destRoot],
    { encoding: 'utf8', timeout: 15_000, windowsHide: true })
    assert.equal(result.status, 0, result.stderr || `staging exited ${result.status}`)
    for (const [name, bytes] of Object.entries(payloads)) {
      assert.deepEqual(fs.readFileSync(join(destRoot, runtime, 'conpty', name)), bytes)
    }
    assert.equal(classifyNativeBinary(join(destRoot, runtime, 'pty.node')), process.platform)
  } finally {
...
test('classifyNativeBinary detects ELF as linux', () => {
  const tmp = fs.mkdtempSync(join(os.tmpdir(), 'zeus-stage-'))
  try {
...
test('darwin staging ships the Swift helper and the rewritten windows.js', () => {
  const tmp = fs.mkdtempSync(join(os.tmpdir(), 'zeus-stage-'))
  try {
    const srcRoot = join(tmp, 'get-windows')
    const destRoot = join(tmp, 'dest')
...
test.skipIf(process.platform === 'win32')('darwin staging makes the Swift helper executable on POSIX hosts', () => {
  const tmp = fs.mkdtempSync(join(os.tmpdir(), 'zeus-stage-'))
  try {
    const srcRoot = join(tmp, 'get-windows')
    const destRoot = join(tmp, 'dest')
    ...
    makeFakeGetWindows(srcRoot)
    fs.chmodSync(join(srcRoot, 'main'), 0o644)
