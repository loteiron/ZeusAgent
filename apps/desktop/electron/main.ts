...
import { isZeusAgentOwnedVenvDaemon } from './venv-holder-select'
...
const USER_DATA_OVERRIDE = process.env.ZEUS_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.ZEUS_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.ZEUS_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with ZEUS_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[zeus] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[zeus] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. ZEUS_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[zeus] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[zeus] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// it). The value arrives via ZEUS_DESKTOP_PASSWORD_STORE, bridged by the
// `zeus desktop` launcher from detection or `desktop.password_store` in
...
if (PASSWORD_STORE.warning) {
  console.warn(`[zeus] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[zeus] using password-store backend: ${PASSWORD_STORE.store}`)
}
...
// never go through `zeus desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose ZeusAgent sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[zeus] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[zeus] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[zeus] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[zeus] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[zeus] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('zeus:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized ZeusAgent burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[zeus] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[zeus] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[zeus] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[zeus] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// ZEUS_HOME — the user-facing root for everything ZeusAgent-related. Mirrors
// scripts/install.ps1's $ZeusAgentHome and scripts/install.sh's $ZEUS_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\zeus (matches install.ps1)
//   macOS / Linux: ~/.zeus (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.zeus directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\zeus yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// ZEUS_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// ZEUS_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.zeus / %LOCALAPPDATA%\zeus.
function resolveZeusAgentHome() {
  if (process.env.ZEUS_HOME) {
    return normalizeZeusAgentHomeRoot(process.env.ZEUS_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'zeus-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a ZEUS_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\zeus and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('ZEUS_HOME')

    if (fromRegistry) {
      return normalizeZeusAgentHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'zeus')
    const legacy = path.join(app.getPath('home'), '.zeus')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.zeus setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.zeus')
}

const ZEUS_HOME = resolveZeusAgentHome()

function pathWithZeusAgentManagedNode(...entries) {
  const managed = zeusManagedNodePathEntries(ZEUS_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_ZEUS_ROOT — the canonical mutable ZeusAgent install. Same path
...
// boots, going straight to `resolveZeusAgentBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_ZEUS_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_ZEUS_ROOT, '.zeus-bootstrap-complete')
...
// active-profile.json records which ZeusAgent profile the desktop launches its
// local backend as. When set, startZeusAgent() passes `zeus --profile <name>
// dashboard …`, which deterministically pins ZEUS_HOME (see
// _apply_profile_override in zeus_cli/main.py) and bypasses the sticky
// ~/.zeus/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors zeus_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// zeusDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under ZEUS_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by zeus_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(ZEUS_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (zeus_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.ZEUS_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.ZEUS_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.ZEUS_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.ZEUS_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.ZEUS_DESKTOP_APP_NAME || 'ZeusAgent'
...
// 'zeus:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.zeus) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.zeus')
}

// Seed the native About panel with the live ZeusAgent version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `zeus update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveZeusAgentVersion(),
...
// startZeusAgent(); this pool only holds EXTRA profile
...
// The legacy ZEUS_DESKTOP_POOL_* env vars remain the initial-value fallback
...
function readPersistedPoolLimits() {
  try {
    ...
    const fromEnv = clampPoolLimits({
      maxBackends: Number(process.env.ZEUS_DESKTOP_POOL_MAX) || undefined,
      idleMs: Number(process.env.ZEUS_DESKTOP_POOL_IDLE_MS) || undefined
    })
...
function logPoolSpawnFailure(label: string, error: unknown): void {
  if (isBackgroundSlotWaitTimeout(error)) {
    ...
    rememberLog(
      `ZeusAgent backend for profile ${label} failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
  }
}
...
//   * WSL2 IPC stall  = the renderer's `zeus:backend:touch` roundtrips
...
const POOL_KEEPALIVE_FRESH_MS = Math.max(
  120_000,
  Number(process.env.ZEUS_DESKTOP_POOL_KEEPALIVE_FRESH_MS) || 4 * 60_000
)
...
// onto the error so subsequent startZeusAgent() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning zeus serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start ZeusAgent backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
function rememberLog(chunk) {
  ...
  if (zeusLog.length > 300) {
    zeusLog.splice(0, zeusLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-zeus-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('zeus:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the zeus:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('zeus:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_ZEUS_ROOT
  })
}
...
// The Tauri updater writes ZEUS_HOME/.zeus-update-in-progress for the whole
...
// (`force_kill_other_zeus`, taskkill /IM zeus.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(ZEUS_HOME)),
...
const BUNDLE_SWAP_RELAUNCH_FLAG = '--zeus-bundle-swap-relaunched'
...
// The detached updater swaps the packaged bundle on disk AFTER `zeus update`
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — ZeusAgent will start automatically when it completes…',
        12
      )
    },
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(ZEUS_HOME)
    ...
    if (result && result.ok && result.manual) {
      ...
      dialog.showErrorBox(
        'ZeusAgent update did not finish',
        `${result.message}\n\nDetails: ${path.join(ZEUS_HOME, 'logs', 'desktop-update-handoff.log')}`
      )
    }
  } catch (err) {
  ...
  if (outcome === 'timeout') {
    ...
    await advanceBootProgress('backend.update-restart', 'Restarting ZeusAgent to load the updated app…', 14)
...
function findOnPath(command) {
  ...
  // shell-script shim named `zeus` — must not shadow `zeus.cmd`/`zeus.exe`.
...
function unwrapWindowsVenvZeusAgentCommand(command, backendArgs) {
  return resolveVenvZeusAgentCommand(command, backendArgs, {
    isWindows: IS_WINDOWS,
    isCommandScript,
    fileExists,
    directoryExists,
...
// spawns `zeus serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `zeus` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'zeus_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `zeus --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, ZEUS_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveZeusAgentBackend step 4); execFileSync of a .cmd without
...
function isZeusAgentSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'zeus_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.ZEUS_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several ZeusAgent deps (notably pywinpty's Rust-built
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the zeus-import
...
// findGitBash — locate bash.exe on Windows. Resolves ZEUS_GIT_BASH_PATH
...
// normal ZEUS_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\zeus\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'zeus', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'zeus', 'git', 'bin', 'git.exe'))
  }
...
function recentZeusAgentLog() {
  return zeusLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's zeus root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_ZEUS_ROOT.
// ZEUS_DESKTOP_ZEUS_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.ZEUS_DESKTOP_ZEUS_ROOT && path.resolve(process.env.ZEUS_DESKTOP_ZEUS_ROOT),
    !IS_PACKAGED && isZeusAgentSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isZeusAgentSourceRoot(ACTIVE_ZEUS_ROOT) ? ACTIVE_ZEUS_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_ZEUS_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('zeus:updates:progress', merged)
  }
}
      ...
      const req = https.get(
        url,
        {
          headers: {
            ...
            'User-Agent': 'zeus-desktop-update-check'
          },
          timeout: 10_000
        },
        res => {
...
// Windows that binary owns ALL repo mutation — running `zeus update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(ZEUS_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `zeus update` can write
// fresh entry points. On Windows this is the file the running backend
// `zeus.exe` holds open; on POSIX it's never mandatory-locked.
function venvZeusAgentShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'zeus.exe')
    : path.join(updateRoot, 'venv', 'bin', 'zeus')
}
...
// Kill only ZeusAgent-OWNED venv daemons (the memory plugin's hindsight daemon:
...
// venv files mapped. External holders (a user terminal running `zeus`,
...
function killZeusAgentOwnedVenvDaemons(updateRoot) {
  if (!IS_WINDOWS) {
    return
  }
  ...
  try {
    ...
    holders = (Array.isArray(parsed) ? parsed : [parsed]).filter(p =>
      isZeusAgentOwnedVenvDaemon(p?.ExecutablePath, p?.CommandLine, scriptsDir)
    )
  } catch {
  ...
  for (const holder of holders) {
    ...
    if (Number.isInteger(pid) && pid > 0) {
      rememberLog(`[updates] stopping ZeusAgent-owned venv daemon (hindsight) PID ${pid} before hand-off`)
...
// only signals the direct child, so on Windows a backend `zeus.exe` that
// spawned its own grandchildren (a `zeus` REPL, a pty terminal session, the
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  if (decision.action === 'fail') {
    ...
    throw new Error(
      `ZeusAgent backend (PID ${child.pid}) died before its identity could be recorded: ${decision.reason}${outputTail?.describe() ?? ''}`
    )
  }
  ...
  if (decision.action === 'degrade') {
    ...
    rememberLog(
      `WARNING: process start marker probe failed for live ZeusAgent backend PID ${child.pid}; ` +
  ...
  try {
    ...
    child.zeusBackendIdentity = identity
    ...
    await waitForBackendExit(child)
    throw new Error(
      `Could not persist ownership for the ZeusAgent backend: ${error.message}${outputTail?.describe() ?? ''}`
    )
  }
}

function releaseBackendChild(child) {
  const identity = child?.zeusBackendIdentity
...
// `zeusProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `zeus.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a zeus
// REPL, a pty terminal, the gateway) keep `zeus.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const zeusProcess = backendConnectionState.getProcess()
  ...
  // unlock — the shim probe only covers venv\Scripts\zeus.exe, but the
  // backend is `python.exe -m zeus_cli.main serve`, which need not hold
  // the shim at all (#74805 first-attempt race).
  const initialPids = []

  if (zeusProcess && Number.isInteger(zeusProcess.pid)) {
    initialPids.push(zeusProcess.pid)
  }
  ...
  stopBackendTreesForUpdate(zeusProcess, {
  ...
  // `zeus gateway stop --all`: the CLI discovers every profile's gateway
  ...
  stopGatewayBeforeUpdate(venvZeusAgentShimPath(updateRoot), ZEUS_HOME)

  // Reap ZeusAgent-OWNED venv daemons the tree-kill above cannot reach: the
  ...
  killZeusAgentOwnedVenvDaemons(updateRoot)

  const shim = venvZeusAgentShimPath(updateRoot)
  ...
  const gate = await waitForBackendRelease(
    initialPids,
    {
      ...
      collectStragglerPids: () => {
        ...
        const currentZeusAgentProcess = backendConnectionState.getProcess()

        if (currentZeusAgentProcess && Number.isInteger(currentZeusAgentProcess.pid)) {
          stragglers.push(currentZeusAgentProcess.pid)
        }
...
// `zeus update`). Instead we spawn the staged ZeusAgent-Setup binary with
// --update and quit, so it can run `zeus update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `zeus update` → relaunch),
      ...
      // in: the ZEUS_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `zeus update` card once; their next update pulls it.
      return await applyUpdatesPosixHandoff(opts)
    }

    if (!updater) {
      // No staged updater binary — this is a CLI-installed user (they ran
      // `zeus desktop`, never the Tauri installer that self-copies
      // zeus-setup.exe into ZEUS_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `zeus` on PATH / in the venv, so the
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `zeus update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `zeus update` if branch detection fails.
        }

        rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
        emitUpdateProgress({ stage: 'manual', message: command, percent: null })

        return { ok: true, manual: true, command, zeusRoot: updateRoot }
      }

      rememberLog('[updates] no staged updater; using repo hand-off script for CLI install')
    }

    const handoffConflict = updateHandoffConflict(ZEUS_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating ZeusAgent — this window will close and the updater will open. Don’t reopen ZeusAgent yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(ZEUS_HOME, rememberLog)
    ...
    // zeus.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running zeus, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the ZeusAgent install open ' +
        '(a second ZeusAgent window or a terminal running zeus?). Close it and retry.'

      emitUpdateProgress({ stage: 'error', message, percent: null })
      startZeusAgent().catch(() => {})

      if (IS_WINDOWS) {
        // The pre-gate `gateway stop --all` (#70337) took every profile's
        // gateway down for an update that never happened — bring them back.
        startGatewaysAfterUpdateAbort(venvZeusAgentShimPath(updateRoot))
      }
    ...
    // ZeusAgent processes running from this venv.  The updater normally refuses
    ...
    // `zeus update` will run (the venv shim is locked while we live).
    //
    ...
    // checkout, so each `zeus update` refreshes the code that drives the
    ...
    if (scriptHandoff) {
      ...
      child = spawnUpdaterProcess(wrapped.command, wrapped.args, {
        ...
        env: {
          ...process.env,
          ZEUS_HOME,
          ZEUS_UPDATE_STARTED_AT: String(updateStartedAt),
          PATH: pathWithZeusAgentManagedNode(venvBin)
        },
        detached: true,
        stdio: 'ignore'
      })
      ...
      // The `zeus update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(ZEUS_HOME, child.pid, { startedAt: updateStartedAt })
      }
      ...
      // abort with "Another ZeusAgent update is already running (PID <itself>)" —
      ...
      if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
        writeUpdateMarker(ZEUS_HOME, child.pid)
    ...
    if (!handoffOutcome.ok) {
      const message = `Update failed to start: ${handoffOutcome.message}. ZeusAgent will keep running — try again, or run \`zeus update\` from a terminal.`
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvZeusAgent = path.join(venvBin, IS_WINDOWS ? 'zeus.exe' : 'zeus')
  const venvPython = path.join(venvBin, IS_WINDOWS ? 'python.exe' : 'python')

  // The updater invokes the venv's ZeusAgent launcher, which in turn requires the
  ...
  // marker-only install through --update dead-ends at "Could not find the zeus
  ...
  const updaterArgs = chooseUpdaterArgs(
    {
      hasBootstrapMarker: fileExists(path.join(updateRoot, '.zeus-bootstrap-complete')),
      hasVenvZeusAgent: fileExists(venvZeusAgent),
...
// Python-level pre-update snapshot inside `zeus update`.
function preflightStateDb(zeusHome, rememberLog) {
  const stateDbPath = path.join(zeusHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(zeusHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(zeusHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(zeusHome, old))
            } catch {
              void 0
            }
          }
...
// out, runs `zeus update`, swaps/relaunches the app bundle, and writes
// .zeus-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'zeus update', percent: null })

    return { ok: true, manual: true, command: 'zeus update', zeusRoot: updateRoot }
  }
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...
      ZEUS_HOME,
      ZEUS_UPDATE_STARTED_AT: String(updateStartedAt),
      PATH: pathWithZeusAgentManagedNode(path.join(updateRoot, 'venv', 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating ZeusAgent — this window will close. Don’t reopen ZeusAgent yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
// Marker-independent: is the canonical install at ACTIVE_ZEUS_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isZeusAgentSourceRoot(ACTIVE_ZEUS_ROOT) &&
    fileExists(venvPython) &&
    canImportZeusAgentCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_ZEUS_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `zeus
...
function resolveWebDist() {
  const override = process.env.ZEUS_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set ZEUS_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: zeus desktop --force-build`
    )
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: zeus desktop --force-build`
  )
...
function resolveZeusAgentCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/ZeusAgent.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveZeusAgentCwd(), sanitized: Boolean(trimmed) }
  }
...
// createActiveBackend — build a backend pointing at ACTIVE_ZEUS_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `ZeusAgent at ${ACTIVE_ZEUS_ROOT}`,
    command,
    args: ['-m', 'zeus_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      zeusHome: ZEUS_HOME,
      pythonPathEntries: [ACTIVE_ZEUS_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_ZEUS_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveZeusAgentBackend(backendArgs) {
  // 1. Explicit override -- ZEUS_DESKTOP_ZEUS_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.ZEUS_DESKTOP_ZEUS_ROOT && path.resolve(process.env.ZEUS_DESKTOP_ZEUS_ROOT)

  if (overrideRoot && isZeusAgentSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `ZeusAgent source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `zeus` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isZeusAgentSourceRoot.)
  if (!IS_PACKAGED && isZeusAgentSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `ZeusAgent source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active ZeusAgent runtime at ${ACTIVE_ZEUS_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `zeus` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    ZEUS_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.ZEUS_DESKTOP_IGNORE_EXISTING !== '1') {
    let zeusCommand = null
    const zeusOverride = process.env.ZEUS_DESKTOP_ZEUS

    if (zeusOverride) {
      const resolvedOverride = findOnPath(zeusOverride)

      if (resolvedOverride) {
        zeusCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(zeusOverride, { isWsl: IS_WSL })) {
        zeusCommand = zeusOverride
      } else {
        rememberLog(`Ignoring Windows ZeusAgent override under WSL: ${zeusOverride}`)
      }
    } else {
      zeusCommand = findOnPath('zeus')
    }

    if (zeusCommand) {
      if (looksLikeDesktopAppBinary(zeusCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving ZeusAgent CLI: ${zeusCommand}`)
        zeusCommand = null
      }
    }

    if (zeusCommand) {
      const unwrapped = unwrapWindowsVenvZeusAgentCommand(zeusCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `zeus` shim
      ...
      const shellForProbe = isCommandScript(zeusCommand)

      // ZEUS_DESKTOP_ZEUS is an explicit deployment override (used by
      ...
      if (shouldTrustZeusAgentOverride(zeusOverride) || verifyZeusAgentCli(zeusCommand, { shell: shellForProbe })) {
      ...
      rememberLog(
        `Ignoring existing ZeusAgent CLI at ${zeusCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed zeus_cli module via system Python.
  ...
  if (python) {
    ...
    // having zeus_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\zeus\zeus-agent\venv.
    if (canImportZeusAgentCli(python)) {
      return {
        kind: 'python',
        label: `installed zeus_cli module via ${python}`,
        command: python,
        args: ['-m', 'zeus_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: zeus_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveZeusAgentBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'ZeusAgent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no ZeusAgent install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'ZeusAgent recovery was handed off to ZeusAgent Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      zeusHome: ZEUS_HOME,
      logRoot: path.join(ZEUS_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('ZeusAgent install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `ZeusAgent bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(ZEUS_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startZeusAgent() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // zeus:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveZeusAgentBackend(backend.args))
  }
  ...
  if (!isZeusAgentSourceRoot(ACTIVE_ZEUS_ROOT)) {
    throw new Error(
      `ZeusAgent install at ${ACTIVE_ZEUS_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. ZeusAgent' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\zeus\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `zeus` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for ZeusAgent on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch ZeusAgent.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isZeusAgentSourceRoot()
    // plus an importable zeus_cli before it hands back the active runtime.
  ...
  backend.label = `ZeusAgent at ${ACTIVE_ZEUS_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'ZeusAgent runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----zeus-${crypto.randomBytes(12).toString('hex')}`
    ...
    (requestState: any) =>
      new Promise((resolve, reject) => {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
          reject(new Error(`Unsupported ZeusAgent backend URL protocol: ${parsed.protocol}`))
                ...
                reject(
                  new Error(
                    `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                      'The endpoint is likely missing on the ZeusAgent backend.'
                  )
                )
        ...
        req.setTimeout(timeoutMs, () => {
          req.destroy(new Error(`Timed out connecting to ZeusAgent backend after ${timeoutMs}ms`))
        })
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      parsed,
      {
        agent,
        method: 'GET',
        headers: options.bearer ? { Authorization: `Bearer ${options.bearer}` } : { 'X-Zeus-Session-Token': token }
      },
      res => {
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Zeus-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('zeus:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveZeusAgentCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('zeus:preview-file-changed', payload)
}
...
// waitForZeusAgentReady how to read a 401 — rejected session vs gated route.
...
async function waitForZeusAgent(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForZeusAgentReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('zeus:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('zeus:close-preview-requested')
}
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('zeus:preview-nav', command)
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('zeus:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting ZeusAgent...".
function sendPowerResume() {
  ...
  webContents.send('zeus:power-resume')
}
...
// 'zeus:power-battery' push below.
ipcMain.handle('zeus:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('zeus:power-battery', next)
    }
  }
}
...
async function showPluginCompatNoticeOnce() {
  ...
  try {
    notice = pendingPluginCompatNotice(ZEUS_HOME, app.getPath('userData'))
  } catch (err) {
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('zeus:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('zeus:window-state-changed', state)
}
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('zeus:context-menu-spellcheck', {
...
//
// Hosted ZeusAgent gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``zeus_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / zeus #37247).
//     Both are set as HttpOnly cookies (``zeus_session_at`` ~15 min,
//     ``zeus_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
// "not signed in", which on the initial boot path (startZeusAgent → the renderer's
// single-shot boot() with no retry) surfaced as the "ZeusAgent couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "ZeusAgent
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to ZeusAgent Cloud agent…' : 'Sign in to ZeusAgent gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to ZeusAgent backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in zeus_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- ZeusAgent Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:zeus-remote-oauth`:
...
// convention (zeus_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every ZeusAgent surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.nousresearch.com'

function resolvePortalBaseUrl() {
  const raw = process.env.ZEUS_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// authenticates via PRIVY, not the ZeusAgent gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for zeus_session_at/rt that the portal
    ...
    return await new Promise<boolean>(resolve => {
      ...
      try {
        win = new BrowserWindow({
          ...
          height: 720,
          show: false,
          title: 'Renewing ZeusAgent Cloud session…',
          autoHideMenuBar: true,
          webPreferences: {
            contextIsolation: true,
            nodeIntegration: false,
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a ZeusAgent Cloud sign-in.'))
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to ZeusAgent Cloud',
        autoHideMenuBar: true,
        webPreferences: {
          contextIsolation: true,
          nodeIntegration: false,
...
// Discover the hosted (ZeusAgent Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to ZeusAgent Cloud. Open Settings → Gateway, choose ZeusAgent Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your ZeusAgent Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your ZeusAgent Cloud session has expired. Sign in to ZeusAgent Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the ZeusAgent Cloud org tag on cloud-mode entries so Settings can
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every zeus:connections:* call.
 ...
 * editor doesn't carry — cloud `org`, ssh `remoteZeusAgentPath`/`remoteProfile` —
...
// a valid stored value (pins the root ZEUS_HOME explicitly); null means "no
...
// contains "zeus", avoiding false positives from stale gateway.pid files
// whose PID was recycled by the OS to an unrelated process.
function isZeusAgentProcess(pid) {
  try {
...
//   1. Legacy ~/.zeus/active_profile (explicit CLI choice via zeus profile use)
//   2. Running gateway (gateway.pid with verified liveness + zeus identity)
...
function migrateActiveProfileIfMissing() {
  migrateActiveProfileIfMissingPure(DESKTOP_PROFILE_CONFIG_PATH, {
    legacyActivePath: path.join(ZEUS_HOME, 'active_profile'),
    zeusHome: ZEUS_HOME,
    profilesRoot: path.join(ZEUS_HOME, 'profiles'),
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.ZEUS_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.ZEUS_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted ZeusAgent Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteZeusAgentPath: (ssh || savedSsh)?.remoteZeusAgentPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by ZEUS_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the ZeusAgent Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered ZeusAgent Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteZeusAgentPath: input.sshRemoteZeusAgentPath ?? existingBlock.remoteZeusAgentPath,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (zeus #37247), and the gateway middleware transparently
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote ZeusAgent gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote ZeusAgent gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget(webContentsId?: number) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.ZEUS_DESKTOP_REMOTE_TOKEN,
      url: process.env.ZEUS_DESKTOP_REMOTE_URL
    },
    profile,
    registry: readDesktopConnectionsRegistry()
  })
...
async function rollbackSshBootstrapResult(ssh, result, profile, sshConfig, boundaryError) {
  ...
  try {
    const expected = {
      ...
      zeusPath: result.zeusPath,
      zeusHome: result.zeusHome,
    ...
    if (result.platform?.os === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        ssh,
        { zeusPath: result.zeusPath, zeusHome: result.zeusHome, python: result.pythonPath },
        expected
      )
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, metadata, fingerprint, lease) {
  ...
  try {
    ...
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteZeusAgentPath || '')
    const lifecycle = platform.os === 'Windows' ? connectWindowsRemote : remoteLifecycle.connect
    result = await lifecycle({
      ssh,
      profile: resolveRemoteSshDashboardProfile(sshConfig.remoteProfile, profile),
      remoteZeusAgentPath: sshConfig.remoteZeusAgentPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForZeusAgent: (baseUrl, token) => waitForZeusAgent(baseUrl, token, lease.signal, 'token'),
  ...
  await fenceManagedSshBootstrapPublication({
    ...
    publish: () => {
      ...
      sshConnections.set(scope, {
        ...
        host: sshConfig.host,
        hostLabel,
        zeusVersion: result.zeusVersion || '',
        ...
        // per-profile pools also use bare keys. Only startZeusAgent' explicit call
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.zeusVersion || 'zeus (version unknown)'} at ${result.zeusPath || '?'}`
  )
  ...
  return {
    ...connection,
    remoteZeusAgentVersion: result.zeusVersion || '',
    ssh: {
      effectiveConfigFingerprint: sshConfig.effectiveConfigFingerprint,
      host: sshConfig.host,
...
//   2. env override (ZEUS_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
function globalRemoteActive() {
  if (process.env.ZEUS_DESKTOP_REMOTE_URL) {
    return true
  }
...
// env → global) so a startZeusAgent() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every ZeusAgent
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteZeusAgentPath: input.sshRemoteZeusAgentPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteZeusAgentPath || '')
          let zeusPath
          let zeusVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            zeusPath = runtime.zeusPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.zeusPath])
            zeusVersion = inspection.version
            supported = inspection.supported
          } else {
            zeusPath = await remoteLifecycle.locateZeusAgent(ssh, sshConfig.remoteZeusAgentPath || '')
            zeusVersion = await remoteLifecycle.probeZeusAgentVersion(ssh, zeusPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, zeusPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update ZeusAgent on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteZeusAgentPath: zeusPath,
            remoteZeusAgentVersion: zeusVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startZeusAgent())
  ...
  // connect to ZeusAgent gateway". Mirror the renderer's connect here so the test
...
function resetZeusAgentConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const zeusProcess = backendConnectionState.invalidate()
  stopBackendChild(zeusProcess)
...
// startZeusAgent() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetZeusAgentConnection() invalidates it.
  const zeusProcess = backendConnectionState.getProcess()
  const dying = zeusProcess && !zeusProcess.killed ? zeusProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('zeus:connection:applied')
}
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'saved' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('zeus:connections:changed', payload)
    }
  }
}
...
async function ensureBackend(profile, opts: { spawnPriority?: LocalBackendSpawnPriority } = {}) {
  ...
  if (route.backend === 'primary') {
    const connection = await startZeusAgent()
  ...
  // spawning, so two children never share one profile's ZEUS_HOME.
  ...
  const resolveRegistrySshConfig = () => {
    ...
    if (!resolvedRegistrySshConfig) {
      resolvedRegistrySshConfig = normalizeSshConfig({
        ...
        remoteZeusAgentPath: source.remoteZeusAgentPath,
...
async function connectRegistryBackend(
  ...
  if (source.kind === 'ssh') {
    ...
    return {
      ...
      // is only the routing label. zeus:api uses it to translate explicit
      // self-profile query filters into the backend's namespace.
      remoteProfile: sshConfig.remoteProfile || '',
      logs: zeusLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  await waitForZeusAgent(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
function remoteUpdateTargetFromState(state): RemoteUpdateTarget {
  if (!state?.ssh || !state?.zeusPath || !state?.zeusHome) {
  ...
  return {
    ...
    zeusPath: state.zeusPath,
    zeusHome: state.zeusHome,
...
async function openManagedSshUpdateTransport(
  ...
  try {
    const platform: any = await detectRemotePlatform(ssh, config.remoteZeusAgentPath || '')

    if (platform.os === 'Windows') {
      const runtime = platform.zeusPath ? platform : await probeWindowsRemote(ssh, config.remoteZeusAgentPath || '')
      ...
      return {
        ...
        target: {
          ...
          platform: 'Windows',
          zeusPath: runtime.zeusPath,
          zeusHome: runtime.zeusHome,
          pythonPath: runtime.python
        }
      }
    }

    const zeusPath = await remoteLifecycle.locateZeusAgent(ssh, config.remoteZeusAgentPath || '')
    const zeusHome = await remoteLifecycle.probeRemoteZeusAgentHome(ssh)

    return {
      close: () => ssh.close(),
      target: { ssh, platform: platform.os, zeusPath, zeusHome }
    }
  } catch (error) {
    await ssh.close()
    throw error
...
async function drainManagedSshScope(scope) {
  ...
  try {
    ...
    if (state.remotePlatform === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        state.ssh,
        { zeusPath: state.zeusPath, zeusHome: state.zeusHome, python: state.pythonPath },
        expected
      )
    ...
    if (state && state.localPort && state.remotePort && scope.reuseToken) {
      try {
        ...
        if (!forwardClosed) {
          await waitForZeusAgent(`http://127.0.0.1:${state.localPort}`, scope.reuseToken, undefined, 'token')
        } else {
...
// local-spawn portion of startZeusAgent() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  if (remote) {
    await waitForZeusAgent(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  // --profile wins over the inherited ZEUS_HOME env (see _apply_profile_override
  // step 3 in zeus_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveZeusAgentBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const zeusCwd = resolveZeusAgentCwd()
  ...
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(ZEUS_HOME, 'profiles', key))
  )
  rememberLog(`Starting ZeusAgent backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: zeusCwd,
        ZEUS_DASHBOARD_SESSION_TOKEN: token,
        ...
        ...parentIdentityEnv,
        ZEUS_WEB_DIST: webDist,
        ...(readyFile ? { ZEUS_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`ZeusAgent backend for profile "${profile}" failed to start: ${error.message}`)
    void teardownFailedLocalBackend(poolKey, entry).catch(cleanupError => {
      rememberLog(
        `ZeusAgent backend for profile "${profile}" cleanup failed: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}`
      )
    })
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`ZeusAgent backend for profile "${profile}" exited (${signal || code})`)
    releaseLocalBackendSlot(entry)
    releaseBackendChild(child)
    ...
    if (!ready) {
      rejectStart?.(
        new Error(
          `ZeusAgent backend for profile "${profile}" exited before it became ready (${signal || code}).${outputTail.describe()}`
        )
      )
    }
  })
  ...
  await Promise.race([waitForZeusAgent(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `ZeusAgent backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `ZeusAgent backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_zeus_home() recreates the deleted profile directory.
//
...
async function startZeusAgent() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('ZeusAgent Desktop is already running in another window.')
  }

  await reapOrphanedBackendsOnce()

  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startZeusAgent() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving ZeusAgent backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      if (!backendConnectionState.isCurrentAttempt(connectionAttempt)) {
        throw new Error('ZeusAgent backend start was superseded by a newer connection attempt.')
      }

      await advanceBootProgress('backend.remote', `Connecting to remote ZeusAgent backend at ${remote.baseUrl}`, 24)
      await waitForZeusAgent(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote ZeusAgent backend is ready',
        progress: 94,
        running: true,
        error: null
      })

      return createPrimaryRemoteConnection(remote, zeusLog.slice(-80), getWindowState())
    }
    ...
    // deterministic (it wins over the sticky ~/.zeus/active_profile file) and
    // resolves ZEUS_HOME the same way `zeus -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.spawn', `Starting ZeusAgent backend via ${backend.label}`, 84)
    rememberLog(`Starting ZeusAgent backend via ${backend.label}`)
    ...
    const zeusProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin ZEUS_HOME for the child so Python's get_zeus_home()
          // resolves to the SAME location our resolveZeusAgentHome() picked. Without
          // this pin, Python falls back to ~/.zeus on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\zeus, which differs from C:\Users\<u>\.zeus.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets ZEUS_HOME via setx; the desktop
    ...
    primaryOutputTail.attach(zeusProcess)
    ...
    const portAnnouncement = waitForDashboardPortAnnouncement(zeusProcess, {
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, zeusProcess)

    if (!processOwner) {
      stopBackendChild(zeusProcess)
      await waitForBackendExit(zeusProcess)
      releaseBackendChild(zeusProcess)
      throw new Error('ZeusAgent backend start was superseded by a newer connection attempt.')
    }

    zeusProcess.stdout.on('data', rememberLog)
    zeusProcess.stderr.on('data', rememberLog)
    ...
    zeusProcess.once('error', error => {
      releaseBackendChild(zeusProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale ZeusAgent backend error: ${error.message}`)
        rejectBackendStart?.(new Error('ZeusAgent backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`ZeusAgent backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `ZeusAgent backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    zeusProcess.once('exit', (code, signal) => {
      releaseBackendChild(zeusProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale ZeusAgent backend exit (${signal || code})`)
      ...
      rememberLog(`ZeusAgent backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `ZeusAgent backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `ZeusAgent backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentZeusAgentLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for ZeusAgent backend to launch', 86)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for ZeusAgent backend to become ready', 90)
    await Promise.race([waitForZeusAgent(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => zeusProcess.exitCode === null && !zeusProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local ZeusAgent backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'ZeusAgent backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
    ...
    // The backend's plugin discovery just ran and refreshed ZEUS_HOME/.plugin-compat-report.json.
...
function wireCommonWindowHandlers(win, { zoom = true }: { zoom?: boolean } = {}) {
  ...
  // links arrive via `zeus:openExternal`, not here. See window-open-policy.ts.
...
function notifyBrowserPopoutClosed(tabId) {
  ...
  for (const other of BrowserWindow.getAllWindows()) {
    if (!other.isDestroyed()) {
      other.webContents.send('zeus:browser-popout:closed', tabId)
    }
  }
}
...
// here so it can leave the app's bounds and stay visible while ZeusAgent is
...
// pushes pet state over IPC (zeus:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via zeus:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main ZeusAgent app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the ZeusAgent icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see zeus:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep ZeusAgent in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('zeus:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
// window showing only the composer and its scrollback, so ZeusAgent can be driven
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('zeus:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function startHudGameOverlayFeed(win: BrowserWindow) {
  ...
  const push = (state: { active: boolean; app: string }) => {
    if (!win.isDestroyed()) {
      win.webContents.send('zeus:hud:game-overlay', state)
    }
  }
  ...
  const enumerate = async () => {
    ...
    if (!reported) {
      reported = true
      console.warn(`[zeus] HUD cannot enumerate windows: ${windows.reason}`)
    }
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('zeus:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  const win = new BrowserWindow({
    ...
    // `zeus:hud:set-bounds`, which flips resizable on for the call — the
  ...
  // grab, zeus:hud:workspace-transfer temporarily makes the X11 window
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('zeus:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('zeus:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('zeus:quick-entry:shown')
}
...
function createWindow() {
    ...
    callbacks: {
      ...
      onFailedLoadBudgetExhausted: details => {
        ...
        void loadRendererLoadErrorPage(mainWindow, {
          ...
          repairHint: 'zeus desktop --force-build',
  ...
  startZeusAgent().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('zeus:connection', async (_event, profile, extra) => {
...
ipcMain.handle('zeus:connection:for', async (_event, payload) => {
  ...
  // Same single-owner claim as 'zeus:connection', keyed by the composite
...
ipcMain.on('zeus:connection:active-route', (event, route) => {
...
// "Starting ZeusAgent…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('zeus:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: () => resetZeusAgentConnection({ soft: true }),
...
ipcMain.handle('zeus:backend:touch', async (_event, profile) => {
...
ipcMain.handle('zeus:pool-limits:get', async () => ({ ...poolLimits }))
ipcMain.handle('zeus:pool-limits:set', async (_event, raw) => {
...
ipcMain.handle('zeus:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('zeus:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('zeus:window:openInstance', async () => {
  createInstanceWindow()

  return { ok: true }
})
ipcMain.handle('zeus:window:openBrowser', async (_event, tabId) => {
...
ipcMain.handle('zeus:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = resolveZeusAgentBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))

    if (!backend.command) {
      return { ok: false, error: 'ZeusAgent is not installed yet' }
    }
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, ZEUS_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('zeus:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('zeus:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('zeus:zoom:get', event => {
...
ipcMain.on('zeus:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('zeus:backend:recycle', async (_event, profile) => {
...
ipcMain.handle('zeus:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startZeusAgent() call restarts the
...
ipcMain.handle('zeus:bootstrap:repair', async () => {
  // Forceful repair: force the next startZeusAgent() through the full installer
  ...
  // "bypass active runtime" path inside startZeusAgent() and fall through
...
ipcMain.handle('zeus:boot-progress:get', async () => bootProgressState)
ipcMain.handle('zeus:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('zeus:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('zeus:plugin-profile-routes', async (_event, rawProfileNames) => {
...
ipcMain.handle('zeus:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('zeus:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('zeus:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
...
ipcMain.handle('zeus:secret-storage:get', async () => ({ on: secretStoragePolicy().on }))
ipcMain.handle('zeus:secret-storage:set', async (_event: any, on: any) => applySecretStorageEncryption(on === true))
...
ipcMain.handle('zeus:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('zeus:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('zeus:connections:remove', async (_event, id) => {
  const key = String(id || '')
...
ipcMain.handle('zeus:connections:set-primary', async (_event, id) => {
...
ipcMain.handle('zeus:connections:set-launch-mode', async (_event, mode) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('zeus:connections:set-last-used', async (_event, id) => {
  const registry = setLastUsedConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('zeus:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemoteZeusAgentPath: entry.remoteZeusAgentPath
    })
  ...
  if (entry.kind === 'local') {
    const local = await startZeusAgent()
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remoteZeusAgentPath: connection.remoteZeusAgentPath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemoteZeusAgentProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root ZEUS_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('zeus:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// zeus:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
...
ipcMain.handle('zeus:gateway:ws-url-for', async (_event, payload) => {
...
ipcMain.handle('zeus:connections:update-managed', async (_event, rawId) => requestManagedSshUpdate(rawId))

// Fan out `zeus update` to every eligible registered connection at once.
...
ipcMain.handle('zeus:connections:update-all', async (_event, payload) => {
  const registry = readDesktopConnectionsRegistry()
    ...
    registry.connections
      ...
      .map(async connection => {
        ...
        try {
          ...
          const body: any = await postJsonForBackend(descriptor, '/api/zeus/update', {}, { timeoutMs: 15_000 })
...
// analogue of the zeus:api handler's own auth split: OAuth backends prefer a
...
ipcMain.handle('zeus:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('zeus:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startZeusAgent() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('zeus:connection-config:oauth-logout', async (_event, rawUrl) => {
  const baseUrl = normalizeRemoteBaseUrl(rawUrl)
...
// --- ZeusAgent Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('zeus:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('zeus:connection-config:save', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('zeus:connection-config:apply', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('zeus:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
// Persistence-only sibling of zeus:profile:set: records the profile the
...
// the gateway (#79886).
ipcMain.handle('zeus:profile:remember', async (_event, name) => ({
  profile: writeActiveDesktopProfile(name)
}))
ipcMain.handle('zeus:profile:set', async (_event, name) => {
  assertCanMutateManagedPrimaryRouting()
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new ZEUS_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('zeus:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('zeus:requestMicrophoneAccess', async () => {
...
ipcMain.handle('zeus:window:readBelow', async event => {
...
async function handleZeusAgentApiRequest(request) {
  ...
  // backend calls ensure_zeus_home() which recreates the profile directory,
...
ipcMain.handle('zeus:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its ZEUS_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handleZeusAgentApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handleZeusAgentApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('zeus:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('zeus:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('zeus:focus-session', payload.sessionId)
    }

    // Plugin / session-less activation — serializable path (+ optional notifyId
    // for renderer callbacks). Same vocabulary as zeus://index-network/….
    if (payload?.activate || payload?.notifyId) {
      mainWindow.webContents.send('zeus:notification-activate', {
  ...
  notification.on('action', (_actionEvent, index) => {
    ...
    if (payload?.sessionId && !payload?.notifyId && !payload?.activate) {
      mainWindow.webContents.send('zeus:notification-action', { sessionId: payload.sessionId, actionId: action.id })
...
ipcMain.handle('zeus:data-url-read-max:get', () => ({
...
ipcMain.handle('zeus:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('zeus:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('zeus:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('zeus:readFileText', async (_event, filePath) => {
...
// `zeus:readFileText` is the *preview* read and silently truncates at
...
ipcMain.handle('zeus:readPluginSource', async (_event: unknown, filePath: unknown) => {
  const { resolvedPath, stat } = await resolveReadableFileForIpc(filePath, {
...
ipcMain.handle('zeus:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('zeus:writeClipboard', (_event, text) => {
...
ipcMain.handle('zeus:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('zeus:readClipboard', () => clipboard.readText())

ipcMain.handle('zeus:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))

ipcMain.handle('zeus:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))
...
ipcMain.handle('zeus:context-menu:edit', (event, command) => {
...
ipcMain.handle('zeus:context-menu:copy-image', event => {
...
ipcMain.handle('zeus:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('zeus:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.handle('zeus:capturePreview', async (_event, payload) => {
  const guest = electronWebContents.fromId(Number(payload?.webContentsId))

  return capturePreviewContents(guest, payload?.rect, payload?.viewport)
})

ipcMain.handle('zeus:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('zeus:saveClipboardImage', async () => {
...
ipcMain.handle('zeus:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('zeus:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('zeus:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('zeus:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('zeus:active-work', (event, payload) => {
  const id = event.sender.id
...
ipcMain.on('zeus:titlebar-theme', (_event, payload) => {
...
ipcMain.on('zeus:native-theme', (_event, mode) => {
...
ipcMain.on('zeus:translucency:support', event => {
...
ipcMain.on('zeus:keep-awake', (_event, on) => {
...
ipcMain.handle('zeus:quick-entry:settings:get', async () => {
...
ipcMain.handle('zeus:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('zeus:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('zeus:quick-entry:submit', {
...
ipcMain.on('zeus:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('zeus:quick-entry:state', payload)
  }
})

ipcMain.on('zeus:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('zeus:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('zeus:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `zeus:window:openSession`).
...
ipcMain.handle('zeus:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('zeus:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('zeus:preview:reach', async (event, url) => reachablePreviewUrl(event.sender.id, String(url || '')))

ipcMain.handle('zeus:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveZeusAgentCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('zeus:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveZeusAgentCwd()
}))

ipcMain.handle('zeus:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('zeus:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('zeus:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('zeus:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('zeus:resolveFavicon', (_event, url) => resolveFaviconCached(url))

ipcMain.handle('zeus:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('zeus:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: zeusLog.slice(-200) }))
...
ipcMain.on('zeus:logs:renderer-error', (_event, report) => {
...
// Embedded terminal PTY host (zeus:terminal:*) — see terminal-ipc.ts.
...
ipcMain.handle('zeus:updates:check', async () =>
...
ipcMain.handle('zeus:updates:apply', async (_event, payload) =>
...
ipcMain.handle('zeus:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('zeus:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical ZeusAgent version (the one `release.py` bumps in
// zeus_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real ZeusAgent version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveZeusAgentVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'zeus_cli', '__init__.py')
...
// Renderer-bundle skew: `zeus update` moves the SOURCE TREE, but the UI
...
// Re-resolve the live ZeusAgent version and push it into the native About panel
// just before showing it, so an in-place `zeus update` is reflected without
  ...
  void detectRendererSkew().then(skew => {
    app.setAboutPanelOptions({
      ...
      applicationVersion: skew.outOfSync
        ? `${resolveZeusAgentVersion()} — app build out of date, update the desktop app`
        : resolveZeusAgentVersion(),
      copyright: 'Copyright © 2026 Nous Research'
    })
    app.showAboutPanel()
  })
...
ipcMain.handle('zeus:version', async () => {
  const skew = await detectRendererSkew()

  return {
    appVersion: resolveZeusAgentVersion(),
    electronVersion: process.versions.electron,
    nodeVersion: process.versions.node,
    platform: process.platform,
    zeusRoot: resolveUpdateRoot(),
...
// The About page's "Restart ZeusAgent" button (shown when bundleSwapPending):
...
ipcMain.handle('zeus:app:relaunch', async () => {
...
// the actual removal via `zeus uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (zeus_cli/uninstall.py + zeus_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_ZEUS_ROOT
  ...
  const fallback = () => ({
    zeus_home: ZEUS_HOME,
    agent_installed: isZeusAgentSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'zeus_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, ZEUS_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no ZeusAgent agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import zeus_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_ZEUS_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `zeus-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `zeus-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('zeus:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('zeus:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('zeus:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('zeus:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// zeus:// deep links (e.g. zeus://blueprint/morning-brief?time=08:00,
// zeus://mcp/install?name=NAME&config=B64 — the vendor "Add to ZeusAgent"
// button, or zeus://plugin/install?repo=owner/repo). Dev
// (`ZEUS_DESKTOP_DEV_SERVER`) registers zeus-dev:// instead — bare
// Electron or a stale OS handler often owns zeus:// on dev machines.
...
// ---------------------------------------------------------------------------
const ZEUS_PROTOCOL = DEV_SERVER ? 'zeus-dev' : 'zeus'
/** Schemes accepted when parsing inbound URLs (dev accepts both). */
const DEEPLINK_SCHEMES = DEV_SERVER ? ['zeus-dev', 'zeus'] : ['zeus']
...
function handleDeepLink(url) {
  ...
  // zeus://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('zeus:deep-link', payload)
...
ipcMain.handle('zeus:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${ZEUS_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(ZEUS_PROTOCOL, process.execPath, [entry])
    } else {
      app.setAsDefaultProtocolClient(ZEUS_PROTOCOL)
    }

    rememberLog(`[deeplink] registered ${ZEUS_PROTOCOL}:// handler`)
  } catch (err) {
...
// second-instance argv. Without the lock a second `zeus://` launch spawns a
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startZeusAgent), whose
...
app.whenReady().then(() => {
  ...
  // startZeusAgent() finishes resolving the configured backend.
  const primaryProfile = primaryProfileKey()
  ...
  // Win/Linux cold start: the launching zeus:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting ZeusAgent never keeps another app's chord hostage.
