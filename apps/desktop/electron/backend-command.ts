// Backend subcommand routing for the desktop-managed ZeusAgent process.
//
// The desktop app launches its own headless backend via `zeus serve` — it
...
// app hasn't updated yet, or an older `zeus` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional ZeusAgent profile to pin via `--profile`.
 */
 ...
 * `-m zeus_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `zeus_cli/subcommands/dashboard.py` source registers
