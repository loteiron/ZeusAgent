 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `zeus update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.zeusHome, 'ZeusAgent home')
  const zeusPath = validateRemoteValue(target.zeusPath, 'launcher path')
  ...
  const launcherWord = expandRemotePath(zeusPath)

  const updateCommand =
    `env ZEUS_HOME=${homeWord} ` +
    `ZEUS_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'ZEUS_UPDATE_ORIGIN_PROFILE=default ' +
    `ZEUS_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `ZEUS_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:ZEUS_HOME=${psLiteral(home)}`,
    `$env:ZEUS_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:ZEUS_UPDATE_ORIGIN_PROFILE="default"',
    `$env:ZEUS_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:ZEUS_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:ZEUS_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:ZEUS_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:ZEUS_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(zeusPath)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.zeus-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote ZeusAgent updated and every managed SSH profile is ready.'
        : restoreOk
