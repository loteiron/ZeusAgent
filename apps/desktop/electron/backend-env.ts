 ...
 * `iter_zeus_node_dirs()` in zeus_constants.py, which the Electron main
 * process cannot import.
 */
function zeusManagedNodePathEntries(
  zeusHome,
  ...
  const root = pathModule.join(zeusHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const zeusNodeDirs = zeusManagedNodePathEntries(zeusHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([zeusNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeZeusAgentHomeRoot(zeusHome, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!zeusHome) {
    return zeusHome
  }

  const resolved = pathModule.resolve(String(zeusHome))
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). zeus_bootstrap sets this inside the child too, but only
