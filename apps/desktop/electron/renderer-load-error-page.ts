 ...
 *
 * 1. TORN BUNDLE after an update (#95575): `zeus update` replaces the app
...
export interface RendererLoadErrorDetails {
  ...
  /** Repair command hint, e.g. `zeus desktop --force-build`. */
  repairHint?: string
  /**
...
export function buildRendererLoadErrorPage(details: RendererLoadErrorDetails = {}): string {
  ...
  const title = 'ZeusAgent couldn\u2019t start the desktop UI'
  ...
  const repair = details.repairHint ? `<p>Repair with: <code>zeus desktop --force-build</code></p>` : ''
...
<main>
  ...
  <code>zeus desktop --force-build</code>, then restart the app.</p>
