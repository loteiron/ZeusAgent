...
export function resolveDesktopRemoteRoute({
  ...
  if (envUrl) {
    ...
    if (!envToken) {
      throw new Error(
        'ZEUS_DESKTOP_REMOTE_URL is set but ZEUS_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote ZeusAgent backend.'
      )
    }
  ...
  if (!modeIsRemoteLike(config.mode)) {
    ...
    // resolves local and spawns a loopback `zeus serve` the desktop never
