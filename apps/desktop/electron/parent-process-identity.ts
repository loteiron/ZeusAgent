export type ParentWatchdogEnv = {
  ZEUS_PARENT_PID: string
  ZEUS_PARENT_START_MARKER?: string
  ZEUS_PARENT_NONCE?: string
  /** Spawn tag consumed by zeus_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { ZEUS_PARENT_PID: String(pid) }
  env.ZEUS_SPAWN = spawnTag(pid, startMarker)
  ...
  env.ZEUS_PARENT_START_MARKER = startMarker
  env.ZEUS_PARENT_NONCE = nonce

  return env
}

/** Build the `ZEUS_SPAWN` tag mirrored by zeus_cli/process_identity.py.
