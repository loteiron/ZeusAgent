// Profile-delete routing logic for the `zeus:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_zeus_home() and
...
/** Parse a `zeus:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its ZEUS_HOME.
 *
...
/**
 * Route the next `zeus:api` request away from the primary/window backend
 ...
 * ensure_zeus_home() recreates the directory the delete just removed.
 */
