 ...
 *   1. ZEUS_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\zeus\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect ZEUS_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.ZEUS_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'zeus', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'zeus', 'git', 'usr', 'bin', 'bash.exe'))
  }
