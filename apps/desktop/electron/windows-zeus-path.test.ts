// Unit tests for the pure Windows `zeus` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvZeusAgentCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `zeus` shim shadows
//      the real zeus.cmd/zeus.exe.
...
//   3. resolveVenvZeusAgentCommand() — must probe the venv python via
//      canImportZeusAgentCli() before trusting it, or a broken venv gets
...
test('chooseUpdaterArgs: gentle --update when both updater runtime files exist', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvZeusAgent: true, hasVenvPython: true }, 'main'), [
    '--update',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: marker-only install uses --repair when the venv is gone', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvZeusAgent: false, hasVenvPython: false }, 'main'),
    ['--repair', '--branch', 'main']
  )
})

test('chooseUpdaterArgs: partial updater runtimes use --repair', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvZeusAgent: false, hasVenvPython: true }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvZeusAgent: true, hasVenvPython: false }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvZeusAgent: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvZeusAgent: false, hasVenvPython: false }, 'release/1.2'),
    ['--repair', '--branch', 'release/1.2']
  )
})

function makeDeps(overrides: Partial<Parameters<typeof resolveVenvZeusAgentCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportZeusAgentCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    zeusHome: '/fake/zeus-home',
...
test('resolveVenvZeusAgentCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvZeusAgentCommand('/root/venv/Scripts/zeus.exe', [], deps), null)
})

test('resolveVenvZeusAgentCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvZeusAgentCommand('/root/venv/Scripts/zeus.cmd', [], deps), null)
})

test('resolveVenvZeusAgentCommand: returns null when the basename is not zeus/zeus.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvZeusAgentCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvZeusAgentCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvZeusAgentCommand('/root/venv/bin/zeus.exe', [], deps), null)
})

test('resolveVenvZeusAgentCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvZeusAgentCommand('/root/venv/Scripts/zeus.exe', [], deps), null)
})

test('resolveVenvZeusAgentCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportZeusAgentCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvZeusAgentCommand('/root/venv/Scripts/zeus.exe', ['serve'], deps)
...
test('resolveVenvZeusAgentCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvZeusAgentCommand('/root/venv/Scripts/zeus.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'zeus_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvZeusAgentCommand: is case-insensitive on zeus.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvZeusAgentCommand('/root/venv/Scripts/ZEUS.EXE', [], deps))
  assert.ok(resolveVenvZeusAgentCommand('/root/venv/SCRIPTS/zeus.exe', [], deps))
})
