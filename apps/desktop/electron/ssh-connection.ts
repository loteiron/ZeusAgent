...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(ZEUS_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Zeus-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.zeus/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'zeus-desktop-ssh')
  }

  return path.join(os.homedir(), '.zeus', 'desktop-ssh')
}
