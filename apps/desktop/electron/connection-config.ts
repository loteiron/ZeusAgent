 ...
 *     `X-Zeus-Session-Token` header; WS uses `?token=`.
...
// zeus_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - zeus_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - zeus_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / zeus #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-zeus_session_at', '__Secure-zeus_session_at', 'zeus_session_at']
const RT_COOKIE_VARIANTS = ['__Host-zeus_session_rt', '__Secure-zeus_session_rt', 'zeus_session_rt']

// The Nous portal (NAS) does NOT use ZeusAgent gateway session cookies — it is a
...
// Keep this aligned with zeus_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['zeus', 'test', 'tmp', 'root', 'sudo'])
...
/** Which ZeusAgent profile the remote SSH dashboard should actually run as.
...
// ZeusAgent Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteZeusAgentPath = String(entry.remoteZeusAgentPath || '').trim()

  if (remoteZeusAgentPath) {
    out.remoteZeusAgentPath = remoteZeusAgentPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote ZeusAgent installation. Preserve an explicit mapping
  // when it is a valid ZeusAgent profile identifier; otherwise fall back to the
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_zeus_action` registers the (often dynamic, e.g.
 ...
 *     `--profile`, so its `ZEUS_HOME` scopes it.
 *
...
function resolveProfileBackendRoute(profile, opts: ProfileRouteOptions = {}): ProfileBackendRoute {
  ...
  if (scopedProfile === primaryProfile) {
    ...
    // the wire: the dashboard's process ZEUS_HOME can belong to a different
 ...
 * `zeus:api` handler — both routes reach a backend whose namespace is the
...
/**
 * Resolve the two decisions made by the `zeus:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a zeus session ACCESS-token cookie
 ...
 * the ZeusAgent gateway session cookies, so cloud sign-in / discovery liveness
