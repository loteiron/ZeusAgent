...
async function addWorktree(repoPath, options, gitBin) {
  ...
  const branch = sanitizeBranch(opts.branch) || `zeus/${slug}`
...
async function listBranches(repoPath, gitBin) {
  ...
  try {
    // A failed probe must still wait for its sibling to release the cwd on Windows.
    ...
    const locals = names(local.value)
    const localSet = new Set(locals)

    const remotes = names(remote.value).filter(name => {
