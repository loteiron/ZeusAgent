// Launching the ZeusAgent TUI in the user's OWN terminal emulator.
//
...
// `zeus --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's ZeusAgent runtime is often a venv Python invoked
//     as `python -m zeus_cli.main`, not a `zeus` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The ZeusAgent
