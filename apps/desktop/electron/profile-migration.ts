...
export interface MigrationDeps {
  ...
  /** Default profile home (`~/.zeus`). Default's state.db and gateway.pid live here. */
  zeusHome: string
  /** Named-profile root (`~/.zeus/profiles`). Does not contain `default`. */
  profilesRoot: string
  ...
  isZeusAgentProcess: (pid: number) => boolean
  now: () => number
...
/**
 * Production layout: default IS `zeusHome`; named profiles are children of
 * `profilesRoot`. There is no `profiles/default` directory on a normal install.
 */
export function profileStateDbPath(name: string, zeusHome: string, profilesRoot: string): string {
  return name === 'default' ? `${zeusHome}/state.db` : `${profilesRoot}/${name}/state.db`
}

export function profileGatewayPidPath(name: string, zeusHome: string, profilesRoot: string): string {
  return name === 'default' ? `${zeusHome}/gateway.pid` : `${profilesRoot}/${name}/gateway.pid`
}

function resolveZeusAgentHome(profilesRoot: string, zeusHome?: string): string {
  if (zeusHome) {
    return zeusHome
  }

  // Tests that predate zeusHome pass only profilesRoot.
...
/**
 * Return the profile names whose gateway.pid file points to a live zeus process.
 ...
 *
 * `zeusHome` is optional so existing call sites that only pass `profilesRoot`
 * still work: it is derived as the parent of `…/profiles`.
 */
export function findRunningGatewayProfiles(
  profilesRoot: string,
  allProfiles: string[],
  deps: Pick<MigrationDeps, 'existsSync' | 'readFileSync' | 'isZeusAgentProcess'> & { zeusHome?: string }
): string[] {
  const zeusHome = resolveZeusAgentHome(profilesRoot, deps.zeusHome)
  const running: string[] = []

  for (const name of allProfiles) {
    const pidFile = profileGatewayPidPath(name, zeusHome, profilesRoot)
    ...
    if (deps.isZeusAgentProcess(pid)) {
      running.push(name)
    }
  }
...
export function decideMigration(
  ...
  for (const name of candidates) {
    const s = score(profileStateDbPath(name, deps.zeusHome, deps.profilesRoot))
...
/** Default is always a candidate; it is `$ZEUS_HOME`, not `$ZEUS_HOME/profiles/default`. */
...
export function migrateActiveProfileIfMissing(desktopProfileConfigPath: string, deps: MigrationDeps): boolean {
  ...
  // launches `zeus --profile default` and is worse than writing nothing
