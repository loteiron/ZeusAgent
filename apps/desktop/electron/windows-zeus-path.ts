/**
 * windows-zeus-path.ts
 *
 * Pure, dependency-injected pieces of Windows `zeus` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvZeusAgentCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `zeus` shim shadowed the real
 *      zeus.cmd/zeus.exe; the shim then failed the --version probe and
 ...
 *      and ZeusAgent launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvZeusAgentCommand() — unwrapWindowsVenvZeusAgentCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `zeus`) must
 * not shadow `zeus.cmd`/`zeus.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the ZeusAgent launcher that drives `zeus update`.
...
export interface ResolveVenvZeusAgentCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportZeusAgentCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `zeus`/`zeus.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/zeus(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m zeus_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportZeusAgentCli(). A
 * venv whose update died mid-`pip install` still has python.exe + zeus.exe
 ...
 *
 * Returns null when `command` is not a venv zeus shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvZeusAgentCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvZeusAgentCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^zeus(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv ZeusAgent at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing ZeusAgent Python at ${python}`,
    command: python,
