 ...
 *
 * The pre-update handoff kills ZeusAgent-OWNED venv daemons (the memory plugin's
 * hindsight daemon) so the updater never races a mapped shim. External
 * holders (a user terminal running `zeus`, unrelated scripts) must NOT be
...
/**
 * True when a process is a ZeusAgent-owned venv daemon: its exe lives under
 ...
 * spawns DETACHED — it outlives ZeusAgent and holds venv shims mapped).
 */
export function isZeusAgentOwnedVenvDaemon(
