...
test('Windows relaunch gate refuses live and uncertain markers before executing the remote runtime', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = sshWith(async command => {
      ...
      if (script.includes('Get-Command zeus.exe')) {
        return JSON.stringify({
          os: 'Windows',
          arch: 'AMD64',
          zeusHome: 'C:\\Users\\alice\\.zeus',
          zeusPath: 'C:\\ZeusAgent\\zeus.exe',
          python: 'C:\\ZeusAgent\\python.exe'
        })
      }

      if (script.includes('.zeus-update-in-progress')) {
        return observation
      }
    ...
    assert.equal(
      scripts.some(script => script.includes('zeus_cli.windows_ssh_runtime')),
      false
    )
  }
})
...
test('Windows relaunch gate uses strict install-wide marker parsing and fail-closed PID probing', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(ssh, 'C:\\Users\\alice\\.zeus\\profiles\\research')
  assert.match(script, /\.zeus-update-in-progress/)
...
test('Windows probe validates ZeusAgent and Python topology before selection', async () => {
  let script = ''
  ...
  const fallbackJoin = script.indexOf('Join-Path $zeusHome')
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('ZeusAgent is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /ZeusAgent is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\ZeusAgent's\\python.exe" }, 'inspect', [
    'C:\\x y\\zeus.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'zeus_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /ZeusAgent''s/)
  assert.match(script, /C:\\x y\\zeus\.exe/)
})
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { zeusPath: lock.zeusPath, zeusHome: lock.zeusHome }
