...
export function markerPath(zeusHome) {
  return path.join(zeusHome, '.zeus-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(zeusHome)
  let raw
 ...
 *
 * The Tauri-based zeus-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(zeusHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(zeusHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * ZeusAgent to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(zeusHome, opts)
