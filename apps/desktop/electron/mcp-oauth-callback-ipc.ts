 ...
 *   - the browser sees only a minimal "return to ZeusAgent" page.
 */
...
const DONE_HTML =
  ...
  '<p>You can close this window and return to ZeusAgent.</p>' +
...
export function registerMcpOauthCallbackIpc() {
  ...
  ipcMain.handle('zeus:mcp-oauth:listen', async () => {
  ...
  ipcMain.handle('zeus:mcp-oauth:wait', async (_event, id, timeoutMs) => {
  ...
  ipcMain.handle('zeus:mcp-oauth:cancel', (_event, id) => {
    dispose(String(id || ''))
