...
// hardening, ZEUS_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  ipcMain.handle('zeus:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

  ipcMain.handle('zeus:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

  // Reveal a path in the OS file manager (Finder / Explorer / Files).
  ipcMain.handle('zeus:fs:reveal', async (_event, targetPath) => {
  ...
  ipcMain.handle('zeus:fs:openDir', async (_event, dirPath) => {
  ...
  // The LOCAL Desktop runtime-plugin root: `<ZEUS_HOME>/desktop-plugins`,
  // resolved from the main-process ZEUS_HOME (see resolveZeusAgentHome) — NOT from
  // the connected backend. A remote backend reports its own `zeus_home` over
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    // profiles/<name>/, matching the profile-scoped zeus_home the backend
    // reported before this resolver existed. 'default'/unset pins the global root.
    const profile = readActiveDesktopProfile()
    const base = profile && profile !== 'default' ? path.join(zeusHome, 'profiles', profile) : zeusHome
  ...
  ipcMain.handle('zeus:fs:desktopPluginsRoot', async () => localPluginsRoot('desktop-plugins'))

  // The LOCAL logs root (`<ZEUS_HOME>/logs`, profile-aware) — the error
  // card's "Open Logs" action reveals agent.log/gateway.log without the user
  // knowing where ZEUS_HOME lives. Same Electron-local resolution as the
  // plugin roots: valid in every connection mode, created on demand.
  ipcMain.handle('zeus:fs:logsRoot', async () => localPluginsRoot('logs'))

  // The LOCAL agent-plugin root (`<ZEUS_HOME>/plugins`), same Electron-local
  ...
  ipcMain.handle('zeus:fs:agentPluginsRoot', async () => localPluginsRoot('plugins'))

  ipcMain.handle('zeus:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('zeus:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
  ...
  ipcMain.handle('zeus:fs:rename', async (_event, targetPath, newName) => {
  ...
  ipcMain.handle('zeus:fs:writeText', async (_event, filePath, content) => {
  ...
  ipcMain.handle('zeus:fs:trash', async (_event, targetPath) => {
    const target = String(targetPath || '').trim()
