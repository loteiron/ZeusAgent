 ...
 * the three user-facing uninstall modes to the `zeus uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m zeus_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `zeus_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'zeus_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/ZeusAgent.app/Contents/MacOS/ZeusAgent  → …/ZeusAgent.app
 *   Windows: …\ZeusAgent\ZeusAgent.exe                 → …\ZeusAgent  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/ZeusAgent.app/Contents/MacOS/ZeusAgent → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/ZeusAgent.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs ZeusAgent.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]ZeusAgent$/i.test(dir) || /[\\/]zeus-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/zeus
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import zeus_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, zeusHome }) {
 ...
 * modes + `pythonPath`=agentRoot so `import zeus_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but ZeusAgent install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "ZEUS_HOME=${String(zeusHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
