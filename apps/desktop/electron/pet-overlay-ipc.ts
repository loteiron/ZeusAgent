...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('zeus:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('zeus:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('zeus:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('zeus:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('zeus:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('zeus:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('zeus:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('zeus:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('zeus:pet-overlay:control', payload)
  })
}
