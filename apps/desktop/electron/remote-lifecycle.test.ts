...
function fakeSsh(rules: any[] = []) {
  ...
  return {
    ...
    async exec(cmd) {
      ...
      if (cmd.includes('.zeus-update-in-progress') && !cmd.includes('marker_clear()') && !/setsid|nohup/.test(cmd)) {
        return 'CLEAR'
      }
...
test('POSIX relaunch gate refuses live and uncertain install markers without executing ZeusAgent', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = {
      async exec(command) {
        ...
        if (command.includes('ZEUS_HOME')) {
          return '/home/alice/.zeus\n'
        }

        if (command.includes('.zeus-update-in-progress')) {
          return observation
        }
...
test('POSIX relaunch gate permits absent/dead markers and normalizes named-profile homes install-wide', async () => {
  ...
  await assertRemoteInstallUpdateClear(ssh, '/home/alice/.zeus/profiles/research')
  assert.match(commands[0], /home\.parent\.name/)
  assert.match(commands[0], /profiles/)
  assert.match(commands[0], /\.zeus-update-in-progress/)
})
...
test('listRemoteZeusAgentProfiles inventories Mini-style profile dirs without spawning a dashboard', async () => {
  const ssh = fakeSsh([
    [/ZEUS_HOME/, '/Users/zillajr/.zeus\n'],
    [/ls -1/, 'bob\ndixie\ngoose\nrambo\nbob.rollback-old\n']
  ])

  assert.deepEqual(await listRemoteZeusAgentProfiles(ssh), ['default', 'bob', 'dixie', 'goose', 'rambo'])
  assert.equal(
...
test('listRemoteZeusAgentProfiles rejects a hostile ZEUS_HOME', async () => {
  const ssh = fakeSsh([[/ZEUS_HOME/, '/tmp/x; echo pwned\n']])

  await assert.rejects(
    () => listRemoteZeusAgentProfiles(ssh),
    (err: any) => {
...
test('locateZeusAgent prefers the explicit profile path when executable', async () => {
  const ssh = fakeSsh([[/\[ -x .*\/opt\/zeus/, 'OK']])
  assert.equal(await locateZeusAgent(ssh, '/opt/zeus'), '/opt/zeus')
})

test('locateZeusAgent throws (no silent fallback) when an EXPLICIT path is not executable', async () => {
  // command -v WOULD find a different install, but an explicit path must not
  // silently fall back to it — that is the "connected to the wrong zeus" bug.
  const ssh = fakeSsh([
    [/command -v zeus/, '/home/u/.local/bin/zeus\n'],
    [/\[ -x .*\.local\/bin\/zeus/, 'OK']
  ])

  await assert.rejects(
    () => locateZeusAgent(ssh, '/bad/path/zeus'),
    (err: any) => {
      assert.equal(err.kind, 'zeus-not-found')
      assert.match(err.message, /\/bad\/path\/zeus/)
...
test('locateZeusAgent falls back to the login-shell command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v zeus/, '/home/u/.local/bin/zeus\n'],
    [/\[ -x .*\.local\/bin\/zeus/, 'OK']
  ])

  assert.equal(await locateZeusAgent(ssh, ''), '/home/u/.local/bin/zeus')
})

test('locateZeusAgent preserves an installer wrapper instead of resolving its interpreter', async () => {
  // install.sh venv mode writes: exec "$ZEUS_BIN" "$ZEUS_ENTRYPOINT" "$@",
  // where $ZEUS_BIN is the venv python. The old canonicalization returned
  ...
  const ssh = fakeSsh([
    ...
    [/python3 -c/, '/home/u/.zeus/zeus-agent/venv/bin/python\n']
  ])

  assert.equal(await locateZeusAgent(ssh, ''), '/home/u/.local/bin/zeus')
  assert.ok(
    !ssh.calls.some(cmd => cmd.includes('python3 -c')),
    'locateZeusAgent must not shell out to a python3 parser to rewrite the launcher'
  )
})

test('locateZeusAgent returns an explicit remoteZeusAgentPath unchanged', async () => {
  // The override half of #74411: an explicit remoteZeusAgentPath pointing at a
  // wrapper was also canonicalized to its interpreter, so overriding to
  // ~/.local/bin/zeus changed nothing for affected users.
  const ssh = fakeSsh([
    [/\[ -x .*\.local\/bin\/zeus/, 'OK'],
    [/python3 -c/, '/home/u/.zeus/zeus-agent/venv/bin/python\n']
  ])

  assert.equal(await locateZeusAgent(ssh, '~/.local/bin/zeus'), '~/.local/bin/zeus')
  assert.ok(!ssh.calls.some(cmd => cmd.includes('python3 -c')), 'an explicit remoteZeusAgentPath must never be rewritten')
})

test('locateZeusAgent falls back to ~/.local/bin/zeus when the login-shell probe misses', async () => {
  ...
  assert.equal(await locateZeusAgent(ssh, ''), '~/.local/bin/zeus')
})

test('locateZeusAgent tries the conventional venv path last', async () => {
  const ssh = fakeSsh([[/\[ -x .*venv\/bin\/zeus/, 'OK']])
  assert.equal(await locateZeusAgent(ssh, ''), '~/.zeus/zeus-agent/venv/bin/zeus')
})

test('locateZeusAgent throws a zeus-not-found error with an install hint', async () => {
...
test('locateZeusAgent uses a login shell for the command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v zeus/, '/x/zeus'],
    [/\[ -x/, 'OK']
  ])

  await locateZeusAgent(ssh, '')
  assert.ok(
...
test('ownership paths are isolated by ownership ID and spawn nonce', () => {
  assert.equal(ownershipDirectory(OWNERSHIP_ID), `~/.zeus/desktop-ssh/${OWNERSHIP_ID}`)
  assert.equal(lockfilePath(OWNERSHIP_ID), `~/.zeus/desktop-ssh/${OWNERSHIP_ID}/backend.lock.json`)
  assert.equal(spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE), `~/.zeus/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.log`)
})
...
test('metadata and process proof transport failures remain indeterminate', async () => {
  ...
  await assert.rejects(
    () => pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, failure]]), 5, SPAWN_NONCE, '/x/zeus'),
    (error: any) => error.kind === 'transient-transport-error'
  )
})

test('pidIsOurDashboard requires the exact serve ownership nonce', async () => {
  const ours = `/x/zeus serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'OWNED\n']]), 5, SPAWN_NONCE, '/x/zeus'), true)
  assert.equal(
...
test('pidIsOurDashboard accepts the venv entrypoint an installer wrapper execs into', async () => {
  ...
  assert.equal(
    await pidIsOurDashboard(ssh, 5, SPAWN_NONCE, '~/.local/bin/zeus', '/Users/cd9c/.zeus', OWNERSHIP_ID, 'ops'),
    true
  )
  assert.match(ownershipProbe, /zeus-agent.*venv.*bin.*zeus/)
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    const temp = await mkdtemp(path.join(os.tmpdir(), 'zeus wrapper ownership '))
    ...
    const entrypoint = path.join(installDir, 'zeus')
    const launcher = path.join(temp, 'zeus launcher')
    ...
    try {
      ...
      assert.equal(
        await pidIsOurDashboard(ssh, child.pid, SPAWN_NONCE, launcher, '/unrelated/zeus-home', OWNERSHIP_ID, 'ops'),
        true
      )
      assert.equal(
        await pidIsOurDashboard(
...
test('buildSpawnCommand is headless serve, detached, token not in argv', () => {
  const cmd = buildSpawnCommand('/x/zeus', 'work', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  ...
  assert.ok(!cmd.includes('ZEUS_DASHBOARD_SESSION_TOKEN'), 'token env var must not appear')
})
...
test('buildSpawnCommand atomically reserves the ownership slot through spawn and lock publication', () => {
  const cmd = buildSpawnCommand('/x/zeus', 'work', {
    zeusHome: '~/.zeus',
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    ownershipId: OWNERSHIP_ID,
  ...
  assert.ok(cmd.includes('.zeus-update-in-progress.mutex'))
  assert.match(cmd, /fcntl\.flock\(fd,fcntl\.LOCK_EX\)/)
  assert.match(cmd, /os\.O_CLOEXEC/)
  assert.match(
    cmd,
    /subprocess\.run\(\["sh","-c",payload,"zeus-update-mutex",str\(fd\)\],pass_fds=\(fd,\),check=False\)/
  )
  assert.doesNotMatch(cmd, /os\.set_inheritable\(fd,True\)/)
  assert.match(cmd, /zeus-update-child "\$1"/)
...
test.skipIf(process.platform === 'win32')('detached backend does not inherit the update mutex descriptor', async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'zeus-update-mutex-'))
  const zeusPath = path.join(directory, 'zeus')
...
for fd in /proc/$$/fd/*; do
  ...
  case "$target" in
    *zeus-update-in-progress.mutex) printf '%s\\n' "$target" >> ${reportPath} ;;
  esac
done
`,
      { mode: 0o700 }
    ...
    const command = buildSpawnCommand(zeusPath, '', {
      zeusHome: path.join(directory, 'home'),
      logPath
    })
...
test('spawnRemoteDashboard always spawns serve (legacy dashboard path removed)', async () => {
  ...
  await spawnRemoteDashboard(ssh, { zeusPath: '/x/zeus', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID })
...
test('READY_RE accepts both serve and dashboard sentinels', () => {
  assert.equal(READY_RE.exec('ZEUS_BACKEND_READY port=4321')?.[1], '4321')
  assert.equal(READY_RE.exec('ZEUS_DASHBOARD_READY port=8765')?.[1], '8765')
  // The remote log is `>> log 2>&1`, so a stderr chunk without a newline can be spliced onto the sentinel.
  assert.equal(READY_RE.exec('INFO  Started server process [4711]ZEUS_BACKEND_READY port=65238')?.[1], '65238')
})
...
test('spawnRemoteDashboard rejects when no pid is returned', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { zeusPath: '/x/zeus', profile: '', token: 't', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('connect() spawns fresh when there is no lockfile, adopts the served token', async () => {
  const ssh = fakeSsh([
    ...
    [/setsid/, '777\n'],
    [/kill -0 777/, 'ALIVE'],
    [/cat .*\.log/, 'ZEUS_DASHBOARD_READY port=51999\n']
  ])
...
test('managed SSH maps a local scope to a different non-default remote profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '778\n'],
    [/kill -0 778/, 'ALIVE'],
    [/cat .*\.log/, 'ZEUS_BACKEND_READY port=52000\n']
  ])
  ...
  assert.match(spawn, /\.zeus\/desktop-ssh\/[0-9a-f]{32}\/[0-9a-f]{16}\.token/)
...
test('connect() respawns when the requested remote profile differs from the lockfile profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/kill 333/, ''],
    [/--version/, 'ZeusAgent v0.18.2\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/setsid/, '890\n'],
    [/kill -0 890/, 'ALIVE'],
    [/cat .*\.log/, 'ZEUS_DASHBOARD_READY port=52050\n']
  ])
...
test('connect() respawns when the lockfile zeusPath differs from the resolved path', async () => {
  const reuseToken = 'stored-token'
  const lock = ownedLock({ zeusPath: '/old/stale/zeus', tokenFingerprint: fingerprintToken(reuseToken) })
  ...
  const result = await connect(
    connectDeps(ssh, { reuseToken, remoteZeusAgentPath: '/new/zeus', adoptServedToken: async () => 'fresh' })
  )
...
test('connect() respawns when the lockfile protocolVersion is incompatible', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '901\n'],
    [/kill -0 901/, 'ALIVE'],
    [/cat .*\.log/, 'ZEUS_DASHBOARD_READY port=44100\n']
  ])
...
test('connect() fresh spawn writes zeusHome + protocolVersion into the lockfile', async () => {
  const writes: string[] = []
  ...
  const ssh = fakeSsh([
    ...
    [/\[ -x/, 'OK'],
    [/cat .*lock\.json/, ''], // no lockfile
    [/ZEUS_HOME/, '/home/alice/.zeus\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/printf '%s\\n'/, ''],
    [/setsid/, '700\n'],
    [/kill -0 700/, 'ALIVE'],
    [/cat .*\.log/, 'ZEUS_DASHBOARD_READY port=45500\n'],
    [
      /printf '%s' '/,
      c => {
        writes.push(c)
  ...
  assert.match(lockWrite, /"zeusHome":"\/home\/alice\/\.zeus"/)
})
...
test('connect() respawns when the lockfile pid is dead (killed dashboard)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '888\n'],
    [/kill -0 888/, 'ALIVE'],
    [/cat .*\.log/, 'ZEUS_DASHBOARD_READY port=42000\n']
  ])
...
test('connect() respawns when the dashboard is wedged (alive pid, probe fails)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '999\n'],
    [/kill -0 999/, 'ALIVE'],
    [/cat .*\.log/, 'ZEUS_DASHBOARD_READY port=43000\n']
  ])
...
test('validateRemotePath accepts absolute POSIX paths', () => {
  assert.doesNotThrow(() => validateRemotePath('/usr/bin/zeus'))
  assert.doesNotThrow(() => validateRemotePath('/home/user/.zeus/zeus-agent/venv/bin/zeus'))
})

test('validateRemotePath accepts ~/ prefix paths', () => {
  assert.doesNotThrow(() => validateRemotePath('~/bin/zeus'))
  assert.doesNotThrow(() => validateRemotePath('~/.zeus/logs/desktop-ssh.log'))
  assert.doesNotThrow(() => validateRemotePath('~'))
})

test('validateRemotePath accepts paths with spaces and quotes', () => {
  assert.doesNotThrow(() => validateRemotePath('/home/user/my project/zeus'))
...
test('validateRemotePath rejects relative paths', () => {
  assert.throws(() => validateRemotePath('zeus'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('./bin/zeus'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('../etc/passwd'), /absolute|relative/i)
})

test('validateRemotePath rejects NUL and newline', () => {
  assert.throws(() => validateRemotePath('/usr/bin/zeus\x00'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/zeus\n'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/zeus\r'), /unsafe/i)
})

test('validateRemotePath preserves shell metacharacters as path data', () => {
  for (const p of ['/usr/$(whoami)/zeus', '/usr/`id`/zeus', '/usr/a;b|c&d<e>f']) {
...
test('expandRemotePath expands ~/ to "$HOME"/', () => {
  const result = expandRemotePath('~/.zeus/logs/desktop-ssh.log')
...
test('expandRemotePath returns quoted absolute paths unchanged', () => {
  const result = expandRemotePath('/usr/local/bin/zeus')
  assert.ok(result.includes('/usr/local/bin/zeus'))
  assert.ok(!result.includes('eval'))
})

test('expandRemotePath preserves spaces as data', () => {
  const result = expandRemotePath('/home/user/my project/zeus')
...
test('buildSpawnCommand does not embed the token in the command string', () => {
  ...
  assert.ok(!cmd.includes('ZEUS_DASHBOARD_SESSION_TOKEN'), 'env var name must not appear')
})

test('buildSpawnCommand includes --ssh-session-token-file when tokenFilePath is provided', () => {
  const cmd = buildSpawnCommand('/x/zeus', 'work', {
    tokenFilePath: `~/.zeus/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.token`,
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    spawnNonce: SPAWN_NONCE
  })

  assert.match(cmd, /--ssh-session-token-file/)
  assert.match(cmd, /\.zeus\/desktop-ssh\//)
})

test('buildSpawnCommand always uses serve, never dashboard', () => {
  const cmd = buildSpawnCommand('/x/zeus', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /serve --isolated/)
  assert.doesNotMatch(cmd, /\bdashboard\b/)
  assert.doesNotMatch(cmd, /--skip-build/)
...
test('buildSpawnCommand raises the SSH child file limit before execing ZeusAgent', () => {
  const cmd = buildSpawnCommand('/x/zeus', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /ulimit -n 65536 2>\/dev\/null \|\| true; exec env ZEUS_DESKTOP=1/)
...
test('spawnRemoteDashboard removes a token file when upload reporting fails', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { zeusPath: '/x/zeus', profile: '', token: 'tok', ownershipId: OWNERSHIP_ID }),
    /channel closed/
  )
...
test('spawnRemoteDashboard fails with update-required when remote lacks --ssh-session-token-file', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { zeusPath: '/x/zeus', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('readLockfile treats a log path outside the exact ownership and spawn path as skew', async () => {
  const lock = ownedLock({ logPath: '~/.zeus/desktop-ssh/other.log' })
...
test('cleanupStale never deletes a lock-supplied unexpected log path', async () => {
  ...
  await cleanupStale(ssh, OWNERSHIP_ID, ownedLock({ logPath: '~/.zeus/unrelated.log' }))
...
test('pidIsOurDashboard requires an exact nonce option value', async () => {
  const prefix = `/x/zeus serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}ff`
  const suffix = `/x/zeus serve --isolated --ssh-owner-nonce xx${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/zeus'), false)
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/zeus'), false)
})
...
test('remote SSH ownership capability requires both secure bootstrap flags', async () => {
  ...
  assert.equal(await remoteSupportsSshOwnership(supported, '/x/zeus'), true)
  ...
  assert.equal(await remoteSupportsSshOwnership(unsupported, '/x/zeus'), false)
})
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    ...
    const root = await mkdtemp(path.join(os.tmpdir(), 'zeus-argv-shim-'))
    ...
    try {
      ...
      assert.equal(
        argv[2],
        `${fakeHome}/.zeus/.zeus-update-in-progress.mutex`,
      ...
      const base = `${fakeHome}/.zeus/desktop-ssh/${OWNERSHIP_ID}`
