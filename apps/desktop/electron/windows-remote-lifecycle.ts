...
const READY_RE = /^ZEUS_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitZeusAgentPath = '') {
  const explicit = psLiteral(explicitZeusAgentPath)
  ...
  const script = [
    ...
    '$zeusHome=$env:ZEUS_HOME',
    'if(-not $zeusHome){$zeusHome=Join-Path $env:LOCALAPPDATA "zeus"}',
    'Assert-NoReparse $zeusHome $true',
    '$candidate=[IO.Path]::Combine($zeusHome, "zeus-agent\\venv\\Scripts\\zeus.exe")',
    ...
    '$profileCandidate=[IO.Path]::Combine($HOME, "zeus-agent\\.venv\\Scripts\\zeus.exe")',
    ...
    '$fallbackHomeCandidate=Join-Path $zeusHome "zeus-agent\\venv\\Scripts\\zeus.exe"',
    '$fallbackProfileCandidate=Join-Path $HOME "zeus-agent\\.venv\\Scripts\\zeus.exe"',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command zeus.exe -ErrorAction SilentlyContinue',
    ...
    '$zeus=$null',
    'foreach($candidate in $candidates){Assert-NoReparse $candidate $true;$candidatePython=[IO.Path]::Combine([IO.Path]::GetDirectoryName($candidate), "python.exe");Assert-NoReparse $candidatePython $true;try{$item=Get-Item -LiteralPath $candidate -Force -ErrorAction Stop;if(($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0 -and -not $item.PSIsContainer){$zeus=$item.FullName;break}}catch [Management.Automation.ItemNotFoundException]{continue}}',
    'if(-not $zeus){throw "ZeusAgent is not installed on the remote Windows host."}',
    'Assert-NoReparse $zeus $false',
    'if($explicit -and $zeus -ne $explicit){throw "The configured ZeusAgent path is not an executable file."}',
    '$python=[IO.Path]::Combine([IO.Path]::GetDirectoryName($zeus), "python.exe")',
    'Assert-NoReparse $python $false',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;zeusHome=$zeusHome;zeusPath=$zeus;python=$python}|ConvertTo-Json -Compress'
  ].join(';')

  return JSON.parse((await ssh.exec(powerShellCommand(script))).trim())
}

function windowsUpdateMarkerProbeCommand(zeusHome) {
  const script = [
    '$ErrorActionPreference="Stop"',
...
public static class ZeusAgentMarkerNoFollow {
...
`,
    ...
    '}',
    '}',
    `$home=${psLiteral(zeusHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".zeus-update-in-progress"',
    '$result="UNCERTAIN"',
    '$stream=$null;$memory=$null',
    'try{',
    'Assert-NoReparse $marker $true',
    'if(-not (Test-Path -LiteralPath $marker -PathType Leaf)){$result="CLEAR"}else{$stream=[ZeusAgentMarkerNoFollow]::OpenRead($marker)',
 ...
 */
async function assertWindowsRemoteInstallUpdateClear(ssh, zeusHome) {
  let observation = ''

  try {
    observation =
      String(await ssh.exec(windowsUpdateMarkerProbeCommand(zeusHome)))
        .replace(/^\uFEFF/, '')
        .trim()
        .split(/\r?\n/)
        .pop() || ''
...
async function detectRemotePlatform(ssh, explicitZeusAgentPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitZeusAgentPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'zeus_cli.windows_ssh_runtime', operation, ...args]
...
function atomicWindowsSpawnCommand(runtime, reservation: any = {}) {
  const argv = [runtime.python, '-m', 'zeus_cli.windows_ssh_runtime', 'spawn']
  const helper = operation => [runtime.python, '-m', 'zeus_cli.windows_ssh_runtime', operation]

  const script = [
    '$ErrorActionPreference="Stop"',
    `$home=${psLiteral(runtime.zeusHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".zeus-update-in-progress"',
    ...
    reservation.ownershipId
      ...
      : '  if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}',
    reservation.ownershipId
      ? `  $spawned=$spawnLines[-1]|ConvertFrom-Json; $lock=[ordered]@{schemaVersion=2;protocolVersion=1;ownershipId=${psLiteral(reservation.ownershipId)};spawnNonce=${psLiteral(reservation.spawnNonce)};pid=[int]$spawned.pid;creationTimeNs=[string]$spawned.creationTimeNs;port=0;profile=${psLiteral(reservation.profile)};zeusPath=${psLiteral(reservation.zeusPath)};zeusHome=${psLiteral(reservation.zeusHome)};tokenFingerprint=${psLiteral(reservation.tokenFingerprint)};startedAt=${psLiteral(reservation.startedAt)}}|ConvertTo-Json -Compress; ` +
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.zeusPath === 'string' &&
    typeof lock.zeusHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.zeusPath === runtime.zeusPath &&
    lock.zeusHome === runtime.zeusHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteZeusAgentPath)
  await assertWindowsRemoteInstallUpdateClear(ssh, runtime.zeusHome)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.zeusPath])

  if (!inspection.supported) {
    const error: any = new Error('Update ZeusAgent on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.zeusPath = inspection.path
  const zeusVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located zeus at ${runtime.zeusPath}`)
  ...
  try {
    ...
    spawned = await atomicWindowsSpawn(
      ssh,
      runtime,
      JSON.stringify({ ownershipId, spawnNonce, profile, zeusPath: runtime.zeusPath }),
      {
        ownershipId,
        spawnNonce,
        profile,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForZeusAgent(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="ZeusAgent SSH"', 'powershell.exe -NoLogo')
