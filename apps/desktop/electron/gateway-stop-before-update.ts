 ...
 *
 * So we delegate to `zeus gateway stop --all`: the CLI discovers every
 ...
 * force-kills survivors — the same logic `zeus update`'s
...
export function stopGatewayBeforeUpdate(
  ...
  return runGatewayLifecycleCommand(zeusCliPath, ['gateway', 'stop', '--all'], deps)
}
 ...
 */
export function startGatewaysAfterUpdateAbort(zeusCliPath: string, deps: StopGatewayBeforeUpdateDeps = {}): boolean {
  return runGatewayLifecycleCommand(zeusCliPath, ['gateway', 'start', '--all'], deps)
}

function runGatewayLifecycleCommand(zeusCliPath: string, args: string[], deps: StopGatewayBeforeUpdateDeps): boolean {
  ...
  if (deps.spy) {
    deps.spy(zeusCliPath, args)
  }

  if (!existsSync(zeusCliPath)) {
    return false
  }

  try {
    exec(zeusCliPath, args, {
