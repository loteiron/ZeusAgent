...
export interface ZeusAgentReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForZeusAgentReady`).
   */
...
export const REMOTE_UNSIGNED_OAUTH_MESSAGE =
  'Remote ZeusAgent gateway uses OAuth, but you are not signed in. ' +
 ...
 *  - readiness-probe exhaustion in waitForZeusAgentReady().
 *
...
/**
 * True when the backend URL points at a Nous-managed ZeusAgent Cloud instance
 ...
 * `isReauthRequired` so startZeusAgent latches; `needsOauthLogin` alone only
...
export async function waitForZeusAgentReady(baseUrl: string, options: ZeusAgentReadyOptions): Promise<void> {
  ...
  throw new Error(`ZeusAgent backend did not become ready: ${detail}`)
}
