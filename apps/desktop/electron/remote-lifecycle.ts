 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote ZeusAgent dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the ZeusAgent install on the remote (login-shell probe),
 ...
 *     `ZEUS_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForZeusAgent in.
 *
 * The minted ZEUS_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const REMOTE_LOCK_DIR = '~/.zeus/desktop-ssh'
...
// Resolve the remote zeus executable. An EXPLICIT path is honored strictly
...
async function locateZeusAgent(ssh, remoteZeusAgentPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The zeus binary or wrapper script
  ...
  if (remoteZeusAgentPath) {
    if (await isExecutable(remoteZeusAgentPath)) {
      return resolveLauncher(remoteZeusAgentPath)
    }

    const err: any = new Error(
      `The ZeusAgent path you set is not an executable on the remote host: "${remoteZeusAgentPath}". ` +
        'Check the path (it must be the full path to the `zeus` binary on the remote, e.g. ' +
        '~/zeus-agent/.venv/bin/zeus), or clear it to auto-detect.'
    )

    err.kind = 'zeus-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v zeus')}`)).trim()
  ...
  candidates.push('~/.local/bin/zeus')
  candidates.push('/usr/local/bin/zeus')
  candidates.push('~/.zeus/zeus-agent/venv/bin/zeus')
  ...
  const err: any = new Error(
    'ZeusAgent is not installed on the remote host (could not find a `zeus` executable). ' +
      'Install it on the remote with:  curl -fsSL https://hermes-agent.nousresearch.com/install.sh | sh  ' +
      '— or set the ZeusAgent path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<zeus> --version`,
// e.g. "ZeusAgent v0.18.2 ..."), or '' on failure. Surfaces WHICH zeus a
// connection uses, so a stale/unexpected install is visible.
async function probeZeusAgentVersion(ssh, zeusPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(zeusPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". ZeusAgent Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The ZEUS_HOME the remote dashboard will use (explicit env wins, else
// ~/.zeus). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteZeusAgentHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${ZEUS_HOME:-$HOME/.zeus}"')).trim().split('\n').pop()

    return out || '~/.zeus'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote ZeusAgent home.')
...
marker=home/'.zeus-update-in-progress'
try:
 ...
 * bytes; it never imports or executes code from the changing ZeusAgent checkout.
 ...
 */
async function assertRemoteInstallUpdateClear(ssh, zeusHome) {
  const home = assertSafeRemoteHome(zeusHome)
  let observation = ''
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote ZeusAgent install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote ZeusAgent update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote ZeusAgent update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemoteZeusAgentProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteZeusAgentHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote ZeusAgent profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote ZeusAgent home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'zeusPath', 'zeusHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !zeusPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(zeusPath)})\n` +
      // The installer-facing launcher is intentionally preserved for invocation
      // (#74411), but it may `exec python <install-dir>/zeus`, leaving neither
      // launcher nor ZEUS_HOME-derived entrypoint in argv. The ownership-scoped
      // token path + random nonce + exact profile below are the alternative proof.
      `zeus_home=os.path.expanduser(${shq(zeusHome)}) if ${shq(zeusHome)} else ""\n` +
      'expected_entries={expected}\n' +
      'if zeus_home:\n' +
      ' expected_entries.add(os.path.join(zeus_home,"zeus-agent","venv","bin","zeus"))\n' +
...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedPath = expandRemotePath(lock.zeusPath)
  const expectedHome = lock.zeusHome ? expandRemotePath(lock.zeusHome) : "''"
  ...
  const executableMatch = lock.zeusHome
...
function lockMatchesManagedUpdateScope(lock, expected) {
  return Boolean(
    ...
    lock.zeusPath === expected.zeusPath &&
    lock.zeusHome === expected.zeusHome
  )
}
...
expected_path=os.path.expanduser(${py(lock.zeusPath)})
zeus_home=os.path.expanduser(${py(lock.zeusHome)})
expected_entries={expected_path,os.path.join(zeus_home,"zeus-agent","venv","bin","zeus")}
...
// execing ZeusAgent. mutexPath is expandRemotePath() output — a complete shell
...
result=None
try:
 result=subprocess.run(["sh","-c",payload,"zeus-update-mutex",str(fd)],pass_fds=(fd,),check=False)
finally:
 os.close(fd)
...
function buildSpawnCommand(zeusPath, profile, opts: any = {}) {
  const zeus = expandRemotePath(zeusPath)
  ...
  const marker = expandRemotePath(`${remoteInstallRoot(opts.zeusHome || '~/.zeus')}/.zeus-update-in-progress`)

  const updateMutex = expandRemotePath(
    `${remoteInstallRoot(opts.zeusHome || '~/.zeus')}/.zeus-update-in-progress.mutex`
  )
  ...
  const dashCmd =
    ...
    `exec env ZEUS_DESKTOP=1 ${zeus} ${profileArgs}${subCmd}`

  const detachedShell = `eval "exec $1>&-"; ${dashCmd} </dev/null >> ${logPath} 2>&1 & echo $!`
  const detachedSpawn = `child=$("$(command -v setsid || echo nohup)" sh -c ${shq(detachedShell)} zeus-update-child "$1" & echo $!)`
...
async function remoteSupportsSshOwnership(ssh, zeusPath) {
  const zeus = expandRemotePath(zeusPath)

  const out = await ssh.exec(
    `help="$(${zeus} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(
  ssh,
  { zeusPath, profile, token, ownershipId, zeusHome = '~/.zeus', assertInstallClear = async () => {} }
) {
  if (!(await remoteSupportsSshOwnership(ssh, zeusPath))) {
    const err: any = new Error(
      'The remote ZeusAgent install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update ZeusAgent on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    ...
    out = await ssh.exec(
      buildSpawnCommand(zeusPath, profile, {
        spawnNonce,
        tokenFilePath,
        logPath,
        zeusHome,
 ...
 * opened SshConnection, forward/pickLocalPort/waitForZeusAgent, a token-gated
...
async function connect(deps) {
  ...
  const zeusHome = await probeRemoteZeusAgentHome(ssh)
  await assertRemoteInstallUpdateClear(ssh, zeusHome)
  const zeusPath = await locateZeusAgent(ssh, remoteZeusAgentPath)
  log(`located zeus at ${zeusPath}`)
  const zeusVersion = await probeZeusAgentVersion(ssh, zeusPath)

  if (zeusVersion) {
    log(`remote zeus version: ${zeusVersion}`)
  }
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this ZeusAgent Desktop build (${lock.reason}). ` +
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.zeusPath === zeusPath &&
      lock.zeusHome === zeusHome
  ...
  const spawned = await spawnRemoteDashboard(ssh, {
    ...
    ownershipId,
    zeusHome,
    assertInstallClear: () => assertRemoteInstallUpdateClear(ssh, zeusHome)
  })
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForZeusAgent(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)
...
export {
  ...
  ownershipDirectory,
  pidIsOurDashboard,
  probeZeusAgentVersion,
  probeRemoteZeusAgentHome,
  probeRemotePlatform,
  PROTOCOL_VERSION,
  readLockfile,
  READY_RE,
