...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'zeus serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/zeus serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m zeus_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/zeus --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\ZeusAgent Runtime\\zeus.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('zeus chat --query serve'), false)
