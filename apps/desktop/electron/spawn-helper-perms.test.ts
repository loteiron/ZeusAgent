...
test('rewrites an archived node-pty root to the matching unpacked tree exactly once', () => {
  assert.equal(
    writableNodePtyRoot('/ZeusAgent.app/Contents/Resources/app.asar/dist/node_modules/node-pty'),
    '/ZeusAgent.app/Contents/Resources/app.asar.unpacked/dist/node_modules/node-pty'
  )
  assert.equal(
    writableNodePtyRoot('/ZeusAgent.app/Contents/Resources/app.asar.unpacked/dist/node_modules/node-pty'),
    '/ZeusAgent.app/Contents/Resources/app.asar.unpacked/dist/node_modules/node-pty'
  )
})

test('uses the unpacked helper when resolution reports an app.asar node-pty root', () => {
  const archivedRoot = '/ZeusAgent.app/Contents/Resources/app.asar/dist/node_modules/node-pty'
