...
test('parseWorktrees: main checkout + linked worktree', () => {
  ...
  assert.equal(trees[1].branch, 'zeus/feat')
})
...
test('ensureGitRepo: inits a plain dir with a root commit so worktrees branch', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zeus-wt-'))
...
test('switchBranch: switches a normal checkout branch', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zeus-switch-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('listBranches: lists locals and flags the checked-out branch', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zeus-branches-'))
...
test('listBranches: flags a free default branch as default, not checked out', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zeus-branches-default-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('listBranches: a branch claimed by a worktree is flagged checked out', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zeus-branches-wt-'))
...
test('listBranches: empty on a non-repo path', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zeus-nonrepo-'))
...
test.each(['refs/heads', 'refs/remotes'])(
  'listBranches: waits for both Git children when %s fails first',
  async failedRef => {
    const pending = new Map<string, (error: Error | null, stdout: string, stderr: string) => void>()

    const exec = vi.mocked(execFile).mockImplementation(((_command, args, _options, callback) => {
      pending.set(args.at(-1), callback)
    ...
    const result = listBranches(os.tmpdir(), 'git').then(branches => {
      returned = true
    ...
    try {
      assert.deepEqual([...pending.keys()].sort(), ['refs/heads', 'refs/remotes'])
      pending.get(failedRef)(new Error('not a git repository'), '', '')
      pending.delete(failedRef)
      // Drain the promise reactions from the first child without releasing the sibling.
      await new Promise<void>(resolve => setImmediate(resolve))
      assert.equal(returned, false, 'branch lookup must retain ownership of its unfinished Git child')
    } finally {
      for (const callback of pending.values()) {
        callback(null, '', '')
      }

      assert.deepEqual(await result.finally(() => exec.mockRestore()), [])
    }
  }
)

test('addWorktree: existingBranch checks the branch out without a new branch', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zeus-convert-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('addWorktree: existing default branch switches the main checkout, not .worktrees/main', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zeus-convert-default-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('listBaseBranches: lists local branches and flags the default', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zeus-base-branches-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('listBaseBranches: empty on a non-repo path', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zeus-base-nonrepo-'))
...
test('addWorktree: base param branches off a specified local branch', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zeus-base-add-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('addWorktree: base origin/main does not set up upstream tracking', async () => {
  ...
  const remoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zeus-remote-'))
  const cloneDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zeus-clone-'))
...
function seedRemoteAndClone(label, branches) {
  const remoteDir = fs.mkdtempSync(path.join(os.tmpdir(), `zeus-${label}-remote-`))
  const cloneDir = fs.mkdtempSync(path.join(os.tmpdir(), `zeus-${label}-clone-`))
  ...
  execFileSync('git', ['init', '-b', 'main', remoteDir])
  remoteGit('-c', 'user.email=zeus@localhost', '-c', 'user.name=ZeusAgent', 'commit', '--allow-empty', '-m', 'root')
...
test('switchBranch: non-repo dir short-circuits instead of throwing', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zeus-sw-'))
