 ...
 *
 * The Python side (zeus_cli/plugin_compat.py) statically scans the user's enabled external plugins on
 * every CLI/gateway/TUI start and writes ZEUS_HOME/.plugin-compat-report.json when any plugin imports a
...
export function readReport(zeusHome: string): PluginCompatReport | null {
  try {
    const raw = fs.readFileSync(path.join(zeusHome, REPORT_FILE), 'utf8')
...
export function pendingNotice(zeusHome: string, userData: string): PendingNotice | null {
  const report = readReport(zeusHome)
