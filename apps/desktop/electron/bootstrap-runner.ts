 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of ZeusAgent by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.zeus-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(zeusHome) {
  return path.join(zeusHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.zeus/zeus-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(zeusHome) {
  if (!zeusHome) {
    return null
  }

  const candidate = path.join(zeusHome, 'zeus-agent', 'scripts', installScriptName())
...
function cachedScriptPath(zeusHome, commit) {
  return path.join(bootstrapCacheDir(zeusHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}

function downloadInstallScript(ref, destPath) {
  if (!REMOTE_RELEASES_AVAILABLE) {
    return Promise.reject(new Error(SOURCE_UPDATE_MESSAGE))
  }
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(zeusHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(zeusHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, zeusHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass ZEUS_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          ZEUS_HOME: zeusHome || process.env.ZEUS_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, zeusHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, zeusHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--zeus-home', zeusHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, zeusHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, zeusHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, zeusHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(zeusHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, zeusHome, emit })
