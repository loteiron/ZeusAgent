...
export function registerHudIpc({
  ...
  ipcMain.on('zeus:hud:native-drag', event => {
    event.returnValue = hudWindowing().move === 'native-drag'
  })

  ipcMain.on('zeus:hud:windowing', event => {
  ...
  ipcMain.on('zeus:hud:workspace-transfer', (event, transferring) => {
  ...
  ipcMain.handle('zeus:hud:open', async (_event, request) => {
    openHudWindow(
  ...
  ipcMain.handle('zeus:hud:frost', (_event, showing) => {
  ...
  // reaches the bar.
  ipcMain.on('zeus:hud:ignore-mouse', (_event, ignore) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('zeus:hud:begin-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('zeus:hud:end-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('zeus:hud:move-by', (event, delta) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('zeus:hud:set-bounds', (event, bounds) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('zeus:hud:reset-layout', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('zeus:hud:session', (event, sessionId) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('zeus:hud:close', async () => {
    closeHudWindow()
