 ...
 * runtime, remote gateways, ZeusAgent Cloud instances, SSH hosts) that are all
 ...
 * apps/shared/src/backend-scope.ts (`@zeus/shared`) — tsconfig project
...
export async function reuseMatchingPrimarySshBackend({
  ...
  if (
    ...
    String(source.remoteZeusAgentPath || '').trim() !== String(activeSsh.remoteZeusAgentPath || '').trim() ||
...
/** Turn `ls ~/.zeus/profiles` output into roster names. Always includes
...
/**
 * Whether "Update all instances" may drive this connection. ZeusAgent Cloud
 * instances are platform-managed — we never run `zeus update` against them.
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteZeusAgentPath: input.remoteZeusAgentPath,
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/ZeusAgent-managed names; an empty result stores nothing.
 ...
 * `remoteZeusAgentPath`/`remoteProfile`. Only fields the payload explicitly
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'ZeusAgent Cloud' : 'Remote gateway'),
