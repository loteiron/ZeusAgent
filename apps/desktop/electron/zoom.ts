...
export const ZOOM_STORAGE_KEY = 'zeus:desktop:zoomLevel'
 ...
 * but forgot to emit 'zeus:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('zeus:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
 ...
 * only learns of changes through `zeus:zoom:changed`, which never fired —
