...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'zeus-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/zeus-runner-test',
    sourceRepoRoot: null,
    zeusHome: '/tmp/zeus-runner-test',
    logRoot: '/tmp/zeus-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'zeus-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'zeus-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/zeus-agent',
      zeusHome: '/tmp/zeus'
    }),
    ['--dir', '/tmp/zeus-agent', '--zeus-home', '/tmp/zeus', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/zeus-agent', '--zeus-home', '/tmp/zeus', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/zeus', '--zeus-home', '/tmp/home', '--branch', 'main']
  )
})
