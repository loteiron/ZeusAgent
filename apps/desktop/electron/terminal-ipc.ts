...
// registry, and the zeus:terminal:* IPC surface. Extracted from main.ts; the
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    // ZEUS_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
    ...
    const override = (process.env.ZEUS_DESKTOP_SHELL || (isWindows ? '' : process.env.SHELL) || '').trim()
  ...
  function terminalShellEnv() {
    ...
    // /TERM=dumb; some terminals set COLORFGBG which would flip ZeusAgent' TUI into
    ...
    env.TERM_PROGRAM = 'ZeusAgent'
    env.TERM_PROGRAM_VERSION = app.getVersion()

    // Let a zeus/--tui launched in this pane know it's embedded in the desktop
    // GUI (build_environment_hints surfaces this). Distinct from ZEUS_DESKTOP,
    // which marks the agent *backend* and gates cron/gateway behavior.
    env.ZEUS_DESKTOP_TERMINAL = '1'

    return env
  }

  function terminalChannel(id, suffix) {
    return `zeus:terminal:${id}:${suffix}`
  }
  ...
  ipcMain.handle('zeus:terminal:start', async (event, payload = {}) => {
  ...
  ipcMain.handle('zeus:terminal:attach', (event, id) => {
  ...
  ipcMain.handle('zeus:terminal:write', (_event, id, data) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('zeus:terminal:resize', (_event, id, size = {}) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('zeus:terminal:cwd', async (_event, id) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('zeus:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))
