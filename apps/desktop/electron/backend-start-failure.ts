 ...
 * startZeusAgent() re-throw the cached error without re-attempting the connect —
...
/**
 * Whether a startZeusAgent() failure should latch into `backendStartFailure`.
 ...
 * Every subsequent `getConnection`/`api` call re-runs `startZeusAgent`, re-emits
