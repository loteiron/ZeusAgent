...
//   ZEUS_WIN_SSH_HOST   ssh alias/host of the Windows box
//   ZEUS_WIN_SSH_USER   remote user
//   ZEUS_WIN_SSH_ZEUS absolute path to the remote zeus.exe under test
const liveHost = process.env.ZEUS_WIN_SSH_HOST || ''
const liveUser = process.env.ZEUS_WIN_SSH_USER || ''
const configuredZeusAgent = process.env.ZEUS_WIN_SSH_ZEUS || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Zeus-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredZeusAgent)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredZeusAgent.replace('zeus.exe', 'python.exe')}' -m zeus_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredZeusAgent.replace('zeus.exe', 'python.exe')
        const terminate = `& '${python}' -m zeus_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.zeusPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m zeus_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m zeus_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
