...
// `zeus serve` announces ZEUS_BACKEND_READY; the legacy `zeus dashboard`
// backend announces ZEUS_DASHBOARD_READY. Accept either so the desktop spawn
// works against both the headless backend and old/dashboard runtimes.
const _READY_RE = /^ZEUS_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
...
// spliced onto them (`...process [4711]ZEUS_BACKEND_READY port=65238`) and `^` never lines up
// (#103792). Match on a token boundary instead; `port=<digits>` keeps prose mentions out.
export const READY_IN_MERGED_OUTPUT_RE = /(?<!\w)ZEUS_(?:BACKEND|DASHBOARD)_READY port=(\d+)/

// The announcement clock starts the instant the backend process is spawned —
// before uvicorn binds its socket. On a cold install the child must first
// compile and import the whole `zeus_cli.main` → `web_server` → FastAPI/
 ...
 * ZEUS_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS env override (for users on slow
...
function resolvePortAnnounceTimeoutMs(env = process.env) {
  const parsed = Number(env.ZEUS_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS)
...
/**
 * Watch a child process's stdout for the `ZEUS_(BACKEND|DASHBOARD)_READY
...
function waitForDashboardPort(
  ...
  return new Promise((resolve, reject) => {
    ...
    function onExit(code, signal) {
      cleanup()
      reject(new Error(`ZeusAgent backend: exited before port announcement (${signal || code})${describeOutputTail()}`))
    }
    ...
    const timer = setTimeout(() => {
      cleanup()
      reject(new Error(`Timed out waiting for ZeusAgent backend port announcement (${timeoutMs}ms)`))
    }, timeoutMs)
