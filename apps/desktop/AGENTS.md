...
How to build ZeusAgent Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another ZeusAgent surface can also
...
- A **runtime home change** (switching the underlying `ZEUS_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across ZeusAgent — do not assume
