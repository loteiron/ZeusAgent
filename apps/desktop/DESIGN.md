...
## Zeus visual identity — storm blue and copper

Zeus is a desktop workspace for directing and following agent work. Its visual
signature is a compact lightning mark and a copper leading edge on the storm
blue navigation rail. The welcome surface is left-aligned and action-oriented.

| Role                     | Light                 | Dark                  |
| ------------------------ | --------------------- | --------------------- |
| Workspace                | `#EAF0F7`             | `#101D31`             |
| Panel / raised surface   | `#F7FAFD` / `#FFFFFF` | `#172842` / `#203650` |
| Main text                | `#162743`             | `#E5EDF7`             |
| Navigation and focus     | `#315B83`             | `#92B9DD`             |
| Primary actions / copper | `#A85F32`             | `#E6AB80`             |
| Main perimeter           | `#C5D3E2`             | `#38516D`             |

The palette in `themes/presets.ts` is independently authored. Runtime color
seeds drive the whole app; light and dark Zeus surfaces retain their authored
colors instead of being mixed with inherited grey chrome. The old `nous`,
`nous-alt`, `nous-light`, `default` and `gold` settings resolve to `zeus` at the
theme boundary. They are not selectable branded presets. User imports and
third-party palette options remain available.

Body: native Segoe UI Variable Text / Segoe UI, with system fallbacks. Headings:
Bahnschrift / Segoe UI Variable Display, used sparingly through `--z-font-heading`.
Code and terminal: the existing licensed JetBrains Mono, never a display font.
No default network font requests or inherited Collapse/Rules/Mondwest fonts.

Controls have 8px corners; major panels and menus 12px. `--shadow-zeus` owns
raised elevation; `--stroke-zeus` its perimeter. `shadow-nous` / `--stroke-nous`
remain compatibility aliases for existing feature code, with no old styling.
`--z-copper` (fills), `--z-copper-ink` (small labels), `--z-storm`, `--z-radius-control` and `--z-radius-panel` express
these roles. Layout hooks: `z-workspace-shell`, `z-sidebar`, `z-sidebar-brand`,
`z-sidebar-nav`, `z-welcome`, `z-welcome-copy`, `z-workspace-header` and
`z-composer-region`. Hooks do not change behavior or keyboard ownership.

## Principles

1. **Clear workspaces.** Distinguish navigation, conversation and action surfaces
   with storm blue layers, measured spacing and one perimeter per panel.
   Group related content without decorative card nesting.
2. **Borderless elevation for floating panels.** Overlays float on
   `shadow-zeus` + a `--stroke-zeus` hairline, not thick framed boxes. In-panel
...
3. **One primitive per concern.** One `Button`, one set of control variants,
   ...
   fork.
4. **Tokens, not literals.** Reference CSS vars (`--ui-*`, `--shadow-zeus`,
...
```
shadow-zeus           /* downward-weighted, layered contact→ambient falloff */
border-(--stroke-zeus) /* currentColor hairline, theme-adaptive */
```
...
| `--stroke-zeus`                              | the overlay hairline (pairs with `shadow-zeus`)                                             |
...
Never hardcode `border-gray-*`, `bg-white`, `text-black`, etc. `BrandMark`
uses the theme copper and storm tokens with a transparent backdrop.
...
- Buttons use an 8px radius, padding-driven sizes and 150ms color transitions.
  Primary actions use copper; keyboard focus uses the storm blue ring.
  Keep text/link actions boxless and preserve icon target sizes.
...
- **`SearchField`** — a discoverable inset search well with 8px corners, a quiet
  perimeter and storm focus treatment. Content-sized width may shrink. Empty
  lists hide their search field. Do not wrap it in another bordered tile.
...
- **Loading:** `Loader` (`src/components/ui/loader.tsx`) — the default `storm-orbit`
  is a restrained compass sweep. Explicit curve variants remain supported.
  Reduced-motion mode shows a static arc; hidden documents stop frame updates.
  Keep the accessible status label.
...
- Pick the vocabulary by semantic context and reuse the existing icon for an
  ...
  group.
- **`BrandMark`** (`src/components/brand-mark.tsx`) is the Zeus lightning glyph.
  Use a compact transparent mark in chrome and a modest signature on welcome
  or setup surfaces. No inherited character art, white tile or oversized wordmark.

## Motion

- Quick, functional transitions (~100ms on controls). Respect
  `prefers-reduced-motion` for anything beyond a fade.
- Choreographed exits (e.g. onboarding's content fade) stagger per-element
...
- Keyboard focus is visible: 2px storm ring with 3px offset. Never globally
  suppress outlines or ring shadows. Editors retain their own focus behavior.
...
- [ ] Reuse a primitive (`Button`, `SearchField`, `SegmentedControl`,
      ...
      forking one?
- [ ] Tokens (`--ui-*`, `shadow-zeus`, `--stroke-zeus`) — zero raw colors /
      one-off shadows?
...
- [ ] Overlay uses `shadow-zeus` + `border-(--stroke-zeus)`, no hard border?
- [ ] Distinct workspace layers, without decorative card nesting or gratuitous dividers?
