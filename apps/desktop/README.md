...
**The native desktop app for [ZeusAgent](../../README.md) — the self-improving AI agent from [Nous Research](https://nousresearch.com).** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.

<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other ZeusAgent surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to ZeusAgent and hear it back.</td></tr>
...
### Install with ZeusAgent (recommended)

Already have the ZeusAgent CLI? Just run:

```bash
zeus desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing ZeusAgent gateway or install ZeusAgent locally. Local onboarding then walks you through choosing a provider and model.

### Prebuilt installers

Prebuilt installers are built and distributed via [the ZeusAgent Desktop website.](https://hermes-agent.nousresearch.com/).
...
```bash
# throwaway ZEUS_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the ZeusAgent runtime into `ZEUS_HOME`
(`~/.zeus`, or `%LOCALAPPDATA%\zeus` on Windows), using the same layout as a
CLI install.
...
- **ZeusAgent** runs as a headless `zeus serve` process and exposes the
...
1. `ZEUS_DESKTOP_ZEUS_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `ZEUS_DESKTOP_ZEUS`, or `zeus` on `PATH`
5. a system Python that can import the ZeusAgent runtime
...
Desktop supports a managed local backend, explicit remote gateways, and ZeusAgent
...
screen offers **Connect to existing ZeusAgent** before starting the local installer.
...
terminal commands, and file operations run against the remote ZeusAgent host, not
...
transport- or ZeusAgent-managed header names such as `Authorization`, `Cookie`,
`Host`, `Origin`, `Referer`, and `X-Zeus-Session-Token`.
...
Boot logs land in `ZEUS_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\zeus\zeus-agent\.zeus-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\zeus\zeus-agent\venv"
```

> The default ZeusAgent home on Windows is `%LOCALAPPDATA%\zeus`. Set the `ZEUS_HOME` env var if you've relocated it.
