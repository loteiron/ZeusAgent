/**
 * Shared E2E fixtures for the ZeusAgent desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → zeus serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `zeus desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `zeus-e2e-${prefix}-${Math.random()}`))
  const zeusHome = path.join(root, 'zeus-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(zeusHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(zeusHome, 'config.yaml')
 ...
 */
export function writeEnvFile(zeusHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(zeusHome, '.env')
 ...
 */
function writeEmptyConfig(zeusHome: string): void {
  const configPath = path.join(zeusHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - ZEUS_HOME → sandbox zeus-home (isolated config/sessions)
 *  - ZEUS_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - ZEUS_DESKTOP_IGNORE_EXISTING=1 → don't pick up `zeus` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - ZEUS_DESKTOP_ZEUS_ROOT → REPO_ROOT (dev checkout resolution)
 *  - ZEUS_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    ZEUS_HOME: sandbox.zeusHome,
    ZEUS_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    ZEUS_DESKTOP_IGNORE_EXISTING: '1',
    ZEUS_DESKTOP_ZEUS_ROOT: REPO_ROOT,
    ZEUS_DESKTOP_APP_NAME: `ZeusAgentE2E-${Date.now()}`,
    ...
    ZEUS_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated ZEUS_HOME + userData
 * @param env      - the process environment (already has ZEUS_HOME etc.)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.zeusHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via ZEUS_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`zeus serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.zeusHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { ZEUS_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to ZeusAgent backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'ZeusAgent.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'ZeusAgent.app', 'Contents', 'MacOS', 'ZeusAgent')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'zeus')
}
 ...
 * progress without spawning a real ZeusAgent backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * ZEUS_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    ZEUS_DESKTOP_BOOT_FAKE: '1',
    ZEUS_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + zeus-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).ZEUS_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).ZEUS_DESKTOP_ZEUS
  delete (env as Record<string, string | undefined>).ZEUS_DESKTOP_ZEUS_ROOT
