...
/**
 * E2E smoke tests for the packaged ZeusAgent desktop app.
 *
 ...
 * (credential stripping, isolated ZEUS_HOME + userData, unique app name).
 *
...
test('window opens with the ZeusAgent title', async () => {
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      zeusDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).zeusDesktop?.hud?.open({ sessionId: null })
  )
