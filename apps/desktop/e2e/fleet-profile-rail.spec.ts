 ...
 * second gateway, "Homelab", is a REAL second `zeus serve` this spec spawns
 * with its own ZEUS_HOME, profiles and session token, registered in the v2
...
function findZeusAgentBinary(): string {
  const venv = path.join(REPO_ROOT, '.venv', 'bin', 'zeus')

  if (fs.existsSync(venv)) {
    return venv
  }

  const result = spawnSync('which', ['zeus'], { encoding: 'utf8' })

  if (result.status === 0 && result.stdout.trim()) {
    return result.stdout.trim()
  }

  throw new Error('zeus binary not found: create the repo venv (uv sync) or put zeus on PATH')
}
...
/**
 * Spawn a second, fully real `zeus serve` as the remote gateway. Its
 * session token is pinned through ZEUS_DASHBOARD_SESSION_TOKEN so the
...
async function startRemoteGateway(root: string, mockUrl: string, profiles: string[]): Promise<RemoteGateway> {
  ...
  const child: ChildProcess = spawn(
    ...
    ['serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'],
    {
      ...
      env: {
        ...process.env,
        ZEUS_HOME: home,
        ZEUS_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN,
      },
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`remote zeus serve exited early (${child.exitCode}):\n${log}`)
    }

    try {
      const response = await fetch(`${url}/api/status`, {
        headers: { 'X-Zeus-Session-Token': REMOTE_TOKEN },
      })
  ...
  if (Date.now() >= deadline) {
    throw new Error(`remote zeus serve never became ready:\n${log}`)
  }
...
test.describe('fleet profile rail — two registered gateways', () => {
  ...
  test.beforeAll(async () => {
    ...
    seedProfiles(sandbox.zeusHome, ['research'])
