 ...
 * packaged binary) with a real `zeus serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → zeus serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with ZeusAgent title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('ZeusAgent')
  })
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { zeusDesktop?: Record<string, unknown> }).zeusDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → zeus serve is spawned → WS connects → config
