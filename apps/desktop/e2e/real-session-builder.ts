 ...
 * AIAgent loop, using the E2E mock provider configured in `zeusHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(zeusHome: string) {
  ...
  static async start(zeusHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(zeusHome)
