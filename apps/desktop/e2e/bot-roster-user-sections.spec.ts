...
async function seedBot(zeusHome: string, mockUrl: string, name: string): Promise<void> {
  const dir = path.join(zeusHome, 'profiles', name)
  fs.mkdirSync(dir, { recursive: true })
...
test.beforeAll(async () => {
  ...
  for (const name of ['alpha', 'beta', 'gamma']) {
    await seedBot(sandbox.zeusHome, mock.url, name)
  }
...
test('file bots into user sections by menu and drag; rename; delete returns them to Unassigned', async () => {
  ...
  const alphaProfile = path.join(fixture!.sandbox.zeusHome, 'profiles', 'alpha', 'profile.yaml')
