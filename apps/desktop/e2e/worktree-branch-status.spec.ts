...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'ZeusAgent E2E'], { cwd: repo })
...
function configureRepoCwd(zeusHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(zeusHome, mockUrl)
  fs.appendFileSync(path.join(zeusHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(zeusHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.zeusHome, mock.url, repo)
