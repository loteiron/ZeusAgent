...
an unlinked SQLite DB/WAL/SHM generation.  ``zeus_state`` supplies only the
...
_ZEUS_EXECUTABLES = frozenset({"zeus", "zeus-agent", "zeus-acp"})
_ZEUS_PYTHON_MODULES = frozenset({"acp_adapter", "zeus_cli.main"})
_ZEUS_PYTHON_SCRIPTS = frozenset({"zeus_cli/main.py", "run_agent.py"})
...
def _looks_like_zeus(argv: Sequence[str]) -> bool:
    """Return whether argv identifies a supported ZeusAgent execution target."""
    if not argv:
        return False
    program = os.path.basename(argv[0]).lower().removesuffix(".exe")
    if program in _ZEUS_EXECUTABLES:
        return True
    ...
    if kind == "module":
        return normalized in _ZEUS_PYTHON_MODULES
    return any(
        normalized == script or normalized.endswith(f"/{script}")
        for script in _ZEUS_PYTHON_SCRIPTS
    )
        ...
        try:
            ...
            for pid_str in os.listdir("/proc"):
                ...
                try:
                    ...
                    if argv is not None and _looks_like_zeus(argv):
                        cmdline = " ".join(argv)
