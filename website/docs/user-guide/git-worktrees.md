...
title: "Git Worktrees"
description: "Run multiple ZeusAgent agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

ZeusAgent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with ZeusAgent so each session has a clean, isolated working directory.

## Why Use Worktrees with ZeusAgent?

ZeusAgent treats the **current working directory** as the project root:

- CLI: the directory where you run `zeus` or `zeus chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.zeus/config.yaml`
...
ZeusAgent creates `.worktrees/my-experiment/` inside the repo (branch
`zeus/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `zeus-<id>`
...
git worktree add ../repo-feature feature/zeus-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/zeus-experiment` checked out in that directory

Now you can `cd` into the new worktree and run ZeusAgent there:

```bash
cd ../repo-feature

# Start ZeusAgent in the worktree
zeus
```
...
git worktree add ../repo-experiment-a feature/zeus-a
git worktree add ../repo-experiment-b feature/zeus-b
```
...
Each ZeusAgent process:

- Works on its own branch (`feature/zeus-a` vs `feature/zeus-b`).
...
- ZeusAgent checkpoint data under `~/.zeus/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per ZeusAgent experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/zeus-checkpoints-docs`, `feature/zeus-refactor-tests`.
- **Commit frequently**
...
- **Avoid running ZeusAgent from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `zeus -w` (Automatic Worktree Mode)

ZeusAgent has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `zeus/zeus-<hash>`).
...
```bash
zeus -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `zeus -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each ZeusAgent session its own clean checkout.
