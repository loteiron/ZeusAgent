...
If you only run one ZeusAgent agent, you don't need this page — see
...
[Connecting Desktop to Many ZeusAgent Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more ZeusAgent agents that should all
...
(`ai.zeus.gateway-<name>.plist`) or systemd user service
(`zeus-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
zeus profile create coder
zeus profile create personal-bot
zeus profile create research
...
```bash
zeus config set gateway.multiplex_profiles true
zeus gateway restart
```

Equivalently, in the default profile's `~/.zeus/config.yaml`:
...
You do **not** run `zeus gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `zeus gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.zeus/profiles/coder/.env`; the default listener key is rejected.
...
home). `zeus status` reports the multiplexer and the profiles it serves;
`zeus status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/zeus-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: zeus-gateways {start|stop|restart|status|list}"
}
...
```bash
zeus-gateways start      # start every configured profile
zeus-gateways stop       # stop every configured profile
zeus-gateways restart    # restart all
zeus-gateways status     # status across all
zeus-gateways list       # delegates to `zeus gateway list`
```

:::tip
The `default` profile is targeted with `zeus gateway <action>` (no `-p`),
not `zeus -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `zeus -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.zeus.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/zeus-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.zeus.gateway.plist` /
...
```bash
# Default profile
tail -f ~/.zeus/logs/gateway.log
tail -f ~/.zeus/logs/gateway.error.log

# Named profile
tail -f ~/.zeus/profiles/<name>/logs/gateway.log
tail -f ~/.zeus/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.zeus/logs/gateway.log ~/.zeus/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
zeus logs -f                  # follow default profile
zeus -p coder logs -f         # follow one profile
zeus logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
zeus profile list             # profiles + model + gateway state
zeus-gateways status          # full status across every profile
launchctl list | grep zeus    # macOS — PIDs and labels
systemctl --user list-units 'zeus-gateway-*'   # Linux — units
```
...
The default profile uses `~/.zeus/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
zeus config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.zeus/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=zeus --why="gateways running" \
  sleep infinity &
...
`zeus-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
You ran `zeus gateway start` after a previous `zeus gateway stop`. The
...
```bash
ps -ef | grep "zeus_cli.*-p <profile>"
cat ~/.zeus/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.zeus.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.zeus.gateway-<profile>.plist

# Linux
systemctl --user restart zeus-gateway-<profile>.service
```

### Health check

```bash
zeus doctor                  # default profile
zeus -p <profile> doctor     # one profile
```
