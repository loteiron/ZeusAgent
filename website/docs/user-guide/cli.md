...
sidebar_position: 1
title: "CLI Interface"
description: "Master the ZeusAgent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

ZeusAgent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `zeus setup --portal` — and you're ready to `zeus chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
ZeusAgent also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `zeus --tui` — see the [TUI](tui.md) guide.
:::
...
zeus chat --query-file prompt.txt
zeus chat --query-file - < prompt.txt

# With a specific model
zeus chat --model "anthropic/claude-sonnet-4"

# With a specific provider
zeus chat --provider nous        # Use Nous Portal
zeus chat --provider openrouter  # Force OpenRouter

# With specific toolsets
zeus chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
zeus -s zeus-agent-dev,github-auth
zeus chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
zeus --continue             # Resume the most recent CLI session (-c)
zeus --resume <session_id>  # Resume a specific session by ID (-r)
zeus --resume latest        # Resume the most recent session (same as -c)
zeus --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
zeus chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
zeus -w                         # Interactive mode in worktree
zeus -w -z "Fix issue #123"     # Single query in worktree
```

### Worktree cleanup

`zeus -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
```bash
zeus worktree list              # audit: age, size, verdict, reason per tree
zeus worktree prune             # remove safe trees + delete merged branches
zeus worktree prune --dry-run   # show the plan without changing anything
zeus worktree prune --trees-only     # leave local branches alone
zeus worktree prune --branches-only  # leave worktrees alone
```
...
- Trees **in use by a running zeus session** are never touched.
- **Untracked-only scratch** (PR body drafts, notes) is archived to
  `~/.zeus/archive/worktree-prune/` before its tree is removed — never
  destroyed.
...
launches `zeus -w` for days — no longer accumulate merged scratch trees
...
The `zeus plugins` commands manage native ZeusAgent plugins and portable Agent
...
zeus plugins disable <plugin-name>
zeus plugins update <plugin-name>
zeus plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. ZeusAgent currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the ZeusAgent CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The ZeusAgent CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `ZEUS_YOLO_MODE` is on (either `zeus --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
On the `openai-codex` provider, `/usage` also shows any banked usage-limit resets on your ChatGPT account ("You have N resets banked - use /usage reset to activate"). `/usage reset` redeems one banked reset, fully restoring your 5-hour and weekly limits. ZeusAgent refuses to redeem while your limits aren't exhausted (a banked reset restores the full allowance, so spending it early wastes it) — pass `/usage reset --force` to redeem anyway.

### Session Resume Display

When resuming a previous session (`zeus -c` or `zeus --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend ZeusAgent to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.zeus/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. ZeusAgent supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for ZeusAgent replies |
...
```bash
zeus -s zeus-agent-dev,github-auth
zeus chat -s github-pr-workflow -s github-auth
```

ZeusAgent loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.zeus/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.zeus/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain ZeusAgent..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, ZeusAgent also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. ZeusAgent recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches ZeusAgent — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while ZeusAgent is working, ZeusAgent prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend ZeusAgent to the background — just like any terminal process. The shell prints a confirmation:

```
ZeusAgent has been suspended. Run `fg` to bring ZeusAgent back.
```
...
Resume this session with:
  zeus --resume 20260225_143052_a1b2c3
...
```bash
zeus --continue                          # Resume the most recent CLI session
zeus -c                                  # Short form
zeus -c "my project"                     # Resume a named session (latest in lineage)
zeus --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
zeus --resume "refactoring auth"         # Resume by title
zeus --resume latest                     # Resume the most recent session (same as -c)
zeus --resume latest --in ./my-project   # Latest session for ./my-project's workspace
zeus -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `zeus sessions rename <id> <title>` from the command line. Use `zeus sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in ZeusAgent's SQLite state database under `~/.zeus/state.db`. The database keeps:
...
ZeusAgent immediately confirms the task and gives you back the prompt:
