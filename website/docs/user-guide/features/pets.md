...
ZeusAgent can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<ZEUS_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), ZeusAgent renders the sprite at full fidelity when your
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `zeus pets list [query] [--limit N]` |
| List installed pets | `zeus pets list --installed` |
| Install a pet | `zeus pets install <slug> [--select] [--force]` |
| Set the active pet | `zeus pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `zeus pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `zeus pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `zeus pets off` |
| Remove an installed pet | `zeus pets remove <slug>` |
| Diagnose setup | `zeus pets doctor` |

`zeus pets show` flags:
...
Beyond installing pre-made pets from the gallery, ZeusAgent can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `zeus pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `zeus tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `ZEUS_PET_IMAGE_PROVIDER` env var (e.g. `ZEUS_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while ZeusAgent is
...
Run `zeus pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; ZeusAgent uses its own
  profile-scoped `<ZEUS_HOME>/pets/` instead — install through `zeus pets`.

## See also

- The [`zeus-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-zeus-agent.md)
