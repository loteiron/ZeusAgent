...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of ZeusAgent's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple ZeusAgent instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
zeus memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `zeus` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — ZeusAgent syncs them back at session init.
...
The `zeus honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `zeus memory setup honcho` (or run `zeus memory setup` and pick it from the list); the `zeus honcho` subcommand then appears on the next invocation.

```bash
zeus memory setup honcho    # Configure Honcho directly (works before activation)
zeus honcho status          # Connection status, config, and key settings
zeus honcho setup           # Redirects to `zeus memory setup` (post-activation alias)
zeus honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
zeus honcho peer            # Show or update peer names + dialectic reasoning level
zeus honcho mode            # Show or set recall mode (hybrid/context/tools)
zeus honcho tokens          # Show or set token budget for context and dialectic
zeus honcho identity        # Seed or show the AI peer's Honcho identity
zeus honcho sync            # Sync Honcho config to all existing profiles
zeus honcho peers           # Show peer identities across all profiles
zeus honcho sessions        # List known Honcho session mappings
zeus honcho map             # Map current directory to a Honcho session name
zeus honcho enable          # Enable Honcho for the active profile
zeus honcho disable         # Disable Honcho for the active profile
zeus honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `zeus honcho`

If you previously used the standalone `zeus honcho setup`:
...
No re-login or re-setup needed. Run `zeus memory setup` and select "honcho" — the wizard detects your existing config.
