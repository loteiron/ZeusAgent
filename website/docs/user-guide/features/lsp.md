...
ZeusAgent runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. ZeusAgent
...
1. ZeusAgent captures a baseline of current diagnostics for the file.
...
…). ZeusAgent auto-detects the binary on PATH or in
`<ZEUS_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point ZeusAgent at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<ZEUS_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`zeus lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — ZeusAgent installs both packages together when you
run `zeus lsp install typescript` or auto-install fires on first
use.

## CLI

```
zeus lsp status          # service state + per-server install status
zeus lsp list            # registry, optionally --installed-only
zeus lsp install <id>    # eagerly install one server
zeus lsp install-all     # try every server with a known recipe
zeus lsp restart         # tear down running clients
zeus lsp which <id>      # print resolved binary path
```

`zeus lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <ZEUS_HOME>/lsp/bin
...
When `install_strategy: auto`, ZeusAgent installs binaries into
`<ZEUS_HOME>/lsp/bin/`. NPM packages land in
`<ZEUS_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully ZeusAgent-owned and is
...
**single process** per ZeusAgent process: the first Python project spawns
...
**`zeus lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<ZEUS_HOME>/lsp/bin/`. Run
`zeus lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `zeus lsp status`**
...
When `zeus lsp status` shows a `Backend warnings` section, install
...
Check `~/.zeus/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `zeus lsp restart` to clear the set;
