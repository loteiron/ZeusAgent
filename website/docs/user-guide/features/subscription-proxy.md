...
chat completions — use your ZeusAgent-managed provider subscription as their
...
| Use case | "Use ZeusAgent as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. ZeusAgent stores
the refresh token in `~/.zeus/auth.json` — the same place all ZeusAgent
...
interface in `zeus_cli/proxy/adapters/`.
...
If you see `not logged in`, run `zeus portal`. If you see
...
`zeus_cli/proxy/adapters/<provider>.py` and registering it in
