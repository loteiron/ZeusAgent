...
description: "Plugins shipped with ZeusAgent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

ZeusAgent ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.zeus/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a ZeusAgent Plugin](/developer-guide/plugins) to write your own.
...
2. **User** — `~/.zeus/plugins/<name>/`
3. **Project** — `./.zeus/plugins/<name>/` (requires `ZEUS_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `zeus_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `zeus memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `zeus plugins list` and the interactive `zeus plugins` UI), but none load until you explicitly enable them:

```bash
zeus plugins enable disk-cleanup
```

Or via `~/.zeus/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer ZeusAgent. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
zeus plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `zeus plugins enable <name>`.
...
| `zeus-achievements` | dashboard tab | Steam-style collectible badges generated from your real ZeusAgent session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `zeus memory` and `zeus plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ZEUS_HOME` or `/tmp/zeus-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under ZEUS_HOME | always | Never |
...
**State** — everything lives at `$ZEUS_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `ZEUS_HOME` or `/tmp/zeus-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `zeus plugins enable disk-cleanup` (or check the box in `zeus plugins`).

**Disabling again:** `zeus plugins disable disk-cleanup`.
...
**Enabling:** `zeus plugins enable security-guidance` (or check the box in `zeus plugins`).

**Disabling again:** `zeus plugins disable security-guidance`.
...
Traces ZeusAgent turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of ZeusAgent' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `zeus logs`.
...
```bash
zeus tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart ZeusAgent and the next turn ships a trace.

**Setup (manual):**

```bash
pip install langfuse
zeus plugins enable observability/langfuse
```

Then put the credentials in `~/.zeus/.env`:

```bash
ZEUS_LANGFUSE_PUBLIC_KEY=pk-lf-...
ZEUS_LANGFUSE_SECRET_KEY=sk-lf-...
ZEUS_LANGFUSE_BASE_URL=https://cloud.langfuse.com   # or your self-hosted URL
```
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "ZeusAgent turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `ZEUS_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the ZeusAgent session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `zeus chat` session lives under one Langfuse session.

**Verify:**

```bash
zeus plugins list                 # observability/langfuse should show "enabled"
zeus chat -q "hello"              # check the Langfuse UI for a "ZeusAgent turn" trace
```
...
| `ZEUS_LANGFUSE_RELEASE` | — | Release/version tag |
| `ZEUS_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `ZEUS_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `ZEUS_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

ZeusAgent-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — ZeusAgent-prefixed wins when both are set.
...
**Disabling:** `zeus plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.

### NeMo Relay native integration (migration note)

NeMo Relay is no longer a bundled ZeusAgent plugin. Do not run `zeus plugins enable observability/nemo_relay`; ZeusAgent core now owns the Relay session, turn, LLM, and tool lifecycles.

To opt into Relay middleware or exporters, create a standard Relay `plugins.toml`, then set `ZEUS_NEMO_RELAY_PLUGINS_TOML` to that file before starting ZeusAgent. The policy is process-wide for every profile hosted by that ZeusAgent process. See the [NeMo Relay observability configuration](https://docs.nvidia.com/nemo/relay/configure-plugins/observability/about) for ATOF, ATIF, and OpenTelemetry options.

The old `ZEUS_NEMO_RELAY_ATOF_*` and `ZEUS_NEMO_RELAY_ATIF_*` settings no longer activate exporters. `zeus doctor` reports these stale settings when no replacement `plugins.toml` is selected.
...
Both defaults preserve one session scope for the full session. Rotated spans retain the same `session_id` and add `zeus.session.segment` plus `zeus.session.segment_reason` (`compaction` or `max_turns`).
...
- Post-meeting artifacts (transcript, status) saved under `~/.zeus/workspace/meetings/<meeting_id>/`

**Setup:**

```bash
zeus plugins enable google_meet
zeus meet setup   # preflight: playwright, chromium, auth file
zeus meet auth    # opens a browser to sign into Google and saves session state —
...
**Disabling:** `zeus plugins disable google_meet`. Any saved transcripts stay in `~/.zeus/workspace/meetings/` until you remove them.

### zeus-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real ZeusAgent session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with ZeusAgent feature changes.

**How it works:**

- Scans your entire `~/.zeus/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$ZEUS_HOME/plugins/zeus-achievements/state.json`
...
| Secret | Hidden until ZeusAgent detects the first related signal in your history |

**API** — routes mount under `/api/plugins/zeus-achievements/`:
...
**State files** — live under `$ZEUS_HOME/plugins/zeus-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across ZeusAgent updates. |
...
**Enabling:** Nothing to enable — `zeus-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `zeus dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/zeus-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.zeus/plugins/zeus-achievements/` that ships no dashboard. The plugin's state files under `$ZEUS_HOME/plugins/zeus-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other ZeusAgent plugin — see [Build a ZeusAgent Plugin](/developer-guide/plugins). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.zeus/plugins/<name>/`
- Manifest source is reported as `bundled` in `zeus plugins list`
