...
When ZeusAgent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.zeus/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.zeus/config.yaml` (switchable per session
via `/personality`).
...
If you use ZeusAgent' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.zeus/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. ZeusAgent's deliverable
