...
ZeusAgent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
zeus memory setup      # interactive picker + configuration
zeus memory status     # check what's active
zeus memory off        # disable external provider
```

You can also select the active memory provider via `zeus plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.zeus/config.yaml`:
...
When a memory provider is active, ZeusAgent automatically:
...
**Setup Wizard:**
```bash
zeus memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `zeus honcho setup` command still works (it now redirects to `zeus memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
**Config:** `$ZEUS_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$ZEUS_HOME/honcho.json` > `~/.zeus/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/NousResearch/hermes-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/hermes).
...
:::tip Migrating from `zeus honcho`
If you previously used `zeus honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per ZeusAgent profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All ZeusAgent profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per ZeusAgent profile. Host key `zeus` → default; `zeus.<profile>` for others. |
...
```bash
zeus profile create coder --clone
```

`--clone` creates a `zeus.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every ZeusAgent profile, creates host blocks for any profile without one, inherits settings from the default `zeus` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `zeus memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
zeus memory setup    # select "openviking"
# Or manually:
zeus config set memory.provider openviking
```

`zeus memory setup` can reuse or copy connection values from
`~/.openviking/ovcli.conf`. Manual setup uses the active profile's `.env` file;
for the default profile that is `~/.zeus/.env`, and for named profiles use
`~/.zeus/profiles/<profile>/.env`.
...
Peer identity is optional. By default, ZeusAgent sends no peer ID and writes
...
Set `memory.openviking.agent: zeus` to restore the old peer-scoped writes.
...
ZeusAgent sends `User-Agent: openviking-memory-zeus/<version>` on OpenViking
...
**Setup (Platform):**
```bash
zeus memory setup    # select "mem0" → "Platform"
# Or manually:
zeus config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.zeus/.env
```

**Setup (OSS):**
```bash
zeus memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
zeus memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
zeus memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
zeus memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
zeus memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.zeus/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.zeus/.env
```
...
**Config:** `$ZEUS_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.zeus/.env`.
...
| `user_id` | `zeus-user` | User identifier |
| `agent_id` | `zeus` | Agent identifier |
...
**Switching modes:** Re-run `zeus memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
zeus memory setup    # select "hindsight"
# Or manually:
zeus config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.zeus/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p zeus ui start`

**Config:** `$ZEUS_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `zeus` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between ZeusAgent and the User` | Context label for retained memories |
...
**Setup:**
```bash
zeus memory setup    # select "holographic"
# Or manually:
zeus config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.zeus-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$ZEUS_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
zeus memory setup    # select "retaindb"
# Or manually:
zeus config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.zeus/.env
```
...
# Then configure ZeusAgent
zeus memory setup    # select "byterover"
# Or manually:
zeus config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$ZEUS_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
zeus memory setup    # select "supermemory"
# Or manually:
zeus config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.zeus/.env
```
...
Before running `zeus memory setup`, set `base_url` in
`$ZEUS_HOME/supermemory.json`:
...
Then run `zeus memory setup` and enter the API key printed by the local
server. Configuring the endpoint first ensures the setup connection probe also
stays local.

**Config:** `$ZEUS_HOME/supermemory.json`
...
| `container_tag` | `zeus` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `zeus-{identity}` → `zeus-coder`) to isolate memories per ZeusAgent profile
...
| **Requires** | `pip install zeus-memori` + `zeus-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install zeus-memori
zeus-memori install
zeus config set memory.provider memori
zeus memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `zeus-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$ZEUS_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$ZEUS_HOME/` so each profile has its own credentials
