...
title: "Plugins"
description: "Extend ZeusAgent with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

ZeusAgent has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/developer-guide/adding-tools) page is for built-in ZeusAgent
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a ZeusAgent Plugin](/developer-guide/plugins)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.zeus/plugins/` with a `plugin.yaml` and Python code:

```
~/.zeus/plugins/my-plugin/
...
Start ZeusAgent — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.zeus/plugins/hello-world/plugin.yaml`**
...
**`~/.zeus/plugins/hello-world/__init__.py`**

```python
"""Minimal ZeusAgent plugin — registers a tool and a hook."""
...
Drop both files into `~/.zeus/plugins/hello-world/`, restart ZeusAgent, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

The model-facing tool description belongs in `schema["description"]`. The optional `ctx.register_tool(description=...)` value is separate `ToolEntry` registry metadata: when omitted, it defaults to the schema description, but ZeusAgent does not copy it back into a schema that lacks `description`. Prefer defining the text once in the schema. If you provide both values, keep them synchronized; the model sees the schema value.

Project-local plugins under `./.zeus/plugins/` are disabled by default. Enable them only for trusted repositories by setting `ZEUS_ENABLE_PROJECT_PLUGINS=true` before starting ZeusAgent.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `zeus <plugin> <subcommand>` |
...
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `zeus plugins install` |
| Distribute via pip | `[project.entry-points."zeus_agent.plugins"]` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with ZeusAgent — see [Built-in Plugins](/user-guide/features/built-in-plugins) |
| User | `~/.zeus/plugins/` | Personal plugins |
| Project | `.zeus/plugins/` | Project-specific plugins (requires `ZEUS_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `zeus_agent.plugins` entry_points | Distributed packages |
| Nix | `services.zeus-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/getting-started/nix-setup#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

### Plugin sub-categories

Within each source, ZeusAgent also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.zeus/plugins/model-providers/<name>/` and `~/.zeus/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `zeus plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.zeus/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
zeus plugins                    # interactive toggle (space to check/uncheck)
zeus plugins enable <name>      # add to allow-list
zeus plugins disable <name>     # remove from allow-list + add to disabled
```

After `zeus plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

For a reproducible install, pin a full immutable commit (tags, branches, and
abbreviated SHAs are not accepted):

```bash
zeus plugins install owner/repo --ref 0123456789abcdef0123456789abcdef01234567
```

ZeusAgent checks out the commit detached, verifies that `HEAD` exactly matches the
requested SHA, and records the canonical source, installed revision, and pin
status in the current profile. `zeus plugins update` refuses to move a pinned
plugin; choose a new exact commit explicitly with
`zeus plugins install <source> --force --ref <new-commit>`. The
...
Several categories of plugin bypass `plugins.enabled` — they're part of ZeusAgent' built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.zeus/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.zeus/plugins/`.

### Approval transports

An approval transport changes **where a human sees and answers** an existing
ZeusAgent tool-approval request. It does not decide whether a command needs
...
`present` may be synchronous or async. ZeusAgent runs it on a bounded worker and
...
`transport_fallback: builtin`. Without that exact opt-in, ZeusAgent never
materializes the prompt on another surface.

ZeusAgent still owns hardline blocks, sudo-stdin protection, user deny rules,
...
When you upgrade to a version of ZeusAgent that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.zeus/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)

## Available hooks

Plugins can register the 26 lifecycle events currently accepted by `zeus_cli.plugins.VALID_HOOKS`. The **[Event Hooks catalog](/user-guide/features/hooks#shipped-plugin-hook-catalog)** is canonical for exact timing, return handling, payload fields, and privacy notes.
...
ZeusAgent has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.zeus/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and ZeusAgent also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a ZeusAgent Plugin](/developer-guide/plugins) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a ZeusAgent Plugin](/developer-guide/plugins) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a ZeusAgent Plugin](/developer-guide/plugins) · [Extending the CLI](/developer-guide/extending-the-cli) |
| A **subcommand** for `zeus <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/developer-guide/extending-the-cli) |
...
| An **STT backend** (any CLI — whisper.cpp, custom whisper binary, local ASR CLI) | Config-driven (recommended) — declare under `stt.providers.<name>` with `type: command` in `config.yaml`, or set `ZEUS_LOCAL_STT_COMMAND` for the legacy single-command escape hatch. OR Python backend plugin — `ctx.register_transcription_provider()` for Python-SDK engines (OpenRouter, SenseAudio, Gemini-STT, etc.). | [STT Setup](/user-guide/features/tts#stt-custom-command-providers) · [Python plugin guide](/user-guide/features/tts#python-plugin-providers-stt) |
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. ZeusAgent auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `zeus skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.zeus/hooks/<name>/` | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `zeus plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
```bash
zeus plugins                               # unified interactive UI
zeus plugins list                          # table: enabled / disabled / not enabled
zeus plugins search <term>                 # search the community plugin index
zeus plugins install <name>                # install by index name (resolved to repo @ pinned ref)
zeus plugins install user/repo             # install from Git, then prompt Enable? [y/N]
zeus plugins install user/repo --enable    # install AND enable (no prompt)
zeus plugins install user/repo --no-enable # install but leave disabled (no prompt)
zeus plugins update my-plugin              # pull latest (local edits are autostashed and re-applied)
zeus plugins remove my-plugin              # uninstall
zeus plugins enable my-plugin              # add to allow-list
zeus plugins disable my-plugin             # remove from allow-list + add to disabled
zeus plugins capabilities [my-plugin]      # declared vs granted capabilities
```

### One-click install links (Desktop)

ZeusAgent Desktop registers the `zeus://` URL scheme, so a website, README, or
chat message can link straight to a plugin install:

```
zeus://plugin/install?repo=owner/repo            # main install link
zeus://plugin/install?repo=owner/repo&enable=1   # enable the agent plugin after install
zeus://plugin/install?repo=owner/repo&force=1    # replace an existing install
```

Clicking one opens ZeusAgent and shows a **confirmation dialog** — the repo id,
...
Install from Git**. Legacy `zeus://plugin-agent/…` and
`zeus://plugin-desktop/…` URLs route into the same dialog. In dev builds
(`npm run dev`) the scheme is `zeus-dev://`.

Websites need no SDK — a normal anchor works:

```html
<a href="zeus://plugin/install?repo=owner/repo&enable=1">Install in ZeusAgent</a>
```

MCP servers have the equivalent link form — see
[Add to ZeusAgent link](/reference/mcp-config-reference#add-to-zeus-link).
...
When a plugin declares capabilities, `zeus plugins install` (and
`zeus plugins enable`) shows the list with one-line risk descriptions and
...
granted, `zeus plugins update` surfaces the additions and asks again. New
...
`zeus plugins enable <id>` interactively to grant them later.

Inspect the state at any time:

```bash
zeus plugins capabilities             # all plugins with declared/granted capabilities
zeus plugins capabilities my-plugin   # one plugin, declared vs granted
```
...
not a code audit, and ZeusAgent has not reviewed the plugin's code. Only install
...
`zeus plugins search <term>` searches the **community plugin index** — a
static, machine-readable JSON catalog of community plugins. Matching is fuzzy
across name, description, and tags:

```bash
zeus plugins search telegram               # fuzzy search
zeus plugins search                        # browse the whole index
zeus plugins search --capability platform  # filter by declared capability
zeus plugins search media --json           # machine-readable output
zeus plugins search --refresh              # bypass the 24h local cache
```
...
```bash
zeus plugins install zeus-media-studio
```
...
overridable via `zeus config set plugins.index_url <url>`). Fetches are
cached under `~/.zeus/cache/plugin_index.json` for 24 hours; when the
remote is unreachable the stale cache is used, and when there is no cache at
all a bundled seed copy ships with ZeusAgent — so search works fully offline.

**Index entry format.** Each entry is a JSON object:

```json
{
  "name": "zeus-media-studio",
...
(`zeus_cli/data/plugin_index.json` in the repo) is the format reference.

**Submitting a plugin.** The index is maintained as a plain JSON file —
submit a pull request to the
[zeus-plugin-index](https://github.com/NousResearch/hermes-plugin-index)
...
A **plugin pack** is a declarative, shareable YAML file (`zeus-pack.yaml`)
...
version: 1.0.0
plugins:
  - name: zeus-media-studio            # bare community-index name…
...
```bash
zeus plugins pack show ./zeus-pack.yaml     # dry-run review
zeus plugins pack install ./zeus-pack.yaml  # review → confirm → install
zeus plugins pack export > zeus-pack.yaml   # snapshot the current install
zeus plugins pack export --enabled-only       # only plugins.enabled
```
...
same pinned install path as `zeus plugins install --ref <sha>` and record
...
capability-consent prompt — identical to a single `zeus plugins install`.
...
provenance (installed via `zeus plugins install`). Local-only plugins are
...
auto-installed — install those manually for now (`zeus skills`). Wiring
skill-hub ids into pack install is a documented follow-up seam.

### Install-time security scanning

Every `zeus plugins install` and `zeus plugins update` runs a static
...
On `zeus plugins update`, a dangerous verdict on the updated tree
...
Running `zeus plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `zeus plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
- ZeusAgent reuses that session's stored platform, chat, thread, profile, and conversation history. Plugins cannot supply a new chat route through this API.
- ZeusAgent rechecks the stored route against the gateway's current authorisation rules before dispatch.
- Routes that relied only on an adapter-time or upstream authorisation decision are rejected unless ZeusAgent can revalidate them from current core allowlists, pairing, or explicit allow-all configuration.
...
- The route and conversation are pinned while dispatch is pending. ZeusAgent drops the request if topic recovery changes the route or the session rotates before handling starts.
...
:::warning
Only grant gateway injection to plugins you trust. ZeusAgent checks this host API permission and restricts it to existing session routes, but Python plugins run in-process and this setting is not a sandbox.
:::
...
`ctx.call_mcp()` lets a plugin call a tool on one of the user's configured MCP servers — synchronously, from any hook or tool handler — routing through ZeusAgent' existing native MCP client (same connections, trust-tier gates, circuit breaker, and reconnect logic as model-invoked MCP tools; never a parallel client).
