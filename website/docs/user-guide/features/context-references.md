...
Type `@` followed by a reference to inject content directly into your message. ZeusAgent expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- ZeusAgent env: `$ZEUS_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$ZEUS_HOME/skills/.hub/`
