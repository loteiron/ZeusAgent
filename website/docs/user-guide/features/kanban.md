...
description: "Durable SQLite-backed task board for coordinating multiple ZeusAgent profiles"
---
...
ZeusAgent Kanban is a durable task board, shared across all your ZeusAgent profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.zeus/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.
...
The board has two front doors, both backed by the same `~/.zeus/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_request_review`, `kanban_request_changes`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_attach`, `kanban_attach_url`, `kanban_attachments`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `zeus kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `zeus kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
For the full design rationale, comparative analysis against Cline Kanban / Paperclip / NanoClaw / Google Gemini Enterprise, and the eight canonical collaboration patterns, see `docs/zeus-kanban-v1-spec.pdf` in the repository.
...
- **Dispatcher** — a long-lived loop that, every N seconds (default 60): reclaims stale claims, reclaims crashed workers (PID gone but TTL not yet expired), promotes ready tasks, atomically claims, spawns assigned profiles. Runs **inside the gateway** by default (`kanban.dispatch_in_gateway: true`). One dispatcher sweeps all boards per tick; workers are spawned with `ZEUS_KANBAN_BOARD` pinned so they can't see other boards. After `kanban.failure_limit` consecutive spawn failures on the same task (default: 2) the dispatcher auto-blocks it with the last error as the reason — prevents thrashing on tasks whose profile doesn't exist, workspace can't mount, etc.
...
zeus kanban boards list

# Create a new board.
zeus kanban boards create atm10-server \
    --name "ATM10 Server" \
...
zeus kanban --board atm10-server list
zeus kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
zeus kanban boards switch atm10-server
zeus kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
zeus kanban boards rename atm10-server "ATM10 (Prod)"
...
2. `ZEUS_KANBAN_BOARD` env var (set by the dispatcher when spawning a
   worker, so workers can't see other boards).
3. `~/.zeus/kanban/current` — the slug persisted by `zeus kanban
   boards switch`.
4. `default`.
...
`zeus dashboard` → Kanban tab shows a board switcher at the top as soon
...
- **Storage** — files land under
  `<zeus-home>/kanban/attachments/<task_id>/` for the default board, or
  `<zeus-home>/kanban/boards/<slug>/attachments/<task_id>/` for a named
  board. Set `ZEUS_KANBAN_ATTACHMENTS_ROOT` to pin a custom location.
...
zeus kanban create "research AI funding landscape" --assignee researcher
...
When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `zeus kanban show t_abcd`.
...
Override the config flag at runtime via `ZEUS_KANBAN_DISPATCH_IN_GATEWAY=0`
for debugging. Standard gateway supervision applies: run `zeus gateway
...
until one comes up — `zeus kanban create` warns about this at creation
time.

Running `zeus kanban daemon` as a separate process is **deprecated**;
...
zeus kanban create "nightly ops review" \
    --assignee ops \
...
```bash
zeus kanban complete t_abc t_def t_hij --result "batch wrap"
zeus kanban archive  t_abc t_def t_hij
zeus kanban unblock  t_abc t_def
zeus kanban block    t_abc "need input" --ids t_def t_hij
```
...
```bash
zeus -p planner tools enable kanban                      # CLI / TUI / Desktop chats
zeus -p planner tools enable kanban --platform telegram  # a gateway platform
```

Each platform has its own selection under `platform_toolsets.<platform>` in
`config.yaml`; the toolset is also a checkbox in `zeus tools` and the dashboard.
...
**Workers do not shell out to `zeus kanban`.** When the dispatcher spawns a worker it sets `ZEUS_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `zeus kanban` CLI.
...
kanban_show()                                     # no args — uses ZEUS_KANBAN_TASK
...
### Why tools instead of shelling to `zeus kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `zeus kanban complete` *inside* the container, where `zeus` isn't installed and `~/.zeus/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.zeus/kanban.db` regardless of terminal backend.
...
**Zero schema footprint on normal sessions.** A regular `zeus chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `ZEUS_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.
...
```json
{
  ...
  "verification": ["pytest tests/zeus_cli/test_kanban_db.py -q"],
...
2. `cd $ZEUS_KANBAN_WORKSPACE` (via the terminal tool) and do the work there.
...
**Agent-side prevention:** Before the worker exits, ZeusAgent injects up to two
...
guard is active only for dispatcher-spawned workers (`ZEUS_KANBAN_TASK` is
set) and can be disabled with `ZEUS_KANBAN_STOP_NUDGE=0`.
...
```bash
zeus kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

zeus kanban create "audit auth flow" \
    --assignee reviewer \
...
The dispatcher emits one `--skills <name>` flag per skill listed, so the worker spawns with all of them loaded on top of the auto-injected kanban guidance. The skill names must match skills that are actually installed on the assignee's profile (run `zeus skills list` to see what's available); there's no runtime install.
...
```bash
# At creation
zeus kanban create "hard refactor" --assignee coder \
    --model claude-opus-4.6 --provider anthropic

# Or later — takes effect on the next dispatch
zeus kanban set-model t_abcd claude-opus-4.6 --provider anthropic
zeus kanban set-model t_abcd none    # clear the override
```
...
Kanban's per-profile configs make the planner/worker cost split natural. Decomposing a project into well-scoped cards takes frontier-level judgment; executing a card that already carries a clear goal, context, and handoff evidence usually doesn't — and the workers are where the vast majority of tokens are spent, so the worker model is where the cost lives. Run your orchestrator/dispatcher profile on a frontier model and point worker profiles at inexpensive models. Each profile has its own `config.yaml` under `~/.zeus/profiles/<name>/`, and the dispatcher injects the profile-scoped `ZEUS_HOME` when it spawns `zeus -p <assignee>`, so each worker reads its own profile's model settings:

```yaml
# ~/.zeus/config.yaml (orchestrator / dispatcher profile)
model:
  default: "your-frontier-model"

# ~/.zeus/profiles/coder/config.yaml (worker profile)
model:
  default: "your-inexpensive-model"

# ~/.zeus/profiles/researcher/config.yaml (another worker profile)
model:
  default: "your-inexpensive-model"
```

For the occasional quality-sensitive card, pin just that task back to a stronger model with the [per-task model override](#per-task-model-override) (`--model`/`--provider` at create time, `zeus kanban set-model` later, or the dashboard's model dropdown) — no profile edits needed.
...
```bash
zeus kanban create "Translate the docs site to French" \
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `zeus kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.
...
Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `zeus kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `zeus profile create --description "..."`, `zeus profile describe <name> --text "..."`, `zeus profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee` (or the active default profile if that's unset).
...
Config knobs (all under `kanban:` in `~/.zeus/config.yaml`):
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `zeus profile describe --auto`). |
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.zeus/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
If you run `zeus dashboard --host 0.0.0.0`, every plugin route — kanban included — becomes reachable from the network. **Don't do that on a shared host.** The board contains task bodies, comments, and workspace paths; an attacker reaching these routes gets read access to your entire collaboration surface and can also create / reassign / archive tasks.

Tasks in `~/.zeus/kanban.db` are profile-agnostic on purpose (that's the coordination primitive). If you open the dashboard with `zeus -p <profile> dashboard`, the board still shows tasks created by any other profile on the host. Same user owns all profiles, but this is worth knowing if multiple personas coexist.
...
The plugin uses the standard ZeusAgent dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
zeus kanban init                                     # create kanban.db + print daemon hint
zeus kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
zeus kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
zeus kanban show <id> [--json]
zeus kanban assign <id> <profile>                    # or 'none' to unassign
zeus kanban reassign <id>... <profile>               # bulk re-assign tasks to a profile
zeus kanban edit <id> [--title ...] [--body ...]     # edit task title / body / priority in place
        [--priority N]
zeus kanban promote <id>...                          # move todo/blocked tasks to ready (recovery)
zeus kanban schedule <id> --at <ISO8601>             # set/clear a task's scheduled_at start time
zeus kanban diagnostics [--json]                     # board health snapshot (alias: diag)
zeus kanban link <parent_id> <child_id>
zeus kanban unlink <parent_id> <child_id>
zeus kanban claim <id> [--ttl SECONDS]
zeus kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
zeus kanban complete <id>... [--result "..."]
zeus kanban block <id> "<reason>" [--ids <id>...]
zeus kanban unblock <id>...
zeus kanban archive <id>...

zeus kanban request-review <id> [--summary "..."] [--metadata JSON] [--reviewer PROFILE]
zeus kanban request-changes <id> "<required changes>"               # active reviewer -> implementer
zeus kanban reopen-review  <id>... [--reason "..."]                 # changes requested: 'review' -> ready/todo

zeus kanban tail <id>                                # follow a single task's event stream
zeus kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
zeus kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
zeus kanban runs <id> [--json]                       # attempt history (one row per run)
zeus kanban assignees [--json]                       # profiles on disk + per-assignee task counts
zeus kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
zeus kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `zeus gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
zeus kanban stats [--json]                           # per-status + per-assignee counts
zeus kanban log <id> [--tail BYTES]                  # worker log from ~/.zeus/kanban/logs/
zeus kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
        [--chat-type dm|group|channel|thread] [--delivery-mode notify|notify+wake|wake]
zeus kanban notify-list [<id>] [--json]
zeus kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
zeus kanban context <id>                             # what a worker sees
zeus kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
zeus kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
```bash
zeus kanban create "nightly backup audit" \
...
Every `zeus kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `zeus chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `zeus_cli.kanban.run_slash()` entry point that reuses the `zeus kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `zeus kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.zeus/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Dispatcher workers creating tasks through `kanban_create` or `zeus kanban create`
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`zeus kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `zeus kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |

For worked examples of each, see `docs/zeus-kanban-v1-spec.pdf`.
...
_metadata_: `{"changed_files": ["zeus_cli/retry.py", "tests/test_retry.py"], "decisions": ["capped backoff at 60s", "jitter = full"]}`
```
...
zeus kanban create "Fix CI failure from t_impl: test_retry flakes on 3.11" \
    --assignee coder \
    --parent t_impl \
    --body "$(cat <<'EOF'
...
```
hotspot: zeus_cli/kanban_db.py — third conflicting edit to the dispatch loop this wave
```
...
```bash
zeus kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7 \
    --chat-type group --delivery-mode notify+wake
zeus kanban notify-list
zeus kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
zeus kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
zeus kanban runs t_abcd
...
**Bulk close caveat.** `zeus kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`zeus kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `zeus kanban heartbeat $TASK` to signal liveness during long operations. |
...
`zeus kanban tail <id>` shows these for a single task. `zeus kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.zeus/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.

## Design spec

The complete design — architecture, concurrency correctness, comparison with other systems, implementation plan, risks, open questions — lives in `docs/zeus-kanban-v1-spec.pdf`. Read that before filing any behavior-change PR.
