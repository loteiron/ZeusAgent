...
ZeusAgent can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended ZeusAgent integration. It
...
**Fresh installs already have the driver.** The ZeusAgent installer
...
- **`zeus tools`** → pick `🖱️  Computer Use` — installs the driver
...
```
zeus computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `zeus computer-use
status` to verify the install.

Already have cua-driver? ZeusAgent reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `zeus update`, and the first
`computer_use` call of a session, ZeusAgent checks the local version and
...
selected with `ZEUS_CUA_DRIVER_CMD` stays
under your control, so ZeusAgent reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. ZeusAgent autodetection is a
planned cua-driver follow-up, so currently point ZeusAgent at that directory or
...
interface. The built-in toolset provides ZeusAgent actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `zeus computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the ZeusAgent host identity. |
...
```
zeus -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.zeus/config.yaml`.

## Permission modes and logged-in browser profiles

ZeusAgent maps its existing approval UX onto cua-driver's immutable runtime
modes. Permission mode and capability manifest approval are launch settings.
They cannot change after the runtime starts:

| ZeusAgent session | cua-driver mode | Human intervention |
|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal ZeusAgent approvals; Cua stops at its protected boundary |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit ZeusAgent risk acceptance; no runtime Cua prompts |
...
computer_use:
  ...
  capability_manifest: ~/.zeus/cua-manifest.yaml
```
...
for the format). ZeusAgent launches a private runtime with
...
identity instead of resetting with every ZeusAgent build), and ZeusAgent verifies
...
resetting or closing the ZeusAgent session, cancellation cleanup, or process exit
closes that transport session. ZeusAgent also stops private runtimes that it
launched for bounded or unrestricted access. One ZeusAgent
conversation cannot change another runtime's mode or grants. Bounded and
unrestricted modes use a private
embedded service under the ZeusAgent host identity.
...
## `zeus computer-use doctor` — your first triage stop

`zeus computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ zeus computer-use doctor
...
ZeusAgent run declares a public cua-driver **session name** (something like
...
without it. ZeusAgent disables it automatically where it is a known failure
...
ZeusAgent keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the ZeusAgent-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. ZeusAgent
autodetection is a planned cua-driver follow-up, so currently point ZeusAgent at
...
These are **platform deep dives, not duplicates of the ZeusAgent skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **ZeusAgent
...
image capture also saves a bounded, shareable copy under ZeusAgent' image cache
...
ZeusAgent applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.zeus/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. ZeusAgent applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  ZeusAgent agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the ZeusAgent agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. ZeusAgent warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive ZeusAgent from inside the RDP / console
...
On Linux, native Wayland support remains an explicit opt-in. ZeusAgent passes the
...
```
ZEUS_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
ZEUS_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **ZeusAgent disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) ZeusAgent sets
...
When it's on, `zeus computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point ZeusAgent at a binary you built from source instead
of the published release. ZeusAgent resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`ZEUS_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for ZeusAgent testing (see notes).
...
### Option B — point ZeusAgent straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`ZEUS_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
ZEUS_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
ZEUS_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm ZeusAgent is using your build

- `zeus computer-use status` prints the resolved binary path and
  version.
- `zeus computer-use doctor` confirms the binary is reachable and
...
- **ZeusAgent spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  ZeusAgent YOLO, ZeusAgent instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no ZeusAgent
  change needed either way.
- **Local builds skip the version check.** ZeusAgent warns when the
...
**First action when anything's off: run `zeus computer-use doctor`.**
...
Run `zeus computer-use install` to fetch the cua-driver binary, or
run `zeus tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `zeus computer-use doctor` will
...
- **ZeusAgent-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  ZeusAgent `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. ZeusAgent autodetection is a
  planned follow-up; currently point ZeusAgent at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (ZeusAgent among them)
