...
| | |
|---|---|
| Source | Optional — install with `zeus skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research\darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), ZeusAgent |
| License | MIT |
...
import upstream classes into ZeusAgent itself.
...
```bash
mkdir -p ~/.zeus/cache/darwinian-evolver && cd ~/.zeus/cache/darwinian-evolver
...
```bash
cd ~/.zeus/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.zeus/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.zeus/skills/research/darwinian-evolver
DE_DIR=~/.zeus/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside ZeusAgent core.
   Custom driver scripts under `~/.zeus/skills/...` are user-side and fine.
