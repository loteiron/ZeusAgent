...
| | |
|---|---|
| Source | Optional — install with `zeus skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a ZeusAgent core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with ZeusAgent native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer ZeusAgent native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer ZeusAgent native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
Recommended ZeusAgent workflow:
...
## Recommended ZeusAgent usage patterns
...
- Do not silently choose Parallel when ZeusAgent native tools are already sufficient.
