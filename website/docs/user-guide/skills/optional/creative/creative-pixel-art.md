...
| | |
|---|---|
| Source | Optional — install with `zeus skills install official/creative/pixel-art` |
...
import sys
import os
sys.path.insert(0, os.path.expanduser("~/.zeus/skills/creative/pixel-art/scripts"))
...
```bash
cd ~/.zeus/skills/creative/pixel-art/scripts
...
- ffmpeg on PATH (only needed for video — ZeusAgent installs package this)
