...
| | |
|---|---|
| Source | Optional — install with `zeus skills install official/creative/pretext` |
...
This skill exists so ZeusAgent can make **cool demos** with it — the kind people post to X. See `pretext.cool` and `chenglou.me/pretext` for the community demo corpus.
