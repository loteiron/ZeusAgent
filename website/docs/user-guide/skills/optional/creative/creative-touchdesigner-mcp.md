...
| | |
|---|---|
| Source | Optional — install with `zeus skills install official/creative/touchdesigner-mcp` |
...
```
ZeusAgent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${ZEUS_HOME:-$HOME/.zeus}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to ZeusAgent config (if missing)
...
3. **Restart ZeusAgent session** to pick up the new MCP server
