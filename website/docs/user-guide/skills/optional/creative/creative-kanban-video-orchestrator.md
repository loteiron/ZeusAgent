...
| | |
|---|---|
| Source | Optional — install with `zeus skills install official/creative/kanban-video-orchestrator` |
...
short to a music video to an ASCII loop — in a ZeusAgent Kanban pipeline that
...
3. **Generates** a setup script that creates ZeusAgent profiles, project workspace, and the initial kanban task
...
For mapping role → which ZeusAgent skills + toolsets it loads, see
...
3. Creates each ZeusAgent profile via `zeus profile create --clone`
...
7. Fires the initial `zeus kanban create` task assigned to the director
...
```bash
zeus kanban watch --tenant <project-tenant>     # live events
zeus kanban list  --tenant <project-tenant>     # board snapshot
zeus dashboard                                   # visual board UI
```
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `zeus kanban create` call to a specialist profile. The kanban
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `${ZEUS_HOME:-~/.zeus}/.env` or the user's secret store.
