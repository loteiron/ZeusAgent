...
| | |
|---|---|
| Source | Optional — install with `zeus skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the ZeusAgent MCP catalog. The
...
Two halves, in this order: the editor side must be up before ZeusAgent connects.
...
### One-time, ZeusAgent side

    zeus mcp install unreal-engine
...
`~/.zeus/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for ZeusAgent — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. ZeusAgent connects
...
2. Start the ZeusAgent session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through ZeusAgent
they appear as:

| ZeusAgent tool | Purpose |
|---|---|
...
then happens at `zeus mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — ZeusAgent runs batched calls
...
- **Start order matters.** Editor + server up first, then the ZeusAgent
...
- **Timeouts on long operations.** ZeusAgent' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.zeus/config.yaml` for
