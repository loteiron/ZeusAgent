...
| | |
|---|---|
| Source | Optional — install with `zeus skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain\hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), ZeusAgent |
| License | MIT |
...
The script reads `${ZEUS_HOME:-~/.zeus}/.env` for two optional defaults:
...
Helper script: `~/.zeus/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
```bash
python ~/.zeus/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py <command> [args]
```
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${ZEUS_HOME:-~/.zeus}/.env`.
...
```bash
python ~/.zeus/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py dexs

python ~/.zeus/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py \
