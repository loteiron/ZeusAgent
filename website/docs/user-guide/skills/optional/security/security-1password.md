...
| | |
|---|---|
| Source | Optional — install with `zeus skills install official/security/1password` |
| Path | `optional-skills/security\1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by ZeusAgent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during ZeusAgent terminal calls (desktop app flow only)
...
### Service Account (recommended for ZeusAgent)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${ZEUS_HOME:-~/.zeus}/.env` (the skill will prompt for this on first load).
...
## ZeusAgent Execution Pattern (desktop app flow)

ZeusAgent terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/zeus-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/zeus-op.sock"
