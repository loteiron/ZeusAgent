---
title: "Actual Setup — Set up Actual Computer (actual.inc) inference in ZeusAgent"
sidebar_label: "Actual Setup"
description: "Set up Actual Computer (actual.inc) inference in ZeusAgent"
---
...
Set up Actual Computer (actual.inc) inference in ZeusAgent.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `zeus skills install official/devops/actual-setup` |
| Path | `optional-skills/devops\actual-setup` |
| Version | `2.0.0` |
| Author | shl0ms + ZeusAgent |
| License | MIT |
...
Sets up [actual.inc](https://actual.inc) (Actual Computer) as a ZeusAgent inference
...
- User has an `ac_` key and wants ZeusAgent routed through their Actual cluster.
...
- ZeusAgent has **first-class `actual` provider support** (provider id `actual`,
  aliases `actual-computer`, `actualcomputer`, `aci`). Do NOT configure Actual
  as a `custom_providers` / `providers.actual.*` entry on current ZeusAgent — the
...
1. Put the key in `.env` (secrets only — never config.yaml):
   append `ACTUAL_API_KEY=ac_...` to `~/.zeus/.env`.
...
3. Select provider + model:
   ```bash
   zeus config set model.provider actual
   zeus config set model.default "MODEL_ID_FROM_DISCOVERY"
   ```
4. Verify end-to-end:
   ```bash
   zeus chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL_ID
   ```
...
2. Download and load a model (scriptable once authorized):
   ...
   ```
3. Point ZeusAgent at the daemon. `ACTUAL_BASE_URL` with a loopback host flips the
   built-in provider into local no-auth mode automatically — no key needed:
   append `ACTUAL_BASE_URL=http://127.0.0.1:8080` to `~/.zeus/.env`, then:
   ```bash
   zeus config set model.provider actual
   zeus config set model.default "INSTALLED_MODEL_NAME"
   ```
4. Verify (reduced toolset — see context-window pitfall below):
   ```bash
   zeus chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m INSTALLED_NAME -t file,web
   ```
...
1. **reasoning_effort trap (handled by ZeusAgent since the first-class provider).**
   ...
   request still 400s this way on an old ZeusAgent, set a per-model cap:
   `agent.reasoning_overrides.<model>: high` in config.yaml.
2. **Context-window overflow on small local models.** ZeusAgent' default toolset
   ...
   servers emit a bare `data: [DONE]` — ZeusAgent reports
...
5. **Do not create a custom provider named `actual`.** Older setup guides
   ...
   current ZeusAgent the built-in provider wins the name; stale custom blocks
...
```bash
# Relay:
zeus chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL
# Local (small model — reduced toolset):
zeus chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m MODEL -t file,web
