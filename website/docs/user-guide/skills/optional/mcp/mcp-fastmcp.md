...
| | |
|---|---|
| Source | Optional — install with `zeus skills install official/mcp/fastmcp` |
...
| Related skills | [`zeus-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-zeus-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into ZeusAgent or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to ZeusAgent. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.zeus/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.zeus/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is ZeusAgent integration, either:

- configure the server in `~/.zeus/config.yaml` using the `native-mcp` skill, or
...
### ZeusAgent cannot see the deployed server

The server-building part may be correct while the ZeusAgent config is not. Load the `native-mcp` skill and configure the server in `~/.zeus/config.yaml`, then restart ZeusAgent.
