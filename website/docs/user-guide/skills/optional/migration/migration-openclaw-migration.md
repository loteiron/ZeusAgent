---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into ZeusAgent"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into ZeusAgent"
---
...
Import an OpenClaw setup (memories, skills) into ZeusAgent.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `zeus skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration\openclaw-migration` |
| Version | `1.0.0` |
| Author | ZeusAgent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `ZeusAgent`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> ZeusAgent Migration

Use this skill when a user wants to move their OpenClaw setup into ZeusAgent with minimal manual cleanup.
...
```bash
zeus claw migrate              # Full interactive migration
zeus claw migrate --dry-run    # Preview what would be migrated
zeus claw migrate --preset user-data   # Migrate without secrets
zeus claw migrate --overwrite  # Overwrite existing conflicts
zeus claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `zeus setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_zeus.py` to:

- import `SOUL.md` into the ZeusAgent home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into ZeusAgent memory entries
- merge OpenClaw command approval patterns into ZeusAgent `command_allowlist`
- migrate ZeusAgent-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to ZeusAgent working-directory configuration
- copy OpenClaw skills into `~/.zeus/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen ZeusAgent workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.zeus/tts/`
- archive non-secret docs that do not have a direct ZeusAgent destination
...
- `scripts/openclaw_to_zeus.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.zeus/skills/migration/openclaw-migration/scripts/openclaw_to_zeus.py`

Do not guess a shorter path like `~/.zeus/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.zeus/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of ZeusAgent-compatible secrets, currently:
...
ZeusAgent CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in ZeusAgent. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a ZeusAgent workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing ZeusAgent skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python ~/.zeus/skills/migration/openclaw-migration/scripts/openclaw_to_zeus.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.zeus/skills/migration/openclaw-migration/scripts/openclaw_to_zeus.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python ~/.zeus/skills/migration/openclaw-migration/scripts/openclaw_to_zeus.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.zeus/skills/migration/openclaw-migration/scripts/openclaw_to_zeus.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.zeus/skills/migration/openclaw-migration/scripts/openclaw_to_zeus.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.zeus/skills/migration/openclaw-migration/scripts/openclaw_to_zeus.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of ZeusAgent unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty ZeusAgent targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean ZeusAgent destination. Unsupported auth blobs must still be reported as skipped.
...
- ZeusAgent persona state imported
- ZeusAgent memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.zeus/skills/openclaw-imports/`
