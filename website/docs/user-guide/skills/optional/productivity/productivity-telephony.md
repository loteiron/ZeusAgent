...
| | |
|---|---|
| Source | Optional — install with `zeus skills install official/productivity/telephony` |
...
This optional skill gives ZeusAgent practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${ZEUS_HOME:-~/.zeus}/.env`
...
It does **not** turn ZeusAgent into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to ZeusAgent memory
...
### 1) "I want ZeusAgent to own a real phone number"
Use **Twilio**.
...
- pairs well with ZeusAgent `text_to_speech` plus a public file host or tunnel
...
```bash
SCRIPT="$(find ~/.zeus/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
zeus skills search telephony
zeus skills install official/productivity/telephony
```
...
Then save credentials into ZeusAgent:
...
3. Buy it and save it into `${ZEUS_HOME:-~/.zeus}/.env` + state:
```bash
python "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is ZeusAgent calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing ZeusAgent's existing `text_to_speech` support.

Use this when:
- you want the call to use ZeusAgent's configured TTS voice rather than Twilio `<Say>`
...
Recommended ZeusAgent TTS -> Twilio Play workflow:

1. Generate the audio with ZeusAgent `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask ZeusAgent to create the message audio with `text_to_speech`
...
Important note:
- ZeusAgent TTS is great for prerecorded outbound messages
...
- ZeusAgent STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to ZeusAgent memory.
...
- Do not store arbitrary third-party phone numbers in ZeusAgent memory.
...
3. persist that number to `${ZEUS_HOME:-~/.zeus}/.env`
