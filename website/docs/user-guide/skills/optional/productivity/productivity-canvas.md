...
| | |
|---|---|
| Source | Optional — install with `zeus skills install official/productivity/canvas` |
...
4. Name the token (e.g., "ZeusAgent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${ZEUS_HOME:-~/.zeus}/.env`:
...
```bash
CANVAS="python $ZEUS_HOME/skills/productivity/canvas/scripts/canvas_api.py"
