...
| | |
|---|---|
| Source | Optional — install with `zeus skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${ZEUS_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ZEUS_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${ZEUS_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client zeus
```
...
```bash
PUBLISH="${ZEUS_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client zeus
```
...
```bash
DRIVE="${ZEUS_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `zeus`)    |
