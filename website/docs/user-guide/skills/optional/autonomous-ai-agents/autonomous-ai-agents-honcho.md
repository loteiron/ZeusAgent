---
title: "Honcho — Configure and troubleshoot Honcho memory for ZeusAgent"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for ZeusAgent"
---
...
Configure and troubleshoot Honcho memory for ZeusAgent.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `zeus skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for ZeusAgent

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every ZeusAgent profile its own peer identity while sharing a unified view of the user.
...
```bash
zeus honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this ZeusAgent profile's AI peer
...
Honcho models conversations as interactions between **peers**. ZeusAgent creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this ZeusAgent instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each ZeusAgent run |
| `global` | Single session across all directories |

Manual override: `zeus honcho map my-project-name`
...
Each ZeusAgent profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block zeus.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `zeus.coder` host block in `honcho.json`
...
```bash
zeus honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for ZeusAgent when Honcho memory is active.
...
Config file: `$ZEUS_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `zeus honcho setup`. Ensure `memory.provider: honcho` is in `~/.zeus/config.yaml`.

### Memory not persisting across sessions
Check `zeus honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `zeus profile create <name> --clone`. For existing profiles: `zeus honcho sync`.
...
| Command | Description |
|---------|-------------|
| `zeus honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `zeus honcho status` | Show resolved config, connection test, peer info for active profile |
| `zeus honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `zeus honcho disable` | Disable Honcho for the active profile |
| `zeus honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `zeus honcho peers` | Show peer identities across all profiles |
| `zeus honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `zeus honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `zeus honcho sessions` | List known directory-to-session-name mappings |
| `zeus honcho map <name>` | Map current working directory to a Honcho session name |
| `zeus honcho identity` | Seed AI peer identity or show both peer representations |
| `zeus honcho sync` | Create host blocks for all ZeusAgent profiles that don't have one yet |
| `zeus honcho migrate` | Step-by-step migration guide from OpenClaw native memory to ZeusAgent + Honcho |
| `zeus memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `zeus memory status` | Show active memory provider and config |
| `zeus memory off` | Disable external memory provider |
