...
| List/filter by metadata (age, source, cost, tokens, workspace) | `zeus sessions list` / `stats` via terminal |
| Rename | `zeus sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `zeus sessions archive <filters>` |
| Delete (destructive) | `zeus sessions delete` / `zeus sessions prune <filters>` |
| Export before deleting anything valuable | `zeus sessions export --session-id <id> --format md` |
...
`zeus sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `zeus sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `zeus sessions list`
