...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`zeus-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-zeus-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the ZeusAgent terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For ZeusAgent itself, `model.provider: openai-codex` uses ZeusAgent-managed Codex
OAuth from `~/.zeus/auth.json` after `zeus auth add openai-codex`. For the
...
## ZeusAgent Gateway Caveat

When invoking the Codex CLI from a ZeusAgent gateway/service context (for example,
