...
description: Control your smart home with ZeusAgent via Home Assistant integration.
...
ZeusAgent integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "ZeusAgent"
5. Copy the token
...
ZeusAgent registers four tools for smart home control:
...
Configure which events the agent sees in `~/.zeus/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "ZeusAgent".
...
The adapter reads credentials from `~/.zeus/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active ZeusAgent profile
...
running ZeusAgent — `curl -H "Authorization: Bearer <token>" <url>/api/` should
