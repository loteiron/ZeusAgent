---
sidebar_position: 30
title: "ZeusAgent Relay"
description: "Connect ZeusAgent to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---

# ZeusAgent Relay (Connector)
...
ZeusAgent Relay is not a chat platform itself — it is a **connector system** that
...
secret. `zeus gateway enroll` redeems a **single-use enrollment token**
...
1. Resolves a fresh Nous Portal access token from your existing login
   (`~/.zeus/auth.json`) — this proves which Nous org (tenant) you own. If
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.zeus/.env`:
...
`zeus gateway enroll` refuses to run in managed/hosted installs — there the
...
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.zeus/.env`) | Connector relay WebSocket URL. Presence enables the relay platform. |
...
token. Re-login with `zeus auth add nous` (or `zeus setup`) and retry.
...
startup. Restart it (`zeus gateway restart`).
