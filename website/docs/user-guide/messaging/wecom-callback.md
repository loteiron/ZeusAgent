...
Connect ZeusAgent to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
ZeusAgent supports two WeCom integration modes:
...
> Run `zeus gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. ZeusAgent decrypts the message, queues it for the agent
...
(Use `zeus gateway start` only after `zeus gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in ZeusAgent and the token the
...
in `~/.zeus/.env` values around `=` will also break signature checks. After
fixing, restart `zeus gateway run`.
...
Check `zeus gateway run` logs for the bound host/port. If the adapter bound to
