...
Connect ZeusAgent to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between ZeusAgent and the BlueBubbles server
...
Or set environment variables directly in `~/.zeus/.env`:
...
By default, ZeusAgent responds to every authorized BlueBubbles/iMessage DM or group message. To make group chats opt-in, enable mention gating:
...
With `require_mention: true`, DMs still work normally, but group-chat messages are ignored unless they match a mention pattern. If you do not configure custom patterns, ZeusAgent uses conservative defaults for `ZeusAgent` and `@ZeusAgent agent` variants.
...
When someone messages your iMessage, ZeusAgent automatically sends them a pairing code. Approve it with:
```bash
zeus pairing approve bluebubbles <CODE>
```
Use `zeus pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.zeus/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.zeus/.env`):
```bash
...
ZeusAgent will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → ZeusAgent
ZeusAgent → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** ZeusAgent sends messages via the BlueBubbles REST API.
...
| `BLUEBUBBLES_MENTION_PATTERNS` | No | ZeusAgent wake words | JSON array, newline-separated, or comma-separated regex patterns for group mention matching |

Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/.zeus/config.yaml` (default: `true`). There is no corresponding environment variable.
...
### Chat Addressing
You can address chats by email or phone number — ZeusAgent resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `zeus logs gateway` for webhook errors (or `zeus logs -f` to follow in real-time)
