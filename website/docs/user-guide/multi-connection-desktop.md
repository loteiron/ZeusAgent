...
# Connecting Desktop to Many ZeusAgent Instances

Register every ZeusAgent backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and ZeusAgent Cloud instances — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another ZeusAgent gateway…"**) deep-links straight to
...
list of ZeusAgent gateways. Its intro says it plainly: *"Manage this device and
every ZeusAgent gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The ZeusAgent runtime managed by this app." | automatic |
| **Remote gateway** | "A ZeusAgent gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A ZeusAgent install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
| **ZeusAgent Cloud** | "A hosted instance discovered through your ZeusAgent Cloud account." | portal sign-in |
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the ZeusAgent Cloud sign-in/discovery flow at
  the top of the Gateways page — the **ZeusAgent Cloud** kind in the add-connection
...
3. Pick the kind: **Local**, **ZeusAgent Cloud**, **Remote gateway**, or **SSH**.
   (**Local** is disabled while the app-managed local entry exists — which is
   almost always; **ZeusAgent Cloud** directs you to the cloud sign-in/discovery
   flow above.)
4. Fill the fields:
   - **Name** — required, unique; the "device name" shown everywhere this
     instance appears (placeholder: `Homelab`). Max 64 characters.
   - *Remote gateway only:*
     - **Gateway URL** — the base URL of a running `zeus serve` backend,
...
Gateways are machines or hosted backends; profiles are isolated ZeusAgent agents
...
connection is registered) dispatches `zeus update` to every eligible
...
- **ZeusAgent Cloud** instances are skipped with a *"Managed by ZeusAgent Cloud"*
...
panel, ⌘K **Update ZeusAgent**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `zeus serve` is running on the remote host, the
...
- **"Update ZeusAgent Desktop to chat with agents on other connections"** — the
