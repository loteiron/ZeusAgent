...
ZeusAgent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.zeus/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
ZeusAgent stores session history so it can resume conversations, but it does not
...
ZeusAgent explicitly injects for that turn.
...
For example, if a user sends an image and asks ZeusAgent to make a meme from it,
ZeusAgent may inspect that image once with vision and run an image-processing
...
`zeus sessions prune` only when you want to delete old ended sessions from
storage. If `state.db` has simply grown large, start with the non-destructive
option first: `zeus sessions optimize` merges FTS5 index segments and
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`zeus` or `zeus chat`) |
...
A bare `-c` is terminal-aware: each CLI session drops a small breadcrumb file under `~/.zeus/terminal-sessions/` keyed by the terminal it runs in (tty device, tmux pane, kitty window, wezterm pane, Zellij pane, Windows Terminal session, ...). When you run `zeus -c` again in the *same* terminal, ZeusAgent resumes that terminal's own session — so two panes side by side each continue their own conversation instead of both grabbing the globally most-recent one. If there's no breadcrumb for the terminal (first use, deleted session, or a stale breadcrumb older than 30 days), `-c` falls back to the most-recent-session behavior. `-c "name"` and `--resume` are unaffected. Disable with `session.terminal_continue: false` in `config.yaml`.
...
zeus -c "my project"   # → resumes "my project #3"
```
...
zeus --resume 20250305_091523_a1b2c3d4
zeus -r 20250305_091523_a1b2c3d4

# Resume by title
zeus --resume "refactoring auth"

# Resume the most recent session — same lookup as -c
zeus --resume latest

# Or with the chat subcommand
zeus chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `zeus sessions list`.
...
zeus --resume latest --in ./my-project

# Works with the TUI too
zeus --tui --resume latest --in ./my-project
```
...
```bash
zeus --resume 20250305_091523_a1b2c3 --no-restore-cwd
```
...
`zeus sessions list` accepts `--workspace <needle>` to show only sessions whose workspace key (git repo root, else cwd) matches — by path substring or exact directory basename:

```bash
zeus sessions list --workspace my-project
zeus sessions list --workspace ~/code/zeus-agent
```

### Conversation Recap on Resume

When you resume a session, ZeusAgent displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a ZeusAgent session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.zeus/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `zeus -r "<title>"` from the shell) and pick up where the platform left off.
...
ZeusAgent automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `zeus sessions list` or `zeus sessions browse`.
...
```bash
zeus sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), ZeusAgent creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`zeus -c "my project"`), it automatically picks the most recent session in the lineage.
...
ZeusAgent provides a full set of session management commands via `zeus sessions`:
...
# Filter by platform
zeus sessions list --source telegram

# Show more sessions
zeus sessions list --limit 50
```
...
`zeus sessions export` is one surface for every export format, selected with `--format`:
...
zeus sessions export backup.jsonl

# Export sessions from a specific platform
zeus sessions export telegram-history.jsonl --source telegram

# Export a single session
zeus sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
zeus sessions export backup.jsonl --redact
```
...
zeus sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
zeus sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
zeus sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
zeus sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `zeus-traces` dataset (reads `HF_TOKEN`):

```bash
# Trace of the most recent session, to stdout
zeus sessions export --format trace

# One session to a local trace file
zeus sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
zeus sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.zeus/session-exports`).

```bash
# Export one session to Markdown
zeus sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
zeus sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
zeus sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
zeus sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
zeus sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
zeus sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
zeus sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
zeus sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
zeus sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
zeus sessions pin 20250305_091523_a1b2c3d4
zeus sessions pin 20250305 20250306

# Remove the pin
zeus sessions unpin 20250305_091523_a1b2c3d4

# List pinned sessions
zeus sessions pinned

# Machine-readable output, e.g. for a nightly backup of your pin set
zeus sessions pinned --json > pinned-sessions.json
```
...
zeus sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
zeus sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
zeus sessions prune --newer-than 5h

# Explicit window with absolute timestamps
zeus sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
zeus sessions prune --source telegram
zeus sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
zeus sessions prune --newer-than 5h --title "smoke test"   # title substring
zeus sessions prune --older-than 30 --max-messages 3        # tiny sessions
zeus sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
zeus sessions prune --model gpt-5 --older-than 1w           # by model (substring)
zeus sessions prune --provider openrouter --older-than 60   # by billing provider
zeus sessions prune --branch feature/old-experiment         # by git branch
zeus sessions prune --user 12345678 --chat-type group       # by messaging origin
zeus sessions prune --max-tokens 500 --older-than 7         # by token usage
zeus sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
zeus sessions prune --newer-than 5h --dry-run

# Skip confirmation
zeus sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `zeus sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `zeus sessions prune` keeps the 90-day cutoff. Every
...
`zeus sessions archive` takes the same filters as `prune` but soft-hides
...
zeus sessions archive --newer-than 5h

# Archive by title substring, preview first
zeus sessions archive --title "dry run" --dry-run
zeus sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `zeus sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`zeus sessions list` and `/resume` but remain in the database and can be
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`zeus insights`](/reference/cli-commands#zeus-insights).
...
`zeus sessions repair-routing` finds message-bearing session rows with no
...
zeus sessions repair-routing --apply

# Widen/narrow the contiguity window (default 900 seconds)
zeus sessions repair-routing --max-gap-seconds 300
```
...
(`cp ~/.zeus/state.db ~/.zeus/state.db.bak`).


## Importing Sessions from Claude Code and Codex CLI

Started a conversation in another agent CLI? You can pull it into ZeusAgent and
continue it here. ZeusAgent reads Claude Code's session logs
...
zeus sessions import

# Limit to one tool, or point at a specific file
zeus sessions import --from claude
zeus sessions import --from codex ~/.codex/sessions/2026/08/15/rollout-....jsonl
...
`zeus sessions import` creates a new ZeusAgent session titled
`Imported from Claude Code: <first user message>` (or Codex CLI) and prints
the id plus a ready-to-paste `zeus --resume <id>` command.
`--resume @claude` / `--resume @codex` show the same picker and drop you
straight into the imported conversation.

**ZeusAgent Desktop** has the same importer under **Import session** in the
...
read-only preview, and **Continue in ZeusAgent** copies the conversation into the
...
When ZeusAgent cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, ZeusAgent uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to ZeusAgent in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.zeus/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.zeus/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `gateway_routing` table in `~/.zeus/state.db` | Maps session keys to active session IDs (origin metadata, expiry flags) |
| Legacy routing mirror | `~/.zeus/sessions/sessions.json` | Backward-compat mirror of the routing index, written when `gateway.write_sessions_json: true` (the default) |
...
`state.db`; `~/.zeus/sessions/sessions.json` is a **legacy mirror** of it,
...
`zeus sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.zeus/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `zeus sessions list`, the cause is
`state.db` not receiving them — run `zeus sessions repair` and watch for a
...
`*.jsonl` files in `~/.zeus/sessions/`. They are no longer written or
read by ZeusAgent. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every ZeusAgent process in the same `ZEUS_HOME`

Without pruning, `state.db` grows without bound — multi-GB files within weeks were reported on gateway + cron installs. If you would rather keep every ended session forever (the pre-#54189 behavior), turn it off in `~/.zeus/config.yaml`:
...
`in its tip segment`). `zeus sessions export` still works for such sessions.
...
zeus sessions delete <session_id>
