...
description: "Run ZeusAgent directly on an Android phone with Termux"
---

# ZeusAgent on Android with Termux
...
ZeusAgent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop ZeusAgent from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
APT-managed ZeusAgent installs are marked with install method `apt`. ZeusAgent therefore does not run its Git self-updater against package-owned files; use the package manager instead:

```bash
pkg update
pkg upgrade zeus-agent
```

Packaging/repository/signing problems for this option should be reported to the community packaging repositories above. ZeusAgent runtime bugs can still be reported here, keeping in mind that Android/Termux support is best-effort.

---

## Option 1: One-line installer

ZeusAgent now ships a Termux-aware installer path:
...
- links `zeus` into `$PREFIX/bin` so it stays on your Termux PATH
...
ZeusAgent requires **Python >=3.11,&lt;3.14**. Current Termux ships `python`
...
### 5. Put `zeus` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/zeus" "$PREFIX/bin/zeus"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `zeus` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.zeus/.env`.
...
### `zeus doctor` says ripgrep or Node is missing
