...
description: "Diagnose and fix common ZeusAgent cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`zeus gateway` for foreground, or `zeus gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `zeus cron tick`.
...
```bash
date
zeus cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.zeus/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.zeus/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.zeus/.env` |
...
| `local` | Write access to `~/.zeus/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `zeus cron list` for updated `last_error` field (if available).
...
### Check 5: Relay-fronted platforms (ZeusAgent Cloud / Team Gateway)
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `zeus skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `zeus skills list`.
...
2. `~/.zeus/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `zeus cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the ZeusAgent config directory). Verify:
```bash
ls ~/.zeus/scripts/your-script.py   # Must exist
zeus cron edit <job_id> --script ~/.zeus/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `zeus skills install <skill-name>`.
...
Jobs are stored in `~/.zeus/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.zeus/cron/jobs.json
chmod 600 ~/.zeus/cron/jobs.json   # Your user should own it
```
...
```bash
zeus cron list                    # Show all jobs, states, next_run times
zeus cron run <job_id>            # Schedule for next tick (for testing)
zeus cron edit <job_id>           # Fix configuration issues
zeus logs                         # View recent ZeusAgent logs
zeus skills list                  # Verify installed skills
```
...
1. Run the job with `zeus cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.zeus/logs/agent.log` for scheduler messages and `~/.zeus/logs/errors.log` for warnings
