...
Copy-paste blueprints for common automation patterns. Each blueprint uses ZeusAgent's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`zeus webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `zeus webhook subscribe`) |
...
```bash
zeus webhook subscribe github-pr-review \
  --events "pull_request" \
...
3. For each merged PR, check if it modified:
   ...
   - CLI commands (zeus_cli/commands.py, zeus_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (zeus_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
zeus cron create "0 6 * * *" \
  "Run a dependency security audit on the zeus-agent project.

1. cd ~/.zeus/zeus-agent && source .venv/bin/activate
...
```bash
zeus webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
zeus webhook subscribe alert-triage \
...
```python title="~/.zeus/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "ZeusAgent-Monitor/1.0"})
...
zeus cron create "every 30m" \
  ...
  --script ~/.zeus/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
zeus cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to ZeusAgent development." \
...
```bash
zeus webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
zeus webhook subscribe stripe-payments \
...
```bash
zeus cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the zeus-agent codebase.
