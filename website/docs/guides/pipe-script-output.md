...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `zeus send`."
---

# Pipe Script Output to Messaging Platforms

`zeus send` is a small, scriptable CLI that pushes a message to any
messaging platform ZeusAgent is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | zeus send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `zeus
...
zeus send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
zeus send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
zeus send --list

# Filter by platform
zeus send --list telegram
```
...
Any platform ZeusAgent ships adapters for works as a target:
...
`zeus send` resolves the message body in this order:

1. **Positional argument** — `zeus send --to telegram "hi"`
2. **`--file PATH`** — `zeus send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | zeus send --to telegram`

When stdin is a TTY (no pipe), ZeusAgent does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  zeus send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `zeus send` reuses your ZeusAgent config, the same script works on
any host where ZeusAgent is installed — no need to export bot tokens into
...
`zeus send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  zeus send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | zeus send \
...
```bash
./train.py --epochs 200 && \
  zeus send --to telegram "training done" || \
  zeus send --to telegram "training failed (exit $?)"
```
...
zeus send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(zeus send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `zeus send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `zeus send` calls
the platform's REST endpoint directly using credentials from
`~/.zeus/.env` and `~/.zeus/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `zeus gateway start` and retry.
...
The listing is built from `~/.zeus/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`zeus gateway
...
| Approach | Multi-platform | Reuses ZeusAgent creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `zeus send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`zeus send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `zeus send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `zeus send` shares with cron delivery.
