...
This tutorial walks you through setting up a Telegram bot powered by ZeusAgent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **ZeusAgent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.zeus/.env`

:::tip
A $5/month VPS is plenty for running the gateway. ZeusAgent itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team ZeusAgent Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_zeus_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by ZeusAgent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting ZeusAgent Gateway...
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `zeus gateway install` to update the plist.
:::
...
```bash
zeus gateway stop && zeus gateway start
```
...
zeus pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.zeus/.env`:
...
Control how much detail the bot shows when using tools. In `~/.zeus/config.yaml`:
...
Customize how the bot communicates by editing `~/.zeus/SOUL.md`:

For a full guide, see [Use SOUL.md with ZeusAgent](/guides/use-soul-with-zeus).
...
```bash
# From the CLI
zeus cron list          # View all scheduled jobs
zeus cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u zeus-gateway` (Linux) or `~/.zeus/logs/gateway.log` (macOS) |
| Cron job output | `~/.zeus/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.zeus/cron/jobs.json` |
| Pairing data | `~/.zeus/pairing/` |
| Session history | `~/.zeus/sessions/` |
