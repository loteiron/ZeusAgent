...
description: "Find, install, use, and create skills — on-demand knowledge that teaches ZeusAgent new workflows"
---

# Working with Skills

Skills are on-demand knowledge documents that teach ZeusAgent how to handle specific tasks — from generating ASCII art to managing GitHub PRs. This guide walks you through using them day to day.
...
Every ZeusAgent installation ships with bundled skills. See what's available:
...
You can also trigger skills through natural conversation — ask ZeusAgent to use a specific skill, and it will load it via the `skill_view` tool.
...
Official optional skills ship with ZeusAgent but aren't active by default. Install them explicitly:

```bash
# Install an official optional skill
zeus skills install official/research/arxiv
...
What happens:
1. The skill directory is copied to `~/.zeus/skills/`
...
For how to ship skills in your own plugin, see [Build a ZeusAgent Plugin → Bundle skills](/developer-guide/plugins#bundle-skills).
...
When a skill with config is first loaded, ZeusAgent prompts you for the values. They're stored in `config.yaml` under `skills.config.*`.

Manage skill config from the CLI:

```bash
# Interactive config for a specific skill
zeus skills config gif-search

# View all skill config
zeus config get skills.config --json
```
...
```bash
mkdir -p ~/.zeus/skills/my-category/my-skill
```

### 2. Write SKILL.md

```markdown title="~/.zeus/skills/my-category/my-skill/SKILL.md"
---
name: my-skill
...
```bash
zeus chat -q "/my-skill help me with the thing"
```

The skill appears automatically — no registration needed. Drop it in `~/.zeus/skills/` and it's live.

:::info
The agent can also create and update skills itself using `skill_manage`. After solving a complex problem, ZeusAgent may offer to save the approach as a skill for next time.
:::
...
**Let the agent create skills.** After a complex multi-step task, ZeusAgent will often offer to save the approach as a skill. Say yes — these agent-authored skills capture the exact workflow including pitfalls that were discovered along the way.

**Use categories.** Organize skills into subdirectories (`~/.zeus/skills/devops/`, `~/.zeus/skills/research/`, etc.). This keeps the list manageable and helps the agent find relevant skills faster.

**Update skills when they go stale.** If you use a skill and hit issues not covered by it, tell ZeusAgent to update the skill with what you learned. Skills that aren't maintained become liabilities.
