...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  ZeusAgent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to ZeusAgent instantly when PRs are opened or updated.
:::
...
- **ZeusAgent installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  zeus gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.zeus/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure ZeusAgent can access GitHub. Start a chat:
...
Still in the chat, ask ZeusAgent to review a real PR:
...
A skill gives ZeusAgent consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.zeus/skills/code-review
```

Create `~/.zeus/skills/code-review/SKILL.md`:
...
Verify it loaded — start `zeus` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach ZeusAgent your team's standards:
...
2. Teach ZeusAgent your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
zeus gateway status    # Is the gateway running?
zeus cron list         # Is the job enabled?
```
