...
```bash
zeus teams-pipeline token-health
zeus teams-pipeline token-health --force-refresh
```
...
```bash
zeus teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
zeus teams-pipeline maintain-subscriptions
zeus teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: ZeusAgent cron (recommended if you already run the ZeusAgent gateway)

ZeusAgent ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.zeus/scripts/`. First create the script:

```bash
mkdir -p ~/.zeus/scripts
cat > ~/.zeus/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec zeus teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.zeus/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
zeus cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/zeus-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=ZeusAgent Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=zeus
EnvironmentFile=/etc/zeus/env
ExecStart=/usr/local/bin/zeus teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/zeus-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run ZeusAgent Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now zeus-teams-pipeline-maintain.timer
systemctl list-timers zeus-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/zeus teams-pipeline maintain-subscriptions >> /var/log/zeus/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.zeus/.env` at the top of a wrapper script that crontab calls.
...
```bash
zeus teams-pipeline subscriptions   # should show expirationDateTime advanced
zeus teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
zeus teams-pipeline list
zeus teams-pipeline list --status failed
zeus teams-pipeline show <job-id>
```

### Replay a stored job

```bash
zeus teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
zeus teams-pipeline fetch --meeting-id <meeting-id>
zeus teams-pipeline fetch --join-web-url "<join-url>"
zeus teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `zeus teams-pipeline maintain-subscriptions --dry-run`
- inspect `zeus teams-pipeline list --status failed`
...
- run `zeus teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `zeus teams-pipeline run`
...
- [ ] `zeus teams-pipeline validate` returns an OK snapshot
- [ ] `zeus teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (ZeusAgent cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
