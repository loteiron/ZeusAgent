...
[Download the ZeusAgent Desktop installer](https://hermes-agent.nousresearch.com/) for macOS or Windows, then open it. On first launch it finishes setting itself up (usually under a minute).

### 2. Connect Nous Portal

When the app opens, you'll see a "Let's get you set up" screen. Click **Nous Portal** (marked **Recommended**). Your browser opens — create a [Nous Portal](https://portal.nousresearch.com) account (or sign in), choose the **Free** plan, and authorize ZeusAgent. The app connects automatically.
...
Select **Quick Setup**. ZeusAgent opens a browser tab and waits for you to finish the next steps.
...
When prompted to connect your account to ZeusAgent, click **Connect**. You'll see a confirmation once it's linked.
...
- **Don't see the model in the list?** Make sure you finished the Nous Portal connection and that you're on the **Free** plan. In the CLI, `zeus portal info` confirms you're logged in and routing through Nous.
...
- **[Run ZeusAgent with Nous Portal](/guides/run-zeus-with-nous-portal)** — Full Portal walkthrough: models, Tool Gateway, and verification
