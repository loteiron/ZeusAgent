...
title: "Desktop Plugin SDK (@zeus/plugin-sdk)"
description: "Extend the native ZeusAgent Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [ZeusAgent Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `ZeusAgentPlugin`.
It imports one module — `@zeus/plugin-sdk` — and gets everything: the app's
...
`$ZEUS_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across ZeusAgent. This page is the **native
desktop app** (`zeus desktop`) SDK — the `@zeus/plugin-sdk` module and
`$ZEUS_HOME/desktop-plugins/`. The **web dashboard** (`zeus dashboard`) has
its own, unrelated plugin system on `window.__ZEUS_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a ZeusAgent Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$ZEUS_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$ZEUS_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `ZeusAgentPlugin` contract, appear in **Settings → Plugins**,
...
[`zeus-example-plugins`](https://github.com/NousResearch/hermes-example-plugins)
repo.

## Quick start — your first plugin

Create `$ZEUS_HOME/desktop-plugins/hello/plugin.js` (that's `~/.zeus/...`
by default, or `~/.zeus/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.zeus/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@zeus/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, ZeusAgent' }),
      jsx('div', {
...
The only importable specifiers are `@zeus/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `ZeusAgentPlugin`:

```ts
interface ZeusAgentPlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `zeus.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@zeus/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@zeus/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@zeus/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@zeus/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@zeus/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@zeus/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@zeus/plugin-sdk'
...
`window.zeus.send('get-price eth')` (or a declarative
`<button data-zeus-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@zeus/plugin-sdk'
...
for requests; `targetProfile` is the backend ZeusAgent profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from ZeusAgent
...
ctx.os.notify({
  ...
  // Body click → focus ZeusAgent + navigate. Same vocabulary as OS deep links:
  activate: 'zeus://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'zeus://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `zeus://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `zeus://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@zeus/plugin-sdk'
...
```javascript
import { queryClient } from '@zeus/plugin-sdk'
...
a package into `~/.zeus/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.zeus/plugins` is not reachable as a filesystem —
...
the `zeus://` scheme — a plain anchor on your website or README:
...
`dashboard/` subfolder of a regular ZeusAgent plugin and declare it in a
`manifest.json`:
...
zeus-agent codebase directly (`zeus_state`, `zeus_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.zeus/`)
...
(`zeus.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `ZeusAgentPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@zeus/plugin-sdk` alias.
2. It's still lint-fenced to `@zeus/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@zeus/plugin-sdk`, `react`,
...
| Plugin contract | `ZeusAgentPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `ZeusAgentOpenTarget`, `Contribution` |
...
### Agents: the `zeus-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`zeus-desktop-plugins`** skill — it carries the same contract as this page in
...
`$ZEUS_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `zeus logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@zeus/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.zeus/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.zeus/logs/errors.log` for
