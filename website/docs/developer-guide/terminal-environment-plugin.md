...
ZeusAgent runs shell commands through a pluggable set of **terminal backends**.
...
installed under `~/.zeus/plugins/`, registering a backend the user selects
...
| `zeus setup` backend picker | `display_name`, `description`, `setup_instructions()`, `post_setup()` |
| Dashboard terminal-backend picker (probe status) | `probe()` |
| `zeus status` / `zeus doctor` | `doctor_checks()` |
...
```python title="~/.zeus/plugins/acmebox/__init__.py"
...
class AcmeBoxProvider(TerminalEnvironmentProvider):
    ...
    def cache_path_base(self):
        return "~/.zeus"    # where synced cache files land, or None
...
```yaml title="~/.zeus/plugins/acmebox/plugin.yaml"
name: acmebox
version: 0.1.0
...
```bash
zeus plugins enable acmebox
zeus config set terminal.backend acmebox
```
