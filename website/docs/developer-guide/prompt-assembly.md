...
description: "How ZeusAgent builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

ZeusAgent deliberately separates:
...
2. **context** — caller-supplied `system_message`, project context files (`.zeus.md` / `AGENTS.md` / `CLAUDE.md` / `.cursorrules`), then the worktree-dependent git workspace snapshot, operator instructions and platform hints
...
the first host-info paragraph after the rendered `# ZeusAgent runtime environment` boundary, with a
...
```
# Layer 1: Agent Identity (from ~/.zeus/SOUL.md)
You are ZeusAgent, an AI assistant created by Nous Research.
...
The platform hint (Layer 10 above) is the per-surface guidance ZeusAgent
...
`SOUL.md` lives at `~/.zeus/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:

```python
# From agent/prompt_builder.py (simplified)
def load_soul_md() -> Optional[str]:
    soul_path = get_zeus_home() / "SOUL.md"
...
```
You are ZeusAgent, an independent fork of Hermes Agent by Nous Research. Be direct: match the length
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_zeus_md(cwd_path)       # 1. .zeus.md / ZEUS.md (walks to git root)
    ...
    # SOUL.md from ZEUS_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.zeus.md`, `ZEUS.md` | CWD up to git root | ZeusAgent-native project config |
...
- **YAML frontmatter stripped** — `.zeus.md` frontmatter is removed (reserved for future config overrides)
...
`pre_llm_call` plugin context also lands in this API-call-time path: it is appended to the current turn's **user message**, not written into the cached system prompt. When multiple plugins return context, ZeusAgent concatenates those context blocks (see [Hooks → `pre_llm_call`](../user-guide/features/hooks.md#pre_llm_call)).
...
1. `.zeus.md` / `ZEUS.md` (walks to git root)
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs ZeusAgent already loads, rather than editing Python templates in place.

### Use these surfaces first

- `~/.zeus/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.zeus/MEMORY.md` and `~/.zeus/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.zeus.md`, `ZEUS.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
- Skills — package reusable workflows and references without editing core prompt code.
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking ZeusAgent.
- Ephemeral overlays such as `ZEUS_EPHEMERAL_SYSTEM_PROMPT` or prefill messages — add turn-scoped guidance that should not become part of the cached prompt prefix.
...
- if you want to change how ZeusAgent assembles prompts for everyone, change Python and treat it as a code contribution
