...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept ZeusAgent already has applies. The plugin never sees a
...
zeus-agent itself). For the async surface (`acomplete()` /
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — ZeusAgent provider id (e.g. "openrouter")
...
using ZeusAgent built-in auxiliary tasks; it does not permit another plugin's
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different ZeusAgent provider
        # (must be one ZeusAgent already knows about — same names as
        # `zeus model` and config.yaml model.provider).
        ...
        # plugin sends — ZeusAgent does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.zeus/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through ZeusAgent' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing ZeusAgent subsystem:
