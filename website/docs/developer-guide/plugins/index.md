...
title: "Build a ZeusAgent Plugin"
description: "Step-by-step guide to building a complete ZeusAgent plugin with tools, hooks, data files, and skills"
---

# Build a ZeusAgent Plugin

This guide walks through building a complete ZeusAgent plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
ZeusAgent has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/user-guide/features/tts#voice-message-transcription-stt) — set `ZEUS_LOCAL_STT_COMMAND` to an argv-tokenized template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.zeus/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `zeus skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
```bash
zeus plugins install owner/repository --no-enable
zeus plugins list
zeus plugins enable <plugin-name>
```
...
ZeusAgent validates `plugin.json`, Agent Skills frontmatter, fixed component
...
profile-scoped writable directory managed by ZeusAgent.
...
Portable `streamable-http` entries are routed through ZeusAgent' existing native
...
same full-trust posture as other installed ZeusAgent plugins.
...
records it as Published. ZeusAgent keys behavior on the canonical v1.0.0 schema
...
not by one global plugin API number. ZeusAgent does not expose a
...
continue to work after a normal ZeusAgent upgrade.
...
- **Hook payloads are keyword payloads.** New hook data is added as keyword
  ...
  ZeusAgent inspects callback signatures: a legacy callback receives the fields it
...
- **Manifests are open to additions.** Unknown `plugin.yaml` fields are ignored.
  Older ZeusAgent releases can therefore load a plugin whose manifest contains
...
ZeusAgent enforces this contract with frozen external-plugin fixtures discovered
from an isolated `ZEUS_HOME`. Those tests load and invoke the plugin through
...
ZeusAgent's internals were split into `<stem>_<topic>` sibling modules in Sep 2026 (PR #102117). **Internal
...
- **Check your plugin:** `zeus plugins compat /path/to/your/plugin` lists every `file:line` with the
  old path and the new one, and exits 1 while any remain. `COMPAT_MANIFEST.md` in the repo is the full map.
- **What users see:** a notice under the CLI banner, in `zeus doctor` and after `zeus update`, and a
  one-time Desktop dialog naming the plugin. Each resolution through an old path also emits a
  `ZeusAgentPluginCompatWarning` once per process.
- **From 2026-09-14:** plugins that still import old paths are **not loaded** (the reason shows in
  `zeus plugins list`). Users can force-load with `plugins.allow_deprecated_imports: true` until the
...
```bash
mkdir -p ~/.zeus/plugins/calculator
cd ~/.zeus/plugins/calculator
```

### Validate with Plugin Doctor

`zeus plugins doctor [path-or-id]` runs the same directory discovery,
manifest parser, namespaced import, `register(ctx)`, hook registry, and tool
registry used by ZeusAgent itself. It reports invalid hook names, callbacks that do
...
```bash
zeus plugins doctor . --ci
```

Doctor uses a temporary `ZEUS_HOME`, restores plugin registration state after
...
This tells ZeusAgent: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
`llm.task_override` (see `zeus_cli/plugin_capabilities.py` for the
...
`zeus_agent.plugin_capabilities` entry-point group. Each declaration is
named `<plugin-id>.<capability-id>` and points at the same object as your
`zeus_agent.plugins` entry point:

```toml
[project.entry-points."zeus_agent.plugins"]
calculator = "my_pkg:register"

[project.entry-points."zeus_agent.plugin_capabilities"]
"calculator.tools.override" = "my_pkg:register"
```

ZeusAgent reads these from installed metadata without importing your code, so
`zeus plugins capabilities` and the consent flow stay accurate for pip
installs.
...
this ZeusAgent understands still loads with a warning.
...
| `python_dependencies` | list of str | Declared pip requirements (e.g. `"requests>=2.0,<3"`). **Declaration seam only** — ZeusAgent validates them, and `zeus plugins install` / `zeus plugins doctor` surface missing ones with a `pip install` hint, but ZeusAgent **never auto-installs** them. Pin upper bounds. |
...
arbitrary packages into ZeusAgent' shared venv is a conflict and supply-chain
...
4. **Accept `**kwargs`:** ZeusAgent may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `zeus my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but ZeusAgent continues fine
...
Use plugin-relative config keys for user-visible behavior. ZeusAgent resolves them
...
`<ZEUS_HOME>/plugin-data/`. Neither API exposes another plugin's namespace.
...
If your plugin doesn't show up — or shows up but isn't loading — set `ZEUS_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
ZEUS_PLUGINS_DEBUG=1 zeus plugins list
```
...
The same logs are always written to `~/.zeus/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
zeus logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `zeus plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout:** Native packages use `~/.zeus/plugins/<plugin-name>/plugin.yaml` (flat) or one category level. Portable packages use root `plugin.json` in the same locations. Anything deeper is ignored.
...
tree, and `zeus plugins update` / `remove` git-pull or delete it — your
...
# <zeus home>/plugin-data/<name>/ — created on first use
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.zeus/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.zeus/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `zeus plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing ZeusAgent into installing arbitrary packages — only specs ZeusAgent itself ships are eligible |
...
auditable in `~/.zeus/logs/agent.log`. Plugins load after built-in
...
does not ship with ZeusAgent core (user, project, or pip source),
...
plugins:
  entries:
    my-plugin:                    # the plugin's registry key from `zeus plugins list`
...
loader, the plugin is disabled and ZeusAgent continues. The gate exists
...
You normally never edit this key by hand. `zeus plugins enable <name>`
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), ZeusAgent injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers dark mode\n- Last project: zeus-agent"}
...
hooks:
  output_spill:
    ...
    # directory: null      # default: $ZEUS_HOME/hook_outputs
```
...
- **The system prompt is ZeusAgent's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
The canonical list of kinds is `VALID_MIDDLEWARE` in `zeus_cli/middleware.py`:
...
| `llm_request` | `request`, `original_request`, context kwargs | Return `{"request": {...}}` to replace the effective provider kwargs before ZeusAgent sends them. |
...
- Middleware payloads carry `middleware_schema_version` (`zeus.middleware.v1`) alongside the observer telemetry fields.
- Unknown kinds register with a warning instead of failing, so a plugin written against a newer ZeusAgent still loads on an older one.

### Register CLI commands

Plugins can add their own `zeus <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for zeus my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: zeus my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for zeus my-plugin."""
...
After registration, users can run `zeus my-plugin status`, `zeus my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `zeus name` in a terminal |
...
`ctx._cli_ref` is only populated in an **interactive CLI** session. It is `None` in the gateway, in non-interactive `zeus chat -q` runs, and in **kanban-spawned worker sessions** — so any plugin logic that reaches through `_cli_ref` silently no-ops in exactly those contexts. Two stable, session-agnostic APIs cover what hooks actually need:

- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `ZEUS_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
For running a full `zeus <subcommand>` (e.g. `zeus kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "zeus kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive ZeusAgent from a hook.
...
ZeusAgent has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.zeus/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `zeus gateway status` without SDK instantiation.
...
ZeusAgent also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into ZeusAgent without any Python plugin. Declare them in `~/.zeus/config.yaml`:
...
ZeusAgent connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.zeus/hooks/<name>/`:

```yaml
# ~/.zeus/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.zeus/hooks/long-task-alert/handler.py
...
zeus skills search my-workflow --source myorg/skills-repo
zeus skills install myorg/skills-repo/my-workflow
```
...
```bash
pip install zeus-plugin-calculator
# Plugin auto-discovered on next zeus startup
```
...
```python
# Wrong — will break if ZeusAgent passes extra context
def handler(args):
    ...
