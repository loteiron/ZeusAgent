...
sidebar_position: 1
title: "Architecture"
description: "ZeusAgent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of ZeusAgent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ zeus_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # ZeusAgentCLI facade — mixins in zeus_cli/cli_*_mixin.py
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── zeus_state.py           # SQLite session/state database facade (+ zeus_state_*.py siblings)
├── zeus_constants.py       # ZEUS_HOME, profile-aware paths
...
│
├── zeus_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — `zeus` subcommands (parsers in subcommands/, main_*.py)
...
│   ├── skills_config.py      # zeus skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # zeus tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # zeus gateway start/stop
│
...
```text
User input → ZeusAgentCLI.process_input()
...
Three discovery sources: `~/.zeus/plugins/` (user), `.zeus/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `zeus plugins` or `config.yaml`.
...
Exposes ZeusAgent as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`zeus -p <name>`) gets its own ZEUS_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
