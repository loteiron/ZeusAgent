...
sidebar_position: 15
title: "CLI Internals"
description: "How zeus_cli is shaped: slash dispatch, config loaders, the skin engine, the transactional update pipeline, and process-identity rules"
---

# CLI Internals

Companion to `zeus_cli/AGENTS.md` (the rules) — this page holds the longer explanations.

## Update pipeline

The stage-by-stage contract (`plan → snapshot → apply → restart-per-kind → verify → report`) and the
field failure each stage guards are documented in `zeus_cli/AGENTS.md`; user-facing behaviour
...
- Use the canonical matchers: `gateway.status.looks_like_gateway_command_line` (gateway run),
  `zeus_cli.update_cmd._zeus_holder_subcommand` (top-level subcommand of any ZeusAgent argv). Never
...
Built-in skins (`_BUILTIN_SKINS` in `zeus_cli/skin_engine.py`): `default` (classic gold/kawaii),
...
User skins are `~/.zeus/skins/<name>.yaml` with the same keys, activated with `/skin <name>` or
`display.skin: <name>`; the full YAML template is in the
[Skins & Themes](../user-guide/features/skins.md) user guide.

## Profiles: multi-instance support

ZeusAgent supports profiles — fully isolated instances, each with its own `ZEUS_HOME` (config, API
keys, memory, sessions, skills, gateway). `_apply_profile_override()` in `zeus_cli/main.py` sets
`ZEUS_HOME` before any module imports, so every `get_zeus_home()` reference scopes to the active
profile. Profile operations are HOME-anchored (`_get_profiles_root()` returns
`Path.home() / ".zeus" / "profiles"`, not `get_zeus_home() / "profiles"`) so
`zeus -p coder profile list` sees all profiles regardless of which one is active — intentional.
