...
ZeusAgent uses a SQLite database (`~/.zeus/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source files: `zeus_state.py` (facade) plus the `zeus_state_*.py` siblings (schema, fts, search, compression, portability, gateway, ...)

### Desktop profile isolation and compaction generations

Each named profile stores its transcript in its own `$ZEUS_HOME/state.db`,
including when one `zeus serve` process serves several profiles. In-session
...
splice boundary, ZeusAgent excludes only that leading item when it exactly matches
...
`zeus sessions recover` copies the row-bearing tables above into the
...
Abridged — see `SCHEMA_SQL` in `zeus_state_common.py` (applied by `zeus_state_schema.py`) for the full current column list
...
indexed columns — see `SCHEMA_SQL` in `zeus_state_common.py` for the exact SQL.
...
Multiple zeus processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.zeus/state.db
...
Default path: `~/.zeus/state.db`

This is derived from `zeus_constants.get_zeus_home()` which resolves to
`~/.zeus/` by default, or the value of `ZEUS_HOME` environment variable.
