...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to ZeusAgent — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in ZeusAgent tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying ZeusAgent core, use the plugin route instead:

- [Plugins](/user-guide/features/plugins)
- [Build a ZeusAgent Plugin](/developer-guide/plugins)
...
2. **`toolsets.py`** — add tool name to `_ZEUS_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `zeus_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `zeus_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `zeus chat -q "Use the weather tool for London"`
