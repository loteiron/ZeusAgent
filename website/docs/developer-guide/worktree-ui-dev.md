...
| Variable | Meaning |
|----------|---------|
| `ZEUS_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `ZEUS_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `ZEUS_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by ZeusAgent itself — they're private to these helpers. The variables ZeusAgent *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `zeus --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_zeus_root)" || { echo "htui: not in a ZeusAgent checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$ZEUS_MAIN_CHECKOUT/.venv/bin/python" -m zeus_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `ZEUS_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_zeus_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `ZEUS_TUI_DIR` are mutually exclusive
`ZEUS_TUI_DIR` points ZeusAgent at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `zeus --tui --dev` exits with an error. Run `unset ZEUS_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_zeus_root)" || { echo "hgui: not in a ZeusAgent checkout" >&2; return 1; }
  deps="${ZEUS_GUI_DEPS_CHECKOUT:-$ZEUS_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _zeus_link_deps "$desktop" "$deps/apps/desktop"
    _zeus_link_deps "$root" "$deps"
  else
  ...
  trap '_zeus_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    ZEUS_DESKTOP_ZEUS_ROOT="$root" \
    ZEUS_DESKTOP_PYTHON="$ZEUS_MAIN_CHECKOUT/.venv/bin/python" \
    ZEUS_DESKTOP_IGNORE_EXISTING=1 \
    ZEUS_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `ZEUS_DESKTOP_ZEUS_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `zeus`. |
| `ZEUS_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `ZEUS_DESKTOP_IGNORE_EXISTING` | Ignores any `zeus` on `PATH` so it can't shadow the worktree. |
| `ZEUS_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real ZeusAgent checkout.
_zeus_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/zeus_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_zeus_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'zeus_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `zeus --tui --dev` and the `ZEUS_TUI_DIR` prebuild path
...
- [Environment Variables](../reference/environment-variables.md) — every `ZEUS_*` variable ZeusAgent reads
