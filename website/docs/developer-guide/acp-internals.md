...
The ACP adapter wraps ZeusAgent' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
zeus acp / zeus-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.zeus/.env
  -> configure stderr logging
  -> construct ZeusAgentACPAgent
...
- `allow_once` -> ZeusAgent `once`
- `allow_always` -> ZeusAgent `always`
- reject options -> ZeusAgent `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps ZeusAgent tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["zeus-acp"])
...
Instead it reuses ZeusAgent' runtime resolver:

- `acp_adapter/auth.py`
- `zeus_cli/runtime_provider.py`

So ACP advertises and uses the currently configured ZeusAgent provider/credentials. It also always advertises a terminal setup auth method (`zeus-setup`, args `--setup`) so first-run ACP clients can open ZeusAgent' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.zeus/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `zeus-acp` toolset definition
- `zeus_cli/main.py` — `zeus acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `zeus-acp` script
