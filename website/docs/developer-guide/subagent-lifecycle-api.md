...
Plugins can launch and supervise fresh ZeusAgent child sessions without importing
...
active agent turn fails closed with `No active ZeusAgent parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until ZeusAgent can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; ZeusAgent's existing unsafe-tool block remains enforced.
