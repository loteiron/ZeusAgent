...
ZeusAgent uses a dual compression system and Anthropic prompt caching to
...
Configure via `zeus plugins` → Provider Plugins → Context Engine, or edit `config.yaml` directly.

For building a context engine plugin, see [Context Engine Plugins](/developer-guide/context-engine-plugin).

## Dual Compression System

ZeusAgent has two separate compression layers that operate independently:
...
value is kept per `model@host` in `~/.zeus/cache/image_token_costs.json` and
...
compression:
  ...
  codex_app_server_auto: native  # native|zeus|off for Codex app-server thread compaction
...
| `codex_app_server_auto` | `native` | `native`, `zeus`, `off` | Thread-compaction mode for Codex app-server sessions (see below) |
...
excluded because they already unlock the wider window), ZeusAgent raises the
trigger to **85%** (~231K) and shows a notice with the opt-out command. The
notice is shown once per profile — a marker under `$ZEUS_HOME`
...
```bash
zeus config set compression.codex_gpt55_autoraise false
```

To keep the 85% autoraise but hide only the one-time notice:

```bash
zeus config set compression.codex_gpt55_autoraise_notice false
```
...
for ChatGPT-subscription accounts (live-verified Aug 2026). ZeusAgent keeps the
...
`gpt-5.4-900k`). These are ZeusAgent-side aliases: the suffix is stripped before
...
thread context, so ZeusAgent' auxiliary summarizer cannot shrink it — rewriting the
...
- Automatic compaction is controlled by `compression.codex_app_server_auto`:
  the default `native` lets the app-server decide when to compact and ZeusAgent
  records the resulting compaction events (compression counters, session
  events). Set `zeus` to let ZeusAgent' compression threshold initiate
  app-server compaction, or `off` to disable ZeusAgent-initiated automatic
  compaction entirely (codex may still compact natively).

ZeusAgent' local transcript is never rewritten on this runtime — state.db records
the compaction boundary while the visible transcript stays intact. All other
routes (including Codex OAuth chat sessions) keep ZeusAgent' summary compressor.
...
encrypted `compaction` output item. ZeusAgent captures that item into the
...
Anthropic allows a maximum of 4 `cache_control` breakpoints per request. ZeusAgent
...
5. **Model identity is part of the cache key**: Provider-side caches are scoped
   ...
   ZeusAgent can avoid; user-facing docs for `/model`, fallback providers, and
