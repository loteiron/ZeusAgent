...
def render_skill_page(
    ...
    zeus_meta = (fm.get("metadata") or {}).get("zeus") or {}
    tags = zeus_meta.get("tags") or []
    related = zeus_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `zeus skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with ZeusAgent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "ZeusAgent ships with a large built-in skill library copied into `~/.zeus/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "ZeusAgent also syncs bundled skills on `zeus update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.zeus/skills/` tree, it is still shipped with ZeusAgent; restore it with `zeus skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with zeus-agent — install via zeus skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with zeus-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "zeus skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "zeus skills install official/blockchain/solana",
        "zeus skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "zeus skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
