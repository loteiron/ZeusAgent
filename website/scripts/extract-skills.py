...
2. The unified ZeusAgent Skills Index at ``website/static/api/skills-index.json``,
    ...
    for base_dir, source_label in LOCAL_SKILL_DIRS:
        ...
        for root, _dirs, files in os.walk(base_path):
            ...
            if isinstance(metadata, dict):
                zeus_meta = metadata.get("zeus", {})
                if isinstance(zeus_meta, dict):
                    tags = zeus_meta.get("tags", [])
            if not tags:
                tags = fm.get("tags", [])
