...
# 5. Symlinks the 'zeus' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚡ ZeusAgent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/zeus-uv-install.$$.log")"
...
if is_termux; then
    # ZeusAgent currently declares requires-python >=3.11,<3.14. Termux can expose
    ...
    if [ -z "${PYTHON_PATH:-}" ]; then
        if command -v python >/dev/null 2>&1; then
            ...
            echo -e "${RED}✗${NC} Termux Python $PYTHON_FOUND_VERSION is not supported; ZeusAgent requires Python >=3.11,<3.14"
...
# ============================================================================
# PATH setup — symlink zeus into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up zeus command..."

ZEUS_BIN="$SCRIPT_DIR/venv/bin/zeus"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ZEUS_BIN" "$COMMAND_LINK_DIR/zeus"
...
# ============================================================================
# Seed bundled skills into ~/.zeus/skills/
# ============================================================================

ZEUS_SKILLS_DIR="${ZEUS_HOME:-$HOME/.zeus}/skills"
mkdir -p "$ZEUS_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.zeus/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ZEUS_SKILLS_DIR/" 2>/dev/null || true
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m zeus_cli.main setup
fi
