...
The ``zeus update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m zeus_cli.main gateway run`` — image name
    ``pythonw``, not ``zeus.exe`` — so the ``taskkill /IM zeus.exe`` guard
    ...
    # The zeus.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM zeus.exe' in text
