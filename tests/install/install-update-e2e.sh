...
#
# Installs a real, earlier ZeusAgent the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `zeus`.
#
...
# Keep sandbox state out of the default .zeus-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".zeus-sandbox-e2e-$ROUTE"
export ZEUS_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/zeus/.zeus/zeus-agent"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/zeus-agent.git"
...
LOG_DIR="${ZEUS_E2E_LOG_DIR:-$(mktemp -d -t zeus-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED zeus accept FLAG on `zeus update`?
#
...
# that silently rots. `zeus update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "zeus update --help 2>&1" | grep -qF -- "$flag"
}
...
require_zeus_works() {
  local when="$1" out
  out="$(in_sandbox "zeus --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "zeus --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "zeus runs $when"
}

# ── install the earlier ZeusAgent ─────────────────────────────────────────────
...
require_zeus_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: zeus update'
    ...
    # zeus which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="zeus update --yes"
    else
      update_cmd="zeus update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "zeus update failed ($update_cmd)"
    fi
    require_landed_on_target 'zeus update'
    require_zeus_works 'after zeus update'
    ;;
  installer)
    ...
    require_zeus_works 'after installer re-run'
    ;;
esac
