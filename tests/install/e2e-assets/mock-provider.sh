...
#   mock_start <workroot>      start, write config into $ZEUS_HOME
#   mock_stop                  kill the background server
#
# Requires: ASSETS (e2e-assets dir), LOG_DIR, ZEUS_HOME, ok/fail helpers.
...
EOF
  ...
  ok "provider 'mock' configured in $ZEUS_HOME (api $url/v1)"
}
