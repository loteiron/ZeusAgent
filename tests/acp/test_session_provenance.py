...
def test_root_session_no_compression(db):
    ...
    assert prov["currentZeusAgentSessionId"] == "root1"
    assert prov["rootZeusAgentSessionId"] == "root1"
    assert prov["parentZeusAgentSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_zeus_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentZeusAgentSessionId"] == "old"
    assert prov["rootZeusAgentSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousZeusAgentSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootZeusAgentSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"zeus"}
    assert "sessionProvenance" in meta["zeus"]
    assert meta["zeus"]["sessionProvenance"]["currentZeusAgentSessionId"] == "root1"
