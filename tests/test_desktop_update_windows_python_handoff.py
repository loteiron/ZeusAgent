...
`scripts/desktop-update/windows.ps1` drives `zeus update` for the in-app
Desktop updater. It used to invoke the update through the venv's
`venv\\Scripts\\zeus.exe` console-script launcher. On Windows that launcher is
a real process that keeps `zeus.exe` mapped as its running image and spawns
`python.exe` as a child. The update ends in `uv pip install -e .`, which rewrites
the console-script shims -- including the `zeus.exe` the launcher still has
...
Start Menu shortcut points at a `ZeusAgent.exe` that no longer exists.

Driving the update as `python.exe -m zeus_cli.main update` puts the inherited
...
hand-off. The invariant it guards is that every `Invoke-ZeusAgentStep` call site
(the update, its retry, and the desktop rebuild) drives `$pythonExe`, never the
`$zeusExe` shim. `zeus.exe` may still be *named* in the file for the
...
def _handoff_source() -> str:
    ...
    fixture runs a synthetic PowerShell step through ``Invoke-ZeusAgentStep`` to
...
def test_invoke_zeus_step_calls_drive_python_not_the_shim() -> None:
    source = _handoff_source()

    invocations = re.findall(r"Invoke-ZeusAgentStep\s+(\$\w+)", source)
    assert invocations, (
        "Expected at least one Invoke-ZeusAgentStep call in "
    ...
    assert not offenders, (
        "Every Invoke-ZeusAgentStep call in scripts/desktop-update/windows.ps1 "
        "must drive $pythonExe, not the zeus.exe shim. Driving the update "
        "through the shim keeps zeus.exe mapped as a running image, so uv's "
...
def test_update_invocation_uses_module_entrypoint() -> None:
    ...
    assert '@("-m", "zeus_cli.main", "update"' in source, (
        "The update step must invoke `python.exe -m zeus_cli.main update ...` "
        "so the inherited image handle lands on python.exe, which uv never has "
        "to replace."
    )
    assert (
        '@("-m", "zeus_cli.main", "desktop", "--force-build", "--build-only")'
        in source
    ), (
        "The desktop rebuild step must also go through "
        "`python.exe -m zeus_cli.main desktop ...` for the same reason."
    )


def test_update_no_longer_invokes_the_zeus_exe_shim() -> None:
    source = _read()

    assert "Invoke-ZeusAgentStep $zeusExe" not in source, (
        "scripts/desktop-update/windows.ps1 still invokes the update through "
        "the zeus.exe shim (`Invoke-ZeusAgentStep $zeusExe`). That is the "
