"""Tests for zeus_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every ZeusAgent entry point
(zeus, zeus-agent, zeus-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every ZeusAgent entry point imports zeus_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported zeus_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("zeus_bootstrap", None)
    import zeus_bootstrap  # noqa: WPS433
    return zeus_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the ZeusAgent CLI
...
class TestEntryPointsImportBootstrap:
    """Every ZeusAgent entry point must import zeus_bootstrap as its
    ...
    # Entry points that invoke ZeusAgent as a process.  Each one must
    # import zeus_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "zeus_cli/main.py",   # zeus CLI (console_script)
        "run_agent.py",          # zeus-agent (console_script)
        "acp_adapter/entry.py",  # zeus-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import zeus_bootstrap' and that
        ...
        half-finished ``zeus update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``zeus_bootstrap`` as a top-level module) leaves zeus
        ...
        """
        # Resolve relative to the zeus-agent repo root.  Tests live
        # at tests/test_zeus_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # zeus start even when zeus_bootstrap hasn't been
        ...
        assert first_import_name == "zeus_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'zeus_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import zeus_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing ZeusAgent's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("ZEUS_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["ZEUS_PYTHON_SRC_ROOT"] = env
            elif "ZEUS_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["ZEUS_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/zeus")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ZEUS_PYTHON_SRC_ROOT", None)
            else:
                os.environ["ZEUS_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/zeus", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # ZeusAgent root.  The guard must relocate ZeusAgent to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/zeus"])
        assert result[0] == "/opt/zeus"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the ZeusAgent root.
        assert result.index("/opt/zeus") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["ZEUS_PYTHON_SRC_ROOT"] = "/env/zeus"
            hb.harden_import_path()
            assert sys.path[0] == "/env/zeus"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ZEUS_PYTHON_SRC_ROOT", None)
