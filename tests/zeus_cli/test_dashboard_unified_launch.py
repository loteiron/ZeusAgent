...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "zeus_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # ZEUS_HOME.  For a standard install (ZEUS_HOME unset) that root is
        # the platform-native default (~/.zeus), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from zeus_constants import get_default_zeus_root
        assert env.get("ZEUS_HOME") == str(get_default_zeus_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (ZEUS_DESKTOP=1) must NOT
