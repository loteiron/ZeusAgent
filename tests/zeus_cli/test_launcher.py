"""Tests for the top-level `./zeus` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./zeus` should use `zeus_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "zeus"
    called = []

    fake_main_module = types.ModuleType("zeus_cli.main")

    def fake_main():
        called.append("zeus_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "zeus_cli.main", fake_main_module)
