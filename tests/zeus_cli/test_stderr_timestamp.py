"""Tests for zeus_cli.stderr_timestamp."""

import re
import sys

from gateway.restart import EXTERNAL_GATEWAY_SUPERVISOR_ENV
from zeus_cli import stderr_timestamp
...
_LAUNCHD_ENV = {"PATH": "/usr/bin", "XPC_SERVICE_NAME": "ai.zeus.gateway-butler"}
...
def test_main_injects_flag_into_stale_gateway_child(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("XPC_SERVICE_NAME", "ai.zeus.gateway-butler")
    ...
    stale = [sys.executable, "-c", code, "-m", "zeus_cli.main", "gateway", "run", "--replace"]
