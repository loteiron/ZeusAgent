...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (ZEUS_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via ZEUS_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in ZEUS_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from zeus_cli.providers import ZEUS_OVERLAYS
        assert "bedrock" in ZEUS_OVERLAYS
