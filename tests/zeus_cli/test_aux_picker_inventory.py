...
Every aux picker (``zeus model`` → Configure auxiliary models, the
``zeus tools`` vision picker, and any future one) must route through
``zeus_cli.inventory.build_aux_picker_rows()`` so it shows the same
...
import pytest
import yaml
from zeus_cli import main_provider_setup
...
def configured_home(tmp_path, monkeypatch):
    """A ZEUS_HOME with one ``providers:`` entry and one legacy
...
def test_aux_picker_surfaces_user_defined_providers(configured_home):
    ...
    """
    from zeus_cli.inventory import build_aux_picker_rows
...
def test_aux_picker_requests_exhausted_pool_visibility(configured_home):
    ...
    with patch("zeus_cli.model_switch.list_authenticated_providers", _capture):
...
def test_aux_pickers_route_through_the_shared_substrate(configured_home):
    ...
    with (
        patch("zeus_cli.model_switch.list_authenticated_providers", _direct),
        patch("zeus_cli.inventory.build_aux_picker_rows", _substrate),
        patch("zeus_cli.main_provider_setup._prompt_provider_choice", return_value=None),
    ):
    ...
    assert direct_calls == [], (
        ...
        "route through zeus_cli.inventory.build_aux_picker_rows so custom "
