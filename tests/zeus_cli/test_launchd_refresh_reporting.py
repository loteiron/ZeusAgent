...
def _stale_plist(tmp_path, monkeypatch, *, registered: bool):
    plist_path = tmp_path / "com.zeus.plist"
    ...
    monkeypatch.setattr(gw, "get_launchd_label", lambda: "com.zeus.agent")
...
def test_install_repair_warns_instead_of_claiming_success(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("zeus_constants.display_zeus_home", lambda: "~/.zeus-work")
    ...
    assert "~/.zeus-work/logs/launchd-reload.log" in out
