"""Regression tests for #34107 — Docker UID/GID handling in ensure_zeus_home.

When ZeusAgent runs in Docker with ``ZEUS_UID=1000`` / ``ZEUS_GID=911``,
the entrypoint chowns the top-level ``ZEUS_HOME`` once at startup. But
subdirectories created at runtime by ``ensure_zeus_home()`` — especially
...
The fix is a ``_chown_to_zeus_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_zeus_uid_gid
# ---------------------------------------------------------------------------


class TestResolveZeusAgentUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("ZEUS_UID", "1000")
        monkeypatch.setenv("ZEUS_GID", "911")
        from zeus_cli.config import _resolve_zeus_uid_gid
...
class TestChownToZeusAgentUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_zeus_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("ZEUS_UID", raising=False)
        monkeypatch.delenv("ZEUS_GID", raising=False)
        from zeus_cli import config as cfg
