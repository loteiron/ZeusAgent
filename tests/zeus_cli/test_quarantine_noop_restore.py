...
``zeus*.exe`` shims aside (``zeus.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``zeus`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("zeus", "zeus-agent", "zeus-acp", "zeus-gateway"):
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "zeus.exe" in names, "zeus.exe must be restored after a no-op install"
    assert "zeus-acp.exe" in names
    assert "zeus-gateway.exe" in names
    assert (scripts / "zeus.exe").read_bytes() == b"MZ-old-zeus"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "zeus.exe").read_bytes() == b"MZ-new-zeus"
...
# ---------------------------------------------------------------------------
# zeus_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "zeus.exe" in names, "zeus.exe must be restored after a no-op recovery install"
    assert (scripts / "zeus.exe").read_bytes() == b"MZ-old-zeus"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "zeus.exe").write_bytes(b"MZ-new-zeus")
