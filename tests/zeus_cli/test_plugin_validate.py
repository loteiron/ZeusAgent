"""Tests for ``zeus plugins validate`` (zeus_cli/plugin_validate.py).
...
def test_requires_zeus_spec_is_validated(tmp_path):
    manifest = dict(BASE_MANIFEST, requires_zeus=">=0.21")
    d = _make_plugin(tmp_path, manifest=manifest)

    report = validate_plugin_dir(d)

    assert report.ok, report.failures
    assert ("requires_zeus", True, "spec '>=0.21' parses") in report.checks
