...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_zeus_home", lambda: zeus_home)
    monkeypatch.setattr(uninstall, "_is_default_zeus_home", lambda home: False)
    ...
    assert str(zeus_home) in output
    assert project_root.exists()
    assert zeus_home.exists()
