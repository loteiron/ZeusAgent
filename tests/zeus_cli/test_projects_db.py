"""Tests for the per-profile Projects store (zeus_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="ZeusAgent", folders=["/tmp/zeus"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "zeusagent"
    assert proj.name == "ZeusAgent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/zeus"
    assert [f.path for f in proj.folders] == ["/tmp/zeus"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "zeusagent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' ZEUS_HOME.
