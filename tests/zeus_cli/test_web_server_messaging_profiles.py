...
profile's ZEUS_HOME, and the dashboard's own profile stays untouched.
"""
import pytest
import yaml
...
class TestProfileScopedMessagingReads:
    ...
    def test_scoped_read_returns_profile_path_command_and_startup_failure(
        ...
        monkeypatch.setattr(
            ...
            # relying on process-level ZEUS_HOME resolution (#71211).
            lambda *a, **k: {
        ...
        assert payload["gateway_start_command"] == (
            "zeus -p worker_alpha gateway start"
        )
...
def test_named_current_home_matches_unscoped(client, isolated_profiles, monkeypatch):
    from zeus_cli.web_server_profiles import _config_profile_scope, _zeus_home_scope
    from zeus_constants import get_zeus_home
    ...
    with _zeus_home_scope(isolated_profiles["worker_alpha"]):
        with _config_profile_scope("default") as scoped:
            assert scoped is None
            assert get_zeus_home() == isolated_profiles["default"]
...
def test_scoped_enablement_uses_only_own_credentials(client, isolated_profiles, monkeypatch):
    ...
    from zeus_cli.web_server_messaging import _messaging_platform_catalog
...
def test_credential_write_hot_serves_a_multiplexed_profile(client, isolated_profiles, monkeypatch, topology):
    ...
    ``?profile=`` and a pooled ``zeus --profile X serve`` that receives the PUT unscoped (#109088)."""
...
def test_profile_not_in_rescan_result_is_not_reported_as_applied_live(client, isolated_profiles, monkeypatch):
    import zeus_cli.gateway as gateway_cli
    import zeus_cli.gateway_multiplex_served as served_mod
