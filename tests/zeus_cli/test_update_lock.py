"""Cross-process update mutual exclusion (``zeus_cli.update_lock``).

Three surfaces can start an update of one install tree: a terminal ``zeus
...
dashboard-spawned ``zeus update`` was mid-``npm install``, which then failed
...
@pytest.fixture
def marker(tmp_path):
    return tmp_path / ".zeus-update-in-progress"


def test_marker_path_follows_process_zeus_home(tmp_path, monkeypatch):
    """The lock must land where the Rust updater and Electron gate look.

    All three resolve the *process* ZEUS_HOME; a profile-scoped path would
    put the lock somewhere the other two owners never read.
    """
    monkeypatch.setenv("ZEUS_HOME", str(tmp_path))
    assert update_marker_path() == tmp_path / ".zeus-update-in-progress"
...
class TestHandoffFromOrchestratingUpdater:
    """The Tauri updater holds the marker, then spawns ``zeus update``.

    The regression: the child saw its own parent's live marker and exited 2,
    so every GUI update failed with "ZeusAgent is still running" and retrying
...
class TestAncestryHandoff:
    ...
    ``zeus-setup`` under ``~/.zeus`` is only refreshed by a full installer
    ...
    def _liveness_pinned_true(self, monkeypatch):
        monkeypatch.setattr("zeus_cli.update_lock._pid_alive", lambda pid: True)
