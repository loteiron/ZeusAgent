...
def test_fallback_names_configured_platform_whose_extra_failed(monkeypatch, capsys):
    out = _run_fallback_with_failed_extra(
        ...
        missing_features=[("Feishu / Lark", "Run `zeus setup` to install Feishu support.")])
    assert "fail to load them on restart" in out
    assert "Feishu / Lark" in out and "zeus setup" in out
...
def test_configured_features_reports_platform_with_missing_deps(monkeypatch):
    entry = SimpleNamespace(label="Feishu / Lark", check_fn=lambda: False, install_hint="Run `zeus setup`.")
    ...
    with mock.patch("gateway.config.load_gateway_config", return_value=fake_config), \
         ...
         mock.patch("zeus_cli.config.load_config", return_value={}):
        assert main_install_repair._configured_features_missing_deps() == [("Feishu / Lark", "Run `zeus setup`.")]
