...
`zeus update` migrated only the active profile's config.yaml; sibling
...
context-local ZEUS_HOME override.
...
import zeus_cli.update_cmd_config as update_cmd_config
...
def _setup(monkeypatch, tmp_path, active_home: Path):
    ...
    monkeypatch.setattr(
        zeus_constants, "get_process_zeus_home", lambda: active_home
    )
    monkeypatch.setattr(
...
def test_override_is_reset_after_run(monkeypatch, tmp_path):
    ...
    before = get_zeus_home_override()
    update_cmd._migrate_sibling_profile_configs()
    assert get_zeus_home_override() == before
