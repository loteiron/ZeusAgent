...
from zeus_cli.backup import verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    ZEUS_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def zeus_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("zeus_cli.config") or mod == "zeus_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, zeus_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, zeus_home, capsys, monkeypatch):
