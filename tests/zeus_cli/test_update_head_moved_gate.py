"""Tests for the post-pull HEAD-movement gate in ``zeus update``.
...
Before this guard ``zeus update`` printed "✓ Code updated!" and
...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    """Patch the zeus_cli.main helpers ``_cmd_update_impl`` touches.

    ``_m()`` in update_cmd.py lazily returns zeus_cli.main, so patching
    attributes on that module is the canonical test surface (matches
    tests/zeus_cli/test_cmd_update.py).
    """
    ...
    monkeypatch.setattr(
        zeus_main, "_run_pre_update_backup", lambda *a, **k: None
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        zeus_gateway, "find_gateway_pids", lambda all_profiles=False: []
    )
    monkeypatch.setattr(
        zeus_gateway, "supports_systemd_services", lambda: False
    )
    monkeypatch.setattr(
...
def test_update_success_when_head_moves(monkeypatch, tmp_path, capsys):
    ...
    zeus_main.cmd_update(args)  # completes normally (no SystemExit)
