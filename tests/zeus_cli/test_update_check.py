"""Tests for the update check mechanism in zeus_cli.banner."""
...
def configured_release_channel(monkeypatch):
    """Exercise the retained updater in a distribution with a release channel."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    with patch("zeus_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_does_not_cache_none(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "zeus_cli.config.detect_install_method", lambda root: "git"
    )
    monkeypatch.setattr(
        "zeus_cli.config.get_project_root", lambda: repo_dir
    )
    ...
    # Simpler: just patch the get_zeus_home and the repo_dir resolution
    # by making check_for_updates find our fake repo via zeus_home fallback.
    # The code checks Path(__file__).parent.parent/.git first, then falls
    # back to zeus_home / "zeus-agent". We ensure the fallback hits.
    # To do this, we make Path(__file__).parent.parent.resolve() return
    # a path without .git, so it falls through to zeus_home / "zeus-agent".
    real_resolve = Path.resolve

    def fake_resolve(self, *args, **kwargs):
        s = str(self)
        if "banner.py" in s or s.endswith("zeus_cli"):
