...
- Bare `zeus setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `zeus setup --quick` runs the narrower "fill in missing items" flow.
- `zeus setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("zeus_cli.setup.ensure_zeus_home", {}),
        ("zeus_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("zeus_cli.config.is_managed", {"return_value": False}),
        ("zeus_cli.setup.load_config", {"return_value": {}}),
        ("zeus_cli.setup.save_config", {}),
        ("zeus_cli.setup.get_env_value", {"return_value": None}),
        ("zeus_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("zeus_cli.setup._print_setup_summary", {}),
        ("zeus_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("zeus_cli.setup.load_config", {"return_value": {}}),
        ("zeus_cli.setup.save_config", {}),
        ("zeus_cli.auth.get_active_provider", {"return_value": None}),
        ("zeus_cli.setup.get_env_value", {"return_value": None}),
        ("zeus_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `zeus setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="zeus_cli.setup.prompt_choice",
                quick="zeus_cli.setup._run_quick_setup",
                model="zeus_cli.setup.setup_model_provider",
                terminal="zeus_cli.setup.setup_terminal_backend",
                agent="zeus_cli.setup.setup_agent_settings",
                gateway="zeus_cli.setup.setup_gateway",
                tools="zeus_cli.setup.setup_tools",
            )
            from zeus_cli.setup import run_setup_wizard
...
class TestQuickFlag:
    ...
    def test_quick_flag_runs_quick_setup_only(self, existing_install):
        ...
        with ExitStack() as stack:
            ...
            from zeus_cli.setup import run_setup_wizard
            from zeus_cli import setup as setup_mod
...
class TestFreshInstall:
    ...
    def test_reconfigure_on_fresh_install_falls_through(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("zeus_cli.setup.prompt_choice", {"return_value": 0}),
                first="zeus_cli.setup_quick._run_first_time_quick_setup",
            )
            from zeus_cli.setup import run_setup_wizard
            from zeus_cli import setup as setup_mod
    ...
    def test_blank_slate_runs_inside_navigation_step(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("zeus_cli.setup.prompt_choice", {"return_value": 2}),
                blank="zeus_cli.setup_quick._run_blank_slate_setup",
            )
            from zeus_cli import setup as setup_mod
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "zeus_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["zeus", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
