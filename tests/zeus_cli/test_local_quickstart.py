...
def test_quickstart_refuses_when_nothing_fits(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "zeus_cli.local_runtime.catalog.select_variant", lambda *a, **k: None)
...
@pytest.fixture
def capable_machine(monkeypatch):
    """Exercise the real fit policy with a known budget, independent of CI hardware."""
    from zeus_cli.local_runtime.estimator import HardwareBudget

    budget = HardwareBudget(
        usable_vram_bytes=64 << 30,
        total_device_bytes=64 << 30,
        ram_available_bytes=128 << 30,
    )
    monkeypatch.setattr(
        "zeus_cli.local_runtime.hardware.probe_budget", lambda **kwargs: budget,
    )


def test_quickstart_runs_all_three_legs(client, capable_machine, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "zeus_cli.local_runtime.binaries.installed_tags", lambda: [])
    monkeypatch.setattr(
        "zeus_cli.local_runtime.binaries.ensure_runtime_installed",
    ...
    monkeypatch.setattr(
        "zeus_cli.web_routers.local_models.download_file", _fake_download)

    # Leg 3: activation — stub the server start and the model assignment.
    monkeypatch.setattr(
        "zeus_cli.local_runtime.bootstrap.ensure_local_runtime",
        lambda config, force=False: calls.append("server") or None)
    monkeypatch.setattr(
        "zeus_cli.web_routers.local_models._state_endpoint",
        lambda: {"base_url": "http://127.0.0.1:1/v1", "api_key": "k"})
    monkeypatch.setattr(
        "zeus_cli.web_server_config._apply_model_assignment_sync",
...
def test_quickstart_skips_satisfied_legs(client, capable_machine, monkeypatch):
    """Runtime present and model already staged: the response says so and
    the job goes straight to activation."""
    calls: list[str] = []

    monkeypatch.setattr(
        "zeus_cli.local_runtime.binaries.installed_tags", lambda: ["b10362"])
    monkeypatch.setattr(
        "zeus_cli.local_runtime.binaries.ensure_runtime_installed",
        lambda tag, backend, progress=None: calls.append("install"))

    # Every catalog variant reads as staged.
    from zeus_cli.local_runtime.catalog import CATALOG

    all_ids = {v.model_id for e in CATALOG for v in e.variants}
    monkeypatch.setattr(
        "zeus_cli.local_runtime.bootstrap.staged_model_ids", lambda: all_ids)
    monkeypatch.setattr(
        "zeus_cli.web_routers.local_models.download_file",
...
def quickstart_ready(monkeypatch):
    ...
    from zeus_cli.local_runtime.catalog import VariantChoice

    monkeypatch.setattr(
        "zeus_cli.local_runtime.binaries.installed_tags", lambda: ["b10362"])
    monkeypatch.setattr(
        "zeus_cli.local_runtime.catalog.select_variant",
        lambda entry, budget: VariantChoice(variant=entry.variants[0],
                                            zero_spill=True,
                                            reason_key="best-fits"))
    monkeypatch.setattr(
        "zeus_cli.web_routers.local_models._engine_too_old",
...
def test_quickstart_is_single_flight(client, quickstart_ready, monkeypatch):
    ...
    import zeus_cli.web_routers.local_models as lm
