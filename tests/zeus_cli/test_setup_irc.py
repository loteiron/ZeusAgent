"""Tests for IRC gateway configuration via `zeus setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh ZeusAgent install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new ZeusAgent install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#zeus")
            monkeypatch.setenv("IRC_NICKNAME", "zeus-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `zeus setup gateway` experience with IRC present."""
