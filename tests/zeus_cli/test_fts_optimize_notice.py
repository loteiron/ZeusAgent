...
def test_update_notice_offers_v1_trigram_tool_calls_rebuild(tmp_path, monkeypatch, capsys):
    ...
    class FakeSessionDB:
        ...
        _db_needs_fts_storage_upgrade = staticmethod(
            zeus_state.SessionDB._db_needs_fts_storage_upgrade
        )

    monkeypatch.setattr(zeus_constants, "get_zeus_home", lambda: tmp_path)
    monkeypatch.setattr(zeus_state, "SessionDB", FakeSessionDB)
    ...
    assert "zeus sessions optimize-storage" in capsys.readouterr().out
    conn.close()
