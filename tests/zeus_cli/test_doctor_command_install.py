"""Tests for the Command Installation check in zeus doctor."""
...
import zeus_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal ZEUS_HOME + PROJECT_ROOT for doctor tests."""
    home = tmp_path / ".zeus"
    ...
    zeus_bin = venv_bin_dir / "zeus"
    zeus_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    zeus_bin.chmod(0o755)

    monkeypatch.setattr(doctor_mod, "ZEUS_HOME", home)
    monkeypatch.setattr(doctor_mod, "PROJECT_ROOT", project)
    monkeypatch.setattr(doctor_mod, "_DHH", str(home))
    # This fixture exercises command installation, not optional browser downloads.
    monkeypatch.setattr("tools.browser_tool_install.warm_agent_browser_npx_cache", lambda *a, **kw: True)
...
class TestDoctorCommandInstallation:
    ...
    def test_fix_repairs_wrong_symlink(self, monkeypatch, tmp_path):
        home, project, zeus_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create a symlink pointing to wrong target
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "zeus"
        wrong_target = tmp_path / "wrong_zeus"
        ...
        assert cmd_link.resolve() == zeus_bin.resolve()
