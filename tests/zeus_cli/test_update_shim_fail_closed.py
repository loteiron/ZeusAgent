...
The bug class: on Windows, when `zeus.exe`/sibling shims could not be
...
def _make_shims(scripts_dir: Path, names=("zeus", "zeus-gateway")) -> list[Path]:
...
def test_strict_quarantine_refuses_before_install(windows, tmp_path, monkeypatch):
    ...
    # zeus.exe cannot be renamed; zeus-gateway.exe can
    real_rename = Path.rename

    def deny_zeus(self, target):
        if self.name == "zeus.exe":
            raise PermissionError(13, "held open")
        return real_rename(self, target)

    monkeypatch.setattr(Path, "rename", deny_zeus)
    monkeypatch.setattr(main_install_repair, "_zeus_exe_shims", lambda d: shims)
    ...
    assert install_ran == []
    assert "zeus.exe" in exc_info.value.failed_shims
    # The successful rename (zeus-gateway.exe) was rolled back.
    assert (scripts / "zeus-gateway.exe").exists()
    assert not list(scripts.glob("zeus-gateway.exe.old.*"))


def test_non_strict_keeps_warn_and_try(windows, tmp_path, monkeypatch):
    scripts = tmp_path / "venv" / "Scripts"
    shims = _make_shims(scripts, names=("zeus",))
    monkeypatch.setattr(
        Path, "rename",
...
def test_recovery_install_cmd_fail_closed(windows, tmp_path, monkeypatch):
    ...
    _make_shims(scripts, names=("zeus",))
...
def test_refusal_writes_marker_and_exits_2(monkeypatch, capsys):
    ...
    exc = main_install_repair.ShimQuarantineError(["zeus.exe"])
    with pytest.raises(SystemExit) as exit_info:
        update_cmd._refuse_update_for_contended_shims(exc)
    assert exit_info.value.code == 2
    assert wrote == [1]
    out = capsys.readouterr().out
    assert "zeus.exe" in out
