"""Tests for zeus_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $ZEUS_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the ZeusAgent-managed node dir.
"""
...
def _make_zeus_node(zeus_home: Path) -> Path:
    """Create a fake $ZEUS_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = zeus_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    zeus_home = fake_home / ".zeus"
    _make_zeus_node(zeus_home)
    ...
    removed = uninstall.remove_node_symlinks(zeus_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    zeus_home = fake_home / ".zeus"
    node_bin = _make_zeus_node(zeus_home)
