"""Tests for `zeus update --yes / -y` — assume yes for interactive prompts.
...
from zeus_cli.main import cmd_update


@pytest.fixture(autouse=True)
def _isolate_update(isolated_update_runtime, monkeypatch):
    import shutil
    from zeus_cli import managed_uv, update_cmd
...
class TestUpdateYesConfigMigration:
    ...
    @patch("zeus_cli.update_cmd._reload_config_modules")
    @patch("zeus_cli.update_cmd._run_migrate_config_fresh")
    @patch("zeus_cli.update_cmd._run_config_check_fresh", return_value=(1, 2))
    @patch("zeus_cli.config.get_missing_config_fields", return_value=[])
    @patch("zeus_cli.config.get_missing_env_vars", return_value=["NEW_KEY"])
    ...
    def test_no_yes_flag_still_prompts_in_tty(
        ...
        # real ``sys`` module instead of replacing ``zeus_cli.main.sys`` with
        # a MagicMock. The MagicMock approach was flaky under ``pytest-xdist``
        # — a sibling test that imported ``zeus_cli.main`` first could leave
...
class TestUnicodeDecodeErrorInUpdatePrompts:
    ...
    the exception escape and crash `zeus update` mid-flight.
    """
    ...
    def test_upstream_remote_prompt_unicode_decode_error_falls_through_to_skip(
        ...
        from zeus_cli.update_cmd import _sync_with_upstream_if_needed

        with patch(
            "zeus_cli.update_cmd._has_upstream_remote", return_value=False
        ), patch(
            "zeus_cli.update_cmd._should_skip_upstream_prompt", return_value=False
        ), patch(
            "zeus_cli.update_cmd._add_upstream_remote"
        ) as mock_add, patch(
            "builtins.input",
            side_effect=UnicodeDecodeError("utf-8", b"\xff", 0, 1, "invalid byte"),
        ):
