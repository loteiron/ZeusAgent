...
import zeus_cli.doctor as doctor
import zeus_constants
from zeus_cli import config as config_mod
import zeus_cli.gateway as gateway_cli
from zeus_cli import doctor as doctor_mod
from zeus_cli.doctor_config import _has_provider_env_config
import shutil
from zeus_cli import doctor_tools
from zeus_cli import doctor_state
from zeus_cli import doctor_platform
from zeus_cli import doctor_config
...
class TestDoctorPlatformHints:
    def test_termux_package_hint(self, monkeypatch):
        ...
        assert zeus_constants.is_termux() is True
...
class TestDoctorToolAvailabilitySummary:
    ...
    def test_image_gen_without_provider_reports_setup_hint_not_system_dependency(self, monkeypatch):
        ...
        'run zeus setup' summary like any missing key (#9516)."""
        ...
        assert "zeus tools" in image_line and "system dependency" not in image_line and "unavailable" in image_line
        assert "system dependency not met" in next(line for line in out.splitlines() if "homeassistant" in line)
        assert any("zeus setup" in issue for issue in f.issues)
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `zeus doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "ZEUS_HOME", zeus_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("ZEUS_KANBAN_TASK", "probe")
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_zeus_home(self, tmp_path, provider="", memory_config=None):
        """Create a minimal ZEUS_HOME with config.yaml."""
        home = tmp_path / ".zeus"
    ...
    def _run_doctor_and_capture(
        ...
        *,
        ...
        home = self._make_zeus_home(tmp_path, provider, memory_config)
        ...
        monkeypatch.setattr(doctor_mod, "ZEUS_HOME", home)
        ...
        try:
            from zeus_cli import auth as _auth_mod
...
def test_run_doctor_accepts_zeus_provider_ids_that_catalog_aliases(
...
def _doctor_env_for_agent_browser(monkeypatch, tmp_path):
    ...
    branch in run_doctor (zeus_cli/doctor.py ~1557-1605)."""
    home = tmp_path / ".zeus"
    home.mkdir(parents=True, exist_ok=True)
    (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
    project = tmp_path / "project"
...
def test_run_doctor_fix_warms_npx_cache_when_agent_browser_resolves_via_npx(
    ...
    """`zeus doctor --fix` must actually call warm_agent_browser_npx_cache()
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp ZEUS_HOME.

        ``run_doctor`` reads the module-level ``ZEUS_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("ZEUS_HOME")`` alone
        leaves doctor probing the REAL ~/.zeus. On a dev machine with a
    ...
    def test_no_token_and_not_gh_authenticated_shows_warn(self, monkeypatch, tmp_path):
        ...
        from zeus_cli.doctor import run_doctor
        import io, contextlib
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from zeus_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("zeus_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "zeus_cli.doctor", raising=False)
        from zeus_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale ZEUS_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        if ghost is not None:
            env_lines.append(f"ZEUS_MAX_ITERATIONS={ghost}\n")
        (zeus_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "ZEUS_HOME", zeus_home)
        monkeypatch.setattr(doctor_mod, "get_zeus_home", lambda: zeus_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("ZEUS_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("ZEUS_MAX_ITERATIONS", raising=False)
        ...
        # This config-repair test does not exercise the unrelated npm cache warmer.
        # Its success/failure behavior has dedicated tests above.
        monkeypatch.setattr(
            "tools.browser_tool_install.warm_agent_browser_npx_cache", lambda *a, **kw: True,
        )

        buf = io.StringIO()
        with contextlib.redirect_stdout(buf), pytest.raises(SystemExit):
            doctor_mod.run_doctor(Namespace(fix=fix))
        return buf.getvalue(), zeus_home

    def test_detects_drift_warn_only(self, monkeypatch, tmp_path):
        out, zeus_home = self._run_config_section(
            monkeypatch, tmp_path, fix=False, ghost=90, cfg_turns=400,
            os_environ_value=400,  # bridge contaminated os.environ
        )
        assert "ZEUS_MAX_ITERATIONS=90" in out
        assert "shadows" in out
        # Warn-only must NOT mutate .env.
        assert "ZEUS_MAX_ITERATIONS=90" in (zeus_home / ".env").read_text(encoding="utf-8")
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale ZEUS_MAX_ITERATIONS" in out
        env_after = (zeus_home / ".env").read_text(encoding="utf-8")
        assert "ZEUS_MAX_ITERATIONS" not in env_after
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_zeus_tool_progress_warning_says_unsupported_since_floor(self):
        """ZEUS_TOOL_PROGRESS lost its last consumer (the retired v3→4
        ...
        findings = dict(
            doctor_config.collect_deprecated_env_vars({"ZEUS_TOOL_PROGRESS": "true"})
        )
        assert "ignored/unsupported since config floor v12" in findings["ZEUS_TOOL_PROGRESS"]
        # The MODE variant is still read by the gateway fallback → keeps the
        # plain deprecation wording.
        mode = dict(
            doctor_config.collect_deprecated_env_vars({"ZEUS_TOOL_PROGRESS_MODE": "all"})
        )
        assert mode["ZEUS_TOOL_PROGRESS_MODE"] == "display.tool_progress in config.yaml"

    def _run_doctor_with_config(self, monkeypatch, tmp_path, *, config_yaml: str, env_text: str = ""):
        zeus_home = tmp_path / ".zeus"
        zeus_home.mkdir(parents=True)
        (zeus_home / "config.yaml").write_text(config_yaml, encoding="utf-8")
        env_body = env_text if env_text else "OPENAI_API_KEY=sk-test\n"
        (zeus_home / ".env").write_text(env_body, encoding="utf-8")
    ...
    def test_report_does_not_count_as_blocking_issue(self, monkeypatch, tmp_path, capsys):
        ...
        findings = doctor_config.report_deprecated_config_and_env(
            ...
            {"ZEUS_TOOL_PROGRESS_MODE": "verbose"},
        )
        out = capsys.readouterr().out
        assert len(findings) == 2
        assert "Deprecated: delegation.max_async_children" in out
        assert "Deprecated: ZEUS_TOOL_PROGRESS_MODE" in out
...
class TestMacOSTCCGrants:
    ...
    def test_warns_on_cdhash_pinned_dr(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(
            doctor_platform,
            "_macos_desktop_dr",
            lambda app: 'designated => identifier "com.nousresearch.zeus" and cdhash H"97e692f3890f781fa0ad5ad6cb9d769cfaf42628"',
        )
        doctor_platform.check_macos_tcc_grants()
        out = capsys.readouterr().out
        assert "TCC grants will reset after every update" in out
        assert "cdhash-pinned" in out
        assert "zeus update" in out
    ...
    def test_ok_and_repair_info_on_identifier_dr(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(
            doctor_platform,
            "_macos_desktop_dr",
            lambda app: 'designated => identifier "com.nousresearch.zeus"',
        )
        doctor_platform.check_macos_tcc_grants()
        out = capsys.readouterr().out
        ...
        assert "tccutil reset ScreenCapture com.nousresearch.zeus" in out
        assert "toggle" in out
        assert "relaunch" in out

    def test_ok_on_certificate_anchored_dr(self, monkeypatch, capsys, tmp_path):
        """A cert-anchored DR (zeus desktop --setup-tcc-identity, or a
        ...
        monkeypatch.setattr(
            doctor_platform,
            "_macos_desktop_dr",
            lambda app: 'designated => identifier "com.nousresearch.zeus" and certificate root = H"aabbcc"',
        )
        doctor_platform.check_macos_tcc_grants()
        out = capsys.readouterr().out
        assert "TCC signing identity is stable" in out
