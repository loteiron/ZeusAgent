...
When ``ZEUS_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.zeus/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.zeus            (has its own auth.json fixture)
    * Profile     -> tmp_path/.zeus/profiles/coder   (active, ZEUS_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"ZeusAgent is not
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (ZEUS_HOME != ~/.zeus, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    zeus_home = tmp_path / "classic"
    zeus_home.mkdir()
