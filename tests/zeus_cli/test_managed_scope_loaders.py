...
zeus_cli.config.load_config, so the managed overlay has to be wired into each.
...
def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # zeus_time checks an env override first; ensure it's unset so config wins.
    ...
    assert zeus_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (ZEUS_TIMEZONE, ZEUS_REDACT_SECRETS, ZEUS_MAX_ITERATIONS,
    ...
    # writing ZEUS_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml
