"""Venv ownership preflight for ``zeus update`` (#83529).

A venv touched by ``sudo pip`` / ``sudo zeus`` contains root-owned files
(e.g. ``site-packages/zeus_agent-*.dist-info/INSTALLER``). A later normal
``zeus update`` then dies mid-mutation inside ``uv pip install -e .``
("Permission denied (os error 13)") with ``venv/bin/zeus`` already deleted,
...
def _make_fake_venv(tmp_path):
    ...
    (venv / "bin" / "zeus").write_text("#!stub\n")
    (venv / "bin" / "python").write_text("#!stub\n")
    sp = venv / "lib" / "python3.12" / "site-packages"
    dist_info = sp / "zeus_agent-1.0.0.dist-info"
...
def test_foreign_owned_dist_info_child_detected(tmp_path, monkeypatch):
    ...
    installer = str(
        ...
        / "zeus_agent-1.0.0.dist-info" / "INSTALLER"
    )
    # Model a normal user even when the test runner itself is root.
    monkeypatch.setattr(update_cmd_deps.os, "geteuid", lambda: 12345)
...
def test_foreign_owned_refuses_with_chown_hint(tmp_path, monkeypatch, capsys):
    venv = _make_fake_venv(tmp_path)
    zeus_bin = str(venv / "bin" / "zeus")
    monkeypatch.setattr(update_cmd_deps.os, "geteuid", lambda: 12345)
    monkeypatch.setattr(
        update_cmd,
        "_path_uid",
        lambda p: 0 if str(p) == zeus_bin else 12345,
    )
    monkeypatch.setattr(
        update_cmd_deps,
        "_path_uid",
