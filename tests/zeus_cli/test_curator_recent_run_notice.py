...
The notice prints the most recent curator run summary on `zeus update`,
...
session start) so users normally never see the rename map. `zeus update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    from zeus_cli import update_cmd_maint
