...
class TestRepeatRestartWithinCooldown:
    ...
    @patch(
        "zeus_cli.web_server_gateway._gateway_subcommand",
        return_value=["gateway", "restart"],
    )
    @patch("zeus_cli.web_server_gateway._spawn_zeus_action")
    @patch("zeus_cli.web_server_gateway._ACTION_PROCS", {})
    def test_second_request_after_the_child_exits_is_coalesced(
        ...
        from zeus_cli.web_server import _spawn_gateway_restart

        proc = _exited_proc()
        mock_spawn.return_value = proc

        with patch("zeus_cli.gateway._reap_unsupervised_gateway_orphans"), patch(
            "zeus_cli.web_server.time.monotonic", side_effect=[100.0, 103.5]
        ):
    ...
    def test_a_storm_of_requests_produces_exactly_one_restart(
        ...
        with patch("zeus_cli.gateway._reap_unsupervised_gateway_orphans"), patch(
            "zeus_cli.web_server.time.monotonic", side_effect=clock
        ):
    ...
    def test_cooldown_survives_the_action_table_being_cleared(
        ...
        with patch("zeus_cli.gateway._reap_unsupervised_gateway_orphans"), patch(
            "zeus_cli.web_server.time.monotonic", side_effect=[100.0, 102.0]
        ):
            _spawn_gateway_restart()
...
class TestCooldownReleases:
    ...
    def test_request_after_the_window_starts_a_real_restart(
        ...
        with patch("zeus_cli.gateway._reap_unsupervised_gateway_orphans"), patch(
            "zeus_cli.web_server.time.monotonic",
...
class TestExistingBehaviourIsPreserved:
    ...
    def test_live_child_is_still_reused_without_consulting_the_clock(
        ...
        with patch(
            "zeus_cli.web_server_gateway._ACTION_PROCS", {"gateway-restart": live}
        ), patch(
            "zeus_cli.web_server_gateway._ACTION_COMMANDS",
            {"gateway-restart": ("gateway", "restart")},
        ), patch(
            "zeus_cli.gateway._reap_unsupervised_gateway_orphans"
        ):
