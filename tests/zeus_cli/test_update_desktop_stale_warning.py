...
#88251: ``zeus update`` treated a failed desktop pack as non-fatal, printed
...
import zeus_cli.update_cmd_maint as update_cmd_maint
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "zeus_constants.with_zeus_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "zeus_constants.display_zeus_home", lambda: str(tmp_path), raising=False
    )
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_zeus_home", lambda: tmp_path)
