...
def _stage(home_root, name):
    # Machine-scoped models dir (the shared root — tmp ZEUS_HOME IS the root here).
    from zeus_cli.local_runtime.bootstrap import models_dir
...
def test_not_ours_returns_none(zeus_home):
    from zeus_cli.local_runtime.capabilities import managed_model_supports_vision

    assert managed_model_supports_vision("gpt-4o") is None


def test_catalog_vision_model_with_projector_on_disk(zeus_home):
    """Staged catalog model with an mmproj present: True (server down —
    the catalog + on-disk projector answer)."""
    from zeus_cli.local_runtime.bootstrap import assets_dir
    from zeus_cli.local_runtime.capabilities import managed_model_supports_vision
    ...
    _stage(zeus_home, variant.model_id)
    adir = assets_dir()
...
def test_catalog_vision_model_missing_projector_is_blind(zeus_home):
...
def test_lookup_chain_consults_managed_runtime(zeus_home, monkeypatch):
    ...
    monkeypatch.setattr(
        "zeus_cli.local_runtime.capabilities.managed_model_supports_vision",
        lambda mid: True)
...
def test_webp_transcodes_to_png_for_managed_provider(zeus_home, monkeypatch, tmp_path):
...
def test_webp_passes_through_for_cloud_providers(zeus_home, monkeypatch, tmp_path):
...
def test_vision_analyze_normalization_narrows_for_managed(zeus_home, monkeypatch, tmp_path):
