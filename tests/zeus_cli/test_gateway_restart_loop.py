...
Covers:
- Defense 1: gateway stop/restart refuse when _ZEUS_GATEWAY=1
...
from zeus_cli.cron import cron_command
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "zeus gateway stop",
        "zeus gateway uninstall",
        "zeus  gateway  restart",         # double spaces
        "ZeUs Gateway Restart",  # case handled
        "ZEUS GATEWAY RESTART",           # uppercase
    ])
    def test_zeus_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.zeus.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.zeus/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l zeus-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.zeus.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.zeus.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.zeus.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.zeus/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.zeus.gateway-apollo:/a.plist' "
            "'ai.zeus.gateway-cronus:/c.plist' 'ai.zeus.gateway-plutus:/p.plist' "
            "'ai.zeus.gateway:/Users/botuser/Library/LaunchAgents/ai.zeus.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated zeus note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.zeus.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.zeus.gateway',
        "launchctl kick'start' -k gui/501/ai.zeus.gateway",
        "launchctl kick\\start -k gui/501/ai.zeus.gateway",
        'launchctl "kickstart" -k gui/501/ai.zeus.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.zeus.gateway',
        "launchctl dis\\able gui/501/ai.zeus.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.zeus."gateway"',
        # Same class on the systemctl and zeus-CLI branches.
        'systemctl re"start" zeus-gateway',
        'zeus gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.zeus.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.zeus.update-checker',
        'systemctl re"start" zeus-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "zeus cron list",
        "zeus update",
        "zeus config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `zeus gateway start` is benign — starting a gateway from inside a
        ...
        "zeus gateway start",
        "zeus gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway zeus
        # services must not be falsely blocked (the old `.*zeus` matched any
        # zeus token).
        "launchctl unload ai.zeus.update-checker.plist",
        "launchctl restart ai.zeus.daemon",
        ...
        "systemctl restart zeus-meta.service",
        "systemctl restart zeus-cron-helper",
        ...
        "echo after the zeus gateway restarted cleanly",
        "the zeus gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "zeus skill view gateway-notes && echo zeus gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `zeus` is a path component
        # there, not a command.
        "cat '/docs/zeus gateway restart-notes.md'",
        "less /home/user/notes/zeus gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "zeus gateway restart",
        "zeus gateway restart; echo done",
        "zeus gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;zeus gateway restart",
        "true && zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.zeus.gateway"])',
        'subprocess.run(["zeus", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.zeus.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["zeus", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: zeus gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `zeus -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry ZEUS_HOME/ZEUS_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("ZEUS_PROFILE", "zeus")
        monkeypatch.delenv("ZEUS_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "zeus -p zeus gateway stop",
        "zeus -p zeus gateway restart",
        "zeus --profile zeus gateway restart",
        "zeus --profile zeus gateway stop",
        "zeus --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "zeus -v -p zeus gateway restart",
        "zeus -p zeus -v gateway restart",
        "zeus --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "zeus -p venus gateway stop",
        "zeus -p venus gateway restart",
        "zeus --profile venus gateway restart",
        "zeus --profile=venus gateway stop",
        "zeus -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "zeus -p zeus gateway start",
        "zeus -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("zeus gateway restart")
        assert _contains_gateway_lifecycle_command("zeus gateway stop")

    def test_zeus_home_derived_profile(self, monkeypatch):
        # Without ZEUS_PROFILE the guard falls back to the ZEUS_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("ZEUS_PROFILE", raising=False)
        monkeypatch.delenv("ZEUS_PROFILE_NAME", raising=False)
        import zeus_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("zeus -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("zeus -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("zeus -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("zeus gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_zeus_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade zeus then run zeus gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_zeus_gateway_once.sh). The script must live under
        # ZEUS_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("ZEUS_HOME", str(tmp_path / ".zeus"))
        scripts_dir = tmp_path / ".zeus" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_ZEUS_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _ZEUS_GATEWAY=1.

    Issue #37453: systemctl --user restart zeus-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart zeus-gateway",
        "systemctl --user restart zeus-gateway",
        "systemctl stop zeus-gateway.service",
        "zeus gateway restart",
        "zeus gateway uninstall",
        "launchctl kickstart gui/501/ai.zeus.gateway",
        "launchctl bootout gui/501/ai.zeus.gateway",
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart zeus-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.zeus.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-zeus label: label-independent detection is the point
        # (#62891 second reproduction used `ai.zeus.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.zeus.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _ZEUS_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _ZEUS_GATEWAY=1 is in the
        ...
        result = json.loads(tt.terminal_tool(command="zeus gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["zeus gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.zeus.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.zeus.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-zeus systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run zeus gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("zeus gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfezeus gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under ZEUS_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("zeus gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/zeus\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 zeus gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_zeus_home()` falls back to Path.home(), which raises
        RuntimeError when neither ZEUS_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart zeus-gateway' .",
        "grep -rn 'zeus gateway restart' ./logs",
        "rg 'zeus gateway restart' ../archive",
        "grep -c 'systemctl stop zeus-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM zeus_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell zeus gateway restart"',
        'sqlite3 ./db ".system systemctl restart zeus-gateway"',
        'psql ./x -c "\\! systemctl restart zeus-gateway"',
        "grep -r 'zeus gateway restart' . | sh",
        "grep -r 'zeus gateway restart' ./logs | bash",
        "grep -r 'zeus gateway restart' . | sudo sh",
        "grep -r 'x' . ; zeus gateway restart",
        "grep -r 'x' . && systemctl restart zeus-gateway",
        'grep -r "$(zeus gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart zeus-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell zeus gateway restart"',
        'sqlite3 db ".system systemctl restart zeus-gateway"',
        'psql -c "\\! systemctl restart zeus-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    zeus_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run zeus gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZEUS_HOME", str(tmp_path / ".zeus"))
        (tmp_path / ".zeus").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run zeus gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop zeus-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart zeus-gateway' /var/log/syslog",
        "rg 'zeus gateway restart' /home/user/.zeus/logs/",
        "journalctl -u zeus-gateway --grep 'systemctl restart zeus-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'zeus_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell zeus gateway restart"',
        'psql -c "\\! systemctl restart zeus-gateway"',
        "grep 'systemctl restart zeus-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart zeus-gateway",
        'grep "$(systemctl restart zeus-gateway)" f',
        "grep 'restart' log; systemctl restart zeus-gateway",
        'sqlite3 db "SELECT 1"; zeus gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "zeus gateway restart",
        "sudo systemctl stop zeus-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart zeus-gateway%'\""
        )
