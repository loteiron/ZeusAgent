"""A `zeus update` killed while git writes the new tree must leave a recoverable install.
...
checkout import) puts the old tree back so ``zeus update`` can simply run again.
"""
...
# far (the entry module, its package's __init__ and what zeus_bootstrap needs excluded: those run
...
importlib.import_module(entry.rpartition(".")[0] or "zeus_cli")
...
def checkout(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("zeus_cli.main.PROJECT_ROOT", root)
...
def test_killed_pull_is_restored_on_next_launch_and_update_reruns(checkout, monkeypatch):
    ...
    # Another `zeus` launched while an update is mid-pull must not race its git.
    ...
    _pull(root)  # `zeus update` again: a normal fast-forward
    assert _git(root, "rev-parse", "HEAD") == b and not marker.exists()

    # Every console script (`zeus`, `zeus-agent`, `zeus-acp`) repairs before its entry module imports
    # any other checkout module past zeus_bootstrap: any of them may be a half-written file.
    repo = os.path.realpath(Path(er.__file__).parent.parent)
    for entry in ("zeus_cli.main", "cli", "run_agent", "acp_adapter.entry"):
