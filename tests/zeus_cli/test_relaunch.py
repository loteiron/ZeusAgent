"""Tests for zeus_cli.relaunch — unified self-relaunch utility."""

import sys

import pytest

from zeus_cli import relaunch as relaunch_mod


class TestResolveZeusAgentBin:
    def test_prefers_absolute_argv0_when_executable(self, monkeypatch):
        fake = "/nix/store/abc/bin/zeus"
        ...
        assert relaunch_mod.resolve_zeus_bin() == fake
    ...
    def test_resolves_relative_argv0(self, monkeypatch, tmp_path):
        ...
        # Ensure we don't accidentally match a real 'zeus' on PATH
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_zeus_bin() == str(fake)

    def test_falls_back_to_path_which(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])  # not a real path
        monkeypatch.setattr(
            relaunch_mod.shutil, "which", lambda name: "/usr/bin/zeus" if name == "zeus" else None
        )
        assert relaunch_mod.resolve_zeus_bin() == "/usr/bin/zeus"
...
class TestBuildRelaunchArgv:
    def test_uses_bin_when_available(self, monkeypatch):
        monkeypatch.setattr(relaunch_mod, "resolve_zeus_bin", lambda: "/usr/bin/zeus")
        argv = relaunch_mod.build_relaunch_argv(["--resume", "abc"])
        assert argv[0] == "/usr/bin/zeus"
    ...
    def test_can_disable_preserve(self, monkeypatch):
        ...
        assert argv == ["/usr/bin/zeus", "--resume", "abc"]
...
class TestRelaunch:
    def test_calls_execvp(self, monkeypatch):
        ...
        assert calls == [("/usr/bin/zeus", ["/usr/bin/zeus", "--resume", "abc"])]
    ...
    def test_windows_uses_subprocess_not_execvp(self, monkeypatch):
        ...
        zeus).  relaunch() must detect win32 and use subprocess.run +
        sys.exit instead.
        ...
        that motivated it.
        """
        monkeypatch.setattr(relaunch_mod, "resolve_zeus_bin", lambda: r"C:\Users\test\zeus.exe")
        ...
        monkeypatch.setattr(relaunch_mod.sys, "argv", [r"C:\Users\test\zeus.exe"])
        ...
        assert execvp_calls == []
        assert captured_argv == [[r"C:\Users\test\zeus.exe", "chat"]]
    ...
    def test_windows_propagates_child_exit_code(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod, "resolve_zeus_bin", lambda: r"C:\zeus.exe")
...
class TestResolveZeusAgentBinWindowsPyGuard:
    """On Windows, resolve_zeus_bin MUST NOT return a .py path.
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        # Force PATH lookup to return a zeus.exe so the test doesn't
        # exercise the None-fallback path (that's a separate test).
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: r"C:\venv\Scripts\zeus.exe" if name == "zeus" else None,
        )

        bin_path = relaunch_mod.resolve_zeus_bin()
        # Must NOT be the .py — must be the zeus.exe PATH entry.
        assert bin_path == r"C:\venv\Scripts\zeus.exe"
    ...
    def test_posix_still_accepts_py_argv0(self, monkeypatch, tmp_path):
        ...
        script = tmp_path / "zeus"
        script.write_text("#!/usr/bin/env python3\n")
        script.chmod(0o755)
        monkeypatch.setattr(relaunch_mod.sys, "argv", [str(script), "chat"])
        assert relaunch_mod.resolve_zeus_bin() == str(script)

    @pytest.mark.windows_only
    def test_windows_py_argv0_with_no_zeus_on_path_returns_none(self, monkeypatch, tmp_path):
        """Bulletproof fallback: if argv0 is .py on Windows AND zeus.exe
        ...
        assert relaunch_mod.resolve_zeus_bin() is None
