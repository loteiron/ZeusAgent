"""Behavior tests for zeus_cli.inventory.
...
from zeus_cli.inventory import (
    ConfigContext,
...
def test_explicit_only_keeps_anthropic_row_with_oauth_credentials():
    ...
    Claude Code (~/.claude/.credentials.json) and ZeusAgent' own device flow
    ...
    with (
        ...
        patch(
            "zeus_cli.inventory._anthropic_oauth_credentials_present",
            return_value=True,
        ),
    ):
        payload = build_models_payload(ctx, explicit_only=True)
...
def test_anthropic_oauth_presence_accepts_pool_only_oauth_entry():
    ...
    """
    from zeus_cli.inventory import _anthropic_oauth_credentials_present
    ...
    with (
        ...
        patch(
            "zeus_cli.auth.read_credential_pool",
            return_value=[
...
def test_build_models_payload_forwards_refresh_flag():
    ...
    with patch("zeus_cli.model_switch.list_authenticated_providers", side_effect=_capture):
...
def test_list_authenticated_providers_refresh_busts_cache():
    ...
    with patch("zeus_cli.models.clear_provider_models_cache") as clear:
