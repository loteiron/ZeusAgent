"""Tests for _purge_stale_zeus_modules — the class fix for stale
sys.modules breaking the gateway auto-restart after `zeus update`.

Field failure (2026-08-20, Teknium's Linux box): `zeus update` pulled a
checkout where zeus_cli/gateway.py newly imports `line_input` from
zeus_cli.cli_output, but the updater process had cli_output cached from
before that symbol existed. The function-level `from zeus_cli.gateway
...
the ZeusAgent package prefixes so later imports rebuild a self-consistent
...
def _restore_sys_modules():
    ...
    The purge under test evicts real ZeusAgent modules from the cache; later
...
def test_purge_evicts_zeus_prefixed_modules():
    victims = [
        "zeus_cli.cli_output",
        "zeus_cli.gateway",
        "gateway.status",
    ...
    for name in victims:
        if name not in sys.modules:
            ...
            added.append(name)
    try:
        cli_main._purge_stale_zeus_modules()
        for name in victims:
...
def test_purge_protects_executing_modules():
    ...
    cli_main._purge_stale_zeus_modules()
    assert sys.modules.get("zeus_cli.update_cmd") is update_cmd
    assert sys.modules.get("zeus_cli.main") is cli_main
    assert "zeus_cli" in sys.modules
...
def test_purge_preserves_active_update_receipt(tmp_path, monkeypatch):
    ...
    import zeus_cli.update_receipt as receipt
    ...
    try:
        ...
        cli_main._purge_stale_zeus_modules()
        post_purge_receipt = importlib.import_module("zeus_cli.update_receipt")
...
def test_purge_never_raises_on_weird_sys_modules():
    ...
    sys.modules["zeus_cli._purge_test_none"] = None  # type: ignore[assignment]
    try:
        cli_main._purge_stale_zeus_modules()
    finally:
        sys.modules.pop("zeus_cli._purge_test_none", None)
...
def test_stale_symbol_scenario_end_to_end():
    ...
    try:
        ...
        try:
            from zeus_cli.cli_output import line_input  # noqa: F401
            raised = False
        except ImportError:
            raised = True
...
def test_purge_keeps_plan_record_class_identity():
    ...
    from zeus_cli.update_inventory import RuntimeRecord as before

    cli_main._purge_stale_zeus_modules()
    from zeus_cli.update_inventory import RuntimeRecord as after
