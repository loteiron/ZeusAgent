"""Tests for kanban board export / import (``zeus_cli.kanban_transfer``).
...
from zeus_cli import kanban_transfer as kt
from zeus_cli.archive_safe import normalize_archive_parts, safe_extract_targz
...
def kanban_root(tmp_path, monkeypatch):
    ...
    def _use(name: str) -> Path:
        ...
        monkeypatch.setenv("ZEUS_KANBAN_HOME", str(root))
        for var in ("ZEUS_KANBAN_DB", "ZEUS_KANBAN_WORKSPACES_ROOT",
                    "ZEUS_KANBAN_ATTACHMENTS_ROOT", "ZEUS_KANBAN_BOARD"):
...
def test_import_rejects_a_future_format_version(kanban_root, tmp_path):
    ...
    kanban_root("target")
    with pytest.raises(ValueError, match="newer than this ZeusAgent"):
