"""ensure_zeus_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".zeus"
    second = tmp_path / "b" / ".zeus"

    monkeypatch.setenv("ZEUS_HOME", str(first))
    cfg.ensure_zeus_home()

    # Profile switch: ZEUS_HOME moves → the new path is ensured too.
