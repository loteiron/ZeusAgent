"""Tests for the unified provider catalog (zeus_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``zeus model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from zeus_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
