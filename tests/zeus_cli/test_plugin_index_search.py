...
(remote → cache → seed), `zeus plugins search --json`, and install-time
...
from zeus_cli import plugin_index
from zeus_cli.plugin_index import (
    PluginIndexEntry,
    _parse_entries,
    load_index,
    resolve_name,
...
def zeus_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(plugin_index, "get_zeus_home", lambda: tmp_path)
    return tmp_path
...
class TestSearch:
    ...
    def test_exact_name_ranks_first(self):
        results = search_index(self.entries, "zeus-media-studio")
        assert results[0].name == "zeus-media-studio"

    def test_matches_tags(self):
        results = search_index(self.entries, "telegram")
        assert results and results[0].name == "zeus-telegram-business"

    def test_matches_description(self):
        results = search_index(self.entries, "secretary")
        assert [e.name for e in results] == ["zeus-telegram-business"]

    def test_fuzzy_typo_tolerance(self):
        results = search_index(self.entries, "zeus-media-studo")
        assert results and results[0].name == "zeus-media-studio"
...
class TestLoadIndex:
    def test_fresh_cache_wins_without_network(self, zeus_home, monkeypatch):
        _write_cache(zeus_home, SAMPLE)
    ...
    def test_expired_cache_triggers_remote(self, zeus_home, monkeypatch):
        _write_cache(zeus_home, SAMPLE, age_seconds=plugin_index.INDEX_CACHE_TTL + 60)
    ...
    def test_remote_failure_falls_back_to_stale_cache(self, zeus_home, monkeypatch):
        _write_cache(zeus_home, SAMPLE, age_seconds=plugin_index.INDEX_CACHE_TTL + 60)
    ...
    def test_no_cache_no_remote_falls_back_to_seed(self, zeus_home, monkeypatch):
        monkeypatch.setattr(plugin_index, "_fetch_remote", lambda: None)
        entries, source = load_index()
        assert source == "seed"
        assert len(entries) >= 3

    def test_refresh_bypasses_fresh_cache(self, zeus_home, monkeypatch):
        _write_cache(zeus_home, SAMPLE)
    ...
    def test_offline_skips_network(self, zeus_home, monkeypatch):
    ...
    def test_corrupt_cache_ignored(self, zeus_home, monkeypatch):
        cache = zeus_home / "cache" / "plugin_index.json"
        cache.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_remote_fetch_writes_cache(self, zeus_home, monkeypatch):
...
class TestResolveName:
    ...
    def test_exact_unique(self):
        entry, candidates = resolve_name(self.entries, "zeus-media-studio")
        assert entry is not None and entry.repo == "NousResearch/hermes-media-studio"

    def test_case_insensitive(self):
        entry, _ = resolve_name(self.entries, "ZeUs-Media-Studio")
        assert entry is not None

    def test_unique_partial(self):
        entry, _ = resolve_name(self.entries, "telegram")
        assert entry is not None and entry.name == "zeus-telegram-business"

    def test_ambiguous_partial(self):
        entry, candidates = resolve_name(self.entries, "zeus")
...
class TestInstallResolution:
    def test_bare_name_detection(self):
        from zeus_cli.plugins_cmd import _looks_like_bare_index_name

        assert _looks_like_bare_index_name("zeus-media-studio")
    ...
    def test_install_resolves_name_and_pins_ref(self, zeus_home, monkeypatch):
        from zeus_cli import plugins_cmd
        ...
        with pytest.raises(SystemExit):
            plugins_cmd.cmd_install("zeus-media-studio", enable=False)
        assert captured["identifier"] == "NousResearch/hermes-media-studio"
        assert captured["ref"] == "e" * 40

    def test_install_explicit_ref_beats_index_pin(self, zeus_home, monkeypatch):
        from zeus_cli import plugins_cmd
        ...
        monkeypatch.setattr(plugins_cmd, "_install_plugin_core", fake_core)
        with pytest.raises(SystemExit):
            plugins_cmd.cmd_install("zeus-media-studio", enable=False, ref="d" * 40)
        assert captured["ref"] == "d" * 40

    def test_install_ambiguous_name_lists_candidates_and_exits(
        self, zeus_home, monkeypatch, capsys
    ):
        from zeus_cli import plugins_cmd
        ...
        with pytest.raises(SystemExit) as exc:
            plugins_cmd.cmd_install("zeus", enable=False)
        assert exc.value.code == 1
        assert not called
        out = capsys.readouterr().out
        assert "ambiguous" in out
        assert "zeus-media-studio" in out
        assert "zeus-telegram-business" in out

    def test_install_unknown_name_exits(self, zeus_home, monkeypatch, capsys):
        from zeus_cli import plugins_cmd
    ...
    def test_owner_repo_passthrough_skips_index(self, zeus_home, monkeypatch):
...
class TestCmdSearch:
    def test_json_output(self, zeus_home, monkeypatch, capsys):
        from zeus_cli import plugins_cmd
        ...
        assert payload["results"][0]["name"] == "zeus-telegram-business"
    ...
    def test_no_results_message(self, zeus_home, monkeypatch, capsys):
        from zeus_cli import plugins_cmd
    ...
    def test_dispatch_routes_search(self, zeus_home, monkeypatch):
        from zeus_cli import plugins_cmd
