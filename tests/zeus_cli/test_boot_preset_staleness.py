...
def test_presets_stale_when_a_staged_model_has_no_section(zeus_home):
    from zeus_cli.local_runtime.bootstrap import _presets_stale

    _stage(zeus_home, "model-a")
    _stage(zeus_home, "model-b")
    _write_presets(zeus_home, "model-a")
    assert _presets_stale() is True


def test_presets_current_when_every_staged_model_is_covered(zeus_home):
    from zeus_cli.local_runtime.bootstrap import _presets_stale
...
def test_no_models_is_never_stale(zeus_home):
    from zeus_cli.local_runtime.bootstrap import _presets_stale
...
def test_boot_replaces_incumbent_with_stale_presets(zeus_home, monkeypatch):
    ...
    import zeus_cli.local_runtime.bootstrap as boot
    ...
    stopped = {}
    monkeypatch.setattr(
        "zeus_cli.local_runtime.endpoint._managed_state",
    ...
    monkeypatch.setattr(
        "zeus_cli.local_runtime.binaries.ensure_runtime_installed", fake_boot)
...
def test_refresh_bounces_an_adopted_server(zeus_home, monkeypatch):
...
def test_refresh_no_server_anywhere_is_a_noop(zeus_home, monkeypatch):
    import zeus_cli.local_runtime.bootstrap as boot

    monkeypatch.setattr(boot, "_SUPERVISOR", None)
    monkeypatch.setattr(
        "zeus_cli.local_runtime.endpoint._managed_state", lambda: None)
