...
def test_fast_serve_launch_dispatches_only_unambiguous_serve(monkeypatch) -> None:
    ...
    monkeypatch.setattr(sys, "argv", ["zeus", "serve", "--host", "127.0.0.1", "--port", "0"])
    ...
    monkeypatch.setenv("ZEUS_DISABLE_FAST_SERVE_LAUNCH", "1")
    monkeypatch.setattr(sys, "argv", ["zeus", "serve"])
    assert main_mod._try_fast_serve_launch() is False
    monkeypatch.delenv("ZEUS_DISABLE_FAST_SERVE_LAUNCH")
