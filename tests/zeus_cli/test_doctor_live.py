"""Tests for ``zeus doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from zeus_cli import doctor_live
from zeus_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `zeus doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("zeus_cli.doctor.ZEUS_HOME", tmp_path / "home")
        monkeypatch.setattr("zeus_cli.doctor.PROJECT_ROOT", tmp_path / "root")
