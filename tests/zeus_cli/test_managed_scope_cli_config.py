...
zeus_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `zeus config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_zeus_home = get_zeus_home()`` at import time (module
    singleton), so monkeypatching ZEUS_HOME after import doesn't move it.
    ...
    this only matters for tests that swap ZEUS_HOME.)
    """
    import cli
