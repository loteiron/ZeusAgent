"""Contract tests for zeus_cli.local_runtime — Rollouts 1+2.

Per the design's verification plan: relationships and contracts, no
change-detector tests, real imports against temp ZEUS_HOME (the autouse
...
from zeus_cli.local_runtime.detect import DetectedServer, probe_port
...
def test_sha256_mismatch_rejects(tmp_path, monkeypatch):
    ...
    from zeus_cli.local_runtime import binaries
...
def test_llamacpp_endpoint_resolution_prefers_managed(tmp_path, monkeypatch, stub_server):
    ...
    monkeypatch.setenv("ZEUS_HOME", str(tmp_path / ".zeus"))
    port, handler = stub_server
    from zeus_cli.local_runtime import endpoint as ep
...
def test_llamacpp_endpoint_stale_state_falls_through(tmp_path, monkeypatch):
    ...
    with socket.socket() as s:
        ...
        dead_port = s.getsockname()[1]
    from zeus_cli.local_runtime import endpoint as ep
    from zeus_cli.local_runtime.detect import DEFAULT_PROBE_PORTS
    from zeus_cli.local_runtime.supervisor import state_path
...
def test_llamacpp_dead_server_raises_friendly_error(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "zeus_cli.local_runtime.endpoint.resolve_llamacpp_endpoint",
        lambda *a, **k: None)
...
def test_resolution_kicks_boot_when_no_thread_is_booting(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ZEUS_HOME", str(tmp_path / ".zeus"))
    from zeus_cli.local_runtime import bootstrap as bs
    from zeus_cli.local_runtime import endpoint as ep
    from zeus_cli.local_runtime.supervisor import state_path
...
def test_staged_models_requires_every_split_part(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ZEUS_HOME", str(tmp_path / ".zeus"))
    import zeus_cli.local_runtime.bootstrap as bs
...
def test_bootstrap_skips_boot_with_no_staged_models(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("zeus_cli.local_runtime.binaries.ensure_runtime_installed", _boom)
...
def test_endpoint_identity_stable_across_supervisor_instances(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ZEUS_HOME", str(tmp_path / ".zeus"))
    from zeus_cli.local_runtime import supervisor as sup_mod
    from zeus_cli.local_runtime.supervisor import LlamaServerSupervisor
    ...
    key_file = tmp_path / ".zeus" / "runtimes" / "llamacpp" / ".api_key"
...
def test_runtime_provider_seam_llamacpp_alias(tmp_path, monkeypatch, stub_server):
    ...
    from zeus_cli.runtime_provider import _resolve_named_custom_runtime
...
def test_local_runtime_config_defaults_shape():
    ...
    from zeus_cli.config_defaults import DEFAULT_CONFIG
...
def test_bootstrap_disabled_is_noop(tmp_path, monkeypatch):
    monkeypatch.setenv("ZEUS_HOME", str(tmp_path / ".zeus"))
    from zeus_cli.local_runtime import bootstrap
...
def test_bootstrap_failure_never_raises(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "zeus_cli.local_runtime.binaries.ensure_runtime_installed", boom)
