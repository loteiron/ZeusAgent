...
def zeus_home(tmp_path, monkeypatch):
    ...
    # /etc/zeus on the dev/CI box can't influence the test.
    monkeypatch.setenv("ZEUS_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(zeus_home, monkeypatch):
...
def test_env_expansion_in_user_config(zeus_home, monkeypatch):
    from zeus_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_zeus_dotenv(zeus_home=str(home))
