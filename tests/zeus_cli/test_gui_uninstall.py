"""Tests for zeus_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import zeus_cli.gui_uninstall as gu


def _make_agent(zeus_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = zeus_home / "zeus-agent"
    (agent_root / "zeus_cli").mkdir(parents=True)
    (agent_root / "zeus_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(zeus_home: Path) -> None:
    """Create the source-built GUI artifacts a `zeus desktop` run produces."""
    desktop = zeus_home / "zeus-agent" / "apps" / "desktop"
    ...
    (zeus_home / "zeus-agent" / "node_modules").mkdir(parents=True)
    (zeus_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(zeus_home: Path) -> None:
    (zeus_home / "config.yaml").write_text("x: 1\n")
    (zeus_home / ".env").write_text("KEY=secret\n")
    (zeus_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(zeus_home)
    _make_gui_build(zeus_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(zeus_home)
    ...
    assert summary["zeus_home"] == str(zeus_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `zeus desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    zeus_home = tmp_path / ".zeus"
    _make_agent(zeus_home)
    icon = lde.icon_path(zeus_home / "zeus-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(zeus_home)
    ...
    assert lde.icon_path(zeus_home / "zeus-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (zeus_home / "zeus-agent" / "zeus_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".zeus")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import zeus_cli.uninstall as uninstall
