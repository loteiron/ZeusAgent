"""Dashboard ZeusAgent Console websocket tests."""
...
@pytest.fixture
def console_client(monkeypatch, _isolate_zeus_home):
...
def test_console_ws_cancel_returns_to_prompt(console_client, monkeypatch):
    from zeus_cli.console_engine import ConsoleResult, ZeusAgentConsoleEngine

    def slow_execute(self, line: str, *, confirmed: bool = False):
        time.sleep(0.2)
        return ConsoleResult("ok", output="late", command=line)

    monkeypatch.setattr(ZeusAgentConsoleEngine, "execute", slow_execute)
