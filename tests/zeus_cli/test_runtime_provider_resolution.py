...
def test_openai_key_bound_to_another_host_never_reaches_openrouter(monkeypatch, openai_base_url, expected_key):
    ...
    from zeus_cli.runtime_provider_backends import _resolve_openrouter_runtime
    monkeypatch.setattr(rp, "_get_model_config", lambda: {})
...
def test_explicit_openrouter_config_mirror_bypasses_pool(monkeypatch):
    ...
    # The canonical URL that `zeus setup` persists is NOT a mirror: the pool must still serve it.
...
class TestProviderEntryApiKeyEnvAlias:
    ...
    def test_snake_case_api_key_env_normalizes_to_key_env(self):
        from zeus_cli.config import _normalize_custom_provider_entry
        entry = {
            "name": "vendor",
    ...
    def test_valid_fields_set_lists_key_env(self):
        ...
        from zeus_cli.config import _VALID_CUSTOM_PROVIDER_FIELDS
...
def test_minimax_oauth_runtime_returns_anthropic_messages_mode(monkeypatch):
    ...
    from zeus_cli.auth import MINIMAX_OAUTH_GLOBAL_INFERENCE
