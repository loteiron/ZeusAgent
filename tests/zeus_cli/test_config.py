"""Tests for zeus_cli configuration management."""
    ...
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            if sys.platform == "win32":
                # Windows default is %LOCALAPPDATA%\zeus — see
                # zeus_constants._get_platform_default_zeus_home.
                ...
                assert home == base / "zeus"
            else:
                assert home == Path.home() / ".zeus"


class TestEnsureZeusAgentHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from zeus_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
    ...
    _PRE_REWRITE_DEFAULT_SOUL = (
        "You are ZeusAgent, an intelligent AI assistant created by Nous "
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``zeus logs`` surfaces it)
    ...
    def test_corrupt_config_is_backed_up(self, tmp_path, capsys):
        ...
        """
        from zeus_cli import config as cfg_mod
...
class TestSaveAndLoadRoundtrip:
    ...
    def test_atomic_config_write_refuses_unparseable_existing_config(self, tmp_path):
        ...
        from zeus_cli.config import atomic_config_write
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_quotes_values_containing_hash(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ZEUS_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ZEUS_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ZEUS_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ZEUS_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestSanitizeEnvLines:
    ...
    def test_migrate_reports_normalized_line_formatting(self, capsys):
        ...
        with (
            patch("zeus_cli.config.sanitize_env_file", return_value=2),
            patch(
                "zeus_cli.config.check_config_version",
                return_value=(latest_version, latest_version),
            ),
            patch("zeus_cli.config.read_raw_config", return_value={}),
            patch("zeus_cli.config.get_missing_env_vars", return_value=[]),
            patch("zeus_cli.config.get_missing_config_fields", return_value=[]),
            patch("zeus_cli.config.get_missing_skill_config_vars", return_value=[]),
        ):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from zeus_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """ZEUS_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `zeus setup`) lets a
        ...
        via config.yaml; ZEUS_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from zeus_cli.config import OPTIONAL_ENV_VARS
        assert "ZEUS_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `zeus setup` surface it (previously only Honcho was listed, leaving
...
class TestConfigSupportFloor:
    ...
    def test_v11_config_is_refused_and_untouched(self, tmp_path, capsys):
        ...
        assert "run `zeus setup` to regenerate" in captured.out
    ...
    def test_floor_message_uses_display_zeus_home(self):
        from zeus_cli.config_migrations import support_floor_message
        from zeus_constants import display_zeus_home

        msg = support_floor_message()
        assert f"{display_zeus_home()}/config.yaml" in msg
...
class TestCustomProviderCompatibility:
    ...
    def _run_ladder(current_ver: int):
        from zeus_cli.config_migrations import run_migrations
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or selected ZeusAgent runtime/security controls.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next ZeusAgent
    process on next startup via the dotenv → ``os.environ`` chain in
    ``zeus_cli/env_loader.py``.
    ...
    def _zeus_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        "allowed_key",
        [
            "ZEUS_LANGFUSE_PUBLIC_KEY",
            "ZEUS_SPOTIFY_CLIENT_ID",
            "ZEUS_QWEN_BASE_URL",
            "ZEUS_MAX_ITERATIONS",
        ],
    )
    def test_zeus_integration_keys_still_writable(self, allowed_key):
        """``ZEUS_*`` overall is NOT blocked.
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "ZEUS_ENV_PATH",
            "ZEUS_OPTIONAL_MCPS",
            "ZEUS_COPILOT_ACP_COMMAND",
            "ZEUS_COPILOT_ACP_ARGS",
            "ZEUS_YOLO_MODE",
            "ZEUS_ACCEPT_HOOKS",
            "ZEUS_REDACT_SECRETS",
            "ZEUS_INTERACTIVE",
            ...
            "ZEUS_CRON_SESSION",
            "ZEUS_SINGLE_QUERY_SESSION",
            "ZEUS_SESSION_KEY",
            "ZEUS_SESSION_PLATFORM",
        ],
    )
    def test_zeus_security_control_keys_are_not_writable(self, protected_key):
    ...
    def test_preexisting_optional_mcps_override_still_loads(self, tmp_path):
        ...
        from zeus_cli.config import invalidate_env_cache

        catalog = tmp_path / "custom-mcp-catalog"
        (tmp_path / ".env").write_text(
            f"ZEUS_OPTIONAL_MCPS={catalog}\n",
            encoding="utf-8",
        )
        invalidate_env_cache()

        assert load_env()["ZEUS_OPTIONAL_MCPS"] == str(catalog)

    @pytest.mark.parametrize(
        ("key", "expected"),
        [
            ("Path", "PATH"),
            ("Zeus_Yolo_Mode", "ZEUS_YOLO_MODE"),
            ("Zeus_Optional_Mcps", "ZEUS_OPTIONAL_MCPS"),
            ("Zeus_Copilot_Acp_Command", "ZEUS_COPILOT_ACP_COMMAND"),
            ("Zeus_Copilot_Acp_Args", "ZEUS_COPILOT_ACP_ARGS"),
        ],
    )
    def test_windows_policy_names_are_case_insensitive(self, key, expected):
        from zeus_cli.config import _env_var_policy_name
    ...
    def test_windows_env_assignment_matching_is_case_insensitive(self, prefix):
        from zeus_cli.config import _env_line_defines_key
...
"""
    ...
    def test_persist_migration_writes_full_read_raw_config(self, tmp_path):
        from zeus_cli.config import _persist_migration, read_raw_config
    ...
    def test_preserves_existing_codex_app_server_auto_value(self, tmp_path):
        with patch.dict(os.environ, {"ZEUS_HOME": str(tmp_path)}):
            self._write(
                ...
                "_config_version: 31\n"
                "compression:\n"
                "  codex_app_server_auto: zeus\n",
            )

            migrate_config(interactive=False, quiet=True)

            raw = yaml.safe_load((tmp_path / "config.yaml").read_text())
            assert raw["compression"]["codex_app_server_auto"] == "zeus"
...
def test_default_config_has_no_duplicate_top_level_keys():
    ...
    import ast
    import zeus_cli.config as cfg_mod
...
class TestConfigCommandFailClosedSurface:
    """`zeus config set/unset` must exit cleanly (no traceback) when the
