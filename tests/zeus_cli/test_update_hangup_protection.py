"""Tests for SIGHUP protection and stdout mirroring in ``zeus update``.

Covers ``_UpdateOutputStream``, ``_install_hangup_protection``, and
``_finalize_update_output`` in ``zeus_cli/main.py``.  These exist so
that ``zeus update`` survives a terminal disconnect mid-install
...
from zeus_cli.main_dashboard import _UpdateOutputStream, _finalize_update_output, _install_hangup_protection
from zeus_cli.update_cmd import _log_only_write, _print_update_completion, _run_logged_subprocess


def test_update_completion_includes_bounded_action_identity(monkeypatch, capsys):
    monkeypatch.setenv("ZEUS_ACTION_ID", "a" * 32)
    ...
    monkeypatch.setattr("zeus_cli.update_cmd._branch_head_suffix", lambda: "")

    _print_update_completion("✓ Update complete!")

    assert capsys.readouterr().out.splitlines() == [
        "✓ Update complete!",
        f"=== zeus-update completed {'a' * 32} ===",
    ]


def test_update_completion_rejects_untrusted_action_identity(monkeypatch, capsys):
    monkeypatch.setenv("ZEUS_ACTION_ID", "not-safe\nforged")
    monkeypatch.setattr("zeus_cli.update_cmd._branch_head_suffix", lambda: "")
...
class TestInstallHangupProtection:
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        import zeus_cli.config as _cfg
        if hasattr(_cfg, "_ZEUS_HOME_CACHE"):
            _cfg._ZEUS_HOME_CACHE = None  # type: ignore[attr-defined]
        ...
        try:
            ...
            assert "zeus update started" in contents
        finally:
    ...
    def test_non_fatal_if_log_setup_fails(self, monkeypatch):
        """If get_zeus_home() raises, stdio must be left untouched but SIGHUP still handled."""
        prev_out, prev_err = sys.stdout, sys.stderr
        ...
        monkeypatch.setattr(
            "zeus_cli.config.get_zeus_home", _boom, raising=True
        )
