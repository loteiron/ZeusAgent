...
the SAME provider universe as ``zeus model`` (the CLI/TUI picker).
...
def test_every_zeus_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `zeus model` universe."""
    ...
    assert not missing, (
        "providers shown in `zeus model` but not configurable in the desktop "
