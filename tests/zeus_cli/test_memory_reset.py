"""Tests for the `zeus memory reset` CLI command.
...
- Profile-scoped reset (uses ZEUS_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake ZEUS_HOME with memory files."""
    zeus_home = tmp_path / ".zeus"
    memories = zeus_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("ZEUS_HOME", str(zeus_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nZeusAgent repo is at ~/.zeus/zeus-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `zeus memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        zeus_home = tmp_path / ".zeus"
        (zeus_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("ZEUS_HOME", str(zeus_home))
