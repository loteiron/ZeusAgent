...
plugin SDK (``window.__ZEUS_PLUGIN_SDK__``): ``fetchJSON`` (JSON REST),
``authedFetch`` (uploads / blob downloads), and ``buildWsUrl`` /
``buildWsAuthParam`` (WebSockets). These handle BOTH dashboard auth modes —
loopback (``X-Zeus-Session-Token`` header) and gated OAuth
(``zeus_session_at`` cookie / single-use ``?ticket=``).

Plugins that hand-roll ``fetch`` / ``WebSocket`` and read
``window.__ZEUS_SESSION_TOKEN__`` directly send an empty token in gated mode
...
_FORBIDDEN = "__ZEUS_SESSION_TOKEN__"
...
def test_there_are_plugin_bundles_to_check() -> None:
    ...
    # kanban + zeus-achievements are bundled today; assert at least one is
