...
Run from the PR worktree:
    ...
    cd ~/.zeus/zeus-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.zeus/zeus-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.zeus/zeus-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a zeus-agent checkout"
)
...
# Isolated ZEUS_HOME for the config write.
