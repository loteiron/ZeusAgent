...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("zeus.gateway.up"), _M("zeus.gateway.active_agents"),
        _M("zeus.gateway.busy"), _M("zeus.gateway.drainable"),
        _M("zeus.gateway.restart_requested"),
        _M("zeus.platform.up"), _M("zeus.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("zeus.cron.scheduler.heartbeat_age_seconds"),
        _M("zeus.cron.scheduler.last_success_age_seconds"),
        _M("zeus.cron.scheduler.catch_up_occurrences"),
        _M("zeus.cron.jobs.enabled"), _M("zeus.cron.jobs.running"),
        _M("zeus.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "ZeusAgent-owned Relay transport health" in text
