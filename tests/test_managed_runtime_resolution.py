"""Guard: ZeusAgent-owned subprocesses must not resolve managed runtimes by bare PATH.

ZeusAgent installs runtimes for itself — ``uv`` at ``$ZEUS_HOME/bin/uv``, Node at
``$ZEUS_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in ZeusAgent's own
...
* when a system copy also exists, the one ZeusAgent does not own wins — which is
...
``iter_zeus_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes ZeusAgent provisions into ZEUS_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not ZeusAgent-owned subprocess code: plugins ship their own
...
_ALLOWED: dict[tuple[str, str], str] = {
    ...
    ("zeus_cli/update_cmd_deps.py", "uv"): (
    ...
    ("zeus_cli/update_cmd_deps.py", "npm"): (
...
def _source_files() -> list[Path]:
    ...
    # (zeus_agent-<version>/) gets created and deleted concurrently, and
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a ZeusAgent-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$ZEUS_HOME/bin (uv) and $ZEUS_HOME/node are not on an "
        ...
        "  node/npm -> zeus_constants.find_node_executable()\n"
        "  PATH env -> zeus_constants.iter_zeus_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(zeus_constants, helper))


def test_managed_uv_helpers_exist():
    from zeus_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
