"""Behavioral tests for inherited Termux Python-selection helpers.

This does not assert that the disabled hosted installer can be used to install
the fork. The helper is loaded through its read-only manifest; commands and
interpreter versions are synthetic and all install paths are temporary.
"""
...
SETUP_ZEUS_SH = REPO_ROOT / "setup-zeus.sh"
...
def _termux_env(tmp_path: Path, bin_dir: Path) -> dict[str, str]:
    ...
    env.update({
        ...
        "ZEUS_HOME": str(tmp_path / "home" / ".zeus"),
...
def _run_install_prerequisites(tmp_path: Path) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run(
        [bash, "-c", 'source "$1" --manifest >/dev/null; NON_INTERACTIVE=true; '
         'run_stage_protocol prerequisites', "legacy-helper-test", str(INSTALL_SH)],
        env=env,
        text=True,
...
def _copy_setup_checkout(tmp_path: Path) -> Path:
    ...
    checkout.mkdir()
    shutil.copy2(SETUP_ZEUS_SH, checkout / "setup-zeus.sh")
    return checkout
...
def _run_setup(tmp_path: Path) -> subprocess.CompletedProcess[str]:
    ...
    bash = shutil.which("bash") or "/bin/bash"
    return subprocess.run(
        [bash, str(checkout / "setup-zeus.sh")],
        env=env,
        input="n\n",
        text=True,
        stdout=subprocess.PIPE,
...
def test_install_stage_rejects_post_install_unsupported_default(tmp_path: Path) -> None:
    ...
    assert "ZeusAgent requires Python >=3.11,<3.14" in result.stdout
