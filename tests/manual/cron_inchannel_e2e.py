...
Uses a throwaway ZEUS_HOME so it never touches ~/.zeus. No real names.
"""
...
def _fresh_home():
    """Point ZEUS_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_zeus_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["ZEUS_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER ZEUS_HOME is set.
...
# a different get_zeus_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"ZEUS_HOME (throwaway): {HOME}")
