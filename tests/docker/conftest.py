...
Override the image with ``ZEUS_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("ZEUS_TEST_IMAGE", "zeus-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``ZEUS_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("ZEUS_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged zeus user
# ---------------------------------------------------------------------------
#
# Background: every ZeusAgent runtime path inside the container drops to UID
# 10000 (the ``zeus`` user) via ``s6-setuidgid zeus``. ``docker exec``
...
# fails (PermissionError swallowed) for zeus, so a test that ran as root
...
# helpers below run every probe under ``-u zeus`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: zeus).
