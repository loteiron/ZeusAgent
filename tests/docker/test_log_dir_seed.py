"""Runtime smoke test for Docker $ZEUS_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the zeus user after container boot.
...
registered later runs its log service as the dropped zeus user and
...
def test_logs_gateways_seeded_and_zeus_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by zeus after boot."""
    ...
    assert "logs=zeus" in r.stdout, (
        f"logs/ not owned by zeus: {r.stdout}"
    )
    assert "gateways=zeus" in r.stdout, (
        f"logs/gateways/ not owned by zeus: {r.stdout}"
    )
