...
Build the real image and verify at runtime:
  ...
  1. /opt/zeus is not writable by the zeus user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and ZEUS_DISABLE_LAZY_INSTALLS are set
  3. /opt/zeus/.install_method contains "docker" (code-scoped stamp)
  4. $ZEUS_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $ZEUS_HOME is healed (removed) on boot
"""
...
def test_install_tree_not_writable_by_zeus(
    built_image: str, container_name: str,
) -> None:
    """The zeus user must not be able to modify /opt/zeus.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/zeus as the zeus user
        "touch /opt/zeus/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"zeus user can write to /opt/zeus (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/zeus/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"zeus user can write to /opt/zeus/.venv: {r.stdout}"
    )


def test_zeus_disable_lazy_installs_and_dont_write_bytecode(
    built_image: str, container_name: str,
) -> None:
    """The container must set PYTHONDONTWRITEBYTECODE and
    ZEUS_DISABLE_LAZY_INSTALLS=1 so no .pyc files are written to the
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $ZEUS_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $ZEUS_HOME was not healed on boot: "
        f"{r.stdout}"
    )
