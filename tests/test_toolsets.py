...
class TestToolsetConsistency:
    ...
    def test_zeus_platforms_share_core_tools(self):
        """All zeus-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on zeus-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["zeus-cli", "zeus-telegram", "zeus-discord", "zeus-whatsapp", "zeus-slack", "zeus-signal", "zeus-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_zeus_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("zeus-whatsapp")
...
class TestResolveToolsetMemo:
    ...
    def test_second_resolution_is_cached(self, monkeypatch):
        ...
        first = resolve_toolset("zeus-cli")
        second = resolve_toolset("zeus-cli")
        ...
        assert (
            "zeus-cli", True, registry_id, generation
    ...
    def test_memo_result_matches_fresh_resolution(self):
        ...
        toolsets_mod._resolve_toolset_memo.clear()
        first = resolve_toolset("zeus-cli", include_registry=False)
        second = resolve_toolset("zeus-cli", include_registry=False)
        assert first == second
