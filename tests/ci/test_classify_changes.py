...
CASES = {
    ...
    "product python → nix": (["zeus_cli/config.py"], _lanes(python=True, scan=True)),
    ...
    # files change — not on every zeus_state.py PR.
    ...
    "python source alone → no desktop_updater lane": (["zeus_state.py"], _lanes(python=True, scan=True)),
...
def test_ci_jobs_only_gate_on_detect_outputs_that_detect_actually_declares():
    ...
    """
    ci = _yaml(".github/upstream-workflows/ci.yaml")
