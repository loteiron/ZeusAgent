"""Tests for pip entry-point provider discovery (zeus_agent.plugins group).

Verifies that ``providers/__init__.py`` imports provider plugins exposed via a
distribution's ``zeus_agent.plugins`` entry point, supporting both a
