"""display_zeus_home() renders POSIX separators on every platform.

Maintainer catch (#95681 arc): on Windows with a custom ZEUS_HOME under
the user profile (e.g. AppData/Local/zeus), ``"~/" +
str(home.relative_to(Path.home()))`` produced the mixed-separator chimera
``~/AppData\\Local\\zeus`` — which then leaked into every consumer that
appends sub-paths (the skill_manage schema showed the agent
``~/AppData\\Local\\zeus/skills/``). The ``~/`` shorthand implies POSIX
...
class TestDisplayZeusAgentHomePosix(unittest.TestCase):
    def test_nested_home_renders_forward_slashes(self):
        ...
        nested = fake_userhome / "AppData" / "Local" / "zeus"
        with patch.object(hc, "get_zeus_home", return_value=nested):
            out = hc.display_zeus_home()
        self.assertEqual(out, "~/AppData/Local/zeus")
        self.assertNotIn("\\", out)

    def test_default_home_unchanged(self):
        import zeus_constants as hc

        with patch.object(hc, "get_zeus_home",
                          return_value=Path.home() / ".zeus"):
            out = hc.display_zeus_home()
        self.assertEqual(out, "~/.zeus")

    def test_outside_home_falls_back_to_absolute(self):
        import zeus_constants as hc

        outside = Path("/opt/zeus-custom") if os.name != "nt" else Path("C:/opt/zeus-custom")
        with patch.object(hc, "get_zeus_home", return_value=outside):
            out = hc.display_zeus_home()
