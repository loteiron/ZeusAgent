...
binary download are exercised manually by `zeus secrets bitwarden
...
def zeus_home(tmp_path, monkeypatch):
    """Point ZeusAgent at an isolated home directory."""
    home = tmp_path / ".zeus"
    home.mkdir()
    monkeypatch.setenv("ZEUS_HOME", str(home))
    # Some modules cache get_zeus_home; clear if needed.
    import zeus_constants
    if hasattr(zeus_constants, "_ZEUS_HOME_CACHE"):
        zeus_constants._ZEUS_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(zeus_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from zeus_cli.env_loader import _apply_external_secret_sources
