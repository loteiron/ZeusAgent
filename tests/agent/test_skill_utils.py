...
class TestParseConfigStringList:
    """#86661: `zeus config set` and JSON-mode editor saves store lists as
    ...
    def test_python_literal_array_string_parses(self):
        # `zeus config set` can persist single-quoted Python-literal forms.
...
def test_skill_config_home_vars_use_subprocess_home(tmp_path, monkeypatch):
    ...
    zeus_home = tmp_path / "data"
    subprocess_home = zeus_home / "home"
    subprocess_home.mkdir(parents=True)
    (zeus_home / "config.yaml").write_text("", encoding="utf-8")
    monkeypatch.setenv("ZEUS_HOME", str(zeus_home))
    monkeypatch.setenv("HOME", str(zeus_home))
