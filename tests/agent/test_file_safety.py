...
class TestCacheFileReadBlocking:
    """Internal ZeusAgent cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        zeus_home = tmp_path / ".zeus"
        cache = zeus_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._zeus_home_path", return_value=zeus_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal ZeusAgent cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        zeus_home = tmp_path / ".zeus"
        hub = zeus_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_zeus_home(self, tmp_path):
        """The env basename guard does not depend on ZEUS_HOME resolution."""
        zeus_home = tmp_path / ".zeus"
        zeus_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        zeus_home = tmp_path / ".zeus"
        cache = zeus_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
