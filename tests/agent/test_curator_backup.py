...
def backup_env(monkeypatch, tmp_path):
    """Isolate ZEUS_HOME + reload modules so every test starts clean."""
    home = tmp_path / ".zeus"
    home.mkdir()
    (home / "skills").mkdir()
    ...
    # Reload so get_zeus_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd zeus_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under ZEUS_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level ZEUS_DIR picks up the tmp HOME."""
    import zeus_constants
    importlib.reload(zeus_constants)
