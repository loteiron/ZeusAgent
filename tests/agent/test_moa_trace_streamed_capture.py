...
path via ``zeus chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp ZEUS_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point ZEUS_HOME at a temp dir and turn moa.save_traces on."""
    zeus_home = tmp_path / ".zeus"
    zeus_home.mkdir()
    monkeypatch.setenv("ZEUS_HOME", str(zeus_home))

    # save_moa_turn reads config via zeus_cli.config.load_config; stub it to
