...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate ZEUS_HOME under tmp_path."""
    monkeypatch.setenv("ZEUS_HOME", str(tmp_path / ".zeus"))
    (tmp_path / ".zeus").mkdir()

    # Force the constants/image cache helpers to re-read ZEUS_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("zeus_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_zeus_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under ZEUS_HOME — gateway cleanup
