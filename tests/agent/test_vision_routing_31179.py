...
def isolated_home(monkeypatch):
    """Temp ZEUS_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="zeus_test_31179_")
    zeus_home = os.path.join(test_home, ".zeus")
...
def _fresh_modules():
    """Drop cached zeus modules so each test reloads against current env."""
    _drop_reload_targets()
