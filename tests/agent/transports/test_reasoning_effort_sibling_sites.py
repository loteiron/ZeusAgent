...
class TestKimiEffortVocabulary:
    def test_k3_maps_full_zeus_ladder(self):
        expected = {
            "minimal": "low",
            "low": "low",
            "medium": "high",
        ...
        for zeus_level, wire_level in expected.items():
            kw = _kimi_kwargs(
                "kimi-k3", {"enabled": True, "effort": zeus_level}
            )
            assert kw["reasoning_effort"] == wire_level, zeus_level
