...
class TestOrgPullIsWiredIn:
    ...
    def test_sync_pull_command_refreshes_org_mirror(self):
        ...
        assert "maybe_pull_org_skills" in main_src, (
            "`zeus sync pull` must also refresh the org mirror."
        )
    ...
    def test_no_internal_jargon_in_user_facing_strings(self):
        ...
        targets = [
            root / "zeus_cli" / "subcommands" / "sync.py",
            root / "zeus_cli" / "subcommands" / "skills.py",
        ]
        banned = re.compile(
...
class TestSkillSyncIsOneCommand:
    """Every Skill Sync verb lives under `zeus sync` for launch.
    ...
    belong elsewhere. `propose` in particular used to sit under `zeus
    ...
    def test_propose_is_a_sync_subcommand(self):
        sync_src = self._src("zeus_cli", "subcommands", "sync.py")
        assert '"propose"' in sync_src, (
            "`propose` must be a `zeus sync` subcommand."
        )

    def test_propose_is_not_under_skills(self):
        skills_src = self._src("zeus_cli", "subcommands", "skills.py")
        assert '"propose"' not in skills_src, (
            "`propose` must NOT remain under `zeus skills` — Skill Sync is "
            "one command for launch."
        )

    def test_sync_usage_lists_propose(self):
        main_src = self._src("zeus_cli", "main_platform_setup.py")
        usage_start = main_src.index("usage: zeus sync ")
        usage_block = main_src[usage_start : usage_start + 1400]
        assert "propose" in usage_block, (
            "`zeus sync` usage must list the propose verb."
        )
...
class TestLocalEditsSurviveOrgUpdates:
    ...
    def test_auto_propose_defaults_off(self, monkeypatch):
        ...
        monkeypatch.delenv("ZEUS_SYNC_ORG_AUTO_PROPOSE", raising=False)
        monkeypatch.setattr(
            "zeus_cli.config.load_config", lambda: {}, raising=False
        )
    ...
    def test_auto_propose_can_be_enabled_by_env(self, monkeypatch):
        ...
        monkeypatch.setenv("ZEUS_SYNC_ORG_AUTO_PROPOSE", "1")
