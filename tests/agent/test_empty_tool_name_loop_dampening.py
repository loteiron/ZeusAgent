...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated ZEUS_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="zeus_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".zeus"))
    prev_home = os.environ.get("ZEUS_HOME")
    os.environ["ZEUS_HOME"] = os.path.join(test_home, ".zeus")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("zeus_"):
            del sys.modules[mod]
    ...
    try:
        ...
        # references into zeus_cli.*/tools.* and their monkeypatches would otherwise
        ...
        if prev_home is None:
            ...
            os.environ["ZEUS_HOME"] = prev_home
