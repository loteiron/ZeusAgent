...
from zeus_cli import providers as _providers
...
class TestApiModeRouting:
    ...
    def test_determine_api_mode_honors_the_model_for_nous(self):
        ...
        empty-mode path) must still land Claude on Messages — the ZeusAgent
...
class TestClientShape:
    ...
    def test_portal_bearer_does_not_also_send_env_anthropic_api_key(
        ...
        constructor omits it. ZeusAgent loads that env from ~/.zeus/.env, so
...
class TestPortalBodyFields:
    ...
    def test_portal_tags_reach_the_messages_request(self):
        from agent.portal_tags import zeus_client_tag

        tags = self._build()["extra_body"]["tags"]

        assert "product=zeus-agent" in tags
        assert zeus_client_tag() in tags
