...
# ---------------------------------------------------------------------------
# Helpers — set up a fake ZeusAgent root with two profiles, monkeypatch the
...
def _set_active_home(monkeypatch, zeus_home: Path):
    """Point file_safety._zeus_home_path at a specific profile dir."""
    import agent.file_safety as fs
...
class TestResolveActiveProfileName:
    def test_default_when_home_is_root(self, fake_zeus, monkeypatch):
        _set_active_home(monkeypatch, fake_zeus["default_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "default"


    def test_falls_back_to_default_on_resolution_failure(self, fake_zeus, monkeypatch):
        """If ZEUS_HOME resolution raises, return 'default' rather than crashing the tool."""
        import agent.file_safety as fs

        def _boom():
            raise RuntimeError("simulated")

        monkeypatch.setattr(fs, "_zeus_home_path", _boom)
