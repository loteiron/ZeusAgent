...
``zeus_subprocess_env`` / ``_sanitize_subprocess_env``, which the factory
...
``ZEUS_HOME`` or secret-scrub propagation.
...
SCAN_DIRS = ("agent", "zeus_cli", "tools", "gateway", "cron", "tui_gateway")
SCAN_ROOT_FILES = ("cli.py", "zeus_constants.py")
...
ALLOWED_RAW_SPAWN_ENV_FILES = {
    # THE owner module: _sanitize_subprocess_env / zeus_subprocess_env /
