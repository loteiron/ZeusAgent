"""Shared fixtures for the zeus-agent test suite.
...
2. **Isolated ZEUS_HOME.** ZEUS_HOME points to a per-test tempdir so
   code reading ``~/.zeus/*`` via ``get_zeus_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".zeus"`` instead of the canonical
   ``get_zeus_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ZEUS_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox ZEUS_HOME before ANY test module is imported ──────────────────
# `zeus_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_zeus_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.zeus/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes ZEUS_HOME, but fixtures run
...
# the REAL ZeusAgent root — capture it BEFORE the sandbox rewires ZEUS_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.zeus (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("ZEUS_KANBAN_HOME", "").strip()
_PRE_SANDBOX_ZEUS_HOME = os.environ.get("ZEUS_HOME", "")


def _zeus_home_points_at_production(value: str) -> bool:
    """True when a pre-set ZEUS_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    ZEUS_HOME=~/.zeus``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``zeus_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.zeus`` — the escape vector that landed pytest
    ...
    ZEUS_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".zeus").resolve()
    except Exception:
        return True
...
if _zeus_home_points_at_production(os.environ.get("ZEUS_HOME", "")):
    _SESSION_ZEUS_HOME = tempfile.mkdtemp(prefix="zeus-test-home-")
    os.environ["ZEUS_HOME"] = _SESSION_ZEUS_HOME
    atexit.register(shutil.rmtree, _SESSION_ZEUS_HOME, True)
...
# like a real CLI") — which used to disarm zeus_state's live-DB guard in
# the child at the same moment the child lost the ZEUS_HOME redirect.
# ZEUS_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# zeus_state_guard._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export ZEUS_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["ZEUS_TEST_ISOLATION"] = os.environ.get("ZEUS_HOME", "") or "1"

#: ZEUS_HOME as it stood when conftest was imported - i.e. before any test
...
ZEUS_HOME_AT_CONFTEST_IMPORT = os.environ.get("ZEUS_HOME", "")
...
# ZEUS_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_ZEUS_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``zeus_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a ZeusAgent agent session inherits them and
    # zeus_constants home-resolution helpers prefer them over monkeypatched
    ...
    "ZEUS_MAX_ITERATIONS",
    "ZEUS_SESSION_PLATFORM",
    "ZEUS_SESSION_CHAT_ID",
    "ZEUS_SESSION_CHAT_NAME",
    "ZEUS_SESSION_CHAT_TYPE",
    "ZEUS_SESSION_THREAD_ID",
    "ZEUS_SESSION_SOURCE",
    "ZEUS_SESSION_KEY",
    "ZEUS_GATEWAY_SESSION",
    "ZEUS_CRON_SESSION",
    "_ZEUS_GATEWAY",
    "ZEUS_PLATFORM",
    "ZEUS_MODEL",
    "ZEUS_INFERENCE_MODEL",
    "ZEUS_INFERENCE_PROVIDER",
    "ZEUS_TUI_PROVIDER",
    "ZEUS_MANAGED",
    "ZEUS_MANAGED_DIR",
    "ZEUS_DEV",
    "ZEUS_CONTAINER",
    "ZEUS_EPHEMERAL_SYSTEM_PROMPT",
    "ZEUS_TIMEZONE",
    "ZEUS_REDACT_SECRETS",
    "ZEUS_BACKGROUND_NOTIFICATIONS",
    "ZEUS_EXEC_ASK",
    "ZEUS_HOME_MODE",
    "ZEUS_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.zeus/kanban.db instead of the per-test ZEUS_HOME.
    "ZEUS_KANBAN_DB",
    "ZEUS_KANBAN_BOARD",
    "ZEUS_KANBAN_HOME",
    "ZEUS_KANBAN_WORKSPACES_ROOT",
    "ZEUS_KANBAN_LOGS_ROOT",
    "ZEUS_KANBAN_TASK",
    "ZEUS_KANBAN_WORKSPACE",
    "ZEUS_KANBAN_RUN_ID",
    "ZEUS_KANBAN_CLAIM_LOCK",
    "ZEUS_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    # otherwise-unrelated config tests away from the default "zeus" host.
    "ZEUS_HONCHO_HOST",
    ...
    "ZEUS_DASHBOARD_OAUTH_CLIENT_ID",
    "ZEUS_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ZEUS_HOME to per-test tempdirs so code that
    reads ``~/.zeus/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ZEUS_* vars that could change test semantics.
    for name in _ZEUS_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("ZEUS_HONCHO_HOST", "zeus")

    # 3. Redirect ZEUS_HOME to a per-test tempdir. Code that reads
    #    ``~/.zeus/*`` via ``get_zeus_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.zeus/*`` via
    #    ``Path.home() / ".zeus"`` instead of ``get_zeus_home()``
    #    is a bug to fix at the callsite.
    fake_zeus_home = tmp_path / "zeus_test"
    fake_zeus_home.mkdir()
    (fake_zeus_home / "sessions").mkdir()
    (fake_zeus_home / "cron").mkdir()
    (fake_zeus_home / "memories").mkdir()
    (fake_zeus_home / "skills").mkdir()
    monkeypatch.setenv("ZEUS_HOME", str(fake_zeus_home))
    ...
    # zeus_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("ZEUS_TEST_ISOLATION", str(fake_zeus_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("ZEUS_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. zeus_state computes ``DEFAULT_DB_PATH = get_zeus_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from zeus_state import ...``) that
    ...
    zeus_state_mod = sys.modules.get("zeus_state")
    if zeus_state_mod is not None and hasattr(zeus_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            zeus_state_mod, "DEFAULT_DB_PATH", fake_zeus_home / "state.db"
        )
    ...
    monkeypatch.setenv("ZEUS_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.zeus/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import zeus_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path ZEUS_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_zeus_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from zeus_cli import kanban_db_dispatch as _kbd_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.zeus. This autouse
...
# allow-list because test-level fixtures legitimately move ZEUS_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session ZEUS_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``ZEUS_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) ZeusAgent root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_ZEUS_HOME and not _zeus_home_points_at_production(
        _PRE_SANDBOX_ZEUS_HOME
    ):
        # ZEUS_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_zeus_root().resolve()
    # No pre-existing ZEUS_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".zeus").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.zeus`` captured at import time. Hermetic tests that legitimately
    move ZEUS_HOME to sibling tempdirs are unaffected.

    Only patches when ``zeus_cli.kanban_db_connect`` is *already imported*
    ...
    """
    _kdb = sys.modules.get("zeus_cli.kanban_db")
    _kdbc = sys.modules.get("zeus_cli.kanban_db_connect")
    if _kdb is None or _kdbc is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import zeus_cli.kanban_db`
...
# ``zeus_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL ZeusAgent root. This fixture wires the
# test-side knobs:
...
#     installs where ZEUS_HOME is not ~/.zeus) into the guard's
...
# so subprocess children that import zeus_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("zeus_state")
    ...
    if _PRE_SANDBOX_ZEUS_HOME and not _zeus_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_ZEUS_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local ZeusAgent home override redirects every later
    # ``get_zeus_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from zeus_constants import get_zeus_home_override, set_zeus_home_override

        if get_zeus_home_override() is not None:
            set_zeus_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal zeus config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``zeus-gateway`` process
...
# correlated with a ``tests/zeus_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> zeus-gateway``
...
def _wal_is_usable() -> bool:
    """True when ZeusAgent will actually put a database into WAL mode here.

    ZeusAgent refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running ZeusAgent can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a ZeusAgent managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``zeus_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_zeus_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_zeus_home`` fixture
    redirects ``ZEUS_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.zeus/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``zeus_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["ZEUS_VOICE_TTS"] = "1"``. The
...
#      ``zeus_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``zeus_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``zeus_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``zeus_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# ZeusAgent runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.zeus``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where ZeusAgent falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — ZeusAgent uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart zeus-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting zeus/python patterns are also blocked.
    """
    ...
    def _matches_zeus_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _ZEUS_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_zeus_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch zeus-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m zeus_cli.main".
                if (
                    "zeus" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live zeus-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting zeus/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `zeus update` (or the
        # equivalent `python -m zeus_cli.main update`).  These commands
        ...
        if "update" in low and (
            # zeus update / zeus update --gateway / setsid bash -c ... zeus update
            ("zeus" in low and "update" in low.split())
            or
            # python -m zeus_cli.main update --gateway
            ("zeus_cli" in low and "update" in low.split())
            or
            # venv/bin/zeus update  (absolute path variant used in tests)
            (".venv/bin/zeus" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`zeus update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_ZEUS_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import zeus_cli.voice as _voice
    except Exception:
