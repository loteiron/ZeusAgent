"""The conftest WAL gate must agree with zeus_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``zeus_state``. That is deliberate: importing
``zeus_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
``~/.zeus`` before the per-test ``ZEUS_HOME`` redirect, which makes tests
...
def test_conftest_gate_agrees_with_zeus_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and zeus_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_zeus_state_at_collection():
    """The gate must stay import-free of zeus_state.

    Importing it during collection caches DEFAULT_DB_PATH from the real
    ~/.zeus, so tests read live production sessions instead of a tempdir.
    Reading the source is not an option here (banned), so assert on behavior:
    the gate must work with ``zeus_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "zeus_state" or name.startswith("zeus_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported zeus_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.zeus during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("zeus_state", None)
