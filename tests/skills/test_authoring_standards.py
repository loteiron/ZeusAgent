...
def test_required_frontmatter_fields(p):
    ...
    zeus = (fm.get("metadata") or {}).get("zeus") or {}
    if not (zeus.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.zeus.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    zeus = (fm.get("metadata") or {}).get("zeus") or {}
    dangling = [
        rs for rs in (zeus.get("related_skills") or []) if rs not in _all_names()
    ]
