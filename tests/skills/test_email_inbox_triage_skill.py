...
def test_frontmatter_required_fields():
    ...
    zeus = fm["metadata"]["zeus"]
    assert zeus["tags"]
    assert "related_skills" in zeus
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("ZeusAgent"), "human contributor must be credited first"
...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["zeus"]["related_skills"]:
        hits = (
