...
"""
    ...
    assert "Tyler Williams > Active Projects: ZeusAgent" in entries
...
def test_unreadable_config_is_refused_not_overwritten(tmp_path: Path):
    ...
    ``zeus_cli/agent_import.py``.
    """
    mod = load_module()
    migrator, config_path = _allowlist_migrator(
        mod, tmp_path, MALFORMED_ZEUS_CONFIG)
...
def test_unreadable_config_blocks_later_config_steps_instead_of_partial_writes(
    ...
    assert config_path.read_text(encoding="utf-8") == MALFORMED_ZEUS_CONFIG
...
def test_symlinked_config_stays_a_symlink(tmp_path: Path):
    """Managed deployments symlink ~/.zeus/config.yaml into a dotfiles repo.
...
def test_unreadable_config_refused_by_model_config_too(tmp_path: Path):
    ...
    config_path = target / "config.yaml"
    config_path.write_text(MALFORMED_ZEUS_CONFIG, encoding="utf-8")
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # user at ~/.zeus/config.yaml in its post-migration summary — it never
    # writes to either. Under skills-guard-v2 (#92021) these score as
    # informational _ref findings (the old critical agent_config_mod /
    # zeus_config_mod findings no longer fire for bare mentions), so the
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_ref", "zeus_config_ref"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``ZeusAgent``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "ZeusAgent prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told ZeusAgent to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "ZeusAgent config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "ZeusAgent uses tools well"
    # All-lowercase matches → lowercase ``zeus``; this preserves the
    # real filesystem path ``~/.zeus`` (ZeusAgent home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "zeusagent should always respond concisely"
