...
class TestGoogleWorkspaceCredentialFiles:
    ...
    def test_entries_are_registered_when_files_exist(self, tmp_path):
        ...
        zeus_home.mkdir()
        (zeus_home / "google_token.json").write_text("{}")
        (zeus_home / "google_client_secret.json").write_text("{}")
        ...
        try:
            ...
            assert "/root/.zeus/google_token.json" in container_paths
            assert "/root/.zeus/google_client_secret.json" in container_paths
        finally:
            clear_credential_files()
