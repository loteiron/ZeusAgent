...
import zeus_state
import zeus_state_holders
import zeus_state_schema
from zeus_state import SessionDB
from zeus_state_common import FTS_REBUILD_DEFERRAL_KEY, FTS_STALE_KEY, LEGACY_FTS_SQL, LEGACY_FTS_TRIGRAM_SQL, SCHEMA_SQL, _FTS_TRIGGERS
from zeus_state_dbfile import _concrete_state_db_holder_pids, _is_inactive_orphan_desktop_holder
...
class TestRuntimeFtsRebuild:
    def test_reap_candidates_exclude_uninspectable_holder_suspicions(
        ...
        assert _concrete_state_db_holder_pids(
            db_path,
            [
                (222, "uninspectable holder: python -m zeus_cli.main serve --port 0"),
    ...
    @pytest.mark.parametrize(
        "argv",
        (
            ("journalctl", "-u", "zeus-agent.service"),
            ("grep", "zeus-agent", "/var/log/syslog"),
            (
                "/usr/sbin/tailscaled",
                "be-child",
                "ssh",
                "--cmd=python -m zeus_cli.main gateway",
            ),
            ("tmux", "new-session", "/opt/zeus-agent/.venv/bin/zeus gateway"),
            ("python3", "/opt/zeus-agent/tools/check_state.py"),
            ("zeus-monitor", "gateway"),
            ("zeusctl", "serve"),
            ("python3", "worker.py", "zeus_cli.main"),
            ("python3", "-m", "other.module", "zeus_cli.main"),
            ("python3", "-c", "zeus_cli.main"),
            ("python3", "-Icprint('zeus_cli.main')", "zeus_cli/main.py"),
        ),
    )
    def test_uninspectable_non_zeus_process_is_not_a_holder(self, argv):
        assert not zeus_state_holders._looks_like_zeus(argv)

    @pytest.mark.parametrize(
        "argv",
        (
            ("/usr/local/bin/zeus", "gateway"),
            ("/usr/local/bin/zeus-agent", "serve"),
            ("/usr/local/bin/zeus-acp", "--stdio"),
            ("/usr/bin/python3", "-m", "zeus_cli.main", "gateway"),
            ("/usr/bin/python3", "-m", "acp_adapter"),
            ("/usr/bin/python3", "-Im", "zeus_cli.main", "gateway"),
            ("/usr/bin/python3", "-mzeus_cli.main", "gateway"),
            ("/usr/bin/python3", "-W", "ignore", "-m", "zeus_cli.main"),
            ("/usr/bin/python3", "-Xdev", "-m", "zeus_cli.main"),
            (
                "/opt/zeus-agent/.venv/bin/python",
                "/opt/zeus-agent/zeus_cli/main.py",
                "gateway",
            ),
            ("python.exe", "--", "zeus_cli/main.py", "gateway"),
            ("python3", "/opt/zeus-agent/run_agent.py", "--query", "hello"),
        ),
    )
    def test_uninspectable_zeus_process_remains_a_holder(self, argv):
        assert zeus_state_holders._looks_like_zeus(argv)
    ...
    def test_foreign_holder_detection_proc_readlink_deleted_wal(
        ...
        monkeypatch.setattr(zeus_state_holders.os, "getpid", lambda: 111)
        real_listdir = os.listdir
        def _listdir(path):
            if isinstance(path, str):
                path = path.replace("/proc", str(proc_root))
            return real_listdir(path)
        monkeypatch.setattr(zeus_state_holders.os, "listdir", _listdir)
        real_readlink = os.readlink
        def _readlink(path):
            path = path.replace("/proc", str(proc_root))
            return real_readlink(path)
        monkeypatch.setattr(zeus_state_holders.os, "readlink", _readlink)
        real_stat = os.stat
        ...
        monkeypatch.setattr(zeus_state_holders.os, "stat", _stat)

        holders = zeus_state_holders.foreign_state_db_holders(db_path)
    ...
    def test_foreign_holder_ignores_same_path_with_different_file_identity(
        ...
        assert zeus_state_holders.foreign_state_db_holders(db_path) == []
    ...
    def test_foreign_holder_uninspectable_process_cmdline_fallback(
        ...
        flagged when /proc/<pid>/cmdline identifies it as a ZeusAgent process."""
        db_path = tmp_path / "state.db"
        ...
        # PID 222's cmdline is world-readable and looks like ZeusAgent
        cmdline_path = proc_root / "222" / "cmdline"
        cmdline_path.write_bytes(
            b"python3\x00-m\x00zeus_cli.main\x00chat\x00"
        )
        ...
        def _fake_argv(pid):
            ...
            try:
                ...
                return None
        monkeypatch.setattr(zeus_state_holders, "_read_proc_argv", _fake_argv)

        holders = zeus_state_holders.foreign_state_db_holders(db_path)
        # Should include PID 222 with the cmdline info
        assert len(holders) == 1
        assert holders[0][0] == 222
        assert "zeus_cli.main" in holders[0][1]
    ...
    def test_structural_corruption_propagates_without_live_fts_mutation(
        ...
        from zeus_state import StateDbCorruptError
    ...
    def test_repeated_deferrals_reap_inactive_orphan_then_rebuild(
        ...
        monkeypatch.setattr(zeus_state_schema.time, "time", lambda: 120.0)
...
class TestPhysicalCorruptionAcceptance:
    ...
    def test_canonical_btree_corruption_fails_closed(
        ...
        try:
            caplog.clear()
            with caplog.at_level("WARNING", logger="zeus_state"):
                with pytest.raises(sqlite3.DatabaseError) as caught:
