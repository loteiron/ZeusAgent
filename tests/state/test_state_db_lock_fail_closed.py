...
work on a file several ZeusAgent processes share (gateway service, the Desktop
app's `zeus serve` backend, CLI sessions, the TUI slash worker):

* `zeus_state_common.fts_rebuild_admission` — full structural FTS rebuilds
* `zeus_state_repair._cross_process_repair_lock`   — writable_schema surgery / VACUUM
