...
cannot prove belongs to this ZEUS_HOME.

Design contract (v3, after andrexibiza's second review): ownership is decided
by the persisted identity record ALONE — exact ``_same_zeus_home`` equality
bound to the live target by exact PID + start-time. A readable live argv
carries no ZEUS_HOME, so it can never prove home ownership; it only feeds a
...
def profile_env(tmp_path, monkeypatch):
    """Isolated ZEUS_HOME mirroring tests/zeus_cli/test_profiles.py."""
...
def _record(pid=424242, start=111222333, home=None, argv=None):
    return {
        "pid": pid,
        "kind": "zeus-gateway",
        "argv": argv or ["python", "-m", "zeus_cli.main", "gateway", "run"],
        "start_time": start,
        "zeus_home": home,
    }
...
class TestRecordAuthority:
    def test_valid_bound_same_home_record_allows(self, profile_env):
        ...
        with (
            patch(
                ...
                return_value=_record(home=str(profile_env / ".zeus")),
            ),
            patch(
                "gateway.status._get_pid_path",
                return_value=profile_env / ".zeus" / "gateway.pid",
            ),
            patch(
            ...
            patch(
                "gateway.status._get_process_zeus_home",
                return_value=profile_env / ".zeus",
            ),
        ):
        ...
        with (
            patch(
                ...
                return_value=_record(
                    home="/home/other/.zeus/profiles/timothy"
                ),
            ),
            patch(
                "gateway.status._get_pid_path",
            ...
            patch(
                "gateway.status._get_process_zeus_home",
                return_value=profile_env / ".zeus" / "profiles" / "tim",
            ),
        ):
    ...
    def test_legacy_record_without_home_refuses(self, profile_env):
        """A pre-zeus_home-stamping record cannot prove ownership."""
        from gateway.run import _replace_target_belongs_to_other_profile

        legacy = _record(home=None)
        legacy.pop("zeus_home")
    ...
    def test_unbound_record_wrong_pid_refuses(self, profile_env):
        ...
        with (
            patch(
                "gateway.status._read_pid_record",
                return_value=_record(pid=999999, home=str(profile_env / ".zeus")),
            ),
            patch(
                "gateway.status._get_pid_path",
                return_value=profile_env / ".zeus" / "gateway.pid",
    ...
    def test_unbound_record_stale_start_time_refuses(self, profile_env):
        ...
        with (
            patch(
                "gateway.status._read_pid_record",
                return_value=_record(start=1, home=str(profile_env / ".zeus")),
            ),
            patch(
                "gateway.status._get_pid_path",
                return_value=profile_env / ".zeus" / "gateway.pid",
...
class TestArgvConsistencyCheck:
    ...
    def test_prefix_collision_is_not_a_conflict(self, profile_env):
        ...
        tim_home = Path("/home/x/.zeus/profiles/tim")
        # Foreign target advertising timothy — NOT ours.
        assert (
            conflict("python -m zeus_cli.main --profile timothy gateway run", tim_home)
            is True
        )
        # Our own exact name stays consistent.
        assert (
            conflict("python -m zeus_cli.main --profile tim gateway run", tim_home)
            is False
        )
        assert (
            conflict("python -m zeus_cli.main -p tim gateway run", tim_home)
            is False
        )

    def test_explicit_home_flag_exact_compare(self, profile_env):
        """ZEUS_HOME= on the argv compares path-exactly, not by prefix."""
        from gateway.run import (
            _looks_like_profile_conflict_from_cmdline as conflict,
        )
        ...
        assert (
            conflict(
                "python -m zeus_cli.main ZEUS_HOME=/home/x/.zeus/profiles/timothy gateway run",
                tim_home,
            )
            is True
        )
        assert (
            conflict(
                "python -m zeus_cli.main --zeus-home /home/x/.zeus/profiles/tim/ gateway run",
                tim_home,
            )
    ...
    def test_consistency_contradiction_refuses_even_with_agreeing_record(
        ...
        with (
            ...
            patch(
                ...
                return_value="python -m zeus_cli.main --profile other-profile gateway run",
            ),
            patch(
                "gateway.status._get_process_zeus_home",
                return_value=profile_env / ".zeus",
