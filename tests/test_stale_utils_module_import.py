...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.zeus/zeus-agent/utils.py)
...
2. ``zeus update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``zeus_cli/main.py`` (the ``zeus update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``zeus_constants`` after the
