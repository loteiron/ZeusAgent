"""Tests for the Ctrl+S prompt stash wiring inside ZeusAgentCLI.

The state machine itself is covered by tests/cli/test_prompt_stash.py. These
tests verify the cli.py side:
  - ZeusAgentCLI.__init__ creates a PromptStash
...
def _make_cli(**kwargs):
    """Create a ZeusAgentCLI with prompt_toolkit stubbed out."""
    _clean_config = {
        "model": {
...
class TestStashStateInit:
    ...
    def test_stash_starts_empty(self, cli):
        ...
        # module, which re-imports zeus_cli.prompt_toolkit stubs and can
...
class TestKeybindingRegistration:
    ...
    ``ZeusAgentCLI.run()``'s local ``KeyBindings`` instance, so they are not
