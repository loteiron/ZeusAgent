...
SessionDB.session_gateway_runtime (zeus_state.py) — the round trip that
makes `zeus --resume` reopen a session on the model/provider it actually
...
def _make_stub(**overrides):
    """Bare ZeusAgentCLI the way resume paths see it (no __init__)."""
    stub = object.__new__(cli_mod.ZeusAgentCLI)
