...
class TestBranchPushedExact:
    def test_exact_match_true(self, repo_with_bare_origin):
        import cli
        repo = repo_with_bare_origin
        wt = _mk_worktree(repo, "zeus-x", "zeus/zeus-x", push=True)
        heads = worktree_ops._fetch_remote_branch_heads(str(repo))
    ...
    def test_never_pushed_false(self, repo_with_bare_origin):
        import cli
        repo = repo_with_bare_origin
        wt = _mk_worktree(repo, "zeus-z", "zeus/zeus-z", push=False)
        heads = worktree_ops._fetch_remote_branch_heads(str(repo))
        assert worktree_ops._worktree_branch_pushed_exact(str(wt), heads) is False
    ...
    def test_none_heads_false(self, repo_with_bare_origin):
        import cli
        repo = repo_with_bare_origin
        wt = _mk_worktree(repo, "zeus-n", "zeus/zeus-n", push=True)
...
class TestStartupPrunerPushedTier:
    ...
    def test_pushed_lane_tree_reaped_branch_kept(self, repo_with_bare_origin):
        import cli
        repo = repo_with_bare_origin
        wt = _mk_worktree(repo, "zeus-pushed", "zeus/zeus-pushed", push=True)
        ...
        assert _branch_exists(repo, "zeus/zeus-pushed"), (
    ...
    def test_unpushed_lane_still_preserved(self, repo_with_bare_origin):
        import cli
        repo = repo_with_bare_origin
        wt = _mk_worktree(repo, "zeus-unpushed", "zeus/zeus-unp", push=False)
    ...
    def test_ahead_of_pushed_lane_preserved(self, repo_with_bare_origin):
        ...
        repo = repo_with_bare_origin
        wt = _mk_worktree(
            repo, "zeus-ahead", "zeus/zeus-ahead",
            push=True, extra_commit_after_push=True,
        )
    ...
    def test_pushed_but_dirty_preserved(self, repo_with_bare_origin):
        import cli
        repo = repo_with_bare_origin
        wt = _mk_worktree(repo, "zeus-pdirty", "zeus/zeus-pdirty", push=True)
    ...
    def test_offline_remote_preserves(self, repo_with_bare_origin, tmp_path):
        import cli
        repo = repo_with_bare_origin
        wt = _mk_worktree(repo, "zeus-off", "zeus/zeus-off", push=True)
...
class TestCronWorktreeMaintenance:
    ...
    def test_repo_discovery_requires_worktrees_dir(self, monkeypatch, tmp_path):
        ...
        assert repo.resolve() not in {Path(item).resolve() for item in repos}

        # Adding .worktrees/ makes it eligible.
        (repo / ".worktrees").mkdir()
        repos = sched._worktree_maintenance_repos()
        assert repo.resolve() in {Path(item).resolve() for item in repos}

    def test_maintenance_prunes_via_real_pruner(self, repo_with_bare_origin, monkeypatch):
        import cron.scheduler as sched

        repo = repo_with_bare_origin
        wt = _mk_worktree(repo, "zeus-cronreap", "zeus/zeus-cronreap", push=True)
        ...
        assert _branch_exists(repo, "zeus/zeus-cronreap")
