...
def _isolate(tmp_path, monkeypatch):
    """Isolate ZEUS_HOME so tests don't touch real config."""
    home = tmp_path / ".zeus"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal ZeusAgentCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        "display": {"tool_progress": "new"},
        "terminal": {},
    }), patch("cli.get_tool_definitions", return_value=[]), \
         patch("zeus_cli.banner.build_welcome_banner"):
        from cli import ZeusAgentCLI
        obj = ZeusAgentCLI.__new__(ZeusAgentCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["zeus-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below ZeusAgent' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly ZeusAgent' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above ZeusAgent' minimum context length."""
