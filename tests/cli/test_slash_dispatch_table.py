"""Parity guard for the slash-command dispatch table in cli.ZeusAgentCLI.
...
def test_every_old_branch_resolves_to_a_handler():
    for name in OLD_CHAIN_COMMANDS:
        entry = ZeusAgentCLI._slash_handler(name)
        assert entry is not None, name
        method_name, pass_arg = entry
        assert callable(getattr(ZeusAgentCLI, method_name)), name
        assert isinstance(pass_arg, bool)
    # explicit table entries are only the ones the naming convention can't cover
    for name, (method_name, pass_arg) in ZeusAgentCLI._SLASH_DISPATCH.items():
...
def test_registry_names_resolve_into_the_table():
    from zeus_cli.commands import COMMAND_REGISTRY, resolve_command

    for name in ZeusAgentCLI._SLASH_DISPATCH:
...
def _cli():
    c = ZeusAgentCLI.__new__(ZeusAgentCLI)
...
def test_dispatch_return_semantics_and_side_effects():
    c = _cli()
    with patch.object(ZeusAgentCLI, "_toggle_yolo", return_value=None) as m, \
    ...
    c = _cli()
    with patch.object(ZeusAgentCLI, "_handle_resume_command") as m:
    ...
    c = _cli()
    with patch.object(ZeusAgentCLI, "_handle_handoff_command", return_value=False):
        assert c.process_command("/handoff telegram") is False
    with patch.object(ZeusAgentCLI, "_handle_handoff_command", return_value=True):
        assert c.process_command("/handoff telegram") is True
    with patch.object(ZeusAgentCLI, "_handle_update_command", return_value=True):
        assert c.process_command("/update") is False
    with patch.object(ZeusAgentCLI, "_handle_update_command", return_value=False):
