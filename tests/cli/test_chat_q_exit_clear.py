...
def test_print_exit_summary_clears_screen_by_default(monkeypatch):
    ...
    fake = FakeCLI()
    cli_mod.ZeusAgentCLI._print_exit_summary(fake)  # default clear_screen=True
...
def test_print_exit_summary_skips_clear_when_clear_screen_false(monkeypatch):
    ...
    fake = FakeCLI()
    cli_mod.ZeusAgentCLI._print_exit_summary(fake, clear_screen=False)
...
def test_single_query_main_skips_clear_on_exit_summary(monkeypatch):
    ...
    monkeypatch.setattr(cli_mod, "ZeusAgentCLI", FakeCLI)
