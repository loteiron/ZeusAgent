...
The defect: zeus_state already knows macOS `fsync()` does not guarantee
...
import zeus_state
from zeus_state import repair_state_db_schema
from zeus_state_repair import _connect_repair_durable
...
def test_repair_path_has_no_bare_connects() -> None:
    ...
    """
    # The repair/probe helpers live in zeus_state_repair; zeus_state only
    # re-imports them.
    import zeus_state_repair

    source = Path(zeus_state_repair.__file__).read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(zeus_state_repair.__file__))
