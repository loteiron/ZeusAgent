...
@pytest.fixture
def slack_media_dns(monkeypatch):
    """Keep the SSRF check active without depending on external Slack DNS."""
    import socket

    def lookup(host, port=None):
        assert host == "files.slack.com", host
        return [(socket.AF_INET, socket.SOCK_STREAM, 6, "", ("93.184.216.34", port or 443))]

    monkeypatch.setattr("tools.url_safety._getaddrinfo", lookup)


@pytest.mark.usefixtures("slack_media_dns")
