...
(/opt/zeus/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable ZEUS_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_zeus_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable ZEUS_HOME."""
    ...
    monkeypatch.setattr(
        "zeus_constants.get_zeus_home", lambda: zeus_home
    )
    ...
    expected = zeus_home / "scripts" / "whatsapp-bridge"
