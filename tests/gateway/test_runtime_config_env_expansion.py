...
def gateway_home(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("ZEUS_PREFILL_MESSAGES_FILE", raising=False)
    monkeypatch.delenv("ZEUS_EPHEMERAL_SYSTEM_PROMPT", raising=False)
    monkeypatch.delenv("ZEUS_GATEWAY_BUSY_INPUT_MODE", raising=False)
    monkeypatch.delenv("ZEUS_RESTART_DRAIN_TIMEOUT", raising=False)
    monkeypatch.delenv("ZEUS_BACKGROUND_NOTIFICATIONS", raising=False)
    return tmp_path
