...
def test_session_store_default_db_uses_runtime_zeus_home(tmp_path, monkeypatch):
    """SessionStore must honor runtime ZEUS_HOME when opening the default DB.

    Regression for the import-time DEFAULT_DB_PATH freeze: importing
    zeus_state before a fixture redirected ZEUS_HOME used to pin every
    default SessionDB() at the developer's real ~/.zeus/state.db.
    """
    config = GatewayConfig()
    fake_home = tmp_path / "alt_zeus_home"
    fake_home.mkdir()
    monkeypatch.setenv("ZEUS_HOME", str(fake_home))
...
class TestReadmeSentinel:
    ...
    def test_save_writes_readme_sentinel_first(self, tmp_path):
        ...
        assert "zeus sessions list" in raw["_README"]
