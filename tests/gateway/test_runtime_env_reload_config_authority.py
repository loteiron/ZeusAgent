...
ZEUS_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env ZEUS_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``zeus config get terminal.backend`` still says local.
    """
    zeus_home = tmp_path / ".zeus"
    zeus_home.mkdir()
    (zeus_home / "config.yaml").write_text(
    ...
    (zeus_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
