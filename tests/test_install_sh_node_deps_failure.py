"""Behavioral coverage for inherited Node stage helpers, not a hosted installer.

The source distribution deliberately refuses the legacy executable entrypoint.
Load its read-only manifest, then exercise the helper with synthetic npm/uv
commands and a temporary checkout. Source-setup failure handling is separately
covered by tests/zeus_cli/test_source_setup.py.
"""
...
""",
    ...
    env.update(
        {
            ...
            "ZEUS_INSTALL_DIR": str(install_dir),
    ...
    proc = subprocess.run(
        [
            "bash",
            "-c",
            'source "$1" --manifest >/dev/null; JSON_OUTPUT=true; '
            'SKIP_BROWSER=true; SKIP_COMPUTER_USE=true; run_stage_protocol node-deps',
            "legacy-helper-test",
            str(INSTALL_SH),
        ],
        cwd=REPO_ROOT,
