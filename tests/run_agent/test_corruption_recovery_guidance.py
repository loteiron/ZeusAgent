...
3. Improved "corrupt" cause wording in _format_turn_completion_explanation
   with the full recovery path (zeus doctor, sqlite3 .recover, backups)
"""
...
def test_format_turn_completion_corrupt_includes_recovery_options():
    ...
    assert "zeus doctor" in explanation
    assert ".recover" in explanation
    assert "backups" in explanation
    assert "Freeing disk space will not help" in explanation


def test_gateway_corruption_banner_backups_dir_follows_zeus_home(monkeypatch, tmp_path):
    """The gateway broadcast's step 3 must name the live backups dir, not ~/.zeus (#104250).

    Pre-update backups live at ``<zeus_root>/backups`` (``zeus_cli/backup.py``); a
    custom-ZEUS_HOME gateway must not be told to restore from a directory that never
    held its backups.
    """
    import asyncio
    ...
    assert "~/.zeus/backups" not in sent[0]
...
def test_format_turn_completion_corrupt_never_names_the_live_db():
    ...
    assert 'sqlite3 ~/.zeus/state.db ".recover"' not in explanation
