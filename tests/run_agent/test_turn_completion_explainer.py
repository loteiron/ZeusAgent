...
def _make_agent(max_iterations: int = 10, config: dict | None = None) -> AIAgent:
    with (
        ...
        patch("zeus_cli.config.load_config", return_value=config or {}),
...
def test_explanation_persistence_turn_lease_cause_is_specific():
    ...
    assert "zeus doctor" not in lower
...
def test_explanation_persistence_corrupt_cause_never_says_free_space():
    ...
    assert "zeus doctor" in lower
    assert "free some space" not in lower
    assert "full disk" not in lower


def test_explanation_persistence_corrupt_backups_dir_follows_zeus_home(monkeypatch, tmp_path):
    """Step 3 must name the backups dir under the ACTIVE home, not ~/.zeus (#104250).

    Pre-update backups live at ``<zeus_root>/backups`` (``zeus_cli/backup.py``), so a
    custom-ZEUS_HOME deployment told to restore from ``~/.zeus/backups/`` is misdirected
    mid data-loss incident: that directory may not exist at all, or may hold an unrelated
    install's backups.
    """
    custom_home = tmp_path / "custom-zeus-home"
    monkeypatch.setenv("ZEUS_HOME", str(custom_home / "profiles" / "research"))
    out = AIAgent._format_turn_completion_explanation(
        "session_persistence_failed", "corrupt"
    )
    assert f"{custom_home / 'backups'}" in out
    assert "~/.zeus/backups" not in out
...
def test_classify_persistence_error_categories():
    ...
    from zeus_state import classify_persistence_error
...
def test_classify_persistence_error_reuses_disk_full_markers():
    """The disk bucket delegates to zeus_state_errors.is_disk_full_error, so
...
def test_classify_persistence_error_compression_busy_is_distinct():
    ...
    from zeus_state import SessionCompressionInProgressError
    from zeus_state_errors import CompressionSessionBusyError
    from zeus_state import classify_persistence_error
...
def test_classify_persistence_error_turn_lease_lost_is_distinct():
    from zeus_state import classify_persistence_error
    from zeus_state_errors import SessionTurnLeaseLostError
...
def test_persistence_error_causes_tuple_matches_classifier():
    ...
    from zeus_state import classify_persistence_error
    from zeus_state_errors import PERSISTENCE_ERROR_CAUSES
...
def test_explainer_enabled_by_default():
    ...
    with patch.dict(os.environ, {}, clear=False):
        os.environ.pop("ZEUS_TURN_COMPLETION_EXPLAINER", None)
...
def test_explainer_disabled_via_env():
    agent = _make_agent()
    with patch.dict(
        os.environ, {"ZEUS_TURN_COMPLETION_EXPLAINER": "0"}, clear=False
    ):
...
def test_explainer_config_read_once_then_cached():
    ...
    with patch.dict(os.environ, {}, clear=False):
        os.environ.pop("ZEUS_TURN_COMPLETION_EXPLAINER", None)
        with patch("zeus_cli.config.load_config", counting_load):
...
def test_classify_persistence_error_quarantined_handle_is_corrupt() -> None:
    ...
    from zeus_state import StateDbCorruptError, classify_persistence_error
