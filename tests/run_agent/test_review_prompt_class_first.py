...
The review prompts steer the background review agent toward evidence-based
updates when a durable lesson emerges, with a strong bias toward:
...
def test_skill_review_prompt_requires_evidence_without_a_write_quota():
    """Routine chat must not manufacture lessons or force skill writes."""
    prompt = AIAgent._SKILL_REVIEW_PROMPT.lower()
    assert "supported by evidence" in prompt
    assert "durable lesson" in prompt
    assert "saving nothing is correct" in prompt
