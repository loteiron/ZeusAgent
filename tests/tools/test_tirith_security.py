...
@pytest.mark.skipif(not _tirith_mod.is_platform_supported(), reason="Scanner has no native binary on this platform")
@pytest.mark.parametrize("fail_open", [False, True])
@pytest.mark.parametrize("failure", [OSError("broken scanner"), subprocess.TimeoutExpired("tirith", 1)])
def test_circuit_breaker_preserves_configured_failure_policy(monkeypatch, fail_open, failure):
    monkeypatch.setattr(_tirith_mod, "_load_security_config", lambda: {
        "tirith_enabled": True, "tirith_path": "tirith",
        "tirith_timeout": 1, "tirith_fail_open": fail_open,
    })
    with patch.object(_tirith_mod.subprocess, "run", side_effect=failure) as runner:
        verdicts = [check_command_security("echo checked") for _ in range(5)]
    assert {verdict["action"] for verdict in verdicts} == {"allow" if fail_open else "block"}
    assert runner.call_count == 3  # stop spawning a broken binary, retain the policy
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(zeus_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(zeus_home / "bin" / "tirith")
    ...
    def test_ensure_installed_non_blocking(self):
        ...
        with patch("tools.tirith_security._load_security_config",
                   return_value={"tirith_enabled": True, "tirith_path": "tirith",
                                 "tirith_timeout": 5, "tirith_fail_open": True}), \
             patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._zeus_bin_dir", return_value="/nonexistent"), \
             patch("tools.tirith_security._is_install_failed_on_disk", return_value=False), \
...
class TestZeusAgentHomeIsolation:
    def test_zeus_bin_dir_respects_zeus_home(self):
        """_zeus_bin_dir must use ZEUS_HOME, not hardcoded ~/.zeus."""
        from tools.tirith_security import _zeus_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"ZEUS_HOME": tmpdir}):
            result = _zeus_bin_dir()
