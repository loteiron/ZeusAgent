...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_env._scrub_child_env — broad ZEUS_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: ZEUS_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_zeus_allowlist_and_secret_blocks():
    from tools.code_execution_env import _scrub_child_env

    env = {
        # operational allowlist → kept
        "ZEUS_HOME": "/h", "ZEUS_PROFILE": "p",
        "ZEUS_CONFIG": "/c.yaml", "ZEUS_ENV": "/e",
        "ZEUS_DELEGATED_CHILD_CONTEXT": "1",
        # other ZEUS_* → dropped (broad prefix removed)
        "ZEUS_BASE_URL": "https://x", "ZEUS_INTERACTIVE": "1",
    ...
    for kept in (
        ...
        "ZEUS_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "ZEUS_BASE_URL", "ZEUS_INTERACTIVE", "ZEUS_KANBAN_TASK",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped ZEUS_* vars."""
    import logging
