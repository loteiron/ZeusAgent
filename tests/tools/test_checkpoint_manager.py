...
def checkpoint_base(tmp_path):
    """Isolated checkpoint base — never writes to ~/.zeus/."""
...
class TestStoreInit:
    ...
    def test_legacy_migration_archives_prev2_repos(
        ...
        (fake_repo / "ZEUS_WORKDIR").write_text(str(work_dir) + "\n")
...
class TestSafeRestore:
    """Safe restore: preserve user hand-edits, revert only ZeusAgent-authored changes.
    ...
    def test_safe_restore_skips_file_user_edited_after_agent(self, mgr, work_dir):
        ...
        # ZeusAgent writes the file, then the user modifies it afterwards.
        (work_dir / "main.py").write_text("agent version\n")
        mgr.record_agent_write(str(work_dir / "main.py"))
        ...
        result = mgr.restore(str(work_dir), base, safe=True)
        assert result["success"] is True
        # Content no longer matches what ZeusAgent last wrote → preserved.
    ...
    def test_safe_restore_removes_agent_created_file_keeps_user_edit(self, mgr, work_dir):
        ...
        # ZeusAgent creates a brand-new file after the checkpoint...
        ...
        result = mgr.restore(str(work_dir), base, safe=True)
        assert result["success"] is True
        # User edit preserved; ZeusAgent-created file removed (not in checkpoint).
    ...
    # shape as a file ZeusAgent created — and the delete branch treats absence as
...
def _seed_legacy_repo(base: Path, name: str, workdir: Path, mtime: float = None) -> Path:
    ...
    (shadow / "ZEUS_WORKDIR").write_text(str(workdir) + "\n")
    (shadow / "info").mkdir()
...
class TestPruneCheckpointsOrphanAllowlist:
    ...
    def test_end_to_end_timing_change_during_confirmation_prompt(self, tmp_path, monkeypatch):
        ...
        `zeus checkpoints prune`: the preview shows one pre-v2 orphan; a
        ...
        be deleted.
        """
        import zeus_cli.checkpoints as checkpoints_cli
...
class TestPruneSweepsTmpPackDebris:
    ...
    def test_sweeps_debris_even_when_no_ref_moved(self, checkpoint_base, tmp_path, monkeypatch):
        import tools.checkpoint_manager as cm
        monkeypatch.setattr(cm, "CHECKPOINT_BASE", checkpoint_base)
        monkeypatch.setattr("zeus_cli.gitlock._git_proc_running", lambda: False)
        work = tmp_path / "proj"
        work.mkdir()
        (work / "f").write_text("f")
        CheckpointManager(enabled=True).ensure_checkpoint(str(work), "seed")
