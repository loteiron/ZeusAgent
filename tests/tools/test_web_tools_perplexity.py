...
def test_extract_dispatch_snippets_per_url_and_missing_key(monkeypatch):
    ...
    # HTTP is already synthetic; keep these two DNS answers deterministic while
    # still executing the production URL-safety checks.
    original_resolver = url_safety._getaddrinfo

    def resolve(host, port=None, *args, **kwargs):
        if host in {"tokio.rs", "docs.rs"}:
