...
from tools.skills_hub_search import (
    ZEUS_INDEX_TTL, _load_zeus_index, create_source_router, parallel_search_sources, unified_search,
)
...
class TestParseFrontmatterQuick:
    def test_valid_frontmatter_including_nested_yaml(self):
        ...
        nested = "---\nname: test\nmetadata:\n  zeus:\n    tags: [a, b]\n---\n\nBody.\n"
        assert GitHubSource._parse_frontmatter_quick(nested)["metadata"]["zeus"]["tags"] == ["a", "b"]
    ...
    def test_browseable_trusted_repos_have_taps(self):
        ...
        for repo in TRUSTED_REPOS:
            assert repo in tap_repos, (
                ...
                "browsable via `zeus skills browse`."
            )
...
def _make_index_source(skills):
    """Build a ZeusAgentIndexSource pre-loaded with a fixed skill list."""
    from tools.skills_hub_official import ZeusAgentIndexSource
    src = ZeusAgentIndexSource(auth=GitHubAuth())
    src._index = {"skills": skills}
    src._loaded = True
    return src


class TestZeusAgentIndexSearch:
...
class TestProviderFilter:
    ...
    def test_unified_search_provider_filter_keeps_index_source(self):
        ...
        src = MagicMock()
        src.source_id.return_value = "zeus-index"
...
class TestOptionalSkillSourceLiveRepoFallback:
    ...
    searchable and installable without `zeus update` (live-repo fallback)."""
...
class TestInstallPathSafety:
    ...
    def test_install_from_quarantine_rejects_category_bucket_overwrite(self, tmp_path):
        ...
        Regression test for GitHub issue #75983: ``zeus skills install … --name
...
# ---------------------------------------------------------------------------
# _load_zeus_index — centralized index fetch (Browse-hub landing / search)
# ---------------------------------------------------------------------------


class TestLoadZeusAgentIndex:
    ...
    def _isolate_cache(monkeypatch, tmp_path):
        ...
        cache_file = tmp_path / "zeus-index.json"
        monkeypatch.setattr("tools.skills_hub_search._zeus_index_cache_file", lambda: cache_file)
        return cache_file
    ...
    def test_persistent_decoding_error_falls_back_to_stale_cache(
        ...
        old = time.time() - (ZEUS_INDEX_TTL + 100)
        import os
