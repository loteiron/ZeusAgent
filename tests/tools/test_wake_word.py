"""Tests for tools.wake_word — the "Hey ZeusAgent" hotword detector.
...
def test_config_defaults_and_clamping():
    ...
    assert ww.wake_phrase({"phrase": "hey zeus"}) == "hey zeus"
    assert ww.wake_phrase({}) == "hey zeus"
...
def test_looks_like_path():
    ...
    assert _looks_like_path("models/hey_zeus.onnx")
...
def test_load_wake_word_config_guards_non_dict(monkeypatch):
    monkeypatch.setattr(
        "zeus_cli.config.load_config", lambda: {"wake_word": "oops"}
    )
...
def test_requirements_openwakeword_available(monkeypatch):
    ...
    r = ww.check_wake_word_requirements(
        {"provider": "openwakeword", "phrase": "hey zeus"}
    )
    assert r["available"] is True
    assert r["provider"] == "openwakeword"
    assert r["phrase"] == "hey zeus"
...
def test_openwakeword_ensures_base_models_for_custom_path(monkeypatch):
    ...
    eng = ww._OpenWakeWordEngine(
        {"provider": "openwakeword", "openwakeword": {"model": "/models/hey_zeus.onnx"}}
    )
    assert calls["download"] == [["/models/hey_zeus.onnx"]]
    assert eng._labels == ["hey_zeus"]


def test_bundled_hey_zeus_model_ships_on_disk():
    # The "hey zeus" wake word works out of the box only if the model is
