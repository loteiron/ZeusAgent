...
class TestWriteDenyExactPaths:
    ...
    def test_zeus_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.zeus/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "zeus_root"
        ...
        # Sanity check: ZEUS_HOME does point to the profile dir, not the root.
        from zeus_constants import get_zeus_home, get_default_zeus_root
...
class TestWriteAllowed:
    ...
    def test_zeus_control_files_requested_writable(self):
