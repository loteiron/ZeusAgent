...
Covers:
- :class:`ZeusAgentTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.ZeusAgentMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _ZEUS_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# ZeusAgentTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = ZeusAgentTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ZEUS_HOME", str(tmp_path))
        storage = ZeusAgentTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (ZeusAgentMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: ZeusAgentTokenStorage, **context_attrs):
    ...
    provider = _ZEUS_PROVIDER_CLS.__new__(_ZEUS_PROVIDER_CLS)
    provider._zeus_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZEUS_HOME", str(tmp_path))
        storage = ZeusAgentTokenStorage("mgr-srv")
        ...
        with patch.object(
            _ZEUS_PROVIDER_CLS.__bases__[-1], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZEUS_HOME", str(tmp_path))
        storage = ZeusAgentTokenStorage("flow-srv")
        ...
        with patch.object(
            _ZEUS_PROVIDER_CLS.__bases__[-1],
            "async_auth_flow",
