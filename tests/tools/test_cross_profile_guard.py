...
@pytest.fixture
def fake_zeus(tmp_path, monkeypatch):
    """Build a two-profile ZeusAgent layout and point ZEUS_HOME at
    the zeus-security profile (matching the original-incident shape).
    """
    root = tmp_path / "fake-zeus"
    (root / "skills" / "shared-skill").mkdir(parents=True)
    (root / "skills" / "shared-skill" / "SKILL.md").write_text(
        "---\nname: shared-skill\ndescription: default copy.\n---\n"
    )

    sec_home = root / "profiles" / "zeus-security"
    (sec_home / "skills").mkdir(parents=True)

    coder_home = root / "profiles" / "coder"
    (coder_home / "skills").mkdir(parents=True)

    monkeypatch.setenv("ZEUS_HOME", str(sec_home))

    import zeus_constants
    monkeypatch.setattr(zeus_constants, "get_default_zeus_root", lambda: root)

    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_zeus_home_path", lambda: sec_home)
    monkeypatch.setattr(fs, "_zeus_root_path", lambda: root)
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_zeus):
        from tools.file_tools import write_file_tool
        target = fake_zeus["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_allowed_guard_retired(self, fake_zeus):
        ...
        from tools.file_tools import write_file_tool
        target = fake_zeus["root"] / "skills" / "shared-skill" / "SKILL.md"
    ...
    def test_non_zeus_path_unaffected(self, fake_zeus, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_allowed_guard_retired(self, fake_zeus):
    ...
    def test_cross_profile_patch_bypass(self, fake_zeus):
        from tools.file_tools import patch_tool
        target = fake_zeus["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_writes_through_guard_retired(self, fake_zeus):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_zeus, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_zeus["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up ZEUS_HOME (set in
        ...
        assert "not found in active profile 'zeus-security'" in err
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.zeus/profiles/<name>/."""
        # Don't set ZEUS_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_zeus_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_zeus_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_zeus):
        """When active profile is 'zeus-security', the prompt warns
        ...
        assert "Active ZeusAgent profile" in src
        assert "cross_profile=True" not in src  # guard retired
        assert "~/.zeus/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active ZeusAgent profile: default" in src
        assert "Active ZeusAgent profile: {active_profile}" in src
