...
was gated on ``[ -f "$ZEUS_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        f"{env_setup}"
        f'ZEUS_HOME="{home}"\n'
        # In tests we run unprivileged; as_zeus is a passthrough then.
        'as_zeus() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    ZeusAgent loads $ZEUS_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/zeus/.env.example -> $ZEUS_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $ZEUS_HOME/.env from it on first boot"
    )
