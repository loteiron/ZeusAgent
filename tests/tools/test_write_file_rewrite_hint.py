...
def test_rewrite_of_a_large_file_with_few_changes_gets_a_patch_hint(tmp_path):
    ...
    assert not json.loads(read_file_tool(str(f), task_id="t")).get("error")
...
def test_new_files_small_files_and_real_rewrites_get_no_hint(tmp_path):
    ...
    small.write_text("a\nb\n", encoding="utf-8")
    assert not json.loads(read_file_tool(str(small), task_id="t")).get("error")
    result = json.loads(write_file_tool(str(small), "a\nc\n", task_id="t"))
    assert "hint" not in result and not result.get("error")
    big = tmp_path / "big.py"
    big.write_text(_big(), encoding="utf-8")
    assert not json.loads(read_file_tool(str(big), task_id="t")).get("error")
    rewritten = "\n".join(f"other {i}: " + "y" * 40 for i in range(800)) + "\n"
    result = json.loads(write_file_tool(str(big), rewritten, task_id="t"))
    assert "hint" not in result and not result.get("error")
