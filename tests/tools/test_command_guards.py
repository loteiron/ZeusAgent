...
def _mode_manual(monkeypatch):
    ...
    The test conftest redirects ZEUS_HOME to an empty tempdir, so the
...
def _clean_state():
    ...
    saved = {}
    for k in ("ZEUS_INTERACTIVE", "ZEUS_GATEWAY_SESSION", "ZEUS_EXEC_ASK", "ZEUS_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["ZEUS_INTERACTIVE"] = "1"
        session_key = os.getenv("ZEUS_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No ZEUS_INTERACTIVE or ZEUS_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
