...
def _is_zeus_cli(argv) -> bool:
    """Match the delivery CLI by basename — local_delivery_command may
    resolve the venv-relative zeus next to the interpreter (#93590)."""
    name = str(argv[0]).rsplit("\\", 1)[-1].rsplit("/", 1)[-1]
    return name in ("zeus", "zeus.exe")
...
def _transport_calls(calls):
    ...
    return [argv for argv in calls if argv and _is_zeus_cli(argv)]
