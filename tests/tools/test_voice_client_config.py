...
Real config files in a temp ZEUS_HOME, real resolution chains (no mocked
...
def voice_home(tmp_path, monkeypatch):
    """Isolated ZEUS_HOME + reloaded config modules; yields a config writer."""
    home = tmp_path / ".zeus"
    home.mkdir()
    ...
    for var in (
        ...
        "DEEPINFRA_API_KEY", "ZEUS_LOCAL_STT_LANGUAGE",
    ):
