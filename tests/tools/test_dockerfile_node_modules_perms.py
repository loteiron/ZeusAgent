...
Older images made /opt/zeus/ui-tui and /opt/zeus/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV ZEUS_TUI_DIR=/opt/zeus/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R zeus:zeus /opt/zeus/ui-tui" not in text
    assert "chown -R zeus:zeus /opt/zeus/node_modules" not in text
