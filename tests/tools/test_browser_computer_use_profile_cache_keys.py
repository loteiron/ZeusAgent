...
def _under(home):
    return set_zeus_home_override(str(home))
...
def test_cached_computer_approval_and_release_remain_in_their_profile(two_homes, monkeypatch):
    import tools.computer_use.tool as cu
    calls = []
    def allow(*args):
        calls.append(args)
        return "approve_session"
    monkeypatch.setattr(cu, "_approval_callback", allow)
    a, b = two_homes
    tok = _under(a)
    try:
        assert cu._request_approval("click", {}, "same") is None
        assert cu._request_approval("click", {}, "same") is None
        assert len(calls) == 1
    finally:
        reset_zeus_home_override(tok)
    ...
    try:
        assert cu._request_approval("click", {}, "same") is None
        assert len(calls) == 2
        cu.release_computer_use_session("same")
    finally:
        reset_zeus_home_override(tok)
    tok = _under(a)
    try:
