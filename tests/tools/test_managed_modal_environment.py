...
def _restore_tool_and_agent_modules():
    ...
    original_modules = {
        ...
        if name in {"tools", "agent", "zeus_cli"}
    ...
    try:
        ...
        _reset_modules(("tools", "agent", "zeus_cli"))
...
def _install_fake_tools_package(*, credential_mounts=None):
    ...
    zeus_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["zeus_cli"] = zeus_cli
    sys.modules["zeus_cli.config"] = types.SimpleNamespace(
        get_zeus_home=lambda: Path(tempfile.gettempdir()) / "zeus-home",
    )
...
def test_managed_modal_rejects_host_credential_passthrough():
    _install_fake_tools_package(
        credential_mounts=[{
            ...
            "container_path": "/root/.zeus/token.json",
        }]
    )
    managed_modal = _load_tool_module("tools.environments.managed_modal", "environments/managed_modal.py")
