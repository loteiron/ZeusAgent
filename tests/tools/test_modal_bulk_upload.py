...
class TestModalBulkUpload:
    ...
    def test_tar_archive_contains_all_files(self, monkeypatch, tmp_path):
        ...
        files = [
            (str(src_a), "/root/.zeus/credentials/a.json"),
            (str(src_b), "/root/.zeus/skills/b.py"),
        ]
        ...
        with tarfile.open(fileobj=buf, mode="r:gz") as tar:
            ...
            assert "root/.zeus/credentials/a.json" in names
            assert "root/.zeus/skills/b.py" in names

            # Verify content
            a_content = tar.extractfile("root/.zeus/credentials/a.json").read()
            assert a_content == b"cred_content"
            b_content = tar.extractfile("root/.zeus/skills/b.py").read()
    ...
    def test_stdin_chunked_for_large_payloads(self, monkeypatch, tmp_path):
        ...
        files = [(str(src), "/root/.zeus/large.bin")]
        ...
        with tarfile.open(fileobj=buf, mode="r:gz") as tar:
            names = tar.getnames()
            assert "root/.zeus/large.bin" in names
