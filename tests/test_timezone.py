"""
Tests for timezone support (zeus_time module + integration points).
...
def _reset_zeus_time_cache():
    """Reset the zeus_time module cache."""
    zeus_time.reset_cache()


# =========================================================================
# zeus_time.now() — core helper
# =========================================================================

class TestZeusAgentTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_zeus_time_cache()

    def teardown_method(self):
        _reset_zeus_time_cache()
        os.environ.pop("ZEUS_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["ZEUS_TIMEZONE"] = "Asia/Kolkata"
        result = zeus_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["ZEUS_TIMEZONE"] = "UTC"
        result = zeus_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["ZEUS_TIMEZONE"] = "America/New_York"
        result = zeus_time.now()
        assert result.tzinfo is not None
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["ZEUS_TIMEZONE"] = "Europe/London"
        tz = zeus_time.get_timezone()
        assert isinstance(tz, ZoneInfo)
        assert str(tz) == "Europe/London"

    def test_cache_isolated_by_active_profile_config(self, tmp_path, monkeypatch):
        """Switching ZEUS_HOME must not reuse another profile's timezone."""
        ...
        monkeypatch.delenv("ZEUS_TIMEZONE", raising=False)

        monkeypatch.setenv("ZEUS_HOME", str(first_home))
        assert str(zeus_time.get_timezone()) == "Asia/Tokyo"

        # Multiplexed profile runtime scopes switch ZEUS_HOME in one process.
        monkeypatch.setenv("ZEUS_HOME", str(second_home))
        assert str(zeus_time.get_timezone()) == "America/New_York"
    ...
    def test_concurrent_profile_resolution_never_mixes_zones(
        ...
        def worker(key: str) -> None:
            barrier.wait()
            for _ in range(200):
                token = set_zeus_home_override(str(homes[key]))
                try:
                    tz = zeus_time.get_timezone()
                    if str(tz) != zones[key]:
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When ZEUS_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) ZEUS_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("ZEUS_TIMEZONE=" + os.environ.get("ZEUS_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "ZEUS_TIMEZONE=NOT_SET" in result["output"], (
            "ZEUS_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When ZEUS_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("ZEUS_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=zeus_tz) which shifted
        absolute time when system-local tz != ZeusAgent tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when ZeusAgent tz
        ...
        # Use a ZeusAgent timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _zeus_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["ZEUS_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_zeus_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of ZeusAgent timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["ZEUS_TIMEZONE"] = "US/Eastern"
        _reset_zeus_time_cache()
