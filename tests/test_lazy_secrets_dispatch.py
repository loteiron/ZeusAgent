...
1. `zeus secrets bitwarden setup --help` works (dispatch path)
2. `zeus update --check` works (update path)
3. `zeus secrets bitwarden disable` works (handler execution)
4. `zeus secrets onepassword status` works (lazy backend loads on demand)
...
def _run_zeus(args: list[str], timeout: int = 30) -> subprocess.CompletedProcess[str]:
    """Run zeus CLI as a subprocess from repo root.
    ...
    connectivity — but ``zeus update --check`` really does ``git fetch``
    ...
    env.update(
        {
            ...
            "GIT_CONFIG_KEY_0": "url.file:///nonexistent-zeus-test-remote/.insteadOf",
            "GIT_CONFIG_VALUE_0": "https://",
            "GIT_CONFIG_KEY_1": "url.file:///nonexistent-zeus-test-remote/.insteadOf",
            "GIT_CONFIG_VALUE_1": "git@",
        }
    )
    return subprocess.run(
        [sys.executable, "-m", "zeus_cli.main"] + args,
        capture_output=True,
        text=True,
        cwd=str(repo_root),
        env=env,
...
class TestSecretsDispatchE2E:
    ...
    def test_bitwarden_status(self) -> None:
        """`zeus secrets bitwarden status` must exit 0 (runs lazy backend)."""
        result = _run_zeus(["secrets", "bitwarden", "status"])
    ...
    def test_bitwarden_disable(self) -> None:
        """`zeus secrets bitwarden disable` must exit 0."""
        result = _run_zeus(["secrets", "bitwarden", "disable"])
        assert result.returncode == 0, (
    ...
    def test_onepassword_status(self) -> None:
        """`zeus secrets onepassword status` must exit 0 (1Password lazy backend)."""
        result = _run_zeus(["secrets", "onepassword", "status"])
        assert result.returncode in (0, 1), (
    ...
    def test_onepassword_setup_help(self) -> None:
        """`zeus secrets onepassword setup --help` must exit 0."""
        result = _run_zeus(["secrets", "onepassword", "setup", "--help"])
...
class TestUpdatePathE2E:
    ...
    These tests invoke the real `zeus update --check` path as a subprocess.
    ...
    def test_update_check_clean(self) -> None:
        """`zeus update --check` must not load cryptography._rust."""
        result = _run_zeus(["update", "--check"])
    ...
    def test_main_update_check_crypto_absent_in_sys_modules(self) -> None:
        """Decisive invariant: invoking main() with argv=['zeus','update','--check']
...
sys.argv = ['zeus', 'update', '--check']

import zeus_cli.main as m
...
# resolves the zeus_cli.main module's lazily re-exported attribute —
# so the patch must land on update_cmd._cmd_update_check.
# Exercise the legacy handler as if a fork release channel were configured.
# The real source-only refusal is covered in test_source_distribution.py.
with patch('zeus_cli.distribution.REMOTE_RELEASES_AVAILABLE', True), \\
     patch('zeus_cli.update_cmd._cmd_update_check', capture_update_check):
    try:
        m.main()
