...
from zeus_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(zeus_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <zeus_home>/plugins/<name> and opt it in."""
    plugin_dir = zeus_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ZEUS_HOME", str(tmp_path / "zeus_empty"))
