...
def zeus_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves ZEUS_HOME.
...
def server(zeus_home, monkeypatch):
    ...
    # module-level ``_zeus_home = get_zeus_home()`` snapshot freezes
    ...
    monkeypatch.setattr(mod, "_zeus_home", zeus_home)
...
def test_moa_bare_returns_usage(server, session, zeus_home):
    _write_moa_config(zeus_home, """
moa:
