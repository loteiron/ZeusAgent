    ...
    with patch.dict(
        "sys.modules",
        {
            "zeus_constants": MagicMock(
                get_zeus_home=MagicMock(return_value="/tmp/zeus_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'zeus-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'zeus-release' patched"
    }
