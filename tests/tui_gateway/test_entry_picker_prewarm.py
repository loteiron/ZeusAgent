...
guard is covered in ``tests/zeus_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``zeus_cli.model_switch_providers.prewarm_picker_cache_async``
...
import zeus_cli.model_switch_providers
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from zeus_cli.model_switch,
