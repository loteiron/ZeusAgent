...
def server():
    ...
    # e.g. zeus_cli.active_sessions would bind the mocked get_zeus_home
    ...
    with patch.dict("sys.modules", {
        "zeus_constants": MagicMock(get_zeus_home=MagicMock(return_value="/tmp/zeus_test")),
        "zeus_cli.env_loader": MagicMock(),
        "zeus_cli.banner": MagicMock(),
        "zeus_state": MagicMock(),
    }):
        import importlib
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `ZEUS_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("ZEUS_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("ZEUS_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_idle_reaper_rearms_missing_ws_orphan_timer(server, monkeypatch, tmp_path, closed_transport):
    ...
    successor = subprocess.run(
        [
            ...
            "-c",
            (
                "from zeus_cli.active_sessions import try_acquire_active_session; "
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/zeus-agent-dev": {"name": "zeus-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "zeus-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_scopes_skill_lookup_to_session_profile(server, tmp_path):
    ...
    profile_home rather than the gateway process's ambient ZEUS_HOME
    ...
    # The gateway's own ZEUS_HOME (the test-isolation tempdir, no
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    """Real config + skin files: activating a skin (as `zeus config set` does)
    ...
    import zeus_cli.skin_engine as skin_engine

    (tmp_path / "skins").mkdir()
    (tmp_path / "skins" / "midnight.yaml").write_text(
        "name: midnight\ndescription: t\ncolors:\n  banner_title: '#00ffcc'\n  background: '#001010'\n"
    )
    monkeypatch.setattr(skin_engine, "get_zeus_home", lambda: tmp_path)
    ...
    # Activate midnight, as `zeus config set display.skin midnight` would.
