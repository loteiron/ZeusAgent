...
def live_home(monkeypatch):
    """A REAL isolated ZEUS_HOME with a config.yaml + state.db on disk."""
    tmp = Path(tempfile.mkdtemp(prefix="zeus-live-staleprov-"))
    home = tmp / ".zeus"
    ...
    # zeus_constants caches the resolved home at first read — the env var
    ...
    import zeus_cli.config as hconfig
    ...
    # conftest also re-pins zeus_state.DEFAULT_DB_PATH per test; pin it to
    # THIS home so server._get_db() opens the same real state.db we seed.
    monkeypatch.setattr(zeus_state, "DEFAULT_DB_PATH", home / "state.db")
    monkeypatch.setattr(server, "_db", None, raising=False)
    monkeypatch.setattr(server, "_db_error", None, raising=False)
    monkeypatch.setattr(server, "_zeus_home", str(home), raising=False)
    yield home, server
