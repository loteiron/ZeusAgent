...
def progress(tmp_path):
    ...
    status = tmp_path / "zeus-update-status"
...
def test_every_state_carries_a_clock(progress):
    ...
    for state, message in [
        ...
        ("done", ""),
        ("manual", "Reopen ZeusAgent to finish."),
        ("error", "Update failed."),
    ]:
...
# Stands in for `zeus update`, and reports the stage that was on screen
...
n="$(cat "$ZEUS_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$ZEUS_TEST_CALLS"
for f in "$TMPDIR"/zeus-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$ZEUS_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$ZEUS_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "zeus-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    zeus = install_root / "venv" / "bin" / "zeus"
    zeus.write_text(FAKE_ZEUS)
    zeus.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "ZEUS_TEST_CAPTURE": str(capture),
        "ZEUS_TEST_CALLS": str(calls),
        "ZEUS_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".zeus-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`zeus update` is the longest wait in the hand-off and the one the
