...
def test_pyproject_pins_match_lazy_deps_pins():
    ...
    places. When they drift, `zeus update` resolves the pyproject extra
    ...
    assert not drift, (
        ...
        "for every shared package — otherwise `zeus update` downgrades the "
...
def test_dingtalk_extra_includes_qrcode_for_qr_auth():
    """DingTalk's QR-code device-flow auth (zeus_cli/dingtalk_auth.py)
...
def test_every_lazy_deps_exact_pin_matches_uv_lock():
    ...
    install ships the locked version, and the ``zeus update`` lazy-refresh
    ...
    assert not drift, (
        ...
        "every package the core lock also ships — otherwise `zeus update` "
...
def test_huggingface_hub_lazy_pin_matches_uv_lock():
    ...
    a feature from mere package presence, the `zeus update` lazy-refresh
    ...
    assert lazy_pin == locked, (
        ...
        "huggingface-hub`), or `zeus update` will churn/downgrade the "
