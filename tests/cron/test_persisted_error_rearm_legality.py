...
def cron_store(tmp_path, monkeypatch):
    ...
    (zeus_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("ZEUS_HOME", str(zeus_home))
    monkeypatch.setattr(J, "ZEUS_DIR", zeus_home)
    monkeypatch.setattr(J, "CRON_DIR", zeus_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", zeus_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", zeus_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_zeus_now", lambda: now):
            due = J.get_due_jobs()
