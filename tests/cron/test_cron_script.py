...
- Path resolution (absolute, relative to ZEUS_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp ZEUS_HOME."""
    zeus_home = tmp_path / ".zeus"
    zeus_home.mkdir()
    (zeus_home / "cron").mkdir()
    (zeus_home / "cron" / "output").mkdir()
    (zeus_home / "scripts").mkdir()
    monkeypatch.setenv("ZEUS_HOME", str(zeus_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "ZEUS_DIR", zeus_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", zeus_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", zeus_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", zeus_home / "cron" / "output")
...
class TestRunJobScript:
    ...
    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        """Cron scripts must not inherit ZeusAgent provider env (SECURITY.md §2.3)."""
        from tools.environments.local_env_policy import _ZEUS_PROVIDER_ENV_BLOCKLIST
        from cron.scheduler_script import _run_job_script

        # sorted() so the probed var is deterministic across runs
        # (frozenset iteration order varies with PYTHONHASHSEED).
        blocked_var = sorted(_ZEUS_PROVIDER_ENV_BLOCKLIST)[0]
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.zeus/scripts/ must be rejected."""
        from cron.scheduler_script import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("ZEUS_SESSION_PLATFORM") is None
        assert os.environ.get("ZEUS_SESSION_CHAT_ID") is None
        assert os.environ.get("ZEUS_SESSION_CHAT_NAME") is None
...
class TestScriptTimeoutTreeKill:
    ...
    def test_timeout_leaves_no_setsid_grandchild(self, cron_env, monkeypatch):
        ...
        monkeypatch.setenv("ZEUS_CRON_SCRIPT_TIMEOUT", "2")
