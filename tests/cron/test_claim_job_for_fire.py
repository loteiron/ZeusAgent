...
These exercise the real store against a temp ZEUS_HOME (no mocks) per the
...
def temp_home(tmp_path, monkeypatch):
    ...
    # cron.jobs caches no home at import; get_zeus_home() reads the env live.
    yield tmp_path
...
def test_heartbeat_does_not_pin_fire_fence_while_jobs_lock_contended(temp_home, monkeypatch):
    ...
    owner = claimed["by"]
    claimed_at = datetime.fromisoformat(claimed["at"])
    monkeypatch.setattr(jobs, "_zeus_now", lambda: claimed_at + timedelta(seconds=30))
