"""Run a real ZeusAgent CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_zeus_executable(zeus_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "zeus",
        Path(".venv") / "Scripts" / "zeus.exe",
    ):
        candidate = zeus_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("zeus")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "ZeusAgent executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="ZeusAgent source checkout containing .venv/bin/zeus",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated ZEUS_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["zeus.client.active"] != [
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['zeus.client.active']}"
        )
    [model] = by_name["zeus.model_route.count"]
    expected_model = {
        "name": "zeus.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['zeus.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["zeus.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['zeus.task_run.started']}"
        )
    [terminal] = by_name["zeus.task_run.finished"]
    ...
    [tool] = by_name["zeus.tool_call.count"]
    ...
    lifecycle = by_name["zeus.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["zeus.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The ZeusAgent development environment requires jsonschema"
        ) from exc
    ...
    if metrics["zeus.client.active"] != [
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['zeus.client.active']}"
        )
    [model] = metrics["zeus.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['zeus.model_route.count']}"
        )
    [terminal] = metrics["zeus.task_run.finished"]
    ...
    [tool] = metrics["zeus.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["zeus.skill.lifecycle.count"]
    ...
    loads = metrics["zeus.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    zeus_repo = args.zeus_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    zeus = _resolve_zeus_executable(zeus_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="zeus-relay-shared-metrics-"))
    home = root / "zeus-home"
    ...
    try:
        ...
        env = os.environ.copy()
        env["ZEUS_HOME"] = str(home)
        python_paths = [str(zeus_repo)]
    ...
    (root / "zeus.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "zeus.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"ZeusAgent exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("ZeusAgent model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("ZeusAgent did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("ZeusAgent did not print the mock model response")
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "zeus.shared_metrics.v2.schema.json",
    )

    print("ZeusAgent -> NeMo Relay shared-metrics smoke test passed")
