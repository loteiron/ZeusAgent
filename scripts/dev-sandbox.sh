...
unprivileged `zeus` user, giving the layout most people have —
$ZEUS_HOME/zeus-agent plus a ~/.local/bin launcher. Pass --root for the FHS
...
Environment:
  ZEUS_DEV_SANDBOX_DIR    Sandbox directory name, relative to the repo root
                            (default: .zeus-sandbox).

Examples:
  # create a sandbox, install this branch as `main`, and then drop to a shell,
  # skipping `zeus setup` & the browser tools for speed.
...
INSTALL_REF=""
UPSTREAM_URL="${ZEUS_DEV_SANDBOX_UPSTREAM:-https://github.com/NousResearch/hermes-agent.git}"
...
GIT_ROOT="${ZEUS_SANDBOX_SOURCE_ROOT:-$(git rev-parse --show-toplevel)}"
...
SANDBOX_DIR_NAME="${ZEUS_DEV_SANDBOX_DIR:-.zeus-sandbox}"
...
if [ "$PERSISTENT" = true ]; then
  ...
  SANDBOX_ROOT="$(mktemp -d -t zeus-sandbox.XXXXXX)"
...
if [ -n "$INSTALL_REF" ]; then
  ...
  UPSTREAM_REPO="$(mktemp -d -t zeus-sandbox-upstream.XXXXXX)"
...
if [ "$INSTALL_SHORTCUT" = true ]; then
  ...
  set -- bash -c '
    ...
    if [ "$install_status" -eq 0 ] && [ -f /work/promote-main ]; then
      ...
      if git --git-dir=/work/repos/zeus-agent.git update-ref refs/heads/main "$next_main"; then
...
# A user-level install writes the `zeus` launcher to ~/.local/bin and the
# checkout to $ZEUS_HOME; both live under the sandbox HOME, which is bound
...
SOURCE_REF="$COMMIT"
SNAPSHOT_REPO=""
FAKE_REPO="$SANDBOX_ROOT/root/repos/zeus-agent.git"
git -C "$SANDBOX_ROOT/root/repos" init --bare -q zeus-agent.git
if [ -n "$INSTALL_REF" ]; then
...
if [ -n "$(git -C "$GIT_ROOT" status --porcelain)" ]; then
  ...
  SNAPSHOT_REPO="$(mktemp -d -t zeus-sandbox-snapshot.XXXXXX)"
  git -C "$SNAPSHOT_REPO" init -q
  git -C "$SNAPSHOT_REPO" fetch -q "$GIT_ROOT" "$COMMIT"
  git -C "$SNAPSHOT_REPO" config user.name 'ZeusAgent sandbox'
...
if [ -n "$INSTALL_REF" ]; then
  git --git-dir="$FAKE_REPO" fetch -q --force "$SOURCE_REPO" \
    "$SOURCE_REF:refs/zeus-sandbox/next"
...
if [ ! -f "$SANDBOX_ROOT/root/certs/ca.pem" ]; then
  if ! ca_error="$(OPENSSL_CONF="$SANDBOX_ROOT/root/certs/openssl.cnf" \
    ...
    -subj '/CN=ZeusAgent dev sandbox CA' \
