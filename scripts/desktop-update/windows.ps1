...
# (%ZEUS_HOME%\zeus-setup.exe). That binary has no self-update path --
# copy_self_to_zeus_home deliberately no-ops during --update -- so every
...
#
# This script lives in the repo checkout, so EVERY `zeus update` refreshes
...
#     -InstallRoot <path>   repo checkout (ZEUS_HOME\zeus-agent)
#     -Branch <ref>         branch to update against
#     -DesktopPid <pid>     the Electron main process to wait out
#     [-RelaunchExe <path>] ZeusAgent.exe to start when done (omit = no relaunch)
#     [-NoUi]               headless (tests); default shows a progress window
#     [-NoMarkerCleanup]    leave .zeus-update-in-progress in place (tests)
#
...
# .zeus-update-result.json for the relaunched Desktop to surface, and
# relaunches the Desktop so the user is never left stranded.
#
# Marker: we claim ZEUS_HOME\.zeus-update-in-progress with OUR pid as
# step 0 (the wrapper cmd.exe pid the Desktop saw is useless -- it exits
# immediately), retaining ZEUS_UPDATE_STARTED_AT from the Desktop hand-off.
# zeus_cli/update_lock.py's ancestry rule lets our
# `zeus update` child adopt the claim; electron/update-marker.ts parks a
...
$ZeusAgentHome = if ($InstallRoot) { Split-Path -Parent $InstallRoot } else { $TempDir }
$MarkerPath = Join-Path $ZeusAgentHome ".zeus-update-in-progress"
$LogDir = Join-Path $ZeusAgentHome "logs"
$LogPath = Join-Path $LogDir "desktop-update-handoff.log"
$ResultPath = Join-Path $ZeusAgentHome ".zeus-update-result.json"
$script:Ui = $null
$script:UiStage = "ZeusAgent will open once done."   # until the first gate; matches ui.html
...
function Stop-UiServer([switch]$LeaveWindow) {
    ...
    # profile plus any stale zeus-update-ui-* leftovers from interrupted
    ...
    # update marker (.zeus-update-in-progress) serialises hand-offs, so no
    # other run's profile can be in active use here.
    try {
        $profileDirs = @()
        if ($script:UiServer.Profile) { $profileDirs += $script:UiServer.Profile }
        Get-ChildItem -LiteralPath $TempDir -Directory -Filter "zeus-update-ui-*" -ErrorAction SilentlyContinue |
...
function Publish-UiProgress([string]$Message) {
    ...
    # stderr remain asynchronously drained in Invoke-ZeusAgentStep and are never
    ...
    if ($htmlPath -and $browser) {
        ...
        if ($server) {
            try {
                ...
                $browserProfile = Join-Path $TempDir ("zeus-update-ui-{0}" -f $PID)
                $browserArgs = @(
...
function Show-ErrorFinale([string]$Message) {
    ...
    # log tail -- `zeus debug share` uploads the real evidence and the
    ...
    try {
        ...
        $ui.Sub.Text = "Run `"zeus debug share`" in a terminal to send a report."
...
function Show-ManualFinale([string]$Message) {
    ...
    # they reopen ZeusAgent themselves.
    if ($script:UiServer) {
...
function Start-DesktopRelaunch {
    ...
    # update it can remove the old ZeusAgent.exe before writing the replacement,
    ...
    # DO NOT spawn ZeusAgent.exe as our child: Electron/Chromium calls
                ...
                if (Initialize-ForegroundSupport) {
                    ...
                    while ((Get-Date) -lt $deadline) {
                        ...
                        if ($hwnd -ne [System.IntPtr]::Zero) {
                            [ZeusAgentHandoff.Win32]::ShowWindow($hwnd, 9) | Out-Null  # SW_RESTORE
                            [ZeusAgentHandoff.Win32]::SetForegroundWindow($hwnd) | Out-Null
    ...
    if (-not $spawned) {
        ...
        # verify by watching for a fresh ZeusAgent process.
        try {
...
# closes it. `zeus update` deliberately runs its build steps with stdout
# inherited (zeus_cli/main.py, the tee-stderr runner), so the tree under a
...
if ($env:ZEUS_UPDATE_PIPE_DRAIN_SECONDS) {
    $parsedGrace = 0
    if ([int]::TryParse($env:ZEUS_UPDATE_PIPE_DRAIN_SECONDS, [ref]$parsedGrace) -and $parsedGrace -ge 0) {
...
if ($env:ZEUS_UPDATE_STEP_IDLE_SECONDS) {
    $parsedIdle = 0
    if ([int]::TryParse($env:ZEUS_UPDATE_STEP_IDLE_SECONDS, [ref]$parsedIdle) -and $parsedIdle -gt 0) {
        $script:StepIdleTimeoutSeconds = $parsedIdle
    }
}

# Silence on the pipes is NOT silence in the update. `zeus update` captures
# the (very loud) Electron/vite build into logs/update.log instead of its own
# stdout (zeus_cli/update_cmd.py, the update-log tee), so a real update is
...
if ($env:ZEUS_UPDATE_PROGRESS_LOG) {
    $script:StepProgressLogPath = $env:ZEUS_UPDATE_PROGRESS_LOG
}
...
public static class ZeusAgentUpdateJob {
...
function Invoke-ZeusAgentStep([string]$Exe, [string[]]$ZeusAgentArgs, [string]$Tag, [int]$StartupTimeoutSeconds = 0) {
    ...
    # afterwards, where `zeus debug share` picks it up.
    #
    ...
    # .zeus-update-result.json, clearing .zeus-update-in-progress,
    ...
    # ZeusAgent" until the user killed something by hand. Losing the tail of a
    ...
    try {
        ...
        $started = [ZeusAgentUpdateJob]::StartAssigned($Exe, $arguments)
    } finally {
...
# the page can be QA'd in any browser; ZEUS_SELFTEST_FAIL=1 exercises the
# error state, ZEUS_SELFTEST_HOLD_SECONDS delays the terminal event.
if ($SelfTestUi) {
    ...
    $hold = 6
    if ($env:ZEUS_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:ZEUS_SELFTEST_HOLD_SECONDS }
    Publish-UiProgress "Testing quiet update"
    Start-Sleep -Seconds $hold
    if ($env:ZEUS_SELFTEST_FAIL) {
...
# -SelfTestPipeDrain: prove Invoke-ZeusAgentStep survives a leaked pipe ------
# The #90455 deadlock needs no update, no checkout and no ZeusAgent install to
...
#            update log, the shape of every real `zeus update` build (output
...
if ($SelfTestPipeDrain) {
    ...
    if ($env:ZEUS_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:ZEUS_SELFTEST_HOLD_SECONDS }
    $floodKb = 8192
    if ($env:ZEUS_SELFTEST_FLOOD_KB) { $floodKb = [int]$env:ZEUS_SELFTEST_FLOOD_KB }
    # $PSHOME is this interpreter's own directory -- no hardcoded system path.
    $powershell = Join-Path $PSHOME "powershell.exe"
    $stamp = [Guid]::NewGuid().ToString("N")
    $childPs1 = Join-Path $TempDir "zeus-pipe-drain-$stamp.ps1"
    $floodPs1 = Join-Path $TempDir "zeus-pipe-flood-$stamp.ps1"
    $pidFile = Join-Path $TempDir "zeus-pipe-drain-$stamp.pid"
    $stallPs1 = Join-Path $TempDir "zeus-step-stall-$stamp.ps1"
    $stallPidFile = Join-Path $TempDir "zeus-step-stall-$stamp.pid"
    $stallGrandchildPidFile = Join-Path $TempDir "zeus-step-stall-grandchild-$stamp.pid"
    $logStallPs1 = Join-Path $TempDir "zeus-step-logstall-$stamp.ps1"
    $logStallProgress = Join-Path $TempDir "zeus-step-logstall-$stamp.update.log"
...
'@
    ...
    # merely loud. `zeus update` is this shape -- the Electron/vite build
...
'@
    ...
    $stall = Invoke-ZeusAgentStep $powershell @(
    ...
    if ($stallAlive) { $problems += "stalled child pid $stallPid remained alive after Invoke-ZeusAgentStep returned" }
    if ($stallGrandchildAlive) { $problems += "stalled descendant pid $stallGrandchildPid remained alive after Invoke-ZeusAgentStep returned" }
...
try {
    ...
    try {
        ...
        $startedAt = 0L
        $hasStartedAt = [int64]::TryParse($env:ZEUS_UPDATE_STARTED_AT, [ref]$startedAt)
    ...
    Publish-UiProgress "Waiting for ZeusAgent to close"
    if ($DesktopPid -gt 0) {
        ...
        if (Get-Process -Id $DesktopPid -ErrorAction SilentlyContinue) {
            ...
            $finalCode = 4
            $finalMsg = "Update aborted: the ZeusAgent window (pid $DesktopPid) did not exit within 30s. Nothing was changed. Close ZeusAgent fully and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    Publish-UiProgress "Preparing ZeusAgent files"
    $shim = Join-Path $InstallRoot "venv\Scripts\zeus.exe"
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the ZeusAgent install open (venv\Scripts\zeus.exe locked after 20s). Nothing was changed. Close other ZeusAgent windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    # --force skips only the zeus.exe shim guard, which step 2 just PROVED
    ...
    #
    # DRIVE THE UPDATE THROUGH venv\Scripts\python.exe, NOT venv\Scripts\zeus.exe.
    # `uv pip install -e .` has to replace the console-script shims, so
    # _quarantine_running_zeus_exe must first rename the running zeus.exe
    # out of the way. On Windows that rename fails whenever ANY child process
    # spawned from that zeus.exe is still alive: a child inherits a handle on
    # the parent image, and the resulting sharing violation is indistinguishable
    # from a user leaving a second ZeusAgent window open. It is the inherited
    ...
    # install whose Start Menu shortcut points at a ZeusAgent.exe that no longer
    ...
    #
    # Running the same code as `python.exe -m zeus_cli.main update` puts the
    ...
    if (-not (Test-Path -LiteralPath $pythonExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $pythonExe is missing. The install needs repair (run the ZeusAgent installer or `zeus doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    $updateArgs = @("-m", "zeus_cli.main", "update", "--yes", "--gateway", "--force", "--branch", $Branch)
    ...
    # collides with the "close all ZeusAgent windows" sentinel.
    try {
        $updateHelp = & $pythonExe -m zeus_cli.main update --help 2>$null | Out-String
        if ($updateHelp -match "--keep-stash") {
            $updateArgs += "--keep-stash"
        } else {
            Write-HandoffLog "installed zeus predates --keep-stash; running without it"
        }
    } catch {
    ...
    $res = Invoke-ZeusAgentStep $pythonExe $updateArgs "update"
    Write-HandoffLog "zeus update exit code: $($res.Code)"

    $retryPolicyPath = Join-Path $PSScriptRoot "retry-policy.ps1"
    if (Test-Path -LiteralPath $retryPolicyPath) {
        . $retryPolicyPath
        $shouldRetry = Test-ZeusAgentUpdateShouldRetry -ExitCode $res.Code -InstallRoot $InstallRoot
    } else {
    ...
    # `zeus update` treats a Desktop GUI build failure as NON-fatal (prints
    ...
    if ($res.Code -eq 0 -and $res.Output -match "Desktop build failed") {
        Write-HandoffLog "zeus update reported a desktop build failure (non-fatal there, fatal here); retrying build"
        Publish-UiProgress "Rebuilding Desktop"
        $rebuild = Invoke-ZeusAgentStep $pythonExe @("-m", "zeus_cli.main", "desktop", "--force-build", "--build-only") "rebuild"
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        $verifyCode = "import zeus_cli.main; from pathlib import Path; from zeus_cli.desktop_update_verify import verify_windows_desktop_update; verify_windows_desktop_update(Path.cwd())"
        $verify = Invoke-ZeusAgentStep $pythonExe @("-c", $verifyCode) "verify"
        if ($verify.Code -ne 0) {
            $finalCode = 8
            $finalMsg = "The updated ZeusAgent runtime or Desktop build failed verification. Repair the installation and review antivirus quarantine before retrying."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    }
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        ...
        $finalCode = 6
        $finalMsg = "Code and dependencies updated, but the Desktop app REBUILD FAILED - you are running the previous build. Run `zeus desktop --force-build` from a terminal to retry."
    } else {
        $finalCode = $res.Code
        $finalMsg = "Update failed (exit $($res.Code)). Run `zeus debug share` in a terminal to send a report."
    }
    exit $finalCode
} finally {
    ...
    #   3. only then the terminal UI state — done means "ZeusAgent is back",
    ...
    if (-not $script:TreeSafeToFinalize) {
        ...
        $finalCode = 7
        $finalMsg = "Update recovery could not stop every updater process. ZeusAgent was not restarted to avoid overlapping the active install. Wait for it to finish or restart Windows, then reopen ZeusAgent."
        ...
        if ($finalCode -ne 0) {
            ...
            Publish-UiProgress "Opening ZeusAgent"
            ...
            if (-not $cameBack -and $RelaunchExe) {
                ...
                $finalMsg = "Update complete. Reopen ZeusAgent to finish (it could not restart itself)."
