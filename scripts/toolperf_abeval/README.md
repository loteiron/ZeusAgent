...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  ZeusAgent home, same model, same tasks, same reps.
...
1. Create a dedicated ZeusAgent home with credentials for the models under test:
   ...
   integration at it; no ZeusAgent observability plugin needs to be enabled.
