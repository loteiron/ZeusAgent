...
measured in the production session DB) through `zeus chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    ZEUS_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured ZeusAgent home with credentials for the
...
""".strip(),
            ...
            env.update({
                ...
                "ZEUS_HOME": str(HOME),
                "ZEUS_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "zeus_cli.main", "chat", "--query", q,
