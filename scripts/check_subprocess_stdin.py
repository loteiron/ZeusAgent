...
When ZeusAgent runs in TUI mode, the gateway child process communicates with
...
# ``get_zeus_home() / "plugins"`` (user) and ``./.zeus/plugins/`` (project,
# gated behind ``ZEUS_ENABLE_PROJECT_PLUGINS``) — see
# ``zeus_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``zeus_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import zeus_constants (this script
    ...
    # plugins in get_zeus_home()/plugins/ and project plugins in
    # ./.zeus/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_zeus_home() / "plugins"]
    if os.environ.get("ZEUS_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".zeus" / "plugins")
