# ============================================================================
# ZeusAgent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$ZeusAgentHome = $(if ($env:ZEUS_HOME) { $env:ZEUS_HOME } else { "$env:LOCALAPPDATA\zeus" }),
    [string]$InstallDir = $(if ($env:ZEUS_HOME) { "$env:ZEUS_HOME\zeus-agent" } else { "$env:LOCALAPPDATA\zeus\zeus-agent" }),
    ...
    # builds apps/desktop into a launchable ZeusAgent.exe.
    #
    # Why opt-in:
    #   * ZeusAgent-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched ZeusAgent.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live ZeusAgent.exe
    ...
    #     `zeus desktop` already builds on demand.
    [switch]$IncludeDesktop
)

$ErrorActionPreference = "Stop"

throw "ZeusAgent source distribution: extract the full package and run python scripts/setup_zeus.py --web."
...
# ZEUS_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[zeus] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'ZeusAgentInstall.LongPath').Type) {
            Add-Type -Namespace 'ZeusAgentInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [ZeusAgentInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -ZeusAgentHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('ZeusAgentHome')) {
    $ZeusAgentHome = ConvertTo-LongPath $ZeusAgentHome
} else {
    $ZeusAgentHome = ConvertTo-LongPath $(
        if ($env:ZEUS_HOME) { $env:ZEUS_HOME } else { "$env:LOCALAPPDATA\zeus" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:ZEUS_HOME) { "$env:ZEUS_HOME\zeus-agent" } else { "$env:LOCALAPPDATA\zeus\zeus-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: ZeusAgentHome=$ZeusAgentHome InstallDir=$InstallDir"
}
...
function Write-Banner {
    ...
    Write-Host "|             * ZeusAgent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $ZeusAgentHome)) {
        New-Item -ItemType Directory -Force -Path $ZeusAgentHome | Out-Null
    }
    $envFile = Join-Path $ZeusAgentHome ".env"
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `zeus update`'s npx
    ...
    $prefixDir = Join-Path $ZeusAgentHome "node"
...
function Install-Uv {
    # ZeusAgent owns its own uv at $ZeusAgentHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (zeus_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `zeus update` stay in sync.
    $managedUv = Join-Path $ZeusAgentHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $ZeusAgentHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $ZeusAgentHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $ZeusAgentHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $ZeusAgentHome "bin"
        ...
        # (zeus_cli/managed_uv.py looks only at $ZeusAgentHome\bin\uv.exe).
...
# Put the ZeusAgent-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone zeus-setup.exe run, a user typing `npm`) silently
...
# Upgrade the ZeusAgent-managed Node tree's bundled npm into $NpmRange when
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "ZeusAgent-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("zeus-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and ZEUS_HOME wasn't propagated).
...
function Initialize-ManagedPythonEnvironment {
    # Python used by ZeusAgent belongs to the checkout, never to another
    # application or a user-level uv configuration. Keep this aligned with
    # zeus_cli.managed_uv.managed_python_env(), which owns the update path.
    foreach ($name in @(
    ...
    $managedRoot = Join-Path $InstallDir ".zeus-runtime\python"
...
function Resolve-AvailablePythonVersion {
    # Return the path and minor version of the first ZeusAgent-managed interpreter
    ...
    #
    # Under ZeusAgent-Setup.exe each stage runs in a fresh powershell.exe. The
    ...
    foreach ($ver in $candidates) {
        ...
        try {
            ...
            throw "Failed to resolve ZeusAgent-managed Python $ver`: $_"
        } finally {
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun ZeusAgent setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which ZeusAgent uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\zeus\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    ZeusAgent needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the ZeusAgent copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\zeus\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``ZEUS_GIT_BASH_PATH`` (User scope) so ZeusAgent can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a ZeusAgent-managed PortableGit install instead..."
    }

    # Download PortableGit into $ZeusAgentHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $ZeusAgentHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent ZeusAgent features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  ZeusAgent needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``ZEUS_GIT_BASH_PATH`` (User env scope) so ZeusAgent can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $ZeusAgentHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $ZeusAgentHome\git\usr\bin\bash.exe
    $candidates += "$ZeusAgentHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$ZeusAgentHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("ZEUS_GIT_BASH_PATH", $candidate, "User")
            $env:ZEUS_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set ZEUS_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- ZeusAgent may not find Git Bash."
    Write-Info "If needed, set ZEUS_GIT_BASH_PATH manually to your bash.exe path."
}
...
# range used to provision the ZeusAgent-managed tree. Keeping this probe separate
...
function Test-SystemNodeReady {
    ...
    if (Test-NodeVersionOk $version) {
        ...
        Write-Warn "Node.js $version is unsupported (ZeusAgent requires Node 22.22+, 24.11+, or 26+)"
        return $false
    }
    ...
    if ($npmVersion) {
        Write-Warn "Node.js $version uses npm $npmVersion, which does not satisfy ZeusAgent requirement $npmRange"
    } else {
...
function Test-Node {
    ...
    Write-Info "Using a ZeusAgent-managed Node.js installation instead..."

    # Prefer a ZeusAgent-managed Node from a previous run over a too-old system one.
    $managedNode = "$ZeusAgentHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$ZeusAgentHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$ZeusAgentHome\node"
        Write-Success "Node.js $version found (ZeusAgent-managed)"
        ...
        Update-ManagedNpm "$ZeusAgentHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing ZeusAgent-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $ZeusAgentHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $ZeusAgentHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\zeus-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$ZeusAgentHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$ZeusAgentHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                $stamp = [Guid]::NewGuid().ToString("N")
                $staged = "$ZeusAgentHome\node.new-$stamp"
                $backup = "$ZeusAgentHome\node.old-$stamp"
                ...
                if (Test-Path "$ZeusAgentHome\node") {
                    try {
                        Rename-Item "$ZeusAgentHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "ZeusAgent-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$ZeusAgentHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $ZeusAgentHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\zeus-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
    throw "ZeusAgent source distribution: run python scripts/setup_zeus.py from the extracted source package."
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `zeus
                    # update` path (#4735).
                    ...
                    $stashName = "zeus-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. zeus-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``zeus update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if ZeusAgent behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running zeus processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                $zipPath = "$env:TEMP\zeus-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\zeus-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `zeus update` (see the notes at the shared clone-path
    ...
    # next `zeus update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under ZeusAgent-Setup.exe
    ...
    if (-not $resolvedPython) {
        throw "ZeusAgent-managed Python is unavailable. Run install.ps1 -Stage python first."
    }
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running zeus process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running zeus processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*ZeusAgent_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (zeus.exe) plus its child tree.
            & taskkill /F /T /IM zeus.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM zeus.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m zeus_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not zeus.exe.
            ...
            # ZeusAgent children re-exec through .zeus-runtime, so killing only
            ...
            # user (see zeus_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-ZeusAgent " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `zeus update` / the
...
function Restore-VenvBackup {
    ...
    # venv so ZeusAgent (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `zeus.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install zeus-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and zeus.exe broken with
    ...
    # here, not 30 seconds later when the user runs `zeus`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close ZeusAgent processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close ZeusAgent processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves ZeusAgent and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register zeus.exe in dist-info/RECORD but fail to
        ...
        # `zeus` command while zeus-agent.exe / zeus-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m zeus_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `zeus dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\zeus_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `zeus dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `zeus dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: zeus_cli/web_server.py"
        }
    }
...
function Install-ZeusAgentCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the zeus launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring zeus.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "zeus.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the zeus command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # zeus_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("zeus", "zeus-acp")) {
    ...
    $requiredExe = Join-Path $Destination "zeus.exe"
    $requiredCmd = Join-Path $Destination "zeus.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the zeus command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up zeus command..."
    
    if ($NoVenv) {
        $zeusBin = "$InstallDir"
    } else {
        # $ZeusAgentHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `zeus update`'s autostash
        ...
        # installer staged under zeus-agent\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-ZeusAgentCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $zeusBin = "$ZeusAgentHome\bin"
        Install-ZeusAgentCommandLaunchers -Root $InstallDir -Destination $zeusBin | Out-Null
    }
    ...
    #   zeus-agent\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The zeus-agent\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept zeus via $zeusBin)"
        }
    }
    
    if ($currentPath -notlike "*$zeusBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$zeusBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $zeusBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set ZEUS_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\zeus instead
    # of the Unix default ~/.zeus
    $currentZeusAgentHome = [Environment]::GetEnvironmentVariable("ZEUS_HOME", "User")
    if (-not $currentZeusAgentHome -or $currentZeusAgentHome -ne $ZeusAgentHome) {
        [Environment]::SetEnvironmentVariable("ZEUS_HOME", $ZeusAgentHome, "User")
        Write-Success "Set ZEUS_HOME=$ZeusAgentHome"
    }
    $env:ZEUS_HOME = $ZeusAgentHome
    
    # Update current session
    $env:Path = "$zeusBin;$env:Path"
    
    Write-Success "zeus command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.zeus-bootstrap-complete which tells the ZeusAgent
    ...
    # ZeusAgent-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `zeus update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".zeus-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the ZEUS_HOME directory structure ($ZeusAgentHome, default %LOCALAPPDATA%\zeus)
    New-Item -ItemType Directory -Force -Path "$ZeusAgentHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZeusAgentHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZeusAgentHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZeusAgentHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZeusAgentHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZeusAgentHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZeusAgentHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZeusAgentHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZeusAgentHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$ZeusAgentHome\.env"
    ...
    # Create config.yaml
    $configPath = "$ZeusAgentHome\config.yaml"
    ...
    # (the default PS5 behaviour), and ZeusAgent's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$ZeusAgentHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in zeus_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $ZeusAgentHome"
    
    # Seed bundled skills into $ZeusAgentHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $ZeusAgentHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $ZeusAgentHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$ZeusAgentHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $ZeusAgentHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (ZeusAgent-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'zeus setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\zeus-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\zeus-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\zeus-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `zeus tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $ZeusAgentHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only ZeusAgent' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside ZeusAgent' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'zeus tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `zeus
...
# paths still lazy-install via install_cua_driver() (zeus_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `zeus computer-use install` runs. Bounded
        ...
        # ZeusAgent' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'zeus tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: zeus computer-use install"
            }
        } else {
...
# the final `electron` -> `ZeusAgent` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable ZeusAgent.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_zeus_desktop command
    # resolves apps/desktop/release/win-unpacked/ZeusAgent.exe directly,
    ...
    # enforces the supported Node lines and prepends the ZeusAgent-managed Node to
    ...
    # The --dir mode produces an unpacked ZeusAgent.exe in
    ...
    # are NOT trying to work around). The ZeusAgent icon + product name are
    # stamped onto ZeusAgent.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\zeus-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\ZeusAgent.exe",
        "$desktopDir\release\win-arm64-unpacked\ZeusAgent.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no ZeusAgent.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The ZeusAgent icon + identity are stamped onto ZeusAgent.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, zeus-agent#38216).
    ...
    #    ZeusAgent.exe. We deliberately do NOT point them at `zeus desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'ZeusAgent.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'ZeusAgent.lnk')
        )
        ...
        # with the ZeusAgent icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.zeus/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'zeus setup'."
        return
    }
    ...
    # Run zeus setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m zeus_cli.main setup
    } else {
        python -m zeus_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $zeusCmd = "$InstallDir\venv\Scripts\zeus.exe"
    ...
    $whatsappSession = "$ZeusAgentHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'zeus whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: zeus gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$ZeusAgentHome\logs\gateway.log"
            Start-Process -FilePath $zeusCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$ZeusAgentHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: zeus gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: zeus gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$ZeusAgentHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$ZeusAgentHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ZeusAgentHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ZeusAgentHome\zeus-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   zeus              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   zeus setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   zeus config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   zeus config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   zeus gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   zeus update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning ZeusAgent repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    # Managed Python lives under $InstallDir\.zeus-runtime, so the checkout
...
if ($IncludeDesktop) {
    ...
    # (ZeusAgent-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding ZeusAgent to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
