...
ZEUS_HOME so the operator's own telemetry state is untouched.
...
def main() -> int:
    scratch = Path(tempfile.mkdtemp(prefix="zeus-telemetry-e2e-"))
    os.environ["ZEUS_HOME"] = str(scratch)
    ...
    from zeus_cli.observability.shared_metrics_send_config import (
        resolve_send_config,
    )
    ...
    from zeus_cli.observability.shared_metrics_sender import (
    ...
    for index, metric_count in ((0, 3), (1, 140)):
        ...
        payload = {
            "schema_version": "zeus.shared_metrics.v2",
    ...
    print(f"scratch ZEUS_HOME : {scratch}")
    ...
    print("  aws s3 ls --recursive "
          "s3://zeus-agent-telemetry-staging-767397871023-us-west-2-an/raw/ "
          "| tail -20")
    return 0
