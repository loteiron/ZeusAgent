...
# changing PATH, installing software, or touching the user's ZeusAgent home.

$repoRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path))
$installScript = Join-Path $repoRoot 'scripts\install.ps1'
$testRoot = Join-Path $env:TEMP ("zeus-node-compatibility-test-" + [Guid]::NewGuid().ToString('N'))
$ZeusAgentHome = Join-Path $testRoot 'home'
$InstallDir = Join-Path $testRoot 'missing-checkout'
. $installScript -ZeusAgentHome $ZeusAgentHome -InstallDir $InstallDir
