...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[zeus] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[zeus`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[zeus`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        ZeusAgentHome = $(if ($paths) { $paths.zeus_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\zeus\zeus-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-zeus'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-zeus') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
