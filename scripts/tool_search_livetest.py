#!/usr/bin/env python3
"""Live test harness for ZeusAgent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any zeus imports.
ORIGINAL_HOME = os.environ.get("ZEUS_HOME")
ORIGINAL_AUTH = Path.home() / ".zeus" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.zeus/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.zeus/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="zeus_ts_live_"))
    zeus_home = home_dir / ".zeus"
    zeus_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, zeus_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".zeus" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, zeus_home / ".env")
        ...
        load_zeus_dotenv(zeus_home=str(Path.home() / ".zeus"))
    ...
    (zeus_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return zeus_home
...
def reset_module_state():
    """Drop cached modules so the new ZEUS_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "zeus_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original ZEUS_HOME
    if ORIGINAL_HOME is not None:
        os.environ["ZEUS_HOME"] = ORIGINAL_HOME
    else:
