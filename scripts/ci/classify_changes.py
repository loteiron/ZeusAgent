...
# tree generates llms.txt — the index every LLM (ZeusAgent included, via the
# zeus-agent skill) reads to learn what ZeusAgent can do — and
...
_MCP_CATALOG_FILES = {"zeus_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (ZeusAgent-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``zeus serve`` backend, the Docker image)
