# Behavioral test for install.ps1's zeus launcher staging (PR #92092,
...
#
# The test lifts the real Install-ZeusAgentCommandLaunchers function from the
...
# the caller (Set-PathVariable passes $ZeusAgentHome\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-ZeusAgentCommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-ZeusAgentCommandLaunchers not found in $installPs1"
}
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'zeus-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'zeus-agent'
    ...
    $missingThrew = $false
    try {
        Install-ZeusAgentCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
        $missingThrew = $_.Exception.Message -like '*required launcher not found*'
    }
    Assert-True $missingThrew 'missing zeus.exe fails the launcher stage'
    ...
    $zeusV2 = [byte[]](77, 90, 2)
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'zeus.exe'), $zeusV1)
    Set-Content -Path (Join-Path $installRoot 'venv\pyvenv.cfg') `
        -Value "home = X" -Encoding Ascii

    $staged = Install-ZeusAgentCommandLaunchers -Root $installRoot -Destination $binDir
    Assert-True ($staged -eq $binDir) 'returns the destination it staged into'
    Assert-BytesEqual $zeusV1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'zeus.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'zeus-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'zeus.exe'), $zeusV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'zeus-acp.exe'), $acp)
    Install-ZeusAgentCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $zeusV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'zeus.exe'))) `
        'installer refreshes an existing ZeusAgent launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'zeus-acp.exe'))) `
    ...
    Install-ZeusAgentCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'zeus.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'zeus.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'zeus.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'zeus.exe')) -and $cmdBody.Contains('%*')) `
