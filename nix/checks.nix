...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      zeus-agent = self'.packages.default;
      zeusVenv = zeus-agent.zeusVenv;
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.zeus-agent = programs;
              services.zeus-agent = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.zeus-agent. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.zeus-agent);
      ...
      configKeys = pkgs.runCommand "zeus-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${zeusVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "zeus-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "zeus-build-package" { } ''
          echo "PASS: package built at ${zeus-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "zeus-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.ZEUS_TEST = "1";
                environmentFiles = [ "/run/secrets/zeus-env" ];
                zeusHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "zeus" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "zeus" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.zeusAgentSetup.data;

            failures =
              lib.optional (names != [
                "zeus-agent"
                "zeus-backend"
              ]) "expected zeus-agent + zeus-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/zeus gateway" (argvOf "zeus-agent")
              ) "gateway process does not run `zeus gateway`: ${argvOf "zeus-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/zeus serve" (argvOf "zeus-backend")
              ) "backend process does not run `zeus serve`: ${argvOf "zeus-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "zeus-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/zeus-check/.zeus" (envOf n)) names
              ) "gateway and backend must share one ZEUS_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.ZEUS_HOME or null != "/home/zeus-check/.zeus"
              ) "programs.zeus-agent.enable must export ZEUS_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "zeus-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/zeus-check/.zeus/SOUL.md" activation
              ) "zeusHomeFiles must install into ZEUS_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads ZEUS_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "ZEUS_MANAGED=home-manager" (envOf n)) names
              ) "processes must report ZEUS_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "zeus-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "zeus-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            cases = [
              ...
              {
                name = "zeusHomeFiles needs no directory";
                ok = accepts { zeusHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "zeus-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one ZEUS_HOME ───────────────
        # `programs.enable` exports ZEUS_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.zeus
        # while the services use the ZEUS_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/zeus-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                zeusHome = "/home/zeus-check/.zeus-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "zeus-desktop") cfg.home.packages;
            ...
            # second ZeusAgent runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "zeus-agent") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.zeus-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.zeus-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one zeus-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "ZEUS_HOME" != "/home/zeus-check/.zeus-work"
              ) "the launcher must carry ZEUS_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "ZEUS_HOME")})"
              ++ lib.optional (
                setValue "ZEUS_MANAGED" != "home-manager"
              ) "the launcher must report ZEUS_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "ZEUS_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "ZEUS_DESKTOP_ZEUS" != "${lib.head agentPackages}/bin/zeus"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "ZEUS_DESKTOP_ZEUS")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "ZEUS_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "ZEUS_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "ZEUS_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "ZEUS_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "ZEUS_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "zeus-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares ZEUS_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set ZEUS_HOME" wrapper
              ) "the launcher must carry ZEUS_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "ZEUS_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "ZEUS_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? zeus-backend || cfg.launchd.agents ? zeus-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "zeus-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `zeus`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.zeus-agent.enable = true;";
              }
              {
                value = false;
                expect = "programs.zeus-agent.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "zeus-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "zeus-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "zeus" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."zeus-agent-setup".text;

            failures =
              lib.optional (names != [
                "zeus-agent"
                "zeus-backend"
              ]) "expected zeus-agent + zeus-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/zeus gateway" (execOf "zeus-agent")
              ) "gateway unit does not run `zeus gateway`: ${execOf "zeus-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/zeus dashboard" (execOf "zeus-backend")
              ) "backend unit does not run `zeus dashboard`: ${execOf "zeus-backend"}"
              ++ lib.optional (
                units.zeus-agent.environment.ZEUS_HOME != units.zeus-backend.environment.ZEUS_HOME
              ) "gateway and backend must share one ZEUS_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/zeus/.zeus/SOUL.md" activation
              ) "zeusHomeFiles must install into ZEUS_HOME";
          ...
          in
          pkgs.runCommand "zeus-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs zeus, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.zeus-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/zeus serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec zeus directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "zeus-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "zeus-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec zeus, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/zeus dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "zeus-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        # $ZEUS_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          ...
          in
          pkgs.runCommand "zeus-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'ZEUS_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--zeus-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "zeus-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "zeus-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${zeus-agent}/bin/zeus || (echo "FAIL: zeus binary missing"; exit 1)
          test -x ${zeus-agent}/bin/zeus-agent || (echo "FAIL: zeus-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${zeus-agent}/bin/zeus --version 2>&1 | grep -qi "zeus" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "zeus-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in zeus zeus-agent zeus-acp; do
            test -x ${zeus-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "zeus-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking zeus --help ==="
          ${zeus-agent}/bin/zeus --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${zeus-agent}/bin/zeus --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "zeus-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${zeus-agent}/share/zeus-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${zeus-agent}/share/zeus-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "ZEUS_BUNDLED_SKILLS" ${zeus-agent}/bin/zeus || \
            (echo "FAIL: ZEUS_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: ZEUS_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${zeus-agent}/share/zeus-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${zeus-agent}/share/zeus-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "ZEUS_OPTIONAL_SKILLS" ${zeus-agent}/bin/zeus || \
            (echo "FAIL: ZEUS_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, ZEUS_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "zeus-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${zeus-agent}/share/zeus-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${zeus-agent}/share/zeus-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "ZEUS_BUNDLED_PLUGINS" ${zeus-agent}/bin/zeus || \
            (echo "FAIL: ZEUS_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: ZEUS_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "zeus-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${zeus-agent}/share/zeus-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${zeus-agent}/share/zeus-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${zeus-agent}/share/zeus-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "ZEUS_BUNDLED_LOCALES" ${zeus-agent}/bin/zeus || \
            (echo "FAIL: ZEUS_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: ZEUS_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + ZEUS_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && ZEUS_BUNDLED_LOCALES=${zeus-agent}/share/zeus-agent/locales \
            ${zeusVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with ZEUS_BUNDLED_LOCALES set"; exit 1)
        ...
        # ZEUS_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "zeus-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${zeus-agent}/share/zeus-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${zeus-agent}/share/zeus-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "ZEUS_OPTIONAL_MCPS" ${zeus-agent}/bin/zeus || \
            (echo "FAIL: ZEUS_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: ZEUS_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${zeus-agent}/bin/zeus mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: zeus mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "zeus-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${zeus-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${zeus-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "ZEUS_TUI_DIR" ${zeus-agent}/bin/zeus || \
            (echo "FAIL: ZEUS_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: ZEUS_TUI_DIR set in wrapper"
        ...
        # Verify ZEUS_NODE is set in wrapper and points to Node 26+
        # (ZeusAgent pins its toolchain to Node 26 everywhere)
        zeus-node = pkgs.runCommand "zeus-node-version" { } ''
          set -e
          echo "=== Checking ZEUS_NODE in wrapper ==="
          grep -q "ZEUS_NODE" ${zeus-agent}/bin/zeus || \
            (echo "FAIL: ZEUS_NODE not set in wrapper"; exit 1)
          echo "PASS: ZEUS_NODE present in wrapper"

          ZEUS_NODE=$(sed -n "s/^export ZEUS_NODE='\(.*\)'/\1/p" ${zeus-agent}/bin/zeus)
          test -x "$ZEUS_NODE" || (echo "FAIL: ZEUS_NODE=$ZEUS_NODE not executable"; exit 1)
          echo "PASS: ZEUS_NODE executable at $ZEUS_NODE"

          NODE_MAJOR=$("$ZEUS_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, ZeusAgent requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All ZEUS_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify ZEUS_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "zeus-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(ZEUS_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking ZEUS_MANAGED guards ==="
          check_blocked "config set" ${zeus-agent}/bin/zeus config set model foo
          check_blocked "config edit" ${zeus-agent}/bin/zeus config edit
        ...
        extra-python-packages = let
          ...
          zeusWithExtra = zeus-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "zeus-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${zeusWithExtra}/bin/zeus || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${zeusWithExtra}/bin/zeus || \
          ...
          if grep -q "PYTHONPATH" ${zeus-agent}/bin/zeus; then
        ...
        extra-dependency-groups = let
          zeusWithGroups = zeus-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "zeus-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${zeusWithGroups}"
          echo "venv: ${zeusWithGroups.zeusVenv}"
        ...
        messaging-variant = pkgs.runCommand "zeus-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.zeusVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "zeus-config-roundtrip" {
          ...
          merge_and_load() {
            local zeus_home="$1"
            export ZEUS_HOME="$zeus_home"
            ${configMergeScript} ${nixSettings} "$zeus_home/config.yaml"
            ${zeusVenv}/bin/python3 -c '
import json, sys
