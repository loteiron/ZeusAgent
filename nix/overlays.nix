# nix/overlays.nix — Expose pkgs.zeus-agent for external NixOS configs
#
...
# `pkgs.zeus-agent`, `nix build .#default`, and the NixOS module's
...
{ inputs, ... }:
{
  flake.overlays.default = final: _: {
    zeus-agent = inputs.self.packages.${final.stdenv.hostPlatform.system}.default;
  };
}
