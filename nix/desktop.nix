# nix/desktop.nix — ZeusAgent Desktop (Electron) app build + wrapper
#
# `zeusAgent` is the fully-built `.#default` package — it ships the
# `zeus` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `ZEUS_DESKTOP_ZEUS` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing ZeusAgent CLI").
# No reimplementation of the agent resolution in this wrapper.
{
  ...
  # ZEUS_HOME and ZEUS_MANAGED here, which gives the app the same state
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "zeus-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "zeus-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = zeusNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "zeus-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe zeusNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/zeus-desktop $out/bin
    cp -r ${renderer}/* $out/share/zeus-desktop/
    ...
    substituteInPlace $out/share/zeus-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/zeus-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # ZEUS_DESKTOP_ZEUS to the absolute path of the nix-built `zeus`
    # binary so the desktop's resolver step 4 ("existing ZeusAgent CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/zeus-desktop \
      --add-flags "$out/share/zeus-desktop" \
      --set ZEUS_DESKTOP_ZEUS "${lib.getExe zeusAgent}" \
    ...
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/zeus.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../zeus_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/zeus-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/zeus.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/zeus.desktop
    runHook postInstall
  '';
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for ZeusAgent";
    homepage = "https://github.com/NousResearch/hermes-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "zeus-desktop";
  };
}
