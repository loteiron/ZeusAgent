...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # ZeusAgent derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            zeus-agent = prev.zeus-agent.overrideAttrs (_old: {
              ZEUS_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $ZEUS_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$ZEUS_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        zeus-agent = prev.zeus-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "zeus-agent-env" {
    zeus-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "zeus-agent-editable-env" {
    zeus-agent = dependency-groups;
  };
}
