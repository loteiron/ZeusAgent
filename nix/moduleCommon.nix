...
#
# `services.zeus-agent` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  zeusHome, programs.zeus-agent (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "zeus-config-attrs";
    description = "ZeusAgent YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $ZEUS_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "ZeusAgent";
      ...
      package = mkOption {
        ...
        description = "The zeus-agent package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The ZeusAgent configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `zeus config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $ZEUS_HOME/.env. ZeusAgent reads that file at each start, with
          load_zeus_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."zeus/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that ZeusAgent refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          ZeusAgent reads SOUL.md and memories/ from ZEUS_HOME, so put those
          files in `zeusHomeFiles`.
      ...
      zeusHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into ZEUS_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          ZeusAgent reads SOUL.md and the memory files from ZEUS_HOME and not
          from the working directory. Declare those files here, or ZeusAgent
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the zeus plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. ZeusAgent discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          zeus_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as zeus (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in zeus-agent's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `zeus gateway`.";
      };
      ...
      # ── The backend: `zeus serve` or `zeus dashboard` ──────────────
      # `zeus serve` and `zeus dashboard` are the same entry point,
      # zeus_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that ZeusAgent Desktop
      ...
      # controls an external gateway, with `zeus gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that ZeusAgent
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            ZEUS_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. ZeusAgent Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."zeus/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `zeus` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.zeus-agent.installPackage was removed. ZeusAgent now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.zeus-agent.enable = ${lib.boolToString (value != false)};  # the zeus CLI, and ZEUS_HOME for your shells
        programs.zeus-agent.desktop.enable = true;  # the desktop application

      `services.zeus-agent` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "zeus-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "zeus-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "zeus-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "zeus-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "zeus-agent: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # ZEUS_MANAGED variable of the service. The value tells the shell
    ...
    in
    ''
      # Directories. The service units and ZeusAgent make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${zeusHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. ZeusAgent
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${zeusHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${zeusHome}/config.yaml
            ${run}chmod ${modes.config} ${zeusHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "zeus-managed" managedSystem} ${zeusHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${zeusHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${zeusHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${zeusHome}/auth.json"
        else
          ''
            if [ ! -e ${zeusHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${zeusHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree zeusHome cfg.zeusHomeFiles}
      ...
      ${run}find ${zeusHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "zeus-agent: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${zeusHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  #
  # `exec` on the last line keeps zeus as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "zeus-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "zeus-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "zeus-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          ZEUS_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export ZEUS_DASHBOARD_SESSION_TOKEN

          if [ -z "$ZEUS_DASHBOARD_SESSION_TOKEN" ]; then
            echo "zeus-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "zeus-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "zeus-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "zeus-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "zeus-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "zeus-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "zeus-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "ZeusAgent web dashboard and desktop backend"
    else
      "ZeusAgent backend for ZeusAgent Desktop";

  # The environment that each ZeusAgent process needs, from either module.
  #
  # managedSystem gives the value of ZEUS_MANAGED. The CLI reads that
    ...
    [
      {
        ...
        message = ''
          ...
          To give ZeusAgent an identity and a memory, use
          ${optionPath}.zeusHomeFiles instead. Those files go to
          ZEUS_HOME. ZeusAgent reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of ZEUS_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
