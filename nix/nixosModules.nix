# nix/nixosModules.nix — the NixOS module for zeus-agent
#
...
#
# Container mode: zeus runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $ZEUS_HOME/.env
# and read by zeus at startup — no container recreation needed for env changes.
#
# Tool resolution: the zeus wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }:
{
  flake.nixosModules.default =
    ...
    let
      ...
      # ZEUS_HOME via addToSystemPackages, so they can save settings through the
      ...
      # zeus user + sudo on first boot (writable layer persists), then
      # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
      containerEntrypoint = pkgs.writeShellScript "zeus-container-entrypoint" ''
        set -eu

        ZEUS_UID="''${ZEUS_UID:?ZEUS_UID must be set}"
        ZEUS_GID="''${ZEUS_GID:?ZEUS_GID must be set}"

        # ── Group: ensure a group with GID=$ZEUS_GID exists ──
        # Check by GID (not name) to avoid collisions with pre-existing groups
        # (e.g. GID 100 = "users" on Ubuntu)
        EXISTING_GROUP=$(getent group "$ZEUS_GID" 2>/dev/null | cut -d: -f1 || true)
        if [ -n "$EXISTING_GROUP" ]; then
          GROUP_NAME="$EXISTING_GROUP"
        else
          GROUP_NAME="zeus"
          if command -v groupadd >/dev/null 2>&1; then
            groupadd -g "$ZEUS_GID" "$GROUP_NAME"
          elif command -v addgroup >/dev/null 2>&1; then
            addgroup -g "$ZEUS_GID" "$GROUP_NAME" 2>/dev/null || true
          fi
        fi

        # ── User: ensure a user with UID=$ZEUS_UID exists ──
        PASSWD_ENTRY=$(getent passwd "$ZEUS_UID" 2>/dev/null || true)
        if [ -n "$PASSWD_ENTRY" ]; then
          TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
          TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
        else
          TARGET_USER="zeus"
          TARGET_HOME="/home/zeus"
          if command -v useradd >/dev/null 2>&1; then
            useradd -u "$ZEUS_UID" -g "$ZEUS_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
          elif command -v adduser >/dev/null 2>&1; then
            adduser -u "$ZEUS_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
          fi
        fi
        mkdir -p "$TARGET_HOME"
        chown "$ZEUS_UID:$ZEUS_GID" "$TARGET_HOME"
        chmod 0750 "$TARGET_HOME"

        # Ensure ZEUS_HOME is owned by the target user.
        ...
        if [ -n "''${ZEUS_HOME:-}" ] && [ -d "$ZEUS_HOME" ]; then
          find "$ZEUS_HOME" \! -user "$ZEUS_UID" -exec chown "$ZEUS_UID:$ZEUS_GID" {} +
        fi
        ...
        if [ ! -f /var/lib/zeus-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
          ...
          touch /var/lib/zeus-tools-provisioned
        fi

        if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/zeus ]; then
          mkdir -p /etc/sudoers.d
          echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/zeus
          chmod 0440 /etc/sudoers.d/zeus
        fi
        ...
        if command -v setpriv >/dev/null 2>&1; then
          exec setpriv --reuid="$ZEUS_UID" --regid="$ZEUS_GID" --init-groups "$@"
      ...
      # so they can update without recreation. Env vars go through $ZEUS_HOME/.env.
      ...
      containerModeFile = pkgs.writeText "zeus-container-mode" ''
        # Written by the NixOS activation script. Do not edit manually.
        backend=${cfg.container.backend}
        container_name=${containerName}
        exec_user=${cfg.user}
        zeus_bin=${containerDataDir}/current-package/bin/zeus
      '';

      # Default: /var/lib/zeus/workspace → /data/workspace.
      ...
      commonServiceConfig = {
        ...
        # so interactive users in the zeus group can read/write them.
        UMask = "0007";
      ...
      // common.processEnvironment { inherit zeusHome; };
    ...
    in
    {
      options.services.zeus-agent =
        ...
        // (
          with lib;
          {
            ...
            stateDir = mkOption {
              type = types.str;
              default = "/var/lib/zeus";
              description = "State directory. Contains .zeus/ subdir (ZEUS_HOME).";
            };
            ...
            addToSystemPackages = mkOption {
              ...
              default = false;
              description = ''
                Add the zeus CLI to environment.systemPackages and export
                ZEUS_HOME system-wide (via environment.variables) so interactive
            ...
            container = {
              ...
              hostUsers = mkOption {
                ...
                default = [ ];
                description = ''
                  Interactive users who get a ~/.zeus symlink to the service
                  stateDir. These users are automatically added to the zeus group.
                '';
                example = [ "sidbin" ];
              };
            };
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          # ── Host CLI ──────────────────────────────────────────────────────
          # Add the zeus CLI to system PATH and export ZEUS_HOME system-wide
          # so interactive shells share state (sessions, skills, cron) with the
          # gateway service instead of creating a separate ~/.zeus/.
          (lib.mkIf cfg.addToSystemPackages {
            environment.systemPackages = [ effectivePackage ];
            environment.variables.ZEUS_HOME = zeusHome;
          })
              ...
              ++ [
                {
                  ...
                  message = "services.zeus-agent: backend.mode is not supported together with container.enable — the container runs the gateway only.";
                }
              ];
          }

          # ── Per-user profile for extraPackages ───────────────────────────
          # Wire extraPackages into the zeus user's per-user profile so the
          ...
          (lib.mkIf
            (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != [ ])
            {
              warnings = [
                ''
                  services.zeus-agent: container.enable is true and container.hostUsers
                  is set, but addToSystemPackages is false. Without a host-installed zeus
                  binary, container routing will not work for interactive users.
                  Set addToSystemPackages = true or ensure zeus is on PATH.
                ''
              ];
            }
          )
          ...
          # ── Directories ───────────────────────────────────────────────────
          {
            systemd.tmpfiles.rules = [
              ...
              "d ${zeusHome}                  2770 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.stateDir}/home           0750 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.workingDirectory}        2770 ${cfg.user} ${cfg.group} - -"
            ]
            ++ map (d: "d ${zeusHome}/${d} 2770 ${cfg.user} ${cfg.group} - -") common.stateSubdirs;
          }

          # ── Activation: link config + auth + documents ────────────────────
          {
            system.activationScripts."zeus-agent-setup" =
              lib.stringAfter
                (
                ...
                ''
                  ...
                  chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${zeusHome} ${cfg.stateDir}/home ${cfg.workingDirectory}
                  chmod 2770 ${cfg.stateDir} ${zeusHome} ${cfg.workingDirectory}
                  ...
                  find ${zeusHome} -maxdepth 1 \
                    \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
                    -exec chmod g+rw {} + 2>/dev/null || true
                  for _subdir in ${lib.concatStringsSep " " common.stateSubdirs}; do
                    mkdir -p "${zeusHome}/$_subdir"
                    chown ${cfg.user}:${cfg.group} "${zeusHome}/$_subdir"
                    chmod 2770 "${zeusHome}/$_subdir"
                    find "${zeusHome}/$_subdir" -type f \
                      -exec chmod g+rw {} + 2>/dev/null || true
                  done

                  ${common.mkStateScript {
                    inherit pkgs cfg zeusHome;
                    workingDirectory = cfg.workingDirectory;
                  ...
                  chown -h ${cfg.user}:${cfg.group} ${zeusHome}/plugins/nix-managed-* 2>/dev/null || true
                  ...
                  ${
                    if cfg.container.enable then
                      ''
                        install -o ${cfg.user} -g ${cfg.group} -m 0644 ${containerModeFile} ${zeusHome}/.container-mode
                      ''
                    else
                      ''
                        rm -f ${zeusHome}/.container-mode
                        ...
                        ${lib.concatStringsSep "\n" (
                          map (
                            ...
                            let
                              ...
                              symlinkPath = "${userHome}/.zeus";
                            in
                            ''
                              if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${zeusHome}" ]; then
                                rm -f "${symlinkPath}"
                                echo "zeus-agent: removed symlink ${symlinkPath}"
                              fi
                            ''
                  ...
                  # Create ~/.zeus -> stateDir/.zeus for each hostUser so the
          ...
          (lib.mkIf (!cfg.container.enable) {
            systemd.services.zeus-agent = {
              ...
              # $ZEUS_HOME/.env by the activation script. load_zeus_dotenv()
          ...
          (lib.mkIf cfg.container.enable {
            ...
            systemd.services.zeus-agent = {
              description = "ZeusAgent Gateway (container)";
              wantedBy = [ "multi-user.target" ];
              after = [
              ...
              preStart = ''
                ...
                if [ "$NEED_CREATE" = "true" ]; then
                  ...
                  ZEUS_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
                  ZEUS_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
                  ...
                  ${containerBin} create \
                    ...
                    --env ZEUS_UID="$ZEUS_UID" \
                    --env ZEUS_GID="$ZEUS_GID" \
                    --env ZEUS_HOME=${containerDataDir}/.zeus \
                    --env ZEUS_MANAGED=true \
                    --env HOME=${containerHomeDir} \
                    ${lib.concatStringsSep " " cfg.container.extraOptions} \
                    ${cfg.container.image} \
                    ${containerDataDir}/current-package/bin/zeus gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
