# nix/web.nix — ZeusAgent Web Dashboard (Vite/React) frontend build
{ zeusNpmLib, ... }:
zeusNpmLib.buildNpmPackage {
  dirs = [
    "web"

    # @zeus/shared ships as a file: workspace dep of web, so its source
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../zeus_cli/web_dist for the
