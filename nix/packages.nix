# nix/packages.nix — ZeusAgent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./zeus-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.zeusTui;
        web = full.zeusWeb;
        desktop = full.zeusDesktop;

        update-npm-lockfile = full.zeusNpmLib.updateNpmLockfile;
      };
    };
}
