"""Unified removal contract for every credential source ZeusAgent reads from.

Readers live in ``agent.credential_pool``; what is unified here is **removal**:
``zeus auth remove <provider> <N>`` must make the entry stay gone across
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    """env:<VAR> — clear from ~/.zeus/.env; hint when the shell exports it."""
    from zeus_cli.config import get_env_path, remove_env_value
    ...
    # .env as utf-8-sig like zeus_cli/config.py: a BOM-sensitive read would
    ...
    if env_in_process and not env_in_dotenv:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to ZeusAgent.",
            f"  The pool entry is now suppressed — ZeusAgent will ignore "
            f"{env_var} until you run `zeus auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_zeus_pkce(provider: str, removed) -> RemovalResult:
    """~/.zeus/.anthropic_oauth.json is ours — delete it outright."""
    from zeus_constants import get_zeus_home

    result = RemovalResult()
    oauth_file = get_zeus_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared ZeusAgent Anthropic OAuth credentials")
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.append(
        "Run `zeus model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.extend([
        ...
        "Run `zeus auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `zeus auth add copilot` to re-enable if needed.",
    ])
...
_REGISTRY: List[RemovalStep] = [
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `zeus auth add anthropic` to re-enable if needed.",
        ),
        description="~/.claude/.credentials.json",
    ),
    RemovalStep(
        provider="anthropic", source_id="zeus_pkce",
        remove_fn=_remove_zeus_pkce,
        description="~/.zeus/.anthropic_oauth.json",
    ),
    RemovalStep(
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `zeus auth add qwen-oauth` to re-enable if needed.",
        ),
