...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # codex needs LLM provider creds but must not receive Tier-1 ZeusAgent secrets (gateway/GitHub/infra tokens).
        ...
        # `os.environ.copy()` also handed it every Tier-1 ZeusAgent secret — gateway bot tokens, GitHub auth,
        ...
        spawn_env = zeus_subprocess_env(inherit_credentials=True)
        if env:
        ...
        # Native shell children remain unowned. Only ZeusAgent' managed MCP tool
        ...
        owned_task = os.environ.get("ZEUS_KANBAN_TASK") and is_dispatcher_owned_worker_context()
        if owned_task:
            for key in (*KANBAN_ENV_KEYS, "ZEUS_KANBAN_DB", "ZEUS_KANBAN_BOARD"):
                if key in os.environ:
                    cmd += ["-c", f"mcp_servers.zeus-mcp.env.{key}={json.dumps(os.environ[key])}"]
            cmd += ["-c", f'mcp_servers.zeus-mcp.env.{DELEGATED_CHILD_ENV_MARKER}=""']
        ...
        if owned_task:
            kanban_db = spawn_env.get("ZEUS_KANBAN_DB")
            default_root = os.path.join(spawn_env.get("ZEUS_HOME", os.path.expanduser("~/.zeus")), "kanban")
            kanban_root = os.path.dirname(kanban_db) if kanban_db else spawn_env.get("ZEUS_KANBAN_ROOT", default_root)
            cmd += [
    ...
    def initialize(
        self, client_name: str = "zeus", client_title: str = "ZeusAgent",
