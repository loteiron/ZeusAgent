...
Owns one Codex thread per ZeusAgent session: drives ``turn/start``, consumes
...
# ZeusAgent' tools.terminal.security_mode -> Codex permissions profile id.
# Missing config -> workspace-write (Codex's own default).
_ZEUS_TO_CODEX_PERMISSION_PROFILE = {
...
class CodexAppServerSession:
    """One Codex thread per ZeusAgent session, lifetime owned by AIAgent. Not thread-safe: one caller at a time."""
    ...
    def __init__(
        ...
        self._permission_profile = permission_profile or _ZEUS_TO_CODEX_PERMISSION_PROFILE.get(
            os.environ.get("ZEUS_TERMINAL_SECURITY_MODE", "auto"), "workspace-write"
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(client_name="zeus", client_title="ZeusAgent", client_version=_get_zeus_version())
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Answer a codex server request (approval / elicitation) via ZeusAgent' approval flow.
    ...
    def _respond_elicitation(self, params: dict) -> dict:
        """MCP elicitation: auto-accept our own zeus-tools server (opted in by enabling the runtime;
        exposes nothing codex's shell can't do); decline others so the user opts in via codex's own flow."""
        action = "accept" if (params.get("serverName") or "") == "zeus-tools" else "decline"
...
# ZeusAgent approval choice -> codex decision (app-server-protocol v2). "deny" and
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map a ZeusAgent approval choice onto codex's approval decision wire value."""
...
def _get_zeus_version() -> str:
    """Best-effort ZeusAgent version string for codex's userAgent line."""
    try:
