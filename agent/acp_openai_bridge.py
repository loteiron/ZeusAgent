"""OpenAI-shape bridge shared by ZeusAgent' ACP clients.

ACP has no OpenAI-style ``tools``/``tool_calls`` channel, so ZeusAgent' tool schemas travel INTO the
...
read/edit/execute tools forwards only ZeusAgent' agent-level tools, since re-offering overlapping ones
makes ZeusAgent redo finished work.
"""
...
class StreamChunks(list):
    """Chunk list that also carries response-level attributes (e.g. ``zeus_projected_messages``)
    ZeusAgent reads off the ``create`` result; a plain list would drop them on the stream path."""
