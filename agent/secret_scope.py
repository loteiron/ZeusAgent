...
_GLOBAL_ENV_EXACT = frozenset({
    # ZeusAgent runtime / deployment
    "ZEUS_HOME", "ZEUS_PROFILE", "ZEUS_GATEWAY_LOCK_DIR",
    "ZEUS_MAX_ITERATIONS", "ZEUS_API_TIMEOUT",
    "ZEUS_REDACT_SECRETS", "ZEUS_NOUS_TIMEOUT_SECONDS",
    "_ZEUS_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "ZEUS_KANBAN_DB", "ZEUS_KANBAN_WORKSPACES_ROOT", "ZEUS_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "ZEUS_KANBAN_",
    "ZEUS_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    from zeus_cli.config import _parse_env_value
...
def build_profile_secret_scope(zeus_home: Path) -> Dict[str, str]:
    ...
    secrets = load_env_file(Path(zeus_home) / ".env")
    try:
        from zeus_cli.env_loader import get_secret_source_values
        external_secrets = get_secret_source_values(Path(zeus_home))
    except Exception:
        external_secrets = {}
