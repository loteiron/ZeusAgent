...
def _print_nous_401_diagnostics(agent: Any, api_error: Exception) -> None:
    ...
    _plines(
        agent,
        "   Troubleshooting:",
        "     • Re-authenticate: zeus auth add nous",
        "     • Check credits / billing: https://portal.nousresearch.com",
        f"     • Verify stored credentials: {display_zeus_home()}/auth.json",
...
def _print_anthropic_401_diagnostics(agent: Any, key: Any) -> None:
    ...
    if is_token_provider(key):
        ...
        _plines(
            ...
            "   Run `zeus doctor` for credential-chain diagnostics, or",
    ...
    _dhh = display_zeus_home()
    _plines(
        agent,
        "   Troubleshooting:",
        f"     • Check ANTHROPIC_TOKEN in {_dhh}/.env for ZeusAgent-managed OAuth/setup tokens",
        ...
        "     • Legacy cleanup: zeus config set ANTHROPIC_TOKEN \"\"",
        "     • Clear stale keys: zeus config set ANTHROPIC_API_KEY \"\"",
    )
...
def recover_after_classification(
    ...
    if (
        ...
        try:
            from zeus_cli.models_reasoning_caps import refresh_reasoning_caps_async
...
def _print_nonretryable_auth_guidance(
    ...
    if provider in {"openai-codex", "xai-oauth", "nous"} and status_code == 401:
        if provider == "openai-codex":
            _vlines(
                ...
                "      2. Then run `zeus auth` to re-authenticate.",
            )
        elif provider == "xai-oauth":
            _vlines(
                agent,
                "   💡 xAI OAuth token was rejected (HTTP 401). To fix:",
                "      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `zeus model`.",
            )
        else:  # nous
            _vlines(
                agent,
    ...
    _vlines(
        ...
        "      • Is the key valid? Run: zeus setup",
...
def nonretryable_client_error_result(
    ...
    if classified.reason == FailoverReason.content_policy_blocked:
        _vlines(
            ...
            "        zeus fallback add   (interactive picker — same as `zeus model`)",
        )
    ...
    if classified.reason == FailoverReason.content_policy_blocked:
        _policy_response = (
            ...
            "(not a ZeusAgent/gateway failure).\n\n"
...
def max_retries_exhausted_result(
    ...
    if _is_thinking_timeout:
        _vlines(
            ...
            "in `~/.zeus/config.yaml` to extend the per-call timeout. (ZeusAgent's built-in floor is 600s for "
...
def log_api_error_attempt(
    ...
    if getattr(api_error, "status_code", None) == 404:
        try:
            from zeus_cli.model_normalize import suggest_prefixed_model_id
        ...
        if _suggestion:
            _blines(
                ...
                f"      Did you mean '{_suggestion}'?  Re-pick it with `zeus model`.",
            )
