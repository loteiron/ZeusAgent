...
Installs go to a ZeusAgent-owned staging dir, ``<ZEUS_HOME>/lsp/bin/``, so the
...
missing binary skips the server and ``zeus lsp status`` reports it).
...
# ``bin`` in ``<ZEUS_HOME>/lsp/bin/`` first, then on PATH.  ``extra_pkgs``
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # the host so `zeus lsp status` reports its presence.
...
def zeus_lsp_bin_dir() -> Path:
    """Return the ZeusAgent-owned bin staging dir for LSP servers."""
    from zeus_constants import get_zeus_home

    p = get_zeus_home() / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(zeus_lsp_bin_dir() / name):
...
def _link_into_bin(target: Path) -> str:
    ...
    link = zeus_lsp_bin_dir() / target.name
    if not link.exists():
        try:
...
def _install_npm(pkg: str, bin_name: str, extra_pkgs: Optional[list] = None) -> Optional[str]:
    ...
    # Managed npm first: $ZEUS_HOME/node isn't on an arbitrary process's
    # PATH, so a bare which() would miss the Node that ZeusAgent installed.
    npm = find_node_executable("npm")
    if npm is None:
        logger.info("[install] cannot install %s: no usable npm found", pkg)
        return None
    staging = zeus_lsp_bin_dir().parent  # <ZEUS_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    ...
    pip_target = zeus_lsp_bin_dir().parent / "python-packages"
...
def detect_status(pkg: str) -> str:
    """Return ``installed``, ``missing``, or ``manual-only`` (for ``zeus lsp status``; spawns nothing)."""
    recipe = INSTALL_RECIPES.get(pkg)
...
__all__ = ["INSTALL_RECIPES", "try_install", "detect_status", "zeus_lsp_bin_dir"]
