...
inferred from the atlas shape so ZeusAgent renders both legacy 8-row sheets and
...
class PetState(str, Enum):
    """Animation state a pet can be shown in (ZeusAgent names; Codex rows say ``jumping``/``running`` for ``jump``/``run``)."""
...
# Legacy ZeusAgent/petdex row order (top -> bottom) for the older 8-row, 9-column atlas.
...
# Canonical ZeusAgent names -> accepted row-name aliases in descending preference.
