...
In a code workspace on an interactive surface ZeusAgent adopts a **coding posture**: a
...
from zeus_cli._subprocess_compat import bounded_git_probe

logger = logging.getLogger("zeus.coding_context")
...
# Operating brief. Tool names referenced here are in the coding toolset and _ZEUS_CORE_TOOLS.
...
def _agent_config_value(config: Optional[dict[str, Any]], key: str, default: Any, *, readonly: bool) -> Any:
    ...
    if config is None:
        try:
            from zeus_cli.config import load_config, load_config_readonly
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        apply it only when the user hasn't pinned a selection (``--toolsets``, ``ZEUS_TUI_TOOLSETS``)."""
...
def is_coding_context(*, platform: Optional[str] = None, cwd: Optional[str | Path] = None, config: Optional[dict[str, Any]] = None) -> bool:
    """Whether ZeusAgent should operate in its coding posture right now."""
