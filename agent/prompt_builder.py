...
from zeus_constants import (
    get_zeus_home, get_skills_dir, is_wsl, reset_zeus_home_override, set_zeus_home_override,
)
...
def _find_zeus_md(cwd: Path) -> Optional[Path]:
    """Nearest ``.zeus.md`` / ``ZEUS.md`` from *cwd* up to the git root, else None."""
    ...
    for directory in [current, *current.parents] if stop_at else [current]:
        found = next((directory / n for n in (".zeus.md", "ZEUS.md") if (directory / n).is_file()), None)
...
DEFAULT_AGENT_IDENTITY = (
    ...
    "You are ZeusAgent, an independent fork of Hermes Agent by Nous Research. Be direct: match the length of your reply to the weight of the ask "
...
ZEUS_AGENT_HELP_GUIDANCE = (
    # Injected only when skill_view exists AND the zeus-agent skill is installed (system_prompt.py slot
    # resolution). No "when the two differ" clause: docs-are-authoritative already carries the precedence.
    "You run on ZeusAgent (derived from Hermes Agent by Nous Research). When the user needs help with ZeusAgent itself — configuring, "
    ...
    "authoritative reference and always holds the latest, most up-to-date information. The `zeus-agent` "
    "skill has the actual commands and proven workflows — load it with skill_view(name='zeus-agent') "
    "before configuring, modifying, or troubleshooting ZeusAgent so you don't guess or invent workarounds."
)

# Variant for sessions without the skills toolset (e.g. Blank Slate): naming skill_view() there would dangle.
ZEUS_AGENT_HELP_GUIDANCE_NO_SKILLS = (
    "You run on ZeusAgent (derived from Hermes Agent by Nous Research). When the user needs help with ZeusAgent itself — configuring, "
    "setting up, using, extending, or troubleshooting it — or when you need to understand your own features, "
...
KANBAN_GUIDANCE = (
    ...
    "You have been assigned ONE task from the shared board at `~/.zeus/kanban.db`. Your task id is in "
    "`$ZEUS_KANBAN_TASK`; your workspace is `$ZEUS_KANBAN_WORKSPACE`. The `kanban_*` tools in your schema are your "
    ...
    "2. **Work inside the workspace.** `cd $ZEUS_KANBAN_WORKSPACE` before any file operations. The workspace is "
    ...
    "- **Workspace.** `cd $ZEUS_KANBAN_WORKSPACE` first. For a `worktree` kind with no `.git`, `git worktree add "
    "<path> ${ZEUS_KANBAN_BRANCH:-wt/$ZEUS_KANBAN_TASK}` from the main repo, then cd there. For a project-linked "
    "task the workspace is a fresh `<repo>/.worktrees/<task-id>` and `$ZEUS_KANBAN_BRANCH` a deterministic "
    ...
    "sits in `ready` forever). Ground every assignee in a real profile (`zeus profile list`, or ask the user), and "
    "express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n\n"
    "- Do not shell out to `zeus kanban <verb>` for board operations. Use the `kanban_*` tools — they work across "
...
# cost that compounds over a long conversation. The zeus-agent runtime already executes a batch of tool
...
# to zeus-agent's Python prompt-assembly architecture.
...
STEER_CHANNEL_NOTE = (
    ...
    "Mid-turn, the user can steer you: ZeusAgent appends their message to the end of a tool result, wrapped exactly as:\n"
    f"{STEER_MARKER_OPEN}\n<their message>\n{STEER_MARKER_CLOSE}\n"
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    gated = (
        (True,
         "[Note: this message came from HUD mode — a small floating ZeusAgent "
         ...
         "rather than anything inside ZeusAgent. read_window_below identifies that app."),
        (True,
...
PLATFORM_HINTS = {
    ...
    "tui": (
        ...
        "You are in the ZeusAgent terminal UI (TUI). Files: there is no attachment channel and MEDIA:/path tags "
    ...
    "desktop": (
        ...
        "You are chatting inside the ZeusAgent desktop app, a graphical chat surface. Markdown renders with full GitHub "
        ...
        "or it measures full-bleed. Widgets talk back: data-zeus-send=\"prompt\" on any clickable element (or "
        "window.zeus.send(\"prompt\")) sends that prompt as a hidden user turn — answer it by updating the widget's "
...
def _run_backend_probe(env_type: str, terminal_tool) -> str:
    ...
    env = _create_environment(
        ...
        # snapshot. A normal SSHEnvironment would upload the whole ~/.zeus tree just to run `uname`,
...
def _remote_backend_hint(backend: str) -> str:
    ...
    if probe:
        return lead + (
            f"this {backend} environment — NOT on the machine where ZeusAgent itself is running. The host OS, "
            f"home, and cwd of the ZeusAgent process are irrelevant; only the following backend state matters:\n{probe}"
        )
    description = (
    ...
    return lead + (
        f"{description} — NOT on the machine where ZeusAgent itself runs. The backend probe didn't respond at "
...
def _embedder_environment_hint() -> str:
    """Embedder-supplied environment description: ZEUS_ENVIRONMENT_HINT (container ENV)
    wins over config.yaml ``agent.environment_hint``. Read once at prompt-build time."""
    return (os.getenv("ZEUS_ENVIRONMENT_HINT") or "").strip() or str(
...
RUNTIME_ENVIRONMENT_HEADING = "# ZeusAgent runtime environment"
RUNTIME_ENVIRONMENT_END = "<!-- End ZeusAgent runtime environment -->"
...
def _skills_prompt_snapshot_path() -> Path:
    return get_zeus_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("ZEUS_PLATFORM") or os.environ.get("ZEUS_SESSION_PLATFORM")
    if platform:
        return platform
    get_session_env = getattr(sys.modules.get("gateway.session_context"), "get_session_env", None)
    try:
        return (get_session_env("ZEUS_SESSION_PLATFORM") if get_session_env else "") or ""
    except Exception:
        return ""
...
def build_skills_system_prompt(
    ...
    ``skills_dir_override`` makes home resolution EXPLICIT: a build thread that never bound the ZEUS_HOME
    ...
    if skills_dir_override is not None:
        ...
        _home_token = set_zeus_home_override(str(skills_dir.parent))
    else:
    ...
    try:
        ...
        if _home_token is not None:
            reset_zeus_home_override(_home_token)
...
def load_soul_md(context_length: Optional[int] = None, home_override: "Path | None" = None) -> Optional[str]:
    """SOUL.md from ZEUS_HOME (identity slot #1), or None.

    Callers must pass ``skip_soul=True`` to ``build_context_files_prompt`` so it isn't injected twice.
    ``home_override`` pins the profile home (a thread that lost the ZEUS_HOME ContextVar reads the wrong one).

    ``home_override`` scopes the read to an explicit profile home (the agent knows its own home from its
    session_db path). Without it, resolution is ambient — which on a thread that lost the ZEUS_HOME
    ...
    """
    try:
        from zeus_cli.config import ensure_zeus_home
        ensure_zeus_home()
    except Exception as e:
        logger.debug("Could not ensure ZEUS_HOME before loading SOUL.md: %s", e)
    soul_path = (Path(home_override) if home_override is not None else get_zeus_home()) / "SOUL.md"
...
def _load_zeus_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.zeus.md / ZEUS.md — nearest match walking up to the git root."""
    zeus_md_path = _find_zeus_md(cwd_path)
    content = _read_context_file(zeus_md_path) if zeus_md_path else ""
    if not content:
        return ""
    label = str(zeus_md_path.relative_to(cwd_path)) if zeus_md_path.is_relative_to(cwd_path) else zeus_md_path.name
    return _context_section(_strip_yaml_frontmatter(content), label, ".zeus.md", zeus_md_path, context_length)
...
def build_context_files_prompt(
    ...
    Only ONE project context type loads, first found wins: .zeus.md/ZEUS.md (walk to git root) →
    AGENTS.md chain (git root → cwd) → CLAUDE.md (cwd) → .cursorrules + .cursor/rules/*.mdc (cwd). SOUL.md
    from ZEUS_HOME is independent and always included unless *skip_soul* (already the identity slot).
    """
    cwd_path = Path(cwd if cwd is not None else os.getcwd()).resolve()
    # A FALLBACK-picked cwd inside the ZeusAgent install tree must not gain system-prompt authority (the desktop
    # default would load this repo's contributor AGENTS.md). An explicit cwd is honored verbatim.
    # An explicitly configured cwd is honored verbatim — the ZeusAgent tree is a legitimate workspace when the
    # user deliberately points a session at it — and CLI-style surfaces pass
    # allow_install_tree_fallback=True because their launch dir IS the user's shell cwd (developing ZeusAgent
    # in-tree). See #64590.
    from agent.runtime_cwd import _is_install_tree
    if cwd is None and not allow_install_tree_fallback and _is_install_tree(cwd_path):
        logger.warning(
            "skipping project-context discovery: working-directory resolution fell back to the ZeusAgent "
            "install tree (%s) — set terminal.cwd to your project directory", cwd_path,
        )
        sections = []
    else:
        sections = [_load_zeus_md(cwd_path, context_length) or _load_agents_md(cwd_path, context_length)
