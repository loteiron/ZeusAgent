...
try:
    import zeus_cli as _zeus_cli

    _ZEUS_VERSION = str(_zeus_cli.__version__)
except Exception:
    _ZEUS_VERSION = "0.0.0"
_API_CLIENT = f"zeus-agent/{_ZEUS_VERSION}"  # client context per Gemini's partner-integration guidance
...
_FREE_TIER_GUIDANCE = (
    ...
    "ZeusAgent typically makes 3-10 API calls per user turn, so the free tier is exhausted in a handful of "
...
_STANDARD_KEY_GUIDANCE = (
    ...
    "generativelanguage.googleapis.com). Then update GEMINI_API_KEY / GOOGLE_API_KEY in ~/.zeus/.env and "
...
_MISSING_KEY_ERROR = (
    ...
    "environment / ~/.zeus/.env (get one at https://aistudio.google.com/app/apikey), or run `zeus setup` to "
...
class GeminiAPIError(Exception):
    """Error shape compatible with ZeusAgent retry/error classification."""
...
class GeminiNativeClient:
    ...
    # (No ZEUS_SKIP_ASYNC_WRAP — the async path has a real conversion, AsyncGeminiNativeClient.)
