...
YAML files in ``<ZEUS_HOME>/skill-bundles/`` (``name``, ``description``,
...
def _bundles_dir() -> Path:
    """Bundles directory: ``ZEUS_BUNDLES_DIR`` override (tests) or ``<ZEUS_HOME>/skill-bundles``."""
    override = os.environ.get("ZEUS_BUNDLES_DIR")
    return Path(override).expanduser() if override else get_zeus_home() / "skill-bundles"
...
# File-level CRUD — used by `zeus bundles`.
