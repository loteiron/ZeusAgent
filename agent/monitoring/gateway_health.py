...
def build_gateway_health_snapshot(
    ...
    base = {
        ...
        "zeus.supervision_mode": mode if mode in _SUPERVISION_MODES else "unknown",
    }
    ...
    metrics: list[GatewayMetric] = [
        metric("zeus.gateway.up", int(bool(gateway_running))),
        metric("zeus.gateway.active_agents", active_agents),
        metric("zeus.gateway.busy", int(bool(busy))),
        metric("zeus.gateway.drainable", int(bool(drainable))),
        metric("zeus.gateway.restart_requested", int(bool(runtime.get("restart_requested")))),
        metric("zeus.gateway.state", 1, **{"zeus.gateway.state": gateway_state}),
    ]
    fatal_count = 0
    events: list[GatewayHealthEvent | GatewayDiagnosticEvent] = []
    ...
    for platform, pdata in platforms.items():
        ...
        pattrs = {"zeus.platform": str(platform), "zeus.platform.state": state}
        metrics.append(metric("zeus.platform.up", int(state in _RUNNING_PLATFORM_STATES), **pattrs))
        metrics.append(metric("zeus.platform.degraded", int(is_degraded), **pattrs, **{"zeus.error_code": error_code}))
        if is_degraded:
