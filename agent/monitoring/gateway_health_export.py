...
_OBSERVABLE_METRIC_NAMES = (
    "zeus.gateway.up", "zeus.gateway.state", "zeus.gateway.active_agents", "zeus.gateway.busy",
    "zeus.gateway.drainable", "zeus.gateway.restart_requested", "zeus.gateway.background_work",
    "zeus.gateway.background_delegations", "zeus.platform.up", "zeus.platform.degraded",
    "zeus.cron.scheduler.heartbeat_age_seconds", "zeus.cron.scheduler.last_success_age_seconds",
    "zeus.cron.scheduler.catch_up_occurrences", "zeus.cron.jobs.enabled", "zeus.cron.jobs.running",
    "zeus.cron.jobs.overdue",
)
...
class GatewayHealthExportRuntime:
    ...
    def shutdown(self) -> None:
        ...
        if closeables:
            worker = threading.Thread(target=_close, name="zeus-gateway-health-export-shutdown", daemon=True)
            worker.start()
...
def _read_runtime_snapshot(config: Dict[str, Any]):
    ...
    try:
        ...
        for name, read in (
            ("zeus.gateway.background_work", _read_background_work_count),
            ("zeus.gateway.background_delegations", _read_background_delegations_count),
        ):
...
def _start_metric_provider(config: Dict[str, Any], sdk: Dict[str, Any]) -> Any:
    ...
    meter = provider.get_meter("zeus.gateway.health")
...
class GatewayDiagnosticLogStreamer(EmitterStreamer):
    ...
    def __init__(self, config: Dict[str, Any], sdk: Dict[str, Any]):
        ...
        self._logger = self._provider.get_logger("zeus.gateway.diagnostics")
        self._sdk = sdk
        self.exported = 0
...
def start_gateway_health_export(config: Dict[str, Any]) -> GatewayHealthExportRuntime:
    ...
    if metrics_on or diagnostics_on:
        try:
            ...
            logger.warning("monitoring.gateway_health_export.enabled but OTLP SDK is unavailable; install 'zeus-agent[otlp]'", exc_info=True)
    ...
    if diagnostics_on:
        try:
            ...
            thread = threading.Thread(target=_run, name="zeus-gateway-health-export", daemon=True)
            thread.start()
