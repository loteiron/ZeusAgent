...
Every ZeusAgent request to the Nous Portal (main loop, auxiliary client, fallback
paths) must carry the same product-attribution tags, sent in OpenAI-compatible
``extra_body['tags']``: ``["product=zeus-agent", "client=zeus-client-v<__version__>"]``.
The version is read live from ``zeus_cli.__version__`` — do NOT pre-compute it
...
_affinity_scope: ContextVar[Optional[str]] = ContextVar("zeus_affinity_scope", default=None)
...
def zeus_client_tag() -> str:
    """``client=zeus-client-v<MAJOR>.<MINOR>.<PATCH>`` ("unknown" if zeus_cli is unimportable)."""
    try:
        from zeus_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"client=zeus-client-v{__version__}"
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    """
    tags = ["product=zeus-agent", zeus_client_tag()]
