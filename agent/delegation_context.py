...
A ZeusAgent process may itself be a Kanban dispatcher worker with ZEUS_KANBAN_* in
...
_DELEGATED_CHILD_CONTEXT: ContextVar[bool] = ContextVar("zeus_delegated_child_context", default=False)
...
_NON_DISPATCHER_OWNED_CONTEXT: ContextVar[bool] = ContextVar("zeus_non_dispatcher_owned_context", default=False)

DELEGATED_CHILD_ENV_MARKER = "ZEUS_DELEGATED_CHILD_CONTEXT"

KANBAN_ENV_KEYS: tuple[str, ...] = (
    "ZEUS_KANBAN_TASK", "ZEUS_KANBAN_RUN_ID", "ZEUS_KANBAN_CLAIM_LOCK",
    "ZEUS_KANBAN_GOAL_MODE", "ZEUS_KANBAN_GOAL_MAX_TURNS",
)
...
def is_dispatcher_owned_worker_context() -> bool:
    """The single predicate every ``ZEUS_KANBAN_*`` identity gate should use."""
...
def delegated_child_subprocess_env(
    ...
    """
    if not (is_delegated_child_process_context() or os.environ.get("ZEUS_KANBAN_TASK")
            or (env and (env.get("ZEUS_KANBAN_TASK") or env.get(DELEGATED_CHILD_ENV_MARKER)))):
