...
``zeus pause`` writes a sentinel at ``$ZEUS_HOME/ESTOP``; ``zeus resume``
...
(fail safe, e.g. ``touch ~/.zeus/ESTOP``). Ported from gastownhall/gastown estop.go (MIT).
"""
...
# Same profile-aware / fleet-root resolvers the file-safety guards use (fail-open to ~/.zeus).
from agent.file_safety import _zeus_home_path as _zeus_home, _zeus_root_path as _canonical_root
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel this process would write on `zeus pause`."""
    return _zeus_home() / SENTINEL_NAME


def _candidate_sentinel_paths() -> list:
    """Profile home first, then the fleet root if it is a different directory: a profile
    gateway (ZEUS_HOME=~/.zeus/profiles/<n>) must still honor an operator's ~/.zeus/ESTOP."""
...
def paused_reply() -> Optional[str]:
    ...
    return f"⏸️ ZeusAgent is paused{tag}. New work is on hold; run `zeus resume` to pick things back up."
...
def check_paused(component: str, logger: logging.Logger) -> bool:
    ...
    if first:
        ...
        logger.info(
            "%s dispatch paused by global emergency stop%s — remove with `zeus resume` (%s)",
