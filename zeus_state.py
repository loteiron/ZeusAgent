#!/usr/bin/env python3
"""SQLite state store for ZeusAgent: session metadata, message history, model
...
from zeus_state_common import escape_like as _escape_like, stat_db_file_identity as _stat_db_file_identity
from zeus_state_errors import (
...
from zeus_state_readpool import _READ_POOL_MAX, _proc_fd_targets, _read_budget_for
from zeus_state_sessions import SessionSessionsMixin
from zeus_state_fts import SessionFtsSetupMixin, load_fts5_cjk_extension
from zeus_state_portability import SessionPortabilityMixin
from zeus_state_telegram import SessionTelegramTopicsMixin
from zeus_state_schema import SessionSchemaMixin
import zeus_state_holders as _state_holders
from zeus_state_dbfile import (
...
from zeus_state_messages import SessionMessagesMixin
from zeus_state_wal import _WAL_INCOMPAT_MARKERS, apply_database_pragmas, apply_wal_with_fallback
from zeus_state_repair import _claim_repair_attempt, preflight_db_writability, repair_state_db_schema
from zeus_state_titles import SessionTitlesMixin
from zeus_state_usage import SessionUsageMixin
from zeus_state_maintenance import SessionMaintenanceMixin
from zeus_state_gateway import SessionGatewayMixin
from zeus_state_compression import SessionCompressionMixin
from zeus_state_search import SessionSearchMixin
...
def _configured_transcript_limit(key: str, fallback: int = _MAX_SAFE_MESSAGES) -> int:
    ...
    try:
        from zeus_cli.config import load_config_readonly
...
DEFAULT_DB_PATH = _IMPORT_DEFAULT_DB_PATH = get_zeus_home() / "state.db"
...
def _default_db_path() -> Path:
    ...
    ``get_zeus_home()`` is resolved fresh (a runtime ZEUS_HOME redirect works regardless of import)."""
    return DEFAULT_DB_PATH if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH else get_zeus_home() / "state.db"


# Live-DB guard knobs live HERE (not in zeus_state_guard): the hermetic conftest monkeypatches
# ``zeus_state._STATE_DB_GUARD_BYPASS`` (``@pytest.mark.live_system_guard_bypass`` escape hatch)
# and ``_EXTRA_DENY_ROOTS`` (the pre-sandbox root, so custom-ZEUS_HOME deployments are covered).
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    ``ZEUS_HOME`` together, which is precisely the state in which it writes to production (#82770).
    """
    ...
    for root in roots:
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real ZeusAgent root {root}). "
                "Tests must run against a temporary ZEUS_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "ZEUS_HOME. If this test genuinely needs the live database, mark it with "
...
def divert_session_transcript_jsonl(session_id: str, messages) -> "Optional[Path]":
    """Append pending messages to ZEUS_HOME/sessions/<id>.jsonl (state.db was replaced under a
    live process). Returns the path, or None if nothing to write."""
    sid = str(session_id or "").strip()
    if not sid or not messages:
        return None
    sessions_dir = get_zeus_home() / "sessions"
...
# connection per resolved path via zeus_state_registry.acquire(); one-shots use SessionDB() + close().
...
# ── Process-wide shared SessionDB registry (#90837) ── lives in zeus_state_registry.py (acquire /
# release / close_all / release_or_close). Long-lived in-process callers (gateway, tui_gateway, cron,
# in-process tools) share ONE writer connection per resolved path via zeus_state_registry.acquire(); CLI
...
class SessionDB(
    ...
    # SQLite's deterministic busy handler convoys under many zeus processes: keep its
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        # Opened via zeus_state_registry.acquire(): close() releases a refcount instead.
        # Set True when this instance is opened via zeus_state_registry.acquire(). Makes close() a no-op so the
    ...
    def _open_writer(self) -> None:
        ...
        try:
            ...
            if not repair_state_db_schema(self.db_path).get("repaired"):
                raise
            self._connect_and_init_with_lock_patience()
        # FTS optimization is OPT-IN (`zeus db optimize`); no background worker races session lifecycle.
    ...
    def _handle_quarantine_if_zeroed(self, already_locked: bool = False) -> None:
        ...
        msg = (
            ...
            f"Restore from {self.db_path.parent / 'state-snapshots'} via `zeus snapshot list` / "
            f"`zeus snapshot restore <id>` if available. "
            ...
            try:
                ...
                if isinstance(exc, sqlite3.OperationalError):
                    if "locked" in err_msg or "busy" in err_msg:
                        ...
                        raise sqlite3.OperationalError(
                            f"database is locked (another ZeusAgent process held the "
    ...
    def _halt_db_corrupt(self, exc: BaseException) -> None:
        ...
        logger.error(
            ...
            "gateway and run `zeus sessions recover --source %s --inspect-only`.", self.db_path, exc,
            self.db_path,
        )
    ...
    def _foreign_state_db_holders(self) -> List[Tuple[int, str]]:
        """Foreign processes holding this DB or its WAL sidecars (see zeus_state_holders)."""
    ...
    def close(self):
        ...
        When this instance is shared (opened via ``zeus_state_registry.acquire``), ``close()`` RELEASES one
        ...
        if self._shared_registry_owned:
            from zeus_state_registry import release
            release(self)
            return
            ...
            if self._conn:
                if self._db_corrupt:  # quarantined: no checkpoint over a damaged image
                    logger.warning(
                        ...
                        "then run `zeus sessions recover --source %s --inspect-only`.", self.db_path,
...
_PLUGIN_COMPAT_LAZY = {
    'AUTO_VACUUM_MIN_FREELIST_RATIO': ('zeus_state_common', 'AUTO_VACUUM_MIN_FREELIST_RATIO'),
    'ActivityProvenance': ('agent.session_activity', 'ActivityProvenance'),
    'CompressionSessionBusyError': ('zeus_state_errors', 'CompressionSessionBusyError'),
    'CompressionSessionClosedError': ('zeus_state_errors', 'CompressionSessionClosedError'),
    'DEFERRED_INDEX_SQL': ('zeus_state_common', 'DEFERRED_INDEX_SQL'),
    'FTS_CJK_STALE_KEY': ('zeus_state_common', 'FTS_CJK_STALE_KEY'),
    'FTS_CJK_TABLE_SQL': ('zeus_state_fts', 'FTS_CJK_TABLE_SQL'),
    'FTS_CJK_TRIGGER_SQL': ('zeus_state_fts', 'FTS_CJK_TRIGGER_SQL'),
    'FTS_REBUILD_DEFERRAL_KEY': ('zeus_state_common', 'FTS_REBUILD_DEFERRAL_KEY'),
    'FTS_SQL': ('zeus_state_common', 'FTS_SQL'),
    'FTS_STALE_KEY': ('zeus_state_common', 'FTS_STALE_KEY'),
    'FTS_STORAGE_VERSION': ('zeus_state_common', 'FTS_STORAGE_VERSION'),
    'FTS_TRIGRAM_SQL': ('zeus_state_common', 'FTS_TRIGRAM_SQL'),
    'LEGACY_FTS_SQL': ('zeus_state_common', 'LEGACY_FTS_SQL'),
    'LEGACY_FTS_TRIGRAM_SQL': ('zeus_state_common', 'LEGACY_FTS_TRIGRAM_SQL'),
    'MAX_FTS5_QUERY_CHARS': ('zeus_state_common', 'MAX_FTS5_QUERY_CHARS'),
    'PERSISTENCE_ERROR_CAUSES': ('zeus_state_errors', 'PERSISTENCE_ERROR_CAUSES'),
    'SCHEMA_SQL': ('zeus_state_common', 'SCHEMA_SQL'),
    'SCHEMA_VERSION': ('zeus_state_common', 'SCHEMA_VERSION'),
    'SESSION_STATUS_COMPLETE': ('zeus_state_sessions', 'SESSION_STATUS_COMPLETE'),
    'SESSION_STATUS_EMPTY': ('zeus_state_sessions', 'SESSION_STATUS_EMPTY'),
    'SESSION_STATUS_ERROR': ('zeus_state_sessions', 'SESSION_STATUS_ERROR'),
    'SESSION_STATUS_INTERRUPTED': ('zeus_state_sessions', 'SESSION_STATUS_INTERRUPTED'),
    ...
    'SessionTurnLeaseLostError': ('zeus_state_errors', 'SessionTurnLeaseLostError'),
    'WalUnsupportedError': ('zeus_state_wal', 'WalUnsupportedError'),
    'apply_durability_barriers': ('zeus_state_repair', 'apply_durability_barriers'),
    'classify_session_status': ('zeus_state_sessions', 'classify_session_status'),
    'collect_state_db_stats': ('zeus_state_dbfile', 'collect_state_db_stats'),
    'count_db_holders': ('zeus_state_dbfile', 'count_db_holders'),
    'describe_skill_invocation': ('agent.skill_commands', 'describe_skill_invocation'),
    'fts5_cjk_so_path': ('zeus_state_fts', 'fts5_cjk_so_path'),
    'is_advisory_lock_contention': ('zeus_state_common', 'is_advisory_lock_contention'),
    'is_automatic_end_reason': ('zeus_state_common', 'is_automatic_end_reason'),
    'is_disk_full_error': ('zeus_state_errors', 'is_disk_full_error'),
    'is_sqlite_wal_reset_vulnerable': ('zeus_state_wal', 'is_sqlite_wal_reset_vulnerable'),
    'is_transient_sqlite_error': ('zeus_state_errors', 'is_transient_sqlite_error'),
    'iter_deleted_sqlite_sidecar_holders': ('zeus_state_dbfile', 'iter_deleted_sqlite_sidecar_holders'),
    'release_or_close': ('zeus_state_registry', 'release_or_close'),
    'report_startup_progress': ('zeus_startup_watchdog', 'report_startup_progress'),
    'resolve_journal_mode': ('zeus_state_wal', 'resolve_journal_mode'),
    'resolve_synchronous_level': ('zeus_state_wal', 'resolve_synchronous_level'),
    'sanitize_context': ('agent.memory_manager', 'sanitize_context'),
    'sqlite_source_id': ('zeus_state_wal', 'sqlite_source_id'),
    'workspace_key': ('zeus_state_sessions', 'workspace_key'),
}
