# ZeusAgent - Development Guide

Instructions for AI coding assistants and developers working on the zeus-agent codebase.
...
- Install the full Python test extras with `python scripts/setup_zeus.py --test-all`.
  Active CI disables automatic test retries; record environment restrictions explicitly.
- Use `scripts/github_source.py` for public source snapshots. Keep private handoff
  evidence, conversations and live state out of Git. A heuristic scan is not a proof
  that all credentials are absent.

## What ZeusAgent Is

ZeusAgent is a personal AI agent that runs the same agent core across a CLI, a messaging
...
Read the balance right: ZeusAgent ships a **lot**. Most merges are bug fixes to reported
...
- **Expand reach at the edges.** New adapters, channels, providers, models, desktop/TUI/
  ...
  the existing setup/config UX (`zeus tools`, `zeus setup`, auto-install) rather than
...
- **E2E validation, not just green unit mocks.** Anything touching resolution chains, config
  ...
  real path with real imports against a temp `ZEUS_HOME`. Mocks hide integration bugs.
...
- **New `ZEUS_*` env vars for non-secret config.** `.env` is for secrets only. Behavioral
...
- **Outbound telemetry / usage attribution without opt-in gating.** No analytics,
  ...
  (config gate + setup prompt + `zeus tools` toggle) exists. Park behind a label.
...
- **Third-party products integrated into the core tree.** Observability backends, vendor
  ...
  backend we don't own. Ship as a **standalone plugin repo** (`~/.zeus/plugins/` or pip
...
2. **CLI command + skill** — config/state/infra expressible as shell commands; the agent runs
   `zeus <subcommand>` guided by a skill. Default for subscriptions, scheduled tasks,
   service setup (`zeus webhook`, `zeus cron`, `zeus tools`).
...
4. **Plugin** — third-party/niche/user-specific; lives in `~/.zeus/plugins/` or a pip
...
drive a locally spawned backend, one over SSH, one behind URL + token, or ZeusAgent Cloud, and
only the first two carry `ZEUS_DESKTOP=1`. An env-keyed gate is a silent no-op on the other
topologies — the tool is stripped from the schema while the platform hint tells the model it
is "inside the ZeusAgent desktop app". The pattern:

- **The toolset is the surface gate.** Keep such tools off `_ZEUS_CORE_TOOLS` and in a named
...
- **Ask which identity you mean.** `ZEUS_DESKTOP=1` legitimately means "this backend was
  spawned by the app" (cron ticker, web-dist handling). It does NOT mean "a GUI is watching";
  the embedded terminal pane (`zeus --tui` against that backend) is the counterexample.
...
```
`scripts/run_tests.sh` probes `.venv`, then `venv`, then `$HOME/.zeus/zeus-agent/venv`
...
├── toolsets.py           # TOOLSETS dict, _ZEUS_CORE_TOOLS
├── cli.py                # ZeusAgentCLI (REPL, slash dispatch) + zeus_cli/cli_*_mixin.py
├── zeus_state.py       # SessionDB facade; zeus_state_*.py siblings
├── zeus_constants.py   # get_zeus_home(), display_zeus_home() — profile-aware paths
├── zeus_logging.py     # agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # turn_*.py loop phases, providers, memory, compression, prompt builder
├── zeus_cli/           # CLI subcommands, setup, config, plugins loader, skins, updater
...
├── ui-tui/               # Ink (React) terminal UI — `zeus --tui`
...
**User state:** `~/.zeus/config.yaml` (settings), `~/.zeus/.env` (secrets only),
`~/.zeus/logs/` (`agent.log` INFO+, `errors.log` WARNING+, `gateway.log`); all
profile-aware via `get_zeus_home()`. Browse logs with `zeus logs [--follow] [--level] [--session]`.
...
families: `zeus_state.py` (21), `gateway/run.py` (15), `tools/mcp_tool.py` (15),
`zeus_cli/kanban.py` (14), `zeus_cli/web_server.py` (13 + 24 routers), `zeus_cli/auth.py`
(12), `tools/browser_tool.py` (11), `cli.py` (12 `zeus_cli/cli_*_mixin.py`), `run_agent.py`
...
- **Compat pointers are OFF LIMITS in-tree.** Old import paths kept alive for external plugins
  ...
  `-W error::zeus_cli.plugin_compat.ZeusAgentPluginCompatWarning` catches them in the suite.
...
- **Never infer process identity from argv substrings** (`"serve" in cmdline`) — the bug class
  ...
  `zeus_cli.update_cmd._zeus_holder_subcommand`; flag sets are DERIVED from the parser
  (`_holder_value_flags()`), never hand-written; match FULL cmdlines and truncate only for
  display. Details: `zeus_cli/AGENTS.md`.
- **Never hardcode `~/.zeus`.** `get_zeus_home()` for code paths, `display_zeus_home()`
  for user-facing text (both from `zeus_constants`). Hardcoding breaks profiles (5 bugs in
  PR #3575). Module-level constants are fine — they cache after `_apply_profile_override()`
  sets `ZEUS_HOME`. Profile operations themselves are HOME-anchored
  (`_get_profiles_root()` = `Path.home()/.zeus/profiles`) so `zeus -p x profile list`
  sees all profiles — intentional, not a bug.
- **Argparse alias dispatch:** `add_parser("list", aliases=["ls"])` sets `dest` to the literal
  the user typed (`"ls"`). Dispatch must accept both (caught PTY-testing `zeus webhook ls`).
- **Don't wire in dead code without E2E validation.** Unshipped code was dead for a reason;
  E2E the real resolution chain with real imports against a temp `ZEUS_HOME` first.
...
- **Stale branches can silently revert recent fixes.** Preserve local edits and refresh
  the branch using the repository's reviewed merge/rebase process. Never reset a dirty
  checkout or rewrite shared history without explicit authorization. Verify the final
  diff; unexpected deletions are a red flag.
...
vars unset, `TZ=UTC`, `LANG=C.UTF-8`, `ZEUS_HOME` → temp dir, and per-file subprocess
...
- **Inherited runner option:** a failing FILE can be retried in a fresh subprocess (`--file-retries`;
  `ZEUS_TEST_FILE_RETRIES=0` disables). Pass-on-retry is green but printed under `⚠ FLAKY`
...
- **Placement:** JavaScript behavior tests belong in the relevant vitest workspace.
  The inherited change classifier is archived with upstream CI; the active fork CI
  always declares its Python, platform and JavaScript lanes.
- **Tests must not write to `~/.zeus/`.** The autouse `_isolate_zeus_home` fixture in
  `tests/conftest.py` redirects `ZEUS_HOME`; never hardcode `~/.zeus/` in tests. Profile
  tests also mock `Path.home()` so `_get_profiles_root()` / `_get_default_zeus_home()` stay
  in the temp dir (pattern: `tests/zeus_cli/test_profiles.py`):
  ```python
  @pytest.fixture
  def profile_env(tmp_path, monkeypatch):
      home = tmp_path / ".zeus"; home.mkdir()
      monkeypatch.setattr(Path, "home", lambda: tmp_path)
      monkeypatch.setenv("ZEUS_HOME", str(home))
      return home
  ```
  Tests that `patch.object(Path, "home", ...)` must ALSO set `ZEUS_HOME` — code reads the
  env var, not `Path.home()/.zeus`.
...
`tests/zeus_cli/test_venv_holder_windows_live.py` on a real `windows-latest` runner (real
...
| `cli.py`, `zeus_cli/`, `main.py` | `zeus_cli/AGENTS.md` | CLI mixins, `_SLASH_DISPATCH`, slash registry, config system + loaders, skins, `zeus update` pipeline, profiles / multiplex |
...
| `plugins/`, `zeus_cli/plugins*.py` | `plugins/AGENTS.md` | Plugin kinds, native compat contract, in-tree policy, Sep-2026 compat window |
| `tui_gateway/`, `ui-tui/` | `tui_gateway/AGENTS.md` | Process model, JSON-RPC transport, key surfaces, slash flow, dev commands |
| `web/`, `zeus_cli/web_routers/` | `web/AGENTS.md` | Dashboard embeds the real TUI; what React may and may not rebuild |
...
| `cron/`, kanban (`zeus_cli/kanban*.py`, `tools/kanban_tools.py`, `plugins/kanban/`) | `cron/AGENTS.md` | Scheduler invariants, job fields, kanban board/dispatcher |
