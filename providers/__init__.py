...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with zeus-agent)
2. User plugins: ``$ZEUS_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``zeus_agent.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$ZEUS_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$ZEUS_HOME/plugins/model-providers/`` if it exists."""
    try:
        from zeus_constants import get_zeus_home

        d = get_zeus_home() / "plugins" / "model-providers"
...
def _installed_plugins_dir() -> Path | None:
    """Return ``$ZEUS_HOME/plugins/`` if it exists.

    This is where ``zeus plugins install`` clones a plugin — flat, one
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple ZEUS_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_zeus_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``zeus_agent.plugins``
    ...
    * **Provider targets only.** The ``zeus_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$ZEUS_HOME``) keep their
    ...
    try:
        from zeus_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$ZEUS_HOME/plugins/model-providers/<name>/``
      2b. Plugins installed by ``zeus plugins install`` at
          ``$ZEUS_HOME/plugins/<name>/`` that declare ``kind: model-provider``
    ...
    # 0. Pip-installed plugins — entry points in the ``zeus_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$ZEUS_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with zeus-agent.
    ...
    # 2. User plugins — under $ZEUS_HOME/plugins/model-providers/<name>/.
    ...
    # 2b. Plugins installed by ``zeus plugins install`` / the plugin index.
    #     Those clone into $ZEUS_HOME/plugins/<name>/ — flat, NOT under
