...
Run from the repo with a clean environment and a temporary ZEUS_HOME:
...
from zeus_cli import heartbeat  # noqa: E402
    ...
    with patch.object(heartbeat, "time", SimpleNamespace(time=lambda: clock.now)):
        ...
        if not base_poller:
            ...
            runner.session_store = SessionStore(
                sessions_dir=Path(os.environ["ZEUS_HOME"]) / "sessions", config=runner.config)
