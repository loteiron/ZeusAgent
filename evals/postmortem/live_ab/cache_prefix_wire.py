...
message prefix between call N and N+1. If ZeusAgent strips prior-turn thinking, call N+1's messages[:k]
...
#   python cache_prefix_wire.py <repo_root> <A|B> [--zeus-home DIR]   (default ZEUS_HOME: the real one, for credentials)
sys.path.insert(0, sys.argv[1])
os.environ.setdefault("ZEUS_HOME", os.path.expanduser("~/.zeus"))
if "--zeus-home" in sys.argv:
    os.environ["ZEUS_HOME"] = sys.argv[sys.argv.index("--zeus-home") + 1]
...
time.sleep(1)
log = subprocess.run(f"grep -h '\\[{sid}\\]' ~/.zeus/logs/agent.log | grep 'API call #'", shell=True, capture_output=True).stdout.decode("utf-8", "replace")
