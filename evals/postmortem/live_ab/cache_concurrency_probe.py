...
Runs N independent ZeusAgent ``AIAgent`` sessions concurrently, each the same ~8-step tool loop whose
...
Results that motivated zeus-agent #104284 / #104421 and NousResearch/api#227 (2026-09-05/06,
...
Usage:
  ...
  providers: nous (Portal creds from ZEUS_HOME), openrouter (OPENROUTER_API_KEY or --api-key),
             anthropic (ANTHROPIC_API_KEY or --api-key)
Cost: ~$50 per 20x6 arm on Fable 5.1 at the 5m tier. Every run starts from the real ZeusAgent request
...
def _parse():
    ...
    ap.add_argument("--repo", required=True, help="zeus-agent checkout to import from")
    ...
    ap.add_argument("--wire", choices=["chat", "native"], default=None,
                    help="force the wire (default: what ZeusAgent would pick for the provider/model)")
...
os.environ.setdefault("ZEUS_HOME", os.path.expanduser("~/.zeus"))
import anthropic
...
def _request_record(model, system, tools, messages):
    first = messages[0] if messages else {}
    content = first.get("content")
    text = content if isinstance(content, str) else "".join(
        block.get("text", "") for block in (content or []) if isinstance(block, dict))
    worker_match = re.search(r"\[probe-session (\d+)\]", text)

    def sha(value):
        return hashlib.sha256(json.dumps(value, sort_keys=True, default=str).encode()).hexdigest()[:10]

    return dict(worker=int(worker_match.group(1)) if worker_match else None,
                call=len(messages), t_start=time.time(), model=model, n_msgs=len(messages),
                system_sha=sha(system), tools_sha=sha(tools), msg_shas=[sha(m) for m in messages])


def patched_stream(self, **kw):
    msgs = kw.get("messages") or []
    rec = _request_record(kw.get("model"), kw.get("system"), kw.get("tools"), msgs)
...
def patched_create(self, *a, **kw):
    ...
    rec = _request_record(kw.get("model"), sysm, kw.get("tools"), nonsys)
    rec["extra_body"] = kw.get("extra_body")
...
def creds():
    ...
    from zeus_cli.auth_nous import resolve_nous_runtime_credentials
