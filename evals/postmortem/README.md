...
- **`forensics/`** reads a *copy* of a ZeusAgent `state.db` (plus rotated `agent.log*` and git) and
...
- A ZeusAgent checkout with its venv (`.venv/bin/python`). NeMo Relay is not required for the forensics;
  it is what the run itself used for the wire captures in `live_ab/cache_prefix_wire.py`.
- For forensics: a **copy** of `~/.zeus/state.db` (never point at the live file; `sqlite3 state.db ".backup copy.db"`
  or `cp` while ZeusAgent is idle) and, optionally, the rotated `~/.zeus/logs/agent.log*`.
- For `--live` probes: real credentials in `ZEUS_HOME` (they spend cents per run).
...
$P -m evals.postmortem.forensics.logcalls   --db state_copy.db --logs "$HOME/.zeus/logs/agent.log*"
...
ZeusAgent recorded (an estimator, not an invoice).
...
```bash
# offline probes (fake providers, temp ZEUS_HOME), main vs a branch or integration checkout:
...
Forensic lanes: five parallel ZeusAgent subagents (2026-09-04), rewritten here to take `--db`/`--root`
