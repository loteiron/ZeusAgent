...
def run_probe(repo, receipt):
    ...
    owner = Path(os.environ["ZEUS_HOME"])
    ...
    with serving(Provider()) as provider, serving(CallbackServer(
        ...
        reloaded = asyncio.run(ZeusAgentTokenStorage("positive", zeus_home=owner).get_tokens())
        ...
        override = set_zeus_home_override(other)
        try:
...
def main():
    ...
    if args.cold_probe:
        sys.path.insert(0, str(repo))
        logging.disable(logging.CRITICAL)
        from zeus_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    if not args.isolated_worker:
        with tempfile.TemporaryDirectory(prefix="zeus-oauth-http-") as temp:
            home = Path(temp)
            (home / ".zeus").mkdir()
            env = {"PATH": os.environ.get("PATH", ""), "HOME": temp,
                   "ZEUS_HOME": str(home / ".zeus"), "LANG": "C.UTF-8", "TZ": "UTC",
                   "PYTHONNOUSERSITE": "1"}
