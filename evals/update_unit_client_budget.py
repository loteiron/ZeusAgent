...
os.environ.clear()
os.environ.update(allowed, HOME=str(home), ZEUS_HOME=str(home / "zeus"))
...
def scoped_systemctl(argv, *, timeout):
    if "list-units" in argv:
        ...
        argv = [arg for arg in argv if arg not in ("zeus-gateway*", "zeus-serve*")] + [unit + ".service"]
...
unit = f"zeus-serve-audit-089c35aa-{tag}"
cmd = ["systemctl", "--user"]
def run(args):
