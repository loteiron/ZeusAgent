...
Each probe runs in a FRESH subprocess with an isolated ZEUS_HOME so nothing is cached across reps.
...
ENV = {**os.environ, "ZEUS_HOME": HOME, "PYTHONPATH": TREE, "PYTHONDONTWRITEBYTECODE": "0", "OPENROUTER_API_KEY": "sk-bench-placeholder",
       "ZEUS_SKIP_UPDATE_CHECK": "1", "NO_COLOR": "1", "TERM": "dumb", "COLUMNS": "120"}
for k in list(ENV):
    if k.startswith(("ZEUS_SESSION", "ZEUS_PROFILE")): ENV.pop(k)
...
IMPORT_TARGETS = ["run_agent", "cli", "zeus_cli.main", "gateway.run", "tools.registry", "zeus_state", "tui_gateway.server", "zeus_cli.web_server", "model_tools", "agent.prompt_builder"]
probe = r'''
...
tree=os.getcwd(); foreign=[m for m,mod in list(sys.modules.items()) if getattr(mod,"__file__",None) and "zeus-agent" in mod.__file__ and not mod.__file__.startswith(tree) and "site-packages" not in mod.__file__]
...
# 2. CLI end-to-end startup: `zeus --version`, `zeus --help`, `zeus doctor --help`, `zeus config get model` (no network)
CLI = {"version": ["zeus_cli/main.py", "--version"], "help": ["zeus_cli/main.py", "--help"], "config_get": ["zeus_cli/main.py", "config", "get", "model"], "tools_list": ["zeus_cli/main.py", "tools", "--help"], "skills_help": ["zeus_cli/main.py", "skills", "--help"]}
cli = {}
...
try:
    ...
    T("resolve_toolset_zeus_default_ms", lambda: resolve_toolset("zeus-default"), 200)
...
try:
    import zeus_state, tempfile, uuid
    from pathlib import Path
    db=zeus_state.SessionDB(Path(tempfile.mkdtemp())/"s.db") if hasattr(zeus_state,"SessionDB") else None
    if db:
        sid=str(uuid.uuid4())
