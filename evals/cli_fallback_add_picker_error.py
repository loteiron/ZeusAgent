"""Drive the real ``zeus fallback add`` under a Linux PTY into an ordinary picker error.
...
def _persisted_model(root: Path, env: dict) -> dict:
    ...
    out = subprocess.run(
        [sys.executable, "-c", "import json; from zeus_cli.config import load_config; "
...
def run(root: Path, output: Path) -> dict:
    with tempfile.TemporaryDirectory(prefix="zeus_test_fallback_") as home:
        hh = Path(home) / ".zeus"
        hh.mkdir()
        ...
        env = {"PATH": os.environ["PATH"], "HOME": home, "ZEUS_HOME": str(hh),
        ...
        proc = subprocess.Popen([sys.executable, "-m", "zeus_cli.main", "fallback", "add"],
