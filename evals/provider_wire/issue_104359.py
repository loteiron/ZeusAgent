...
os.chdir(home)
Path(os.environ["ZEUS_HOME"]).mkdir()
...
Path(os.environ["ZEUS_HOME"], "config.yaml").write_text(yaml.safe_dump(config), encoding="utf-8")
...
db = SessionDB(Path(os.environ["ZEUS_HOME"]) / "state.db")
