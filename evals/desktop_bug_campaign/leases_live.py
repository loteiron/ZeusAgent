...
def backend(port):
    from zeus_cli.web_server import app, start_server
...
''')
    ...
    (home / 'user').mkdir()
    env.update(ZEUS_HOME=str(home), ZEUS_DASHBOARD_SESSION_TOKEN='lease-probe',
    ...
    def control(port, action, sid):
        req = urllib.request.Request(f'http://127.0.0.1:{port}/lease-probe/{action}/{sid}', data=b'', method='POST', headers={'X-Zeus-Session-Token': 'lease-probe'})
