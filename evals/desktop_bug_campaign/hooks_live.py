...
env = {k: v for k, v in os.environ.items() if not (k.startswith('ZEUS_') or k.endswith(('_API_KEY', '_TOKEN')))}
env.update(HOME=str(out / 'os-home'), ZEUS_HOME=str(home), ZEUS_DASHBOARD_SESSION_TOKEN='hooks-fixture-token', ZEUS_IGNORE_RULES='1', PYTHONPATH=str(a.repo))
cmd = [sys.executable, '-m', 'zeus_cli.main', 'serve', '--host', '127.0.0.1', '--port', str(a.port), '--skip-build']
