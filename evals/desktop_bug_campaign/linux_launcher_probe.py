...
def main():
    ...
    env = {"PATH": "/usr/bin:/bin", "HOME": str(root / "home"),
           "ZEUS_HOME": str(root / "home/.zeus"),
           "XDG_DATA_HOME": str(root / "xdg"), "LANG": "C.UTF-8",
           "ZEUS_NONINTERACTIVE": "1"}
    Path(env["ZEUS_HOME"]).mkdir(parents=True)
    venv_dir = root / "venv"
    ...
    install = (
        ...
        "from zeus_cli.linux_desktop_entry import install_desktop_entry; "
        f"sys.argv[0]={str(repo / 'zeus')!r}; "
    ...
    rows["venv_import"] = run([str(python), "-I", "-c", "import yaml,zeus_cli.main; print(yaml.__version__)"], env, "/")
    rows["base_import_negative"] = run([str(python.resolve()), "-I", "-c", "import yaml,zeus_cli.main"], env, "/")
    ...
    custom = original.replace(b"Name=ZeusAgent\n", b"Name=ZeusAgent custom\n").replace(b"Terminal=false", b"Terminal=true")
    entry.write_bytes(custom)
    config = Path(env["ZEUS_HOME"]) / "config.yaml"
