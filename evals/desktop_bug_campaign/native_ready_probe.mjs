...
const home = mkdtempSync(join(tmpdir(), 'zeus-native-ready-'))
const token = randomUUID()
const env = {
  PATH: process.env.PATH, HOME: home, ZEUS_HOME: home,
  LANG: 'C.UTF-8', PYTHONUNBUFFERED: '1', ZEUS_NONINTERACTIVE: '1',
  ZEUS_DASHBOARD_SESSION_TOKEN: token, ZEUS_SERVE_HEADLESS: '1',
  ZEUS_PARENT_PID: String(process.pid),
}
const child = spawn(process.argv[2] || join(repo, '.venv/bin/python'),
  ['-m', 'zeus_cli.main', 'serve', '--host', '127.0.0.1', '--port', '0', '--isolated'],
...
try {
  ...
  for (let i = 0; i < 20; i++) {
    for (const path of ['/api/status', '/api/profiles']) {
      const response = await fetch(`http://127.0.0.1:${port}${path}`, { headers: { 'X-Zeus-Session-Token': token } })
