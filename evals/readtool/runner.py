"""Run the read-tool eval through the REAL ZeusAgent AIAgent.

For each task: fresh temp ZEUS_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    zeus_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".zeus"
    ...
    try:
        ...
        shutil.rmtree(zeus_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.zeus/.env; set +a  — then relaunch."
        )
