...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ZEUS_HOME", os.path.expanduser("~/.zeus")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ZEUS_HOME", Path.home() / ".zeus")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# ZeusAgent config paths
# ═══════════════════════════════════════════════════════════════════

ZEUS_HOME = Path(os.getenv("ZEUS_HOME", Path.home() / ".zeus"))
CONFIG_PATH = ZEUS_HOME / "config.yaml"
PREFILL_PATH = ZEUS_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/ZeusAgent models — already uncensored, just needs clean prompt
    "zeus": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "zeus" in model_lower or "nous" in model_lower:
        return "zeus"
...
def _get_current_model() -> tuple:
    """Read current model and provider from ZeusAgent config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.zeus/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart ZeusAgent for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart ZeusAgent for changes to take effect.")
