...
All per-subject data lives under PDD_DATA_DIR (default: $ZEUS_HOME/unbroker),
which is the same trust boundary ZeusAgent uses for .env and OAuth tokens.
"""
...
def zeus_home() -> Path:
    return Path(os.environ.get("ZEUS_HOME") or (Path.home() / ".zeus"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else zeus_home() / "unbroker"
