...
The design is **ZeusAgent-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
zeus skills install official/security/unbroker
```

Then start a new ZeusAgent session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a ZeusAgent session:
...
- **PII handling.** Dossiers live under the ZeusAgent home directory (`0600`, optionally
