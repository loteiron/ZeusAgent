...
Usage (via cron with --no-agent):
    ...
    zeus cron create zeus-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$ZEUS_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name zeus-issues --repo NousResearch/hermes-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the ZeusAgent .env file
(``${ZEUS_HOME:-~/.zeus}/.env``) to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "ZeusAgent-Watcher/1.0",
    }
