...
Usage (via cron with --no-agent):
    ...
    zeus cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$ZEUS_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "ZeusAgent-Watcher/1.0"})
