...
is typically only in `~/.zeus/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike ZeusAgent,
...
`zeus-custom-providers` skill, pitfall 2). ZeusAgent hits this because it forwards
