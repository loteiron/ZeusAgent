...
Usage:
  ...
  <server_name> matches the files in $ZEUS_HOME/mcp-tokens/<server>.json etc.
...
def _zeus_home():
    # Prefer ZeusAgent' own resolver (profile-safe); fall back to env then ~/.zeus.
    try:
        from zeus_constants import get_zeus_home
        return str(get_zeus_home())
    except Exception:
        return os.environ.get("ZEUS_HOME") or os.path.expanduser("~/.zeus")


def _tokens_dir():
    return os.path.join(_zeus_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "zeus-diag", "version": "1.0"}},
        },
        headers={
