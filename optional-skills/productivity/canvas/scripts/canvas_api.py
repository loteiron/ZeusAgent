#!/usr/bin/env python3
"""Canvas LMS API CLI for ZeusAgent.
...
def _check_config():
    ...
    if missing:
        zeus_env = os.path.join(
            os.environ.get("ZEUS_HOME", os.path.expanduser("~/.zeus")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {zeus_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for ZeusAgent"
    )
