...
the actual `setup.sh` that creates the project workspace, configures ZeusAgent
...
2. **Create profiles** — `zeus profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.zeus/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `zeus kanban create` assigned to the director
...
```bash
zeus profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.zeus/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.zeus/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most ZeusAgent
...
Each profile gets a `SOUL.md` at `~/.zeus/profiles/<name>/SOUL.md` that
...
```bash
zeus kanban create "Direct production of <video title>" \
    --assignee director \
...
the ZeusAgent `.env` (`${ZEUS_HOME:-$HOME/.zeus}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _zeus_env="${ZEUS_HOME:-$HOME/.zeus}/.env"
    if grep -q "^${var}=" "$_zeus_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_zeus_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_zeus_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY zeus ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY zeus OPENROUTER_API_KEY || exit 1
# ...
```
