...
Maps each role archetype to the ZeusAgent skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public zeus-agent
...
## ZeusAgent skills relevant to video production

### Visual / rendering skills (`zeus-agent/skills/creative/`)
...
### Design / pre-production skills (`zeus-agent/skills/creative/`)
...
### Audio / media skills (`zeus-agent/skills/creative/` + `skills/media/`)
...
These are **not** ZeusAgent skills but external CLIs / APIs that profiles invoke.
...
## Built-in ZeusAgent tools for media review

These are native ZeusAgent tools — not invoked via terminal but through their own
...
key is present in `${ZEUS_HOME:-~/.zeus}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every ZeusAgent profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${ZEUS_HOME:-~/.zeus}/.env` → environment variable.
...
When a new ZeusAgent-public video skill ships:
