...
zeus kanban watch --tenant <project-slug>

# Snapshot of the board
zeus kanban list --tenant <project-slug>
zeus kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
zeus kanban stats --tenant <project-slug>

# Visual dashboard (browser)
zeus dashboard

# Inspect a specific task (includes comments + events)
zeus kanban show <task-id>

# Follow a single task's event stream
zeus kanban tail <task-id>
```

Verify available subcommands with `zeus kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `zeus kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `zeus kanban block <id>` then `zeus kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `zeus kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `zeus kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `zeus profile list` (profile exists?), `zeus status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(zeus kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
zeus kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
zeus kanban block <task-id>
zeus kanban archive <task-id>

# Diagnose what happened
zeus kanban show <task-id>      # task body, comments, recent events
zeus kanban tail <task-id>      # follow the live event stream
zeus kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `zeus kanban create` for the director
...
while true; do
    clear
    zeus kanban list --tenant <slug>
    echo "---"
    zeus kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `zeus kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `zeus profile list` and start
