#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `zeus kanban list` and
...
def zeus_available() -> bool:
    return shutil.which("zeus") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed zeus CLI."""
    try:
        out = subprocess.run(
            ["zeus", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `zeus kanban list`
    out = subprocess.run(
        ["zeus", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    rows = []
...
def main():
    ...
    if not zeus_available():
        print("ERROR: 'zeus' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
