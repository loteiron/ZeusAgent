...
### 1. Start order: editor first, ZeusAgent second

ZeusAgent probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW ZeusAgent session.
...
restart the editor, restart the ZeusAgent session.
...
server fails to bind (Output Log) or ZeusAgent' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.zeus/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for ZeusAgent

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. ZeusAgent' connection
lives in `~/.zeus/config.yaml` via `zeus mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms ZeusAgent — just don't
mistake it for the ZeusAgent setup step.
...
explicitly warns against overlapping calls. ZeusAgent executes same-turn tool
...
### 9. Timeouts: ZeusAgent gives up before Unreal does

ZeusAgent' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If ZeusAgent runs elsewhere (SSH backend,
