...
## 5. ZeusAgent Configuration

### Config location

`$ZEUS_HOME/config.yaml` (defaults to `~/.zeus/config.yaml` when `ZEUS_HOME` is unset)
...
Restart the ZeusAgent session for changes to take effect. The MCP connection is
