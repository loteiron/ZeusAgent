...
ZEUS_HOME_DIR="${ZEUS_HOME:-$HOME/.zeus}"
ZEUS_CFG="${ZEUS_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure ZeusAgent config has twozero_td MCP entry ──
if [[ ! -f "$ZEUS_CFG" ]]; then
    echo -e " ${FAIL} ZeusAgent config not found at ${ZEUS_CFG}"
    manual_steps+=("Create ${ZEUS_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ZEUS_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in ZeusAgent config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to ZeusAgent config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ZEUS_CFG} manually"); }
    manual_steps+=("Restart ZeusAgent session to pick up config change")
fi
