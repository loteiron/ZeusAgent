...
timeout. Config is env-only (``ZEUS_STARTUP_WATCHDOG=0``,
``ZEUS_STARTUP_WATCHDOG_TIMEOUT_S``) because config.yaml parsing is itself in scope.
...
ENV_STARTUP_WATCHDOG = "ZEUS_STARTUP_WATCHDOG"
ENV_STARTUP_WATCHDOG_TIMEOUT_S = "ZEUS_STARTUP_WATCHDOG_TIMEOUT_S"
...
# Module singleton: the arm sites (zeus_cli.main / zeus_cli.gateway /
...
def _process_zeus_home() -> Path:
    """ZEUS_HOME for diagnostic files — stdlib-only replica of the zeus_constants default."""
...
def get_startup_watchdog_dump_path(home: Optional[Path] = None) -> Path:
    """Return ``<ZEUS_HOME>/logs/gateway-startup-watchdog.log``."""
    base = home if home is not None else _process_zeus_home()
    return base.joinpath(*_DUMP_RELATIVE)


def startup_watchdog_disabled() -> bool:
    """True when ``ZEUS_STARTUP_WATCHDOG`` opts out explicitly."""
...
def arm_startup_watchdog(
    ...
    *,
    ...
    ``ZEUS_STARTUP_WATCHDOG=0`` or when the thread could not be started.
    """
    global _handle
    try:
