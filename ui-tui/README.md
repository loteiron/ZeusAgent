...
React + Ink terminal UI for ZeusAgent. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ZEUS_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `zeus setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.zeus/.zeus_history` or under `ZEUS_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `zeus setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/zeus-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external zeus setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.zeus/tui-parent.log
    ...
    types/
      zeus-ink.d.ts            type declarations for @zeus/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent ZeusAgentCLI subprocess for slash commands
```
