 ...
 * `zeus-boot-background` / `zeus-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports ZEUS_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.zeus matches get_zeus_home()'s default.
const bootFilePath = () => join(process.env.ZEUS_HOME ?? join(homedir(), '.zeus'), 'tui-theme-boot.json')

// Never touch the user's real ~/.zeus from test runs (the TS suite has no
// ZEUS_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into ZEUS_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.ZEUS_TUI_THEME || env.ZEUS_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.ZEUS_TUI_BACKGROUND
  ) {
    env.ZEUS_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.ZEUS_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.ZEUS_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into ZEUS_TUI_THEME.
