...
const logDir = join(process.env.ZEUS_HOME?.trim() || join(homedir(), '.zeus'), 'logs')
...
// a real ~/.zeus (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('zeus-tui: parent lifecycle log unavailable\n')
    }
  }
}
