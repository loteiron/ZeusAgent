...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.ZEUS_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep ZeusAgent on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via ZEUS_TUI_TRUECOLOR=1.
