import { Box, Text, useStdout } from '@zeus/ink'
...
// A compact command-deck header leaves the first screen for the task.
// User-authored art remains available through the skin contract.
export function Banner({ maxWidth, t }: { maxWidth?: number; t: Theme }) {
  ...
  const customLines = t.bannerLogo ? logo(t.color, t.bannerLogo) : null

  return (
    <Box flexDirection="column" marginBottom={1} width={cols}>
      {customLines && artWidth(customLines) < cols ? (
        <ArtLines lines={customLines} />
      ) : (
      ...
      {cols >= 40 && (
        <Text color={t.color.muted} wrap="truncate-end">
          Describe a task. Follow the work. Keep the context.
        </Text>
      )}
    </Box>
  )
...
export function SessionPanel({ info, maxWidth, sid, t }: SessionPanelProps) {
  ...
  const wide = Boolean(t.bannerHero) && cols >= 90 && leftW + 40 < cols
  ...
  // zeus_cli/banner.py) and the "connected" label on the collapse toggle.
    ...
    <Box flexDirection="column" width="100%">
      ...
      {typeof info.update_behind === 'number' && info.update_behind > 0 && (
        <Text bold color={t.color.warn}>
          ...
          <Text bold color={t.color.warn}>
            {info.update_command || 'zeus update'}
          </Text>
          <Text bold={false} color={t.color.warn} dimColor>
            {' '}
            to update
