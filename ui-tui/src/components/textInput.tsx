import type { InputEvent, Key } from '@zeus/ink'
import * as Ink from '@zeus/ink'
 ...
 * but that accepts `key.meta` on macOS — and zeus-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.ZEUS_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which zeus-ink decodes as meta+'d'; without this branch it
