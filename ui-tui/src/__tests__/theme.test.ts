...
// ZEUS_TUI_THEME=light (or ZEUS_TUI_BACKGROUND set to something
...
describe('DEFAULT_THEME', () => {
  it('has brand defaults', async () => {
    ...
    expect(DEFAULT_THEME.brand.name).toBe('ZeusAgent')
    expect(DEFAULT_THEME.brand.prompt).toBe('❯')
    expect(DEFAULT_THEME.brand.tool).toBe('┊')
  })

  it('has color palette', async () => {
    const { DEFAULT_THEME, DARK_SEEDS, contrastRatio } = await importThemeWithCleanEnv()

    expect(DEFAULT_THEME.color.primary).toBe(DARK_SEEDS.primary)
    expect(contrastRatio(DEFAULT_THEME.color.text, DARK_SEEDS.bg)).toBeGreaterThanOrEqual(4.5)
    expect(contrastRatio(DEFAULT_THEME.color.error, DARK_SEEDS.bg)).toBeGreaterThanOrEqual(4.5)
  })
})
...
describe('detectLightMode', () => {
  ...
  it('honors ZEUS_TUI_LIGHT on/off', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ ZEUS_TUI_LIGHT: '1' })).toBe(true)
    expect(detectLightMode({ ZEUS_TUI_LIGHT: 'true' })).toBe(true)
    expect(detectLightMode({ ZEUS_TUI_LIGHT: 'on' })).toBe(true)
    expect(detectLightMode({ ZEUS_TUI_LIGHT: '0' })).toBe(false)
    expect(detectLightMode({ ZEUS_TUI_LIGHT: 'off' })).toBe(false)
  })
  ...
  it('lets ZEUS_TUI_LIGHT=0 override a light COLORFGBG', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ COLORFGBG: '0;15', ZEUS_TUI_LIGHT: '0' })).toBe(false)
  })

  it('honors ZEUS_TUI_THEME=light/dark as a symmetric explicit override', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ ZEUS_TUI_THEME: 'light' })).toBe(true)
    expect(detectLightMode({ ZEUS_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '0;15', ZEUS_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '15;0', ZEUS_TUI_THEME: 'light' })).toBe(true)
  })

  it('uses ZEUS_TUI_BACKGROUND luminance when COLORFGBG is missing', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ ZEUS_TUI_BACKGROUND: '#ffffff' })).toBe(true)
    expect(detectLightMode({ ZEUS_TUI_BACKGROUND: '#000000' })).toBe(false)
    expect(detectLightMode({ ZEUS_TUI_BACKGROUND: '#1e1e1e' })).toBe(false)
    // Three-char hex normalises like CSS.
    expect(detectLightMode({ ZEUS_TUI_BACKGROUND: '#fff' })).toBe(true)
    // Garbage falls through to the default-dark path.
    expect(detectLightMode({ ZEUS_TUI_BACKGROUND: 'not-a-colour' })).toBe(false)
  })
  ...
  it('rejects partially-invalid hex instead of silently truncating', async () => {
    ...
    expect(detectLightMode({ ZEUS_TUI_BACKGROUND: '#fffgff' })).toBe(false)
    expect(detectLightMode({ ZEUS_TUI_BACKGROUND: 'ffggff' })).toBe(false)
    expect(detectLightMode({ ZEUS_TUI_BACKGROUND: '#xyz' })).toBe(false)
    // Wrong length also rejected (no implicit padding/truncation).
    expect(detectLightMode({ ZEUS_TUI_BACKGROUND: '#fffff' })).toBe(false)
    expect(detectLightMode({ ZEUS_TUI_BACKGROUND: '#fffffff' })).toBe(false)
  })
...
describe('fromSkin', () => {
  ...
  // ZEUS_TUI_THEME=light would flip the base palette and make these
  ...
  it('derives completion current background from resolved completion background (polarity-compatible)', async () => {
    ...
    const { fromSkin } = await importThemeWithEnv({ ZEUS_TUI_BACKGROUND: '#ffffff' })
  ...
  it('skinIsLight: the authored background decides; host detection only when absent', async () => {
    const { skinIsLight } = await importThemeWithEnv({ ZEUS_TUI_BACKGROUND: '#ffffff' })
...
describe('background-aware adaptation (OSC-11 light terminals)', () => {
  it('renders a dark-authored skin on light like minimumContrastRatio hosts do (the standardized look)', async () => {
    const { contrastRatio, fromSkin } = await importThemeWithEnv({ ZEUS_TUI_BACKGROUND: '#ffffff' })
  ...
  it('leaves the same skin untouched on a dark background', async () => {
    const { fromSkin } = await importThemeWithEnv({ ZEUS_TUI_BACKGROUND: '#1e1e2e' })
    const { color } = fromSkin(SLATE_COLORS, {})
  ...
  it('empty skin on a light background resolves to the light base palette', async () => {
    const { fromSkin, LIGHT_THEME } = await importThemeWithEnv({ ZEUS_TUI_BACKGROUND: '#ffffff' })
  ...
  it('base palettes are fixed points of the adaptation', async () => {
    ...
    const light = await importThemeWithEnv({ ZEUS_TUI_BACKGROUND: '#ffffff' })

    expect(light.fromSkin({}, {}).color).toEqual(light.LIGHT_THEME.color)
  })

  it('defaultThemeForCurrentBackground follows a late ZEUS_TUI_BACKGROUND write', async () => {
    ...
    expect(defaultThemeForCurrentBackground({ ZEUS_TUI_BACKGROUND: '#ffffff' }).color).toEqual(LIGHT_THEME.color)
  })
