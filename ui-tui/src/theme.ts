import type { SkinBranding, SkinColors } from '@zeus/shared/skin'
...
// Mirrors @zeus/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @zeus/ink is bundled separately from packages/.
...
const BRAND: ThemeBrand = {
  ...
  icon: 'Z/',
  prompt: '❯',
  welcome: 'Describe your task. Use /help to explore commands.',
  goodbye: 'Workspace closed. See you next time.',
  tool: '┊',
  helpHeader: 'ZeusAgent / Commands'
}
...
export function buildPalette(seeds: ThemeSeeds, isLight: boolean): ThemeColors {
  ...
  return {
    ...
    statusBg: surface,
    statusFg: seeds.statusFg ?? tones.statusFg,
...
// Ink, storm blue and copper keep the same hierarchy on light terminals.
 ...
 *  answer when known (cached in ZEUS_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.ZEUS_TUI_BACKGROUND ?? '').trim()
...
/** Derive secondary tones for custom skins from their authored identity.
 * The default Zeus palette supplies its own muted text and surface fills;
 * custom palettes keep this established mixing behavior. */
export function deriveTones(seeds: {
  ...
  return {
    // Preserve hue while separating body, label and muted roles.
...
// Terminal. Explicit ZEUS_TUI_THEME / COLORFGBG signals above still win,
...
// `ZEUS_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `ZEUS_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `ZEUS_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `ZEUS_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default ZeusAgent palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.ZEUS_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.ZEUS_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.ZEUS_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into ZEUS_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into ZEUS_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (ZeusAgent navy
  //    surfaces, cool muted text) only carry over for the skinless default — a
  //    skin with its own identity derives its fills from its own seeds.
  const identityFills: Partial<ThemeSeeds> = hasSkinColors
    ? {}
    : {
        activeRow: base.activeRow,
        muted: base.muted,
  ...
  // the derived value already carries the identity seeds (e.g. ZeusAgent navy).
  const activeRow =
