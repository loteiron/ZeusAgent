import type { RunExternalProcess } from '@zeus/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`zeus ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching zeus: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`zeus ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
