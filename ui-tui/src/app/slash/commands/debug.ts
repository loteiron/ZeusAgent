...
import { terminalBackgroundHex } from '@zeus/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $ZEUS_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see ZEUS_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['ZEUS_TUI_BACKGROUND', process.env.ZEUS_TUI_BACKGROUND ?? '(unset)'],
            ['ZEUS_TUI_THEME', process.env.ZEUS_TUI_THEME ?? '(unset)'],
