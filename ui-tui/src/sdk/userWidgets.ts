...
/**
 * User widget apps — ZeusAgent authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$ZEUS_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.zeus/plugins/`: files under ZEUS_HOME execute
...
const widgetsDir = () => join(process.env.ZEUS_HOME?.trim() || join(homedir(), '.zeus'), 'tui-widgets')
 ...
 *  every change, so a widget ZeusAgent writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or ZeusAgent) ever writes must hot-load too — a 10s poll here read
