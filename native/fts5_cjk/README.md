...
storage discipline as the other indexes). On a populated database, run
    ...
    zeus sessions optimize-storage

to backfill it; new messages are indexed live either way. Set
`sessions.cjk_fts: false` in `~/.zeus/config.yaml` to disable. Override
the .so location with `ZEUS_FTS5_CJK_SO`.
