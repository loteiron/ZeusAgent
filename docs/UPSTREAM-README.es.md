> ZeusAgent source fork: use [README.md](README.md) for installation and verification status. This inherited translation may describe upstream deployment services.
...
# ZeusAgent ⚡
<p align="center">
  <a href="https://hermes-agent.nousresearch.com/">ZeusAgent</a> | <a href="https://hermes-agent.nousresearch.com/">ZeusAgent Desktop</a>
</p>
<p align="center">
...
> **Nota:** En Windows nativo, ZeusAgent funciona sin WSL — la CLI, el gateway, la TUI y las herramientas funcionan de forma nativa. Si prefieres usar WSL2, el comando de Linux/macOS de arriba también funciona allí. ¿Encontraste un error? Por favor [crea un issue](https://github.com/NousResearch/hermes-agent/issues).
...
El instalador se encarga de todo: uv, Python 3.11, Node.js, ripgrep, ffmpeg, **y un Git Bash portátil** (MinGit, descomprimido en `%LOCALAPPDATA%\zeus\git` — no requiere administrador, completamente aislado de cualquier instalación de Git del sistema). ZeusAgent usa este Git Bash incluido para ejecutar comandos de shell.
...
> **Android / Termux:** La ruta manual probada está documentada en la [guía de Termux](https://hermes-agent.nousresearch.com/docs/getting-started/termux). En Termux, ZeusAgent instala el extra `.[termux]` curado porque el extra completo `.[all]` actualmente incluye dependencias de voz incompatibles con Android.
>
> **Windows:** Windows nativo es totalmente compatible — el comando de PowerShell de arriba instala todo. Si prefieres usar WSL2, el comando de Linux también funciona allí. La instalación nativa de Windows se encuentra en `%LOCALAPPDATA%\zeus`; WSL2 instala en `~/.zeus` como en Linux.

Después de la instalación:

```bash
source ~/.bashrc    # recargar shell (o: source ~/.zshrc)
zeus              # ¡empieza a chatear!
```
...
```bash
zeus              # CLI interactiva — inicia una conversación
zeus model        # Elige tu proveedor y modelo LLM
zeus tools        # Configura qué herramientas están habilitadas
zeus config set   # Establece valores de configuración individuales
zeus gateway      # Inicia el gateway de mensajería (Telegram, Discord, etc.)
zeus setup        # Ejecuta el asistente de configuración completo
zeus claw migrate # Migra desde OpenClaw (si vienes de OpenClaw)
zeus update       # Actualiza a la última versión
zeus doctor       # Diagnostica cualquier problema
```
...
ZeusAgent funciona con cualquier proveedor que quieras — eso no cambiará. Pero si prefieres no recopilar cinco claves API separadas para el modelo, búsqueda web, generación de imágenes, TTS y un navegador en la nube, **[Nous Portal](https://portal.nousresearch.com)** las cubre todas bajo una sola suscripción:
...
Esto te autentica vía OAuth, establece Nous como tu proveedor y activa el Tool Gateway. Comprueba qué está conectado en cualquier momento con `zeus portal info`. Detalles completos en la [página de documentación del Tool Gateway](https://hermes-agent.nousresearch.com/docs/user-guide/features/tool-gateway).
...
ZeusAgent tiene dos puntos de entrada: inicia la interfaz de terminal con `zeus`, o ejecuta el gateway y habla con él desde Telegram, Discord, Slack, WhatsApp, Signal o Email. Una vez en una conversación, muchos comandos de barra son compartidos entre ambas interfaces.

| Acción                              | CLI                                           | Plataformas de mensajería                                                         |
| ----------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------- |
| Empezar a chatear                   | `zeus`                                      | Ejecuta `zeus gateway setup` + `zeus gateway start`, luego envía un mensaje al bot |
...
Si vienes de OpenClaw, ZeusAgent puede importar automáticamente tu configuración, memorias, habilidades y claves API.

**Durante la configuración inicial:** El asistente de configuración (`zeus setup`) detecta automáticamente `~/.openclaw` y ofrece migrar antes de que comience la configuración.

**En cualquier momento después de instalar:**

```bash
zeus claw migrate              # Migración interactiva (preset completo)
zeus claw migrate --dry-run    # Vista previa de qué se migraría
zeus claw migrate --preset user-data   # Migrar sin secretos
zeus claw migrate --overwrite  # Sobreescribir conflictos existentes
```
...
- **Habilidades** — habilidades creadas por el usuario → `~/.zeus/skills/openclaw-imports/`
...
Consulta `zeus claw migrate --help` para todas las opciones, o usa la habilidad `openclaw-migration` para una migración guiada interactiva por el agente con vistas previas de dry-run.
...
Inicio rápido para colaboradores — clona y comienza con `setup-zeus.sh`:

```bash
git clone https://github.com/NousResearch/hermes-agent.git
cd zeus-agent
./setup-zeus.sh     # instala uv, crea venv, instala .[all], enlaza ~/.local/bin/zeus
./zeus              # detecta automáticamente el venv, no necesitas hacer `source` primero
```
...
- 🔌 [computer-use-linux](https://github.com/avifenesh/computer-use-linux) — Servidor MCP de control de escritorio Linux para ZeusAgent y otros hosts MCP, con árboles de accesibilidad AT-SPI, entrada Wayland/X11, capturas de pantalla y targeting de ventanas del compositor.
- 🔌 [ZeusAgentClaw](https://github.com/AaronWong1999/hermesclaw) — Puente WeChat comunitario: Ejecuta ZeusAgent y OpenClaw en la misma cuenta de WeChat.
