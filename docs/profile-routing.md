...
`zeus config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.zeus/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `ZEUS_HOME`, secret scope, and
