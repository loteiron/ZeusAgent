...
ZeusAgent gateway daemon, exported over OTLP/HTTP to an operator-configured
...
capture is a separate plane served by ZeusAgent's native NeMo Relay SDK
...
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `zeus.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `zeus.platform.up/degraded` with bounded `error_code` attributes |
...
`zeus.gateway.active_agents`, `zeus.gateway.background_work`, and
`zeus.gateway.background_delegations` are complementary. `active_agents`
...
```bash
zeus monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'zeus-agent[otlp]'`),
...
a plain `zeus gateway run`: the exporter lives in the gateway process, so
...
`zeus.gateway.up`, `zeus.platform.up`, and `zeus.platform.degraded`.
...
absent_over_time(zeus_gateway_up[5m])

# Locally owned bridge is explicitly down.
zeus_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
zeus_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
zeus_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
zeus_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(zeus_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `zeus.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
zeus.status = failed|unknown
zeus.delivery_outcome = failed|not_configured
zeus.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `zeus.job_key`;
...
ZeusAgent-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log /tmp/zeus_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log /tmp/zeus_otel_capture.jsonl --wait 8
...
`zeus.error_class = ...` list in this file's alert section and the enum's unit
...
```bash
zeus monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `zeus monitoring` CLI intentionally exposes `status` only. This first
release covers only ZeusAgent-owned service-health and operational-diagnostic
signals, including ZeusAgent-owned Relay transport health. Team Gateway's
