# ZeusAgent Observer Hooks

ZeusAgent observer hooks are the read-only telemetry contract for plugins that
...
They do not replace ZeusAgent' planner, model providers, memory, tool registry,
...
ZeusAgent also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "zeus.observer.v1"
```

Hook callbacks are fail-open. ZeusAgent catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so ZeusAgent only builds
...
The native NeMo Relay SDK integration maps ZeusAgent session, turn, LLM, and tool
