...
ZeusAgent includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into ZeusAgent and does not require a ZeusAgent observability plugin. ZeusAgent remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
ZeusAgent execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `zeus-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the ZeusAgent `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `ZEUS_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `ZEUS_NEMO_RELAY_ATOF_*` and `ZEUS_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, ZeusAgent does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

ZeusAgent requires NeMo Relay 0.8.3 or later within the 0.8 release line. That
line provides the provider-codec and canonical tool-result contracts ZeusAgent
uses for managed provider and tool calls.

## Runtime Dependency and Data Boundary

ZeusAgent installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the ZeusAgent process so configured interceptors can
...
Collection remains off unless ZeusAgent policy enables it:
...
opt-in. Set `ZEUS_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, ZeusAgent does not invoke Relay's plugin initializer, so Relay
...
selected file cannot be loaded, ZeusAgent reports the error and does not invoke
...
retain the same `session_id` and add `zeus.session.segment` plus
`zeus.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not a ZeusAgent
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that ZeusAgent process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, ZeusAgent logs:
...
profile's Relay scope stack. Native dynamic plugins are loaded into the ZeusAgent
process and share the same policy boundary.

Run profiles in separate ZeusAgent processes when they require different trust
...
ZeusAgent core owns one Relay host and one isolated Relay session scope per ZeusAgent
...
marks do not require ZeusAgent plugin registration. Shared-metrics marks must
...
```text
ZeusAgent turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> ZeusAgent shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

ZeusAgent sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that ZeusAgent used for the logical call, such as
...
New calls use `zeus.model_route.count`. The previous
`zeus.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `zeus.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; ZeusAgent
version, OS family, architecture, and install method remain bounded package
resources. Concurrent ZeusAgent processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `zeus.task_run`, parented to
the owning ZeusAgent session. The start counter contains only bounded execution
...
provider attempts for the same ZeusAgent API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their ZeusAgent tool-call
...
cancellations. Active task ownership follows the task ID if ZeusAgent rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`zeus.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. ZeusAgent
...
tool or plugin names. ZeusAgent does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `zeus.tool_approval` marks and
...
Successful skill mutations emit `zeus.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `zeus.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. ZeusAgent derives reuse and patch-generation
...
```text
$ZEUS_HOME/telemetry/shared_metrics/metrics.sqlite3
$ZEUS_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the ZeusAgent version,
...
schema field name, its current scope is one `ZEUS_HOME`, so it is more
...
Deleting `$ZEUS_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `ZEUS_HOME`. To reset it, stop ZeusAgent
processes and remove `$ZEUS_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real ZeusAgent CLI turn against the deterministic local model server:
...
`$ZEUS_HOME/telemetry/shared_metrics/outbox/` to the ZeusAgent telemetry ingest
...
Removing `$ZEUS_HOME/telemetry/shared_metrics` still resets local identity,
