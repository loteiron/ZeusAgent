...
This document is the formal interface between the **ZeusAgent gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `zeus gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`zeus gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `zeus` CLI.
