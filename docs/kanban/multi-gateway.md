...
ZeusAgent supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.zeus/config.yaml`:
...
Or set the env var: `ZEUS_KANBAN_DISPATCH_IN_GATEWAY=false`
