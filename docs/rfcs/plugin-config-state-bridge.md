...
or change arbitrary ZeusAgent configuration.
...
small host fact requested by the RFC. Settings writes use ZeusAgent'
...
`ZEUS_HOME`, so one globally loaded plugin context follows multiplexed
...
```text
<ZEUS_HOME>/plugin-data/<plugin-data-namespace>/state.json
```
...
- **Profile isolation:** the data root resolves from the active context-local
  ZeusAgent home on every operation.
...
The implementation is covered with real temporary-ZeusAgent-home tests for:
