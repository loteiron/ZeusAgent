...
## 2026-07-13: Scope plugin manager state by ZeusAgent home/profile (keyed cache)

Status: Accepted

Context:
ZeusAgent supports multiple profiles via different ZeusAgent home directories.
Homes are switched two ways in a running process: the `ZEUS_HOME`
environment variable (single-profile CLI/gateway processes), and the
context-local `set_zeus_home_override()` (`zeus_constants.py`), which
...
`get_zeus_home() / "plugins"`, and context-engine plugins (e.g.
`zeus-lcm`) capture profile-scoped state — such as the LCM database
path — at registration time. A single-slot cache meant:

1. Switching homes via `set_zeus_home_override()` was invisible to a
   naive "did `ZEUS_HOME` change" check, so the singleton silently kept
...
2. Even when a fresh `PluginManager` *was* created for a new home, plugin
   modules are imported into `sys.modules` as `zeus_plugins.<slug>` by
   ...
   `zeus_plugins.<slug>.<submodule>`, and Python's import machinery
...
- Replace the single-slot singleton with a cache keyed on the *resolved*
  ZeusAgent home path (`_plugin_managers_by_home: Dict[Path, PluginManager]`).
  `get_plugin_manager()` resolves the current home via `get_zeus_home()`
  (which itself already consults `get_zeus_home_override()` before
...
- Per-profile LCM instances (and any other context-engine plugin) use
  ...
  through `ZEUS_HOME` or `set_zeus_home_override()`.
...
- Regression coverage exercises the real production path
  (`set_zeus_home_override()`) rather than only the env-var path, and
