...
_profile_runtime_scope(profile_home)           (gateway/run.py)
   ├── set_zeus_home_override(home)          config / state.db / skills /
...
- `get_secret(name)` resolves: global allowlist → active scope → fallback.
  ...
  - multiplexing **on**, no scope installed: **raises `UnscopedSecretError`**
    ...
    value.
- A small allowlist (`ZEUS_HOME`, `ZEUS_PROFILE`, proxy settings,
...
reload helper: `zeus_cli.env_loader.load_zeus_dotenv` skips the
...
## The ZEUS_HOME override

`zeus_constants.py` holds a context-local override consulted by
`get_zeus_home()` before the `ZEUS_HOME` env var. Everything that resolves
...
automatically. `get_process_zeus_home()` exists for the few machine-level
assets that must not follow the override. `zeus_home_key()` gives
...
`profiles_to_serve(multiplex, profile_allowlist)` in `zeus_cli/profiles.py`
...
DB handles are resolved at call time through the active ZEUS_HOME override —
...
ZEUS_HOME override. Asset writes are atomic, type- and size-capped.
...
| Built-in tool registry | Built-ins are process-global; plugin-registered tools are overlaid per profile via `zeus_home_key()`. |
...
- `agent/secret_scope.py`, `zeus_constants.py`, `gateway/profile_routing.py`,
  `gateway/run.py` (`_profile_runtime_scope`), `zeus_cli/profiles.py`
