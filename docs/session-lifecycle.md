...
The canonical transcript store is SQLite via `SessionDB` (from `zeus_state`). The
...
Counts consecutive restarts via a JSON file (`{ZEUS_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `zeus update`, `zeus gateway restart`,
or `/restart`.
