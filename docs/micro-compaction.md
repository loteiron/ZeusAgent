...
has to be thrown away or summarized. ZeusAgent has always done this in one batch:
...
ZeusAgent folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. ZeusAgent's normal session flush is
...
With `micro_compact` unset or `false` ZeusAgent behaves exactly as it always has:
...
Defaults to `$ZEUS_HOME/logs/agent.log`. It reports passes, outcome mix, net
