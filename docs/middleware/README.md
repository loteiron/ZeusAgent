# ZeusAgent Middleware

ZeusAgent middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing ZeusAgent'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before ZeusAgent calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving ZeusAgent retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving ZeusAgent guardrails,
...
- `telemetry_schema_version`: currently `zeus.observer.v1`
- `middleware_schema_version`: currently `zeus.middleware.v1`
...
ZeusAgent stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, ZeusAgent runs
them as a nested chain in registration order. Middleware failures are fail-open:
ZeusAgent logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, ZeusAgent applies middleware in this order:
...
For each tool call, ZeusAgent applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal ZeusAgent pre-execution path against the effective arguments:
...
For isolated local testing, use one `ZEUS_HOME` for plugin enablement and the
agent run:

```bash
export ZEUS_HOME=/tmp/zeus-middleware-test
mkdir -p "$ZEUS_HOME"
zeus plugins enable <plugin-name>
zeus chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run zeus plugins enable <plugin-name>
uv run zeus chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["zeus_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/zeus-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape ZeusAgent expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, ZeusAgent treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, ZeusAgent preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. ZeusAgent does not convert downstream
