...
`zeus doctor` reports the explicit repair command.
...
repaired or restored file. Do not run `zeus doctor --fix` while the gateway
is still up. Next steps:

```bash
zeus gateway stop
ZEUS_HOME="$HOME/.zeus" zeus sessions recover --source "$HOME/.zeus/state.db" --inspect-only
# if recoverable:
ZEUS_HOME="$HOME/.zeus" zeus sessions recover --source "$HOME/.zeus/state.db" --output "$HOME/recovered-state.db"
```
...
```bash
zeus gateway stop
ZEUS_HOME="$HOME/.zeus" zeus sessions repair --check-only
ZEUS_HOME="$HOME/.zeus" zeus sessions repair
```
