#!/bin/sh
# shellcheck shell=sh
# /opt/zeus/bin/zeus — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `zeus` user (UID 10000). When an operator runs `docker exec <c> zeus ...`
# the default UID is root (0), and any file the command writes under
# $ZEUS_HOME — auth.json, .env, config.yaml — ends up root-owned and
# unreadable to the supervised gateway. The most common manifestation: the
# user runs `docker exec <c> zeus login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: ZeusAgent is not logged into Nous
# Portal" on every incoming message — even though `docker exec <c> zeus
...
# Fix
# ---
# This shim sits at /opt/zeus/bin/zeus and is placed earliest on PATH.
# When invoked as root, it drops to the zeus user (via s6-setuidgid)
# before exec'ing the real venv binary, so anything that writes under
# $ZEUS_HOME is uid-aligned with the supervised processes. When invoked
# as any non-root UID — including the supervised processes themselves,
# `docker exec --user zeus`, kanban subagents, etc. — it short-circuits
...
# (/opt/zeus/.venv/bin/zeus), so the second hop cannot re-enter this
# shim regardless of PATH state. No sentinel env var needed.
#
# Opt-out: set ZEUS_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
# state via the zeus CLI. Default is to drop.

set -e

REAL=/opt/zeus/.venv/bin/zeus

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "zeus-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user zeus`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi

# Root, with opt-out set? Honor it.
case "${ZEUS_DOCKER_EXEC_AS_ROOT:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        exec "$REAL" "$@"
        ;;
esac

# Root, no opt-out. Drop to the zeus user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "zeus-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "zeus-shim: re-run with --user zeus or set ZEUS_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the zeus user's home before dropping privileges. Without
