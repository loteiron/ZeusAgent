#!/bin/sh
# shellcheck shell=sh
# /opt/zeus/docker/main-wrapper.sh — wraps the container's CMD with
...
# with-contenv before touching ZEUS_HOME / PATH. On the non-PID-1
# fallback path the Dockerfile env is still intact, so we skip the
# re-exec and continue directly.
#
# Routing:
#   no args                       → exec `zeus` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `zeus <args>` (subcommand passthrough)
#
# Drop to zeus via s6-setuidgid, but skip it when already non-root.
set -e

if [ -z "${ZEUS_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${ZEUS_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export ZEUS_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset ZEUS_MAIN_WRAPPER_ENV_READY

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid zeus "$@"; exec "$@"; }
...
# an arbitrary non-root, non-zeus UID, the bootstrap was skipped and the
# baked image dirs (owned by the zeus build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u zeus)" ]; then
    cat >&2 <<EOF
[zeus] ERROR: container started with --user $cur_uid (an arbitrary, non-zeus UID) — not supported.
...
# to the zeus user's home before dropping privileges so libraries that
...
_zeus_orig_cwd="${ZEUS_ORIG_CWD:-$PWD}"
...
# the user's command so `zeus chat` starts in the Docker -w
...
# ZeusAgent subcommand pass-through.
drop zeus "$@"
