...
# ZeusAgent WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: ZEUS_TINI_SHIM_TARGET overrides the `/init` path (and
# ZEUS_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${ZEUS_TINI_SHIM_TARGET:-/init}"
WRAPPER="${ZEUS_TINI_SHIM_WRAPPER:-/opt/zeus/docker/main-wrapper.sh}"
