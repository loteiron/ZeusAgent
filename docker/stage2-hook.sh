...
#
# Wired into the image as /etc/cont-init.d/01-zeus-setup by the
...
ZEUS_HOME="${ZEUS_HOME:-/opt/data}"
INSTALL_DIR="/opt/zeus"

# Drop to zeus via s6-setuidgid, but skip it when already non-root.
as_zeus() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid zeus "$@"; }
...
# starts non-root. The baked install tree under /opt/zeus is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $ZEUS_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass ZEUS_UID/ZEUS_GID — or the PUID/PGID aliases — which the
...
# `id -u zeus` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# zeus UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u zeus)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-zeus UID).
...
and the baked /opt/zeus install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e ZEUS_UID=\$(id -u) -e ZEUS_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the zeus user to that UID/GID at boot and chowns the data
...
# --- Bootstrap ZEUS_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid zeus mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom ZEUS_HOME paths whose parents only
# root can create (e.g. `ZEUS_HOME=/home/zeus/.zeus` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for ZEUS_UID/ZEUS_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  ZEUS_UID/
# ZEUS_GID still win when both are set.  See #15290, salvages #25872.
ZEUS_UID="${ZEUS_UID:-${PUID:-}}"
ZEUS_GID="${ZEUS_GID:-${PGID:-}}"

if [ -n "${ZEUS_UID:-}" ] && validate_uid_gid "$ZEUS_UID" && [ "$ZEUS_UID" != "$(id -u zeus)" ]; then
    echo "[stage2] Changing zeus UID to $ZEUS_UID"
    usermod -u "$ZEUS_UID" zeus
fi
if [ -n "${ZEUS_GID:-}" ] && validate_uid_gid "$ZEUS_GID" && [ "$ZEUS_GID" != "$(id -g zeus)" ]; then
    echo "[stage2] Changing zeus GID to $ZEUS_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$ZEUS_GID" hermes 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised zeus user (UID 10000)
...
# zeus process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes zeus. Idempotent across container
...
#   - zeus is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G hermes 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] zeus already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" hermes 2>/dev/null; then
        echo "[stage2] Added zeus to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group zeus failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When ZEUS_UID is remapped or the top-level $ZEUS_HOME isn't owned by
# the runtime zeus UID, restore ownership to zeus — but ONLY for the
# directories zeus actually writes to. The full $ZEUS_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of zeus-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_zeus_uid=$(id -u zeus)
...
chown_zeus_tree() {
    ...
    chown -R zeus:zeus "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_zeus_owner() {
    target="$1"
    find "$target" \( ! -user zeus -o ! -group zeus \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$ZEUS_HOME" 2>/dev/null)" != "$actual_zeus_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $ZEUS_HOME (targeted) to zeus ($actual_zeus_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $ZEUS_HOME: chown the directory itself (not its contents)
    # so zeus can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$ZEUS_HOME"; then
        :
    else
        chown zeus:zeus "$ZEUS_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $ZEUS_HOME failed (rootless container?) — continuing"
    fi
    # ZeusAgent-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by zeus (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$ZEUS_HOME/$sub" ] && tree_has_non_zeus_owner "$ZEUS_HOME/$sub"; then
            chown_zeus_tree "$ZEUS_HOME/$sub"
        fi
    done
fi
...
# zeus user. Hosted/container instances keep mutable state under
# $ZEUS_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# ZEUS_DISABLE_LAZY_INSTALLS=1. Keeping /opt/zeus root-owned and
...
# $ZEUS_HOME/lazy-packages dir on the data volume (Dockerfile sets
# ZEUS_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to zeus
...
# Always reset ownership of $ZEUS_HOME/profiles to zeus on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> zeus …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as zeus and walks
...
if [ -d "$ZEUS_HOME/profiles" ] && tree_has_non_zeus_owner "$ZEUS_HOME/profiles"; then
    chown_zeus_tree "$ZEUS_HOME/profiles"
fi

# Always reset ownership of $ZEUS_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged zeus runtime even
...
# gate as profiles/).
if [ -d "$ZEUS_HOME/cron" ] && tree_has_non_zeus_owner "$ZEUS_HOME/cron"; then
    chown_zeus_tree "$ZEUS_HOME/cron"
fi

# Always ensure logs/gateways is zeus-owned (#45258). Formerly healed by
...
# top-level $ZEUS_HOME is mis-owned, so a warm volume with zeus-owned
# ZEUS_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid zeus mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as zeus.
if [ -d "$ZEUS_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$ZEUS_HOME/logs/gateways"; then
        :
    else
        chown zeus:zeus "$ZEUS_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# zeus pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged zeus gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $ZEUS_HOME is
...
# and cron/.
if [ -d "$ZEUS_HOME/platforms/pairing" ] && tree_has_non_zeus_owner "$ZEUS_HOME/platforms/pairing"; then
    chown_zeus_tree "$ZEUS_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$ZEUS_HOME/pairing" ] && tree_has_non_zeus_owner "$ZEUS_HOME/pairing"; then
    chown_zeus_tree "$ZEUS_HOME/pairing"
fi

# Reset ownership of zeus-owned top-level state files on every boot.
# The targeted data-volume chown above only covers zeus-owned
# *subdirectories*; loose state files living directly under $ZEUS_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> zeus …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged zeus runtime then hits
...
# sweep so host-owned files in a bind-mounted $ZEUS_HOME are never
...
# entries of zeus_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$ZEUS_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$ZEUS_HOME/$f"; then
            :
        else
            chown zeus:zeus "$ZEUS_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the zeus runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$ZEUS_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$ZEUS_HOME/config.yaml"; then
        :
    else
        chown zeus:zeus "$ZEUS_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$ZEUS_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as zeus user ---
# Run as zeus via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $ZEUS_HOME values
...
as_zeus mkdir -p \
    ...
    "$ZEUS_HOME/home" \
    "$ZEUS_HOME/pairing" \
    "$ZEUS_HOME/platforms/pairing" \
    "$ZEUS_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/zeus/.install_method (see Dockerfile), NOT written here into
# $ZEUS_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $ZEUS_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.zeus:/opt/data) and sometimes shared with a
...
# 'zeus update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $ZEUS_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/zeus stamp, so deleting the data-dir copy is safe).
if [ -f "$ZEUS_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$ZEUS_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$ZEUS_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$ZEUS_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$ZEUS_HOME/$dest"; then
            :
        else
            as_zeus cp "$INSTALL_DIR/$src" "$ZEUS_HOME/$dest"
        fi
    fi
}
...
# the first-boot seed above depends on /opt/zeus/.env.example being present
...
# not generate one. ZeusAgent loads $ZEUS_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$ZEUS_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$ZEUS_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $ZEUS_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$ZEUS_HOME/.env" ] && ! refuse_symlinked_path "clean" "$ZEUS_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$ZEUS_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$ZEUS_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$ZEUS_HOME/.env"; then
        :
    else
        if [ ! -f "$ZEUS_HOME/.env" ]; then
            ...
            (umask 077 && as_zeus touch "$ZEUS_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$ZEUS_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$ZEUS_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$ZEUS_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $ZEUS_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $ZEUS_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi
...
if [ -f "$ZEUS_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$ZEUS_HOME/.env"; then
        :
    else
        chown zeus:zeus "$ZEUS_HOME/.env" 2>/dev/null || true
        chmod 600 "$ZEUS_HOME/.env" 2>/dev/null || true
    fi
fi
...
# as the unprivileged `zeus` user — without this it gets EACCES on every
...
# zeus group (any group member could e.g. stop/suspend this machine), not
...
if [ -S /.fly/api ]; then
    if refuse_symlinked_path "chgrp/chmod" /.fly/api; then
        :
    elif chgrp zeus /.fly/api 2>/dev/null && chmod g+w /.fly/api 2>/dev/null; then
        echo "[stage2] Granted zeus group access to the Fly Machines API socket"
    else
...
# $ZEUS_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `zeus update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set ZEUS_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$ZEUS_HOME/config.yaml" ]; then
    s6-setuidgid zeus "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$ZEUS_HOME/auth.json" ] && [ -n "${ZEUS_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$ZEUS_HOME/auth.json"; then
        :
    else
        printf '%s' "$ZEUS_AUTH_JSON_BOOTSTRAP" > "$ZEUS_HOME/auth.json"
        chown zeus:zeus "$ZEUS_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$ZEUS_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via ZEUS_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$ZEUS_HOME/auth.json" ] && [ -n "${ZEUS_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$ZEUS_HOME/auth.json"; then
        :
    else
        s6-setuidgid zeus "$INSTALL_DIR/.venv/bin/python" \
...
# can set ZEUS_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# ZEUS_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$ZEUS_HOME/gateway_state.json" ] && \
        [ "${ZEUS_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$ZEUS_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$ZEUS_HOME/gateway_state.json"
        chown zeus:zeus "$ZEUS_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$ZEUS_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_zeus "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/zeus/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI ZeusAgent spawns for the
...
# main-wrapper.sh propagates it into the supervised ``zeus`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-zeus, dashboard, etc.).
