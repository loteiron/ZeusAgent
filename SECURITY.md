# ZeusAgent Security Policy

This document describes ZeusAgent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Use **Security → Report a vulnerability** on the ZeusAgent repository from
which you obtained this fork, if its maintainer has enabled private reporting.
This source snapshot does not configure an independent security contact or a
response-time commitment. If private reporting is unavailable, ask the repository
maintainer to establish a private channel; do not post secrets, personal data,
or a working exploit in a public issue. Nous Research's inbox and advisory tracker
are not the support channel for this independent fork. **No bug bounty is offered.**
...
- Environment details (`zeus --version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted ZeusAgent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in ZeusAgent's documentation
...
ZeusAgent supports two OS-level isolation postures. They address
...
ZeusAgent supports this in two ways:

- **ZeusAgent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, ZeusAgent's in-process heuristics
...
ZeusAgent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in ZeusAgent
itself. Bugs in ZeusAgent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   ZeusAgent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, ZeusAgent's own documentation, or reasonable
  operator expectations would predict — including cases where
  ZeusAgent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  zeus-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not ZeusAgent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in ZeusAgent. Bugs in ZeusAgent
...
- MCP server launches retain their supply-chain guards. The active source-fork
  CI validates the selected source boundary, dependencies via locked installation,
  tests and builds; it is not a complete dependency vulnerability audit.
  Upstream audit/publishing workflows are archived and inactive.

---

## 5. Disclosure

- Agree on a disclosure timeline privately with this fork's maintainer.
  An upstream policy does not constitute a response guarantee for ZeusAgent.
- Use the repository's private advisory thread or a maintainer-confirmed private
  channel. Never send credentials or confidential reports to an unverified address.
- Ask reporters how they wish to be credited and respect requests for anonymity.
