"""Centralized logging setup for ZeusAgent.
...
# in ZeusAgent (TUI, gateway, hy_memory, MCP servers, CLI commands all log) —
...
from zeus_constants import get_config_path, get_zeus_home, mkdir_under_zeus_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_zeus_session_injector", False):
        return
    ...
    def _session_record_factory(*args, **kwargs):
        ...
        try:
            record.zeus_home = str(get_zeus_home().resolve())  # type: ignore[attr-defined]
        except Exception:
            record.zeus_home = ""  # type: ignore[attr-defined]
        return record
    _session_record_factory._zeus_session_injector = True  # type: ignore[attr-defined]
...
# Logger name prefixes per component; used by _ComponentFilter and ``zeus logs --component``.
COMPONENT_PREFIXES = {
    ...
    "gateway": ("gateway", "zeus_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("zeus_cli", "cli"),
    "cron": ("cron",),
    "gui": ("zeus_cli.web_server", "zeus_cli.pty_bridge", "tui_gateway", "uvicorn"),
}


def setup_logging(
    *,
    zeus_home: Optional[Path] = None,
    ...
    """Configure the ZeusAgent logging subsystem; returns the ``logs/`` directory.
    ...
    home = zeus_home or get_zeus_home()
    log_dir = mkdir_under_zeus_home(home / "logs")
...
def setup_verbose_logging() -> None:
    ...
    if any(getattr(h, "_zeus_verbose", False) for h in root.handlers):
        return
    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._zeus_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from zeus_cli.config import is_managed
...
def _new_file_handler(
    ...
    mkdir_under_zeus_home(path.parent)
...
class _ProfileRoutingFileHandler(logging.Handler):
    """Route queued records to the log file for their ZeusAgent home.
    ...
    def __init__(self, existing: RotatingFileHandler, profile_homes: Sequence[Path]) -> None:
        ...
        self._zeus_routed_log_path = resolved
    ...
    def _home_for_record(self, record: logging.LogRecord) -> Path:
        raw_home = getattr(record, "zeus_home", "")
        try:
...
def _register_queued_handler(handler: logging.Handler) -> None:
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._zeus_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    if listener is None:
        return
    t = threading.Thread(target=lambda: _quietly(listener.stop), name="zeus-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_zeus_queue", False):
...
def _add_rotating_handler(
    ...
    *,
    ...
    for existing in _queued_file_handlers:
        ...
        if getattr(existing, "_zeus_routed_log_path", None) == resolved or (
...
def _read_logging_config():
    ...
    try:
        ...
        # zeus_cli.main's early parse (one config.yaml parse per process);
        # fall back to a direct parse for bare zeus_logging consumers.
        try:
            from zeus_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
