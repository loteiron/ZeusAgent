...
Ledger path resolution (first wins):
  --ledger PATH
  $ZEUS_CITATION_LEDGER
  $ZEUS_HOME/cache/citations/ledger.json
"""
...
from _zeus_home import get_zeus_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("ZEUS_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_zeus_home() / "cache" / "citations" / "ledger.json"
