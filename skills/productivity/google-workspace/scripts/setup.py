#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for ZeusAgent.
...
# Ensure sibling modules (_zeus_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _zeus_home import display_zeus_home, get_zeus_home

ZEUS_HOME = get_zeus_home()
TOKEN_PATH = ZEUS_HOME / "google_token.json"
CLIENT_SECRET_PATH = ZEUS_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = ZEUS_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same ZeusAgent profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the ZeusAgent Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the ZeusAgent Docker image's "
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to ZeusAgent home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_zeus_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for ZeusAgent")
