#!/usr/bin/env python3
"""Google Workspace API CLI for ZeusAgent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing ZeusAgent-facing JSON contract and falls back to the Python client
...
from _zeus_home import get_zeus_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ZEUS_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for ZeusAgent")
