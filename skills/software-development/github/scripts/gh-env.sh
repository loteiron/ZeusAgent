#!/usr/bin/env bash
# GitHub environment detection helper for ZeusAgent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _zeus_env="${ZEUS_HOME:-$HOME/.zeus}/.env"; [ -f "$_zeus_env" ] && grep -q "^GITHUB_TOKEN=" "$_zeus_env" 2>/dev/null; then
