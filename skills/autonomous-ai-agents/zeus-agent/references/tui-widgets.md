...
Author widget apps for the ZeusAgent TUI (`zeus --tui`): glanceable ambient
...
- The TUI must be in use (`zeus --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.zeus/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.zeus/logs/tui_gateway_crash.log` if a widget never appears.
