# Themes / Skins — Author a ZeusAgent Color Theme

Author a ZeusAgent **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`zeus_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.zeus/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for ZeusAgent itself.
...
- Write access to the ZeusAgent home dir — `~/.zeus` by default, or `$ZEUS_HOME`
  / the active profile's dir. Skins live in `<zeus-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `zeus config set`).
...
3. `write_file` it to `<zeus-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `zeus config set display.skin default` (or they can `/skin default`).
...
```
zeus skin set <key> <hex>      # e.g. zeus skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `zeus skin set background "#08201f"` changes
only the background; `zeus skin use <name>` / `zeus skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.zeus`** when a profile is active — resolve the real home
  from `$ZEUS_HOME` first, falling back to `~/.zeus`.
...
- **Never hand-edit `config.yaml` to activate.** Use `zeus config set
...
- **You apply it, not the user.** `zeus config set display.skin <name>` is
...
- `read_file` the written `<zeus-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `zeus config get display.skin` and confirm it reports `<name>`.
