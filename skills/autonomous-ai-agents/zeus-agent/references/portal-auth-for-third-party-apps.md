...
## Layer 1 — Is this thing a ZeusAgent plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the ZeusAgent process**. Its LLM calls go through ZeusAgent's already-configured provider. | Already uses Portal if user's ZeusAgent is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside ZeusAgent. That LLM call is already authenticated
via ZeusAgent's provider config. The plugin's own server auth (e.g.
...
What ZeusAgent has that *feels* like OAuth — `zeus login --provider nous`
opening a browser, user signs in, token lands in `~/.zeus/auth.json` — is a
**ZeusAgent-specific browser flow**. Under the hood it produces a credential
ZeusAgent uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read ZeusAgent's existing Portal credential out of `~/.zeus/auth.json`.
...
rides on the credential ZeusAgent already holds.

Where this could live in ZeusAgent:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for ZeusAgent itself.
...
1. First decide: ZeusAgent plugin (runs inside ZeusAgent) or separate app? If plugin,
   it already uses Portal via ZeusAgent's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   ZeusAgent agent itself") is fully supported. ZeusAgent treats agent
...
pitch in the Tool Gateway announcement is specifically about ZeusAgent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when ZeusAgent is configured to use Portal as its provider. It is
