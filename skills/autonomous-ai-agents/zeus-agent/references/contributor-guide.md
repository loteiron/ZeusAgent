...
Config: `~/.zeus/config.yaml` (settings), `~/.zeus/.env` (API keys) — both under `$ZEUS_HOME` when it is set.
...
`_ZEUS_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_zeus_home()` for paths,
never hardcode `~/.zeus`. For custom/local-only tools, write a plugin in
`~/.zeus/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `zeus_cli/commands.py`
...
- Tests auto-redirect `ZEUS_HOME` to temp dirs — never touch real `~/.zeus/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/zeus_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_zeus_home()` from `zeus_constants` for all paths (profile-safe)
