...
Edit with `zeus config edit` or `zeus config set section.key value`.
...
`zeus config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `zeus tools` (interactive) or `zeus tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_ZEUS_CORE_TOOLS` is the default bundle most platforms inherit).
