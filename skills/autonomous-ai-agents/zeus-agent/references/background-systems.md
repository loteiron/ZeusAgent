...
the `cronjob` tool, the `zeus cron` CLI (`list`, `add`, `edit`,
...
- **CLI:** `zeus curator <verb>` — `status`, `usage`, `run`, `pause`,
...
- **Cost:** the deterministic inactivity/prune sweep runs for free. The
  ...
  **off by default** — opt in with `curator.consolidate: true` or
  `zeus curator run --consolidate`. Routine background curation costs
  zero tokens.
- **Telemetry:** sidecar at `~/.zeus/skills/.usage.json` holds
...
Users drive it via `zeus kanban <verb>`; dispatcher-spawned workers
see a focused `kanban_*` toolset gated by `ZEUS_KANBAN_TASK`, and
...
- **Isolation:** board is the hard boundary (workers get
  `ZEUS_KANBAN_BOARD` pinned in env); tenant is a soft namespace
