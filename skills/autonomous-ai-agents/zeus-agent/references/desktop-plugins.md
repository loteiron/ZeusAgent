...
Write plugins for the ZeusAgent desktop app: statusbar items, layout panes,
...
system (`~/.zeus/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$ZEUS_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$ZEUS_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The ZeusAgent desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$ZEUS_HOME/desktop-plugins/` (usually
  `~/.zeus/desktop-plugins/`).

## How to Run

1. Create `$ZEUS_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.zeus/...` by default, or `~/.zeus/profiles/<profile>/...` under a
...
The ONLY import surface is `@zeus/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from ZeusAgent
  ...
  (`zeus://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.zeus/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@zeus/plugin-sdk`, `react`, and
