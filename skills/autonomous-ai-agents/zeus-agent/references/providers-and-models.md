...
Set via `zeus model` (picker) or `zeus setup`. 35+ provider profiles ship as
...
| nous | OAuth device code | `zeus auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `zeus auth add openai-codex` |
| qwen-oauth | OAuth | `zeus auth add qwen-oauth` |
| minimax-oauth | OAuth | `zeus auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`zeus auth`).
Fallback chain when the primary fails: `zeus fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`zeus_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   zeus config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
