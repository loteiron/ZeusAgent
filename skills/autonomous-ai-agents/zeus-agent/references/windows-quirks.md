...
ZeusAgent runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`zeus config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually ZeusAgent's env scrubber dropping
...
ZeusAgent-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every ZeusAgent tool and most Windows APIs. Prefer forward slashes in code
