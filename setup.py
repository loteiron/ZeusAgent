...
ZeusAgent (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``ZEUS_NIX_BUILD=1`` on the
ZeusAgent package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("ZEUS_NIX_BUILD") == "1"

_BLOCK_MESSAGE = (
    "Building wheels or sdists for zeus-agent is not supported.\n"
    "ZeusAgent is distributed via the shell installer, Docker image, or Nix.\n"
    ...
    "the ZeusAgent Nix derivation sets ZEUS_NIX_BUILD=1. If it does, file a bug."
)
...
# Root single-file modules (``run_agent``, ``zeus_state``, ``toolsets``...)
