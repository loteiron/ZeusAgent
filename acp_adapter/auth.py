"""ACP auth helpers — detect and advertise ZeusAgent authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "zeus-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active ZeusAgent runtime provider, or None if unavailable.
...
def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for ZeusAgent.

    The ACP registry requires at least one usable auth method in the initial
    handshake. A fresh Zed install may have no ZeusAgent credentials yet, so the
    ...
    if provider:
        methods.append(AuthMethodAgent(
            ...
            description=f"Authenticate ZeusAgent using the currently configured {provider} runtime credentials.",
        ))
    methods.append(TerminalAuthMethod(
        id=TERMINAL_SETUP_AUTH_METHOD_ID, name="Configure ZeusAgent provider", type="terminal", args=["--setup"],
        description=("Open ZeusAgent' interactive model/provider setup in a terminal. "
                     "Use this when ZeusAgent has not been configured on this machine yet."),
    ))
    return methods
...
def has_provider() -> bool:
    """Return True if ZeusAgent can resolve any runtime provider credentials."""
