"""
ZeusAgent MCP Server — expose messaging conversations as MCP tools (`zeus mcp serve`).
...
Matches OpenClaw's 9-tool channel bridge surface plus the ZeusAgent-specific
channels_list. Client config: {"mcpServers": {"zeus": {"command": "zeus", "args": ["mcp", "serve"]}}}
"""
...
logger = logging.getLogger("zeus.mcp_serve")
...
def _zeus_home() -> Path:
    try:
        ...
        return Path(os.environ.get("ZEUS_HOME", Path.home() / ".zeus"))


def _get_sessions_dir() -> Path:
    return _zeus_home() / "sessions"


def _read_state_db_mtime() -> float:
    try:
        return (_zeus_home() / "state.db").stat().st_mtime
...
def _load_channel_directory() -> dict:
    ...
    return _read_json(_zeus_home() / "channel_directory.json")
...
class EventBridge:
    ...
    event queue with waiter support (the ZeusAgent analogue of OpenClaw's WebSocket
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the ZeusAgent MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(f"MCP server requires the 'mcp' package. Install with: {sys.executable} -m pip install 'mcp'")
    mcp = MCPServer("zeus", instructions=(
        "ZeusAgent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the ZeusAgent MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
