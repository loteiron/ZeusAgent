...
LINTERS = {
    ...
    # Post-write diagnostics may use an installed compiler, never download one.
    '.ts': 'npx --offline --yes=false -- tsc --noEmit {file} 2>&1',
    '.go': 'go vet {file} 2>&1',
...
_LINTER_UNUSABLE_PATTERNS = {
    'npx': (
        ...
        'enotcached',  # no installed/cached compiler; offline mode must stay offline
        'npm error canceled',  # installation explicitly declined (npm 10+)
        'npm err! canceled',  # same refusal on older supported npm versions
    ),
    'rustfmt': (
