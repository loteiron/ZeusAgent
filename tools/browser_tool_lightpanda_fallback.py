...
def lightpanda_engine_status() -> Tuple[bool, str]:
    ...
    precedence with config-only gates (no network I/O) for ``/browser status`` / ``zeus doctor``.
    """
    _bt = _origin()
    ...
    return True, "Browser Use mode: ZeusAgent spawns `lightpanda serve` per session"
