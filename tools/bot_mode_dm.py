...
local → ``zeus -p <name> chat --in ~ -c "Bot Chat" --create-if-missing -Q
--query-file <tmp>``; peer → ``zeus peer dm <peer>[/<name>] < <tmp>``; both via
...
# delivery runner (``python bot_mode_dm.py --run-delivery …``); ZeusAgent helpers import lazily.
...
_DM_DIR_NAME = "zeus-dm"
_DM_STALE_SECONDS = 24 * 60 * 60

# '<peer>/<agent>' — peer names are lowercase (``zeus peer`` normalizes them).
...
def _default_home() -> str:
    return os.getenv("ZEUS_HOME") or os.path.expanduser("~/.zeus")
                ...
                "properties": {
                    "target": {
                        ...
                        "description": (
                            ...
                            "('researcher', 'zeus' for the default agent), or "
...
def _resolve_local_name(target: str, roster: list[str]) -> Optional[str]:
    """Map a target handle to a profile name ('zeus' → 'default')."""
...
def message_agent_tool(target: str = "", message: str = "", task_id: Optional[str] = None, agent: Any = None) -> str:
    ...
    try:
        from tools.bot_mode_probe import (
            BOT_CHAT_TITLE, _handle, _zeus_root, _peers, _profile_name as _self_profile_name, _roster,
            is_bot_mode_managed,
        )
    ...
    root, me = _zeus_root(Path(home)), _self_profile_name(Path(home))
    ...
    if peer_match or raw_target.lower() in peers:
        ...
        # Pin the registry-owning profile: `zeus peer` resolves bot_peers via the profile-scoped
        ...
        return _start_delivery(["zeus", "-p", _self_profile_name(root), "peer", "dm", dm_target], content,
    ...
    return _start_delivery(["zeus", "-p", resolved, *BOT_CHAT_TURN_ARGS], content, f"@{_handle(resolved)}",
...
def cleanup_bot_dm_cache(max_age_hours: float = _DM_STALE_SECONDS / 3600, *, now: float | None = None) -> int:
    ...
    locations = [(temp_root, "zeus-dm-*.txt"), (temp_root, "zeus-relay-dm-*.txt")]
...
def _delivery_lock(argv: list[str], *, stdin_file: bool):
    ...
    if stdin_file or len(argv) < 3 or cli not in ("zeus", "zeus.exe") or argv[1] != "-p":
        return contextlib.nullcontext()
    from tools.bot_mode_probe import _zeus_root
    from tools.bot_relay import acquire_turn_lock

    return acquire_turn_lock(_zeus_root(Path(_default_home())), argv[2])
...
def _run_local_turn(argv: list[str], dm_file: str) -> int:
    ...
    reason = next((line.removeprefix("zeus-refusal-reason: ").strip()
                   for line in stderr_text.splitlines()
                   if line.startswith("zeus-refusal-reason: ")), None)
