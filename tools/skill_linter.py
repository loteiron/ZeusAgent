...
def _check_frontmatter(frontmatter: Dict[str, Any], skill_dir: Optional[Path]) -> Iterator[LintFinding]:
    ...
    zeus_meta = meta.get("zeus") if isinstance(meta, dict) else None
    if not isinstance(zeus_meta, dict):
        yield _warn("missing-metadata", "frontmatter is missing metadata.zeus.{tags, related_skills}.")
    elif "tags" not in zeus_meta:
        yield _warn("missing-metadata", "metadata.zeus.tags is missing.")
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("zeus", "zeusagent", "agent", "zeus agent") and (
        author != "ZeusAgent"):
        yield _warn("author-caps", f"author '{author}' should be 'ZeusAgent' (proper caps) "
