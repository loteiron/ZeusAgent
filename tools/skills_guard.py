...
_ZEUS_CONFIG_FILES = r'\.zeus/(?:config\.yaml|SOUL\.md)'
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.zeus/\.env|\~/\.zeus/\.env',
     "zeus_env_access", "critical", "exfiltration", "directly references ZeusAgent secrets file"),
    ...
    # quarantine only acts on "dangerous") but high for ZeusAgent/other config (setup docs routinely say
    ...
    (_prose_modify_re(_ZEUS_CONFIG_FILES),
     "zeus_config_mod", "high", "persistence", "modification language aimed at ZeusAgent configuration files (verify intent)"),
    (_shell_write_re(_ZEUS_CONFIG_FILES),
     "zeus_config_mod_shell", "critical", "persistence", "shell write (redirect/sed -i/tee/cp/mv) targeting ZeusAgent configuration files"),
    (r'\.zeus/config\.yaml|\.zeus/SOUL\.md',
     "zeus_config_ref", "low", "persistence", "references ZeusAgent configuration files (informational; only modification intent is scored)"),
...
# `.skillignore` is ZeusAgent-native; `.clawhubignore` is honored for skills published through ClawHub.
