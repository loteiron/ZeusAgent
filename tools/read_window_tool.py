#!/usr/bin/env python3
"""Read which OS window sits directly underneath the ZeusAgent desktop window.
...
def read_window_below_tool(callback: Optional[Callable] = None) -> str:
    """Return the window underneath the ZeusAgent window as a JSON string."""
    return read_pane(callback, (), (
        "read_window_below is only available in the ZeusAgent desktop app.",
        "",
...
READ_WINDOW_BELOW_SCHEMA = {
    ...
    "description": (
        "Identify the app window directly behind the ZeusAgent desktop window "
