...
~/.zeus/skills/, tracking each synced skill's origin hash in .bundled_manifest (v2 "name:hash"
...
from zeus_constants import get_bundled_skills_dir, get_zeus_home, get_optional_skills_dir
...
# retarget ZEUS_HOME after import, and frozen constants would resolve (and for
...
# once under the launch ZEUS_HOME and later scope requests to a different profile via
# set_zeus_home_override(). See #65828.
_ZEUS_HOME_AT_IMPORT = ZEUS_HOME
...
def _zeus_home() -> Path:
    return _live(ZEUS_HOME, _ZEUS_HOME_AT_IMPORT, get_zeus_home)


def _skills_dir() -> Path:
    return _live(SKILLS_DIR, _SKILLS_DIR_AT_IMPORT, lambda: _zeus_home() / "skills")
...
# Written by `zeus profile create --no-skills` / installer `--no-skills`: sync seeds only
# essential skills. Mirrors zeus_cli.profiles.NO_BUNDLED_SKILLS_MARKER (no CLI import here).
NO_BUNDLED_SKILLS_MARKER = ".no-bundled-skills"


def _get_bundled_dir() -> Path:  # ZEUS_BUNDLED_SKILLS env first, then repo-relative
    ...
    for candidate in st.active_index.get(skill_name, []):
        ...
        if not _matches_origin_hash(candidate, origin_hash):  # moving a customized copy would edit user work
            st.say(
                ...
                f"Run `zeus skills reset {skill_name} --restore` to move to the new location.")
            continue
        try:
        ...
        if dest.exists():
            ...
            if _dir_hash(dest) == bundled_hash:
                ...
                st.say(
                    ...
                    f"by this name — yours was kept. Run `zeus skills reset {skill_name}` to "
...
def sync_skills(quiet: bool = False) -> dict:
    """Sync bundled skills into ~/.zeus/skills/ using the manifest; returns the per-category
    result dict. Opted-out profiles seed ONLY ESSENTIAL_SKILLS (the system prompt always
    points at ``zeus-agent``)."""
    essential_only = (_zeus_home() / NO_BUNDLED_SKILLS_MARKER).exists()
...
def _rmtree_writable(path: Path) -> None:
    ...
    child of the active skills root (bad join / missing ZEUS_HOME / malicious manifest entry).
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.zeus/skills/ ...")
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (_zeus_home() / NO_BUNDLED_SKILLS_MARKER).exists()
