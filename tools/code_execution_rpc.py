...
def _rpc_token_ok(request: dict, rpc_token: str) -> bool:
    ...
    if not isinstance(request, dict) or not rpc_token:
        return False
    supplied = request.get("token")
    if not isinstance(supplied, str):
        return False
    try:
        return secrets.compare_digest(supplied.encode(), rpc_token.encode())
    except UnicodeError:
        return False
...
def _handle_rpc_request(request: dict, *, allowed_tools: frozenset, tool_call_counter: list,
    ...
    if not isinstance(tool_name, str) or not isinstance(tool_args, dict):
        return tool_error("Invalid RPC request: tool must be a string and args must be an object")
    ...
    try:
        ...
        while True:
            ...
            while b"\n" in buf:
                ...
                # JSON may contain escaped lone surrogates after decoding user
                # input. Keep them escaped on the wire instead of losing service.
        ...
        try:
            ...
            for req_file in req_files:
                ...
                if not _rpc_token_ok(request, rpc_token):
                    ...
                    env.execute(f"rm -f {quoted_req_file}", cwd="/", timeout=5)
                    continue
                # Validate before dispatch: formatting an invalid sequence afterward
                # leaves the request queued and repeats its side effects on every poll.
                seq = request.get("seq", 0)
                if type(seq) is not int or seq < 0:
                    logger.debug("Invalid RPC sequence in %s", req_file)
                    env.execute(f"rm -f {quoted_req_file}", cwd="/", timeout=5)
                    continue
                ...
                encoded_result = base64.b64encode(
                    tool_result.encode("utf-8", errors="backslashreplace")).decode("ascii")
                env.execute(
