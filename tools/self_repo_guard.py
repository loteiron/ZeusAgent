...
def _block_message(operation: str, root: Path) -> str:
    ...
    scratch = (Path(zeus_home).expanduser() if zeus_home else Path.home() / ".zeus") / "scratch"
    return (
        f"Blocked: `{operation}` would rewrite ZeusAgent's live source checkout "
        ...
        "is pushed. To change this checkout, stop ZeusAgent, run the command externally, then restart "
        "ZeusAgent.")
