...
_zeus_config_resolved: str | None = None
_zeus_config_resolved_loaded = False
_real_zeus_home_cached: str | None = None
_real_zeus_home_loaded = False
...
def _zeus_home_real() -> str:
    from zeus_constants import get_zeus_home
    return os.path.realpath(str(get_zeus_home()))


def _get_zeus_config_resolved() -> str | None:
    """Return the resolved absolute path of the ZeusAgent config file (cached)."""
    return _cached_lookup("_zeus_config_resolved", "_zeus_config_resolved_loaded", _config_path_resolved,
                          lambda: str(Path(_expand_tilde("~/.zeus/config.yaml")).resolve()))


def _get_real_zeus_home() -> str | None:
    """Return the realpath of the authoritative ZeusAgent home (cached)."""
    return _cached_lookup("_real_zeus_home_cached", "_real_zeus_home_loaded", _zeus_home_real,
                          lambda: os.path.realpath(_expand_tilde("~/.zeus")))
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    zeus_config = _get_zeus_config_resolved()
    if zeus_config and zeus_config in candidates:
        return (
            f"Refusing to write to ZeusAgent config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.zeus/config.yaml directly or use 'zeus config' instead.")
    return None
...
# vector (AGENTS.md / CLAUDE.md / SOUL.md / .cursorrules / project .zeus tree).
...
def _protected_instruction_config() -> tuple[bool, list[str]]:
    ...
    try:
        from zeus_cli.config import load_config, cfg_get
        cfg = load_config()
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # ~/.zeus itself is governed by its own guards (config.yaml hard-block,
    # mirror guard, write_approval); this gate targets PROJECT-LOCAL files only.
    # Must run before the ``.zeus`` component rule, which would match the home.
    real_home = _get_real_zeus_home()
    ...
    for candidate in (normalized, resolved):
        ...
        if base_lower in _PROTECTED_INSTRUCTION_BASENAMES or any(
            ...
            return base
        # Project-local .zeus config dirs (<repo>/.zeus/config.yaml) steer
        # behavior too. Only the IMMEDIATE parent counts — matching any ancestor
        # would gate every write inside a checkout living under ~/.zeus.
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".zeus":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side ZeusAgent mirror prefix for persistent Docker file tools."""
    try:
        ...
        if env is not None:
            ...
            return "/root/.zeus" if persistent_docker else None
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    ZeusAgent state (a write the host never reads). Not profile isolation — that guard was
