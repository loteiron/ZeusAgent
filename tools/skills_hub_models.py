...
def _zeus_tags(fm: dict) -> Any:
    """``metadata.zeus.tags`` from parsed frontmatter, or ``[]`` (unvalidated type)."""
    metadata = fm.get("metadata", {})
    zeus_meta = metadata.get("zeus", {}) if isinstance(metadata, dict) else None
    return zeus_meta.get("tags", []) if isinstance(zeus_meta, dict) else []
