...
_terminal_scope_var: ContextVar = ContextVar("zeus_terminal_scope", default=None)
...
def build_profile_terminal_scope(zeus_home: "Any") -> Dict[str, str]:
    ...
    """
    from zeus_cli.config import TERMINAL_CONFIG_ENV_MAP, _terminal_env_value
    ...
    if config_exists:
        from zeus_cli.config import fast_safe_load
...
def install_profile_terminal_scope(zeus_home: "Any") -> Token:
    """Build AND install a profile's policy; on failure install the refusal scope. Never raises."""
    try:
        return set_terminal_scope(build_profile_terminal_scope(zeus_home))
...
@contextmanager
def install_and_reset_profile_terminal_scope(zeus_home: "Any") -> Iterator[None]:
    """Install the profile's terminal policy for a bounded turn/fire. Never raises."""
    token = install_profile_terminal_scope(zeus_home)
    try:
        yield
    finally:
