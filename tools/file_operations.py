...
NOT_REGULAR_SENTINEL = "__zeus_not_regular__"
...
MISSING_SENTINEL = "__zeus_missing__"

_READ_SENTINEL_PREFIX = "__ZEUS_RF_"
_WRITE_SENTINEL_PREFIX = "__ZEUS_WF_"
...
class ShellFileOperations(LintMixin, SearchMixin, FileOperations):
    ...
    def _escape_native_tool_arg(self, arg: str) -> str:
        ...
        understand the MSYS ``/c/...`` form and ZeusAgent disables MSYS argument
    ...
    def _try_read_utf16(self, path: str, offset: int, limit: int,
        ...
        snippet = (
            ...
            "    if size > MAX:\n"
            "        print('ZEUS_UTF16:NO'); sys.exit(0)\n"
            "    with open(p, 'rb') as f:\n"
            "        data = f.read()\n"
            "    sample = data[:SAMPLE]\n"
            "    enc = None\n"
        ...
        marker = stdout.find("ZEUS_UTF16:OK")
        if result.exit_code != 0 or marker < 0:
            return None
        payload = stdout[marker + len("ZEUS_UTF16:OK"):].strip()
        try:
    ...
    def _native_read_enabled(self) -> bool:
        ...
        file_operations holds Git-Bash-style paths there). ``ZEUS_NATIVE_FILE_READ=0``
        turns the fast path off."""
        flag = os.environ.get("ZEUS_NATIVE_FILE_READ", "1").strip().lower()
