...
def _capture_required_environment_variables(
    ...
    if _is_gateway_surface() and not env_var_enabled("ZEUS_INTERACTIVE"):
        try:
            ...
            hint = (f"Secure secret entry is not available. Load this skill in the local CLI to be "
                    f"prompted, or add the key to {display_zeus_home()}/.env manually.")
...
def _is_gateway_surface() -> bool:
    if env_var_enabled("ZEUS_GATEWAY_SESSION"):
        return True
    from gateway.session_context import get_session_env
    return bool(get_session_env("ZEUS_SESSION_PLATFORM"))
