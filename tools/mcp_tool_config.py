...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    for key in ("ZEUS_KANBAN_DB", "ZEUS_KANBAN_BOARD"):
...
def _node_fallback(command: str) -> str:
    ...
    zeus_home = os.path.expanduser(os.getenv("ZEUS_HOME", os.path.join(home, ".zeus")))
    # /usr/local/bin: canonical Node location (from-source Linux, ZeusAgent Docker image, Intel Homebrew),
    # needed when a hand-authored env.PATH omits it — npx's shebang re-execs /usr/bin/env node.
    candidates = (os.path.join(zeus_home, "node", "bin", command), os.path.join(home, ".local", "bin", command),
...
def _npx_bin_candidates(bin_dir: str, name: str, *, windows: Optional[bool] = None) -> list:
    ...
    extension (same precedence as ``zeus_constants._candidate_node_command_names``). ``windows``
...
def _npx_cached_bin(args: list) -> Optional[tuple]:
    ...
    for nothing (~48 MB private memory per MCP server, measured); ZeusAgent already supervises the
