...
CHECKPOINT_PATH = get_zeus_home() / "processes.json"
        ...
        if _IS_LINUX:
            try:
                ...
                if binary:
                    ...
                    probe_unit = f"zeus-probe-scope-{os.getpid()}-{uuid.uuid4().hex[:8]}"
...
def _is_supervised_gateway_process() -> bool:
    """Whether this process is the live, supervised ZeusAgent gateway itself.
    Supervisor markers and ``_ZEUS_GATEWAY`` are inherited by every descendant (and
    ...
    if os.environ.get("_ZEUS_GATEWAY") != "1":
        return False
    try:
...
def _build_systemd_scope_argv(shell_argv: List[str], unit_suffix: str) -> List[str]:
    ...
    if binary is None:
        ...
        return shell_argv
    return _systemd_scope_argv(binary, f"zeus-worker-{unit_suffix}", *shell_argv)
...
class ProcessRegistry(ProcessCheckpointMixin):
    ...
    def _config_value(section: str, key: str, fallback):
        ...
        from zeus_cli.config import DEFAULT_CONFIG, cfg_get, read_raw_config
    ...
    def _scope_argv(self, session: ProcessSession, safe_command: str, unit_suffix: str, label: str) -> List[str]:
        ...
        if in_supervised_gateway and _systemd_run_user_scope_available():
            session.systemd_unit = f"zeus-worker-{unit_suffix}.scope"
    ...
    def is_session_waiting(self, session_id: str) -> bool:
        """Whether a goal loop (``zeus_cli.goals`` wait barrier) should stay parked on
    ...
    def wait_for_pending_completions(
        ...
        A one-shot CLI run (``zeus -q/-Q/-z``) exits when its turn ends; a background
        ...
        Bot Mode handoff REPLIES are the visible casualty (#90879): a recipient invoked as ``zeus -p <bot>
    ...
    def _reconcile_local_exit(self, session: "ProcessSession") -> None:
        ...
        descendant (e.g. a daemon from ``zeus update``) holds the pipe open, poll()
        ...
        process (e.g. a daemon spawned by `zeus update` restarting the gateway) is still holding the
    ...
    def request_close_terminal(self, session_id: str) -> dict:
        ...
        if self.on_close is None:
            return {"status": "error", "error": "close_terminal is only available in the ZeusAgent desktop app."}
