...
# Sensitive write targets, matched via ~ / $HOME / $ZEUS_HOME spellings. The resolved absolute
# home is folded into these forms at detection time by _normalize_command_for_detection(), so no
# import-time path snapshot (stale once ZEUS_HOME is set after import) lives in the patterns.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_ZEUS_ENV_PATH = (
    r'(?:~\/\.zeus/|(?:\$home|\$\{home\})/\.zeus/|(?:\$zeus_home|\$\{zeus_home\})/)' r'\.env\b'
)
# ~/.zeus/config.yaml IS the security policy (approvals.mode, yolo, allowlist) and the config cache is mtime-keyed,
# so a write takes effect mid-session. Terminal-side coverage (sed -i, tee, >, cp) pairs the file_tools deny.
_ZEUS_CONFIG_PATH = (
    r'(?:~\/\.zeus/|(?:\$home|\$\{home\})/\.zeus/|(?:\$zeus_home|\$\{zeus_home\})/)' r'config\.yaml\b'
)
...
_SENSITIVE_WRITE_TARGET = (
    rf'(?:{_SYSTEM_CONFIG_PATH}|/dev/sd|{_SSH_SENSITIVE_PATH}|{_ZEUS_ENV_PATH}|{_ZEUS_CONFIG_PATH}|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]zeus[^\n]*\.env\b', "access to ZeusAgent secrets (Windows path)"),
    # ── end of Windows tier
    ...
    # between `zeus` and `gateway` (`zeus -p ade gateway restart`) are allowed so a profile flag can't slip past.
    (r'\bzeus\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart zeus gateway (kills running agents)"),
    (r'\bzeus\s+update\b', "zeus update (restarts gateway, kills running agents)"),
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart zeus-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart zeus-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(zeus|gateway|cli\.py)\b', "kill zeus/gateway process (self-termination)"),
    ...
    # launchctl-driven gateway stop/restart on macOS (label `ai.zeus.gateway`). Two independent lookaheads, NOT a
    # sequential match: a for-loop building the label from a list defined EARLIER (`for item in 'ai.zeus...'; do
    # launchctl bootout "$label"`) never has "zeus" after the verb, and that slipped past and restarted 4 gateways
    # with zero approval. Erring broad is correct for an approval gate: an extra prompt is cheap.
    (r'(?=[\s\S]*\blaunchctl\s+(?:stop|kickstart|bootout|unload|kill|disable|remove)\b)(?=[\s\S]*\b(?:zeus|ai\.zeus)\b)', "stop/restart zeus launchd service (kills running agents)"),
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a credential/SSH/shell-rc/ZeusAgent file (key implant, login-time
    ...
    # ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files, ~/.zeus/config.yaml/.env), but cp/mv/install was
    ...
    # sed -i on ZeusAgent config/.env bypasses the redirection/tee rules; pairs the file_tools
    # write_file/patch deny so the terminal side is not an open door.
    # In-place edit of a ZeusAgent-managed security file (~/.zeus/config.yaml or .env). sed -i bypasses the
    # redirection/tee patterns above because it mutates the file directly. See #14639.
    (rf'\bsed\s+-[^\s]*i.*(?:{_ZEUS_CONFIG_PATH}|{_ZEUS_ENV_PATH})', "in-place edit of ZeusAgent config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_ZEUS_CONFIG_PATH}|{_ZEUS_ENV_PATH})', "in-place edit of ZeusAgent config/env (long flag)"),
    ...
    # coverage from #14639.
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_ZEUS_CONFIG_PATH}|{_ZEUS_ENV_PATH})', "in-place edit of ZeusAgent config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    command = re.sub(r'\\\r?\n', '', command)
    # Fold absolute user/ZeusAgent home prefixes to ~/ and ~/.zeus/ so the static patterns catch /home/alice/.bashrc
    # and C:\Users\alice\.bashrc. Resolved at detection time (not import time) so it tracks HOME/ZEUS_HOME set
    # later. MUST run before the backslash strip (which would dissolve C:\Users\alice to C:Usersalice). ZeusAgent home
    # first: on Windows it nests under the user home, and folding the user home first would eat the prefix it needs.
    command = _rewrite_resolved_zeus_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    bare home is never folded. Filesystem roots and relative paths cannot identify a user home;
    a single absolute directory such as /root can. Match only at a path-token boundary."""
    components = [c for c in re.split(r"[/\\]+", path) if c] if path else []
    drive_path = bool(re.match(r"^[A-Za-z]:[/\\]", path))
    rooted_path = path.startswith(("/", "\\"))
    if not components or (drive_path and len(components) < 2) or not (drive_path or rooted_path):
        return None
    # A suffix such as /backup/root is not the absolute home /root. Rewriting it
    # would both misclassify the command and broaden suggested approval patterns.
    boundary = r"(?<![\w./\\:~-])"
    prefix = "" if drive_path else r"[/\\]+"
    flags = re.IGNORECASE if drive_path or path.startswith("\\\\") else 0
    return re.compile(boundary + prefix + r"[/\\]+".join(re.escape(c) for c in components) + _PATH_TAIL, flags)
...
def _rewrite_resolved_zeus_home(command: str) -> str:
    """Resolved ZEUS_HOME (and its realpath) -> ``~/.zeus/`` so the _ZEUS_CONFIG_PATH /
    _ZEUS_ENV_PATH rules match Docker/gateway deployments that spell the absolute path."""
    try:
        ...
        return command
    return _fold_home_prefixes(command, paths, "~/.zeus")
...
_GATEWAY_LIFECYCLE_SPLICE_DESCRIPTION = "stop/restart zeus gateway via shell-spliced verb (kills running agents)"
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one ZeusAgent ad-hoc temp script."""
    try:
    ...
    return (
        ...
        and re.fullmatch(r"zeus-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
    )
...
def _is_shell_token_spliced_gateway_lifecycle(command: str) -> bool:
    ...
    verb is an ARGUMENT, so ``launchctl kick"start" -k gui/501/ai.zeus.gateway`` auto-approved.
    Delegates to ``cron.lifecycle_guard`` (shlex-tokenized, anchored on a zeus-gateway
