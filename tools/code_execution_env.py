...
# prefix, the exact-name ZEUS_ allowlist, or (Windows) an OS-essential name.
# The broad "ZEUS_" prefix is deliberately NOT safe — it leaked config vars
# without a secret substring (ZEUS_BASE_URL, ZEUS_KANBAN_DB, *_WEBHOOK).
# ZEUS_RPC_SOCKET / ZEUS_RPC_DIR / TZ / HOME are injected after scrubbing.
...
# imports may read at import time. ZEUS_DELEGATED_CHILD_CONTEXT must ride
# along or a child that imports ZeusAgent code loses the Kanban mutation guard
# while still inheriting ZEUS_HOME.
_ZEUS_CHILD_ALLOWED = frozenset({
    "ZEUS_HOME", "ZEUS_PROFILE", "ZEUS_CONFIG", "ZEUS_ENV", "ZEUS_DELEGATED_CHILD_CONTEXT",
})
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    (4) operational ZEUS_* pass by exact name; (5) on Windows the
    ...
    scrubbed = {}
    # Non-secret ZEUS_* vars no allowlist admits are dropped on purpose; a script importing a
    ...
    for k, v in source_env.items():
        ...
        if (any(k.startswith(p) for p in _SAFE_ENV_PREFIXES)
                or k in _ZEUS_CHILD_ALLOWED
                or (is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS)):
            scrubbed[k] = v
        elif k.startswith("ZEUS_"):
            _dropped_zeus.append(k)
    if _dropped_zeus:
        logger.debug(
            "execute_code: dropped %d non-allowlisted ZEUS_* var(s) from the "
            ...
            len(_dropped_zeus), ", ".join(sorted(_dropped_zeus)),
        )
    ...
    # non-allowlisted ZEUS_* values in otherwise ordinary execution environments.
    if scoped.get(DELEGATED_CHILD_ENV_MARKER):
        for key in (DELEGATED_CHILD_ENV_MARKER, "ZEUS_KANBAN_DB", "ZEUS_KANBAN_BOARD"):
            if key in scoped:
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    from zeus_constants import apply_subprocess_home_env
    child_env = _scrub_child_env(os.environ)
    child_env["ZEUS_RPC_SOCKET"] = rpc_endpoint
    child_env["ZEUS_RPC_TOKEN"] = rpc_token
    ...
    # Only TZ reaches the child; ZEUS_TIMEZONE is an internal setting.
    _tz_name = os.getenv("ZEUS_TIMEZONE", "").strip()
    if _tz_name:
        child_env["TZ"] = _tz_name
    child_env.pop("ZEUS_TIMEZONE", None)
    apply_subprocess_home_env(child_env)
    # PYTHONPATH: the staging dir (zeus_tools.py) must always be importable even when project
    # mode changes CWD. ZeusAgent's root is added ONLY when the child runs in ZeusAgent's Python env —
    # exposing ZeusAgent's site-packages to an external interpreter can mix incompatible compiled
    # extensions (3.12 NumPy under a 3.9 venv). Inherited ZeusAgent-owned entries are stripped first.
    # Before re-injecting PYTHONPATH, strip ZeusAgent-owned entries that leaked through _scrub_child_env
    # (PYTHONPATH is in _SAFE_ENV_PREFIXES so it passes the scrub). They are redundant for same-ZeusAgent-
    ...
    _strip_zeus_owned_pythonpath(child_env)
    _existing_pp = child_env.get("PYTHONPATH", "")
    _pp_parts = [tmpdir]
    if _uses_zeus_python_environment(child_python):
        _pp_parts.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    elif child_python not in _external_env_logged:
        # Surface once per interpreter so "import zeus_constants fails" is diagnosable.
        _external_env_logged.add(child_python)
        logger.info("execute_code: child interpreter %s is outside the ZeusAgent "
                    "environment; zeus root omitted from PYTHONPATH", child_python)
    if _existing_pp:
...
# Interpreter paths already reported as outside the ZeusAgent environment.
...
def _uses_zeus_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to ZeusAgent's active Python environment. Short-circuits when
    it IS the running interpreter (by path or realpath — covers ``uv run`` venvs) so no probe
    runs on the default strict path and a flaky probe can never drop the zeus root."""
