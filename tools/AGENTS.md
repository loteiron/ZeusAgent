...
For custom/local-only tools do NOT edit core: create `~/.zeus/plugins/<name>/plugin.yaml` +
...
1. `tools/your_tool.py`:
   ...
   ```
2. `toolsets.py`: add the name to `_ZEUS_CORE_TOOLS` (all platforms) or a new toolset. **Required**
   — discovery registers the schema, but a tool is only exposed if a toolset names it.
   `_ZEUS_CORE_TOOLS` is the default bundle every platform's base toolset inherits, not dead code.
...
- **Paths in schema descriptions use `display_zeus_home()`** (schema is built at import, after
  `_apply_profile_override()` set `ZEUS_HOME`). **State files use `get_zeus_home()`**, never
  `Path.home()/.zeus`, so each profile gets its own state.
...
- New tools integrate with existing setup UX (`zeus tools`, `zeus setup`, auto-install) rather
  than a raw env var; secrets go in `OPTIONAL_ENV_VARS` (`zeus_cli/AGENTS.md`).
...
`zeus tools` (curses) or `tools.<platform>.enabled/disabled` in config.yaml. `browser_exec`
...
imports against a temp `ZEUS_HOME` (see `tests/tools/test_approval_config_readonly.py`).
