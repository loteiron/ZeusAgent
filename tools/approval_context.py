...
# ZeusAgent session id (observability identity, distinct from the gateway routing session_key), forwarded to approval
...
# os.environ["ZEUS_INTERACTIVE"] races: one session's `finally` restore can clobber another's set mid-run, dropping
...
_zeus_interactive_ctx: contextvars.ContextVar[str | None] = _ctx("zeus_interactive", None)


def set_zeus_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context instead of mutating os.environ."""
    return _zeus_interactive_ctx.set("1" if interactive else "")


def reset_zeus_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_zeus_interactive_context`."""
    _zeus_interactive_ctx.reset(token)


def _is_interactive_cli() -> bool:
    """True for an interactive CLI/ACP session (contextvar first, env fallback)."""
    ctx_val = _zeus_interactive_ctx.get()
    return is_truthy_value(ctx_val) if ctx_val is not None else env_var_enabled("ZEUS_INTERACTIVE")
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("ZEUS_SESSION_KEY", default)
...
def _is_unattended_platform_approval_context() -> bool:
    ...
    Webhook, msgraph_webhook, and api_server sessions bind ``ZEUS_SESSION_PLATFORM`` like chat gateways
...
def _is_single_query_approval_context() -> bool:
    """True for a single-query (-q) session: ``zeus chat -q`` exports
    ``ZEUS_INTERACTIVE=1`` (so sudo password prompts work) but nobody is waiting
    ...
    return is_truthy_value(_session_env("ZEUS_SINGLE_QUERY_SESSION"))


def _is_gateway_approval_context() -> bool:
    """True inside a gateway/API session that can answer an approval.

    Legacy integrations set ZEUS_GATEWAY_SESSION; concurrent paths bind
    ZEUS_SESSION_PLATFORM via contextvars. Cron is NEVER a gateway approval
    ...
    if _is_cron_approval_context() or _is_unattended_platform_approval_context():
        return False
    return env_var_enabled("ZEUS_GATEWAY_SESSION") or bool(_get_session_platform())
...
def _should_fall_through_to_cli_approval(*, is_cli: bool, approval_callback, notify_cb) -> bool:
    ...
    ``ZEUS_EXEC_ASK`` (or a platform marker) can leak into an interactive CLI
