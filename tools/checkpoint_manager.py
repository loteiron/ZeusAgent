...
config / ``--checkpoints`` flag.  One store under ``~/.zeus/checkpoints/`` so git dedupes
blobs across projects (pre-v2 one-repo-per-workdir re-stored ~40 MB each): ``store/`` bare
repo with per-project ``refs/zeus/<hash16>``, ``indexes/<hash16>``, ``projects/<hash16>.json``
...
from zeus_cli.gitlock import clear_stale_tmp_packs
from utils import env_int

logger = logging.getLogger(__name__)

CHECKPOINT_BASE = get_zeus_home() / "checkpoints"

_STORE_DIRNAME, _INDEXES_DIRNAME, _PROJECTS_DIRNAME, _LEDGERS_DIRNAME = "store", "indexes", "projects", "ledgers"
_REFS_PREFIX, _LEGACY_PREFIX, _PRUNE_MARKER_NAME = "refs/zeus", "legacy-", ".last_prune"
...
DEFAULT_EXCLUDES = [
    ...
    ".git/", ".hg/", ".svn/", ".worktrees/",  # VCS + worktrees (ZeusAgent convention — don't snapshot siblings)
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("ZEUS_CHECKPOINT_TIMEOUT", 30)))
...
_STORE_GIT_CONFIG = (("user.email", "zeus@local"), ("user.name", "ZeusAgent Checkpoint"),
...
def _load_ledger(store: Path, dir_hash: str) -> Dict[str, Dict]:
    ...
    ``write_file``/``patch`` produced, so restores can tell ZeusAgent' writes from later user edits."""
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    users may want to recover; the archive falls under retention and ``zeus checkpoints
    ...
    logger.info("Migrated pre-v2 checkpoint repos to %s. "
                "Clear with `zeus checkpoints clear-legacy` when safe.", legacy_root)
    return legacy_root
...
def _pre_v2_shadow_repos(base: Path) -> List[Dict]:
    ...
    for child in base.iterdir() if base.exists() else ():
        ...
        try:
            if (child / "ZEUS_WORKDIR").exists():
                workdir = (child / "ZEUS_WORKDIR").read_text(encoding="utf-8").strip()
...
class CheckpointManager:
    ...
    def record_agent_write(self, file_path: str) -> None:
        """Record the content hash of a file ZeusAgent just wrote (agent-write ledger), so safe-mode
    ...
    def safe_restore_plan(self, working_dir: str, commit_hash: str) -> Dict:
        """Classify files changed since ``commit_hash``: ``restore`` = still matching what ZeusAgent
        last wrote (or deleted since); ``skipped`` = user-edited afterwards or never written by
        ZeusAgent.  ``ledger_empty`` => no ledger, callers fall back to a full restore."""
        ...
        for rel in filter(None, names_out.split("\x00")):
            ...
            zeus_authored = recorded is not None and _hash_file(abs_path) in (None, recorded)
            out["restore" if zeus_authored else "skipped"].append(rel)
    ...
    def restore(self, working_dir: str, commit_hash: str, file_path: str = None,
        ...
        the user hand-edited after ZeusAgent' last write untouched (agent-write ledger); the result
    ...
    def _apply_safe_restore_deletes(self, p: _ProjectRefs, commit_hash: str,
        ...
        from the checkpoint is ZeusAgent-created (delete to restore) — unless ``max_file_size_mb`` kept
...
def auto_prune_from_config() -> Dict[str, object]:
    ...
    cleanup is only via explicit ``zeus checkpoints prune``. Never raises."""
    try:
