...
GitHub releases to $ZEUS_HOME/bin/tirith in a background thread -- SHA-256 always verified,
...
def _failure_marker_path() -> str:
    return os.path.join(str(get_zeus_home()), ".tirith-install-failed")
...
# --- Auto-install ---
def _zeus_bin_dir() -> str:
    """$ZEUS_HOME/bin, created if needed."""
    os.makedirs(d := os.path.join(str(get_zeus_home()), "bin"), exist_ok=True)
    return d
...
def _download_file(url: str, dest: str, timeout: int = 10):
    # These are public release assets. Do not attach ambient GitHub credentials:
    # urllib may carry Authorization through the release CDN redirect as well.
    # The GET has no body and never includes commands, files, or environment data.
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ZEUS_HOME/bin/tirith -> ``(installed_path,
    ...
    try:
        ...
        if src is None:
            return None, reason
        dest = os.path.join(_zeus_bin_dir(), "tirith")
        try:
...
def _find_local_tirith() -> str | None:
    """Cheap local lookup for the default "tirith": PATH, then $ZEUS_HOME/bin."""
    zeus_bin = os.path.join(_zeus_bin_dir(), "tirith")
    return shutil.which("tirith") or (zeus_bin if _is_executable(zeus_bin) else None)
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    $ZEUS_HOME/bin/tirith → install; failures cached in-process and on disk for 24h). On a miss
...
def check_command_security(command: str) -> dict:
    ...
    if _circuit_open:
        return _fail(cfg["tirith_fail_open"], "tirith disabled (circuit breaker)",
                     "tirith unavailable (circuit breaker, fail-closed)")
