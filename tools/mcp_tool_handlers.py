...
_NEEDS_REAUTH_MSG = (
    "MCP server '{s}' requires re-authentication. Run `zeus mcp login {s}` (or delete the tokens file under "
    "~/.zeus/mcp-tokens/ and restart). Do NOT retry this tool — ask the user to re-authenticate.")
...
def _check_circuit_breaker(server_name: str) -> Optional[str]:
    ...
    if _core._server_errors_all_application.get(server_name):
...
def _render_content_blocks(result, server_name: str) -> Tuple[str, int]:
    ...
    # cache those via the gateway's image-cache helper so they flow through ZeusAgent' MEDIA: tag convention
    ...
    # (gnanirahulnutakki), both too stale to cherry-pick. #10848's approach (integrate with ZeusAgent' MEDIA
