...
class MCPServerHealthMixin:
    ...
    def _make_logging_callback(self):
        """``logging_callback`` forwarding server ``notifications/message`` into ZeusAgent logging (SDK default drops them).

        Routes MCP ``notifications/message`` log notifications from the server into ZeusAgent' logging
        (agent.log via zeus_logging), tagged with the server name. Without this, the SDK's default
