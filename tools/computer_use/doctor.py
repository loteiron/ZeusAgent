"""`zeus computer-use doctor` — thin client for cua-driver's `health_report` MCP tool. cua-driver owns the health
...
def _build_identity(binary: str, report: Report) -> Report:
    """ZeusAgent-side identity block comparing resolved binary vs health_report."""
...
def run_doctor(driver_cmd: Optional[str] = None, *, include: Sequence[str] = (), skip: Sequence[str] = (), json_output: bool = False,
    ...
    if json_output:
        # Additive envelope: upstream keys preserved, identity under zeus_identity (and environment under
        # zeus_environment when present) so overall/checks parsers keep working.
        payload = {**report, "zeus_identity": identity}
        if environment:
            payload["zeus_environment"] = environment
