...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        try:
            from zeus_constants import get_zeus_home as _get_zeus_home
            _zeus_home = _get_zeus_home()
        except (ModuleNotFoundError, ImportError):
            _zeus_home = _Path.home() / ".zeus"
        self._thread_count_store = _ThreadCountStore(_zeus_home / "google_chat_thread_counts.json")
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        from zeus_constants import get_zeus_home as _get_zeus_home
        return _get_zeus_home() / "google_chat_bot_id.json"
    ...
    async def _build_message_event(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> Optional[MessageEvent]:
        """Parse a Chat API message into a zeus MessageEvent."""
    ...
    async def send_clarify(
        ...
        def _button(text: str, choice: str) -> Dict[str, Any]:
            return {"text": text, "action": "zeus_clarify", "parameters": {"clarify_id": clarify_id, "choice": choice}}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'ZeusAgent is thinking…' marker (Chat has no typing API); ``send()``
        ...
        body = _thread_body(getattr(self.config, "typing_status_text", None) or "ZeusAgent is thinking…", thread_id)
...
Walkthrough:
  ...
  3. Create a Pub/Sub topic (e.g. zeus-chat-events) and a Pull subscription.
...
def interactive_setup() -> None:
    """``zeus setup`` wizard: print GCP instructions, prompt for env vars, persist to ``~/.zeus/.env``."""
    from zeus_cli.cli_output import print_info, print_success, print_warning, prompt, prompt_yes_no
    ...
    print()
    print_success("Google Chat configuration saved to ~/.zeus/.env")
    print_info("Restart the gateway: zeus gateway restart")
...
async def _standalone_send(
    ...
    gateway runner is not in-process, e.g. ``zeus cron``). Needs SA credentials and a
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `zeus setup` to install Google Chat support.",
        ...
        platform_hint=(
            ...
            "confirmations instead. While you are generating a response, a 'ZeusAgent is thinking…' marker message "
