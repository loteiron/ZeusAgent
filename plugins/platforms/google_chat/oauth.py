...
legacy single-user mode). Files under ``${ZEUS_HOME}``: ``google_chat_user_tokens/
...
from zeus_constants import display_zeus_home, get_zeus_home
...
# human-readable under ``ls ~/.zeus/google_chat_user_tokens/``.
...
def _token_rel(email: Optional[str]) -> str:
    """ZEUS_HOME-relative token file: per-user under the tokens dir, else the legacy path."""
...
def _user_tokens_dir() -> Path:
    return get_zeus_home() / "google_chat_user_tokens"


def _token_path(email: Optional[str] = None) -> Path:
    """Per-user token path for ``email``, or the legacy single-user path."""
    return get_zeus_home() / _token_rel(email)


def _client_secret_path() -> Path:
    return get_zeus_home() / "google_chat_user_client_secret.json"


def _pending_auth_path(email: Optional[str] = None) -> Path:
    if email:
        return get_zeus_home() / "google_chat_user_oauth_pending" / f"{_sanitize_email(email)}.json"
    return get_zeus_home() / "google_chat_user_oauth_pending.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Run `zeus setup` to install Google Chat support."
        )
        return None
    try:
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("Run `zeus setup` to repair the managed installation, then retry.")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into ZEUS_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    print(f"Profile path: {display_zeus_home()}/{_token_rel(email)}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for ZeusAgent (native attachment delivery)"
    )
