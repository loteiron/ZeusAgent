...
Small Node helper that bridges ZeusAgent to Photon's Spectrum SDK
(`spectrum-ts`).  ZeusAgent is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Zeus-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and ZeusAgent can receive inbound messages
...
The ZeusAgent plugin's `zeus photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. ZeusAgent is Python, so replies go through this sidecar
