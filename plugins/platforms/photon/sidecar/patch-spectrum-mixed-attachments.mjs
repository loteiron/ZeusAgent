...
// `message.content.text` before ZeusAgent can see it. We rewrite the two inbound
...
const MARKER = "ZeusAgent patch: Preserve mixed text + attachment iMessage payloads";
...
export function patchSpectrumTs(root = scriptDir()) {
  ...
  for (const file of files) {
    ...
    // ZeusAgent to patch; keep the legacy v8 path below for older pinned installs.
    if (
