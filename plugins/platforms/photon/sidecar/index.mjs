// ZeusAgent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> ZeusAgent): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (ZeusAgent -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Zeus-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `zeus photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so ZeusAgent can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "zeus-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply ZeusAgent'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local ZeusAgent allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of ZeusAgent, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-zeus-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when ZeusAgent supplied them so a known-good
