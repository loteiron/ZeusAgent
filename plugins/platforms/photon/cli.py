"""``zeus photon ...`` CLI subcommands (registered via ``ctx.register_cli_command()``):
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `zeus photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None, help="Project name (default: 'ZeusAgent')")
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    try:
        from zeus_cli.config import write_platform_config_field
    ...
    print("\n✓ Photon setup complete.\n  Start the gateway:  zeus gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_deps_installed() else '✗ run `zeus photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`zeus photon telemetry on|off`)")
    return 0


def _telemetry_enabled() -> bool:
    """PHOTON_TELEMETRY from env / ~/.zeus/.env; truthy set mirrors the sidecar's."""
    try:
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `zeus photon telemetry on` / `zeus photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.zeus/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  zeus gateway restart")
    return 0
...
def gateway_setup() -> None:
    """Run Photon first-time setup from the unified `zeus gateway setup` wizard (same flow
    as ``zeus photon setup``; phone is prompted when stdin is a TTY)."""
