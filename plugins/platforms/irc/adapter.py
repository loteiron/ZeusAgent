"""IRC Platform Adapter for ZeusAgent — stdlib asyncio only, zero external dependencies.

config.yaml ``gateway.platforms.irc.extra`` keys: server, port (6697), nickname (zeus-bot), channel,
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = _env_or_extra(extra, "IRC_NICKNAME", "nickname", "zeus-bot")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :ZeusAgent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            with contextlib.suppress(Exception):
                await self._send_raw("QUIT :ZeusAgent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        elif command == "433":  # ERR_NICKNAMEINUSE — retry: zeus_, hermes_1, hermes_2...
...
def interactive_setup() -> None:
    """`zeus gateway setup` flow (lazy zeus_cli imports keep the plugin importable outside the CLI)."""
    ...
    if existing_server:
        ...
        if not prompt_yes_no("Reconfigure IRC?", False):
            return
    info("Connect ZeusAgent to an IRC network. Uses Python stdlib — no extra packages needed.",
    ...
    for label, env, what in (("Bot nickname (e.g. zeus-bot)", "IRC_NICKNAME", "Nickname"),
                             ("Channel to join (e.g. #zeus — comma-separate for multiple)", "IRC_CHANNEL", "Channel")):
    ...
    print()
    print_success("IRC configuration saved to ~/.zeus/.env")
    print_info("Restart the gateway for changes to take effect: zeus gateway restart")
...
async def _sa_register(conn: _StandaloneConn, nick_base: str, server_password: str) -> Optional[Dict[str, Any]]:
    ...
    await conn.raw(f"NICK {standalone_nick}")
    await conn.raw(f"USER {standalone_nick} 0 * :ZeusAgent (cron)")
...
async def _standalone_send(pconfig, chat_id: str, message: str, *, thread_id: Optional[str] = None,
    ...
    nick_base = _env_or_extra(extra, "IRC_NICKNAME", "nickname", "zeus-bot").rstrip("_0123456789-")[:24] or "zeus-bot"
