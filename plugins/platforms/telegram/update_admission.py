...
ZeusAgent home, so a replacement adapter or a restarted gateway still drops updates that
...
def bounded_put(mapping, key, value, capacity):
    """Bound completed history without a high-watermark (Telegram IDs can decrease)."""
    if key not in mapping and len(mapping) >= capacity:
        for stale in list(mapping)[:max(1, capacity // 2)]:
