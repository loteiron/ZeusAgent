...
(enabled/policy/allow_from) falling back independently. Config ~/.zeus/feishu_comment_rules.json (mtime-cached,
hot-reload); pairing store ~/.zeus/feishu_comment_pairing.json."""
...
# long after profile/ZEUS_HOME overrides have been applied, so freezing is safe.
RULES_FILE = get_zeus_home() / "feishu_comment_rules.json"
PAIRING_FILE = get_zeus_home() / "feishu_comment_pairing.json"
...
def _main() -> int:
    try:
        __import__("zeus_cli.env_loader", fromlist=["load_zeus_dotenv"]).load_zeus_dotenv()
    except Exception:
        pass
