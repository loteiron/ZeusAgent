# A2A — Agent-to-Agent protocol for ZeusAgent
...
peer (another ZeusAgent, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
zeus gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, ZeusAgent serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.zeus/a2a_audit.jsonl`.
- Conversations persist to `~/.zeus/a2a_conversations/` — they survive context
