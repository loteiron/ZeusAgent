...
catalog, filter to the ``zeus`` integration, apply a manifest via
...
def fetch_zeus_templates(url: str | None = None) -> list[dict]:
    """Return catalog entries tagged for the ``zeus`` integration."""
    catalog = _get_json(url or catalog_url())
    entries = catalog.get("templates", []) if isinstance(catalog, dict) else []
    return [e for e in entries if "zeus" in (e.get("integrations") or [])]
