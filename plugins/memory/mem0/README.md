...
Behavioral settings live in `$ZEUS_HOME/mem0.json` (set them via `zeus memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.zeus/.env`.
...
| `user_id` | `zeus-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh ZeusAgent session and call `mem0_search` — it connects to your server.
...
```bash
zeus memory setup mem0 --mode oss \
...
```bash
zeus memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$ZEUS_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.zeus/mem0_qdrant"}}
  }
}
```
...
```bash
zeus memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$ZEUS_HOME/mem0.json`).
