...
- OpenViking server running and reachable from ZeusAgent

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
ZeusAgent can identify older servers that expose the legacy status-only health
...
connection values into ZeusAgent, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.zeus/.env`; for a named profile use
...
OpenViking's server config is separate from ZeusAgent:
...
ZeusAgent-side provider config is read from environment variables in the active
profile's `.env`:
...
When `OPENVIKING_API_KEY` is set, ZeusAgent lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
ZeusAgent sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
ZeusAgent also sends `User-Agent: openviking-memory-zeus/<version>` on
OpenViking requests. This standard harness identifier contains the ZeusAgent
...
does not ask for a peer ID. Without a configured peer, ZeusAgent sends neither
...
order remains environment, linked OpenViking config, then ZeusAgent YAML. To use
...
on the old implicit `zeus` peer now use user memory for new writes. Without
...
Set `agent: zeus` to restore peer-scoped writes. Memories written at user
...
`viking_remember` creates a one-shot `zeus-remember-<random>` OpenViking
...
not advertised or used. The one-shot session is separate from the live ZeusAgent
...
Use the same OpenViking profile and credentials as ZeusAgent for manual recovery.
...
ZeusAgent built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| ZeusAgent action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under user memory, or the configured peer memory directory |

Built-in `replace` and `remove` operations are not mirrored because ZeusAgent
...
file URIs, such as
`viking://user/default/peers/zeus/memories/preferences/mem_abc123.md` (any
