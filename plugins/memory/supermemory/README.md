...
- Hosted: API key from [app.supermemory.ai/integrations?connect=zeus](http://app.supermemory.ai/integrations?connect=hermes)
...
Before running `zeus memory setup`, add the local endpoint to
...
Then run `zeus memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$ZEUS_HOME/supermemory.json`
...
| `container_tag` | `zeus` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `zeus-{identity}` → `zeus-coder`). |
...
`https://api.supermemory.ai`. ZeusAgent resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: zeus`, and document writes stamp
`metadata.sm_source: zeus`. This is a **functional routing key, not telemetry**:
it groups ZeusAgent-written memories into a dedicated "ZeusAgent" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per ZeusAgent profile:

```json
{
  "container_tag": "zeus-{identity}"
}
```

For a profile named `coder`, this resolves to `zeus-coder`. The default profile resolves to `zeus-default`. Without `{identity}`, all profiles share the same container.
