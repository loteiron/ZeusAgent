...
```bash
zeus memory setup honcho   # configure Honcho directly (works on a fresh install)
zeus memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
zeus config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.zeus/.env
```

> `zeus honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `zeus memory setup honcho`.
...
Set `"recallSync": true` in `$ZEUS_HOME/honcho.json` (at the root or under
`hosts.zeus`), or enable **Current-query recall** in the memory settings or
`zeus honcho setup`. An explicit host-level `false` overrides root-level `true`.
...
cycle (not per pass). Select a fast, inexpensive model under `zeus model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ZEUS_HOME/honcho.json` | Profile-local (isolated ZeusAgent instances) |
| 2 | `~/.zeus/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active ZeusAgent profile: `zeus` (default) or `zeus_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `zeus honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | ZeusAgent session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`zeus-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`zeus`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-zeus-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening zeus in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — ZeusAgent session ID (timestamp + hex). Every `zeus` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple ZeusAgent profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`zeus`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active ZeusAgent profile: `zeus` (default) or `zeus_<profile>` (e.g. `zeus -p coder` -> host key `zeus_coder`). Older `zeus.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `ZEUS_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `zeus-agent`) |
...
|---------|-------------|
| `zeus memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `zeus honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `zeus memory setup`) |
| `zeus honcho status` | Show resolved config for active profile |
| `zeus honcho enable` / `disable` | Toggle Honcho for active profile |
| `zeus honcho mode <mode>` | Change recall or observation mode |
| `zeus honcho peer --user <name>` | Update user peer name |
| `zeus honcho peer --ai <name>` | Update AI peer name |
| `zeus honcho tokens --context <N>` | Set context token budget |
| `zeus honcho tokens --dialectic <N>` | Set dialectic max chars |
| `zeus honcho map <name>` | Map current directory to a session name |
| `zeus honcho sync` | Create host blocks for all ZeusAgent profiles |
