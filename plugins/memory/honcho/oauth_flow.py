...
core (the code can arrive via the loopback listener here or a ``zeus://``
...
# Loopback redirect registered for the ZeusAgent OAuth client. IP-literal so the browser can't resolve the
...
_CALLBACK_HTML = _CALLBACK_PAGE.format(
    title="Honcho connected", body="Connected to Honcho. You can close this tab and return to ZeusAgent."
).encode()
...
def start_loopback_flow_background(
    *, config_path: Path | None = None, host: str | None = None, source: str = "zeus-desktop",
    timeout: float = 300.0,
) -> dict[str, str]:
    ...
    global _flow_thread
    # Resolve under the caller's profile scope NOW — a context-local ZEUS_HOME override can't reach the worker.
