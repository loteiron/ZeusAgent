"""zeus-memory-store — holographic memory plugin (MemoryProvider): structured fact storage with entity
resolution, trust scoring, and HRR-based compositional retrieval. Original plugin by dusterbloom (PR #2351).
Config in $ZEUS_HOME/config.yaml under plugins.zeus-memory-store: db_path ($ZEUS_HOME/memory_store.db),
...
def _load_plugin_config() -> dict:
    try:
        from zeus_cli.config import load_config_readonly  # canonical: managed-scope overlay + ${VAR} expansion
        return cfg_get(load_config_readonly(), "plugins", "zeus-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, zeus_home):
        """Write config to config.yaml under plugins.zeus-memory-store."""
        config_path = Path(zeus_home) / "config.yaml"
        try:
            import yaml
            from zeus_cli.config import read_user_config_raw  # raw read: merged defaults must not be persisted
            existing = read_user_config_raw(config_path)
            existing.setdefault("plugins", {})["zeus-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        return [
            {"key": "db_path", "description": "SQLite database path", "default": f"{display_zeus_home()}/memory_store.db"},
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _zeus_home = str(get_zeus_home())
        db_path = self._config.get("db_path", _zeus_home + "/memory_store.db")
        if isinstance(db_path, str):  # expand $ZEUS_HOME so paths resolve to the active profile
            db_path = db_path.replace("$ZEUS_HOME", _zeus_home).replace("${ZEUS_HOME}", _zeus_home)
