...
A plugin MUST NOT modify `run_agent.py`, `cli.py`, `gateway/run.py`, `zeus_cli/main.py`, etc.
...
`zeus memory setup` → `provider.post_setup(zeus_home, config)`, never a parallel top-level
...
- **No new in-tree memory providers (May 2026).** `plugins/memory/` is closed (honcho, mem0,
  ...
  the same path, integrated via `zeus memory setup` / `post_setup()`.
- **No new third-party-product plugins (June 2026).** Observability/metrics backends, vendor SaaS
  ...
  (`~/.zeus/plugins/` or pip entry point) promoted in Discord `#plugins-skills-and-skins`. Reason:
...
until `zeus_cli.plugin_compat.COMPAT_REMOVAL_DATE`, when the commit that added them is reverted.
`zeus_cli/plugin_compat.py` is the single source: `scan_plugin` (AST scan), `compat_report`
(hits across enabled external plugins, cached to `ZEUS_HOME/.plugin-compat-report.json`,
refreshed by discovery), `removal_in_effect`, `warn_once`. Surfaces: CLI banner notice,
`zeus plugins compat` (shows affected user plugins), `zeus doctor`, post-update notices, the
...
(`scripts/check_compat_pointers.py` in CI; `-W error::zeus_cli.plugin_compat.ZeusAgentPluginCompatWarning`).
External-plugin compat is handled ONCE here — never add per-PR re-export shims.

## Tests

`tests/plugins/`. Load through real discovery with a temp `ZEUS_HOME`; assert behaviour (tool
