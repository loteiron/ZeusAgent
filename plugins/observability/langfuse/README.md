...
This plugin ships bundled with ZeusAgent but is **opt-in** — it only loads when
...
zeus tools  # → Langfuse Observability
...
Set these in `~/.zeus/.env` (or via `zeus tools`):
...
zeus chat -q "hello"              # then check Langfuse for a "ZeusAgent turn" trace
```

Generation observations include the ZeusAgent system prompt when the provider
uses a separate `system` param (Anthropic Messages API). Open an **LLM call**
child span to inspect `role: system` (truncated via `ZEUS_LANGFUSE_MAX_CHARS`).

## Optional tuning

```bash
ZEUS_LANGFUSE_ENV=production       # environment tag
ZEUS_LANGFUSE_RELEASE=v1.0.0       # release tag
ZEUS_LANGFUSE_SAMPLE_RATE=0.5      # sample 50% of traces
ZEUS_LANGFUSE_MAX_CHARS=12000      # max chars per field (default: 12000)
ZEUS_LANGFUSE_CAPTURE=sanitized    # content capture mode (see below)
ZEUS_LANGFUSE_DEBUG=true           # verbose plugin logging
```

## Capture modes

`ZEUS_LANGFUSE_CAPTURE` controls how much *content* (prompts, responses,
...
```bash
zeus plugins disable observability/langfuse
```
