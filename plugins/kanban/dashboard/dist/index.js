/**
 * ZeusAgent Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.zeus/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__ZEUS_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__ZEUS_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "zeus-kanban-dot-triage",
    todo: "zeus-kanban-dot-todo",
    ready: "zeus-kanban-dot-ready",
    running: "zeus-kanban-dot-running",
    blocked: "zeus-kanban-dot-blocked",
    review: "zeus-kanban-dot-review",
    done: "zeus-kanban-dot-done",
    archived: "zeus-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-zeus-task";
  ...
  const LS_BOARD_KEY = "zeus.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="zeus-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])zeus-kanban-md-code\1/i.test(attrs)) {
      return ' class="zeus-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "zeus-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "zeus-kanban-md",
  ...
  // standard `drop` event and our `zeus-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("zeus-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("zeus-kanban-column--drop");
          if (target) target.classList.add("zeus-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("zeus-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("zeus-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("zeus-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__ZEUS_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "zeus-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "zeus-kanban-attention",
        "zeus-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "zeus-kanban-attention-bar" },
        h("span", { className: "zeus-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "zeus-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "zeus-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "zeus-kanban-attention-row",
                  "zeus-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "zeus-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "zeus-kanban-attention-row-id" }, task.id),
                h("span", { className: "zeus-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "zeus-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "zeus-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "zeus-kanban-diag-action-btn",
      action.suggested ? "zeus-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " zeus-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".zeus-kanban-drawer-comment-row input, .zeus-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "zeus-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("zeus-kanban-diag", sevClass) },
      h("div", { className: "zeus-kanban-diag-header" },
        h("span", { className: "zeus-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "zeus-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "zeus-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "zeus-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "zeus-kanban-diag-data-row" },
                  h("span", { className: "zeus-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "zeus-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "zeus-kanban-diag-data-row" },
                h("span", { className: "zeus-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "zeus-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "zeus-kanban-diag-reassign-row" },
            h("span", { className: "zeus-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "zeus-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "zeus-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "zeus-kanban-diag-msg",
              msg.ok ? "zeus-kanban-diag-msg--ok" : "zeus-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "zeus-kanban-section" },
      h("div", { className: "zeus-kanban-section-head-row" },
        h("span", { className: "zeus-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "zeus-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "zeus-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "zeus-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "zeus-kanban-docs-link",
      title: "Open ZeusAgent Kanban docs in a new tab",
      "aria-label": "ZeusAgent Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `zeus kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "zeus-kanban-msg-ok" : "zeus-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "zeus-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "zeus-kanban-boardswitcher" },
      h("div", { className: "zeus-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "zeus-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "zeus-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "zeus-kanban-dialog-title" },
        ...
        h("div", { className: "zeus-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned ZeusAgent profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "zeus-kanban-bulk" },
      h("span", { className: "zeus-kanban-bulk-count" },
      ...
      h("div", { className: "zeus-kanban-bulk-priority",
      ...
      h("div", { className: "zeus-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different ZeusAgent profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "zeus-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("zeus-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("zeus-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "zeus-kanban-trash",
        dragOver ? "zeus-kanban-trash--drop" : "",
        props.draggingTaskId ? "zeus-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "zeus-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "zeus-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".zeus-kanban-card")) return true;
      if (target.closest && target.closest(".zeus-kanban-column-add")) return true;
      if (target.closest && target.closest(".zeus-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("zeus-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("zeus-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".zeus-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ...
      className: cn(
        "zeus-kanban-columns",
        isScrollable ? "zeus-kanban-columns--scrollable" : "",
        isPanning ? "zeus-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("zeus-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("zeus-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "zeus-kanban-column",
        dragOver ? "zeus-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "zeus-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "zeus-kanban-col-check",
        ...
        h("span", { className: cn("zeus-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "zeus-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "zeus-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "zeus-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "zeus-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "zeus-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "zeus-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "zeus-kanban-lane" },
                  h("div", { className: "zeus-kanban-lane-head" },
                    h("span", { className: "zeus-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "zeus-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "zeus-kanban-card--stale-red";
    if (age >= tier.amber) return "zeus-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".zeus-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "zeus-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "zeus-kanban-card",
        props.selected ? "zeus-kanban-card--selected" : "",
        props.failed ? "zeus-kanban-card--failed" : "",
        props.draggingSource ? "zeus-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "zeus-kanban-card-content" },
          h("div", { className: "zeus-kanban-card-row" },
            h("label", {
              className: "zeus-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "zeus-kanban-card-check",
            ...
            h("span", { className: "zeus-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or zeus kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "zeus-kanban-warning-badge",
                    "zeus-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "zeus-kanban-needs-assignee",
          ...
          h("div", { className: "zeus-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "zeus-kanban-card-row zeus-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "zeus-kanban-assignee",
                            title: `Assigned to ZeusAgent profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "zeus-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "zeus-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "zeus-kanban-ago",
  ...
  // centered modal reusing the zeus-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "zeus-kanban-dialog zeus-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "ZeusAgent profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "ZeusAgent profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "zeus-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "zeus-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "zeus-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "zeus-kanban-drawer-close",
        ...
        data ? h("div", { className: "zeus-kanban-drawer-comment-foot" },
          h("div", {
            className: "zeus-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "zeus-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "zeus-kanban-section" },
      h("div", { className: "zeus-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "zeus-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "zeus-kanban-drawer-body" },
      h("div", { className: "zeus-kanban-drawer-title" },
        h("span", { className: cn("zeus-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "zeus-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "zeus-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "zeus-kanban-section" },
            h("div", { className: "zeus-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "zeus-kanban-section" },
            h("div", { className: "zeus-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "zeus-kanban-done-no-result zeus-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "zeus-kanban-section" },
            h("div", { className: "zeus-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "zeus-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "zeus-kanban-section" },
        h("div", { className: "zeus-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "zeus-kanban-comment" },
            h("div", { className: "zeus-kanban-comment-head" },
              h("span", { className: "zeus-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "zeus-kanban-diag-action-btn",
      ...
      h("div", { className: "zeus-kanban-section" },
        h("div", { className: "zeus-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "zeus-kanban-comment" },
                h("div", { className: "zeus-kanban-comment-head" },
                  h("span", { className: "zeus-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "zeus-kanban-comment-ago" },
      ...
      h("div", { className: "zeus-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "zeus-kanban-event",
              isDiag ? "zeus-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "zeus-kanban-event-header" },
                  h("span", { className: "zeus-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "zeus-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "zeus-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "zeus-kanban-event-header-plain" },
                  h("span", { className: "zeus-kanban-event-kind" }, e.kind),
                  h("span", { className: "zeus-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "zeus-kanban-event-phantom-row" },
                  h("span", { className: "zeus-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "zeus-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "zeus-kanban-event-payload" },
    ...
    return h("div", { className: "zeus-kanban-section" },
      h("div", { className: "zeus-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "zeus-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `zeus-kanban-run--${r.outcome || r.status || "ended"}`
          : "zeus-kanban-run--active";
        return h("div", { key: r.id, className: cn("zeus-kanban-run", outcomeClass) },
          h("div", { className: "zeus-kanban-run-head" },
            h("span", { className: "zeus-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "zeus-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "zeus-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "zeus-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "zeus-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "zeus-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "zeus-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "zeus-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "zeus-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "zeus-kanban-pre zeus-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "zeus-kanban-meta-row" },
      h("span", { className: "zeus-kanban-meta-label" }, props.label),
      h("span", { className: "zeus-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "zeus-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "zeus-kanban-meta-row" },
        h("span", { className: "zeus-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "zeus-kanban-meta-value zeus-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "zeus-kanban-meta-row" },
        h("span", { className: "zeus-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "zeus-kanban-meta-value zeus-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "zeus-kanban-meta-row" },
        h("span", { className: "zeus-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "zeus-kanban-meta-value zeus-kanban-editable",
    ...
    return h("div", { className: "zeus-kanban-meta-row" },
      h("span", { className: "zeus-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "zeus-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "zeus-kanban-section" },
      h("div", { className: "zeus-kanban-section-head-row" },
        h("span", { className: "zeus-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "zeus-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "zeus-kanban-section" },
      h("div", { className: "zeus-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "zeus-kanban-deps-row" },
        h("span", { className: "zeus-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "zeus-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "zeus-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "zeus-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "zeus-kanban-dep-chip-x",
      ...
      h("div", { className: "zeus-kanban-deps-row" },
        h("span", { className: "zeus-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "zeus-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "zeus-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "zeus-kanban-section" },
      ...
      h("div", { className: "zeus-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "zeus-kanban-home-sub zeus-kanban-home-sub--on"
              : "zeus-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__ZEUS_PLUGINS__ && typeof window.__ZEUS_PLUGINS__.register === "function") {
    window.__ZEUS_PLUGINS__.register("kanban", KanbanPage);
  }
})();
