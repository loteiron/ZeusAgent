...
Every handler is a thin wrapper around ``zeus_cli.kanban_db`` (the same code paths the CLI
...
from zeus_cli import kanban_db
...
from zeus_cli.kanban_db import KANBAN_ATTACHMENT_MAX_BYTES, _collision_free_path, _safe_attachment_name
...
def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    ...
    try:
        from zeus_cli import web_server_chat as _ws
    except Exception:
        return True
...
def _with_board_pinned(board: Optional[str], fn: Callable[[], Any]) -> Any:
    ...
    ``ZEUS_KANBAN_BOARD`` env var (concurrent requests for different boards would cross-write)."""
...
def _run_aux(board: Optional[str], module: str, fn: str, task_id: str, author: Optional[str]) -> Any:
    """Run a slow auxiliary-LLM task helper (``zeus_cli.<module>.<fn>``) with the board pinned;
    the module is imported lazily so a missing aux client can't break plugin load."""
    def _run():
        return getattr(importlib.import_module(f"zeus_cli.{module}"), fn)(task_id, author=(author or None))
...
def get_board(
    ...
    (``ZEUS_KANBAN_BOARD`` env → on-disk ``current`` pointer → ``default``)."""
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    with _board_conn(board) as (board, conn), _value_error_400():
        ...
        # dashboard backend may run under a different ZEUS_HOME than the board's profile.
        if task and task.status == "ready" and task.assignee:
            try:
                from zeus_cli.kanban import _check_dispatcher_presence
                from zeus_constants import get_zeus_home
                running, message = _check_dispatcher_presence(zeus_home=get_zeus_home())
...
class UpdateTaskBody(BaseModel):
    ...
    # Handoff fields forwarded to complete_task on -> 'done' (parity with ``zeus kanban complete``).
...
def list_diagnostics(
    ...
    consumed by ``zeus kanban diagnostics`` when the dashboard runs."""
    with _board_conn(board) as (board, conn):
...
def reclaim_task_endpoint(task_id: str, payload: ReclaimBody, board: Optional[str] = Query(None)):
    ...
    (``zeus kanban reclaim <task_id> --reason ...``)."""
    with _board_conn(board) as (board, conn):
...
def specify_task_endpoint(task_id: str, payload: SpecifyBody, board: Optional[str] = Query(None)):
    """Flesh out a triage task via the auxiliary LLM (``zeus kanban specify``). Non-OK is NOT
...
def reassign_task_endpoint(task_id: str, payload: ReassignBody, board: Optional[str] = Query(None)):
    ...
    (``zeus kanban reassign <task_id> <profile> [--reclaim]``)."""
    with _board_conn(board) as (board, conn):
...
def _active_profile_name() -> str:
    """Current ZeusAgent profile name for notify-sub ownership."""
    try:
...
def model_options():
    ...
    (same substrate as the Models page) so it can't offer a pair ZeusAgent rejects. Skips pricing
    and custom-provider probes: a slow/offline local endpoint must not hang the drawer."""
    try:
        from zeus_cli.inventory import build_models_payload, load_picker_context
...
async def export_board_endpoint(slug: str, body: ExportBoardBody):
    ...
    from zeus_cli import kanban_transfer
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    with _errors_to_500("failed to update profile"):
        ...
        if canon == "default":
            from zeus_constants import get_zeus_home  # type: ignore
            profile_dir = Path(get_zeus_home())
        else:
...
def decompose_task_endpoint(task_id: str, payload: DecomposeBody, board: Optional[str] = Query(None)):
    """Fan a triage task out into child tasks via the auxiliary LLM (``zeus kanban decompose``).
