...
Let the zeus agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where zeus runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `zeus meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `zeus meet` CLI |
...
| `cli.py` | `zeus meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ZEUS_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `zeus meet node {run,list,approve,remove,status,ping}` |
...
zeus meet install                                      # pip + Chromium
zeus meet setup                                        # preflight
zeus meet auth                                         # optional
zeus meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
zeus meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.zeus/.env
zeus meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
zeus meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
zeus meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.zeus/.env
zeus meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, zeus will **not** switch your system audio input automatically — the
...
zeus plugins enable google_meet
zeus meet node run --display-name my-mac --host 0.0.0.0 --port 18789
