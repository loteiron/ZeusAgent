...
zeus meet install                 # pip deps + Chromium (transcribe only)
zeus meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
zeus meet auth                    # optional; skips guest-lobby wait
zeus meet setup                   # preflight checks
```

`zeus meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or ZEUS_MEET_REALTIME_KEY in ~/.zeus/.env
```
...
pip install playwright websockets && python -m playwright install chromium
zeus plugins enable google_meet
zeus meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
zeus meet node approve my-mac ws://<mac-ip>:18789 <token>
zeus meet node ping my-mac                   # confirm reachable
```

Run `zeus meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A ZeusAgent agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `zeus meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `ZEUS_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `zeus_meet_src`). |
...
Local:
```
$ZEUS_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$ZEUS_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `zeus meet node approve`.
