...
One active meeting at a time, recorded in ``$ZEUS_HOME/workspace/meetings/.active.json``
...
def _root() -> Path:
    return Path(get_zeus_home()) / "workspace" / "meetings"
...
def start(url: str, *, out_dir: Optional[Path] = None, headed: bool = False,
          auth_state: Optional[str] = None, guest_name: str = "ZeusAgent", duration: Optional[str] = None,
    ...
    for name in ("transcript.txt", "status.json"):
        with contextlib.suppress(OSError):
            (out / name).unlink()
    env = {**os.environ, "ZEUS_MEET_URL": url, "ZEUS_MEET_OUT_DIR": str(out),
           "ZEUS_MEET_GUEST_NAME": guest_name}
    for value, var in (
        (headed and "1", "ZEUS_MEET_HEADED"),
        (auth_state, "ZEUS_MEET_AUTH_STATE"),
        (duration, "ZEUS_MEET_DURATION"),
        (mode, "ZEUS_MEET_MODE"),  # bot defaults to transcribe when unset (v1 behavior)
        (realtime_model, "ZEUS_MEET_REALTIME_MODEL"),
        (realtime_voice, "ZEUS_MEET_REALTIME_VOICE"),
        (realtime_instructions, "ZEUS_MEET_REALTIME_INSTRUCTIONS")):
        if value:
            env[var] = value
    ...
    if not realtime_api_key:
        ...
        realtime_api_key = get_secret("ZEUS_MEET_REALTIME_KEY") or get_secret("OPENAI_API_KEY")
    if realtime_api_key:
        env["ZEUS_MEET_REALTIME_KEY"] = realtime_api_key
