...
Auto-tracks and cleans up ephemeral files created during ZeusAgent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ZEUS_HOME` and `/tmp/zeus-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ZEUS_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `ZEUS_HOME` or `/tmp/zeus-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ZEUS_HOME/disk-cleanup/` is itself excluded
- `$ZEUS_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
