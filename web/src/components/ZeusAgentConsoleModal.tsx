...
interface ZeusAgentConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function ZeusAgentConsoleModal({ open, onClose }: ZeusAgentConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("zeus> ");
  const inputPromptRef = useRef("zeus> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "zeus> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to ZeusAgent Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="zeus-console-title"
    >
      <div
        className={cn(
          themedBody,
