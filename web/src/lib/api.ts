import { buildZeusAgentWebSocketUrl } from "@zeus/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/hermes/). The Python backend
// injects ``window.__ZEUS_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__ZEUS_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const ZEUS_BASE_PATH = readBasePath();
const BASE = ZEUS_BASE_PATH;
...
declare global {
  interface Window {
    __ZEUS_SESSION_TOKEN__?: string;
    __ZEUS_BASE_PATH__?: string;
    ...
    __ZEUS_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-Zeus-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__ZEUS_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``zeus update``, ``zeus gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__ZEUS_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__ZEUS_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__ZEUS_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``zeus_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__ZEUS_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Zeus-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``zeus_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__ZEUS_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export async function buildWsUrl(
  ...
  return buildZeusAgentWebSocketUrl({
...
export const api = {
  ...
  updateZeusAgent: () =>
    fetchJSON<ActionResponse>("/api/zeus/update", { method: "POST" }),
  checkZeusAgentUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/zeus/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** zeus-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``zeus update``. */
  can_update_zeus?: boolean;
  config_path: string;
  ...
  /** NS-656: disk-usage rollup for the ZEUS_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the ZEUS_HOME volume — no staleness dimension, so no
 * sampled_at. */
