...
const STORAGE_KEY = "zeus-dashboard-theme";
...
const FONT_STORAGE_KEY = "zeus-dashboard-font";
 ...
 *  `_THEME_NAMED_ASSET_KEYS` in `zeus_cli/web_server.py`. */
...
const CUSTOM_CSS_STYLE_ID = "zeus-theme-custom-css";
...
function applyCustomCSS(css: string | undefined) {
  ...
  if (!el) {
    ...
    el.setAttribute("data-zeus-theme-css", "true");
...
function injectFontStylesheet(url: string | undefined) {
  ...
  link.rel = "stylesheet";
  link.href = url;
  link.setAttribute("data-zeus-theme-font", "true");
...
function applyTheme(theme: DashboardTheme) {
  ...
  const color = theme.palette.background.hex.replace("#", "");
  const rgb = color.length === 3 ? color.split("").map((c) => c + c).join("") : color;
  const brightness = [0.299, 0.587, 0.114].reduce((sum, weight, index) =>
    sum + Number.parseInt(rgb.slice(index * 2, index * 2 + 2), 16) * weight, 0);
  root.style.colorScheme = brightness > 140 ? "light" : "dark";
      ...
      .then((resp) => {
        ...
        if (resp.themes?.length) {
          setAvailableThemes(
            resp.themes.map((t) => ({
              name: t.name,
              label: BUILTIN_THEMES[t.name]?.label ?? t.label,
              description: BUILTIN_THEMES[t.name]?.description ?? t.description,
