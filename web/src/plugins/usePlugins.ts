...
import { api, ZEUS_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "zeus:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${ZEUS_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${ZEUS_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?zeus_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-zeus-plugin", manifest.name);
