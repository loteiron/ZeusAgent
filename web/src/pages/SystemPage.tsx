...
import { ZeusAgentConsoleModal } from "@/components/ZeusAgentConsoleModal";
...
export default function SystemPage() {
  ...
  // spawned `zeus import` runs non-interactively (stdin is /dev/null), so
  ...
  const loadAll = useCallback(() => {
    Promise.allSettled([
      ...
      api.checkZeusAgentUpdate(false),
    ])
  ...
  const checkForUpdate = useCallback(
    async (force = false) => {
      if (status?.can_update_zeus === false) return;
      setCheckingUpdate(true);
      try {
        const info = await api.checkZeusAgentUpdate(force);
        setUpdateInfo(info);
        if (force) {
    ...
    [showToast, status?.can_update_zeus],
  );
  ...
  const applyUpdate = async () => {
    ...
    if (status?.can_update_zeus === false) {
      showToast(
        "ZeusAgent updates are managed outside this dashboard.",
        "success",
      );
      return;
    }
    try {
      const resp = await api.updateZeusAgent();
      if (!resp.ok) {
        showToast(
          resp.message ??
      ...
      setActiveAction(resp.name ?? "zeus-update");
  ...
  const canUpdateZeusAgent = status?.can_update_zeus !== false;
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <ConfirmDialog
        open={canUpdateZeusAgent && updateConfirmOpen}
        onCancel={() => setUpdateConfirmOpen(false)}
        onConfirm={() => void applyUpdate()}
        title="Update ZeusAgent?"
        description={
          updateInfo && updateInfo.behind && updateInfo.behind > 0
            ? `This will run 'zeus update' (${updateInfo.update_command}) and pull ${updateInfo.behind} new commit${updateInfo.behind === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`
            : `This will run 'zeus update' (${updateInfo?.update_command ?? "zeus update"}) and restart the gateway when it finishes.`
        }
        ...
        >
          <div className={cn(themedBody, "relative w-full max-w-lg border border-border bg-card shadow-2xl flex flex-col")}>
            ...
            <header className="p-5 pb-3 border-b border-border">
              <h2 className="font-sans text-display text-base tracking-wider">
                New shell hook
              </h2>
            </header>
        ...
        <Card>
          <CardContent className="py-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-3 gap-x-6 text-sm">
              ...
              <div>
                <div className="text-xs uppercase tracking-wider text-muted-foreground">ZeusAgent</div>
                <div className="flex items-center gap-2">
                  <span>v{stats?.zeus_version}</span>
                  {canUpdateZeusAgent &&
                    updateInfo &&
          ...
          <CardContent className="flex flex-col gap-3 py-4">
            ...
            {!portal?.logged_in && (
              <p className="text-xs text-muted-foreground">
                Log in with <span className="font-mono">zeus portal</span>.
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="flex flex-col gap-4 py-4">
            ...
            <div className="flex flex-col gap-3 border-t border-border pt-4 sm:flex-row sm:items-end">
              <div className="grid min-w-0 flex-1 gap-2">
                ...
                <Input
                  ...
                  placeholder="$ZEUS_HOME/backups/zeus-backup.zip"
                />
              </div>
              <Button
                size="sm"
            ...
            <ConfirmDialog
              ...
              title="Restore full ZeusAgent backup?"
              description={`This will overwrite your current ZeusAgent configuration, skills, sessions, and data with the contents of ${backupImportLabel(importConfirmTarget)}. This cannot be undone.`}
              destructive
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground max-w-prose">
                    ...
                    returns links to send the ZeusAgent team. Pastes auto-delete
                    after 6 hours.
                  </span>
                </div>
              </div>
