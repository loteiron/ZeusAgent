...
const PROVIDER_GROUPS: { prefix: string; name: string; priority: number }[] = [
  ...
  { prefix: "ZEUS_QWEN_", name: "DashScope (Qwen)", priority: 2 },
...
// Mirror of the backend env-name guard (zeus_cli/config.py _ENV_VAR_NAME_RE).
      ...
      >
        {sections.map((s) => (
          <button
            ...
            className="shrink-0 cursor-pointer px-2 py-0.5 font-sans text-display text-xs tracking-wider text-text-secondary hover:text-foreground border border-border/50 hover:border-foreground/30 transition-colors"
          >
            {s.label}
          </button>
        ))}
      ...
      <div className="flex items-center justify-between">
        <div className="flex flex-col gap-1">
          <p className="text-sm text-muted-foreground">
            {t.env.description} <code>~/.zeus/.env</code>
          </p>
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          {section.unsetEntries.length > 0 && (
            <button
              ...
              className="shrink-0 cursor-pointer border-0 bg-transparent p-0 font-sans text-xs tracking-[0.08em] text-text-secondary transition-colors hover:text-foreground"
            >
