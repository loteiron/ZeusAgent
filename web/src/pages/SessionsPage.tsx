...
const SOURCE_CONFIG: Record<string, { icon: typeof Terminal; color: string }> =
  {
    ...
    zeus_flow: { icon: Play, color: "text-warning" },
...
const NO_MATCHING_SESSION_SOURCE = "__zeus_dashboard_no_matching_source__";
...
function SnippetHighlight({ snippet }: { snippet: string }) {
  ...
  return (
    <p className="font-sans normal-case mt-0.5 min-w-0 max-w-full truncate text-xs text-text-secondary">
      {parts}
    </p>
  );
}
            ...
            <div className="flex min-w-0 flex-1 flex-col gap-0.5">
              <div className="flex min-w-0 items-center gap-2">
                {renaming ? (
                  ...
                  <span
                    className={`font-sans normal-case min-w-0 flex-1 truncate text-sm ${hasTitle ? "font-medium" : "text-muted-foreground italic"}`}
                  >
                    {hasTitle
                      ? session.title
                      : session.preview
...
export default function SessionsPage() {
        ...
        const res = await fetch(api.exportSessionUrl(id, rowProfile(id)), {
          ...
          headers: {
            "X-Zeus-Session-Token":
              (window as unknown as { __ZEUS_SESSION_TOKEN__?: string })
                .__ZEUS_SESSION_TOKEN__ ?? "",
          },
        });
  ...
  return (
    <div className="flex min-w-0 w-full max-w-full flex-col gap-4">
      ...
      {stats && (
        <div className="flex flex-wrap items-center gap-x-6 gap-y-2 rounded-lg border border-border bg-card px-5 py-4">
        ...
        <div className="rounded-lg border border-border bg-card">
          <div className="flex items-center justify-between gap-2 border-b border-border px-3 py-2">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className="text-xs font-sans tracking-[0.12em] truncate">
                {activeAction === "restart"
                  ? t.status.restartGateway
                  : t.status.updateZeusAgent}
              </span>
        ...
        <div className="flex w-full min-w-0 flex-wrap items-center gap-2 sm:gap-3">
          <div className="flex min-w-0 flex-1 flex-wrap items-center gap-2 sm:gap-3">
            ...
            <div ref={sourceMenuRef} className="relative shrink-0">
              ...
              {sourceMenuOpen && (
                <div
                  className="absolute left-0 top-full z-30 mt-1 w-[18rem] max-w-[calc(100vw-2rem)] rounded-lg border border-border bg-card shadow-lg"
                >
            ...
            {showList && emptyCount > 0 && !isSearching && (
              <Button
                ...
                prefix={<Eraser />}
              >
                <span className="font-sans normal-case text-xs">
      ...
      {showList && selectedIds.size > 0 && (
        ...
        >
          <span className="font-sans normal-case text-xs text-primary tabular-nums">
            ...
            <Card className="min-w-0 max-w-full overflow-hidden">
              ...
              <CardContent className="grid min-w-0 gap-3">
                {recentSessions.map((s) => (
                  <div
                    key={s.id}
                    className="flex min-w-0 max-w-full flex-col gap-2 rounded-lg border border-border bg-card p-4 sm:flex-row sm:items-center sm:justify-between"
                  >
                    <div className="flex min-w-0 flex-1 flex-col gap-1">
                      <span
                        className={`font-sans normal-case min-w-0 truncate text-sm ${s.title ? "font-medium" : "text-muted-foreground italic"}`}
                      >
                        {s.title ??
                          (s.preview
                      ...
                      {s.preview && s.title && (
                        <p className="font-sans normal-case min-w-0 max-w-full text-xs leading-snug text-text-tertiary [overflow-wrap:anywhere]">
                          {s.preview}
                        </p>
                      )}
                    </div>
