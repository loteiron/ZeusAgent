...
const SIDEBAR_COLLAPSED_KEY = "zeus-sidebar-collapsed";
...
export default function App() {
  ...
  const navGroups = [
    { label: locale === "tr" ? "Çalışma alanı" : "Workspace", paths: ["/chat", "/sessions", "/files", "/analytics", "/cron"] },
    { label: locale === "tr" ? "Yetenekler" : "Capabilities", paths: ["/models", "/skills", "/plugins", "/mcp", "/channels", "/webhooks"] },
    { label: locale === "tr" ? "Yönetim" : "Administration", paths: ["/profiles", "/config", "/env", "/pairing", "/logs", "/system", "/docs"] },
  ];
  ...
  return (
    ...
    >
      ...
      <header
        className={cn(
          "zeus-mobile-header lg:hidden fixed top-0 left-0 right-0 z-40 min-h-14",
      ...
      >
        ...
        <img src="./zeus-mark.svg" alt="" className="h-7 w-7" />
        <span className="zeus-wordmark">ZEUS</span>
      </header>
      ...
      {mobileOpen && (
        <Button
          ...
          onClick={closeMobile}
          className={cn(
            "lg:hidden fixed inset-0 z-40 p-0 block rounded-none",
            "bg-black/70",
          )}
        />
      )}
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
        <div className="flex min-h-0 min-w-0 flex-1">
          <aside
            ...
            className={cn(
              "fixed top-0 left-0 z-50 flex h-dvh max-h-dvh w-72 min-h-0 flex-col font-sans",
          ...
          >
            <div
              className={cn(
                "zeus-brand-bar flex shrink-0 items-center gap-2",
                "border-b border-current/20",
            ...
            >
              ...
              >
                ...
                <img src="./zeus-mark.svg" alt="" className="h-10 w-10 shrink-0" />
                <div>
                  <span className="zeus-wordmark">ZEUS</span>
                  <span className="zeus-brand-caption">{locale === "tr" ? "Ajan çalışma alanı" : "Agent workspace"}</span>
                </div>
              </div>
            ...
            >
              {navGroups.map((group) => (
                <div className="zeus-nav-section" key={group.label}>
                  <span className={cn("zeus-nav-heading", isDesktopCollapsed && "lg:hidden")}>{group.label}</span>
                  <ul className="flex flex-col">
                    {sidebarNav.coreItems.filter((item) => group.paths.includes(item.path)).map((item) => (
                      <SidebarNavLink
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="zeus-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="zeus-sidebar-plugin-nav-heading"
                  >
  ...
  return (
    ...
    >
      <NavLink
        ...
        className={({ isActive }) =>
          cn(
            "zeus-nav-link group/nav relative flex items-center gap-3",
            "px-5 py-2.5",
...
function SidebarSystemActions({
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'zeus update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "zeus update";
    return (
      t.status.updateZeusAgentConfirmMessage ??
      `This will run 'zeus update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateZeusAgentConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateZeusAgent) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateZeusAgent,
      runningLabel: t.status.updatingZeusAgent,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateZeusAgentConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateZeusAgentConfirmTitle ?? `${t.status.updateZeusAgent}?`}
    />
    </>
  );
}
