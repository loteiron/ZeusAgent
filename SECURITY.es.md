# Política de Seguridad de ZeusAgent

Este documento describe el modelo de confianza de ZeusAgent, identifica el
...
Usa **Security → Report a vulnerability** en el repositorio de ZeusAgent del que
obtuviste este fork, si su responsable habilitó los informes privados. Esta
distribución no configura un contacto de seguridad propio ni garantiza un plazo
de respuesta. Si esa opción no existe, pide al responsable un canal privado sin
publicar secretos, datos personales ni un exploit funcional. El buzón y el
repositorio de Nous Research no son el soporte de este fork independiente.
**No se ofrece un programa de recompensas.**
...
- Detalles del entorno (`zeus --version`, SHA del commit, SO, versión de Python).
...
ZeusAgent es un agente personal de un solo inquilino. Su postura es
...
- **Proceso del agente.** El intérprete Python que ejecuta ZeusAgent,
...
- **Envolvente de confianza.** El conjunto de recursos a los que un operador ha otorgado
  implícitamente acceso a ZeusAgent al ejecutarlo — típicamente, todo lo que
  la propia cuenta de usuario del operador puede alcanzar en el host.
- **Postura.** Una declaración explícita en la documentación o código de ZeusAgent
...
ZeusAgent admite dos posturas de aislamiento a nivel de SO. Abordan
...
ZeusAgent admite esto de dos maneras:

- **La propia imagen Docker de ZeusAgent y la configuración de Compose.** Más
...
Bajo una envoltura de proceso completo, las heurísticas en proceso de ZeusAgent
...
ZeusAgent filtra el entorno que pasa a sus componentes en proceso de
...
Un plugin malicioso o con errores no es una vulnerabilidad en ZeusAgent
en sí mismo. Los errores en la ruta de instalación o descubrimiento de plugins de ZeusAgent
...
4. **Dentro del conjunto autorizado, todos los llamadores tienen la misma confianza.**
   ZeusAgent no modela capacidades por llamador dentro de un único adaptador.
...
- Violaciones de la documentación del modelo de confianza: código que se comporta
  contrariamente a lo que esta política, la propia documentación de ZeusAgent o
  las expectativas razonables del operador predecirían — incluyendo casos donde
  ZeusAgent ha documentado una postura sobre cómo su salida debe ser
...
- **Configuraciones documentadas de emergencia.** Compensaciones seleccionadas por el operador
  ...
  la seguridad de zeus-home, y similares. Los informes contra esas
...
- **Habilidades y plugins contribuidos por la comunidad.** Las habilidades de terceros
  ...
  están en la superficie de revisión del operador, no en la superficie de confianza de ZeusAgent
  (§2.4, §2.5). Una habilidad o plugin que haga algo
  malicioso es el modo de falla esperado de uno que no fue
  revisado, no una vulnerabilidad en ZeusAgent. Los errores en la ruta de
  instalación de habilidades o plugins de ZeusAgent que impidan al
...
- Se conservan las protecciones de cadena de suministro de los servidores MCP.
  La CI activa comprueba el código seleccionado, la instalación con dependencias
  bloqueadas, las pruebas y las compilaciones; no constituye una auditoría completa
  de vulnerabilidades. Los workflows de auditoría y publicación originales están
  archivados e inactivos.

---

## 5. Divulgación

- Acuerda el plazo de divulgación en privado con el responsable de este fork.
  La política original no es una garantía de respuesta de ZeusAgent.
- Usa el hilo privado del repositorio o un canal confirmado por su responsable.
  No envíes credenciales ni informes confidenciales a una dirección no verificada.
- Consulta cómo desea aparecer la persona que informa y respeta su anonimato.
