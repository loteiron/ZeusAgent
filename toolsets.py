...
_ZEUS_WEBHOOK_SAFE_TOOLS = ["web_search", "web_extract", "vision_analyze", "clarify"]
...
def _bundle(description, extras=()):
    """A `zeus-*` platform bundle: the shared core tools plus optional platform extras."""
    return _ts(description, _ZEUS_CORE_TOOLS + list(extras))


def _core_without(*excluded, kanban=True):
    """_ZEUS_CORE_TOOLS minus *excluded* (and, unless kanban=True, every kanban_* tool); order preserved."""
    return [t for t in _ZEUS_CORE_TOOLS if t not in excluded and (kanban or not t.startswith("kanban_"))]
...
TOOLSETS = {
    ...
    "x_search": _ts(
        ...
        "(SuperGrok OAuth or XAI_API_KEY). Off by default; enable in `zeus tools` → "
        "X (Twitter) Search.",
        ["x_search"],
    ),
    ...
    "video_gen": _ts(
        ...
        "separate tools. Configure via ``zeus tools`` → Video Generation.",
    ...
    "browser": _ts(
        ...
        "iframes, hold-click)",
        [t for t in _ZEUS_CORE_TOOLS if t.startswith("browser_")],
    ),
    "cronjob": _ts(
    ...
    "kanban": _ts(
        ...
        "the kanban dispatcher (ZEUS_KANBAN_TASK env set). The dispatcher runs "
        ...
        [t for t in _ZEUS_CORE_TOOLS if t.startswith("kanban_")],
    ),
    ...
    # config (see the non-configurable-toolset recovery loop in zeus_cli/tools_config.py).
    "coding": _ts(
    ...
    # Full ZeusAgent toolsets (CLI + messaging platforms). All share the core tools;
    # there is deliberately no agent-callable send_message tool. zeus-acp is the
    ...
    "zeus-cli": _bundle("Full interactive CLI toolset - all default tools plus cronjob management"),

    # Mirrors zeus-cli; `zeus tools` platform config filters it down and
    # _get_platform_tools() drops _DEFAULT_OFF_TOOLSETS unless user-enabled.
    "zeus-cron": _bundle("Default cron toolset - same core tools as zeus-cli; gated by `zeus tools`"),
    "zeus-telegram": _bundle("Telegram bot toolset - full access for personal use (terminal has safety checks)"),
    "zeus-discord": _bundle(
        "Discord bot toolset - full access (terminal has safety checks via dangerous "
        "command approval)",
        ["discord", "discord_admin"],
    ),
    "zeus-whatsapp": _bundle("WhatsApp bot toolset - similar to Telegram (personal messaging, more trusted)"),
    "zeus-slack": _bundle("Slack bot toolset - full access for workspace use (terminal has safety checks)"),
    "zeus-signal": _bundle("Signal bot toolset - encrypted messaging platform (full access)"),
    "zeus-bluebubbles": _bundle("BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server"),
    "zeus-homeassistant": _bundle("Home Assistant bot toolset - smart home event monitoring and control"),
    "zeus-email": _bundle("Email bot toolset - interact with ZeusAgent via email (IMAP/SMTP)"),
    "zeus-mattermost": _bundle("Mattermost bot toolset - self-hosted team messaging (full access)"),
    "zeus-matrix": _bundle("Matrix bot toolset - decentralized encrypted messaging (full access)"),
    "zeus-dingtalk": _bundle("DingTalk bot toolset - enterprise messaging platform (full access)"),
    "zeus-feishu": _bundle("Feishu/Lark bot toolset - enterprise messaging via Feishu/Lark (full access)", _FEISHU_TOOLS),
    "zeus-weixin": _bundle("Weixin bot toolset - personal WeChat messaging via iLink (full access)"),
    "zeus-qqbot": _bundle("QQBot toolset - QQ messaging via Official Bot API v2 (full access)"),
    "zeus-wecom": _bundle("WeCom bot toolset - enterprise WeChat messaging (full access)"),
    "zeus-wecom-callback": _bundle("WeCom callback toolset - enterprise self-built app messaging (full access)"),
    "zeus-yuanbao": {
        "description": "Yuanbao Bot 元宝消息平台工具集 - 群信息、成员查询、私聊、贴纸表情",
        "tools": _ZEUS_CORE_TOOLS + _YUANBAO_TOOLS,
        "module": "tools.yuanbao_tools",
        "includes": [],
    },
    "zeus-sms": _bundle("SMS bot toolset - interact with ZeusAgent via SMS (Twilio)"),
    "zeus-webhook": _ts("Webhook toolset - receive and process external webhook events", _ZEUS_WEBHOOK_SAFE_TOOLS),
    "zeus-gateway": _ts(
        "Gateway toolset - union of all messaging platform tools",
        [],
        includes=[
            "zeus-telegram", "zeus-discord", "zeus-whatsapp", "zeus-slack",
            "zeus-signal", "zeus-bluebubbles", "zeus-homeassistant", "zeus-email",
            "zeus-sms", "zeus-mattermost", "zeus-matrix", "zeus-dingtalk",
            "zeus-feishu", "zeus-wecom", "zeus-wecom-callback", "zeus-weixin",
            "zeus-qqbot", "zeus-webhook", "zeus-yuanbao",
        ],
    ),
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """A bundle's tools minus _ZEUS_CORE_TOOLS (one level of includes).

    Disabling a `core + extras` bundle must not strip the core tools every other
    toolset shares. One `includes` pass suffices (only zeus-gateway nests
    bundles). Unknown names: full resolution minus core.
    """
    core = set(_ZEUS_CORE_TOOLS)
...
def _plugin_platform_bundle(name: str) -> List[str]:
    """Implicit `zeus-<platform>` bundle for a registered plugin platform: core
    tools plus whatever the plugin registered under the platform name. [] otherwise."""
    if not name.startswith("zeus-"):
        return []
    platform_name = name[len("zeus-"):]
    try:
