...
The provider list shown by ``zeus model`` (CLI/TUI) and the desktop Settings → Providers tabs
...
come from :data:`zeus_cli.auth.PROVIDER_REGISTRY` (credential truth); ``display_name`` /
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `zeus model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """One descriptor per provider in the ``zeus model`` universe (:data:`CANONICAL_PROVIDERS`,
    ...
    PROVIDER_REGISTRY = _safe_import("zeus_cli.auth", "PROVIDER_REGISTRY", {})
    OPTIONAL_ENV_VARS = _safe_import("zeus_cli.config", "OPTIONAL_ENV_VARS", {})
    ...
    ZEUS_OVERLAYS = _safe_import("zeus_cli.providers", "ZEUS_OVERLAYS", {})
    try:
