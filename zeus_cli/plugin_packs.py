...
def resolve_pack_plugins(pack: PluginPack) -> List[ResolvedPackPlugin]:
    ...
    for entry in pack.plugins:
        if entry.install_identifier is not None:
            ...
            continue
        try:
            from zeus_cli.plugin_index import load_index, resolve_name
...
def render_pack_review(console, pack: PluginPack, resolved: List[ResolvedPackPlugin]) -> None:
    ...
    if pack.skills:
        ...
        console.print("[dim]Install them manually, e.g. `zeus skills install <id>`.[/dim]")
    console.print(
...
def export_pack(*, enabled_only: bool = False, pack_name: str = "my-zeus-pack") -> tuple[str, List[str]]:
    ...
    import yaml
    from zeus_cli.plugins_cmd import _get_enabled_set, _plugins_dir, _read_install_metadata
    ...
    doc: dict[str, Any] = {
        "name": pack_name,
        "description": "Exported by `zeus plugins pack export`.",
        "version": "1.0.0",
        "plugins": entries,
    }
    if config:
...
def _load_and_review(console, source: str):
    ...
    from zeus_cli.plugins_cmd import _fail
    try:
...
def cmd_pack_show(source: str) -> None:
    """``zeus plugins pack show <path-or-url>`` — dry-run review."""
    from zeus_cli.plugins_cmd import _console
    console = _console()
    ...
    console.print("\n[dim]Dry run only. Install with `zeus plugins pack install ...`.[/dim]")


def cmd_pack_install(source: str, *, force: bool = False) -> None:
    """``zeus plugins pack install <path-or-url>``: mandatory review screen -> one pack-level
    consent -> pinned fan-out installs -> per-plugin capability consent. Exit 1 if any failed."""
    from zeus_cli.plugins_cmd import _ask_yes, _console, _fail, _is_tty
    console = _console()
    pack, resolved = _load_and_review(console, source)
...
def cmd_pack_export(*, enabled_only: bool = False, name: str = "my-zeus-pack") -> None:
    """``zeus plugins pack export [--enabled-only]`` — pack YAML on stdout."""
...
def pack_command(args) -> None:
    """Dispatch ``zeus plugins pack <action>``."""
    handler = _PACK_ACTIONS.get(getattr(args, "pack_action", None))
    if handler is None:
        from zeus_cli.plugins_cmd import _console, _fail
        _fail(_console(), "[red]Error:[/red] Usage: zeus plugins pack {install|export|show}")
    handler(args)
...
_PACK_ACTIONS = {
    ...
    "export": lambda args: cmd_pack_export(
        ...
        name=getattr(args, "name", None) or "my-zeus-pack"),
