"""Post-setup install hooks and installed-state predicates for `zeus tools` provider rows."""
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    ZeusAgent-managed ``$ZEUS_HOME/bin`` copy does."""
...
def _post_setup_lightpanda() -> None:
    ...
    if lightpanda_bin:
        ...
        if os.name == "nt":
            _print_info("    Lightpanda has no native Windows build; run ZeusAgent under WSL2.")
...
def _post_setup_agent_browser(post_setup_key: str) -> None:
    ...
    global/ZeusAgent-managed install), so there is no ``npm install`` step here."""
    ...
    try:
        ...
        # PATH/node_modules probes above. Mirror the rung zeus_cli.doctor uses so this probe can't diverge
        ...
        # agent-browser is no longer a root package.json dependency (#43564) — it resolves lazily via npx
        # (or a global/ZeusAgent-managed install) instead of a local `npm install`, so there's no node_modules/
    ...
    # Reuse the runtime resolution cascade (PATH -> Homebrew/ZeusAgent-managed node -> npx) rather than
    # a bare shutil.which — ZeusAgent-managed-Node-only setups resolve agent-browser/npx only that way.
    try:
...
_PIP_POST_SETUP_HOOKS: dict = {
    "faster_whisper": _pip_hook(
        ...
        on_install=("Model sizes: tiny, base (default), small, medium, large-v3",
                    "Change via stt.local.model in ~/.zeus/config.yaml")),
    "kittentts": _pip_hook(
    ...
    "piper": _pip_hook(
        ...
        always=("Default voice: en_US-lessac-medium (downloaded on first TTS call)",
                ...
                "Switch voices by setting tts.piper.voice in ~/.zeus/config.yaml")),
    "ddgs": _pip_hook(
...
def _post_setup_spotify() -> None:
    # Full `zeus auth spotify` flow: no client_id yet → interactive wizard (persists to ~/.zeus/.env)
    # then PKCE; existing app → OAuth only.
    from types import SimpleNamespace
    try:
        from zeus_cli.auth import login_spotify_command
    except Exception as exc:
        _print_warning(f"    Could not load Spotify auth: {exc}")
        _info_lines("Run manually: zeus auth spotify")
        return
    ...
    try:
        ...
        _info_lines("Run later: zeus auth spotify")
    except Exception as exc:
...
def _post_setup_langfuse() -> None:
    ...
    try:
        from zeus_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
        ...
        _info_lines("Run manually: zeus plugins enable observability/langfuse")
    _info_lines("Restart ZeusAgent for tracing to take effect.", "Verify: zeus plugins list")
...
def _post_setup_xai_grok() -> None:
    ...
    try:
        from zeus_cli.setup import prompt_choice, prompt as _setup_prompt
        from zeus_cli.setup_tts import _run_xai_oauth_login_from_setup
        from zeus_cli.config import save_env_value
    except Exception as exc:
        _print_warning(f"    Could not load setup helpers: {exc}")
        _info_lines("Run later: zeus auth add xai-oauth   (or set XAI_API_KEY)")
        return
    ...
    idx = prompt_choice(
        ...
        choices=["Sign in with xAI Grok OAuth (SuperGrok / Premium+) — browser login",
                 ...
                 "Skip — configure later via `zeus auth add xai-oauth`"])
    if idx == 0:
        if _run_xai_oauth_login_from_setup():
            _print_success("    Logged in — xAI will use these OAuth credentials")
        else:
            _print_warning("    xAI Grok OAuth login did not complete. Run later: zeus auth add xai-oauth")
    elif idx == 1:
        ...
        if api_key:
            ...
            _print_warning("    No API key provided. Run later: zeus auth add xai-oauth")
    else:
...
def run_post_setup_command(args) -> int:
    """``zeus tools post-setup <key>`` — non-interactive runner the dashboard spawns so the GUI can drive
    backend setup without re-implementing install logic. Exit code: 0 ok, 2 unknown key."""
    key = getattr(args, "post_setup_key", None)
    if not key:
        _print_error("Usage: zeus tools post-setup <key>")
        return 2
...
# Python deps installed via ``zeus tools`` aren't in the managed runtime's locked ``all`` sync, so a
...
def active_restorable_python_tool_dependencies() -> list[str]:
    """Return ``zeus tools`` Python dependencies present in this runtime."""
    return [
...
def _agent_browser_installed() -> bool:
    ...
    *and* the Chromium build it drives (or the Lightpanda engine, which needs no Chromium), so "Run
    ...
    from zeus_cli.nous_subscription import _local_browser_runnable
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from zeus_cli.nous_subscription import _has_agent_browser
