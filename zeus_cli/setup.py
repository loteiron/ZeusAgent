"""Interactive setup wizard for ZeusAgent (config lives in ~/.zeus/).
...
``zeus_cli.setup.<name>`` keep working.
"""
...
from zeus_cli.curses_ui import MenuNavigationEvent, MenuNavigationStart
# Config helpers are re-exported (tests patch them on this module). display_zeus_home is
# imported lazily at call sites (stale-module safety during zeus update).
from zeus_cli.config import (
    cfg_get, DEFAULT_CONFIG, get_zeus_home, get_config_path, get_env_path, load_config, save_config,
    save_env_value, remove_env_value, get_env_value, ensure_zeus_home,
)
from zeus_cli.colors import Colors, color
from zeus_cli.cli_output import print_error, print_info, print_success, print_warning
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚡ ZeusAgent Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    _info("The interactive wizard cannot be used here.", None,
          "Configure ZeusAgent using environment variables or config commands:",
          "  zeus config set model.provider custom",
          "  zeus config set model.base_url http://localhost:8080/v1",
          "  zeus config set model.default your-model-name", None,
          "Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.",
          "Run 'zeus setup' in an interactive terminal to use the full wizard.", None)
...
_SETUP_NAVIGATION: ContextVar[_SetupNavigationState | None] = ContextVar("zeus_setup_navigation", default=None)
...
def _setup_navigation_scope():
    ...
    from zeus_cli.curses_ui import reset_menu_navigation_handler, set_menu_navigation_handler
...
def run_setup_action_with_navigation(
    ...
    ``zeus model`` that use the wizard's pickers outside ``run_setup_wizard``."""
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from zeus_cli.curses_ui import curses_radiolist
...
def is_noninteractive() -> bool:
    ...
    with ``stdin=DEVNULL`` and ``ZEUS_NONINTERACTIVE=1`` (``zeus_cli/web_server.py``), where a
    prompt that aborts on EOF would kill the spawned action — callers fall back to their default."""
    return os.environ.get("ZEUS_NONINTERACTIVE", "").strip().lower() in {"1", "true", "yes", "on"}


def prompt_yes_no(question: str, default: bool = True) -> bool:
    """Prompt for yes/no. Ctrl+C exits; empty input, ``ZEUS_NONINTERACTIVE=1`` or a
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from zeus_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    print()
    _prompt_and_save_env_var(var, "  ✓ Saved", "  Skipped (configure later with 'zeus setup')")
...
def setup_model_provider(config: dict, *, quick: bool = False):
    """Configure the inference provider and default model via the ``zeus model`` flow (one code
    ...
    from zeus_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: zeus model")
    ...
    # keep safe defaults (configure via `zeus auth add` / `zeus setup tts`).
    config.clear()
...
def _apply_default_agent_settings(config: dict):
    ...
    # config.yaml is authoritative for max_turns (the gateway bridges it into ZEUS_MAX_ITERATIONS);
    ...
    _info("  Max iterations: 150", "  Tool progress: all", "  Compression threshold: 0.50",
          "  Run `zeus setup agent` later to customize.")
...
_TOOL_PROGRESS_HELP = (
    ...
    "  log     — Silent in chat; write every tool call to ~/.zeus/logs/tool_calls.log (gateway only)",
)
...
def setup_tools(config: dict, first_install: bool = False):
    """`zeus setup tools` == `zeus tools`: platform selection → toolset toggles → provider keys.
    ``first_install`` selects the simplified flow (no platform menu, prompts for all missing keys)."""
    from zeus_cli.tools_config import tools_command
...
def _record_send_consent_change(*, enabled: bool) -> None:
    ...
    try:
        from zeus_cli.observability.shared_metrics import SharedMetricsStore
        from zeus_cli.observability.shared_metrics_sender import reconcile_send_consent
        from zeus_cli.sqlite_util import write_txn
...
# zeus_cli.setup. They import this module lazily inside bodies, so this is cycle-free.

from zeus_cli.setup_tts import setup_tts  # noqa: E402
from zeus_cli.setup_terminal import setup_terminal_backend  # noqa: E402
from zeus_cli.setup_platforms import setup_gateway  # noqa: E402
from zeus_cli.setup_summary import _print_setup_summary  # noqa: E402,F401
from zeus_cli.setup_migration import _offer_openclaw_migration, _skip_configured_section  # noqa: E402
from zeus_cli.setup_quick import _run_portal_one_shot, _run_quick_setup  # noqa: E402
...
def _run_setup_section(config: dict, section: str) -> None:
    """``zeus setup <section>``: run one SETUP_SECTIONS entry under the banner."""
    ...
    if entry is None:
        ...
        return
    label, func = entry
    _print_banner(f"│     ⚡ ZeusAgent Setup — {label:<34s} │")
...
def _run_full_setup(config: dict, zeus_home, *, is_existing: bool, migration_ran: bool) -> None:
    ...
    _info(f"Config file:  {get_config_path()}", f"Secrets file: {get_env_path()}",
          f"Data folder:  {zeus_home}", f"Install dir:  {PROJECT_ROOT}", None,
          "You can edit these files directly or use 'zeus config edit'")
    if migration_ran:
    ...
    def _gateway_step() -> None:
        ...
        from zeus_cli.gateway import ensure_gateway_service
...
def _run_setup_wizard_impl(args):
    ...
    ``zeus setup <section>`` from SETUP_SECTIONS."""
    ...
    from zeus_cli.auth import get_active_provider
    is_existing = bool(get_env_value("OPENROUTER_API_KEY") or get_env_value("OPENAI_BASE_URL")
                       or get_active_provider() is not None)
    _print_banner("│             ⚡ ZeusAgent Setup Wizard                │",
                  "├─────────────────────────────────────────────────────────┤",
                  "│  Let's configure your ZeusAgent installation.       │",
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_setup_steps([("Quick Setup", lambda: _run_quick_setup(config, zeus_home))])
            return
        print_header("Reconfigure", gap=True)
        print_success("You already have ZeusAgent configured.")
        _info("Running the full wizard — each prompt shows your current value.",
              ...
              "Tip: jump straight to a section with 'zeus setup model|terminal|",
        ...
        migration_ran = _offer_openclaw_migration(zeus_home)  # before configuration begins
        if migration_ran:
            config = load_config()
        setup_mode = prompt_choice("How would you like to set up ZeusAgent?", [label for label, _ in _FIRST_TIME_MODES], 0)
        label, runner = _FIRST_TIME_MODES[setup_mode]
        if runner is not None:
            from zeus_cli import setup_quick
            _run_setup_steps([(label, lambda: getattr(setup_quick, runner)(config, zeus_home, is_existing))])
            return
    _run_full_setup(config, zeus_home, is_existing=is_existing, migration_ran=migration_ran)
    ...
    _print_setup_summary(config, zeus_home)
...
_PLUGIN_COMPAT_LAZY = {
    'get_nous_subscription_features': ('zeus_cli.nous_subscription', 'get_nous_subscription_features'),
    'get_optional_skills_dir': ('zeus_constants', 'get_optional_skills_dir'),
