...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via ZeusAgent-managed OAuth credentials."""

    auth_hint = "zeus auth add xai-oauth --type oauth"
    ...
    def get_credential(self) -> UpstreamCredential:
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    "No xAI OAuth credentials found. Run `zeus auth add xai-oauth --type oauth` first."
                )
            entry = pool.select()
            if entry is None:
                raise RuntimeError(
                    "No available xAI OAuth credentials found. Run "
                    "`zeus auth reset xai-oauth` or re-authenticate with `zeus auth add xai-oauth --type oauth`."
                )
            self._pool = pool
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `zeus auth add xai-oauth --type oauth`."
            )
        base_url = str(
