...
Split out of ``zeus_cli/config.py``; every name is re-imported there, so
``zeus_cli.config.<name>`` keeps resolving (and monkeypatching) as before. Functions that call
``get_compatible_custom_providers`` / ``load_config*`` import them lazily from ``zeus_cli.config``
...
logger = logging.getLogger("zeus_cli.config")
...
_KNOWN_PROVIDER_KEYS = {
    # ``provider`` duplicates the ``providers.<name>`` mapping key and is unused here, but ZeusAgent'
...
def _normalize_custom_provider_entry(
    ...
    # ``models_discovered`` marks a mapping auto-discovered by ZeusAgent, not hand-curated.
...
def get_custom_provider_model_capability(
    ...
    from zeus_cli.config import get_compatible_custom_providers, load_config_readonly
