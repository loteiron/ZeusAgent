...
One read-only pass answering, BEFORE any mutation: which ZeusAgent runtimes run on this machine, how
each is deployed, which ones this update touches, and how each restarts. Every collector is a
side-effect-free probe, so ``zeus update --plan`` is safe on a live fleet.
"""
...
@dataclass
class RuntimeRecord:
    """One running (or expected) ZeusAgent runtime on this machine."""
...
class UpdatePlan:
    ...
    update_mechanism: str = "zeus update"
...
def _detect_supervisor_for_pid(pid: int, service_pids: set, windows_service_pids: set | None = None) -> str:
    ...
    with suppress(Exception):
        from zeus_cli.gateway import is_macos, supports_systemd_services
...
def describe_restart_mechanism(mechanism: str, profile: str) -> str:
    ...
    return _MECHANISM_DESCRIPTIONS.get(mechanism) or (
        f"zeus -p {profile} gateway restart" if profile != "default" else "zeus gateway restart"
    )
...
def _collect_install_shape(plan: UpdatePlan) -> None:
    with _probe("Install-method probe"):
        from zeus_cli.config import detect_install_method, get_managed_system, recommended_update_command_for_method
...
def _supervisor_classifier() -> Callable[[int], str]:
    ...
    with _probe("Service-PID probe"):
        from zeus_cli.gateway import _get_service_pids
    ...
    with _probe("Windows SCM service-ownership probe"):
        from zeus_cli.gateway import find_windows_gateway_services
...
def _collect_gateway_runtimes(plan: UpdatePlan, profile_homes: list, seen: set[int]) -> None:
    ...
    with _probe("Gateway-state inventory"):
        ...
        from zeus_cli.update_receipt import _socket_identity
    ...
    with _probe("PID-file gateway inventory"):
        from zeus_cli.gateway import find_profile_gateway_processes
...
def _collect_ledger_runtimes(plan: UpdatePlan, seen: set[int]) -> None:
    ...
    (a manual `zeus serve --host <ip>` for a remote Desktop, a long-lived `zeus dashboard`).
...
def collect_runtime_inventory() -> UpdatePlan:
    ...
    with _probe("Profile enumeration"):
        from zeus_cli.update_receipt import _profile_homes
...
def print_update_plan(plan: UpdatePlan) -> None:
    ...
    if not plan.runtimes:
        print("  Running ZeusAgent services: none detected — code swap only.")
        return
...
def _serve_unit_matches_profile(profile: str, unit: object) -> bool:
    """Does *unit* name a ``zeus-serve*``/``zeus-dashboard*`` unit for *profile*? (OWN vocabulary;
    the gateway's ``zeus-gateway*`` names never cover serve/dashboard runtimes.)

    Exact names only — ``work`` must not claim ``zeus-serve-workbench`` — and a scope prefix
    (``user/zeus-serve``) is tolerated because the restart phase records scope-qualified identities in
    some lists. See #100479.
    """
    name = str(unit).removesuffix(".service").rsplit("/", 1)[-1]
    suffix = "" if profile == "default" else f"-{profile}"
    return name in {f"zeus-serve{suffix}", f"zeus-dashboard{suffix}"}
...
def _gateway_service_matches_profile(profile: str, service: object) -> bool:
    ...
    Never substring-match: ``foo`` must not claim ``zeus-gateway-foobar.service``.
    Launchd labels are ``ai.zeus.gateway`` / ``ai.zeus.gateway-<profile>`` — they do
    not contain the substring ``zeus-gateway``, so a successful macOS kickstart must
    still credit the planned default gateway. A scope prefix (``user/zeus-gateway``,
    ``gui/501/ai.zeus.gateway``) is stripped the same way serve units are.
    """
    name = str(service).removesuffix(".service").rsplit("/", 1)[-1]
    if profile == "default":
        return name in {"zeus-gateway", "ai.zeus.gateway", "gateway", "gateway-default"}
    return name in {f"zeus-gateway-{profile}", f"ai.zeus.gateway-{profile}", f"gateway-{profile}"}
...
def match_runtime_outcomes(
    ...
    See #91277.
    They never borrow the gateway's outcome: ``relaunched_profiles`` and ``zeus-gateway*`` name a
...
def report_unaccounted_runtimes(outcomes: list[dict[str, Any]]) -> bool:
    ...
    if any(o.get("kind") not in _SERVE_KINDS for o in missed):
        print("      zeus gateway restart                # active profile")
        print("      zeus -p <profile> gateway restart   # named profile")
    if any(o.get("kind") in _SERVE_KINDS for o in missed):
        ...
        # See #100479.
        print("      systemctl --user restart zeus-serve.service   # unit-managed serve")
        print("      relaunch `zeus serve` / `zeus dashboard` / the Desktop app")
    return True
...
def record_plan_in_receipt(plan: UpdatePlan) -> None:
    ...
    try:
        import zeus_cli.update_receipt as ur
