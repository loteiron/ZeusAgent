...
Split out of ``zeus_cli/main.py``. Names that still live in main (``PROJECT_ROOT``, ...)
...
logger = logging.getLogger("zeus_cli.main")
...
def _print_tui_exit_summary(session_id: Optional[str], active_session_file: Optional[str] = None) -> None:
    ...
    from zeus_cli.main import _resolve_last_session
    target = (
    ...
    print(f"\nResume this session with:\n  zeus --tui --resume {target}")
    if title:
        print(f'  zeus --tui -c "{title}"')
    print(f"\nSession:        {target}")
    if title:
        print(f"Title:          {title}")
...
def _tui_selected_workspace_keys(tui_dir: Path, ws_root: Path) -> set:
    ...
    from zeus_cli.main import _is_termux_startup_environment
    try:
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @zeus/ink is missing or node_modules is behind package-lock.json.
    ...
    if not (ws_root / "node_modules" / "@zeus" / "ink" / "package.json").is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_DIRS = ("src", "packages/zeus-ink/src")
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/zeus-ink/package.json",
    "packages/zeus-ink/index.js",
    "packages/zeus-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    ``ZEUS_TUI_FORCE_BUILD=1`` forces a rebuild."""
    force = (os.environ.get("ZEUS_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    the resolved node dir to PATH. ``ZEUS_SKIP_NODE_BOOTSTRAP=1`` disables auto-install."""
    from zeus_cli.main import PROJECT_ROOT
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("ZEUS_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "ZEUS_HOME": zeus_home},
    ...
    extras += [Path(zeus_home) / "node" / "bin", Path.home() / ".local" / "bin"]
    for extra in extras:
        s = str(extra)
...
def _find_bundled_tui(zeus_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if zeus_cli_dir is None:
        zeus_cli_dir = Path(__file__).parent
    bundled = zeus_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    tracked files after ``zeus update``); True when the directory exists afterwards.

    On Windows an antivirus / NTFS filter driver can leave tracked ``ui-tui/`` files deleted in the working
    tree after ``zeus update`` (HEAD stays intact; the files just vanish — see issue #49145). Those files
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("ZEUS_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this ZeusAgent checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `zeus update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the ZeusAgent checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `zeus --tui`\n"
        "If the checkout is still inconsistent, run `zeus update --force`.",
        file=sys.stderr)
    sys.exit(1)
...
def _tui_node_bin(bin: str) -> str:
    """Resolve ``node``/``npm`` for the TUI launch, or exit with a hint. ``ZEUS_NODE`` wins for node;
    ``find_node_executable()`` sees the managed ``$ZEUS_HOME/node`` tree a bare which() misses."""
    if bin == "node":
        env_node = os.environ.get("ZEUS_NODE")
        if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
            return env_node
    from zeus_constants import find_node_executable
    path = find_node_executable(bin)
    if not path and bin == "node":
        with contextlib.suppress(Exception):
            from zeus_cli.dep_ensure import ensure_dependency
...
def _install_tui_dependencies(tui_dir: Path, *, termux_startup: bool) -> None:
    ...
    def _run_tui_install() -> subprocess.CompletedProcess:
        from zeus_constants import with_zeus_node_path
        ...
        return subprocess.run(
            ...
            env=_npm_lifecycle_env(with_zeus_node_path()))
    ...
    if result.returncode != 0:
        ...
        from zeus_cli.npm_engine import maybe_repair_npm_engine
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (ZEUS_TUI_DIR prebuilt or esbuild)."""
    from zeus_cli.main import _is_termux_startup_environment
    _ensure_tui_node()

    # Footgun: --dev against a prebuilt bundle that has no source/node_modules.
    ext_dir = os.environ.get("ZEUS_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with ZEUS_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset ZEUS_TUI_DIR (e.g. `unset ZEUS_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr)
        sys.exit(1)
    ...
    # zeus_cli/tui_dist/entry.js but never ui-tui/ (git checkouts only).
    # 1. A prebuilt install (Docker image, Nix build, or prior `npm run build`) ships
    #   zeus_cli/tui_dist/entry.js but never ships ui-tui/ at all (that directory only exists in a git
    ...
    if tui_dev:
        # --dev runs src/entry.tsx directly, but @zeus/ink resolves through
        # packages/zeus-ink/dist/entry-exports.js; a stale dist after a pull
        # leaves newer hooks/components missing at runtime. Prebuild it here.
        npm = _tui_node_bin("npm")
        _run_tui_npm_build(npm, tui_dir / "packages" / "zeus-ink", "TUI dev prebuild failed.")
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from zeus_cli.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    from zeus_cli.main import PROJECT_ROOT
    src_root = str(env.get("ZEUS_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["ZEUS_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("ZEUS_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["ZEUS_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("ZEUS_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["ZEUS_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["ZEUS_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    import tempfile
    # TUI child is a zeus process: propagate the profile-home contract via
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="zeus-tui-active-session-", suffix=".json")
    os.close(active_session_fd)
    env["ZEUS_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env.setdefault("NODE_ENV", "development" if tui_dev else "production")

    wt_info = None
    if worktree:
        wt_info = _setup_tui_worktree()
        env["ZEUS_CWD"] = wt_info["path"]
    ...
    for key, value in (
        ("ZEUS_MODEL", model), ("ZEUS_INFERENCE_MODEL", model),
        ("ZEUS_TUI_PROVIDER", provider), ("ZEUS_INFERENCE_PROVIDER", provider),
        ("ZEUS_TUI_TOOLSETS", ",".join(_normalize_tui_toolsets(toolsets))),
        ("ZEUS_TUI_SKILLS", skills_value),
        ("ZEUS_TUI_QUERY", query), ("ZEUS_TUI_IMAGE", image),
        ("ZEUS_TUI_CHECKPOINTS", "1" if checkpoints else None),
        ("ZEUS_TUI_PASS_SESSION_ID", "1" if pass_session_id else None),
        ("ZEUS_TUI_MAX_TURNS", str(max_turns) if max_turns is not None else None),
        ("ZEUS_TUI_TOOL_PROGRESS", "verbose" if verbose else "off" if quiet else None),
        ("ZEUS_ACCEPT_HOOKS", "1" if accept_hooks else None)):
        if value:
            env[key] = value
    ...
    # ZEUS_TUI_RESUME is an internal hand-off to the Ink app. We start from a
    # full os.environ snapshot, so a stale exported value would make a plain
    # `zeus --tui` try to resume a non-existent session; only forward the id
    # argparse resolved for this invocation.
    env.pop("ZEUS_TUI_RESUME", None)
    if resume_session_id:
        env["ZEUS_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `zeus update`;
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``ZEUS_KANBAN_BOARD`` so in-process tools and shelled-out
    ``zeus kanban`` calls agree even if a concurrent ``boards switch`` flips the file mid-turn.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls (``zeus kanban …``) resolve
    the board on different paths: the env-pin if set, otherwise the global ``<root>/kanban/current`` file. A
    concurrent ``zeus kanban boards switch`` from another session can flip the file mid-turn, so the same
    ...
    """
    if os.environ.get("ZEUS_KANBAN_BOARD"):
        return
    with contextlib.suppress(Exception):
        from zeus_cli.kanban_db import get_current_board
        os.environ["ZEUS_KANBAN_BOARD"] = get_current_board()


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.zeus/skills/`` with the bundled library (idempotent, milliseconds when synced).
...
def _resolve_use_tui(args) -> bool:
    ...
    ``ZEUS_TUI=1`` → TUI; ``display.interface`` config; default classic.

    The TTY gate is load-bearing: ambient preferences must never hijack a piped
    ``zeus chat -q`` (kanban workers, cron) — the Ink no-TTY bail-out exits 0 and
