...
def _warm_gateway_module() -> None:
    ...
    for mod in (
        "zeus_cli.gateway",
        "zeus_cli.auth",  # provider auth state → copilot_auth → subprocess
        "zeus_cli.copilot_auth",
        "zeus_cli.runtime_provider",
        "zeus_cli.skin_engine",  # resolve_skin() config + skin engine init
        "zeus_cli.inventory",  # provider catalogs + models.dev cache
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    """Publish the port through an atomic ready file when ``ZEUS_DESKTOP_READY_FILE`` is set.
    ...
    """
    target = os.environ.get("ZEUS_DESKTOP_READY_FILE")
    if not target:
        return
...
def _is_serve_orphaned(
    ...
    *,
    ...
    ``ZEUS_PARENT_PID`` is the Electron PID, not necessarily our PPID (the
    Windows ``zeus.exe`` launcher adds shims), so never compare getppid().
...
def _start_parent_death_watchdog() -> None:
    ...
    """
    raw_pid = os.environ.get("ZEUS_PARENT_PID")
    # Empty inherited values mean "absent", not "marker present but blank".
    start_marker = os.environ.get("ZEUS_PARENT_START_MARKER") or None
    nonce = os.environ.get("ZEUS_PARENT_NONCE") or None
    ...
    if has_marker and (not _valid_parent_start_marker(start_marker or "") or not nonce or nonce != nonce.strip()):
        ...
        _log.warning(
            "Parent-death watchdog disabled: unusable ZEUS_PARENT_START_MARKER=%r / nonce; "
    ...
    try:
        poll = max(0.5, float(os.environ.get("ZEUS_SERVE_WATCHDOG_POLL_S", "2.0")))
    except (TypeError, ValueError):
        poll = 2.0
...
def _report_port_in_use(host: str, port: int) -> None:
    ...
    print(
        ...
        "'zeus serve' / 'zeus dashboard' backend or the ZeusAgent gateway. "
