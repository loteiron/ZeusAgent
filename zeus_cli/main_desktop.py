"""Desktop (Electron) app: build/stamp, stage-and-swap pack, exe integrity gate, macOS signing/TCC, Linux sandbox, launch (zeus gui/desktop).
...
from zeus_cli.main_tui_launch import _npm_lifecycle_env
...
def _desktop_stamp_path() -> Path:
    """Path of the desktop build stamp under $ZEUS_HOME."""
    from zeus_constants import get_zeus_home
    return get_zeus_home() / "desktop-build-stamp.json"
...
def _renderer_bundle_dir(desktop_dir: Path, *, source_mode: bool) -> Optional[Path]:
    ...
    # macOS: …/ZeusAgent.app/Contents/MacOS/ZeusAgent → …/Contents/Resources
    resources = (
...
def _desktop_packaged_executable_in(release_dir: Path) -> Optional[Path]:
    ...
    *release_dir* is electron-builder's ``directories.output`` — the live ``apps/desktop/release`` or a
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/ZeusAgent.app/Contents/MacOS/ZeusAgent"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / d / "ZeusAgent.exe" for d in ("win-unpacked", "win-ia32-unpacked", "win-arm64-unpacked")
        ]
    else:
        candidates = [
            release_dir / d / n for d in ("linux-unpacked", "linux-arm64-unpacked") for n in ("zeus", "ZeusAgent")
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # mtime can hand a wrong-architecture ZeusAgent.exe to the launcher. Prefer
        ...
        # wrong-architecture ZeusAgent.exe to the launcher, which Windows rejects with "This app can't run on
        # your computer" (#69179).
...
# PLACE: before-pack.mjs wipes ``release/<platform>- unpacked`` (or the mac ``ZeusAgent.app``) and the Electron
# unpack + asar + rename then rebuild it. Any failure after that wipe — corrupt cached zip, blocked
# download, missing dep, disk full — leaves the user with NO app, and ``zeus update`` used to report
...
# (Desktop → zeus-setup --update → `zeus update` → `zeus desktop --build-only` → relaunch) rebuilds
# ZeusAgent.exe on the end user's machine and used to verify only that the file EXISTS before declaring
...
# ZeusAgent.exe is caught BEFORE the updater replaces the working app, and the previous build can be restored
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    emulated process arch) and rejected a correctly-built ARM64 ``ZeusAgent.exe``. Binding
...
def _windows_native_machine() -> str:
    ...
    chain runs an x64 zeus-setup.exe (and thus x64 Python) on Windows-on-ARM devices, where
...
def _ensure_desktop_exe_launchable(desktop_dir: Path, packaged_executable: Optional[Path]) -> tuple:
    ...
    print(f"✗ The built ZeusAgent.exe failed its integrity check: {error}\n    at: {packaged_executable}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working ZeusAgent.exe from backup.")
        print("    Your existing version was kept and still works. Run `zeus desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `zeus desktop --force-build` to rebuild, or re-run the ZeusAgent")
...
def _redownload_electron_dist(project_root: Path, env: dict, *, mirror: Optional[str] = None) -> bool:
    ...
    if not installer.is_file():
        return False
    from zeus_constants import find_node_executable, with_zeus_node_path
    ...
    dl_env = with_zeus_node_path(env)
    if mirror:
...
def _desktop_macos_local_codesign(app: Path, *, desktop_dir: Path, identity: str = "-") -> bool:
    ...
    #    the outer ZeusAgent.app component.
...
def _desktop_macos_relaunchable_fixup(
    ...
    "ZeusAgent is damaged" and loses every grant. Clear quarantine xattrs, then sign
    ...
    if exe is None:
        return True
    # exe = .../ZeusAgent.app/Contents/MacOS/ZeusAgent  ->  app bundle = .../ZeusAgent.app
    app = exe.parents[2]
...
def _macos_create_signing_identity(
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="zeus-tcc-"))
    try:
        ...
        def _export_p12(extra_args: list) -> None:
            subprocess.run(
                [
                    ...
                    "-out", str(p12), "-passout", "pass:zeuslocal",
                ],
                capture_output=True, check=True)
...
def _desktop_macos_setup_tcc_identity(identity: str = "ZeusAgent Local Signing") -> bool:
    ...
    print(
        ...
        "stuck, reset it with:  tccutil reset All com.nousresearch.zeus"
    )
    return True
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ ZeusAgent Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ ZeusAgent Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for ZeusAgent Desktop (Linux only, best-effort).

    ``Exec`` and ``Icon`` are absolute so the entry works outside a login shell.
    ``zeus uninstall --gui`` removes it.
    """
    from zeus_cli.main import PROJECT_ROOT
    try:
        from zeus_cli.linux_desktop_entry import install_desktop_entry, is_supported
        if not is_supported():
            return
...
def _install_desktop_workspace_deps(npm: str, env: dict) -> None:
    ...
    from zeus_cli.main import PROJECT_ROOT
    from zeus_cli.main_web_build import _run_npm_install_deterministic
    ...
    nixos_env = with_zeus_node_path(_nixos_build_env())
...
def _run_desktop_pack_with_recovery(
    ...
    if build_result.returncode != 0 and staging_dir is not None and _staged_exe() is None:
        ...
        if restored:
            ...
            for p in purged:
                print(f"    - {p}")
            # The purge can't remove a win-unpacked tree whose ZeusAgent.exe is
...
def _promote_staged_desktop_app(desktop_dir: Path, staging_dir: Path) -> Path:
    ...
    # ZeusAgent.exe Windows cannot load. Verified on the STAGED exe, so a failure
...
def _build_desktop_app(desktop_dir: Path, *, source_mode: bool, npm: str, env: dict) -> Optional[Path]:
    ...
    if not source_mode:
        ...
        # A running desktop instance holds ZeusAgent.exe locked on Windows, so the
    ...
    if build_result.returncode != 0:
        ...
        if sys.platform == "win32":
            print("  If this says \"Access is denied\" on ZeusAgent.exe, close any")
            print("  running ZeusAgent desktop window and retry.")
        print("  If the log shows Electron download retries, rebuild via a mirror:")
        print("    ELECTRON_MIRROR=<mirror-base-url> zeus desktop --force-build")
...
def _desktop_launch_env(args: argparse.Namespace) -> tuple[dict, list[str]]:
    ...
    from zeus_constants import with_zeus_node_path
    # with_zeus_node_path() copies os.environ when called with no arg.
    env = with_zeus_node_path()
    for attr, key in (
        ("fake_boot", "ZEUS_DESKTOP_BOOT_FAKE"), ("ignore_existing", "ZEUS_DESKTOP_IGNORE_EXISTING")):
        if getattr(args, attr, False):
            env[key] = "1"
    if getattr(args, "zeus_root", None):
        env["ZEUS_DESKTOP_ZEUS_ROOT"] = str(Path(args.zeus_root).expanduser().resolve())
    cwd = getattr(args, "cwd", None)
    env["ZEUS_DESKTOP_CWD"] = str(Path(cwd).expanduser().resolve()) if cwd else os.getcwd()

    config_electron_flags, config_disable_gpu, config_password_store, config_ozone_hint = (
        _desktop_launch_options())
    if config_disable_gpu != "auto" and "ZEUS_DESKTOP_DISABLE_GPU" not in os.environ:
        env["ZEUS_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if sys.platform == "linux" and "ZEUS_DESKTOP_PASSWORD_STORE" not in os.environ:
        password_store = (
            config_password_store if config_password_store != "auto" else _detect_linux_password_store()
        )
        if password_store:
            env["ZEUS_DESKTOP_PASSWORD_STORE"] = password_store
...
def cmd_gui(args: argparse.Namespace):
    ...
    from zeus_cli.main import PROJECT_ROOT
    from zeus_cli.main_install_repair import _resolve_node_runtime_npm
    desktop_dir = PROJECT_ROOT / "apps" / "desktop"
    ...
    if getattr(args, "setup_tcc_identity", False):
        identity = getattr(args, "identity", None) or "ZeusAgent Local Signing"
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  zeus gui")
            sys.exit(1)
    ...
    if source_mode:
        print("→ Launching ZeusAgent Desktop from source build...")
    ...
    if not source_mode:
        print(f"→ Launching packaged ZeusAgent Desktop: {' '.join(launch_command)}")
