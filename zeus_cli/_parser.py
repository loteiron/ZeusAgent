"""Top-level argparse construction for the zeus CLI.

Only the top-level parser and the ``chat`` subparser live here. Every other subparser (model,
gateway, sessions, …) is built by ``zeus_cli/subcommands/<group>.py`` and wired in
...
# (it sets ``ZEUS_HOME`` and strips itself from ``sys.argv``), so it isn't on the parser.
...
# live parser fails. The derived path is authoritative; tests/zeus_cli/
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    argparse surface — the drift that made ``zeus --reasoning high chat …`` misread ``high`` as
...
def _inherited_flag(parser, *args, **kwargs):
    """``parser.add_argument`` + tag the Action ``inherit_on_relaunch`` for ``zeus_cli.relaunch``."""
...
_EPILOGUE = """
Examples:
    zeus                        Start interactive chat
    zeus chat -q "Hello"        Single query mode
    zeus --tui                  Launch the modern TUI (or set display.interface: tui)
    zeus --cli                  Force the classic REPL (overrides display.interface: tui)
    zeus -c                     Resume the most recent session
    zeus -c "my project"        Resume a session by name (latest in lineage)
    zeus --resume <session_id>  Resume a specific session by ID
    zeus --resume latest        Resume the most recent session (same as -c)
    zeus --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    zeus setup                  Run setup wizard
    zeus logout                 Clear stored authentication
    zeus auth add <provider>    Add a pooled credential
    zeus auth list              List pooled credentials
    zeus auth remove <p> <t>    Remove pooled credential by index, id, or label
    zeus auth reset <p> [t]     Clear exhaustion status for a provider, or one credential
    zeus auth priority <p> <t> <n>  Move a pooled credential to priority n (0 = tried first)
    zeus auth refresh <p> [t]   Refresh a pooled OAuth credential and clear its cooldown
    zeus model                  Select default model
    zeus fallback [list]        Show fallback provider chain
    zeus fallback add           Add a fallback provider (same picker as `zeus model`)
    zeus fallback remove        Remove a fallback provider from the chain
    zeus config                 View configuration
    zeus config edit            Edit config in $EDITOR
    zeus config set model gpt-4 Set a config value
    zeus gateway                Run messaging gateway
    zeus -s zeus-agent-dev,github-auth
    zeus -w                     Start in isolated git worktree
    zeus gateway install        Install gateway background service
    zeus sessions list          List past sessions
    zeus sessions browse        Interactive session picker
    zeus sessions rename ID T   Rename/title a session
    zeus logs                   View agent.log (last 50 lines)
    zeus logs -f                Follow agent.log in real time
    zeus logs errors            View errors.log
    zeus logs --since 1h        Lines from the last hour
...
def _add_top_level_flags(parser: argparse.ArgumentParser) -> None:
    ...
    inherited(parser, "-m", "--model", default=None, help=(
        ...
        "Applies to -z/--oneshot and --tui. Also settable via ZEUS_INFERENCE_MODEL env var."))
    inherited(parser, "--provider", default=None, help=(
        "Provider override for this invocation (e.g. openrouter, anthropic). "
        "Applies to -z/--oneshot and --tui. The persistent provider lives in config.yaml "
        "under model.provider — use `zeus setup` or edit the file to change it."))
    ...
    inherited(parser, "--accept-hooks", action="store_true", default=False, help=(
        ...
        "without a TTY prompt.  Equivalent to ZEUS_ACCEPT_HOOKS=1 or "
    ...
    inherited(parser, "--ignore-user-config", action="store_true", default=False,
              help="Ignore ~/.zeus/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)")
...
def _build_chat_parser(subparsers) -> argparse.ArgumentParser:
    ...
    ``zeus -m foo chat`` argparse first sets ``args.model`` from the top-level parser, then
    ...
    unless the flag is actually passed after the subcommand (tests/zeus_cli/
    test_argparse_flag_propagation.py).
    """
    chat_parser = subparsers.add_parser(
        "chat", help="Interactive chat with the agent",
        description="Start an interactive chat session with ZeusAgent")
    ...
    inherited(chat_parser, "--accept-hooks", action="store_true", default=SUPPRESS, help=(
        "Auto-approve any unseen shell hooks declared in config.yaml "
        "without a TTY prompt (see also ZEUS_ACCEPT_HOOKS env var and "
    ...
    inherited(chat_parser, "--ignore-user-config", action="store_true", default=SUPPRESS,
              help="Ignore ~/.zeus/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.")
    ...
    inherited(chat_parser, "--safe-mode", action="store_true", default=SUPPRESS,
              help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from ZeusAgent itself.")
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="zeus", description="ZeusAgent - AI assistant with tool-calling capabilities",
