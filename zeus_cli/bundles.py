"""Implementation of the ``zeus bundles`` CLI subcommand."""
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  zeus bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``zeus bundles`` argparse tree (called from zeus_cli/main.py, which owns the
    top-level subparser)."""
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `zeus bundles list`")
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``zeus bundles <subcommand>`` to the right handler."""
