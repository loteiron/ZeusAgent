...
def _serve_unit_recovery_available() -> bool:
    """Can a fresh process restart ``zeus-serve*`` units on this host?"""
...
def _qualified_serve_skips(skip_units) -> list[dict]:
    ...
    ``zeus-serve.service`` can exist in BOTH the user and system manager (two processes)."""
    rows: list[dict] = []
...
def _run_fresh_recovery_process(
    ...
    """Spawn ``zeus_cli.update_restart_recovery --stdin`` detached from this process; None when it
    could not run (no systemd-run in gateway mode, OSError, timeout) — the caller fails closed."""
    command = [sys.executable, "-m", "zeus_cli.update_restart_recovery", "--stdin"]
    env = os.environ.copy()
    env["ZEUS_UPDATE_RESTART_RECOVERY"] = "1"
    for marker in ("_ZEUS_GATEWAY", "ZEUS_GATEWAY", "ZEUS_GATEWAY_MODE"):
        env.pop(marker, None)
...
def _recover_gateway_restart_after_abort(
    ...
    part of the identity, not decoration: ``zeus-serve.service`` can exist in both the user and the system
    ...
    """
    from zeus_cli.update_cmd import _gateway_recovery_partition
...
def _warn_stale_serve_runtimes(rows) -> None:
    """Name the serve/dashboard processes still on pre-update code: ``zeus serve`` hosts
    ...
    print(
        "    Restart them before using ZeusAgent again, e.g. `systemctl --user restart zeus-serve.service`"
        " or by relaunching `zeus serve` / the Desktop app.")
