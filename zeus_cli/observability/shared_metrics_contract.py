"""Bounded product contract for the first ZeusAgent shared-metrics slice."""
...
SCHEMA_KEY = "zeus.metrics.schema_version"
SCHEMA_VERSION = "zeus.metrics.event.v2"
MODEL_CALL_SCOPE = "zeus.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "zeus.task_run"
TOOL_CALL_SCOPE = "zeus.tool_call"
CLIENT_ACTIVE_MARK = "zeus.client.active"
TOOL_APPROVAL_MARK = "zeus.tool_approval"
SKILL_LIFECYCLE_MARK = "zeus.skill.lifecycle"
SKILL_LOAD_MARK = "zeus.skill.load"
SUBSCRIBER_NAME = "zeus.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "zeus.client.active"
LEGACY_MODEL_CALL_METRIC = "zeus.model_call.count"
MODEL_ROUTE_METRIC = "zeus.model_route.count"
TASK_STARTED_METRIC = "zeus.task_run.started"
TASK_FINISHED_METRIC = "zeus.task_run.finished"
TOOL_CALL_METRIC = "zeus.tool_call.count"
TOOL_APPROVAL_METRIC = "zeus.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "zeus.skill.lifecycle.count"
SKILL_LOAD_METRIC = "zeus.skill.load.count"
...
CLIENT_RESOURCE_KEYS = frozenset({"architecture", "zeus_version", "install_method", "os_family"})
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted ZeusAgent installation method."""
    normalized = _norm(value)
    return _allowlisted("nixos" if normalized == "nix" else normalized, CLIENT_INSTALL_METHODS)


def client_resource(
    zeus_version: Any, *, os_name: Any, architecture: Any, install_method: Any
) -> dict[str, str]:
    """Build the bounded client resource attached to aggregate packages."""
    version = str(zeus_version or "").strip()
    return {
        "architecture": client_architecture(architecture),
        "zeus_version": version if 0 < len(version) <= 64 else "unknown",
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("zeus_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its ZeusAgent logical scope."""
    metadata = _relay_metadata(
        event, RUNTIME_SCHEMA_KEY, RUNTIME_SCHEMA_VERSION, "zeus.call_role"
    )
    call_role = (metadata or {}).get("zeus.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from zeus_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map ZeusAgent terminal state to bounded (outcome, end_reason, termination)."""
...
_TOOL_CATEGORY_PREFIXES = (
    ...
    (("discord", "email", "feishu", "zeus-yuanbao", "slack", "sms"), "communication"),
)


def tool_category(kwargs: dict[str, Any]) -> str:
    """Map ZeusAgent registry toolset metadata to a low-cardinality category."""
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal ZeusAgent tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to ZeusAgent."""
