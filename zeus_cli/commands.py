"""Slash command registry for the ZeusAgent CLI and gateway.
...
from zeus_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Key in ``zeus_cli.slash_exec.EXECUTORS`` (a string, not a callable: keeps this module
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of ZeusAgent config/state", "Session",
    ...
    CommandDef("stop", "Stop current work and pause session schedules", "Session",
    ...
    CommandDef("goal", "Set a standing goal ZeusAgent works on across turns until achieved", "Session",
    ...
    CommandDef("autonom", "Complete work autonomously without questions in this session", "Session",
               aliases=("autonomous",), args_hint="[task | on | off | status]",
               subcommands=("on", "off", "status"), argument_mode="mixed", busy_policy="dispatch"),
    CommandDef("evidence", "Inspect workspace checks, failures, freshness and baseline changes (read only)", "Info",
               cli_only=True, args_hint="[status]", subcommands=("status",), desktop="terminal"),
    CommandDef("experience", "Inspect learned failures, repairs and source freshness (read only)", "Info",
               args_hint="[list|status|recall <words>|show <id>]", subcommands=("list", "status", "recall", "show"),
               busy_policy="dispatch", argument_mode="mixed"),
    ...
    CommandDef("plan", "Write a markdown implementation plan to .zeus/plans/ without executing anything", "Session",
               args_hint="[task]"),
    ...
    CommandDef("wake", "Toggle the 'Hey ZeusAgent' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]", subcommands=("on", "off", "status")),
    CommandDef("busy", "Control how messages behave while ZeusAgent is working", "Configuration",
    ...
    CommandDef("reload-skills", "Re-scan ~/.zeus/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update ZeusAgent to the latest version", "Info",
               busy_policy="dispatch", desktop="terminal"),
    CommandDef("version", "Show ZeusAgent version", "Info", aliases=("v",),
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``zeus skills list`` and the agent's ``/skill-name`` dispatch but
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'discord_skill_commands_by_category': ('zeus_cli.commands_platforms', 'discord_skill_commands_by_category'),
    'slack_app_manifest': ('zeus_cli.commands_platforms', 'slack_app_manifest'),
    'slack_native_slashes': ('zeus_cli.commands_platforms', 'slack_native_slashes'),
    'slack_subcommand_map': ('zeus_cli.commands_platforms', 'slack_subcommand_map'),
    'telegram_bot_commands': ('zeus_cli.commands_platforms', 'telegram_bot_commands'),
    'telegram_menu_commands': ('zeus_cli.commands_platforms', 'telegram_menu_commands'),
    'telegram_menu_max_commands': ('zeus_cli.commands_platforms', 'telegram_menu_max_commands'),
}
