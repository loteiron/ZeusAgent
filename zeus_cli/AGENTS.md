# zeus_cli/ + cli.py — CLI, slash commands, config, skins, updater, profiles

Applies on top of the root `AGENTS.md`. Long-form: `website/docs/developer-guide/cli-internals.md`.

## CLI architecture

`cli.py` holds `ZeusAgentCLI` (REPL loop, config, slash dispatch); behaviour lives in mixins
`zeus_cli/cli_commands_mixin.py`, `cli_stream_mixin.py`, `cli_status_bar_mixin.py`,
...
`ZeusAgentCLI._SLASH_DISPATCH` (`canonical -> (method name, pass_arg)`), falling back to a
`_handle_<name>_command` method by naming convention. **There is no `elif` ladder — do not add one.**
Skill slash commands (`agent/skill_commands.py`) scan `~/.zeus/skills/` and inject as a **user
message**, never into the system prompt (prompt caching).

Rules: all interactive menu-pickers use curses (`zeus_cli/curses_ui.py`; example
`zeus_cli/tools_config.py`). Never emit `\033[K` (ANSI erase-to-EOL) in spinner/display code —
...
## Slash command registry (`zeus_cli/commands.py`)
...
`zeus_cli/goal_command.py::dispatch_goal_command` owns `/goal` parsing and manager
...
## Config system (`zeus_cli/config.py`)
...
- **Three loaders — know which you're in:** `load_cli_config()` (CLI, `cli.py`); `load_config()`
  (`zeus tools/setup`, most subcommands, `zeus_cli/config.py`, merges `DEFAULT_CONFIG`); raw
...
## Skin engine (`zeus_cli/skin_engine.py`)
...
`load_skin(name)` (user `~/.zeus/skins/*.yaml` → built-ins → default; missing values inherit
...
## Update pipeline (`zeus update`) — transactional; every stage guards a real field failure
...
- **Plan** (`update_inventory.py`, `zeus update --plan`): read-only inventory — install kind, all
...
- **Restart-per-kind**: systemd and launchd restarts are FLEET-WIDE (every `zeus-gateway*` unit /
  `ai.zeus.gateway*` LaunchAgent), drain-first (SIGUSR1), with per-unit/per-label failure
...
- **Report**: every run writes a machine-readable receipt to `~/.zeus/logs/update_receipts/`
...
`_apply_profile_override()` in `zeus_cli/main.py` sets `ZEUS_HOME` before any module import, so
every `get_zeus_home()` scopes to the active profile (rules in root). Profiles are independent
