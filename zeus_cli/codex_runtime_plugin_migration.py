"""Migrate ZeusAgent MCP server config and Codex's installed curated plugins into ~/.codex/config.toml.
"""
...
MIGRATION_END_MARKER = "# end zeus-agent managed section"
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in ZeusAgent config.")
...
# ZeusAgent MCP keys codex understands (transport stdio/http, timeouts, general). Any other key is
# dropped with a warning: ``sampling`` has no codex equivalent; the rest are unknown ZeusAgent keys.
_KNOWN_ZEUS_KEYS = {
...
# (zeus key, codex key, skip note) — timeouts are emitted as floats or skipped when non-numeric.
_TIMEOUT_KEYS = (
...
def _translate_one_server(name: str, zeus_cfg: dict) -> tuple[Optional[dict], list[str]]:
    """Translate one ZeusAgent MCP server config to codex's inline-table dict.

    Returns ``(codex_entry, skipped_keys)``; ``codex_entry`` is None when the config is unusable.
    stdio (``command``) wins over ``url`` when both are set. ZeusAgent' ``transport: sse`` hint is
    informational only — codex auto-negotiates. ``enabled`` is emitted only when explicitly false
    (codex defaults to true).
    """
    if not isinstance(zeus_cfg, dict):
        return None, []
    skipped: list[str] = []
    out: dict[str, Any] = {}
    if zeus_cfg.get("command"):
        if zeus_cfg.get("url"):
            skipped.append("url (both command and url set; preferring stdio)")
        out["command"] = str(zeus_cfg["command"])
        if zeus_cfg.get("args"):
            out["args"] = [str(a) for a in zeus_cfg["args"]]
        if zeus_cfg.get("env"):
            out["env"] = _str_map(zeus_cfg["env"])
        if zeus_cfg.get("cwd"):
            out["cwd"] = str(zeus_cfg["cwd"])
    elif zeus_cfg.get("url"):
        out["url"] = str(zeus_cfg["url"])
        if zeus_cfg.get("headers"):
            out["http_headers"] = _str_map(zeus_cfg["headers"])
        if zeus_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]
    for zeus_key, codex_key, note in _TIMEOUT_KEYS:
        if zeus_key in zeus_cfg:
            try:
                out[codex_key] = float(zeus_cfg[zeus_key])
            except (TypeError, ValueError):
                skipped.append(note)
    if zeus_cfg.get("enabled") is False:
        out["enabled"] = False
    for key in zeus_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            skipped.append(f"{key} (no codex equivalent)")
        elif key not in _KNOWN_ZEUS_KEYS:
            skipped.append(f"{key} (unknown ZeusAgent key)")
    return out, skipped
...
# env-var passthrough (ZEUS_HOME, PYTHONPATH) could carry one in pathological cases.
_TOML_ESCAPES = (
    ("\\", "\\\\"), ('"', '\\"'), ("\b", "\\b"), ("\t", "\\t"),
    ("\n", "\\n"), ("\f", "\\f"), ("\r", "\\r"))
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out += ["# (no MCP servers, plugins, or permissions configured by ZeusAgent)", MIGRATION_END_MARKER]
        return "\n".join(out) + "\n"
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(codex_home=str(codex_home) if codex_home else None) as client:
            client.initialize(client_name="zeus-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    Such dirs are reaped between sessions; a ZEUS_HOME pointing there burned into
...
def _build_zeus_tools_mcp_entry() -> dict:
    """Codex stdio entry launching ZeusAgent' own tool surface as an MCP server (browser/web/
    delegate_task/vision/memory/skills call-backs).

    ZEUS_HOME passes through only IF SET, read from os.environ (not get_zeus_home()): when
    unset the codex subprocess must inherit its launcher's runtime ZEUS_HOME (systemd, gateway,
    kanban), not a migrate-time default burned into config.toml that pins the wrong profile. The
    pytest-tempdir guard keeps a sibling test's monkeypatched ZEUS_HOME out of the user's real
    config. PYTHONPATH passes through so a worktree-launched zeus finds the branch's modules.
    """
    import sys
    env: dict[str, str] = {}
    # ZEUS_HOME passes through IF SET so the MCP subprocess sees the same config / auth / sessions DB as
    # the parent CLI. Read from os.environ (not get_zeus_home()) on purpose: when the env var is unset we
    # want codex's subprocess to inherit whatever ZEUS_HOME its launcher sets at runtime (systemd unit,
    # gateway, kanban dispatcher, custom shell), rather than burning the migrate-time resolved default into
    # config.toml — that would override the launcher's ZEUS_HOME and pin the subprocess to the wrong
    # profile. The pytest-tempdir guard below catches the issue #26250 Bug C scenario: a sibling test's
    # monkeypatch.setenv("ZEUS_HOME", tmp_path) would otherwise leak a transient pytest tempdir into the
    # user's real ~/.codex/config.toml and silently brick codex once the tempdir is GC'd.
    zeus_home = os.environ.get("ZEUS_HOME") or ""
    if zeus_home and not _looks_like_test_tempdir(zeus_home):
        env["ZEUS_HOME"] = zeus_home
    if os.environ.get("PYTHONPATH"):
        env["PYTHONPATH"] = os.environ["PYTHONPATH"]
    # Quiet mode + redaction defaults so the MCP wire stays clean.
    env["ZEUS_QUIET"] = "1"
    env["ZEUS_REDACT_SECRETS"] = env.get("ZEUS_REDACT_SECRETS", "true")
    return {
...
def migrate(
    zeus_config: dict, *, codex_home: Optional[Path] = None, dry_run: bool = False,
    discover_plugins: bool = True, default_permission_profile: Optional[str] = ":workspace",
    expose_zeus_tools: bool = True) -> MigrationReport:
    """Translate ZeusAgent mcp_servers config + Codex curated plugins into ~/.codex/config.toml.
    ...
    approval prompt on every write. ``expose_zeus_tools`` registers ZeusAgent' own tool surface
    (agent/transports/zeus_tools_mcp_server.py, launched on demand by codex over stdio) as an MCP
    ...
    zeus_servers = (zeus_config or {}).get("mcp_servers") or {}
    if not isinstance(zeus_servers, dict):
        report.errors.append("mcp_servers in ZeusAgent config is not a dict; cannot migrate.")
        return report
    translated: dict[str, dict] = {}
    for raw_name, cfg in zeus_servers.items():
        name = str(raw_name)
    ...
    if expose_zeus_tools:
        translated["zeus-tools"] = _build_zeus_tools_mcp_entry()
        if "zeus-tools" not in report.migrated:
            report.migrated.append("zeus-tools")
