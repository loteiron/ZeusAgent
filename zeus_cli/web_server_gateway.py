...
def _probe_gateway_health() -> tuple[bool, dict | None]:
    ...
    """
    from zeus_cli.web_server import _GATEWAY_HEALTH_TIMEOUT, _GATEWAY_HEALTH_URL
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    """
    try:
        from zeus_cli.profiles import _check_gateway_running, profiles_to_serve
...
# ~/.zeus/logs/<action>.log which the dashboard tails.

_ACTION_LOG_DIR: Path = get_zeus_home() / "logs"
...
def _spawn_zeus_action(
    ...
    action_id = (env_overrides or {}).get("ZEUS_ACTION_ID")
    if action_id:
