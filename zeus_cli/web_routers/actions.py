"""Gateway restart/drain, ZeusAgent update and background-action status dashboard routes.

Extracted from ``zeus_cli.web_server``; helpers/state that tests monkeypatch on
...
from zeus_cli.config import format_docker_update_message, recommended_update_command_for_method
from zeus_cli.web_deps import LateState, late
from zeus_cli.web_server_gateway import _ACTION_LOG_FILES
from zeus_cli.web_routers._common import http_failure
...
detect_install_method = late("detect_install_method", "zeus_cli.config")
get_zeus_home = late("get_zeus_home", "zeus_cli.config")
_ACTION_COMMANDS = LateState("_ACTION_COMMANDS", "zeus_cli.web_server_gateway")
_ACTION_IDS = LateState("_ACTION_IDS", "zeus_cli.web_server_gateway")
_ACTION_PROCS = LateState("_ACTION_PROCS", "zeus_cli.web_server_gateway")
_ACTION_RESULTS = LateState("_ACTION_RESULTS", "zeus_cli.web_server_gateway")
...
def _server_path(name: str) -> Path:
    ...
    import zeus_cli.web_server_gateway as gw
...
_UPDATE_ACTION_COMPLETED_RE = re.compile(r"^=== zeus-update completed ([0-9a-f]{32}) ===$")

_MANAGED_EXTERNALLY_MESSAGE = "ZeusAgent updates are managed outside this dashboard in containerized environments."
...
def _durable_completed_update_action_id(lines: List[str]) -> Optional[str]:
    ...
    for index, line in enumerate(lines):
        if line.startswith("=== zeus update started "):
            last_start = index
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``zeus gateway restart`` in the background."""
...
def _update_refused(error: str, message: str, update_command: str) -> Dict[str, Any]:
    _record_completed_action("zeus-update", message, exit_code=1)
    return {
        "ok": False, "pid": None, "name": "zeus-update", "error": error, "message": message,
        "update_command": update_command,
    }


@router.post("/api/zeus/update")
async def update_zeus():
    """Kick off ``zeus update`` in the background."""
    from zeus_cli.distribution import REMOTE_RELEASES_AVAILABLE, source_update_message
    if not REMOTE_RELEASES_AVAILABLE:
        return _update_refused("source_distribution", source_update_message(), "python scripts/setup_zeus.py")
    ...
    existing = _ACTION_PROCS.get("zeus-update")
    if existing is not None and existing.poll() is None:
        response = {"ok": True, "pid": existing.pid, "name": "zeus-update", "already_running": True}
        action_id = _ACTION_IDS.get("zeus-update")
        if action_id:
            response["action_id"] = action_id
        return response

    action_id = secrets.token_hex(16)
    with http_failure("Failed to spawn zeus update", 500, "Failed to start update"):
        proc = _spawn_zeus_action(["update"], "zeus-update", env_overrides={"ZEUS_ACTION_ID": action_id})
    return {"ok": True, "pid": proc.pid, "name": "zeus-update", "action_id": action_id}
...
_NON_APPLYABLE_MESSAGES = {
    ...
    "apt": lambda: "ZeusAgent is managed by Termux APT; run `pkg upgrade zeus-agent`.",
}


@router.get("/api/zeus/update/check")
async def check_zeus_update(force: bool = False):
    """Report whether a ZeusAgent update is available, without applying it.
    ...
    if not REMOTE_RELEASES_AVAILABLE:
        return {
            "install_method": "source-fork", "current_version": __version__, "behind": None,
            "update_available": False, "can_apply": False,
            "update_command": "python scripts/setup_zeus.py", "message": source_update_message(),
        }
    if _dashboard_local_update_managed_externally():
        return {
    ...
    try:
        from zeus_cli.banner import check_for_updates

        if force:
            with contextlib.suppress(OSError):
                (get_zeus_home() / ".update_check").unlink()
...
async def get_action_status(name: str, lines: int = 200):
    ...
    if name == "zeus-update":
        ...
        if durable_update_action_id:
            marker = f"=== zeus-update completed {durable_update_action_id} ==="
...
def _latest_update_receipt_summary() -> Optional[Dict[str, Any]]:
    """Compact summary of the latest receipt (written by EVERY ``zeus update`` run,
    incl. refused/failed), or None; never raises. Steps/skips stay in the full endpoint.

    Phase-1 bullet 3 (#91277): the receipt (written by EVERY ``zeus update`` run since #91283, including
...
@status_router.get("/api/zeus/update/receipt")
async def get_update_receipt():
    ...
    receipt = _read_latest_receipt()
    if not receipt:
        raise HTTPException(status_code=404, detail="No update receipt found (no `zeus update` run recorded).")
