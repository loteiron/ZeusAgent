...
Three layers make every long-lived ZeusAgent process positively identifiable, so reapers (``zeus
update``, Desktop startup sweeps) never guess lineage from PPID archaeology or cmdline matching:
1. spawn tags (``ZEUS_SPAWN`` env stamped by the spawner); 2. a ``(pid, create_time)`` ledger;
...
def install_id(project_root: Optional[Path] = None) -> str:
    ...
    if project_root is None:
        try:
            from zeus_constants import PROJECT_ROOT as _root
...
def build_spawn_tag(purpose: str, *, project_root: Optional[Path] = None) -> str:
    """Value for the child's ``ZEUS_SPAWN`` env var, stamped by the spawner."""
...
def parse_spawn_tag(raw: object) -> Optional[SpawnTag]:
    """Parse a ``ZEUS_SPAWN`` value; ``None`` for anything malformed."""
...
def _ledger_path() -> Path:
    ...
    try:
        ...
        return Path(get_default_zeus_root()) / LEDGER_FILENAME
    except Exception:
        from zeus_cli.config import get_zeus_home

        return Path(get_zeus_home()) / LEDGER_FILENAME
...
def register_self(purpose: str, *, project_root: Optional[Path] = None, detail: Optional[dict] = None) -> bool:
    ...
    try:
        ...
        # 10 tokens: enough for `zeus serve --host X --port N --profile P` while bounding
...
def _desktop_spawner_identity() -> tuple[Optional[int], Optional[float]]:
    """Spawner ``(pid, create_time)`` from the Electron app's ZEUS_PARENT_PID (+ optional
    ...
    try:
        spawner_pid = int(os.environ.get("ZEUS_PARENT_PID", ""))
    except (TypeError, ValueError):
        spawner_pid = 0
    if spawner_pid <= 0:
        return None, None
    marker = os.environ.get("ZEUS_PARENT_START_MARKER", "")
...
def _append_entry(entry: LedgerEntry) -> bool:
    ...
    The thread and file locks cover read/prune/write together. Atomic replacement alone
    cannot prevent two separate processes from overwriting each other's registrations.

    See #91660.
    """
    from zeus_cli.active_sessions import _FileLock

    path = _ledger_path()
    try:
        with _LEDGER_LOCK, _FileLock(path.with_suffix(".lock")):
...
def register_child(pid: int, purpose: str, *, project_root: Optional[Path] = None) -> bool:
    ...
    helpers: arbitrary ``npx``/binary servers never import ZeusAgent code). Records the child's
...
def ledger_entries(*, project_root: Optional[Path] = None) -> list[dict]:
    ...
    try:
        with _LEDGER_LOCK:
            entries = _read_ledger(path)
            if entries is None:
                # Re-read under the writer lock before quarantining; a valid snapshot
                # and an absent ledger remain read-only operations.
                with _FileLock(path.with_suffix(".lock")):
                    entries = _read_ledger_or_quarantine(path)
    except (OSError, RuntimeError):
        logger.debug("spawn ledger read failed", exc_info=True)
        return []
    if entries is None:
        return []
    return [
