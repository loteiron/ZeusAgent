...
resolved through the module object so test patches on ``zeus_cli.setup.<name>`` take effect."""

import logging
from tools import tool_backend_helpers
from zeus_cli import nous_subscription
...
_TTS_SUMMARY_ROWS = {
    ...
    "neutts": ("NeuTTS", "neutts", "run 'zeus setup tts'"),
    "kittentts": ("KittenTTS", "kittentts", "run 'zeus setup tts'")}
...
_STT_SUMMARY_DEFAULT = ("Local Whisper", "faster_whisper", "run 'zeus tools' → Speech-to-Text")
...
_EDIT_WIZARD_ROWS = (
    ("zeus setup", "          Re-run the full wizard"), ("zeus setup model", "    Change model/provider"),
    ("zeus setup terminal", " Change terminal backend"), ("zeus setup gateway", "  Configure messaging"),
    ("zeus setup tools", "    Configure tool providers"))
_EDIT_CONFIG_ROWS = (
    ("zeus config", "         View current settings"), ("zeus config edit", "    Open config in your editor"),
    ("zeus config set <key> <value>", ""))
_READY_ROWS = (
    ("zeus", "              Start chatting"), ("zeus gateway", "      Start messaging gateway"),
    ("zeus doctor", "       Check for issues"))
...
def _vision_row(config, feats):
    ...
    try:
        ...
        ok = False
    return ("Vision (image analysis)", ok, None if ok else "run 'zeus setup' to configure")
...
def _video_gen_row(config, feats):
    # Opt-in via `zeus tools` → Video Generation. Only show the row when a plugin reports
...
def _modal_row(config, feats):
    ...
    if _setup.cfg_get(config, "terminal", "backend") == "modal":
        ...
        return ("Modal Execution", False, "run 'zeus setup terminal'")
...
def _spotify_row(config, feats):
    # OAuth via zeus auth spotify — check auth.json, not env vars
    try:
...
def _print_setup_summary(config: dict, zeus_home):
    ...
    try:
        ...
        print()
        _setup.print_warning("No inference provider is configured — ZeusAgent cannot chat yet.")
        _setup._info("  Finish this one step with either of:",
              "    zeus model            (pick any provider/model)",
              "    zeus setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if available_count < len(tool_status):
        _setup.print_warning("Some tools are disabled. Run 'zeus setup tools' to configure them,")
    ...
    for label, value in (("Settings:", f"  {_setup.get_config_path()}"), ("API Keys:", f"  {_setup.get_env_path()}"),
                         ("Data:", f"      {zeus_home}/cron/, sessions/, logs/")):
...
import zeus_cli.setup as _setup  # noqa: E402  (bottom: zeus_cli.setup imports this module)
