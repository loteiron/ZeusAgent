"""``zeus plugins validate`` — admission checks for a plugin directory.
...
_PROBE_TIMEOUT = 30
_PROBE_SENTINEL = "ZEUS_VALIDATE_JSON:"
...
def _check_requires_zeus(report: ValidationReport, manifest: dict) -> None:
    spec = str(manifest.get("requires_zeus") or "").strip()
    if not spec:
        report.add("requires_zeus", True, "not declared")
        return
    if _requires_zeus_spec_valid(spec):
        report.add("requires_zeus", True, f"spec {spec!r} parses")
    else:
        report.add(
            "requires_zeus",
            False,
            f"requires_zeus spec {spec!r} does not parse "
...
# from zeus so a hostile plugin only sees a bare interpreter.
_PROBE_SCRIPT = r"""
...
def _run_capability_probe(plugin_dir: Path) -> Tuple[Optional[dict], str]:
    ...
    *error* is a human-readable failure description otherwise.
    """
    with tempfile.TemporaryDirectory(prefix="zeus-validate-") as scratch:
        env = dict(os.environ)
        env["ZEUS_HOME"] = scratch
        try:
...
def validate_plugin_dir(plugin_dir: Path) -> ValidationReport:
    ...
    _check_requires_zeus(report, manifest)
