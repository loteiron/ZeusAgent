"""On-demand supply-chain audit for ZeusAgent installs.
...
def _discover_plugins(zeus_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.zeus/plugins``. Plugins typically don't install
...
def _discover_components(
    *, skip_venv: bool = False, skip_plugins: bool = False, skip_mcp: bool = False, zeus_home: Optional[Path] = None
) -> list[Component]:
    """Discover all scannable components across the enabled sources."""
    home = zeus_home or Path(get_zeus_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `zeus security audit`."""
    home = Path(get_zeus_home())
    ...
    components = _discover_components(zeus_home=home, **skips)
    ...
    try:
        findings = run_audit(zeus_home=home, components=components)
