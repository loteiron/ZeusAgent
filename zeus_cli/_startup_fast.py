...
Imported by ``zeus_cli/main.py`` BEFORE its heavy import wall (config, argparse tree, logging,
providers). Everything here must stay **stdlib-only and cheap** (os/sys file probes; no yaml, no
zeus_cli.config, no argparse). Exists so version-printing stops being reimplemented as
...
__all__ = [
    ...
    "print_fast_version_info", "try_fast_version", "is_cli_entrypoint",
]


def is_cli_entrypoint(module_name: str, argv0: str) -> bool:
    """Whether main.py owns argv (module/script execution or a Zeus console script).

    Importers such as pytest and embedded applications have their own -p/--version
    flags. They must not select a Zeus profile or exit merely by importing helpers.
    """
    return module_name == "__main__" or os.path.basename(argv0).lower() in {
        "zeus", "zeus.exe", "zeus-script.py",
    }
...
def active_profile_may_override_home(zeus_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect ZEUS_HOME?"""
    active = (_read_text(os.path.join(zeus_root, "active_profile")) or "").strip()
    return bool(active and active != "default")


def _default_home() -> str:
    return os.path.join(os.path.expanduser("~"), ".zeus")


def _resolved_home() -> str:
    return os.environ.get("ZEUS_HOME", "").strip() or _default_home()
...
def container_mode_may_be_active() -> bool:
    ...
    be active".
    """
    if os.environ.get("ZEUS_DEV") == "1" or is_container_startup_environment():
        return False
    zeus_home = os.environ.get("ZEUS_HOME", "").strip()
    if zeus_home:
        if os.path.exists(os.path.join(zeus_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(zeus_home)))
        return parent_name != "profiles" and active_profile_may_override_home(zeus_home)
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    """THE canonical ``zeus --version`` output (also used by /version).
    ...
    try:
        ...
        from zeus_cli import __release_date__, __version__

        print(f"ZeusAgent v{__version__} ({__release_date__})")
    ...
    try:
        from zeus_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``zeus --version`` before the heavy import wall.
    ...
    ZEUS_TERMUX_DISABLE_FAST_CLI escape hatch.
    """
    if argv is None:
        argv = sys.argv[1:]
    is_termux = is_termux_env()
    if is_termux and os.environ.get("ZEUS_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
