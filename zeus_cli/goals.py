"""Persistent session goals — the Ralph loop for ZeusAgent.
...
# (schema DDL, FTS tables, first zeus_cli.config import) measures ~300ms warm and more on slow
...
def _get_session_db() -> Optional[Any]:
    """Cached SessionDB per ZEUS_HOME (profile switches pick the right DB); None on any failure.
...
def migrate_goal_to_session(old_session_id: str, new_session_id: str, *, reason: str = "") -> bool:
    ...
    try:
        ...
        def transfer(conn):
            old_key, new_key = _meta_key(old_session_id), _meta_key(new_session_id)
            rows = dict(conn.execute("SELECT key, value FROM state_meta WHERE key IN (?, ?)", (old_key, new_key)))
            if old_key not in rows or new_key in rows:
                return False
            state = GoalState.from_json(rows[old_key])
            if state.status == "cleared":
                return False
            # Read, create and archive share one write lock and one commit. A failed
            # child write must never lose the parent; a failed archive must not duplicate work.
            cursor = conn.cursor()
            db.set_meta(new_key, state.to_json(), cursor=cursor)
            state.status = "cleared"
            db.set_meta(old_key, state.to_json(), cursor=cursor)
            return True

        migrated = db._execute_write(transfer)
        if migrated:
...
def _extract_json_object(raw: str) -> Optional[Dict[str, Any]]:
    """Accept a single object with optional prose/fences, preserving JSON string boundaries.

    Decode from the first opening brace only. Never salvage an inner object from
    malformed outer JSON or accept a list of competing decisions.
    """
...
def _parse_judge_response(raw: str) -> Tuple[str, str, bool, Optional[Dict[str, Any]]]:
    ...
    if isinstance(verdict_raw, str):
        ...
        if type(done_val) is bool:
            done = done_val
        elif isinstance(done_val, str) and done_val.strip().lower() in {"true", "yes", "1", "done", "false", "no", "0"}:
            done = done_val.strip().lower() in {"true", "yes", "1", "done"}
        else:
            return "continue", "judge reply contained no valid verdict", True, None
        verdict = "done" if done else "continue"
    if verdict not in {"done", "blocked", "continue", "wait"}:
        return "continue", "judge reply contained an unknown verdict", True, None
    if verdict != "wait":
