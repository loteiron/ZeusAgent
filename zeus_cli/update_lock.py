"""Cross-process mutual exclusion for in-flight ZeusAgent updates.
...
MARKER_NAME = ".zeus-update-in-progress"

# Set by an orchestrating updater (Tauri `zeus-setup --update`) to its own pid before
# spawning `zeus update` as a child stage; the parent holds the marker for its whole run,
...
HANDOFF_PID_ENV = "ZEUS_UPDATE_HANDOFF_PID"
...
# Tauri updater (UPDATE_EXIT_CONCURRENT in update.rs) to show "ZeusAgent is still running".
...
def update_marker_path() -> Path:
    ...
    Uses the *process* ZeusAgent home (never the context-local profile override): the Rust
    updater resolves ``$ZEUS_HOME`` or the platform default and the desktop pins that same
    ...
    return get_process_zeus_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``zeus update`` as a (grand)child, so a live marker
...
def read_live_update(*, path: Path | None = None) -> UpdateHolder | None:
    ...
    if not math.isfinite(started_at) or not _pid_alive(pid) or age > UPDATE_MARKER_MAX_AGE_SECONDS:
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another ZeusAgent update is already running (PID {holder.pid}, "
