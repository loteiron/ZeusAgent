...
All HttpOnly, ``SameSite=Lax`` unless noted, Path = proxy prefix or /: ``zeus_session_at``
(access token; Max-Age = token TTL), ``zeus_session_rt`` (rotating refresh token; written only
when the provider returned one, always cleared on logout/expiry), ``zeus_session_provider``
(non-secret routing hint so an RT is not handed to the wrong provider), ``zeus_session_pkce``
...
cookies set on such a 302, crbug 40508226), ``zeus_sso_attempt`` (auto-SSO loop guard, 60 s).
...
SESSION_AT_COOKIE = "zeus_session_at"
SESSION_RT_COOKIE = "zeus_session_rt"
SESSION_PROVIDER_COOKIE = "zeus_session_provider"
PKCE_COOKIE = "zeus_session_pkce"
SSO_ATTEMPT_COOKIE = "zeus_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    """``Path=/zeus`` under a proxy prefix (no leak to sibling apps), else ``/``."""
...
def set_session_cookies(
    ...
    Nous Portal issues a 24h rotating refresh token (zeus #37247); a provider that omits it returns
