"""Audit log for dashboard-auth events: ``$ZEUS_HOME/logs/dashboard-auth.log``, one JSON object
per line. Token-like fields are stripped before serialisation so refresh tokens / JWTs never
reach disk. Minimal import surface (no ``zeus_constants`` at import time) so early-loading
...
def _resolve_log_path() -> Path:
    ...
    return get_zeus_home() / "logs" / "dashboard-auth.log"
