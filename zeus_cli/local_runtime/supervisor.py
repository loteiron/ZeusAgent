...
from zeus_cli.local_runtime.binaries import server_binary, runtimes_root
...
def state_path() -> Path:
    """Endpoint state for other ZeusAgent processes (provider resolution routes llamacpp-alias
...
def _stable_api_key() -> str:
    ...
    connection errors.
    """
    from zeus_cli.auth import _auth_store_lock

    key_path = runtimes_root() / ".api_key"
    with _auth_store_lock(timeout_seconds=5, target_path=key_path):
        existing = ""
        with suppress(FileNotFoundError, UnicodeError):
        ...
        try:
            atomic_write_text(key_path, key, create_mode=0o600)
...
class LlamaServerSupervisor:
    """Own one llama-server router process for the life of a ZeusAgent session."""
    ...
    def __init__(self, install_dir: Path, models_dir: Path, *,
        ...
        self._state_identity: tuple[int | None, int | None] | None = None
    ...
    def _spawn(self) -> None:
        # stop() must not finish between Popen and publication of server.json.
        with self._lifecycle_lock:
            if self._stopping:
                raise RuntimeError("llama-server supervisor is stopped")
            self._spawn_locked()
    ...
    def _spawn_locked(self) -> None:
        ...
        self._log_handle.write(
            f"\n# spawn: {exe} port={self.port} models_dir={self.models_dir}\n")
    ...
    def start(self, timeout_s: int = 120) -> None:
        ...
        with self._lifecycle_lock:
            if self._stopping:
                raise RuntimeError("llama-server supervisor stopped during startup")
            self._write_state()
    ...
    def _write_state(self) -> None:
        ...
        started = _get_process_start_time(pid) if pid is not None else None
        with _auth_store_lock(timeout_seconds=5, target_path=path):
            atomic_json_write(path, {"base_url": self.base_url, "api_key": self.api_key,
                                     "pid": pid, "process_start_time": started}, mode=0o600)
            self._state_identity = (pid, started)
        ...
        try:
            with _auth_store_lock(timeout_seconds=5, target_path=path):
                ...
                if (isinstance(state, dict)
                        and (state.get("pid"), state.get("process_start_time")) == self._state_identity
                        and state.get("api_key") == self.api_key):
                    path.unlink(missing_ok=True)
        except FileNotFoundError:
            pass
        except (OSError, ValueError) as exc:
            logger.warning("could not remove managed server state (%s); continuing shutdown", exc)
