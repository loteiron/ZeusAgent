...
def _managed_state() -> dict | None:
    """Read validated state including the identity needed by lifecycle callers."""
    ...
    if not isinstance(base_url, str) or not base_url or not isinstance(api_key, str):
        return None
    pid = state.get("pid")
    ...
    try:
        ...
        return None
    # Ownership proof: on the stable port a SECOND install (different ZEUS_HOME) can own
    ...
    if not _pid_alive(pid):
        return None
    return {"base_url": base_url, "api_key": api_key, "pid": pid,
...
def _state_endpoint() -> dict | None:
    ...
    if state is None:
        return None
    return {"base_url": state["base_url"], "api_key": state["api_key"]}
...
def resolve_llamacpp_endpoint(config: dict | None = None,
    ...
    from zeus_cli.local_runtime.detect import detect_server
...
def _boot_in_flight(config: dict | None) -> bool:
    ...
    with suppress(Exception):
        ...
        if not ((config or {}).get("local_runtime") or {}).get("enabled"):
            return False
        from zeus_cli.local_runtime.binaries import manifest_verified, runtimes_root
