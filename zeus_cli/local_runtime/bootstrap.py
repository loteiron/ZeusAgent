...
from zeus_cli.local_runtime.binaries import runtimes_root
from zeus_cli.local_runtime.gguf import SPLIT_PART_RE, model_id_from_stem

logger = logging.getLogger(__name__)

_SUPERVISOR = None  # process-wide singleton; one router per ZeusAgent process
...
def _detect_gpu_vendor() -> str | None:
    ...
    from zeus_cli.local_runtime.hardware import _nvidia_smi_path
...
def models_dir() -> Path:
    ...
    return get_default_zeus_root() / "models"
...
def _presets_stale() -> bool:
    ...
    with suppress(Exception):
        from zeus_cli.local_runtime.presets import read_preset_decisions
...
def _stop_state_server(state: dict) -> bool:
    """Stop a recorded process only while its start identity still matches.

    Old state files without an identity remain usable for requests, but must be
    refreshed by their owning instance before they can authorize a stop.
    """
    from gateway.status import _get_process_start_time
    from zeus_cli.local_runtime.endpoint import _pid_alive

    try:
        if isinstance(state.get("pid"), bool) or not isinstance(state.get("pid"), (int, str)):
            return False
        pid = int(state.get("pid"))
        if pid <= 0:
            return False
        started = state.get("process_start_time")
        if type(started) is not int or started <= 0 or _get_process_start_time(pid) != started:
            logger.warning("cannot verify managed llama-server pid=%s; restart it from its owning instance", pid)
            return False
    ...
    for _ in range(50):
        if not _pid_alive(pid):
            return True
        time.sleep(0.1)
    logger.warning("managed llama-server pid=%s did not stop; refusing a second server", pid)
    return False
...
def refresh_local_runtime() -> bool:
    ...
    try:
        ...
        if _SUPERVISOR is None:
            from zeus_cli.local_runtime.endpoint import _managed_state
...
def _generate_presets(mdir: Path, preset_path: Path) -> Path | None:
    ...
    """
    from zeus_cli.local_runtime.hardware import probe_budget
    from zeus_cli.local_runtime.presets import generate_presets
...
def ensure_local_runtime(config: dict, force: bool = False) -> "object | None":
    ...
    # Another ZeusAgent process may already be supervising — reuse via state, but ONLY while its
    ...
    try:
        from zeus_cli.local_runtime.binaries import (
            default_tag, ensure_runtime_installed, installed_tags, select_backend)
        from zeus_cli.local_runtime.supervisor import LlamaServerSupervisor
...
_PLUGIN_COMPAT_LAZY = {
    'get_zeus_home': ('zeus_constants', 'get_zeus_home'),
}
