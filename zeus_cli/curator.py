"""CLI subcommand: `zeus curator <subcommand>`."""
...
def _print_unmanaged_summary() -> None:
    ...
    print("  never auto-staled or archived — `zeus curator adopt <name>` hands one over")
...
def _cmd_run(args) -> int:
    ...
    if not synchronous:
        print("llm pass running in background — check `zeus curator status` later")
    if dry:
        print(
            "dry-run: no changes applied. Read the report with "
            "`zeus curator status` and run `zeus curator run` (no flag) to apply."
            if synchronous else
...
_PIN_MESSAGES = {
    True: (
        ...
        "external). `zeus curator list-unmanaged` shows which skills the curator tracks.",
        ...
        "it. Run `zeus curator adopt {skill}` to put it under curator management)",
...
def _cmd_list_unmanaged(args) -> int:
    ...
    print("\nadopt one with `zeus curator adopt <name>`, "
          "or all with `zeus curator adopt --all-unmanaged`")
    return 0
...
def _cmd_archive(args) -> int:
    ...
    if skill_usage.get_record(args.skill).get("pinned"):
        print(
            ...
            f"`zeus curator unpin {args.skill}`")
        return 1
...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at ~/.zeus/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_ledger(args) -> int:
    ...
    for r in rows:
        ...
        print(
            ...
            f"{r.get('skill', '?')}{extra}")
    print(
        "\nRoll back a single mutation with `zeus curator rollback <id>`; "
        "whole-tree snapshots remain available via `zeus curator rollback --list`.")
    return 0
...
def _rollback_ledger_entry(args, entry_id: str) -> int:
    ...
    if entry is None:
        print(
            ...
            "See `zeus curator ledger` for entry ids, or use "
...
def _cmd_rollback(args) -> int:
    ...
    if target_path is None:
        if not curator_backup.list_backups():
            print(
                ...
                "`zeus curator backup` or wait for the next curator run.")
        else:
            print(
    ...
    print(
        "\nThis will replace the current ~/.zeus/skills/ tree (a safety "
...
_SUBCOMMANDS = (
    ...
    (
        "backup",
        "Take a manual tar.gz snapshot of ~/.zeus/skills/ "
    ...
    (
        "rollback",
        "Restore ~/.zeus/skills/ from a curator snapshot, or a single "
        "mutation by ledger entry id (see `zeus curator ledger`)",
        _cmd_rollback,
        _arg("entry_id", nargs="?", default=None,
             help="Ledger entry id for single-mutation rollback (from "
                  "`zeus curator ledger`). Omit for whole-tree snapshot rollback."),
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by zeus_cli.main fallthrough)."""
    parser = argparse.ArgumentParser(prog="zeus curator")
    register_cli(parser)
