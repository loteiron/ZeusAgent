"""``zeus plugins`` subcommand parser."""
...
def build_plugins_parser(subparsers, *, cmd_plugins: Callable) -> None:
    ...
    plugins_parser = subparsers.add_parser(
        ...
        description="Install, update, remove, list, or validate native ZeusAgent plugins "
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL, owner/repo shorthand (e.g. anpicasso/zeus-plugin-chrome-profiles), "
            "or a bare plugin name resolved through the community index "
            "(see `zeus plugins search`)")
    plugins_install.add_argument(
    ...
    _install_enable_group.add_argument(
        ...
        help="Install disabled (skip confirmation prompt); enable later with `zeus plugins enable <name>`",
    )
    ...
    plugins_pack = plugins_subparsers.add_parser(
        "pack", help="Declarative, shareable plugin sets (zeus-pack.yaml)",
    ...
    pack_install.add_argument("source", help="Path to a zeus-pack.yaml file, or an https:// URL")
    ...
    pack_export.add_argument(
        "--name", default="my-zeus-pack", help="Pack name to embed in the exported YAML")

    pack_show = pack_subparsers.add_parser(
        "show", help="Dry-run: parse and display a pack without installing")
    pack_show.add_argument("source", help="Path to a zeus-pack.yaml file, or an https:// URL")
