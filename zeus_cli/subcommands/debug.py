...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for ZeusAgent. Use 'zeus debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    zeus debug share              Upload debug report (asks for confirmation)
    zeus debug share --yes        Skip confirmation (for scripts/CI)
    zeus debug share --lines 500  Include more log lines
    zeus debug share --expire 30  Keep paste for 30 days
    zeus debug share --local      Print report locally (no upload)
    zeus debug share --no-redact  Disable upload-time secret redaction
    zeus debug share --nous       Upload to Nous-internal storage (private)
    zeus debug delete <url>       Delete a previously uploaded paste
""")
    ...
    delete_parser = debug_sub.add_parser(
        "delete", help="Delete a paste uploaded by 'zeus debug share'")
