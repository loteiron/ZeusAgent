...
def build_logs_parser(subparsers, *, cmd_logs: Callable) -> None:
    ...
    logs_parser = subparsers.add_parser(
        "logs", help="View and filter ZeusAgent log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log / gui.log / desktop.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    zeus logs                    Show last 50 lines of agent.log
    zeus logs -f                 Follow agent.log in real time
    zeus logs errors             Show last 50 lines of errors.log
    zeus logs gateway -n 100     Show last 100 lines of gateway.log
    zeus logs gui -f             Follow gui.log in real time
    zeus logs desktop -f         Follow desktop.log (Electron app boot/backend)
    zeus logs --level WARNING    Only show WARNING and above
    zeus logs --session abc123   Filter by session ID
    zeus logs --component tools  Only show tool-related lines
    zeus logs --since 1h         Lines from the last hour
    zeus logs --since 30m -f     Follow, starting from 30 min ago
    zeus logs list               List available log files with sizes
""")
