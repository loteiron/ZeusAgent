"""`zeus checkpoints` CLI subcommand.
...
from zeus_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: zeus checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``zeus checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `zeus checkpoints` → status
...
_PLUGIN_COMPAT_LAZY = {
    'cmd_list': ('zeus_cli.plugins_cmd', 'cmd_list'),
}
