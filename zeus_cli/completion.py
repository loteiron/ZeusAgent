"""Shell completion script generation for zeus CLI. Walks the live argparse parser tree, so
    ...
    for cmd, info in subcommands:
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_zeus_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;")
            ...
            cases.append(
                ...
                f"            ;;")
    cases_str = "\n".join(cases)
    return f"""# ZeusAgent bash completion
# Add to ~/.bashrc:
#   eval "$(zeus completion bash)"

_zeus_profiles() {{
    local profiles_dir="$HOME/.zeus/profiles"
...
_zeus_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_zeus_profiles)" -- "$cur"))
        return
    fi
...
complete -F _zeus_completion zeus
"""
    ...
    for cmd, info in subcommands:
        ...
        if cmd == "profile":
            ...
            sub_cases.append(
                ...
                f"                    ;;")
    sub_cases_str = "\n".join(sub_cases)
    return f"""#compdef zeus
# ZeusAgent zsh completion
# Add to ~/.zshrc:
...
_zeus_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.zeus/profiles" ]]; then
        profiles+=($HOME/.zeus/profiles/*(N/:t))
    fi
...
_zeus() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_zeus_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'zeus command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# ZeusAgent fish completion",
        "# Add to your config:",
        "#   zeus completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __zeus_profiles",
        "    echo default",
        "    if test -d $HOME/.zeus/profiles",
        "        for d in $HOME/.zeus/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c zeus -f -s p -l profile"
        " -d 'Profile name' -xa '(__zeus_profiles)'",
        "",
        ...
        if cmd == "profile":  # profile names for the actions that take one
            for action in sorted(_PROFILE_NAME_ACTIONS):
                lines.append(
                    ...
                    f"-a '(__zeus_profiles)' -d 'Profile name'")
    lines.append("")
    return "\n".join(lines)
