"""Default configuration data for ZeusAgent: DEFAULT_CONFIG and OPTIONAL_ENV_VARS.

Pure-data leaf module — must not import from zeus_cli.config. Comments are the user-facing
docs of config.yaml.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["zeus-cli"],
    # journal_mode: SQLite journal mode for every ZeusAgent DB. "wal" default; use "delete" on
    ...
    "session": {
        # Per-terminal `zeus -c`: each CLI session writes a breadcrumb under
        # $ZEUS_HOME/terminal-sessions/<terminal-id>, so bare -c/--continue resumes THIS
    ...
    "agent": {
        ...
        # Default 30 min is a safety valve for wedged agents, not a target latency — an interactive `zeus
        ...
        # ZeusAgent-level retry attempts for API errors (connection drops, timeouts, 5xx) wrapping the
        ...
        # wrapping ZeusAgent (sandbox runner, managed platform) can describe proxy/credential/ mount
        # layout without editing SOUL.md. Env ZEUS_ENVIRONMENT_HINT overrides it.
        "environment_hint": "",
        ...
        # share or `zeus doctor` (status channel only; prompt cache untouched). 0 = no escalation
        ...
        # gateway status (`zeus status` / fleet monitoring). Retries never stop — a signal, not a
        ...
        # ZEUS_LOCAL_STREAM_STALE_TIMEOUT overrides.
    ...
    "terminal": {
        ...
        # sandboxes). Empty = TMPDIR/TMP/TEMP if set, else ZEUS_HOME/cache/terminal (auto-pruned
        ...
        # ZEUS_HOME/home; "real" = force real HOME; "profile" = force ZEUS_HOME/home when it
        ...
        # ["/home/user/.zeus/cache/documents:/output"]. For gateway MEDIA delivery, write to
        ...
        # default for images whose entrypoints must start as root (e.g. the bundled ZeusAgent image,
        # which drops to `zeus` via s6-setuidgid). When on, SETUID/SETGID caps are omitted.
    ...
    "web": {
        ...
        # parallel, firecrawl, keenable; tavily is opt-in keyless via `zeus tools`, not a ring
        # member). Set by the `zeus tools` picker. "free" = always anonymous endpoint even with a
    ...
    "browser": {
        ...
        # Consent to browse with the user's REAL logins locally: runs on a ZeusAgent-managed SNAPSHOT
        # of the ACTIVE default-Chromium profile (Local State -> profile.last_used; cookies, logins,
        # prefs copied and re-synced per fresh session) driven by ZeusAgent' packaged Chromium. The
        # snapshot dir sidesteps Chrome 136+'s default-profile debugging block and never contends
        # with the running browser. Turning off deletes ~/.zeus/browser-profile/ so credentials
        ...
        # runs `zeus browser close-profile` (kills that profile's browser tree, unsaved tabs lost)
    ...
    # write_file/patch call); restore with /rollback. Opt-in via `zeus chat --checkpoints` or
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ~/.zeus/checkpoints/ size (MB); the oldest checkpoint per project
        ...
        # must not guess. Orphans: `zeus checkpoints prune` (`--keep-orphans` to skip).
        "auto_prune": True,
        "retention_days": 7,
        "min_interval_hours": 24,
    },
    # Hard cap (chars) for one auto-loaded context file (SOUL.md, AGENTS.md, CLAUDE.md, .zeus.md,
    ...
    # Same bound for single-query mode (``zeus -q/-z``). With only ONE turn there is no
    ...
    "compression": {
        ...
        # ZeusAgent' summarizer cannot shrink it. native = codex decides; zeus = ZeusAgent' threshold
    ...
    "auxiliary": {
        ...
        # of skills); route cheaper via `zeus model` → auxiliary → Curator.
        "curator": _aux(600),
    ...
    "display": {
        ...
        # Interface bare `zeus`/`zeus chat` launches: "cli" (prompt_toolkit REPL) | "tui" (Ink).
        # Flags win: `--cli` forces the REPL, `--tui` / ZEUS_TUI=1 forces the TUI.
        "interface": "cli",
        # `zeus --tui` auto-resumes the most recent human-facing session (like `zeus -c`).
        # ZEUS_TUI_RESUME=<id> always wins.
        ...
        # `path`/`paths` = ZEUS_HOME-relative YAML files/dirs (or conventional status_phrases.yaml
        ...
        # Gateway runtime footer on the FINAL message, e.g. `model · 68% · ~/projects/zeus`.
        ...
        # CLI/TUI/desktop, managed with `zeus pets`. No effect on prompt caching.
        "pet": {
            "enabled": False,
            "slug": "",   # active pet slug in get_zeus_home()/pets/; empty → first installed
    ...
    "dashboard": {
        ...
        # client disconnects. 0 = park forever. Env: ZEUS_TUI_WS_ORPHAN_REAP_GRACE_S.
        ...
        # newest message older than ZEUS_TUI_SESSION_TTL_S, default 6h) with
        ...
        # plugin. Env ZEUS_DASHBOARD_OAUTH_CLIENT_ID / ZEUS_DASHBOARD_PORTAL_URL win when
        ...
        # no-op. Env ZEUS_DASHBOARD_BASIC_AUTH_USERNAME / _PASSWORD_HASH / _PASSWORD / _SECRET /
        ...
        # ZEUS_DASHBOARD_DRAIN_SECRET; no-op unless >=256-bit, weak secrets rejected
        # (fail-closed). scope = capability label; min_secret_chars in url-safe-b64 chars.
        "drain_auth": {"scope": "drain", "min_secret_chars": 43},
        # Public URL (env ZEUS_DASHBOARD_PUBLIC_URL): full authority (scheme + host + optional
    ...
    "tts": {
        ...
        "piper": {
            ...
            # (~/.zeus/cache/piper-voices/), use_cuda, length_scale (2.0 = twice as slow),
    ...
    # "Hey ZeusAgent" hands-free wake word: always-on, on-device hotword detection that starts a fresh
    ...
    "wake_word": {
        ...
        "openwakeword": {
            # "hey_zeus" | built-in openWakeWord name ("hey_jarvis", "alexa", ...) | path to a
    ...
    # plugins/context_engine/<name>/ or ~/.zeus/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "moa": {
        ...
        # <zeus_home>/moa-traces/<session_id>.jsonl (or trace_dir) for auditing.
        "save_traces": False,
        "trace_dir": "",
    ...
    # and resolved; read-only — creation goes to ~/.zeus/skills/ unless create_dir redirects it.
    "skills": {
        ...
        # ZEUS_HOME, scanned alongside the local dir.
        "create_dir": "",
        # In a git checkout, <root>/.zeus/skills/ and <root>/.agents/skills/ load as the
        # highest-precedence tier — ONLY if the root is in trusted_project_dirs. false = no scan, no
        # untrusted-skills notice.
        "project_discovery": True,
        # Trusted project roots; managed by `zeus skills trust` / `untrust`.
        "trusted_project_dirs": [],
        # Substitute ${ZEUS_SKILL_DIR} / ${ZEUS_SESSION_ID} in SKILL.md content.
        "template_vars": True,
        ...
        # Advisory NVIDIA SkillEvaluator Tier 1 scan on `zeus skills install` (alongside the
        ...
        "write_approval": False,
        # Audit ledger: every skill mutation appends to ~/.zeus/skills/.curator_ledger.jsonl with
        # before/after hashes (blobs under ~/.zeus/.curator_backups/blobs/); powers `zeus
    ...
    # daemon. `zeus curator status` shows the last run.
    "curator": {
        "enabled": True,
        ...
        # aux-model cost. `zeus curator run --consolidate` overrides once.
        "consolidate": False,
        # Also prune bundled built-ins (a suppression list stops `zeus update` restoring them);
        ...
        "prune_builtins": True,
        # TTL purge of skills/.archive/: 0 = never; > 0 lets the explicit `zeus curator purge`
        # delete older archived skills (never automatic; logged in the ledger).
        "archive_ttl_days": 0,
        # Before every real (non-dry-run) pass, snapshot ~/.zeus/skills/ to
        # ~/.zeus/skills/.curator_backups/<utc-iso>/skills.tar.gz (`zeus curator rollback`).
        "backup": {
            "enabled": True,
    ...
    # peerName, sessions, enabled); zeus-specific overrides only here.
    "honcho": {},
    ...
    "whatsapp": {
        # reply_prefix: None = built-in "⚡ *ZeusAgent*" header; "" disables; \n allowed.
    },
    ...
    #   cron job, a -q session (ZEUS_INTERACTIVE=1 but nobody to answer), or an unattended
    ...
    "approvals": {
        ...
        # export ZEUS_INTERACTIVE=1 (for interactive sudo prompts) but have NO user waiting to answer
        ...
        # Approve" → false. ZEUS_TUI_NO_CONFIRM=1 skips the TUI modal.
    ...
    # are given.
    "platform_hints": {},
    # Plugin system. `enabled`/`disabled` lists are written by `zeus plugins enable|disable` and
    ...
    "plugins": {
        ...
        # 2026-09-14 removal date (see COMPAT_MANIFEST.md, `zeus plugins compat`). Stopgap only: the
    ...
    # approvals persist in ~/.zeus/shell-hooks-allowlist.json. Schema + examples:
    # website/docs/user-guide/features/hooks.md.
    "hooks": {},
    # Auto-accept shell-hook registrations without a TTY prompt (also --accept-hooks or
    # ZEUS_ACCEPT_HOOKS=1). Gateway/cron/non-interactive runs need one of these to pick up
    # newly-added hooks.
    ...
    "security": {  # Security: pre-exec scanning via tirith plus related guards.
        ...
        # .zeus config) always need human approval, even under yolo. Extra patterns are fnmatch
        ...
        # banner. Add via `zeus doctor --ack <id>`; remove by editing the list. Catalog:
        # zeus_cli/security_advisories.py.
        "acked_advisories": [],
    ...
    "cron": {
        ...
        # an installed provider (plugins/cron_providers/<name>/ or $ZEUS_HOME/plugins/ <name>/),
        ...
        # Env override: ZEUS_CRON_MAX_PARALLEL.
        ...
        "output_retention": 50,
        # Timeout (seconds) for a no-agent cron script. Env: ZEUS_CRON_SCRIPT_TIMEOUT. Keep in
        ...
        # job's dispatch guard forever. Env: ZEUS_CRON_SESSION_DB_TIMEOUT. 0 = unlimited.
        "session_db_timeout_seconds": 10,
        # Timeout (seconds) per media attachment send during gateway delivery; large attachments
        # (long TTS audio, big exports) need more than 30s. Env: ZEUS_CRON_MEDIA_SEND_TIMEOUT.
    ...
    # promotes dependency-satisfied todos to ready, and fires `zeus -p <assignee> chat -q ...` per
    ...
    "kanban": {
        ...
        # Auto-run the decomposer on Triage tasks every tick. False = manual via `zeus kanban
    ...
    "code_execution": {  # execute_code settings (programmatic tool calls).
        ...
        # relative paths resolve. strict = isolated temp dir with zeus-agent's own python
    ...
    # by tool_search / tool_describe / tool_call bridges and surfaced on demand. Core ZeusAgent tools
    ...
    "logging": {  # File logging to ~/.zeus/logs/: agent.log captures INFO+, errors.log WARNING+.
    ...
    "model_catalog": {
        ...
        # CLI refetches on the next /model or `zeus model` once the cache is older. Network
    ...
    # defaults (200K context, tools on, vision/reasoning off) and get patched. Provider keys: ZeusAgent
    ...
    "monitoring": {
        ...
        "gateway_health_export": {  # Gateway health & diagnostics export.
            ...
            "resource_attributes": {
                "service.name": "zeus-gateway", "deployment.environment.name": "production"
            },
        },
    ...
    "gateway": {  # Gateway settings (messaging platforms: Telegram, Discord, Slack, ...).
        ...
        # to ZEUS_GATEWAY_PLATFORM_CONNECT_TIMEOUT, which wins if set explicitly.
        ...
        # run_gateway() bridges these to ZEUS_STARTUP_WATCHDOG / ZEUS_STARTUP_WATCHDOG_TIMEOUT_S
        # and re-arms the live handle; explicit env wins.
        "startup_watchdog": True,
        "startup_watchdog_timeout_seconds": 300,
        # Keep writing the legacy ~/.zeus/sessions/sessions.json mirror of the routing index
        ...
        # (ZEUS_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent AND a wakeUrl is
        ...
        # ZEUS_GATEWAY_MAX_STARTS / ZEUS_GATEWAY_START_WINDOW_S.
        ...
        # credential/system denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.zeus/.env, auth.json). True:
        # files must be under the ZeusAgent cache, media_delivery_allow_dirs, or fresher than
        # trust_recent_files_seconds — recommended for public-facing gateways so prompt injection
        # can't exfiltrate host secrets. Bridged to ZEUS_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra roots (project/scratch dirs, mounted shares) from which bare file paths may be
        # uploaded; the ZeusAgent cache is always trusted. List of absolute paths or one
        # os.pathsep-separated string; tildes expanded. Bridged to ZEUS_MEDIA_ALLOW_DIRS. Honored
        # in both modes.
        ...
        # pure-allowlist mode. Bridged to ZEUS_MEDIA_TRUST_RECENT_FILES. Only consulted when
        # strict is true.
        "trust_recent_files": True,
        # Recency window in seconds; 600 covers a multi-tool turn. Bridged to
        # ZEUS_MEDIA_TRUST_RECENT_SECONDS. Only consulted when strict is true.
    ...
    # Automatic cleanup of ~/.zeus/state.db, which otherwise grows without bound and slows FTS5
    ...
    "sessions": {
        ...
        "auto_prune": True,
        # Inactive days of ended-session history to keep (= `zeus sessions prune`).
        ...
        # stay until `zeus sessions optimize-storage` runs, since the rebuild is disk-heavy on
        # large DBs. advise = `zeus update` prints a one-line notice with reclaimable size when a
        ...
        # (native/fts5_cjk/build.sh → ~/.zeus/lib/libfts5_cjk.so), 1-2 char CJK terms get exact
        # index matches instead of LIKE scans. True = use when present (inert otherwise); False =
        # never load/serve it. Bridged to ZEUS_CJK_FTS.
        "cjk_fts": True,
        ...
        # Max active messages per session for in-memory export (`zeus sessions export`); checked
    ...
    "doctor": {
        # Per-probe timeout (seconds) for `zeus doctor --live` real-call probes.
        "live_probe_timeout": 10,
    },

    "updates": {
        # Passive version/banner checks only; explicit `zeus update --check` remains enabled.
        "check": True,
        # Pre-update backup. quick = snapshot small critical state (pairing JSONs, cron jobs,
        # config.yaml, .env, auth.json, profile DBs) into <ZEUS_HOME>/state-snapshots/, skipping
        # files >1 GiB; restore via ``/snapshot``. full = quick PLUS a ``zeus backup`` zip in
        # <ZEUS_HOME>/backups/ (``zeus import`` restores; slow on large homes; ``--backup``
        ...
        # update cleanly. `zeus update --switch-branch` overrides to switch for one run.
        "parked_branch_strategy": "switch",
        # Refresh an installed cua-driver during `zeus update` (best-effort, macOS only). Turn off
    ...
    "lsp": {
        ...
        "wait_mode": "document",
        "wait_timeout": 5.0,
        # Missing server binaries: auto = install via npm/go/pip into <ZEUS_HOME>/lsp/bin/ on
    ...
    # (SuperGrok OAuth or XAI_API_KEY) AND the toolset is enabled in `zeus tools`.
    "x_search": {
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # ~/.zeus/.env (or the shell), never in config.yaml.
            ...
            # Last-good fallback for NETWORK/TIMEOUT outages: AES-GCM cache under ~/.zeus/cache/,
            ...
            # Auto-download bws into ~/.zeus/bin/ on first use; False = bws must be on PATH.
            "auto_install": True,
    ...
    "computer_use": {
        # cua-driver's upstream PostHog telemetry defaults ON; ZeusAgent sets
        ...
        # #47032; Linux/WSL2 idle spin #28152). cua-driver ≥ 0.6.x supports --no-overlay; ZeusAgent also calls
    ...
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned binary into ~/.zeus/bin/; False = iron-proxy on PATH.
        "auto_install": True,
    ...
    "desktop": {  # ZeusAgent Desktop (Electron) launch options; only affect `zeus desktop`.
        ...
        # Bridged to ZEUS_DESKTOP_DISABLE_GPU: auto = disable GPU only on remote displays
        ...
        # to ZEUS_DESKTOP_PASSWORD_STORE; ignored off-Linux.
        "password_store": "auto",
    ...
    "local_runtime": {
        # Off = detection-only (ZeusAgent still finds an external llama-server you run).
        "enabled": False,
        # Pinned llama.cpp release tag; bumped by ZeusAgent releases after validation.
        "tag": "b10679",
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": _prov(
        ...
        "OAuth2, not a static API key — this points at the credentials ZeusAgent mints short-lived "
    ...
    "AZURE_FOUNDRY_BASE_URL": _prov(
        "Azure Foundry base URL (set via 'zeus model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": _tool(
        ...
        "read from the ZeusAgent auth store)", "Tool-gateway user token", None, advanced=True),
    ...
    "PORCUPINE_ACCESS_KEY": _tool(
        "Picovoice access key for the Porcupine 'Hey ZeusAgent' wake word engine (optional; "
    ...
    "ZEUS_LANGFUSE_PUBLIC_KEY": _tool("Langfuse project public key (pk-lf-...)",
        "Langfuse public key", "https://cloud.langfuse.com", password=False),
    "ZEUS_LANGFUSE_SECRET_KEY": _tool("Langfuse project secret key (sk-lf-...)",
        "Langfuse secret key", "https://cloud.langfuse.com"),
    "ZEUS_LANGFUSE_BASE_URL": _tool("Langfuse server URL (default: https://cloud.langfuse.com)",
    ...
    "SLACK_ALLOWED_USERS": _msg(
        "Comma-separated Slack member IDs allowed to use ZeusAgent, e.g. U01ABC2DEF3. Without "
    ...
    "MATRIX_USER_ID": _msg("Matrix user ID (e.g. @zeus:example.org)",
    ...
    "MATRIX_DEVICE_ID": _msg(
        "Stable Matrix device ID for E2EE persistence across restarts (e.g. ZEUS_BOT)",
    ...
    "IRC_CHANNEL": _msg("IRC channel to join (e.g. #zeus)", "IRC channel", None),
    "IRC_NICKNAME": _msg("Bot nickname on IRC (default: zeus-bot)", "IRC nickname", None),
    ...
    "API_SERVER_MODEL_NAME": _msg(
        "Model name advertised on /v1/models. Defaults to the profile name (or 'zeus-agent' "
    ...
    "GATEWAY_PROXY_URL": _msg(
        "URL of a remote ZeusAgent API server to forward messages to (proxy mode). When set, the "
    ...
    # ZEUS_TOOL_PROGRESS_MODE (deprecated; use display.tool_progress) is intentionally NOT listed:
    # this dict feeds user-facing surfaces (dashboard keys page, setup checklists), so deprecated
    # knobs stay in config._EXTRA_ENV_KEYS only. ZEUS_TOOL_PROGRESS is unsupported.
    "ZEUS_PREFILL_MESSAGES_FILE": _setting(
        "Path to JSON file with ephemeral prefill messages for few-shot priming",
        "Prefill messages file path", None),
    "ZEUS_EPHEMERAL_SYSTEM_PROMPT": _setting(
