"""Image-authored deployment provenance for immutable ZeusAgent runtimes.

The image bakes ``/etc/zeus/image-provenance.json`` outside ``$ZEUS_HOME`` and the checkout, so a
...
IMAGE_PROVENANCE_PATH = Path("/etc/zeus/image-provenance.json")
