"""Helpers for loading ZeusAgent .env files consistently across entrypoints."""
...
# Once-per-process guards: load_zeus_dotenv() runs repeatedly (user + project env, gateway hot-reload,
...
# env-var name → source label ("bitwarden", …) for externally injected credentials; setup / `zeus
...
# ZEUS_HOME paths already pulled external secrets for: load_zeus_dotenv() runs at import time from
...
# Behavioral routing keys a parent ZeusAgent process injects into child env that silently redirect a profile
...
def get_secret_source_values(zeus_home: str | os.PathLike) -> dict[str, str]:
    """Return the external-secret value snapshot for ``zeus_home``."""
    return dict(_SECRET_SOURCE_VALUES_BY_HOME.get(str(Path(zeus_home).resolve()), {}))


def hydrate_profile_secret_sources(zeus_home: str | os.PathLike) -> dict[str, str]:
    ...
    with _SECRET_SOURCE_CACHE_LOCK:
        return _hydrate_profile_secret_sources(Path(zeus_home))
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        ...
        # Mirror load_zeus_dotenv()'s .op.env bootstrap (1Password token lives in gitignored .op.env)
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `zeus setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    if not path.exists():
        return
    try:
        from zeus_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_zeus_dotenv(
    *,
    zeus_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
    load_external_secrets: bool = True,
) -> list[Path]:
    """Load ZeusAgent env files: ``~/.zeus/.env`` overrides stale shell exports; project ``.env`` is a dev
    fallback that only fills gaps when the user env exists (and overrides shell vars when it does not)."""
    home_path = Path(zeus_home or os.getenv("ZEUS_HOME", Path.home() / ".zeus"))
    ...
    if is_multiplex_active() and get_zeus_home_override() is not None:
    ...
    # Windows (#73381, #86735). 2. A fresh ``zeus update`` retry just completed a deferred dependency
    ...
    # ~/.zeus/.env win on every reload and flip the backend mid-session in long-lived processes.
    ...
    # with override=True — so a stale TERMINAL_ENV=docker left in ~/.zeus/.env (e.g. written by an older
    # `zeus setup` before the user switched terminal.backend in config.yaml) silently wins again on every
    # reload. Startup launchers bridge config→env once, but long-lived processes (gateway per-turn reload,
    # cron standalone runs) call load_zeus_dotenv() repeatedly and used to flip the effective backend back
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    try:
        if Path(home_path).resolve() != _process_zeus_home().resolve():
            return
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    tokens), BEFORE ZeusAgent reads credentials; failures never block startup. Precedence/conflicts/provenance
    ...
    # load_zeus_dotenv() calls per startup don't re-fetch / re-print — error retries within one process
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # populating it lets main.py's early bridge and zeus_logging reuse one parse instead of 3-4.
    if home_path == _process_zeus_home():
        try:
...
def _process_zeus_home() -> Path:
    """The ZEUS_HOME the running process was launched under.

    Must be the *true* process home, ignoring any context-local
    ``set_zeus_home_override`` a per-request task has installed. Both
    ...
    * ``_load_secrets_config`` uses it to decide whether the shared
    ...
    ``zeus_constants.get_process_zeus_home()`` is the override-immune
