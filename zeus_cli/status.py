...
from zeus_cli.auth import AuthError, resolve_provider
from zeus_cli.colors import Colors, color
from zeus_cli.config import get_env_path, get_env_value, get_zeus_home, load_config
from zeus_cli.models import provider_label
from zeus_cli.runtime_provider import resolve_requested_provider
from zeus_cli.vercel_auth import describe_vercel_auth
from zeus_cli.status_auth import (  # renderers wired into _SECTIONS below
    _render_api_keys, _render_apikey_providers, _render_auth_providers, _render_nous_gateway)
from zeus_constants import OPENROUTER_MODELS_URL
...
def _estop_status_line():
    """One-line pause banner for `zeus status`, or None when not paused."""
    try:
    ...
    return f"⏸️  PAUSED (global emergency stop{f' — reason: {reason}' if reason else ''}; `zeus resume` to lift)"
...
def _render_terminal(ctx):
    ...
    if terminal_env in _TERMINAL_ENV_ROWS:
        ...
        _kv_flag("SDK:", importlib.util.find_spec("vercel") is not None, "installed",
                 "missing (install: pip install 'zeus-agent[vercel]')")
        ...
        # (fail-soft — never break `zeus status`).
        try:
...
def _render_cron(ctx):
    ...
    jobs_file = get_zeus_home() / "cron" / "jobs.json"
...
def _render_sessions(ctx):
    ...
    if gateway_rows:
        ...
        if freshest > 0:
            from zeus_cli.timefmt import relative_time
            print(f"  Last activity:{relative_time(freshest):>13}")
    elif not (sessions_file := get_zeus_home() / "sessions" / "sessions.json").exists():
        _kv("Active:", 0)
    else:
        try:
    ...
    try:
        from zeus_cli.active_sessions import (
...
def _render_footer(ctx):
    _banner(("─" * 60, "  Run 'zeus doctor' for detailed diagnostics", "  Run 'zeus setup' to configure"),
            Colors.DIM)
    print()


# Print order of `zeus status`; each renderer takes the shared _StatusContext.
_SECTIONS = (
...
def show_status(args):
    """Show status of all ZeusAgent components."""
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_nous_portal_account_info': ('zeus_cli.nous_account', 'get_nous_portal_account_info'),
