...
_MANAGED_FILES_ROOT_ENV = "ZEUS_DASHBOARD_FILES_ROOT"
...
def _default_zeus_root_is_opt_data() -> bool:
    raw = os.environ.get("ZEUS_HOME", "").strip()
    if not raw:
        return False
    try:
        from zeus_constants import get_default_zeus_root

        root = get_default_zeus_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """True when the dashboard should not offer ``zeus update``.

    Containerized dashboards are updated by the outer launcher/image — except a
    ``git`` install (bind-mounted checkout, e.g. the zeus-webui image), where
    ...
    """
    from zeus_cli.web_server import PROJECT_ROOT
    from zeus_cli.config import detect_install_method
    if _default_zeus_root_is_opt_data():
        return True
    try:
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # install still browses its home). Lock to /opt/data only when the ZeusAgent
    # root actually IS /opt/data or ZEUS_DASHBOARD_FILES_ROOT is set.
    if _default_zeus_root_is_opt_data():
