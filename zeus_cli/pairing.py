...
def pairing_command(args):
    """Handle zeus pairing subcommands."""
    ...
    if handler is None:
        print("Usage: zeus pairing {list|approve|revoke|clear-pending}")
        print("Run 'zeus pairing --help' for details.")
    else:
        handler()
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: zeus pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(f"  To reset sooner, delete the '_lockout:{platform}' entry from ~/.zeus/platforms/pairing/_rate_limits.json\n")
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'zeus pairing list' to see pending requests.\n")
