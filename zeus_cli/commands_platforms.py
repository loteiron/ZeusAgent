...
from zeus_cli.commands import (
    COMMAND_REGISTRY, _is_gateway_available, _iter_plugin_command_entries, _resolve_config_gates)

# Logger name parity with the origin module (tests capture "zeus_cli.commands").
logger = logging.getLogger("zeus_cli.commands")
...
def discord_skill_commands_by_category(
    ...
    parity, external-dir skills are visible via ``zeus skills list`` and the agent's ``/skill-name``
...
# Canonical commands deliberately routed through ``/zeus <command>`` on Slack only: the registry
...
_SLACK_VIA_ZEUS_ONLY = frozenset({
    ...
    "usage"})  # /zeus usage keeps the lookup available; /autonom takes the native control slot.
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    _SLACK_VIA_ZEUS_ONLY are skipped. ``/zeus`` is always first for anything dropped."""
...
def slack_subcommand_map() -> dict[str, str]:
    """name/alias -> "/command" for the Slack ``/zeus`` handler, plugin commands included."""
