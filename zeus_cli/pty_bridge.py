"""PTY bridge for `zeus dashboard` chat tab.
...
``zeus_cli.web_server``. POSIX-only: depends on ``fcntl``, ``termios`` and ``ptyprocess`` (native
...
# ui-tui/packages/zeus-ink/src/ink/termio/host.ts — keep the two in sync.
PTY_HOST_ENV = "ZEUS_PTY_HOST"
        ...
        if not _PTY_AVAILABLE:
            if sys.platform.startswith("win"):
                raise PtyUnavailableError("Pseudo-terminals are unavailable on this platform. "
                                          "ZeusAgent supports Windows only via WSL.")
