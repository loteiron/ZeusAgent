...
Split out of ``zeus_cli.kanban_db``; origin-resident helpers are reached
...
if TYPE_CHECKING:
    from zeus_cli.kanban_db import Task
...
# ``ZEUS_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``.
...
class DispatchResult:
    ...
    Code terminal like ``orion-cc``), not a ZeusAgent profile. Expected steady-state
    ...
    if sweep.exited_hook_payloads and _kb._kanban_observer_consumed("on_kanban_worker_exited"):
        ...
        for hook_fields in sweep.exited_hook_payloads:
            ...
            _kb._fire_kanban_lifecycle_hook(
                ...
                # fire in the DISPATCHER process (gateway-embedded dispatcher or ``zeus kanban
...
def _profile_exists_fn() -> Optional[Callable[[str], bool]]:
    """``zeus_cli.profiles.profile_exists``, or ``None`` when it cannot be
    imported (local import avoids a cycle; callers fall back to trusting the
    assignee)."""
    try:
        from zeus_cli.profiles import profile_exists
    except Exception:
        return None
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    """True iff a ready+assigned+unclaimed task maps to a real ZeusAgent profile.
...
def review_dispatch_enabled() -> bool:
    """Whether review tasks dispatch automatically. Default true (ZeusAgent ships
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so ``ZEUS_KANBAN_DB`` (pins every
...
def _dispatch_lane_task(
    ...
    *,
    ...
    # would fail ``zeus -p <assignee>`` at startup and loop ready→crash→ready
    ...
    if guard_reason is not None:
        ...
        # Event so ``zeus kanban tail`` shows why the task looks stuck.
...
def _module_zeus_argv() -> list[str]:
    """Interpreter-bound ZeusAgent CLI invocation (``zeus_cli.main`` is the
    console-script target — there is no top-level ``zeus`` package)."""
    return [sys.executable, "-m", "zeus_cli.main"]


def _absolute_zeus_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved ZeusAgent shim."""
...
def _zeus_path_argv(path: str) -> list[str]:
    """argv for a resolved ZeusAgent executable path. Windows batch shims
    ...
    if _kb._IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_zeus_argv()
    return [_absolute_zeus_path(path)]


def _resolve_zeus_argv() -> list[str]:
    """Resolve the ``zeus`` invocation as argv for ``Popen``: ``$ZEUS_BIN``
    (path-like -> absolute; bare names keep PATH semantics, never a
    same-directory file), then ``which("zeus")`` (Windows: safe PATH search,
    batch shims fall back to the module form), then ``sys.executable -m
    zeus_cli.main`` for shim-less environments (cron, systemd ``User=``,
    launchd). Mirrors ``gateway.run._resolve_zeus_bin``; local because
    ``zeus_cli`` sits below ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("ZEUS_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _zeus_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _zeus_path_argv(resolved_env_bin)
        return _module_zeus_argv()

    zeus_bin = _safe_which_no_cwd("zeus") if _kb._IS_WINDOWS else shutil.which("zeus")
    if zeus_bin:
        return _zeus_path_argv(zeus_bin)
    return _module_zeus_argv()
...
def _resolve_worker_cli_toolsets(zeus_home: Optional[str]) -> Optional[list[str]]:
    ...
    tools when ``ZEUS_KANBAN_TASK`` is set.
    """
    if not zeus_home:
        return None
    try:
        ...
        token = set_zeus_home_override(zeus_home)
        try:
            cfg = load_config()
        ...
        _kb._log.debug(
            "kanban worker: could not resolve CLI toolsets for ZEUS_HOME=%r (%s)",
            zeus_home,
            exc,
        )
        return None
...
def _worker_argv(task: Task, profile_arg: str, zeus_home: Optional[str]) -> list[str]:
    """Build the ``zeus -p <profile> --cli ... chat -q ...`` worker command."""
    cmd = [
        *_resolve_zeus_argv(),
        "-p", profile_arg,
        ...
        "--cli",
        # Workers run under a profile-scoped ZEUS_HOME and so see that
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(zeus_home)
    if worker_toolsets:
...
def _open_worker_log(task: Task, board: Optional[str]):
    ...
    `zeus kanban log` reads its own file and boards sharing task ids don't
    collide."""
...
def _default_spawn(task: Task, workspace: str, *, board: Optional[str] = None) -> Optional[int]:
    """Fire-and-forget ``zeus -p <profile> chat -q ...`` subprocess.
    ...
    ``ZEUS_KANBAN_DB`` / ``ZEUS_KANBAN_BOARD`` / workspaces_root to the
    board the task was claimed from, so workers cannot see other boards.
    """
    if not task.assignee:
        raise ValueError(f"task {task.id} has no assignee")

    from zeus_cli.profiles import normalize_profile_name, resolve_profile_env
    ...
    # Inject ZEUS_HOME so the worker reads the profile-scoped config.yaml:
    # without it the child's get_zeus_home() falls back to the DEFAULT
    # profile root because `zeus -p` applies its override before
    # zeus_constants is imported.
    try:
        env["ZEUS_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # No profile dir (isolated test fixtures) — the CLI resolves it from
        # ZEUS_PROFILE (set below) instead.
        pass
    if task.tenant:
        env["ZEUS_TENANT"] = task.tenant
    env["ZEUS_KANBAN_TASK"] = task.id
    env["ZEUS_KANBAN_WORKSPACE"] = workspace
    ...
    env["ZEUS_SESSION_SOURCE"] = "kanban"
    ...
    if task.branch_name:
        env["ZEUS_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["ZEUS_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["ZEUS_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    if task.goal_mode:
        env["ZEUS_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["ZEUS_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # match after `zeus -p` rewrites ZEUS_HOME (symlink / Docker layouts).
    env["ZEUS_KANBAN_DB"] = str(_kb.kanban_db_path(board=board))
    env["ZEUS_KANBAN_WORKSPACES_ROOT"] = str(_kb.workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["ZEUS_KANBAN_WORKSPACES_ROOT"])
    # Board slug — defense-in-depth pin if a path is resolved without the
    # DB / workspaces env vars.
    env["ZEUS_KANBAN_BOARD"] = _kb._normalize_board_slug(board) or _kb.get_current_board()
    # kanban_comment reads ZEUS_PROFILE for its default author; `-p` alone
    # doesn't set the env var.
    env["ZEUS_PROFILE"] = profile_arg
    ...
    # `--cli` is the highest-precedence TUI override; dropping ZEUS_TUI covers
    # older zeus builds on PATH that predate the flag's precedence.
    env.pop("ZEUS_TUI", None)

    cmd = _worker_argv(task, profile_arg, env.get("ZEUS_HOME"))
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`zeus` executable not found on PATH. "
            "Install ZeusAgent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    the gateway dispatcher and ``zeus kanban dispatch`` — the standalone
...
from zeus_cli import kanban_db as _kb  # noqa: E402
from zeus_cli import kanban_db_connect as _kbc  # noqa: E402
from zeus_cli import kanban_db_workspace as _kbw  # noqa: E402
