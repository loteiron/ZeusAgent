"""Install and remove the Linux desktop entry (``zeus.desktop``).
...
DESKTOP_ENTRY_NAME = "zeus.desktop"
...
def _can_import_zeus_cli(interpreter: Path) -> bool:
    """Whether *interpreter* can import ``zeus_cli.main`` unaided.
...
def _running_interpreter_fallback() -> str:
    """The RUNNING interpreter — it has ``zeus_cli`` importable by definition."""
    return os.path.abspath(sys.executable)


def resolve_exec_command(project_root: Optional[Path] = None) -> str:
    """Build the absolute ``Exec=`` command line for ``zeus desktop``.

    Prefer the real ``zeus`` launcher; fall back to ``<python> -m zeus_cli.main desktop``.
    """
    from zeus_cli.relaunch import resolve_zeus_bin

    bin_path = _resolve_zeus_bin_for_desktop_entry(resolve_zeus_bin, checkout_root=project_root)
    interpreter = _running_interpreter()
    if not _can_import_zeus_cli(Path(interpreter)):
        ...
        # The candidate interpreter cannot actually import zeus_cli.main (checked in isolated mode from a
    ...
    argv = [interpreter, "-m", "zeus_cli.main", "desktop"]
    if bin_path:
        resolved = Path(bin_path).resolve()
        # A Python launcher whose shebang points OUTSIDE the venv (e.g. the repo's `zeus` script
...
def _inside_checkout(candidate: str, checkout_root: Path, original_argv0: str) -> bool:
    ...
    try:
        ...
        return False
    # Anything shipped in the tree (e.g. the repo `zeus` script) is checkout-internal. Compare
    ...
    for root in {checkout_root, resolved_root}:
        if root is not None and (path == root or root in path.parents):
            return True
    # The `python -m zeus_cli.main` relaunch context surfaces the invoking interpreter as
...
def _resolve_zeus_bin_for_desktop_entry(
    resolve_fn=None,
    checkout_root: Optional[Path] = None,
) -> Optional[str]:
    """Resolve the launcher binary for the persisted ``.desktop`` entry.

    Wraps :func:`zeus_cli.relaunch.resolve_zeus_bin` with one rule: an ``argv[0]`` inside this
    ...
    if resolve_fn is None:
        from zeus_cli.relaunch import resolve_zeus_bin as resolve_fn
    ...
    # An external primary (another install's /opt/.../bin/zeus, a venv console script) wins
    ...
    # /opt/.../bin/zeus from another install method, or a venv console script). It must be evaluated
    ...
    # argv[0] was checkout-internal AND PATH had no `zeus` — common in stripped systemd user
...
def _known_wrapper_candidates():
    ...
    (``/usr/local/bin``), and user (``~/.local/bin``). The wrapper is always named ``zeus``.
    """
    candidates = []
    prefix = os.environ.get("PREFIX")
    if prefix:
        candidates.append(Path(prefix) / "bin" / "zeus")
    if hasattr(os, "geteuid") and os.geteuid() == 0:
        candidates.append(Path("/usr/local/bin/zeus"))
    candidates.append(Path.home() / ".local" / "bin" / "zeus")
    return candidates
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=ZeusAgent\n"
        "GenericName=ZeusAgent Desktop\n"
        "Comment=Launch ZeusAgent Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
        "StartupNotify=true\n"
        "StartupWMClass=ZeusAgent\n"
    )
...
def _hicolor_icon_dest(subdir: str) -> Path:
    return _xdg_data_home() / "icons" / "hicolor" / subdir / "apps" / "zeus.png"
...
def _install_icon_to_hicolor(icon: Path) -> bool:
    """Install the app icon into the user's hicolor tree so ``Icon=zeus`` resolves without an
...
def _launcher_entry_management_enabled() -> bool:
    ...
    rewrite: a hand-edited ``zeus.desktop`` is then left alone instead
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    ...
    icon_value = str(icon) if icon.is_file() else "zeus"
