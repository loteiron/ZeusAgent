...
from zeus_constants import OPENROUTER_BASE_URL, get_zeus_home
from agent.retry_utils import jittered_backoff
from zeus_cli.env_loader import load_zeus_dotenv

# Load .env from ZEUS_HOME first, then project root as a dev fallback.
load_zeus_dotenv(zeus_home=get_zeus_home(), project_env=Path(__file__).parent / ".env")
...
class TrajectoryCompressor:
    ...
    def _init_summarizer(self):
        ...
        if provider:
            ...
            if client is None:
                raise RuntimeError(f"Provider '{provider}' is not configured. Check your API key or run: zeus setup")
