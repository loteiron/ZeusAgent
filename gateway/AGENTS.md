...
(`zeus_cli/AGENTS.md`). Each adapter picks a base toolset (Telegram → `"messaging"`).
...
`if canonical == ...` chains. Registry + adding a command: `zeus_cli/AGENTS.md`.
...
duplicate-final incident (NS-658 canary ledger, zeus#85796 / gateway-gateway#210); violating any
...
(or `ZEUS_BACKGROUND_NOTIFICATIONS`): `concise` (default; one line, failures append an output
...
`zeus serve` (control plane, desktop-spawned child) dies with the app — by design. The messaging
gateway (`gateway run`) SURVIVES the app: the serve backend's `/api/gateway/*` endpoints spawn it
detached (`_spawn_zeus_action` — `start_new_session` / `DETACHED_PROCESS`), so `before-quit`'s
