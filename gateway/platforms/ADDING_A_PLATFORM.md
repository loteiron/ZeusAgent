...
There are two ways to add a platform to the ZeusAgent gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.zeus/plugins/` (or under `plugins/platforms/`
...
**zero changes to core ZeusAgent code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `zeus gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `zeus_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the ZeusAgent core.
...
"zeus-your-platform": {
    ...
    "tools": _ZEUS_CORE_TOOLS,
    "includes": []
},
```

And add it to the `zeus-gateway` composite:

```python
"zeus-gateway": {
    "includes": [..., "zeus-your-platform"]
}
```
...
## 12. Status Display (`zeus_cli/status.py`)
...
## 13. Gateway Setup Wizard (`zeus_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ zeus_cli/ toolsets.py \
  --include="*.py" -l | sort -u
