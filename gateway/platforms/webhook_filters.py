...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.zeus to the active profile home."""
    ...
    if raw == "~/.zeus" or raw.startswith("~/.zeus/"):
        return zeus_home / raw[len("~/.zeus/"):]
    path = Path(raw).expanduser()
    return path if path.is_absolute() else zeus_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script; must live under ZEUS_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from zeus_constants import get_zeus_home
    scripts_root = (get_zeus_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.zeus" or raw_text.startswith("~/.zeus/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        Non-zero exit, empty/``[SILENT]`` stdout, or a ``[SILENT]``/``__zeus_ignore__`` flag drops the
        ...
        if not isinstance(transformed, dict):
            ...
            return False, None
        silenced = transformed.get("[SILENT]") is True or transformed.get("__zeus_ignore__") is True
