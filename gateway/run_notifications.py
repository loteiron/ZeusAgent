...
class GatewayNotificationsMixin:
    ...
    class _UpdatePaths:
        """Marker files ``zeus update --gateway`` and its watcher exchange under ZEUS_HOME."""
    ...
    def _update_paths(cls) -> "GatewayNotificationsMixin._UpdatePaths":
        ...
        return cls._UpdatePaths(
            pending=_zeus_home / ".update_pending.json",
            claimed=_zeus_home / ".update_pending.claimed.json", output=_zeus_home / ".update_output.txt",
            exit_code=_zeus_home / ".update_exit_code",
            prompt=_zeus_home / ".update_prompt.json", response=_zeus_home / ".update_response",
        )
    ...
    async def _watch_update_progress(
        ...
        """Watch ``zeus update --gateway``, streaming output + forwarding prompts.
        ...
        while loop.time() < deadline:
            if paths.exit_code.exists():
                ...
                with _log_suppressed(logging.WARNING, "Update final notification failed: %s"):
                    ...
                    await target.send(
                        "✅ ZeusAgent update finished." if exit_code == 0
                        else "❌ ZeusAgent update failed (exit code {}).".format(exit_code)
                    )
        ...
        if not paths.exit_code.exists():
            ...
            await _flush_buffer()
            with suppress(Exception):
                await target.send("❌ ZeusAgent update timed out after 30 minutes.")
            self._clear_update_markers(paths, session_key)
        ...
        try:
            ...
            if chat_id:
                ...
                if output:
                    if len(output) > 3500:
                        output = "…" + output[-3500:]
                    status = "✅ ZeusAgent update finished." if exit_code == 0 else "❌ ZeusAgent update failed."
                    msg = f"{status}\n\n```\n{output}\n```"
                else:
                    msg = (
                        "✅ ZeusAgent update finished successfully." if exit_code == 0 else
                        "❌ ZeusAgent update failed. Check the gateway logs or run `zeus update` manually for details."
                    )
    ...
    async def _send_restart_notification(self) -> Optional[tuple[str, str, Optional[str]]]:
        ...
        from gateway.run import _zeus_home, _non_conversational_metadata
        notify_path = _zeus_home / ".restart_notify.json"
    ...
    async def _send_home_channel_startup_notifications(
        ...
        message = "♻️ Gateway online — ZeusAgent is back and ready."
    ...
    async def _send_session_db_warning_notifications(self) -> None:
        ...
        from zeus_state import _default_db_path, classify_persistence_error, format_session_db_unavailable
        if classify_persistence_error(error) == "corrupt":
            # Copy-pasteable, so name the real store (profiles / ZEUS_HOME do not live under ~/.zeus).
            db_path = _default_db_path()
            backups_dir = get_default_zeus_root() / "backups"
            message = (
                "⚠️ Session database corruption detected. Messages may not be "
                "persisted. Recovery options:\n"
                "1. Run `zeus doctor --fix`\n"
                "2. Stop the gateway, then recover with:\n"
                f"   zeus sessions recover --source {db_path} "
                "--inspect-only\n"
                "   (if it reports recoverable) zeus sessions recover "
    ...
    async def _inject_watch_notification(
        ...
        if not source:
            # API-server sessions bind the RAW X-Zeus-Session-Id key, not a structured ``agent:...`` key.
