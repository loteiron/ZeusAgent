...
class GatewayGoalCommandsMixin:
    ...
    async def _handle_heartbeat_command(self, event: MessageEvent) -> str:
        ...
        from zeus_cli.heartbeat import parse_interval, format_interval, MIN_INTERVAL_SECONDS
        ...
        return (
            ...
            "elapsed. Lives while the gateway runs — use `zeus cron` for durable schedules."
        )
    ...
    async def _handle_loop_command(self, event: MessageEvent) -> str:
        ...
        try:
            from zeus_cli.loops import LoopManager, dispatch_loop_command, goal_blocks_loop_tick
        except Exception as exc:
