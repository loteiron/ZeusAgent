...
def _normalize_multiplex_profile_allowlist(value: Any) -> Optional[List[str]]:
    ...
    from zeus_cli.profiles import normalize_profile_name, validate_profile_name
...
# of truth for gateway/run.py and zeus_cli/web_server.py validation).
...
class PlatformConfig:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "PlatformConfig":
        ...
        # write them where the docs and ``zeus config set platforms.webhook.port`` put them:
...
class GatewayConfig:
    ...
    sessions_dir: Path = field(default_factory=lambda: get_zeus_home() / "sessions")
    ...
    # route); ZEUS_ROOM_LINK_URL overrides.
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        return cls(
            ...
            sessions_dir=Path(data["sessions_dir"]) if "sessions_dir" in data else get_zeus_home() / "sessions",
...
def load_gateway_config() -> GatewayConfig:
    """Load gateway configuration. Priority: env > ~/.zeus/config.yaml > legacy gateway.json > defaults."""
