...
Defenses 1-2 (``_ZEUS_GATEWAY`` guard on ``zeus gateway stop|restart`` /
...
persisted to ``<ZEUS_HOME>/gateway/restart_loop.json`` and CHAIN while gaps stay
...
def _state_path():
    return get_zeus_home() / "gateway" / "restart_loop.json"
