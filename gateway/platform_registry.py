...
class PlatformEntry:
    ...
    required_env: list = field(default_factory=list)  # ``zeus setup`` display
    ...
    plugin_name: str = ""  # owning manifest so ``zeus gateway setup`` can auto-enable it
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved ZEUS_HOME and overlay the
        ...
        # `zeus` invocation, so the import happens only when a lookup asks for it.
