...
``validate_media_delivery_path`` reads ``ZEUS_MEDIA_DELIVERY_STRICT`` (gateway.strict),
``ZEUS_MEDIA_ALLOW_DIRS`` (gateway.media_delivery_allow_dirs) and
``ZEUS_MEDIA_TRUST_RECENT_FILES`` (gateway.trust_recent_files).  Every delivery
entrypoint (gateway startup, ``zeus cron run``, ``zeus send``) calls
...
_FLAG_ENVS = (("strict", "ZEUS_MEDIA_DELIVERY_STRICT"), ("trust_recent_files", "ZEUS_MEDIA_TRUST_RECENT_FILES"))
_ALLOW_DIRS_ENV = "ZEUS_MEDIA_ALLOW_DIRS"
