...
def flood_wait_seconds(error: Any, default: float = FLOOD_RETRY_DEFAULT_SECONDS) -> float:
    ...
    if text.startswith(FLOOD_ERROR_PREFIX):
        ...
        if raw is not None:
            wait = raw
    # float() accepts NaN/infinity and overflowing exponents. Those are not
    # platform deadlines: infinity would strand a durable reply until expiry.
    return wait if math.isfinite(wait) and wait > 0 else default
