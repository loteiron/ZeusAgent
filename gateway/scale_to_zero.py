...
SCALE_TO_ZERO_ENV = "ZEUS_SCALE_TO_ZERO"  # stamped by NAS when the Labs toggle is on
...
def dashboard_client_heartbeat_path(zeus_home: Optional[os.PathLike | str] = None):
    """Path of the dashboard-client liveness marker under ZEUS_HOME."""
    if zeus_home is None:
        from zeus_constants import get_zeus_home
        zeus_home = get_zeus_home()
    return Path(zeus_home) / DASHBOARD_CLIENT_HEARTBEAT_REL
