...
``gateway.<key>`` form (what ``zeus config set gateway.<key>`` produces) is consulted only when the
...
#
# Top-level settings are also accepted nested under ``gateway:`` (what ``zeus config set
...
def load_yaml_layer(home: Path, gw_data: dict) -> None:
    ...
    # zeus_cli.config.load_config, so managed quick_commands / stt would otherwise be ignored).
