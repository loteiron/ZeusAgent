...
from zeus_state_common import FTS_CJK_STALE_KEY, FTS_STALE_KEY, _FTS_CJK_TRIGGERS, _FTS_TRIGGERS
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("ZEUS_FTS5_CJK_SO")
    return Path(env).expanduser() if env else get_zeus_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("ZEUS_CJK_FTS", "1").strip().lower() not in ("0", "false", "off", "no")
...
class SessionFtsSetupMixin:
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `zeus update` to rebuild the venv with a "
                ...
                if cjk_present:
                    ...
                    if live:
                        ...
                        logger.warning(
                            ...
                            "run `zeus sessions optimize-storage` on a host "
