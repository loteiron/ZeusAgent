 ...
 *
 * The root ``package.json`` is installed by ``zeus update`` on every user,
 ...
 *   workspace graph entirely. ``zeus update`` and ``zeus doctor --fix``
 ...
 *   ``zeus tools`` → Browser Automation → Camofox, and only used at
 *   runtime when ``CAMOFOX_URL`` is set. Its postinstall fetches a ~300MB
 *   Firefox-fork binary, which silently blocked ``zeus update`` for
 ...
 * fails — read the lazy-install guidance in the ``zeus-agent-dev`` skill
...
test('camofox is not in root dependencies (must stay opt-in)', () => {
  ...
  assert.ok(
    ...
    'Camofox is a ~300MB binary-postinstall backend that must stay ' +
      ...
      'Camofox post_setup handler in zeus_cli/tools_config.py so it ' +
      'only installs when the user explicitly selects Camofox via ' +
      '`zeus tools` → Browser Automation → Camofox.'
  )
})
