...
A cron job that restarts/stops the gateway from inside the gateway (``zeus gateway restart``,
``launchctl kickstart ai.zeus.gateway``, ``systemctl restart zeus-gateway``) kills the process,
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: destructive `zeus gateway` ops. `start` is excluded: starting from inside a
    # gateway is benign and a job may legitimately start a sibling profile. The lookbehind keeps
    # `zeus` from being a path component or word tail (`/docs/zeus gateway restart-notes.md`)
    # while every real command position (text start, whitespace, `;`/`&`/`|`, `$(`, backtick,
    # U+FFFD) still matches.
    # See #77173.
    r"(?:(?<![/\w.\-])zeus\s+gateway\s+(?:restart|stop|uninstall)\b)"
    # Branch B: launchctl ops anchored on a zeus-gateway label so unrelated zeus services stay
    ...
    # -l ai.zeus.gateway-<suffix> -- <helper-script>` (or `launchctl bootstrap gui/<uid> <plist>`) creates
    ...
    # Branch C: systemctl ops on a zeus-gateway unit.
...
# split across continuation lines (e.g. `launchctl submit \` / `  -l ai.zeus.gateway-... \` / `  -- ...`,
...
# Branch A2: `zeus -p <profile> gateway restart|stop` (also `--profile <name>` /
...
# `zeus -p venus gateway restart` operates on a sibling profile's gateway and is a legitimate fleet
...
# `$label` next to the verb. These verbs act on an EXISTING job, so the zeus-gateway label anchor
...
_ZEUS_GATEWAY_LABEL_RE = re.compile(r"(?i)\bzeus[.\-]?gateway\b")
...
def _current_profile_name() -> Optional[str]:
    """Profile running the guard: ``ZEUS_PROFILE_NAME``/``ZEUS_PROFILE`` env first, then
    ``zeus_cli.profiles.get_active_profile_name`` (from ``ZEUS_HOME``); ``None`` if neither."""
    for env_name in ("ZEUS_PROFILE_NAME", "ZEUS_PROFILE"):
...
def _contains_launchctl_gateway_lifecycle(normalized_text: str) -> bool:
    ...
    return bool(_LAUNCHCTL_LIFECYCLE_VERBS_RE.search(normalized_text)) and bool(
        _ZEUS_GATEWAY_LABEL_RE.search(normalized_text)
    )


def _contains_direct_or_profile_lifecycle(text: str) -> bool:
    """Distinguish a profile named ``zeus`` from a second executable.

    Only a recognized selector's value is exempt from the adjacent-command
    pattern. Self-targeting selectors, other commands and substitutions are
    still checked independently, including after shell quote normalization.
    """
    profile_value_starts = set()
    for match in _PROFILE_FLAG_LIFECYCLE_PATTERN.finditer(text):
        group = 1 if match.group(1) is not None else 2
        named = match.group(group)
        if _named_profile_is_current(named.strip().strip("\"'")):
            return True
        if named.casefold() == "zeus":
            profile_value_starts.add(match.start(group))
    return any(
        match.start() not in profile_value_starts
...
def contains_gateway_lifecycle_command(text: str) -> bool:
    ...
    ``zeus``/``gateway`` token) — this function is the single choke point
    ...
    # <<'EOF'`) are masked before scanning (#88336): a runbook line "a human can run: zeus gateway
    ...
    if _contains_direct_or_profile_lifecycle(normalized):
        return True
    ...
    for segment in _iter_command_segments(normalized):
        joined = " ".join(segment)
        if joined and _contains_direct_or_profile_lifecycle(joined):
            return True
        stripped = _ARGV_LIST_PUNCTUATION.sub(" ", joined)
        if stripped != joined and _contains_direct_or_profile_lifecycle(stripped):
            return True
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    ``<ZEUS_HOME>/scripts/``) so the guard scans the file that will actually run."""
    ...
    if raw.is_absolute():
        return raw
    try:
        return get_zeus_home() / "scripts" / raw
    except (RuntimeError, OSError):
        # get_zeus_home() falls back to Path.home(), which raises when neither ZEUS_HOME nor
...
def check_gateway_lifecycle(prompt: Optional[str], script: Optional[str] = None) -> None:
    ...
    if script:
        ...
        if resolved_script is not None and _on_cloud_path(resolved_script):
            raise GatewayLifecycleBlocked(
                ...
                "(e.g. ~/.zeus/scripts/) and recreate the job."
            )
    ...
    if python_script:
        ...
        # The direct command regex below still scans the full text, so a literal `zeus gateway restart`
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `zeus gateway restart` from a shell outside "
