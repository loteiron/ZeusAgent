...
schedule via the `cronjob` tool; users via `zeus cron list|add|edit|pause|resume|run|remove` or
...
- File lock `~/.zeus/cron/.tick.lock` prevents duplicate ticks across processes.
...
- The cron ticker runs in the desktop-spawned backend when `ZEUS_DESKTOP=1` — that env var means
...
Durable SQLite-backed board letting multiple profiles/workers collaborate. Users: `zeus kanban
...
- **CLI:** `zeus_cli/kanban.py` facade + 14 `kanban_*.py` siblings (`boards`, `db`, `db_connect`,
...
- **Toolset:** `tools/kanban_tools.py` — `kanban_show, kanban_complete, kanban_request_review,
  ...
  `kanban` (`zeus tools enable kanban --platform <p>`; default-off, in `CONFIGURABLE_TOOLSETS`) get
...
- **Dispatcher:** long-lived loop (default 60s) that reclaims stale claims, promotes ready tasks,
  ...
  (`kanban.dispatch_in_gateway: true`). Standalone: `plugins/kanban/systemd/zeus-kanban-dispatcher.service`.
...
Isolation: **board** is the hard boundary — workers get `ZEUS_KANBAN_BOARD` pinned in their env and
...
`tests/cron/`, `tests/zeus_cli/test_kanban*.py`, `tests/tools/test_kanban*.py`. Schedule parsing
