...
def _open_continuable_cron_thread(job: dict, adapter, chat_id: str, loop) -> Optional[str]:
    ...
    if not callable(create_thread) or loop is None:
        return None
    thread_name = f"ZeusAgent — {job.get('name') or job.get('id', 'cron')}"
    try:
...
def cron_delivery_targets() -> list[dict]:
    ...
    try:
        from zeus_cli.profiles import list_profile_names
...
def _deliver_to_bot_chat(job: dict, content: str, profile: str) -> Optional[str]:
    ...
    ``zeus [-p <profile>] chat --in ~ -c "Bot Chat" --create-if-missing -Q --query-file`` — the
    ...
    if zeus_bin:
        ...
        try:
            ...
            found = _ilu.find_spec("zeus_cli") is not None
        except Exception:
            found = False
        if not found:
            return "bot-chat delivery failed: zeus CLI not resolvable"
        argv = [sys.executable, "-m", "zeus_cli.main"]
    ...
    if profile:
        argv += ["-p", profile]
        # -p owns profile resolution; this scheduler's ZEUS_HOME must not shadow it.
        env.pop("ZEUS_HOME", None)
    ...
    try:
        with tempfile.NamedTemporaryFile(
            "w", encoding="utf-8", suffix=".txt", prefix="zeus-cron-botchat-", delete=False,
        ) as fh:
            fh.write(message)
            query_file = fh.name
...
def _deliver_result(
    ...
    # job's own attempt: a worker's script may dispatch another job in-process (`zeus cron run`),
    # and that nested delivery must not be keyed under the outer execution id.
    external_execution = os.environ.get("_ZEUS_CRON_EXTERNAL_WORKER", "")
    ...
    # persisted as ``last_delivery_unverified`` so `zeus cron list` shows it.
    ...
    # at boot; standalone runs (`zeus cron run`) did not, silently dropping files. Idempotent.
