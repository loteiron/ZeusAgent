...
def _relay_root() -> Path:
    ...
    home = Path(os.getenv("ZEUS_HOME") or os.path.expanduser("~/.zeus"))
    return home.parent.parent if home.parent.name == "profiles" else home


# Per-attempt turn timeout and attempt ceiling for bot_relay.deliver. The Desktop client mirrors
# both (apps/desktop/src/plugins/zeus-bots/relay.ts: RELAY_TURN_ATTEMPT_MS / RELAY_TURN_MAX_ATTEMPTS)
...
def _(rid, params: dict, _root=_relay_root, _run=_run_delivery) -> dict:
    ...
    GATEWAY via the one-turn ``zeus -p <profile> chat -c "Bot Chat"`` transport local DMs use →
    ...
    try:
        ...
        resolved = "default" if profile.lower() == "zeus" else profile
        ...
        fd, tmp = tempfile.mkstemp(prefix="zeus-relay-dm-", suffix=".txt", text=True)
        try:
