...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from zeus_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    """Built-ins + user overrides, via zeus_cli.personality (single owner)."""
...
def _validate_personality(value: str, cfg: dict | None = None) -> tuple[str, str]:
    ...
    from zeus_cli.personality import normalize_personality_name, render_personality_prompt
...
def _prompt_text(value) -> str:
    """Normalize config prompt values from YAML for AIAgent (zeus_cli.personality owns this)."""
    from zeus_cli.personality import prompt_text
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("ZEUS_TUI_SKILLS", "")
...
def _load_fallback_model():
    ...
    ZeusAgentCLI/gateway: ``fallback_providers`` first, legacy ``fallback_model`` merged after)."""
